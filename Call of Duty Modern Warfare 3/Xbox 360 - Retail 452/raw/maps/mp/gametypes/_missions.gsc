...
updateChallenges()
{
	...
	foreach ( challengeRef, levelChallengeData in level.challengeInfo )
	{
		...
		if( isWeaponChallenge( challengeRef ) )
		{
			...
			if( !self IsItemUnlocked( baseWeapon ) )		// make sure the challenge is unlocked
				continue;
		...
		else if( isKillstreakChallenge( challengeRef ) ) 	// make sure the killstreak is unlocked
		{
			if ( !self IsItemUnlocked( challengeRef ) )		// make sure the challenge is unlocked
				continue;
		...
		{
			isUnlocked = self IsItemUnlocked( challengeRef );

			if ( isUnlocked == 0 )							// make sure the challenge is unlocked
				continue;
			else if( isUnlocked == 2 )						// if the challenge wasn't found
			{
