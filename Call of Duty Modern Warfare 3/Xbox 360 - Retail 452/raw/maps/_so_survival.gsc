...
CONST_DEATH_FAIL_WAVE				= 0;	//if dead in this wave, player fails
CONST_CONGRAT_MIN_WAVE				= 5;	//past this wave #, we congrat player when dead with dialog

CONST_START_REV_TIMER				= 120;	//seconds for revivinG coop partner
CONST_MIN_REV_TIMER					= 30;	//min time for reviving coop partner
CONST_REV_TIMER_DECREASE			= 8;	//seconds decreased for reviving coop partner

CONST_WAVE_START_TIMEOUT			= 5;	//timeout that will trigger first wave if player didn't
CONST_WAVE_DELAY_TOTAL				= 35;	//total delay between waves. other delays are subtracted from this
CONST_WAVE_DELAY_BEFORE_READY_UP	= 5;	//delay before players are prompted to ready up
CONST_WAVE_DELAY_COUNTDOWN			= 5;	//time remaining when when the large countdown shows up in the center of the screen
CONST_WAVE_ENDED_TIMER_FADE_DELAY	= 1.75;	//time after a wave ending before the wave timer fades
CONST_WAVE_AI_LEFT_TILL_NEXT_WAVE	= 0;	//if only this many AIs left, next wave starts
CONST_WAVE_AI_LEFT_TILL_AGGRO		= 4;	//if only this many AIs left, AIs become aggressive

CONST_WAVE_REENFORCEMENT_SQUAD		= 1;	//number of squads to re-enforce
CONST_WAVE_REENFORCEMENT_SPECIAL_AI = 2;	//number of special AIs to re-enforce

CONST_CAMP_RESPONSE_INTERVAL		= 8;	//enemies respond to player camping every X seconds

CONST_ARMOR_INITIAL_DISPLAY_TIME	= 14;	//seconds to display armor HP when level starts
CONST_ARMOR_DISPLAY_TIME			= 6;	//seconds to display armor HP when damaged

CONST_DELAYED_ENEMY_PING			= 7;	//seconds delay before enemies show up in minimap, protect spawn locations

CONST_EQUIPMENT_C4_MAX				= 20;
CONST_EQUIPMENT_CLAYMORE_MAX		= 20;
		
// Player load out from table
LOADOUT_TABLE						= "sp/survival_waves.csv";	// loadout is in waves table for now
TABLE_INDEX							= 0;	// Indexing
TABLE_SLOT							= 1;	// Load out slot, such as primary weapon, grenades etc...
TABLE_REF							= 2;	// Reference string of the item
TABLE_AMMO							= 3;	// Ammo for weapon or equipments

// Number of waves per star
CONST_1_STAR_REQ					= 5;	// survival 5 waves to earn 1 star
CONST_2_STAR_REQ					= 10;	// survival 10 waves to earn 2 stars
CONST_3_STAR_REQ					= 20;	// survival 20 waves to earn 3 stars
...
{
	// time formatting
	time_survived = int( min( ( level.challenge_end_time - level.challenge_start_time ), 86400000 ) ); //level.player getplayerdata( "round", "timePlayed" );
	
	ms 		= int( ( time_survived % 1000 )/100 ); // divided by 100 to keep this 1 digit, runded down
	...
	foreach ( player in level.players )
	{
		// give completion XP bonus
		//player thread giveXP( "survival_completion_bonus", player_score );
...
// having to reload
CONST_WEAPON_CHANGE_AMMO_ADJUST_TIME = 10;	// 10 second minimum between ammo adjusts on each collected weapon 
...
update_wave()
{
	...
	while ( 1 )
	{
		...
		// repeat wave patterns, generate as we go the needed wave data array
...
{			
	...
	/#
	if ( int( getdvar( "timescale" ) ) > 1 )
	{
		duration_before_count	= 0;
		duration_count			= 1;
	}
	#/
...
survival_credits()
{
	...
	foreach ( player in level.players )
	{
		...
		// debug
		/#
			debug_credits = getdvarint( "debug_credits" );
			if ( isdefined( debug_credits ) && debug_credits > 0 )
				player.survival_credit = debug_credits;
		#/
...
update_from_xp()
{
	...
	while( 1 )
	{
		...
		// update credits for hud - credits roll to new value
...
update_from_credits()
{
	...
	{
		self.old_xp = self.summary[ "rankxp" ];
		self.old_credits = self.survival_credit;	// must be shared with update_from_xp()
		
		self waittill( "credit_updated", donation ); // "credit_updated" from spending
...
intro_music( type )
{
	...
	//music_alias = "so_survival_easy_music";
...
perk_HUD()
{
	...
	self.perk_icon_HUD.icon_size	= 28;			// Width & height
