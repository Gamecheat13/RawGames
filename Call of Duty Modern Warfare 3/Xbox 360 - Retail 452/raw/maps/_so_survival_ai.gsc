...
// Tweakables: Tables
WAVE_TABLE								= "sp/survival_waves.csv";	// AI and wave data tablelookup

TABLE_INDEX								= 0;	// Indexing
TABLE_BOSS_DELAY						= 1;	// "1" = boss spawn on aggress, "0" = boss spawns immediately
TABLE_WAVE								= 2;	// Wave number
TABLE_SQUAD_TYPE						= 3;	// Squad AI type ref
TABLE_SQUAD_SIZE						= 4;	// Number of squad AIs total
TABLE_SPECIAL							= 5;	// Special AI type
TABLE_SPECIAL_NUM						= 6;	// Number of special AIs
TABLE_BOSS_AI							= 7;	// Boss AIs, separated by spaces
TABLE_BOSS_NONAI						= 8; 	// Boss Chopper/etc...
TABLE_REPEAT							= 9;	// Wave repeating pattern
TABLE_ARMORY_UNLOCK						= 10;	// Armory to be unlocked at the end of this wave

TABLE_AI_REF							= 1;	// AI type ref
TABLE_AI_NAME							= 2;	// String name of AI
TABLE_AI_DESC							= 3;	// String description of the AI
TABLE_AI_CLASSNAME						= 4;	// Classname of the character without weapon
TABLE_AI_WEAPON							= 5; 	// Weapon type goes with Classname
TABLE_AI_ALT_WEAPON						= 6;	// Alt. Weapon such as grenades or RPG
TABLE_AI_HEALTH							= 7;	// Health amount int
TABLE_AI_SPEED							= 8;	// AI movement speed
TABLE_AI_XP								= 9;	// XP/Credit amount of AI
TABLE_AI_BOSS							= 10;	// Boss flag
TABLE_AI_ACCURACY						= 11;	// AI Base accuracy

TABLE_WAVE_DEF_INDEX_START				= 0;	// First index for AI Waves
TABLE_WAVE_DEF_INDEX_END				= 40;	// Last index for AI Waves

TABLE_AI_TYPE_INDEX_START				= 100;	// First index for AI Type Table
TABLE_AI_TYPE_INDEX_END					= 120;	// Last index for AI Type Table

// Tweakables: Generic
CONST_WEAPON_DROP_RATE					= 1;	// 0.0-1.0 rate of weapon drop when dead
...
CONST_AI_UPDATE_DELAY					= 4.0;	// General AI update delay passed to manage_ai_relative_to_player()
CONST_AI_SEARCH_PLAYER_TIME				= 6.0;	// Amount of time an ai will pole to see if it has reached the player before giving up
CONST_LONG_DEATH_REMOVE_DIST_MIN		= 540;	// Min distance the closest player needs to be to allow long death force kill

// Enemy spawn protection
CONST_ENEMY_SPAWN_PROTECTION_TIME		= 1;	// In seconds

// Tweakables: Repeating Wave Difficulty Adjust
CONST_AI_REPEAT_BOOST_HEALTH			= 0.10;	// 0.0-1.0 Percent health increase stacked for each repeated wave
CONST_AI_REPEAT_BOOST_SPEED				= 0.05;	// 0.0-1.0 Percent speed increase stacked for each repeated wave
CONST_AI_SPEED_MAX						= 1.5;  // No AI should ever have their speed scale greater than 1.5
CONST_AI_REPEAT_BOOST_ACCURACY			= 0.2;	// 0.0-1.0 Percent accuracy increase stacked for each repeated wave

// Tweakables: Player Closest Node Tracker
CONST_NODE_CLOSEST_RADIUS_MIN			= 1;	// Check for nodes closest to player greater than this distance away
CONST_NODE_CLOSEST_RADIUS_MAX			= 128;	// Check for nodes closest to player less than this distance away
CONST_NODE_CLOSEST_RADIUS_INCREASE		= 64;	// If a closest node check fails to find a node, increase the radius by this amount and try again
CONST_NODE_CLOSEST_RADIUS_INVALID		= 2048;	// The radius max for the check for nodes should never get this big.
CONST_NODE_CLOSEST_HEIGHT				= 512;	// Check for nodes closest to player within this height distance away
CONST_NODE_CLOSEST_HEIGHT_INCREASE		= 128;	// If a closest node check fails to find a node, increase the height by this amount and try again
...
CONST_GENERIC_AI_ENGAGE_MIN				= 88;	// Engagement min dist used by regular and special AI when aggressing (not dogs or bosses)
CONST_GENERIC_AI_ENGAGE_MIN_FALL_OFF	= 64;	// Engagement min dist fall off used by regular and special AI when aggressing (not dogs or bosses)

// Tweakables: Ai Type Martyrdom
CONST_MARTYRDOM_C4_SOUND_TELL_LENGTH	= 1.5;	// length of the sound used to telegraph the c4 explosion
CONST_MARTYRDOM_C4_TIMER				= 3;	// seconds till c4 explode after dropped by martyrdom AI
CONST_MARTYRDOM_C4_TIMER_SUBSEQUENT		= 0.4;	// time delay before secondary explosions go off
CONST_MARTYRDOM_C4_PHYS_RADIUS			= 256;	// radius of physics push cylinder volume
CONST_MARTYRDOM_C4_PHYS_FORCE			= 2;	// force of the physics push
CONST_MARTYRDOM_C4_QUAKE_SCALE			= 0.4;	// earthquake scale
CONST_MARTYRDOM_C4_QUAKE_TIME			= 0.8;	// earthquake duration
CONST_MARTYRDOM_C4_QUAKE_RADIUS			= 600;	// earthquake range

CONST_MARTYRDOM_C4_DMG_RADIUS			= 192;	// c4 damage range
CONST_MARTYRDOM_C4_DMG_MAX				= 100;	// c4 max damage
CONST_MARTYRDOM_C4_DMG_MIN				= 50;	// c4 min damage
CONST_MARTYRDOM_C4_DANGER_RANGE			= CONST_MARTYRDOM_C4_DMG_RADIUS - 48;	// c4 bad place cylinder height and radius

// Tweakables: Ai Type Claymore
CONST_CLAYMORE_PLACED_MAX				= 6;	// Maximum count of AI claymores that can be placed

CONST_MINE_LOC_UPDATE_DELAY				= 0.5;	// Location weight update delay
CONST_MINE_LOC_WEIGHT_MAX				= 20;	// Location max weight
CONST_MINE_LOC_WEIGHT_INC				= 0.5;	// Weight increase per update: Max reached in 20 updates or 10 seconds
CONST_MINE_LOC_WEIGHT_DECAY				= 0.025;// Weight decrease per update: Min reached in 800 updates or 400 seconds
CONST_MINE_LOC_RANGE_PLAYER				= 512;	// Radius from the player that causes a location to have its weight increased

CONST_MINE_PLANT_CHECK_DELAY			= 2.0;	// Time between claymore planting system checks
CONST_MINE_PLANT_TIME_BETWEEN			= 8.0;	// Minimum time between consecutive plant attempts
CONST_MINE_PLANT_DIST_PLAYER_MIN		= 384;	// Minimum distance that a claymore can be planted from player
CONST_MINE_PLANT_DIST_AI_MAX			= 768;	// Maximum distance that an AI can travel to plant
CONST_MINE_PLANT_HEIGHT_AI_MAX			= 240;	// AI won't travel half this height or above half this height to plant
CONST_MINE_PLANT_WEIGHT_MIN				= 2.0;	// Min weight for a plant to occur

CONST_CLAYMORE_ENT_TIMER				= 0.75;	// seconds till claymore explode after triggered
CONST_CLAYMORE_ENT_TRIG_ANGLE			= 70;	// Angle player has to be in to get detected
CONST_CLAYMORE_ENT_TRIG_DIST_MIN		= 20;	// Minimum claymore trig dist using player vector projected onto claymore facing
CONST_CLAYMORE_ENT_PHYS_RADIUS			= 256;	// radius of physics push cylinder volume
CONST_CLAYMORE_ENT_PHYS_FORCE			= 2;	// force of the physics push
CONST_CLAYMORE_ENT_QUAKE_SCALE			= 0.4;	// earthquake scale
CONST_CLAYMORE_ENT_QUAKE_TIME			= 0.8;	// earthquake duration
CONST_CLAYMORE_ENT_QUAKE_RADIUS			= 600;	// earthquake range
CONST_CLAYMORE_ENT_TRIG_RADIUS			= 192;  // the claymore detonation distance - mplayer is 192
CONST_CLAYMORE_ENT_DMG_RADIUS			= 192;	// claymore damage range
CONST_CLAYMORE_ENT_DMG_MAX				= 100;	// claymore max damage
CONST_CLAYMORE_ENT_DMG_MIN				= 50;	// claymore min damage

// Tweakables: Ai Type Chemical
CONST_CHEMBOMB_ENT_TIMER				= 0.5;	// seconds till chembomb explode after triggered
CONST_CHEMBOMB_ENT_TRIG_RADIUS			= 96;	// the chembomb detonation distance
CONST_CHEMBOMB_CLOUD_LIFE_TIME			= 6.0;	// how long the chemical bomb cloud remains
CONST_CHEMBOMB_CLOUD_BADPLACE_LIFE_TIME	= 1.0;	// length of time in the bad place stays around

CONST_CHEMICAL_TANK_PHYS_RADIUS			= 256;	// radius of physics push cylinder volume
CONST_CHEMICAL_TANK_PHYS_FORCE			= 0.5;	// force of the physics push
CONST_CHEMICAL_TANK_QUAKE_SCALE			= 0.2;	// earthquake scale
CONST_CHEMICAL_TANK_QUAKE_TIME			= 0.4;	// earthquake duration
CONST_CHEMICAL_TANK_QUAKE_RADIUS		= 600;	// earthquake range

CONST_CHEMICAL_TANK_DMG_RADIUS			= 192;	// tank explosion damage range
CONST_CHEMICAL_TANK_DMG_MAX				= 20;	// tank explosion max damage
CONST_CHEMICAL_TANK_DMG_MIN				= 10;	// tank explosion min damage

CONST_CHEMICAL_CLOUD_TRIG_RADIUS		= 96;	// chemical smoke cloud shock radius
CONST_CHEMICAL_CLOUD_LIFE_TIME			= 6.0;	// length of time in seconds the cloud stays around
CONST_CHEMICAL_CLOUD_BADPLACE_LIFE_TIME	= 2.0;	// length of time in seconds the badplace remains
CONST_CHEMICAL_CLOUD_SHOCK_TIME			= 1.5;	// time that the shock lasts for
CONST_CHEMICAL_CLOUD_SHOCK_DELAY		= 1.0;	// time in between shock applications from chemical cloud

// Tweakables: Ai Type Boss Juggernaut
CONST_JUG_POP_HELMET_HEALTH_PERCENT		= 0.33;	// Health percent threshold that causes the helmet to pop off
CONST_JUG_DROP_SHIELD_HEALTH_PERCENT	= 0.50;	// Health percent threshold that causes the Juggernaut to drop his shield
CONST_JUG_WEAKENED						= 250;	// at this health amount, juggernaut is weakened, ex: pain reaction
CONST_JUG_MIN_DAMAGE_PAIN				= 350;	// min amount of damage needed to have pain animations play
CONST_JUG_MIN_DAMAGE_PAIN_WEAK			= 250;	// min amount of damage needed to have pain animations play when weak
CONST_JUG_RUN_DIST						= 1000;	// runs towards player at this distance
CONST_JUG_WEAKENED_RUN_DIST				= 500;	// runs towards player at this distance when weakened and desperate
CONST_JUG_RIOTSHIELD_BULLET_BLOCK		= 1;	// riotshield jug: no reaction when bullet hits shield

// Juggernaut damage shield levels, (0-1) 0= takes no dmg, 1= taks full dmg, >1= increases damage
CONST_JUG_HIGH_SHIELD					= 0.25;	// Bullet Proof Armor
CONST_JUG_MED_SHIELD					= 0.33;	// Bullet Resistant Armor
CONST_JUG_LOW_SHIELD					= 0.75;	// Not bare skin or cloth
CONST_JUG_NO_SHIELD						= 1.0;	// Exposed area not that is not the head or splash damage with head exposed
CONST_JUG_KILL_ON_PAIN					= 9999;	// Exposed head

CONST_CHOPPER_SPEED						= 60;	// Chopper default speed
CONST_CHOPPER_ACCEL						= 20; 	// Chopper default acceleration

// Tweakables: Ai Type Ally
CONST_ALLY_BULLET_SHIELD_TIME			= 20;

// Tweakables: Ai Type Dog
CONST_DOG_SPAWN_OVER_TIME				= 50;	// Dogs will spawn over this many seconds, usually estimate of wave completion time

// Tweakables: Ai Type Dogsplode
CONST_DOGSPLODE_C4_TIMER_NECK_SNAP		= 5;	// Dog C4 timer used if the dog is killed during a neck snap. Gives the player time to escape.
CONST_DOG_TIME_STATIC_TO_DETONATE		= 2000;	// Time in milliseconds the dog needs to be stationary before auto detonating
CONST_DOG_DIST_TO_SENTRY_DETONATE		= 40;	// Distance in inches that the dog must be from a sentry to cause auto detonation
CONST_DOG_SAME_LOC_THRESHOLD			= 10;	// If the dog is within this many inches of it's original postion he hasn't moved
