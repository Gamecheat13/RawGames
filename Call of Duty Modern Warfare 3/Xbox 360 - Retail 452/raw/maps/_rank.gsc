...
hud_width_format()
{
	// BELOW CAUSES DESYNC due to hiDef and wideScreen dvars are not per player and are shared values
	// host thinkgs client uses the same values, thus when script runs differently with them as conditions
	// it becomes nondeterministic
	/*
	// screen formatting
...
getRankXP()
{
	return self getplayerdata( "experience" ); //self.summary[ "rankxp" ];
}
