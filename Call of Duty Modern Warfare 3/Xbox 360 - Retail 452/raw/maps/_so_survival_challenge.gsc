...
CONST_CHALLENGE_SET_NUM			= 2; 	// number of challenges per set

// Tweakables: Tables
CH_TABLE						= "sp/survival_challenge.csv";	// challenge data tablelookup

CH_INDEX						= 0;	// Indexing
CH_REF                          = 1;	// REference String        
CH_NAME                         = 2;	// Name Loc String         
CH_DESC                         = 3;	// Desc Loc String  
CH_SPLASH						= 4;	// Splash message Loc String
CH_ICON							= 5;	// Icon
CH_REQ                          = 6;	// Requirement Int/Str 
CH_XP                           = 7;	// XP reward per completion   
CH_REPEAT                       = 8;	// Repeatable within a wave    
CH_CARRY						= 9;	// Does not reset between waves
CH_WAVE_ACTIVE					= 10;	// Only available starting this wave
CH_WAVE_INACTIVE				= 11;	// Only available upto this wave

// Kill staggering bonus XP
CONST_STAGGER_DECAY				= 1;	//points per second
CONST_STAGGER_KILLPOINTS		= 6;	//points per kill
...
sur_challenge_think()
{
	// reset playerdata values for hud
	
	wait 0.05; // due to setplayerdata cant happen in first frame
	for( i=0;i<5;i++ )
	{
	...
	while ( 1 )
	{
		...
		self surHUD_animate( "challenge" ); // auto enables
...
/*
monitor_challenge_set_completion()
{
	self endon( "death" );
	self endon( "challenge_reset" );
	
	while ( !is_challenge_set_complete( self.completed_ch ) )
	{
		flag_clear( "challenge_monitor_busy" );
		self waittill( "challenge_complete", sur_ch_ref );
		flag_set( "challenge_monitor_busy" );
		
		if ( !isdefined( self.selected_ch_array[ sur_ch_ref ] ) )
			continue;
		
		self.completed_ch[ sur_ch_ref ]++;
	}
	// challenge set is completed
	self notify( "challenge_set_complete" );
}

is_challenge_set_complete( completed_ch_array )
{
	assert( isdefined( completed_ch_array ) );
	
	foreach ( ch in completed_ch_array )
		if ( ch == 0 )
			return false;
...
challenge_func_director( sur_ch_ref )
{
	switch( sur_ch_ref )
	{
		...
		//case "sur_ch_execution":
		//	return ::sur_ch_execution;
		case "sur_ch_streak":
		...
		case "sur_ch_flash":
			return ::sur_ch_flash;
		//case "sur_ch_sidekick":
		//	return ::sur_ch_sidekick;
	}
	return undefined;
}
...
sur_ch_generic( sur_ch_ref )
{
	...
	//index 			= ch_get_index_by_ref( sur_ch_ref );
