...
TABLE_TYPE						= 2;	// item type
TABLE_SLOT						= 2;	// weapon upgarde slot

TABLE_COST						= 3;	// credit cost
TABLE_NAME						= 4;	// name string
TABLE_DESC						= 5;	// description string
TABLE_ICON						= 6;	// item icon
TABLE_UNLOCK					= 7;	// unlock at rank

TABLE_STOCK						= 8;	// number of items for purchase
TABLE_WEAPONUPGRADE				= 8;	// weapon upgrades

TABLE_WEAPONDROPAMMO			= 9; 	// amount of clip and stock ammo in dropped weapon, delimited by a space
TABLE_EXCLUDE					= 9; 	// item is not avilable for some maps
TABLE_STOCK_MAX_IN_PLAY			= 10;	// max stock player has that disables buying of this item
TABLE_SUBTYPE					= 11;	// item sub type, ex: "sniper" where type is "weapon"
...
ARMORY_UNLOCK_WEAPON			= 0;	// unlocks weapon armory at end of wave #
ARMORY_UNLOCK_EQUIPMENT			= 3;
ARMORY_UNLOCK_AIRSUPPORT		= 4;
ARMORY_RELOCATE					= 0;	// player to relocate armory at will - might be nasty
...
CREDIT_SHARE_INCREMENT			= 500;	// amount of credits to share per click
CREDIT_SHARE_BONUS				= 0.1;	// % bonus for credit share bonus

HINT_DISPLAY_ENABLE				= 1;	// enable hints
HINT_DISPLAY_TIME				= 5;	// displays hints for seconds
HINT_DISPLAY_FADE_TIME			= 1;	// seconds for fading in/out

CONST_AMMO_ARMOR_START			= 250;	// Points for regular armor
CONST_AMMO_ARMOR_JUG_START		= 500;	// Points for jug armor
...
armory_preload()
{
	...
	// precaches all weapons avaliable
	...
	// precaches all equipments that requires it
	//PrecacheItem( "c4" ); precached by _sp_killstreaks since it is a killstreak
...
ac130_spawn()
{
	...
	ac130model linkTo( level.ac130, "tag_origin", ( 0, CONST_ORBIT_RADIUS, CONST_ORBIT_HEIGHT ), ( 25, -90, 0 ) );
...
devGiveItem()
{
	...
	setDevDvarIfUninitialized( "scr_devGiveItem", "" );

	while ( 1 )
	{
		// Wait till dvar is set
		while( true )
		{
			if( GetDvar( "scr_devGiveItem" ) != "" )
				break;
			wait 1;
		}

		// If the armory item list isn't populated continue
		if ( !isdefined( level.armory_all_items ) )
		{
			continue;
		}

		item_ref = GetDvar( "scr_devGiveItem" );

		foreach ( player in level.players )
		{
			if ( player armory_can_give( item_ref ) )
			{
				player armory_give( item_ref );
			}
		}

		SetDevDvar( "scr_devGiveItem", "" );
	}
}
...
has_open_slot_right()
{
	// Check the allies. This will not be none if the
	// player has them waiting in action slot 4
	...
	// The above GetWeaponHudIconOverride() call does not work
	// for killstreaks :-/
...
// due to MP weapons having different sub class attachment variations
// ex: a "reflex" on SMG is a "reflexsmg"...
...
// returns the leaner version of the upgrade string "basename"
get_upgrades_on_weapon( weapon )
{
	...
	// If the current weapon is the base weapon there
	// are no upgrades
...
///////////////////////OLD VERSION: ABOVE VERSION ASSUMES MP WEAPONS TO BE FASTER

//	weapon_tok 			= StrTok( weapon, "_" );
//
//	for( i=0; i<weapon_tok.size; i++ )
//	{
//		// if tokens are iw5 and mp, skip
//		if ( weapon_tok[ i ] == "iw5" || weapon_tok[ i ] == "mp" )
//			continue;
//		
//		// if token is valid MP weapon, skip
//		if ( weaponclass( "iw5_" + weapon_tok[ i ] + "_mp" ) != "none" )
//			continue;
//			
//		// if token is valid SP weapon, skip
//		if ( weaponclass( "iw5_" + weapon_tok[ i ] ) != "none" )
//			continue;
//			
//		// if token is valid old SP weapon, skip
//		if ( weaponclass( weapon_tok[ i ] ) != "none" )
//			continue;
//			
//		current_upgrades[ current_upgrades.size ] = weapon_tok[ i ];
//	}
//	
//	return current_upgrades;
}
...
can_give_slotted_explosive( ref )
{
	if ( self HasWeapon( ref ) )
	{
		// claymore is special, it has one in clip and rest in stock,
		// when 0 in clip the GetFractionMaxAmmo() fails
		if ( ref == "claymore" )
		{
