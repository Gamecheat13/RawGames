...
// Tweakables: Loot
LOOT_TABLE						= "sp/survival_loot.csv";		// loot data tablelookup

TABLE_INDEX								= 0;	// indexing
TABLE_REF								= 1;	// reference loot asset name
TABLE_TYPE								= 2;	// loot type
TABLE_NAME								= 3;	// name
TABLE_DESC								= 4;	// description
TABLE_CHANCE							= 5;	// drop probability
TABLE_WAVE_UNLOCK						= 6;	// earliest wave this loot can drop at
TABLE_WAVE_LOCK							= 7;	// wave at which this loot stops dropping
TABLE_RANK								= 8;	// player rank required for this loot to drop
TABLE_VAR1								= 9;	// misc var for special needs

LOOT_INDEX_START						= 0;	// starting index of weapon loot items in string table
LOOT_INDEX_END							= 20;	// ending index of weapon loot items in string table

LOOT_VERSION_INDEX_START				= 100;	// starting index of weapon type permutations
LOOT_VERSION_INDEX_END					= 199;	// starting index of weapon type permutations

CONST_WEAPON_DROP_AMMO_CLIP				= 0.4;	// 0-1.0 rate of max ammo in clip on loot drops
CONST_WEAPON_DROP_AMMO_STOCK			= 0.5;	// 0-1.0 rate of max ammo in stock on loot drops
CONST_WEAPON_DROP_ALT_AMMO_CLIP			= 0.5;	// 0-1.0 rate of max ammo in clip of attachment
CONST_WEAPON_DROP_ALT_AMMO_STOCK		= 0.5;	// 0-1.0 rate of max ammo in stock of attachment


CONST_LOOT_DROP_MIN_WAVES_LAST_DROP		= 2;	// Waves since a loot type has dropped before it can be dropped again (type not version)
CONST_LOOT_LAST_WAVE_DROP_DEFAULT		= -999;	// When a loot is being dropped for the first time, this is the last wave it was dropped.
loot_preload()
{
	...
	foreach( type in level.loot_info_array )
		foreach( loot in type )
			foreach( version in level.loot_version_array )
				assertex( loot.ref != version, "The loot table and the loot version table had an overlapping value of: " + version );	
	#/
}
