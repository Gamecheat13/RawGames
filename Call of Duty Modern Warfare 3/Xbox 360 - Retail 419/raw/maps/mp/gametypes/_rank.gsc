...
giveRankXP( type, value, weapon, sMeansOfDeath, challengeName )
{
	...
	switch( type )
	{
		...
		case "jugg_on_jugg":
			...
			/#
			assertEx( (value < 100000), "Tried to award "+ self.name +"over 100000 XP: " + value );
			#/
	...
	if ( !level.hardcoreMode )
	{
		...
		else
		{
			...
			if ( type == "assist" )
			{
				...
				if( self _hasPerk( "specialty_assists" ) && !isDefined( self.remoteUAV ) )
				{
...
updateWeaponRank( oldxp, weapon )
{
	...
	//// since the weapon ranked up we need to check to see what unlocked so the menus know
	//self thread checkWeaponUnlocks( weapon );
