...
{	
	...
	if ( isDefined( attacker ) && isPlayer( attacker ) )
	{
		...
		if ( giveJugg )
		{			
			nextJugg = getBestTeammate( victim.team );
...
giveJuggLoadout()
{
	...
	if ( isAlive( self ) && self isJuggernaut() )
	{
...
nextJuggTimeout()
{
	...
	newJugg = getBestTeammate( self.team );
	//	if not defined, then the jugg is the only one on the team, just give it back to them
...
getBestTeammate( team )
{
	...
	foreach ( player in level.players )
	{
		if ( player != self && player.team == team && player.jugg_alligience > bestAlligience )
		{
			bestTeammate = player;
