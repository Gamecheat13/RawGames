...
ch_kills( data, time )
{
		...
		!isEnvironmentWeapon( data.sWeapon ) )
	{
		...
		if( player isBuffUnlockedForWeapon( "specialty_holdbreathwhileads", baseWeapon ) && player isBuffEquippedOnWeapon( "specialty_holdbreathwhileads", baseWeapon ) && player _hasPerk( "specialty_holdbreathwhileads" ) && player.holdingBreath )
	...
	if( player IsItemUnlocked( "specialty_autospot" ) && player _hasPerk( "specialty_autospot" ) && player.holdingBreath )
...
vehicleKilled( owner, vehicle, eInflictor, attacker, iDamage, sMeansOfDeath, sWeapon )
{
	...
	if ( isDefined( attacker ) && isPlayer( attacker ) && ( !isDefined( owner ) || attacker != owner ) && !isKillstreakWeapon( sWeapon ) )
	{
		...
		// check lock-on only waepons for this challenge
		switch( sWeapon )
		{
		case "iw5_smaw_mp":
		case "stinger_mp":
...
processChallenge( baseName, progressInc, forceSetProgress )
{
	...
	if( !IsSubStr( baseName, "_daily" ) && !IsSubStr( baseName, "_weekly" ) )
	{
	...
	if ( reachedNewTier )
	{
		...
		// if this is the mastery challenge then we don't need to process it again
		if( !IsSubStr( baseName, "_mastery" ) )
...
masteryChallengeProcess( baseName, progressInc )
{
	...
	if ( isSubStr( baseName, "ch_marksman_" ) )
		prefix = "ch_marksman_";
	else if ( isSubStr( baseName, "ch_expert_" ) )
		prefix = "ch_expert_";
	else if ( isSubStr( baseName, "pr_marksman_" ) )
		prefix = "pr_marksman_";
	else if ( isSubStr( baseName, "pr_expert_" ) )
		prefix = "pr_expert_";
