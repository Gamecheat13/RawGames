...
fx_zone_watcher_either(num,flag,flag2)
{
	for(;;)
	{
		...
		send_activate_zone_msg( num );
		if(level.fx_zone_messages == true)
			IPrintLn("entering fx zone " + num);
		exploder(num);
		
		flag_waitopen_both(flag,flag2);
		send_deactivate_zone_msg( num );
		if(level.fx_zone_messages == true)
			IPrintLn("exiting fx zone " + num);
		PauseExploders( num );
		wait(2.0);
	}
}
...
play_fx_attached_to_chain(chain_ent)
{
	...
	for(x = 0; x < 15; x++)
	{
		fx_tag_origin[x] = spawn_tag_origin();
	}
	...
	for(;;)
	{
		for(i=0;i<chain_ent.v["tags"].size;i++)
		{
			...
			curr_len = chain_ent.v["tag_lens"][i];
			fx_dir = anglestoforward(curr_tag.angles);
			current_tag_origin = fx_tag_origin[t];
			//the linkto offset is probaly not quite correct, don't have time to fix right now
			current_tag_origin linkto(curr_tag, "tag_origin", (fx_dir * curr_len * randomfloat(1.0)), (0,0,0));
			playfxontag(chain_ent.v["fx"],current_tag_origin ,"tag_origin");
			t++;
		}
		wait(chain_ent.v["looptime"]);
	}
...
kill_fx_attached_to_chain(chain_ent)
{
	level waittill(chain_ent.v["kill_notify"]);
	print("message received");
	for(i=0;i<chain_ent.v["tags"].size;i++)
...
=============
///ScriptDocBegin
"Name: play_fx_attached_to_actor(inArgs)"
"Summary: Will play a fx attached along the bones of the AI"
"MandatoryArg: An Ent and FXent in the form of a struct "
"Example: 
	...
	args.v["chainset_override"]=overridearg;//defaults to undefined - define a custom boneset (skeleton) if necessary
											// chainset_name must be "override" unless a name is defined 
	notify1 = play_fx_attached_to_actor(args);//this will return the level notify to kill the fx
...
play_fx_attached_to_actor(inArgs)
{
	ent 				= inArgs.v["ent"];//the actor ent
	fx 					= inArgs.v["fx"];//fx ent
	chain 				= inArgs.v["chain"];//chain name - string
	...
	foreach(curr_chain in chains)
	{
		...
		setup_fx_chain(curr_chain_ent);
		thread play_fx_attached_to_chain(curr_chain_ent);
		thread kill_fx_attached_to_chain(curr_chain_ent);
		chain_ents[chain_ents.size] = curr_chain_ent;
...
//Functions to set up and play fx from a wake volume.  A wake volume is a trigger with an effect set on it.  When players or NPCs walk through it
//they will spawn fx around their feet as they move.
setup_wake_volumes()
{
		wake_triggers = getentarray( "wake_volume", "targetname" );
		if(IsDefined(wake_triggers[0]))
			array_thread( wake_triggers, ::wake_trigger_think);
}
...
wake_trigger_think()
{
	for( ;; )
	{
		...
		if (other ent_flag_exist("in_wake_volume"))
			{}
		else
			other ent_flag_init("in_wake_volume");
		
		if (DistanceSquared( other.origin, level.player.origin ) < 9250000)
		{	
			if (other ent_flag("in_wake_volume"))
			{}
			else
			{
				other thread volume_wakefx(self);
				other ent_flag_set ("in_wake_volume");
				/*if(isDefined (other.ainame))
					print(other.ainame + "has entered the wake volume\n");
				else
					print("player has entered the wake volume\n");*/
			}
		}
	}
}
	...
	for ( ;; )
	{
		if (self IsTouching(parentTrigger))
		{
			...
			if (fx != "null")
			{
				...
				PlayFX( wake_fx, start, up, forward );
			}

		}
		else
		{	
			self ent_flag_clear("in_wake_volume");
				/*if(isDefined (self.ainame))
					print(self.ainame + "has exited the wake volume\n");
				else
					print("player has exited the wake volume\n");*/
			return;
		}
	}
}
...
{
	flipbook_models = GetEntArray( "flipbook_model", "script_noteworthy" );
	array_thread( flipbook_models, ::flipbook_model );
	
	model_flicker = GetEntArray( "model_flicker", "script_noteworthy" );
	array_thread( model_flicker, ::model_flicker );
}

flipbook_model()
{
	//an example of this is in wb_animated_signs_2.map
	model_list = [];
	model_list[0] = self;
	
	//find all associated models and hide them
	currentModel = self;
	for( ;; )
	{
		AssertEx(IsDefined(currentModel.target), ("No target defined for flipbook model at " + currentModel.origin));
		nextModel = getent( currentModel.target, "targetname" );
		
		AssertEx((nextModel.classname == "script_model" || nextModel.classname == "script_brushmodel"), ("Flipbook target at " + nextModel.origin + " not a script model or script brushmodel."));
		if (nextModel == self)
			break;
		nextModel hide();
		// unless its specified to keep the model's location, move models to the origin of the original model
		if (isDefined(nextModel.script_location))
		{
		    	if (nextModel.script_location == "1")
		    		nextModel.origin = nextModel.origin;
		    	else
		    		nextModel.origin = self.origin;
		}
		else
			nextModel.origin = self.origin;
	...
	//play flipbook animation of models
	currentModel = self;
	for( ;; )
	{
		if(IsDefined(currentModel.script_delay))
		{
			if( currentModel.script_delay == 0 )
				wait(RandomFloatRange( .05, .35 ));
			if( currentModel.script_delay == 66 )
				wait(RandomFloatRange( .2, 2.5 ));
			else
				wait(currentModel.script_delay);
		}
		else
			wait(1.0);
		
		nextModel = getent( currentModel.target, "targetname" );
		AssertEx(IsDefined(nextModel), ("Flipbook target model not assigned for model at " + nextModel.origin));
		AssertEx((nextModel.classname == "script_model" || nextModel.classname == "script_brushmodel"), ("Flipbook target at " + nextModel.origin + " not a script model or script brushmodel."));
...
model_flicker()
{
	...
	//show/hide model to flicker
	while ( true )
	{
		if ( !IsDefined( self ) )
			return;
		
		time_between_flicker = RandomFloatRange( 2, 4 );
		number_of_flickers = RandomIntRange( 2, 4 );
