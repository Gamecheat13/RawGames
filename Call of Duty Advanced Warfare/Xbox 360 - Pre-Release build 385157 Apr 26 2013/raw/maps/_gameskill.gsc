...
// 				self notify( "hit_again" ); was cutting off the overlay fadeout
		...
		if ( ratio <= self.gs.healthOverlayCutoff && self.player_health_packets > 1 )
		{
			...
			if ( !wasVeryHurt )
			{
				...
				{
					//Test for hdr and use less blur. Do this as temp fix until blur and hdr is fixed.
					if ( IsUsingHDR() )
					{
						thread blurView( 2, 2 );
					}
...
playerBreathingSound( healthcap )
{
	...
	for ( ;; )
	{
		...
		sound_alias = "breathing_hurt";		
		if ( SoundExists( sound_alias ) )
			self PlayLocalSound( sound_alias );
