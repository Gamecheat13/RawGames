...
main()
{
	...
	//Add test for HDR to use different vision set. Might be able to change this back after Danny looks at blur and screen brightness issue.
	if ( IsUsingHDR() )
	{
		VisionSetPain( "near_death_hdr" );
	}
	else
	{
...
// 	level.ai_array = [];
	...
	//LUI events for showing\hiding the player HUD
	...
	for ( p = 0; p < 7; p++ )
	{
		...
		for ( i = 0; i < triggers.size; i++ )
		{
			...
			if ( IsDefined( triggers[ i ].ambient ) )
				triggers[ i ] thread soundscripts\_audio::trigger_multiple_audio_trigger(true);

			if ( 	 IsDefined( triggers[ i ].script_audio_zones )  // this trigger has multiple uses and may contain any one of these
					...
					triggers[ i ] thread soundscripts\_audio::trigger_multiple_audio_trigger();
...
slideTriggerPlayerThink( trig )
{
	...
	thread soundscripts\_audio::aud_send_msg("start_player_slide_trigger", self);
	...
	self EndSliding();
	thread soundscripts\_audio::aud_send_msg("end_player_slide_trigger", self);
}
...
setupExploders()
{
	...
	//maybe next game I'll bother to try this optimization out on other levels. - Nate.
...
*/		
	...
	else
	{
		...
		transients_to_load = start_array[ "transients_to_load" ];
		if ( IsDefined( transients_to_load ) )
		{
			foreach ( other_trans in level.start_transients )
			{
				if ( !array_contains( transients_to_load, other_trans ) )
				{
					if ( IsTransientLoaded( other_trans ) )
						UnloadTransient( other_trans );
				}
			}
			foreach ( trans_name in transients_to_load )
			{
				if ( !IsTransientLoaded( trans_name ) )
					LoadTransient( trans_name );
			}
		}
...
trigger_vehicle_spline_spawn( trigger )
{
	...
	{
		spawner thread maps\_vehicle_free_drive::spawn_vehicle_and_attach_to_free_path( 70 );
...
trigger_vehicle_getin_spawn( trigger )
{
	...
	vehicle_spawners = GetEntArray( trigger.target, "targetname" );
	array_thread( vehicle_spawners, maps\_vehicle_free_drive::spawn_vehicle_and_attach_to_free_path(undefined, false, true));
}
...
//					println( "Progress = " + progress );
			...
			else
			{
				...
				while (player IsTouching(trigger)) wait(0.25);
			}
		}
	}
}
...
window_destroy()
{
	...
	if( IsDefined( self.script_noteworthy ) )
	{
		...
		{
			if( self.script_noteworthy == "ondamage_appear" ) self Hide();
			level waittillmatch( "glass_damaged", glassID );
...
default_footsteps()
{
	...
	animscripts\utility::setFootstepEffect( "water", 	LoadFX( "vfx/treadfx/footstep_water" ) );
	...
	animscripts\utility::setFootstepEffectSmall( "water", 		LoadFX( "vfx/treadfx/footstep_water" ) );
	...
	*/
	animscripts\utility::setNotetrackEffect( "bodyfall small", 		"J_SpineLower", 		"dirt",		loadfx ( "vfx/treadfx/bodyfall_default_runner" ), "bodyfall_", "_small" );
	animscripts\utility::setNotetrackEffect( "bodyfall small", 		"J_SpineLower", 		"concrete",	loadfx ( "vfx/treadfx/bodyfall_default_runner" ), "bodyfall_", "_small" );
	animscripts\utility::setNotetrackEffect( "bodyfall small", 		"J_SpineLower", 		"asphalt",	loadfx ( "vfx/treadfx/bodyfall_default_runner" ), "bodyfall_", "_small" );
	animscripts\utility::setNotetrackEffect( "bodyfall small", 		"J_SpineLower", 		"rock",		loadfx ( "vfx/treadfx/bodyfall_default_runner" ), "bodyfall_", "_small" );
	
	animscripts\utility::setNotetrackEffect( "bodyfall large", 		"J_SpineLower", 		"dirt",		loadfx ( "vfx/treadfx/bodyfall_default_runner" ), "bodyfall_", "_large" );
	animscripts\utility::setNotetrackEffect( "bodyfall large", 		"J_SpineLower", 		"concrete",	loadfx ( "vfx/treadfx/bodyfall_default_runner" ), "bodyfall_", "_large" );
	animscripts\utility::setNotetrackEffect( "bodyfall large", 		"J_SpineLower", 		"asphalt",	loadfx ( "vfx/treadfx/bodyfall_default_runner" ), "bodyfall_", "_large" );
	animscripts\utility::setNotetrackEffect( "bodyfall large", 		"J_SpineLower", 		"rock",		loadfx ( "vfx/treadfx/bodyfall_default_runner" ), "bodyfall_", "_large" );
	
	/* Will revive this when we get knee impact notetracks
	animscripts\utility::setNotetrackEffect( "knee fx left", 		"J_Knee_LE", 			"dirt",		loadfx ( "vfx/treadfx/footstep_dust" ) );
	animscripts\utility::setNotetrackEffect( "knee fx left", 		"J_Knee_LE", 			"concrete",	loadfx ( "vfx/treadfx/footstep_dust" ) );
	animscripts\utility::setNotetrackEffect( "knee fx left", 		"J_Knee_LE", 			"asphalt",	loadfx ( "vfx/treadfx/footstep_dust" ) );
	animscripts\utility::setNotetrackEffect( "knee fx left", 		"J_Knee_LE", 			"rock",		loadfx ( "vfx/treadfx/footstep_dust" ) );
	animscripts\utility::setNotetrackEffect( "knee fx left", 		"J_Knee_LE", 			"mud",		loadfx ( "impacts/footstep_mud" ) );
	
	animscripts\utility::setNotetrackEffect( "knee fx right", 		"J_Knee_RI", 			"dirt",		loadfx ( "vfx/treadfx/footstep_dust" ) );
	animscripts\utility::setNotetrackEffect( "knee fx right", 		"J_Knee_RI", 			"concrete",	loadfx ( "vfx/treadfx/footstep_dust" ) );
	animscripts\utility::setNotetrackEffect( "knee fx right", 		"J_Knee_RI", 			"asphalt",	loadfx ( "vfx/treadfx/footstep_dust" ) );
	animscripts\utility::setNotetrackEffect( "knee fx right", 		"J_Knee_RI", 			"rock",		loadfx ( "vfx/treadfx/footstep_dust" ) );
