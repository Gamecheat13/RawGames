...
tweakart()
{
	...
	SetDvar( "scr_dof_bias", level.dofDefault[ "bias" ] );
...
updateVisionSet()
{
	...
	ent.r_charLightAmbient        = GetDvar( "r_charLightAmbient" );
	ent.r_viewModelLightAmbient   = GetDvar( "r_viewModelLightAmbient" );
	ent.r_viewModelPrimaryLightUseTweaks = GetDvar( "r_viewModelPrimaryLightUseTweaks" );
	ent.r_viewModelPrimaryLightTweakDiffuseStrength = GetDvar( "r_viewModelPrimaryLightTweakDiffuseStrength" );
	ent.r_viewModelPrimaryLightTweakSpecularStrength = GetDvar( "r_viewModelPrimaryLightTweakSpecularStrength" );
}
...
fovslidercheck()
{
	...
	if ( level.dofDefault[ "bias" ] <= level.dofDefault[ "bias" ] )
	{
		level.dofDefault[ "bias" ] = level.dofDefault[ "bias" ] + 1;
		SetDvar( "scr_dof_bias", level.dofDefault[ "bias" ] );
	}	
}
...
get_fog_filename()
{
	if ( IsUsingHDR() )
	{
		return "\\share\\raw\\maps\\createart\\" + get_template_level() + "_fog_hdr.gsc";
	}
	else
	{
		return "\\share\\raw\\maps\\createart\\" + get_template_level() + "_fog.gsc";
	}
}
...
dumpsettings()
{
	...
	fileprint_launcher( "\t{" );
	fileprint_launcher( "\t\tmaps\\createart\\" + get_template_level() + "_fog_hdr::main();" );
	fileprint_launcher( "\t}" );
    ...
    fileprint_launcher( "rawfile,maps/createart/"+ get_template_level() + "_fog_hdr.gsc"  );
    ...
    fileprint_launcher( "}" );
    fileprint_launcher_end_file( get_fog_filename(), true );
...
isDoFDefault()
{
	...
	if ( level.dofDefault[ "bias" ] != GetDvarInt( "scr_dof_bias" ) )
		return false;
