...
add_start( msg, func, loc_string, optional_func, optional_transients_to_load )
{
    add_start_assert();
	...
	if ( IsDefined( level.start_description ) && IsDefined( level.start_description[ msg ]  ) )
	{
    	...
    	array = add_start_construct( msg, func, loc_string, optional_func, optional_transients_to_load );
...
add_start_construct( msg, func, loc_string, optional_func, optional_transients_to_load )
{
...
display_hint( hint, parm1, parm2, parm3, y_offset )
{
	...
	if ( IsDefined( level.trigger_hint_func[ hint ] ) )
	{
		...
		player thread HintPrint( level.trigger_hint_string[ hint ], level.trigger_hint_func[ hint ], parm1, parm2, parm3, 30, y_offset );
	}
	else
	{
		player thread HintPrint( level.trigger_hint_string[ hint ], undefined, undefined, undefined, undefined, 30, y_offset );
	}
}
...
display_hint_timeout( hint, timeout, parm1, parm2, parm3, y_offset )
{
	...
	player thread HintPrint( level.trigger_hint_string[ hint ], level.trigger_hint_func[ hint ], parm1, parm2, parm3, timeout, y_offset );
}
...
/#
			transition_time = 0;
			VisionSetNaked( vision_set, transition_time, /* override clienttrigger = */true  );
	...
	else
	{
		...
		self VisionSetNakedForPlayer( vision_set, transition_time, /* override clienttrigger = */true );
	}
...
set_art_tweaked_vision_set()
{
	...
	ent = level.vision_set_vision[ level.vision_set_transition_ent.vision_set ];
		...
		setdvar( "scr_charLightAmbient", ent.r_charLightAmbient );
		setdvar( "scr_viewModelLightAmbient", ent.r_viewModelLightAmbient );
		setdvar( "scr_viewModelPrimaryLightUseTweaks", ent.r_viewModelPrimaryLightUseTweaks );
		setdvar( "scr_viewModelPrimaryLightTweakDiffuseStrength", ent.r_viewModelPrimaryLightTweakDiffuseStrength );
		setdvar( "scr_viewModelPrimaryLightTweakSpecularStrength", ent.r_viewModelPrimaryLightTweakSpecularStrength );
...
"Summary: Masks interactives in an array of volumes so they dont use entities. Can be used before or after _load but only after level_precache::main()."
"Module: Utility"
"CallOn: An entity"
"MandatoryArg: <volumes>: Array of volumes."
...
{
	// Note: This function only works for interactives of types that have a ::saveToStructFn defined, and that are listed at the top here.
	combined_interactives = GetEntArray( "interactive_birds", "targetname" );
	more_interactives = GetEntArray( "interactive_vulture", "targetname" );
	combined_interactives = array_combine( combined_interactives, more_interactives );
	
	foreach ( volume in volumes )
	{
		AssertEx( !isDefined( volume.interactives ), "Trying to mask interactives in a volume that already has interactives masked into it." );
		volume.interactives = [];
	}

	foreach ( ent in combined_interactives )
	{
		AssertEx( IsDefined( ent.interactive_type ), ent.targetname + " entity does not have an interactive_type keypair." );
		AssertEx( IsDefined( level._interactive ), ent.targetname + " entity in map, but level._interactive is not defined." );
		AssertEx( IsDefined( level._interactive[ ent.interactive_type ] ), ( ent.targetname + " entity in map, but level._interactive[" + ent.interactive_type + "] is not defined." ) );
		
		temp = level._interactive[ ent.interactive_type ];
		if ( !isDefined( temp.saveToStructFn ) )
			continue;
		foreach ( volume in volumes )
		{
...
"Summary: Activate interactives in a masked volume."
"Module: Utility"
"CallOn: A mask volume"
"Example: activate_interactives_in_volume( vol );"
"SPMP: singleplayer"
///ScriptDocEnd
=============
*/
...
"Summary: Deletes interactives in an array of volumes so they dont use entities."
"Module: Utility"
"MandatoryArg: <volumes>: Array of volumes "
