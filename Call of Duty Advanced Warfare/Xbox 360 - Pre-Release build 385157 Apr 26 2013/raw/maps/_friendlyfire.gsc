...
friendly_fire_think( entity )
{
	...
	for ( ;; )
	{
		...
		// Wrapped to keep it contained for london/innocent
...
missionfail( civilianKilled )
{
	...
	if ( civilianKilled )
		maps\_player_death::set_deadquote( &"SCRIPT_MISSIONFAIL_CIVILIAN_KILLED" );	// You shot a civilian. Watch your fire!
	else if ( IsDefined( level.custom_friendly_fire_message ) )
		maps\_player_death::set_deadquote( level.custom_friendly_fire_message );		// friendly fire will not be tolerated
	else if ( level.campaign == "british" )
		maps\_player_death::set_deadquote( &"SCRIPT_MISSIONFAIL_KILLTEAM_BRITISH" );	// Friendly fire will not be tolerated!
	else if ( level.campaign == "russian" )
		maps\_player_death::set_deadquote( &"SCRIPT_MISSIONFAIL_KILLTEAM_RUSSIAN" );	// You are a traitor to the motherland!
	else
		maps\_player_death::set_deadquote( &"SCRIPT_MISSIONFAIL_KILLTEAM_AMERICAN" );	// Friendly fire will not be tolerated!
