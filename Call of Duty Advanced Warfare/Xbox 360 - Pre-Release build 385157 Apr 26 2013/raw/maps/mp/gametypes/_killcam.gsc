...
)
{
	...
	if (getdvar("scr_killcam_time") == "") {
		if ( sWeapon == "artillery_mp" || sWeapon == "stealth_bomb_mp" || sWeapon == "remote_mortar_missile_mp" || sWeapon == "orbital_laser_mp" )
			camtime = (gettime() - killcamentitystarttime) / 1000 - predelay - .1;
		else if ( level.showingFinalKillcam	|| sWeapon == "agent_mp" )
			camtime = 4.0;
	...
	*/
	...
	if ( timeUntilRespawn && !level.gameEnded || ( isDefined( self ) && isDefined(self.isInFireTeam) && self.isInFireTeam && !level.gameEnded ) )
	{
	...
	if ( isDefined(self.isInFireTeam) && self.isInFireTeam && !level.gameEnded )
		self.timeStartedToWait = GetTime();
