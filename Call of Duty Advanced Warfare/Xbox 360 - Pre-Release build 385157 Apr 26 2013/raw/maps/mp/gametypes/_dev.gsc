...
init()
{
	...
	SetDevDvarIfUninitialized( "scr_devPrintDamage", "" );
...
updateDevSettings()
{
	...
	else if ( GetDvar("scr_testclients_givecarepackage") != "" )
		{
		streakName = GetDvar( "scr_testclients_givecarepackage" );
...
addATestClient( fullBot, team )
{
	...
	if ( fullBot && IsDefined( level.bot_funcs ) && IsDefined( level.bot_funcs["bots_spawn"] ) )
	{		
		ent = [[ level.bot_funcs["bots_spawn"] ]]( 1, team );	// TODO: need to ensure this returns the ent
		return ent;
	}
	else
	{
...
botUseKillstreak( killstreak ) // self == bot
{
	...
	// NOTE: if we want a "bot" to call a killstreak it doesn't matter what kind of bot it is, this is dev only
...
// doesn't function for "full bots" (Bots(AI) in devgui)
...
giveSpecialGrenade()
{
	...
	if( IsDefined( equipment ) )
	{
		foreach( player in level.players )
		{
			...
			if ( equipment == "flash_grenade_mp" || equipment == "ninebang_grenade_mp" )
			...
			else if ( equipment == "paint_grenade_mp" )
				player SetOffhandSecondaryClass( "flash" );
			else 
				player SetOffhandSecondaryClass( "flash" );
...
devWatchWeaponXP() // self == player (host only for now)
{
	...
	weaponName = getBaseWeaponName( weapon );
