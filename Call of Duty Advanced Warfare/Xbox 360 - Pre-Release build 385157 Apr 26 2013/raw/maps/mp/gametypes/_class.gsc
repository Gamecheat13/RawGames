...
{
	customClassLoc = cac_getCustomClassLoc();
	return self getPlayerData( customClassLoc, classIndex, "weaponSetups", weaponIndex, "attachment", 2 );
}
...
{
	customClassLoc = cac_getCustomClassLoc();
	return self getPlayerData( customClassLoc, classIndex, "perkSlots", perkIndex );
}
...
cac_getStreaktype( classIndex )
{
	customClassLoc = cac_getCustomClassLoc();
	return self getPlayerData( customClassLoc, classIndex, "streaktype" );
}
...
{
	customClassLoc = cac_getCustomClassLoc();
	return self getPlayerData( customClassLoc, classIndex, "equipmentSetups", equipmentIndex, "equipment" );
}
...
table_getStreaktype( tableName, classIndex )
{
	return tableLookup( tableName, 0, "loadoutStreaktype", classIndex + 1 );
}
...
cloneLoadout()
{
 	...
 	if (teamName != "none")
	{
		...
		loadoutStreakType = getMatchRulesData( "defaultClasses", teamName, classIndex, "class", "streaktype" );
		...
		loadoutKillstreak4 = recipe_getKillstreak( teamName, classIndex, loadoutStreakType, 3 );
 	...
 	else if( isSubstr( class, "custom" ) )
	{
		...
		loadoutStreakType = cac_getStreaktype( class_num );
		...
		loadoutKillstreak4 = cac_getKillstreak( class_num, loadoutStreakType, 3 );
	...
	else
	{
		...
		loadoutStreakType = table_getStreaktype( level.classTableName, class_num );
		loadoutOffhand = table_getOffhand( level.classTableName, class_num );
		loadoutOffhand = table_getEquipment( level.classTableName, class_num, 1 );
	...
	clonedLoadout["loadoutKillstreak4"] = loadoutKillstreak4;
...
giveLoadout( team, class, allowCopycat, setPrimarySpawnWeapon ) // setPrimarySpawnWeapon only when called during spawn
{
	...
	else if (teamName != "none")
	{
		...
		if ( ( ( loadoutPrimary == "throwingknife" ) || ( loadoutPrimary == "none" ) ) && loadoutSecondary != "none" )
		{
			...
			// TODO: address the weapon perks in this situation			
		...
		loadoutEquipment = getMatchRulesData( "defaultClasses", teamName, classIndex, "class", "equipmentSetup", 0, "equipment" );
		
		//	replace secondary with default in case overkill perk was used to give 2 primaries and perks are disabled
		if ( loadoutSecondary != "none" && !isValidSecondary( loadoutSecondary, loadoutPerks[1], loadoutPerks[2], false ) )
		{
			// TODO: address the weapon perks in this situation			
		...
		loadoutOffhand = getMatchRulesData( "defaultClasses", teamName, classIndex, "class", "equipmentSetup", 1, "equipment" );
	...
	else if ( class == "gamemode" )
	{
		...
		if ( level.killstreakRewards && isDefined( gamemodeLoadout["loadoutStreakType"] ) && gamemodeLoadout["loadoutStreakType"] != "specialty_null" )
		{
			...
			loadoutKillstreak4 = gamemodeLoadout["loadoutKillstreak4"];
		}
	...
	else if( class == "juggernaut" )
	{
		...
		loadoutPerks[0] = "specialty_class_scavenger";
		loadoutPerks[1] = "specialty_class_quickdraw";
	...
	else if ( class == "callback" )
	{
		...
		{
			idx = i+1;
			loadoutPerks[i] = callbackLoadout["loadoutPerk"+idx];
		}
	...
	if ( !isGameModeClass && !isRecipeClass && !(isDefined( self.pers["copyCatLoadout"] ) && self.pers["copyCatLoadout"]["inUse"] && allowCopycat) )
	{
		...
		if ( !isValidAttachment( loadoutPrimaryAttachment3 ) || (  level.rankedMatch && isCustomClass && !self isAttachmentUnlocked( loadoutPrimary, loadoutPrimaryAttachment3 ) ) )
			loadoutPrimaryAttachment3 = table_getWeaponAttachment( level.classTableName, 10, 0, 2 );		
		
		//if ( !isValidWeaponBuff( loadoutPrimaryPerks[3], loadoutPrimary ) || ( level.rankedMatch && isCustomClass && !self isWeaponBuffUnlocked( loadoutPrimary, loadoutPrimaryPerks[3] )) )
		//	loadoutPrimaryPerks[3] = table_getWeaponBuff( level.classTableName, 10, 0 );
		...
		if ( !isValidSecondary( loadoutSecondary, loadoutPerks[1], loadoutPerks[2] ) || (  level.rankedMatch && level.rankedMatch && isCustomClass && !self isItemUnlocked( loadoutSecondary )) )
		{
			// TODO: address the weapon perks in this situation			
			loadoutSecondary = table_getWeapon( level.classTableName, 10, 1 );
			loadoutSecondaryAttachment = "none";
		...
		if ( !isValidAttachment( loadoutSecondaryAttachment3 ) || (  level.rankedMatch && isCustomClass && !self isAttachmentUnlocked( loadoutSecondary, loadoutSecondaryAttachment3 )) )
			loadoutSecondaryAttachment3 = table_getWeaponAttachment( level.classTableName, 10, 1, 2 );		
		
		//if ( loadoutPrimaryPerks[1] == "specialty_twoprimaries" && !isValidWeaponBuff( loadoutSecondaryPerks[3], loadoutSecondary ) || ( level.rankedMatch && isCustomClass && !self isItemUnlocked( loadoutSecondary + " " + loadoutSecondaryPerks[3] )) )
		//	loadoutSecondaryPerks[3] = table_getWeaponBuff( level.classTableName, 10, 1 );
	...
	{
		secondaryName = buildWeaponName( loadoutSecondary, loadoutSecondaryAttachment, loadoutSecondaryAttachment2, loadoutSecondaryAttachment3, self.loadoutSecondaryCamo, self.loadoutSecondaryReticle );
	...
	// Primary Weapon
	primaryName = buildWeaponName( loadoutPrimary, loadoutPrimaryAttachment, loadoutPrimaryAttachment2, loadoutPrimaryAttachment3, self.loadoutPrimaryCamo, self.loadoutPrimaryReticle );
	self _giveWeapon( primaryName );

	if ( IsBot( self ) == false )
	{
		// Bots handle weapon switching internally
	...
	if ( loadoutOffhand == "none" || loadoutOffhand == "specialty_null" )
	...
	else if( loadoutOffhand == "paint_grenade_mp" )
		self SetOffhandSecondaryClass( "flash" );
	else 
		self SetOffhandSecondaryClass( "flash" );
	...
	self loadoutAllPerks( loadoutEquipment, loadoutPerks );
	self maps\mp\perks\_perks::applyPerkStack();
	
	if ( self _hasPerk( "specialty_extraammo" ) )
	{
		fraction = 1.0;
		if ( isdefined( self.extraammo_fraction ) )
		{
			startFraction = self GetFractionMaxAmmo( primaryName );
			fraction = startFraction + ( self.extraammo_fraction * (1 - startFraction) );
		}
		self giveMaxAmmo( primaryName, fraction );
		...
		if ( secondaryName != "none" && getWeaponClass( secondaryName ) != "weapon_projectile" )
		{
			...
			if ( isdefined( self.extraammo_fraction ) )
			{
				startFraction = self GetFractionMaxAmmo( secondaryName );
				fraction = startFraction + ( self.extraammo_fraction * (1 - startFraction) );
			}
			self giveMaxAmmo( secondaryName, fraction );
		}
	}
	...
	if( self.pers["cur_death_streak"] > 0 )
	{
		...
		if ( self getPerkUpgrade( loadoutPrimaryPerks[0] ) == "specialty_rollover" || self getPerkUpgrade( loadoutPrimaryPerks[1] ) == "specialty_rollover" || self getPerkUpgrade( loadoutPrimaryPerks[2] ) == "specialty_rollover" )
		{
	...
	*/
	...
	if ( level.killstreakRewards && !isDefined( loadoutKillstreak1 ) && !isDefined( loadoutKillstreak2 ) && !isDefined( loadoutKillstreak3 ) && !isDefined( loadoutKillstreak4 ) )
	{
		...
		else
		{
			...
			case "support":
				...
				// defaultKillstreak4 is still undefined, assuming default classes don't have the extra slot
				playerData = "defenseStreaks";
				break;
			case "specialist":
				defaultKillstreak1 = table_getKillstreak( level.classTableName, 1, 1 );
			...
			if( IsSubStr( class, "custom" ) )
			{
				...
				loadoutKillstreak4 = self getPlayerData( customClassLoc, self.class_num, playerData, 3 );
			}
			...
			if( IsSubStr( class, "juggernaut" ) || isGameModeClass )
			{
				...
				if ( isGameModeClass && self.streakType == "specialist" )
				{
					...
					self.pers["gamemodeLoadout"]["loadoutKillstreak4"] = loadoutKillstreak4;
				}
			}
			...
			if( !IsDefined( loadoutKillstreak3 ) )
				loadoutKillstreak3 = "none";
			if( !IsDefined( loadoutKillstreak4 ) )
				loadoutKillstreak4 = "none";
			...
			if( !isValidKillstreak( loadoutKillstreak4 ) || ( isCustomClass && !self isItemUnlocked( loadoutKillstreak4 ) )
					|| !( self _hasPerk( "specialty_extra_killstreak" ) ) )
				loadoutKillstreak4 = "none";
		}
	}
	...
	self setKillstreaks( loadoutKillstreak1, loadoutKillstreak2, loadoutKillstreak3, loadoutKillstreak4 );
...
_detachAll()
{
	...
	{
		if ( self.hasRiotShieldEquipped && ( !isDefined( self.hasRiotShieldHidden ) || self.hasRiotShieldHidden == false ) )
		{
...
loadoutAllPerks( loadoutEquipment, loadoutPerks )
{
	...
	//if( loadoutPrimaryPerks[1] == "specialty_twoprimaries" )	// TODO: this needs to be adjusted based on which slot this could be in
	//	loadoutSecondaryPerks[3] = maps\mp\perks\_perks::validatePerk( undefined, loadoutSecondaryPerks[3] );
	...
	if( loadoutEquipment == "none" || loadoutEquipment == "specialty_null" )
		self SetOffhandPrimaryClass( "none" );
	else if ( loadoutEquipment == "flash_grenade_mp" )
		self SetOffhandPrimaryClass( "flash" );
	else if ( loadoutEquipment == "smoke_grenade_mp" || loadoutEquipment == "concussion_grenade_mp" )
	...
	for (i=0; i<6; i++)
	{
		perkUpgrd[i] = tablelookup( "mp/perktable.csv", 1, loadoutPrimaryPerks[i], 8 );
	}
...
watchOffhandUse()
{
	...
	for( ;; )
	{
		self waittill ( "grenade_pullback", newWeapon );
		
		if ( self.hasRiotShieldEquipped )
		{
			self DetachShieldModel( "weapon_riot_shield_mp", "tag_weapon_left" );
			self.hasRiotShieldHidden = true;
		}
		else
		{
			continue;	
		...
		self waittill ( "offhand_end", newWeapon );
			
		if ( self.hasRiotShieldEquipped )
		{
			self AttachShieldModel( "weapon_riot_shield_mp", "tag_weapon_left" );
			self.hasRiotShieldHidden = false;
		}
	}
}
...
anyAttachmentIsScope( attachments )
{
	...
	{
		if( getAttachmentType( a ) == "rail" || a == "zoomscope" )
			return true;
	}
	return false;
}

processAttachments( attachments, baseName, bareWeaponName )
{
	//assert( IsArray( attachments ) );
	...
	if ( needsScopeOverride( baseName, attachments ) )
	{
...
buildWeaponName( baseName, attachment1, attachment2, attachment3, camo, reticle )
{
	...
	//Handle locked on loot attachments here
	if( isLootWeapon( baseName ) )
	{	
		//find out if it's loot
		//figure out if it has a default
		//if it does have a default and no other attachments have been selected, apply the default.
		default_attachment = getDefaultAttachment( baseName );
		
		if( default_attachment != "" )
		{
			if( attachment1 == "none" )
			{
				//this means if we manage to set an attachment we will skip putting the default on this gun
				attachment1 = default_attachment;
			}
		}
	}
	...
	attachments = processAttachments( attachments, baseName, bareWeaponName );
...
setKillstreaks( streak1, streak2, streak3, streak4 )
{
	self.killStreaks = [];
	...
	{
			streak = maps\mp\killstreaks\_map_killstreak::checkMapKillstreak( streak );
			
			//it's possible that the map does not have a map based killstreak and has returned none.
			if( streak == "none" )
				continue;
...
isValidEquipment( refString )
{
	switch ( refString )
	{
		...
		//case "specialty_portable_radar": //portable radar as lethal?
			return true;
		default:
