...
{	
	...
	if ( self.infect_firstSpawn )
	{
		...
		//	track disconnect to update scores
...
chooseFirstInfected()
{
	...
	if ( isDefined ( first.setSpawnpoint ) )
		first maps\mp\perks\_perkfunctions::deleteTI( first.setSpawnpoint );	
...
{	
	...
	if ( isUsingMatchRulesData() && GetMatchRulesData( "defaultClasses", "axis", 0, "class", "inUse" ) )
	{
		...
		level.infect_loadouts["axis"]["loadoutKillstreak4"] = "none";
	}
	else
	{
		...
		level.infect_loadouts["axis"]["loadoutPerks"] = maps\mp\perks\_perks::getEmptyPerks();
	...
	if ( isUsingMatchRulesData() && GetMatchRulesData( "defaultClasses", "axis", 5, "class", "inUse" ) )
	{
		...
		level.infect_loadouts["axis_initial"]["loadoutKillstreak4"] = "none";	
	}
	else
	{
		...
		level.infect_loadouts["axis_initial"]["loadoutPerks"] = maps\mp\perks\_perks::getEmptyPerks();
	...
	else
	{
		...
		level.infect_loadouts["allies"]["loadoutPerks"] = maps\mp\perks\_perks::getEmptyPerks();
