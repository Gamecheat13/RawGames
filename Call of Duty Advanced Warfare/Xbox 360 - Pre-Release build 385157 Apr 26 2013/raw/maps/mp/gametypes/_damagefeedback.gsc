...
updateDamageFeedback( typeHit )
{
	...
	if ( typeHit == "grenadeBlock" )
	{
		self.hud_damagefeedback setShader("damage_feedback_j", 24, 48); // TODO: change feedback
		self playlocalsound("MP_hit_alert"); // TODO: change sound?
	}
	else if ( typeHit == "hitBodyArmor" )
	{
	...
	else if ( typeHit == "mp_solar" )
	{
		self.hud_damagefeedback setShader("damage_feedback", 24, 48);
		
		if ( !IsDefined( self.shouldloopdamagefeedback ) )
		{
			self thread maps\mp\killstreaks\mp_solar::handleDamageFeedbackSound();
