...
handleNormalDeath( lifeId, attacker, eInflictor, sWeapon, sMeansOfDeath )
{
	...
	attacker thread maps\mp\gametypes\_rank::giveRankXP( "kill", value, sWeapon, sMeansOfDeath, undefined );
...
LaunchShield( damage, meansOfDeath )
{
	...
	if( !IsDefined( self.hasRiotShieldHidden ) || self.hasRiotShieldHidden == false )
...
PlayerKilled_internal( eInflictor, attacker, victim, iDamage, sMeansOfDeath, sWeapon, vDir, sHitLoc, psOffsetTime, deathAnimDuration, isFauxDeath )
{
	...
	if ( (sMeansOfDeath != "MOD_SUICIDE") && ( IsAIGameParticipant( victim ) || IsAIGameParticipant( attacker ) ) )
    {
		killing_entity = maps\mp\bots\_bots_util::bot_get_attacker_entity( attacker, eInflictor );
    	
		if ( isReallyAlive( killing_entity ) && IsAIGameParticipant( victim ) )
        {
			victim BotMemoryEvent( "death", sWeapon, killing_entity.origin, victim.origin, killing_entity );
        }
        
        if ( isReallyAlive( killing_entity ) && IsAIGameParticipant( attacker ) )
        {
	...
	victim maps\mp\_matchdata::logPlayerDeath( self.lifeId, attacker, iDamage, sMeansOfDeath, sWeapon, sPrimaryWeapon, sHitLoc );
	...
	if( maps\mp\killstreaks\_killstreaks::streakTypeResetsOnDeath( victim.streakType ) && !( victim _hasPerk( "specialty_undying_killstreaks" ) ) )
...
#/
	...
	if ( self == victim && IsDefined( victim.partner ) && ( !getGametypeNumLives() || self.pers["lives"] ) )
	{
		self thread waitTimerForSpawn();
		
		self updateSessionState( "spectator" );
		victim.forceSpectatorClient = victim.partner getEntityNumber();
		
		self waittill( "abort_fireteam_spawn" );
	}
...
waitTimerForSpawn()
{
	...
	{
		maps\mp\gametypes\_fire_team::initTeamSpawnElements();
	}
...
getKillcamEntity( attacker, eInflictor, sWeapon ) // self == victim
{
	...
	case "airdrop_trap_explosive_mp":	// airdrop trap
	case "bomb_site_mp":				// bomb sites
	case "orbital_laser_mp":			// orbital laser
	...
	case "sky_laser_projectile_mp":		//this is for the laser in mp_laser
		if( IsDefined( eInflictor.samTurret ) && IsDefined( eInflictor.samTurret.killCamEnt ) )
			return eInflictor.samTurret.killCamEnt;
		break;
...
{	
	...
	if ( isKillstreakWeapon( sWeapon )
	    && ( ( isDefined (eAttacker.perks) && eAttacker _hasPerk( "specialty_greater_killstreaks" ) )
	        || ( isDefined (eAttacker.owner) && isDefined (eAttacker.owner.perks) && eAttacker.owner _hasPerk( "specialty_greater_killstreaks" ) ) ) )
	{
		iDamage *= 2;
	}
	
	if ( victim isBlockingGrenade() )
	{
		iDamage *= GetDvarFloat("player_grenadeBlockDamageTaken", 0.5);
	}
	...
	if ( !(iDFlags & level.iDFLAGS_NO_PROTECTION) )
	{
		...
		else// not hitting teammate
		{
			...
			if ( IsDefined( eAttacker ) && !IsPlayer( eAttacker ) && IsDefined( eAttacker.owner ) )
		...
		if ( IsDefined( damager) && damager != victim && iDamage > 0 && ( !IsDefined( sHitLoc ) || sHitLoc != "shield" ) )
		{
			if( iDFlags & level.iDFLAGS_STUN )
				typeHit = "stun";
			else if( victim isBlockingGrenade() )
				typeHit = "grenadeBlock";
	...
	//{
		if ( iDamage > 0 && IsDefined( eAttacker ) && !victim isUsingRemote() && isPlayer( victim ) )
		{
...
#/
	...
	if ( victim.sessionstate != "dead" )
	{
	    ...
	    ReconSpatialEvent( victim.origin, "script_mp_damage: player_name %s, player %d, player_angles %v, hit_loc %s, attacker_name %s, attacker_pos %v, damage %d, weapon %s, damage_type %s", victim.name, victim.clientid, victim.angles, sHitLoc, attacker_name, attacker_origin, iDamage, sWeapon, sMeansOfDeath );
		#/
...
establishUnresolvedCollisionMonitors()
{
	level endon( "game_ended" );
	...
	while (1)
	{
		...
		player thread monitorForUnresolvedCollisions();
	}
}

monitorForUnresolvedCollisions()
{
	level endon( "game_ended" );
	self endon( "disconnect" );
	...
	while (1)
	{
		...
		/#
		Print("Received unresolved collision with the player, defaulting to suicide.\n");
		#/
