...
/*
	Greed
	Objective: pick up and hold flags to score points.
	Map ends:	time limit or first team to reach score limit.
...
/*QUAKED mp_dom_safe_spawn (1.0 1.0 0.0) (-16 -16 0) (16 16 72)
Players spawnlocked at a single flag have a chance to spawn at one of these positions in addition to the ones near their flag.*/
...
main()
{
	...
	SetDvarIfUninitialized( "scr_greed_point_tick_wait", 5 );
}
...
initializeMatchRules()
{
	...
	SetDynamicDvar( "scr_greed_roundlimit", 1 );
	registerRoundLimitDvar( "greed", 1 );		
	SetDynamicDvar( "scr_greed_winlimit", 1 );
	registerWinLimitDvar( "greed", 1 );			
	SetDynamicDvar( "scr_greed_halftime", 0 );
	registerHalfTimeDvar( "greed", 0 );				

	SetDynamicDvar( "scr_greed_returntime", 30 );
	SetDynamicDvar( "scr_greed_promode", 0 );
}
...
onStartGameType()
{
	...
	setObjectiveText( "allies", &"OBJECTIVES_GREED" );
	setObjectiveText( "axis", &"OBJECTIVES_GREED" );

	if ( level.splitscreen )
	{
		setObjectiveScoreText( "allies", &"OBJECTIVES_GREED" );
		setObjectiveScoreText( "axis", &"OBJECTIVES_GREED" );
	}
	else
	{
		setObjectiveScoreText( "allies", &"OBJECTIVES_GREED_SCORE" );
		setObjectiveScoreText( "axis", &"OBJECTIVES_GREED_SCORE" );
	}
	setObjectiveHintText( "allies", &"OBJECTIVES_GREED_HINT" );
	setObjectiveHintText( "axis", &"OBJECTIVES_GREED_HINT" );
...
//	level.spawn_safe["_b"] = [];
//	level.spawn_safe["_c"] = [];
//	spawn_safe = maps\mp\gametypes\_spawnlogic::getSpawnpointArray( "mp_dom_safe_spawn" );
//	foreach( spawnPoint in spawn_safe )
//	{
//		if ( spawnPoint.target == "mp_dom_a_safe_spawns" )
//			level.spawn_safe["_a"][level.spawn_safe["_a"].size] = spawnpoint;
//		if ( spawnPoint.target == "mp_dom_b_safe_spawns" )
//			level.spawn_safe["_b"][level.spawn_safe["_b"].size] = spawnpoint;
//		if ( spawnPoint.target == "mp_dom_c_safe_spawns" )
//			level.spawn_safe["_c"][level.spawn_safe["_c"].size] = spawnpoint;
//	}
//	
...
//	allowed[1] = "hardpoint";
	...
	maps\mp\gametypes\_rank::registerScoreInfo( "hold_flag", 25 );
		
	thread greedFlags();
	thread updateGreedScores();	
}
...
greedFlags()
{
	...
	for ( index = 0; index < level.flags.size; index++ )
	{
		...
		greedFlag = maps\mp\gametypes\_gameobjects::createCarryObject( "neutral", trigger, visuals, (0,0,85) );
		greedFlag maps\mp\gametypes\_gameobjects::setOwnerTeam( "neutral" );
		//domFlag maps\mp\gametypes\_gameobjects::allowUse( "enemy" );
		greedFlag maps\mp\gametypes\_gameobjects::allowCarry( "any" );
//		domFlag maps\mp\gametypes\_gameobjects::setUseTime( 10.0 );
		greedFlag maps\mp\gametypes\_gameobjects::setUseTime( 0.5 );
		greedFlag maps\mp\gametypes\_gameobjects::setUseText( &"MP_SECURING_POSITION" );
		label = greedFlag maps\mp\gametypes\_gameobjects::getLabel();
		greedFlag.label = label;
		greedFlag maps\mp\gametypes\_gameobjects::set2DIcon( "friendly", "waypoint_captureneutral" + label );
		greedFlag maps\mp\gametypes\_gameobjects::set3DIcon( "friendly", "waypoint_captureneutral" + label );
		greedFlag maps\mp\gametypes\_gameobjects::set2DIcon( "enemy", "waypoint_captureneutral" + label );
		greedFlag maps\mp\gametypes\_gameobjects::set3DIcon( "enemy", "waypoint_captureneutral" + label );
		//domFlag maps\mp\gametypes\_gameobjects::setVisibleTeam( "any" );
		greedFlag maps\mp\gametypes\_gameobjects::setVisibleTeam( "any" );
		
		greedFlag maps\mp\gametypes\_gameobjects::setCarryIcon( level.icon2D["allies"] );
		//domFlag.objIDPingFriendly = true;
		greedFlag.objIDPingEnemy = true;
		greedFlag.allowWeapons = true;
//		domFlag.onUse = ::onUse;
//		domFlag.onBeginUse = ::onBeginUse;
//		domFlag.onUseUpdate = ::onUseUpdate;
//		domFlag.onEndUse = ::onEndUse;
		greedFlag.onPickup = ::onPickup;
		//domFlag.onPickupFailed = ::onPickup;
		greedFlag.onDrop = ::onDrop;
		...
		greedFlag.baseeffectforward = anglesToForward( upangles );
		greedFlag.baseeffectright = anglesToRight( upangles );
		
		greedFlag.baseeffectpos = trace["position"];
		
		// legacy spawn code support
		level.flags[index].useObj = greedFlag;
		level.flags[index].adjflags = [];
		level.flags[index].nearbyspawns = [];
		
		greedFlag.levelFlag = level.flags[index];
		
		level.greedFlags[level.greedFlags.size] = greedFlag;
	}
...
onPickup( player )
{
	...
	self maps\mp\gametypes\_gameobjects::set2DIcon( "enemy", level.iconKill2D );
	self maps\mp\gametypes\_gameobjects::set3DIcon( "enemy", level.iconKill3D );
	self maps\mp\gametypes\_gameobjects::set2DIcon( "friendly", level.iconDefendFlag2D );
	self maps\mp\gametypes\_gameobjects::set3DIcon( "friendly", level.iconDefendFlag3D );
...
//	{	
//		statusDialog( "secured"+self.label, team, true );
//		statusDialog( "lost"+self.label, otherTeam, true );
//	}
	...
	player thread giveFlagBearerPoints();
	//The player has picked up a flag. Don't allow the player to pick up any more.
...
onDrop( player )
{
	...
	self maps\mp\gametypes\_gameobjects::setVisibleTeam( "any" );
	self maps\mp\gametypes\_gameobjects::set2DIcon( "enemy", "waypoint_captureneutral" + label );
	self maps\mp\gametypes\_gameobjects::set3DIcon( "enemy", "waypoint_captureneutral" + label );
	self maps\mp\gametypes\_gameobjects::set2DIcon( "friendly", "waypoint_captureneutral" + label );
	self maps\mp\gametypes\_gameobjects::set3DIcon( "friendly", "waypoint_captureneutral" + label );
...
updateGreedScores()
{
	...
	{
		greedFlags = getOwnedGreedFlags();
		
		if ( greedFlags.size )
		{
			for ( i = 1; i < greedFlags.size; i++ )
			{
				greedFlag = greedFlags[i];
				flagScore = getTime() - greedFlag.captureTime;
				for ( j = i - 1; j >= 0 && flagScore > (getTime() - greedFlags[j].captureTime); j-- )
					greedFlags[j + 1] = greedFlags[j];
				greedFlags[j + 1] = greedFlag;
			}
			
			foreach( greedFlag in greedFlags )
			{
				team = greedFlag maps\mp\gametypes\_gameobjects::getOwnerTeam();
...
//		if ( (((getTimePassed() / 1000) > 120 && greedFlags.size < 2) || ((getTimePassed() / 1000) > 300 && greedFlags.size < 3)) && matchMakingGame() )
//		{
...
{
	greedFlags = [];
	foreach ( greedFlag in level.greedFlags )
	{
		if ( greedFlag maps\mp\gametypes\_gameobjects::getOwnerTeam() != "neutral" && isDefined( greedFlag.captureTime ) )
			greedFlags[greedFlags.size] = greedFlag;
	}
...
getTeamFlagCount( team )
{
	...
	for (i = 0; i < level.flags.size; i++) 
	{
		if ( level.greedFlags[i] maps\mp\gametypes\_gameobjects::getOwnerTeam() == team )
			score++;
	}	
	return score;
}
...
flagSetup()
{
	...
	for (i = 0; i < level.greedFlags.size; i++)
	{
...
/*
///ScriptDocBegin
Name: giveFlagBearerPoints( )
Summary: give player score points over time to flag bearers
Module: greed
CallOn: a player
MandatoryArg: N/A
Example: thread player giveFlagBearerPoints();
SPMP: MP
///ScriptDocEnd
*/
giveFlagBearerPoints()
{
	self endon( "death" );
	level endon( "game_ended" );
	while (1)
	{
		waittime = getDvarInt( "scr_greed_point_tick_wait", 5 );
		wait( waittime );		//wait a couple seconds between giving points.
		self thread maps\mp\gametypes\_rank::giveRankXP( "hold_flag", maps\mp\gametypes\_rank::getScoreInfoValue( "hold_flag" ) * self getCapXPScale() );
		maps\mp\gametypes\_gamescore::givePlayerScore( "hold_flag", self );
	}
}
