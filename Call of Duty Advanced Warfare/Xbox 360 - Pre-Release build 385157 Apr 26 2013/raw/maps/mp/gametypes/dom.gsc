...
getNearestFlagPoint( spawnPoint )
{
	...
	foreach( domPoint in level.domFlags )
	{
		...
		{
			dist = GetPathDist( domPoint.levelflag.origin, spawnPoint.origin, 999999 );
		}
...
domDebug()
{
	...
	while( true )
	{
		...
		while( true )
		{
			...
			// show the nearest dom point to each spawn
			foreach( spawnPoint in spawnPoints )
			{
				line( spawnPoint.nearFlagPoint.origin + heightOffset, spawnPoint.origin + heightOffset, (0.2, 0.2, 0.6) );
			}
			...
			foreach( flagLocation in level.flags )
			{
				if ( flagLocation == level.bestSpawnFlag["allies"] )
					print3d( flagLocation.origin + heightOffset, "allies best spawn flag" );
				
				if ( flagLocation == level.bestSpawnFlag["axis"] )
					print3d( flagLocation.origin + heightOffset, "axis best spawn flag" );
			}
