...
onStartGameType()
{
	...
	if ( level.splitscreen )
		level.grnd_timerDisplay setPoint( "TOPLEFT", "TOPLEFT", 80, 0 );
	else
...
setPlayerMessages( player )
{
	...
	if ( level.splitscreen )
		player.grndHUDText setPoint( "TOP LEFT", "TOP LEFT", 80, 18 );
	else
		player.grndHUDText setPoint( "TOP LEFT", "TOP LEFT", 115, 22 );	
	player.grndHUDText.alpha = 1;
	player.grndHUDText.archived = false;
...
getNextZone()
{
	...
	if ( level.grnd_zones.size > 2 )
	{
		...
		else if ( farZones.size )
			zone = farZones[0];	
		else
			zone = closeZones[ randomIntRange( 0, closeZones.size ) ];
		index = zone.index;		
	}
...
locationScoring()
{
	...
	while( true )
	{
		...
		foreach( player in level.players )
		{
			if ( isDefined( player.inGrindZone ) && isReallyAlive( player ) && player.pers["team"] != "spectator" && distance2D( level.grnd_zone.origin, player.origin ) < GRND_ZONE_TOUCH_RADIUS )
			{
