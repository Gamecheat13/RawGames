...
updateMatchBonusScores( winner )
{
	...
	if ( level.teamBased )
	{
		...
		foreach ( player in level.players )
		{
			...
			{
				scale = winnerScale;
				assert( !isdefined( player.didTie ) );
				player.didTie = true;
			...
			playerScore = int( (scale * ((gameLength/60) * spm)) * (player.timePlayed["total"] / gameLength) );
	...
	maps\mp\_braggingrights::resolveBraggingRights();
	...
	foreach ( player in level.players )
	{
		...
		if( !IsDefined( player.braggingRightsLoser ) || !player.braggingRightsLoser )
...
Callback_StartGameType()
{
	...
	if ( !isDefined( game["gamestarted"] ) )
	{
		...
		if ( (!isDefined( axisCharSet ) || axisCharSet == "") )
		{
			if ( !isDefined( game["axis"] ) )
				axisCharSet = "opforce_urban";
			else
				axisCharSet = game["axis"];
		}

		//setup multi team faction, for now just use allies default if available other wise use delta_multicam for all multi-team teams
		if( level.multiTeamBased )
		{
		...
		game["strings"]["S.A.S Win"] = &"MP_SAS_WIN";
		game["strings"]["Spetsnaz Win"] = &"MP_SPETSNAZ_WIN";
	...
	thread maps\mp\gametypes\_eyes_on_minimap::init();
	maps\mp\_threatdetection::init();
...
freezeAllPlayers( delay, additionalClientDvarName1, additionalClientDvarValue1 ) 
{
	...
	foreach ( player in level.players )
	{
		...
		if ( isdefined(additionalClientDvarName1) )
			player setClientDvars( "cg_everyoneHearsEveryone", 1, 
								   "cg_drawSpectatorMessages", 0,
									additionalClientDvarName1, additionalClientDvarValue1 );
		else
			player setClientDvars( "cg_everyoneHearsEveryone", 1, 
								  "cg_drawSpectatorMessages", 0 );
...
endGame( winner, endReasonText, nukeDetonated )
{
	...
	// now using the loot service
	//maps\mp\loot\_loot::GenerateLoot();
...
setWeaponStat( name, incValue, statName )
{
	...
	if( statName == "deaths" )
	{
		...
		tokens = getWeaponNameTokens( name );
