...
onPlayerSpawned()
{
	...
	for(;;)
	{
		...
		self notify ("destroy_heat_ui");
		if ( weaponUsesHeat( self.currentWeaponAtSpawn ) )
			self thread displayHeatUI();
...
displayHeatUI()
{
	self endon("death");
	self endon("disconnect");
	self endon("kill_heat_ui");
	level endon("game_ended");
	
	self childthread destroyHeatUI();
	
	weapon = self getCurrentWeapon();
	
	if ( !IsDefined( self.hud_heat_meter ) )
	{
		self.hud_heat_meter = NewClientHudElem(self);
		self.hud_heat_meter.x = 10;
		self.hud_heat_meter.y = 200;
		self.hud_heat_meter.sort = 6;
		self.hud_heat_meter.horzalign = "left";
		self.hud_heat_meter.vertalign = "top";
		self.hud_heat_meter SetShader( "hudcolorbar", 20, 100 );
	}

	while ( true )
	{
		self.hud_heat_meter.y = 300 - ( 100 * self getWeaponHeatLevel( weapon ) );
		wait( 0.05 );		
	}
}
...
destroyHeatUI()
{
	self waittill("destroy_heat_ui");
	
	self.hud_heat_meter Destroy();
	self notify("kill_heat_ui");
}
...
//		else if ( isLootWeapon(weaponTokens) )
//			weaponName = weaponTokens[0] + "_" + weaponTokens[1] + "_mp";
...
handleScavengerBagPickup( scrPlayer )
{
	...
	//scavenger stack levels
	//20% more bullet pickups
	//40% more bullet pickups, explosive ammo pickups
	//60% more bullet pickups, tactical resupply
	//80% more bullet pickups, lethal resupply
	//100% more bullet pickups
	//120% more bullet pickups
	
	//Giving the player extra lethal and tactical 
	...
	{	
		...
		if( maps\mp\gametypes\_class::isValidOffhand( offhand ) && destPlayer _hasPerk( "specialty_tacticalresupply" ) 
		   || maps\mp\gametypes\_class::isValidEquipment( offhand ) && destPlayer _hasPerk( "specialty_lethalresupply" ) )
		{
			destPlayer SetWeaponAmmoClip( offhand, currentClipAmmo + 1 );
		}
	}

	//Give the player more ammo for their weapons
	...
	foreach ( primary in primaryWeapons )
	{
		...
		if ( getWeaponClass( primary )== "weapon_projectile" && destPlayer _hasPerk( "specialty_explosiveammoresupply" )  )
		{
			//handle launchers, since they can't have a percentage ammo
			addStockAmmo = weaponClipSize( primary );
		}
		else if( isSubStr( primary, "alt" ) && isSubStr( primary, "gl" ) && destPlayer _hasPerk( "specialty_explosiveammoresupply" ) )
		{
			//handle gl attachments
			addStockAmmo = weaponClipSize( primary );
		}
		else
		{
			//handle normal bullet weapons
			maxAmmo = WeaponMaxAmmo( primary );
			
			//percentage of total ammo to resupply off one pickup.
...
detection_grenade_think( position, owner )
{
	...
	thread maps\mp\_threatdetection::detection_grenade_hud_effect( owner, position, 1.75, 1000 );
	thread maps\mp\_threatdetection::detection_highlight_hud_effect( owner, 5 );

	foreach( guy in level.players )
	{
		if ( !IsDefined( guy ) || !IsAlive( guy ) || owner.team == guy.team )
		{
			continue;
		}
		
		if( Distance( guy.origin, position ) < 500 )
		{
			guy maps\mp\_threatdetection::addThreatEvent( [owner], 5, "PAINT_GRENADE", true, false );
...
doNineBang( pos, attacker ) // self == level
{
	level endon( "game_ended" );
...
watchClaymores()
{
	...
	while ( 1 )
	{
		...
		if ( weapname == "claymore" || weapname == "claymore_mp" )
		{
			...
			{
				//no longer detonating since the player can have more than 1
...
equipmentWatchUse( owner )
{
	...
	for ( ;; )
	{
		...
		if ( ammoStock < WeaponMaxAmmo( self.weaponName ) )
		{
			...
			self.trigger delete();
			self delete();
			self notify( "death" ); // this implicitly kills this thread
		}
	}
}
...
{		
	...
	self setMoveSpeedScale( normalizedWeaponSpeed * self.moveSpeedScaler ); //globally increasing move speed
}
...
spawnMine( origin, owner, type, angles )
{
	...
	mine thread createBombSquadModel( "projectile_bouncing_betty_grenade_bombsquad", "tag_origin", owner ); // may have issues with team and owner here	mine thread mineBeacon();
...
mineWatchOwner( mine ) // self == owner
{
	mine endon( "death" );
	level endon ( "game_ended" );
