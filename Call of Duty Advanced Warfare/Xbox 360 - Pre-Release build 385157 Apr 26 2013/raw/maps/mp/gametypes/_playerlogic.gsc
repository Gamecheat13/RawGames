...
spawnPlayer( fauxSpawn )
{
	...
	#/
   if ( isDefined( self.setSpawnPoint ) && ( isDefined( self.setSpawnPoint.notTI ) || self tiValidationCheck() ) )
	{
	...
	self SetClientDvar( "ui_obit_on", true );
	...
	if( getIntProperty( "scr_showperksonspawn", 1 ) == 1 && game["state"] != "postgame" )
	{
		...
		self notify("perks_clean_hiders");	// if we start another timeout thread, kill any that already exist
...
ReconSpawn()
{
	reconRevision_spawnpoints = 2; // use this to separate data in the Recon table if you change what some columns mean.
	...
	if (isdefined(spawnpoints))
	{
		...
		{
			selected = 0;
			lastMinuteSightTracesFailed = 0;
			if (spawnpoint == self.resultspawnpoint)
			{
				selected = self.resultCode;
				lastMinuteSightTracesFailed = self.lastMinuteSightTracesFailed;
			}
			predicted = 0;
			if ( isdefined( self.predictedSpawnPoint ) && spawnpoint == self.predictedSpawnPoint)
				predicted = 1;
			lastupdatetime = spawnpoint.lastupdatetime;
			if (!isdefined(lastupdatetime))
				lastupdatetime = 0;
			weight = spawnpoint.totalScore;
			if (!isdefined(weight))
				weight = 0;
			weight_allyDistance = spawnpoint.weight_allyDistance;
			if (!isdefined(weight_allyDistance))
				weight_allyDistance = 0;
			weight_enemyDistance = spawnpoint.weight_enemyDistance;
			if (!isdefined(weight_enemyDistance))
				weight_enemyDistance = 0;
			weight_avoidDeathLoc = spawnpoint.weight_avoidDeathLoc;
			if (!isdefined(weight_avoidDeathLoc))
				weight_avoidDeathLoc = 0;
			weight_avoidKillerLoc = spawnpoint.weight_avoidKillerLoc;
			if (!isdefined(weight_avoidKillerLoc))
				weight_avoidKillerLoc = 0;
			weight_avoidSameSpawn = spawnpoint.weight_avoidSameSpawn;
			if (!isdefined(weight_avoidSameSpawn))
				weight_avoidSameSpawn = 0;
			weight_domPoints = spawnpoint.weight_domPoints;
			if (!isdefined(weight_domPoints))
				weight_domPoints = 0;
			criticalBitfield = spawnpoint.debugCriticalBitfield;
			if (!isdefined(criticalBitfield))
				criticalBitfield = 0;
			if ( level.teambased )
			{
				sights_friendly = undefined;
				sights_enemy = undefined;
				if (isdefined(spawnpoint.sights))
				{
					sights_friendly = spawnpoint.sights[team];
					sights_enemy = spawnpoint.sights[otherTeam];
				}
				if (!isdefined(sights_friendly))
					sights_friendly = 0;
				if (!isdefined(sights_enemy))
					sights_enemy = 0;
				ReconSpatialEvent( spawnpoint.origin, "script_mp_spawnpoints: life_id %d, revision %d, player_name %s, player %d, type %s, lastupdate %d, predicted %d, selected %d, sights_allies %f, sights_axis %f, weight %f, weight_allyDistance %f, weight_enemyDistance %f, weight_avoidDeathLoc %f, weight_avoidKillerLoc %f, weight_avoidLastSpawn %f, weight_domPoints %f, criticalFactors %d, lastMinuteSightTracesFailed %d",
								  self.lifeId, reconRevision_spawnpoints, self.name, self.clientid, "nearteam", lastupdatetime, predicted, selected, sights_friendly, sights_enemy, weight, weight_allyDistance, weight_enemyDistance, weight_avoidDeathLoc, weight_avoidKillerLoc, weight_avoidSameSpawn, weight_domPoints, criticalBitfield, lastMinuteSightTracesFailed );
			}
			else
			{
				sights = spawnpoint.sights;
				if (!isdefined(sights))
					sights = 0;
				ReconSpatialEvent( spawnpoint.origin, "script_mp_spawnpoints: life_id %d, revision %d, player_name %s, player %d, type %s, lastupdate %d, predicted %d, selected %d, sights %f, weight %f",
								  self.lifeId, reconRevision_spawnpoints, self.name, self.clientid, "nearteam", lastupdatetime, predicted, selected, sights, weight );
			}
		}
		self.resultspawnpoint = undefined;
		if (GetDvarInt("scr_minimapspawndebug") > 0)
		{	// make temp structs for each spawnpoint so we have a record for the minimap draw
			level.minimap_spawnpoints = [];	// this should free any previously allocated spawnpoint structs
			foreach (spawnpoint in spawnpoints)
			{
				sp = spawnstruct();
				if (isdefined(spawnpoint.weight))
					sp.weight = spawnpoint.weight;
				else
					sp.weight = 1;
				sp.sights = spawnpoint.sights;
				sp.origin = spawnpoint.origin;
				sp.ptr = spawnpoint;
				level.minimap_spawnpoints[level.minimap_spawnpoints.size] = sp;
			}
		}
	}
	spawnpoints = self.randomspawnpoints;
	self.randomspawnpoints = undefined;
	if (isdefined(spawnpoints))
	{
		foreach (spawnpoint in spawnpoints)
		{
			...
			if (!isdefined(lastupdatetime))
				lastupdatetime = 0;
			assert(isdefined(spawnpoint.sights));
			ReconSpatialEvent( spawnpoint.origin, "script_mp_spawnpoints: life_id %d, revision %d, player_name %s, player %d, type %s, lastupdate %d, predicted %d, weight %f", 
							  self.lifeId, reconRevision_spawnpoints, self.name, self.clientid, "random", lastupdatetime, predicted, spawnpoint.totalScore );
		}
		if (GetDvarInt("scr_minimapspawndebug") > 0)
		{	// make temp structs for each spawnpoint so we have a record for the minimap draw
			level.minimap_spawnpoints = [];	// this should free any previously allocated spawnpoint structs
	...
	{
		ReconSpatialEvent( player.origin, "script_mp_spawnpos: life_id %d, player_name %s, player %d, team %s",
						  self.lifeId, player.name, player.clientid, player.team );
	}
}
...
spawnIntermission()
{
	...
	if ( level.rankedMatch && ( self.postGamePromotion || ( IsDefined( self.pers["postGameChallenges"] ) && self.pers["postGameChallenges"] ) ) )
	{
		...
		postGameChallenges = 3;
		if( IsDefined( self.pers["postGameChallenges"] ) )
			postGameChallenges = self.pers["postGameChallenges"];
		waitTime = 4.0 + min( postGameChallenges, 3 );		
		while ( waitTime )
		{
			wait ( 0.25 );
			waitTime -= 0.25;
...
Callback_PlayerConnect()
{
	...
	logPrint("J;" + self.guid + ";" + self getEntityNumber() + ";" + self.name + ";" + self.clientid + ";" + self.xuid + "\n");
	...
	if ( game["clientid"] <= 30 && game["clientid"] != getMatchData( "playerCount" ) )
	{
		...
		setMatchData( "players", self.clientid, "xuid", self.xuid );
