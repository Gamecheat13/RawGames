...
addAttachments( weaponName )
{
	...
	fullWeaponName = buildWeaponName( weaponName, attach1, attach2, "none", 0, 0 );
...
initGunHUD()
{
	if ( level.splitscreen )
	{	
		self.gun_progressDisplay[0] = self createFontString( "small", 1.2 );	
		self.gun_progressDisplay[0] setPoint( "TOP LEFT", "TOP LEFT", 80, 0 );
		...
		self.gun_progressDisplay[1] = self createFontString( "small", 1.2 );		
		self.gun_progressDisplay[1] setPoint( "TOP LEFT", "TOP LEFT", 80, 12 );
	...
	else
	{
		...
		self.gun_progressDisplay[1] = self createFontString( "small", 1.2 );		
		self.gun_progressDisplay[1] setPoint( "TOP LEFT", "TOP LEFT", 115, 17 );
		self.gun_progressDisplay[1] setText( "1 / " + level.gun_guns.size );
		self.gun_progressDisplay[1].alpha = 1;
		self.gun_progressDisplay[1].archived = false;
		self.gun_progressDisplay[1].hideWhenInMenu = true;		
...
hideInKillCam()
{
	...
	while ( true )
	{
		if ( showing && ( !isAlive( self ) || isInKillCam() ) )
		{
			self.gun_progressDisplay[0].alpha = 0;
			self.gun_progressDisplay[1].alpha = 0;
			showing = false;
		}
		else if ( !showing && ( isAlive( self ) && !isInKillCam() ) )
		{
			self.gun_progressDisplay[0].alpha = 1;
			self.gun_progressDisplay[1].alpha = 1;
			showing = true;
...
{	
	...
	if ( numGuns )
	{
		for ( i=0; i<numGuns; i++ )
		{
			...
			if( level.gun_guns[i] == "none" )
			{
				level.gun_guns[i] = "iw5_mp5";
			}
		}
	}
	else
...
{	
	...
	level.gun_loadouts["axis"]["loadoutPerks"] = maps\mp\perks\_perks::getEmptyPerks();
