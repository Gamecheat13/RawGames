...
_setPerk( perkName, useSlot, setSlot )
{
	...
	if (isdefined(setSlot))
		self setPerk( perkName, !IsDefined( level.scriptPerks[perkName] ), useSlot, setSlot );
	else
...
{	
	...
	if ( IsDefined( level.killstreakChainingWeapons[weapon] ) && self _hasPerk( "specialty_killstreaks_chain" ) )
	{
		//	only if it came from an earn slot (1-4, 0 is gimme slot, 5 or more is stacked in the gimme slot)
...
{
	return IsSubStr( weaponName, "loot" );
}
...
============= 
///ScriptDocBegin
"Name: getDefaultAttachment( <weaponName> )"
"Summary: Returns default weapon attachment name if a default has been set in the statstable.csv"
"Module: Utility"
"CallOn: None"
"MandatoryArg: <weaponName> The name of the weapon to check."
"Example: default_attachment = getDefaultAttachment( <sWeaponName> );
"SPMP: multiplayer"
///ScriptDocEnd
============= 
*/
getDefaultAttachment( weaponName ) 
{
	row_num = TableLookupRowNum( "mp/statstable.csv", 4, weaponName );
	attachment_name = "";
	default_attachment = TableLookupByRow( "mp/statstable.csv", row_num, 10 );
	if( default_attachment != "" )
	{
		attachment_name = TableLookupByRow( "mp/statstable.csv", row_num, 11 );
	}
...
lightWeightScalar( power )
{
	...
	if (isdefined( power ))
	{
		...
		if (isdefined( self.activemovescalar ))
			val = self.activemovescalar - 1;
		else
...
GetMatchRulesSpecialClass( team, index )
{
	...
	loadoutStreakType = getMatchRulesData( "defaultClasses", team, index, "class", "streaktype" );
	if ( loadoutStreakType != "specialty_null" )
	{
		...
		class["loadoutKillstreak4"] = "none"; // assuming default classes cannot have the extra killstreak
	}
...
updateSessionState( sessionState, statusIcon )
{
	...
	self SetClientDvar( "ui_session_state", sessionState );
}
