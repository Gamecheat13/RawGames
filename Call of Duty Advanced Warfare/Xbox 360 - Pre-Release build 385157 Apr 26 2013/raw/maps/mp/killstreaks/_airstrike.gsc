...
init()
{
	...
	level.stealthbombfx = loadfx ("fx/explosions/stealth_bomb_mp");
	...
	level.dangerMaxRadius["siege_airstrike"] = 100;
	level.dangerMinRadius["siege_airstrike"] = 50;
	level.dangerForwardPush["siege_airstrike"] = 1;
	level.dangerOvalScale["siege_airstrike"] = 6.0;
...
#/
	...
	else if ( streakName == "siege_airstrike" )
	{
		//	override coord and direction and drop one on each airdrop spot
		direction = (0,randomInt(360),0);
		
		for ( i=0; i<level.siege_drops.size; i++ )
		{		
			if ( i )
				wait randomfloatrange( 1.5, 2.5 );
			
			vec = vectorNormalize( level.siege_drops[i].origin - level.siege_target.targets[0].origin );
			coord = level.siege_drops[i].origin + vec * 200;			
			
			flightPath = getFlightPath( coord, direction, planeHalfDistance, heightEnt, planeFlyHeight, planeFlySpeed, planeBombExplodeDistance, "precision_airstrike" );
			
			level thread doPlaneStrike( lifeId, owner, requiredDeathCount, coord, flightPath["startPoint"]+(0,0,randomInt(500)), flightPath["endPoint"]+(0,0,randomInt(500)), flightPath["bombTime"], flightPath["flyTime"], direction, "precision_airstrike" );
		}
	}	
