...
init()
{
	...
	precacheShader( "specialty_siege_resupply_crate" );
	...
	game["strings"]["siege_resupply_hint"] = &"MP_SIEGE_RESUPPLY";
	
	// Blacksmith Killstreak Strings
	game["strings"]["orbital_laser_hint"] = &"MP_ORBITAL_LASER_PICKUP";
	game["strings"]["warbird_hint"] = &"MP_WARBIRD_PICKUP";
	...
	addCrateType(	"airdrop_assault",	"orbital_laser",			4,		::killstreakCrateThink );
	...
	addCrateType(	"airdrop_assault",	"warbird",					3,		::killstreakCrateThink );
...
//	addCrateType(	"airdrop_assault",	"airdrop_juggernaut",		1,		::juggernautCrateThink );
	...
	addCrateType(	"airdrop_osprey_gunner",	"orbital_laser",			5,		::killstreakCrateThink );
	...
	addCrateType(	"airdrop_osprey_gunner",	"warbird",					2,		::killstreakCrateThink );
...
//	addCrateType(	"airdrop_osprey_gunner",	"airdrop_juggernaut",		1,		::juggernautCrateThink );
	...
	addCrateType(	"airdrop_trapcontents",	"orbital_laser",		8,		::trapNullFunc );	
	...
	addCrateType(	"airdrop_trapcontents",	"warbird",				9,		::trapNullFunc );
	...
	// SIEGE DROP			
	addCrateType(	"airdrop_siege",	"supply",			100,		::siegeCrateThink );
	...
	tdmSpawns = getSpawnArray( "mp_tdm_spawn" , "classname" );
	lowSpawn = undefined;
		...
		if( level.multiTeamBased )
		{
			foreach( teamName in level.teamNameList )
			{
				if( teamName != self.team )
					thread leaderDialog( teamName + "_enemy_airdrop_assault_inbound", teamName );
			}
		}
...
watchAirDropWeaponChange( lifeId, kID, dropType )
{
	...
	if( IsDefined( self.threwAirDropMarker ) )
	{
		...
		killstreakWeapon = maps\mp\killstreaks\_killstreaks::getKillstreakWeapon( self.pers["killstreaks"][self.threwAirDropMarkerIndex].streakName );
...
trap_createBombSquadModel() // self == crate
{
	...
	bombSquadModel setModel( "com_plasticcase_trap_bombsquad" );
...
{	
	...
	{	
		if( level.multiTeamBased )
			self thread crateUseTeamUpdater_multiTeams( self.team);
		else
...
Callback_VehicleDamage( inflictor, attacker, damage, iDFlags, meansOfDeath, weapon, point, dir, hitLoc, timeOffset, modelIndex, partName )
{
	...
	if( IsDefined( weapon ) )
	{
		...
		case "sky_laser_projectile_mp":	//laser in mp_laser
			self.largeProjectileDamage = true;		
			modifiedDamage = self.maxHealth * 0.5; // takes about 1 burst of sam rockets
			break;
...
siegeCrateThink( dropType )
{
	self endon( "death" );
	
	level.siege_crates[level.siege_crates.size] = self;
	
	crateSetupForUse( game["strings"]["siege_resupply_hint"], "all", "specialty_siege_resupply_crate" );
	...
	for ( ;; )
	{
		...
		if ( isDefined( self.owner ) && player != self.owner )
		{
			...
			{
				maps\mp\gametypes\_gamescore::givePlayerScore( "siege_sharepackage", self.owner, undefined, true, true );			
				self.owner thread maps\mp\gametypes\_rank::giveRankXP( "siege_sharepackage" );
				self.owner thread maps\mp\gametypes\_rank::xpEventPopup( &"SPLASHES_SHAREPACKAGE" );
			}
		}		
	
		if ( !isDefined( player.siege_resupplyTime ) || ( getTime() - player.siege_resupplyTime ) > 15000 )
		{
			//	track it
			player.siege_resupplyTime = getTime();
			
			//	sound
			player playLocalSound( "ammo_crate_use" );
			
			//	armor
			if ( !isDefined( player.hasLightArmor ) )
				player thread maps\mp\perks\_perkfunctions::giveLightArmor();			
...
deleteCrate()
{
	...
	if ( isDefined( self.dropType ) )
	{
		...
		if ( self.dropType == "airdrop_siege" )
		{
			remainingCrates = [];
			foreach ( crate in level.siege_crates )
			{
				if ( crate != self )
					remainingCrates[remainingCrates.size] = crate;
			}
			level.siege_crates = remainingCrates;
		}
	}
