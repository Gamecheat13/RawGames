...
beginSiegeHarrier()
{
	pathGoal = level.siege_target.targets[0].origin;
	heightEnt = GetEnt( "airstrikeheight", "targetname" );	
	if ( isDefined( heightEnt ) )
		trueHeight = heightEnt.origin[2];
	else if( isDefined( level.airstrikeHeightScale ) )
		...
		trueHeight = 850;	
	pathGoal *= (1,1,0);
	pathGoal = pathGoal + (0,0, trueHeight );	
	
	startPoint = pathGoal + ( AnglesToForward( (0,randomInt(360),0) ) * ( -1 * 15000 ) );

	harrier = self spawnDefensiveHarrier( self.pers["deaths"], self, startPoint, pathGoal );
	harrier.pathGoal = pathGoal;
