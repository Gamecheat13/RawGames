...
init()
{
	...
	level._effect[ "box_explode_mp" ]	= loadfx( "fx/fire/ballistic_vest_death" );
}
...
takeWeaponOnStuck( weap, weapName )
{
	...
	weap playSoundToPlayer( "mp_vest_deployed_ui", self );
	...
	while( true )
	{
		...
		if( self.damageTaken >= self.maxHealth )
		{
			...
			if( IsDefined( self.owner ) )
				self.owner thread leaderDialogOnPlayer( level.boxSettings[ self.boxType ].voDestroyed, undefined, undefined, self.origin );
