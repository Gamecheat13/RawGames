...
init()
{
	...
	makeHeliType( "warbird", "fx/explosions/helicopter_explosion_pavelow", ::defaultLightFX );
	addAirExplosion( "warbird", "fx/explosions/helicopter_explosion_pavelow" );
...
heli_think( lifeId, owner, startNode, heli_team, heliType )
{
	...
	chopper maps\mp\_threatdetection::showInThreatDetection();
...
heli_damage_monitor( type )
{
	...
	for( ;; )
	{
		...
		if( IsDefined( weapon ) )
		{
			...
			case "sky_laser_projectile_mp":		//laser weapon for mp_laser. Should act the same as sam turret
				self.largeProjectileDamage = true;		
				switch( self.heliType )
				{
				case "flares":
...
attack_primary()
{
	self notify("attack_primary");
	self endon( "death" );
	self endon( "crashing" );
	self endon( "leaving" );
	self endon("attack_primary");
	
	DistanceToMoveFromTarget = 10;
	...
	while ( 1 )
	{
		...
		endOfAttackPosition += (0,0,-100);
		
		//Begin strafe attack
		self SetMaxPitchRoll(40, 30);
		self _setVehGoalPos( endOfAttackPosition );
