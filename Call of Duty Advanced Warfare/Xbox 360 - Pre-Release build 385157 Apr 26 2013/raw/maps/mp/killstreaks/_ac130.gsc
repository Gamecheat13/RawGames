...
init()
{
	...
	SetDvar( "ui_ac130usetime", level.ac130_use_duration );
	...
	level._effect[ "ac130_flare" ] 				= loadfx( "vfx/lensflare/flares_cobra" );
	level._effect[ "ac130_light_red" ] 			= loadfx( "vfx/lights/aircraft_light_wingtip_red" );
	level._effect[ "ac130_light_white_blink" ] 	= loadfx( "vfx/lights/aircraft_light_white_blink" );
	level._effect[ "ac130_light_red_blink" ]	= loadfx( "vfx/lights/aircraft_light_red_blink" );
...
setAC130Player( player )
{
	...
	level.ac130.planeModel  maps\mp\_threatdetection::showInThreatDetection( player.team, level, "ac130player_removed" );
				...
				if ( vectorDot( currVecToTarget, missileGroup[ i ].lastVecToTarget ) < 0 )
				{
					if ( isDefined( level.ac130player ) )
					{
						...
						if ( level.teambased )
						{
							if( level.ac130player.team != missileTeam )
							{
								if( isDefined( missileGroup[ i ].samturret.sentrytype ) && missileGroup[ i ].samturret.sentrytype == "sky_laser_turret" )
								{
									RadiusDamage( missileGroup[ i ].origin, 1000, 1000, 1000, player, "MOD_EXPLOSIVE", "sky_laser_projectile_mp" );
								}
								else
								{
