...
// custom unload function specified by a rider
guy_unload_custom( guy, pos )
{
	Assert( IsDefined( guy.customUnloadFunc ) );
    ...
    // custom unload logic and animations played
    success = guy [[ guy.customUnloadFunc ]]( self, pos );
    if ( !success )
    {
    	thread guy_idle( guy, pos );
    }
...
guy_unload( guy, pos )
{
	// totally override the guy_unload function to be something custom.  the guy_unload has become overly complex and ideally the complexity should
	//   be pushed down into the specific vehicle unloads, whereas the guy_unload should just be a simple unload animation playing (IMHO).
	if ( IsDefined( guy.customUnloadFunc ) )
	{
		guy_unload_custom( guy, pos );
		return;
	}
...
guy_vehicle_death( guy, attacker, type )
{
    ...
    if ( IsDefined( guy ) )
    {
        ...
        /#
        	AssertEx( !IsDefined( guy.magic_bullet_shield ) || IsDefined( guy.deletable_magic_bullet_shield ), "Vehicle script is trying to delete magic bullet shield guy " + guy GetEntNum() );
        #/
...
set_pos( guy, maxpos )
{
    ...
    Assert( IsDefined( self.usedPositions ) );
...
convert_guy_to_drone( guy, bKeepguy, bKeepWeapons )
{
    if ( !IsDefined( bKeepguy ) )
		bKeepguy = false;
    if ( !IsDefined( bKeepWeapons ) )
		bKeepWeapons = true;
	...
	// if not keeping weapons, only transfer the first attachment (the head, hopefully)
	for ( i = 0; i < size && (bKeepWeapons || i < 1); i++ )
    {
