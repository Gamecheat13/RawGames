...
ai_wave_spawn( spawners, num_guys )
{
	
	assertex ( isdefined ( spawners ) && isstring ( spawners ), "No spawners defined, or you didn't give me a valid targetname" );
	//assertex ( isdefined ( num_guys ), "You either didn't tell me how many guys to spawn, or you gave me something that's not an integer" );
	
    spawners = getentarray ( spawners, "targetname" );
    wave_guys = [];
    
    assertex ( spawners.size >= num_guys, "You're trying to spawn more guys than you have spawners.  You need more spawners" );
    
    if ( !isdefined ( num_guys ) )
    	num_guys = spawners.size;
...
ai_wave_setgoalvolume( wave_guys, volume )
{
	assertex ( isdefined ( volume ) && isstring ( volume ), "No goal volumes defined, or you didn't specify a valid targetname" );
	
	volume = getent ( volume, "targetname" );
	
	assertex ( isdefined ( volume ), "I didn't get any volumes with the specified targetname.  Check your volume targetnames" );
	
	wait 0.05;
	
	foreach ( guy in wave_guys )
	{
...
ai_wave_monitor_threshold( wave_guys, threshold, message )
{
    level endon ( message );
    
    assertex ( isdefined ( message ) && isstring ( message ), "You either didn't specify a threshold message or you gave me something that's not a string" );
    assertex ( isdefined ( threshold ), "You either didn't specify a threshold, or you gave something that's not an integer" );
    
    while ( true )
    {
        wave_guys = array_removedead ( wave_guys );
        if ( wave_guys.size <= threshold )
...
ai_wave_monitor_retreat( wave_guys, message, retreat_volume )
{
	assertex ( isdefined ( retreat_volume ), "You specified a retreat message, but not a retreat volume." );
	assertex ( isdefined ( message ) && isstring ( message ), "You either didn't specify a retreat message or you gave me something that's not a string" );
    	
	level waittill ( message );
    
	array_setgoalvolume ( wave_guys, retreat_volume );
}

ai_wave_spawn_volume_threshold_retreat( spawners, num_guys, volume, threshold, message, retreat_volume )
{
	          
	wave_guys = ai_wave_spawn( spawners, num_guys );
...
ai_wave_spawn_volume_threshold( spawners, num_guys, volume, threshold, message, retreat_volume )
{
...
ai_wave_spawn_volume( spawners, num_guys, volume )
{
