...
newfallback_overmind( num, group )
{
	...
	{
		if ( ( isdefined( node.script_fallback ) ) && ( node.script_fallback == num ) )
			fallback_nodes = add_to_array( fallback_nodes, node );
	}
...
flood_spawner_think( trigger )
{
	...
	while ( self.count > 0 )
	{
		...
		if ( IsDefined( soldier ) && !IsDefined( soldier.targetname ) )
		{
			if ( IsDefined( self.targetname ) )
			{
				soldier.targetname = self.targetname + "_AI";
			}
		}
...
// Use 'script_wait' and associated keys to control the wait times between waves
pyramid_spawn( trigger )
{
	self endon( "death" );
	...
	currentSpawners = array_randomize(spawners);
	while ( currentSpawners.size > 0 )
	{
		if ( self.count <= 0 )
			return;

		spawner = array_first(currentSpawners);
...
// 			assertEx( 0, "Initial spawning from spawner at " + self.origin + " failed." );
		...
		currentSpawners = array_remove(currentSpawners, spawner);
	...
	while ( self.count > 0 )
	{
		...
		while ( currentSpawners.size > 0 )
		{
			waitframe(); // can only spawn on AI per frame
			...
			currentSpawners = array_removeUndefined( currentSpawners );
