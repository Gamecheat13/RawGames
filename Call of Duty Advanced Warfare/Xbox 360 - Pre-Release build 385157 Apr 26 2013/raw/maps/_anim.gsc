...
anim_custom_animmode_solo( guy, custom_animmode, anime, tag )
{
 ...
 ============= 
///ScriptDocBegin
"Name: anim_first_frame_solo( <guy> , <scene> , <tag>, <animname_override> )"
"Summary: Puts the animating model or AI or vehicle into the first frame of the animated scene. The animation is played relative to the entity that calls the scene"
"Module: Anim"
"CallOn: The root entity that is the point of relativity for the scene, be it node, ai, vehicle, etc."
...
"OptionalArg: <override anim name> : Name to use to look up the animation, instead of guy.animname."
...
anim_first_frame_solo( guy, anime, tag, animname_override )
{
	array = get_anim_position( tag );
	org = array[ "origin" ];
	angles = array[ "angles" ];
	thread anim_first_frame_on_guy( guy, anime, org, angles, animname_override );
}
...
anim_handle_notetrack( scr_notetrack, guy, dialogue_array, tag_owner )
{
	...
	if ( IsDefined( scr_notetrack[ "lui_notify" ] ) )
	{
		LUINotifyEvent( scr_notetrack[ "lui_notify" ], 0 );
	}    	
...
begin_anim_reach( ent, startOrg, start_func, end_func )
{
	...
	// verify that goal position is where they were sent (otherwise will cause a teleport when anim_single used)
	/#
	distance_delta = DistanceSquared( self.origin, startorg );
	AssertEx( (distance_delta < (50 * 50)), "Entity too far away from anim_reach goal, will cause a teleport when animation played. Make sure entity not given another goal during anim_reach." );
	#/
...
=============
///ScriptDocBegin
"Name: anim_loop_solo( <guy> , <anime> , <ender> , <tag>, <animname_override> )"
...
"OptionalArg: <ender>: The loops will stop reoccurring if this is notified to the root entity."
"OptionalArg: <tag>: An optional tag that the scene is relative to"
"OptionalArg: <animname_override>: Override the anim name."
"Example: node thread anim_loop_solo( guy, "wounded_idle" );"
///ScriptDocEnd
=============
*/

anim_loop_solo( guy, anime, ender, tag, animname_override )
{
	self endon( "death" );
	guy endon( "death" );
...
=============
///ScriptDocBegin
"Name: addNotetrack_lui_notify( <animname> , <notetrack> , <theNotify> , <anime> )"
"Summary: Associates a notetrack to send a notify to lui."
"Module: Anim"
"CallOn: An entity"
"MandatoryArg: <animname>: "
"MandatoryArg: <notetrack>: "
"MandatoryArg: <theNotify>: "
"MandatoryArg: <anime>: The animation scene as defined in the map_anim.gsc file "
"Example:  addNotetrack_lui_notify( "van_mate" , "dialog" , &"end_get_in_the_van" , "end_get_in" );"
"SPMP: singleplayer"
///ScriptDocEnd
=============
*/

addNotetrack_lui_notify( animname, notetrack, theNotify, anime )
{
	notetrack = ToLower( notetrack );
	anime = get_generic_anime( anime );
	index = add_notetrack_and_get_index( animname, notetrack, anime );
