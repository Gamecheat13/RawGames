...
debugDvars()
{
	...
	level._effect[ "lighting_fraction" ]  = LoadFX( "fx/test/lighting_fraction" );
...
interactive_warnings()
{
	...
	barrels = common_scripts\utility::array_combine( barrels, GetEntArray( "explodable_barrel", "script_noteworthy" ) );
	if ( ! barrels.size )
		return;

	//convert the old into Destructibles
	foreach ( barrel in barrels )
	{
		barrel.destructible_type	= "explodable_barrel";
		convertmodel				= barrel.model + "2";
		PreCacheModel( convertmodel );
		barrel SetModel( convertmodel ); // cheng added 2 to the names of new models with tags  to avoid conflict
		if ( IsDefined( barrel.target ) )
		{
			tEnt	= GetEnt( barrel.target, "targetname" );
			if ( IsDefined( tEnt ) )
				tEnt.script_destruct_collision = "pre";
			barrel.targetname	= "destructible_toy";
		}
	}
