...
#include soundscripts\_audio_vehicles;
...
aud_init()
{
	...
	if (!IsDefined(level.aud)) // Don't reinit audio if it's already been inited.
	{
		...
		// Blacksmith Dynamic Vehicle System
		VM2_init();
...
////////////////////////////////////////////////////
// STUBS FOR OLD AUD UTILS
// DONT USE, USE NEW SND UTILS
////////////////////////////////////////////////////
aud_set_occlusion(presetname)
{
	snd_set_occlusion(presetname);
}

aud_set_filter(presetname, index_, max_server_calls_per_frame_, set_hud_)
{
	snd_set_filter( presetname, index_, set_hud_ );
}
...
restore_after_deathsdoor()
{
	if (is_deathsdoor_audio_enabled() || isdefined(level._audio.in_deathsdoor))
	{
		...
		thread snd_set_occlusion(level._audio.deathsdoor.occlusion);
		
		thread snd_set_filter( level._audio.deathsdoor.filter0, 0, true );
		thread snd_set_filter( level._audio.deathsdoor.filter1, 1, true );
		thread snd_set_filter_lerp( level._audio.deathsdoor.filterlerp );
...
set_deathsdoor()
{
	...
	level._audio.deathsdoor.filter0 =  snd_get_current_filter_name( 0 );
	level._audio.deathsdoor.filter1 =  snd_get_current_filter_name( 1 );
	level._audio.deathsdoor.filterlerp = snd_get_current_filter_lerp();
	level._audio.deathsdoor.occlusion = snd_get_current_occlusion_name();
	level._audio.deathsdoor.reverb = level._audio.current_reverb;

	if (is_deathsdoor_audio_enabled())
	{	
		thread snd_set_filter("deathsdoor", 0, true);
		thread snd_set_filter_lerp( 1 );
...
{
	return snd_get_soundtable_name();
}
...
///ScriptDocEnd
*/
audio_message(msg, args, notificaion_string)
{
...
_audio_msg_handler(msg, args)
{
	...
	switch(msg)
	{
		...
		{
			level.player snd_play("sewer_bombs");
		}
		break;
...
aud_play_conversation(msg, args)
{
	...
	foreach (line in lines)
	{
		...
		/*
		line.ent snd_play(line.sound, "sounddone");
		...
		*/
		...
		ent snd_play(line.sound, "sounddone");
	...
	while (1)
	{
		...
		if (aud_is_specops() && other != level.player)
		{
			...
			if (IsSplitScreen() && IsDefined(tokens))
			{
				...
				{
					level._audio.specops_zones[1].zonename = tokens[0];
				}
			}
			#/
			continue;
		...
		if (IsDefined(point_a) && IsDefined(point_b))
		{
			...
			{
				// Determine which side of the trigger we're hitting based on the side that matches the current zone.
				// This fixes the problem of spawning inside a zone trigger at a point beyond the 50% mark and having it
				// incorrectly determine we are moving backwards through the trigger.
				if (AZM_get_current_zone() == tokens[0])
				{
					is_backward = false;
				}
				else if(AZM_get_current_zone() == tokens[1])
				{
					is_backward = true;
				}
				else
				{
					// Determine which side of the trigger we're hitting based on position.
					progress = trigger_multiple_audio_progress(point_a, point_b, dist, other.origin);
					if(progress < 0.5)
					{
						assertEx(false, "Error: Trying to blend from either zone \"" + tokens[0] + "\" or zone \"" + tokens[1] + "\" but the current zone is \"" + AZM_get_current_zone() + "\". Forcing starting zone to \"" + tokens[0] + "\".");
						AZM_start_zone( tokens[0] );
						is_backward = false;
					}
					else
					{
						assertEx(false, "Error: Trying to blend from either zone \"" + tokens[0] + "\" or zone \"" + tokens[1] + "\" but the current zone is \"" + AZM_get_current_zone() + "\". Forcing starting zone to \"" + tokens[1] + "\".");
						AZM_start_zone( tokens[1] );
						is_backward = true;
					}
				}
			}
			...
			if (!is_backward) // We're going forward.
			{
				// Going from zone 0 to zone 1.
				
				if (IsDefined(tokens) && IsDefined(tokens[0]))
				{				
					// S1 method, always send an enter message
					if ( IsDefined( level._snd ) && IsDefined(tokens[1]))
					{
						enter_message = "enter_" + tokens[1];
						snd_message("snd_zone_handler", enter_message, tokens[0]);
					}
					else if (IsDefined(self.script_audio_enter_msg))
					{
						// MW3 method, here for backwards compatibility, don't use.
			...
			else // We're going backward.
			{
				...
				{				
					...
					{
						enter_message = "enter_" + tokens[1];
						snd_message("snd_zone_handler", enter_message, tokens[1]);
					}
					else if (IsDefined(self.script_audio_enter_msg))
					{
						// MW3 method, here for backwards compatibility, don't use.
		...
		if (IsDefined(blend_args) && snd_is_zone_filter_setting_enabled())
		{
		...
		if (IsDefined(point_a) && IsDefined(point_b))
		{
			...
			if (progress > 0.5)
			{
				...
				if (IsDefined(tokens) && IsDefined(tokens[1]))
				{
					if (IsDefined(level._snd))
					{
						exit_message = "exit_" + tokens[1];
						snd_message("snd_zone_handler", exit_message, tokens[1]);
					}
					else if (IsDefined(self.script_audio_exit_msg))
					{
						// for backward compatibility on MW3, don't use for S1
			...
			else
			{
				...
				if (IsDefined(tokens) && IsDefined(tokens[0]))
				{
					...
					{
						exit_message = "exit_" + tokens[1];
						snd_message("snd_zone_handler", exit_message, tokens[0]);
					}
					else if (IsDefined(self.script_audio_exit_msg))
					{
						// for backward compatibility on MW3, don't use for S1
...
{	
	...
	if (IsDefined(final_explo_positions) && final_explo_positions.size == 2)
	{
		...
		left_ent snd_play(left_alias);
		right_ent snd_play(right_alias);
...
=============
///ScriptDocBegin
"Name: aud_get_optional_param( <optional_parameter_> , <default_value> )"
"Summary: Returns <optional_parameter_> if it is defined; otherwise, returns <default_value>. "
"Module: Entity"
"CallOn: N/A."
"OptionalArg: <optional_parameter_>: "
"MandatoryArg: <default_value>: "
"Example: vaue_to_use = aud_get_optional_param(1.0, some_param_)"
"SPMP: SP/MP"
///ScriptDocEnd
=============
*/
aud_get_optional_param(default_value, optional_parameter_)
{
	result = default_value;	// Note: undefined is a valid default value.	
...
aud_fade_sound_in(ent, alias_name, vol, fade_in_time, is_loop_)
{
	...
	if (is_loop)
	{
		ent snd_play_loop(alias_name);
	}
	else
	{
		ent snd_play(alias_name);
	}
...
aud_get_envelope_domain(env_points)
{
	if(IsArray(env_points))
	{
		assert(env_points.size > 0);
		return [env_points[0][0], env_points[env_points.size - 1][0]];	
	}
	else
	{
		assert(env_points.env_array.size > 0);
		return [env_points.env_array[0][0], env_points.env_array[env_points.env_array.size - 1][0]];
	}	
}
...
// Called on "the scene base."  (param is the guy that hit the notetrack)
aud_play_pcap_vo(alias, time_code_offset, is_2d_)
{
	use_slowmo			= false;
	delay_is_time_code	= true;
	ent_to_linkto		= self;
	
	if (IsDefined(is_2d_))
	{
		self aud_delay_play_2d_sound(alias, time_code_offset, use_slowmo, delay_is_time_code);
	}
	else
	{
		self aud_delay_play_linked_sound(alias, ent_to_linkto, time_code_offset, use_slowmo, delay_is_time_code);
	}	
}

play_2d_sound_internal(aliasname)
{
	self snd_play(aliasname, "sounddone");
	self waittill("sounddone");
	wait(0.05);
	self delete();
}

aud_wait_delay( delay, use_slowmo_, delay_is_time_code_, fps_ )
{
	if ( IsDefined(delay_is_time_code_) )
	{
		fps = 60;
		if ( IsDefined(fps_) )
	    {
		...
		secs	= floor( delay );
		frames	= ( delay - secs ) * 100;
		delay	= secs + frames * 1/fps;
	}
	wait_time = delay;
...
aud_delay_play_2d_sound_internal(aliasname, delay, use_slowmo_, delay_is_time_code_)
{
	aud_wait_delay(delay, use_slowmo_, delay_is_time_code_);
	
	//IPrintLnBold("aud_delay_play_2d_sound_internal( " + aliasname + ", " + wait_time + " );" );
...
///ScriptDocEnd
*/
aud_delay_play_2d_sound(aliasname, delay, use_slowmo_, delay_is_time_code_, fps_)
{
	ent = thread aud_delay_play_2d_sound_internal(aliasname, delay, use_slowmo_, delay_is_time_code_);
	return ent;
}

aud_delay_play_linked_sound(aliasname, ent_to_linkto, delay, use_slowmo_, delay_is_time_code_, type_, loop_stop_notify_, offset_, init_volume_)
{
	thread aud_delay_play_linked_sound_internal(aliasname, ent_to_linkto, delay, use_slowmo_, delay_is_time_code_, type_, loop_stop_notify_, offset_, init_volume_);
}

aud_delay_play_linked_sound_internal(aliasname, ent_to_linkto, delay, use_slowmo_, delay_is_time_code_, fps_, type_, loop_stop_notify_, offset_, init_volume_)
{
	aud_wait_delay(delay, use_slowmo_, delay_is_time_code_, fps_);
	thread aud_play_linked_sound(aliasname, ent_to_linkto, type_, loop_stop_notify_, offset_, init_volume_);
}


audx_play_linked_sound_internal(type, aliasname, loop_stop_notify_, init_volume_)
{
	if (type == "loop")
	{
		...
		self snd_play_loop(aliasname);
//		if (IsDefined(init_volume_))
//		{
//			self scalevolume(init_volume_, 0);
//		}
...
=============
///ScriptDocBegin
"Name: aud_play_linked_sound( <aliasname> , <ent_to_linkto> , <type_> , <loop_stop_notify_> , <offset_> , <init_volume_> )"
...
"OptionalArg: <type_>: "loop" or "oneshot". If a "loop", you must also supply a loop_stop_notify argument. DEFAULT is "oneshot".
...
"OptionalArg: <offset_>: positional offset sound should play relative to the position of <ent_to_linkto>"
"OptionalArg: <init_volume_>: initial volume (not currently implemented)"
"OptionalArg: <init_pos_>: initial position.  sound will move with the ent_to_linkto but relative to the initial position."
"Example: "
"SPMP: singleplayer"
///ScriptDocEnd
=============
*/
aud_play_linked_sound(aliasname, ent_to_linkto, type_, loop_stop_notify_, offset_, init_volume_, init_pos_)
{
	...
	init_pos = ent_to_linkto.origin;
	if (IsDefined(init_pos_))
	{
		init_pos = init_pos_;	
	}
	ent = spawn("script_origin", init_pos);
	...
	ent thread audx_play_linked_sound_internal(type, aliasname, loop_stop_notify_, init_volume_);
	return ent;
}
...
aud_play_primed_point_source_loop(alias, start_vol_, fade_time_)
{
	...
	// Get optional params.
	start_vol = aud_get_optional_param(1.0, start_vol_);
	fade_time = aud_get_optional_param(1.0, fade_time_);
...
aud_set_music_submix(volume, fade_time_)
{
	...
	{
		MM_add_dynamic_volmod_submix(submix_name, ["music", 1.0, "music_big", 1.0], fade_time_);
