...
#/
	...
	if (IsDefined(zone["occlusion"]))
	{
		...
		{
			snd_set_occlusion(zone["occlusion"]);
		}
		else
		{
...
AZMx_get_preset_from_string_table(presetname, checklevel)
{
	...
	level_stringtable = soundscripts\_audio::get_zone_stringtable();
	preset = [];
...
AZMx_get_blend_args(zone_from_name, zone_to_name)
{
	...
	args.damb1_name = current_zone["dynamic_ambience"];
	/#
	if (num_damb_playing == 1)
	{
		foreach(key, val in level._audio.damb.playing["zone"])
		{
			...
			{
				if (key != current_zone["dynamic_ambience"])
				{
					aud_print_warning("Blending from a dynamic ambience (" + key + ") which isn't the current zone's (" + current_zone["dynamic_ambience"] + ").");
				}
			}
			else
			{
				aud_print_warning("Blending from a dynamic ambience (" + key + ") but the current zone should be empty.");
			}
		}
	}
	#/
...
{	
	...
	if (snd_is_zone_filter_setting_enabled())
	{
		if (Isdefined(args.filter1))  // the "from" is the "main" eq
		{
			has_filter = true;
			snd_set_filter(args.filter1, 0, false);
			/#
		...
		{
			snd_set_filter(undefined, 0, false);
			/#
			set_filter_from_hud("", "");
			#/
		}
		...
		{
			has_filter = true;
			snd_set_filter(args.filter2, 1, false);
			/#
		...
		{
			snd_set_filter(undefined, 1, false);
			/#
			set_filter_to_hud("", "");
			#/
		}
		...
		if (!snd_is_zone_filter_setting_enabled())
		{
			if (IsDefined(args.occlusion1))
			{
				if (args.occlusion1 == "none")
					...
					snd_set_occlusion(args.occlusion1);
			}
		}
		...
		if (!snd_is_zone_filter_setting_enabled())
		{
			if (IsDefined(args.occlusion2))
			{
				if (args.occlusion2 == "none")
					...
					snd_set_occlusion(args.occlusion2);
			}
		}
	}
}
