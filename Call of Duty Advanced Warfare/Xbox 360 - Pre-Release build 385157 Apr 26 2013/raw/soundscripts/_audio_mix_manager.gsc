...
MM_init()
{
	...
	// setup headroom data
	MMx_init_critical_headroom();
	level._audio.mix.headroom = 0.60; // default headroom set to 1.0 by default (dv changed from 0.85 to 0.6 on 9/25/12).
	
	// array of changed presets *this* frame. Used to optimize preset updating
	level._audio.mix.changed_presets = [];
	MMx_update_mix(0, "default"); // make sure we initialize the default submix for headroom	
...
MMx_make_new_submix(preset_name, volume_scale_)
{
	...
	{
		/#
		aud_print_error("Can't set a volmod mix preset with the name, \"default\". That is reserved for the volumemodgroups.def file.");
		#/
		return;
	}
...
// set default value of critical headroom
MMx_init_critical_headroom()
{
	level._audio.mix.critical_headroom = 0.8;
}

// allow for dynamic setting of critical headroom
MM_set_critical_headroom(val)
{
	level._audio.mix.critical_headroom = clamp(val, 0, 1);
}

MM_get_critical_headroom()
{
	return level._audio.mix.critical_headroom;
}
...
MMx_update_volmod_groups(fade_time_)
{
	...
	foreach(volmod, volume in level._audio.mix.volume_products)
	{
		...
		// highest layer of headroom is "dialog_critical"		
		if (volmod != "big_moment" && volmod != "dialog_critical")
	    {
			volume *= MM_get_critical_headroom();
	    }
		
		// normal headroom for non-critical channels
		if (volmod != "big_moment" && volmod != "dialog_critical" && volmod != "dialog" && volmod != "fullvolume" && volmod != "music_big" && volmod != "explosion_critical")
		{
...
MMx_get_preset_from_string_table(presetname, checklevel)
{
	...
	preset = [];
	level_stringtable = soundscripts\_audio::get_mix_stringtable();
...
MMx_parse_volumemodgroups_csv()
{
	stringtable = "sounddata/volumemodgroups.def";
