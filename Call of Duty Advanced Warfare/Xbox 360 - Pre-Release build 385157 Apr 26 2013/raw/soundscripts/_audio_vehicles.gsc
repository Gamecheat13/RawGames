#include soundscripts\_audio_presets_vehicles;
...
VM_init()
{
	...
	if (!IsDefined(level._audio.veh))
	{
		...
		// Global data for all vehicle systems.
		// Defines default values and min/max values.
		...
		// Array of custom callbacks:  One call back is provided for each parameter of each vehicle instance.
		level._audio.veh.callbacks = [];
		
		// Various debug flags (printing various values)
		...
		level._audio.veh.presets = []; 		// loaded from the preset file
		level._audio.veh.maps = [];			// envelopes
		level._audio.veh.instances = []; 	// actually running instances of a vehicle
		
		// Instances of ducked sounds. Ducking happens on looping sounds when a one-shot (start/stop) sound occurs.
...
// One call back is provided for each vehicle instance, 
// so any behaviors that need to be customized must be done in the one user callback.
...
// Core function to call when starting a vehicle preset.
// instance_name:	Must give a unique instance name.. think of it like a handle; you use it to reference the vehicle preset.
// preset_name:		Name of the preset to use in the _audio_presets_vehicles.gsc file.
// vehicle_entity:	Entity to call the vehicle preset on... must be a vehicle.
// fadein_:			Initial fadein time to start the preset sounds.
// startalias_:		The first sound to play when starting the preset (i.e. a key-on or vehicle start-up sound).
// offset_:			An offset vector to displace the position of the vehicle sounds off the vehicle origin.
VM_start_preset(instance_name, preset_name, vehicle_entity, fadein_, startalias_, offset_)
{
	...
	wait(0.25);	// Probably voo-doo to fix a bug.
	
	// If we've already created a vehicle with the same instance name, stop it first.
	if (IsDefined(level._audio.veh.instances[instance_name]))
	{
		VM_stop_preset_instance(instance_name);	// Stops and cleans up.
	}

	// If we gave a startalias_ sound, then play it.
	if (IsDefined(startalias_))
	{
		thread aud_play_linked_sound(startalias_, vehicle_entity, undefined, undefined, offset_);	// Plays over the fade-in of the loops, so no ducking necessary.
	}

	// Initialize the instance entry in the instances array.
	level._audio.veh.instances[instance_name] = [];
	// Start constructing information about the instance...

	// "entity" is a handle to the entity struct
	level._audio.veh.instances[instance_name]["entity"] = vehicle_entity;

	// Optimization: no need to reload the preset from the preset file if we've alreayd loaded it
	// don't reload the preset if we've already loaded it
	if (!IsDefined(level._audio.veh.presets[preset_name])) // if not in the preset cache
	{
		preset = [];
		preset = AUDIO_PRESETS_VEHICLES(preset_name, preset); // Goes into _audio_presets_vehicles.gsc and loads the preset.
		/#
		VMx_validate_preset(preset, preset_name); // try to validate the preset...
		#/
	...
	// this is a bug... don't need to do this again
	level._audio.veh.instances[instance_name]["entity"] = vehicle_entity;
	
	// Parse the preset settings and construct the "args" struct for the playback/vehicle monitoring threads, 
	// which get launched at the end of this function.
	// IMPORTANT NOTE: Each param type gets it's own update thread that monitors and updates that single param mapping.
	foreach (param, settings in level._audio.veh.presets[preset_name])
	{
		// param is the key value in the preset array (using "p"), the value is the settings array
		// the settings big ass array is an array of arrays

		args = spawnstruct();		// args struct is constructing what things to pass to the vehicle update threads.
									// args is built by parsing the preset structs.
		
		args.instance_name = instance_name;
		args.vehicle = vehicle_entity;
		
		// Determine which callback functions are required for this vehicle instance.
		// (param is e.g. "speed", "tilt", etc.)
		// This switch/case sets up all the default callback-related args for each param. The preset can override these and/or add to these args.
		switch (param)
		{
			// Set up the default values for the parameter
			case "tilt": 							// param type
				args.type = "tilt"; 				// attach the type to the args struct
				args.callback = ::VMx_get_tilt; 	// define the default callback function to call during the update loop
				args.min = -45.0; 					// each param type will have different default absolute bounds, which can be overridden via the "range" preset setting.
				args.max = 45.0;
			break;
		...
		// Set common default arguments independent of parameter type.
		...
		// Now, loop through the settings array and get overrides (or add custom parameters that don't need defaults)
		foreach (key, value in settings)
		{
			...
			switch(value[0])
			{
				...
				// this doesn't have a default, used in special case..
				// this was a hack feature in harbor to handle zodiac jump case... should probably not be used
				case "heightmax":
				args.heightmax = value[1];
				break;
				
				// the callback function to call -- allows user to define a custom callback.. shoudl probably have used this to heightmax
				case "callback":
				custom_callback_name = value[1];
				args.custom_callback = level._audio.veh.callbacks[custom_callback_name];
				break;
				
				// overrides the range of the vehicle param
				case "range":
				args.min = min(value[1], value[2]);
				args.max = max(value[1], value[2]);
				break;
				
				// This is for multiple input parameters that contribute to tweaking the same sounds; e.g., speed and throttle.
				// In this case, it was a mechanism to change volume based on the throttle amount (e.g. acceleration)
				// a better mechanism would be to allow multiple input parameters per group of sounds
				case "multiply_by_throttle":
				args.multiply_by_throttle = true;
				break;
								
				// This was not used when shipped.
				case "multiply_by_leftstick":
				args.multiply_by_leftstick = true;
				break;
				
				case "start":
				assert(args.type == "start_stop");						// Make sure this setting is for a "start_stop" param type.
				args.start_alias_data = spawnstruct();					// create a start_alias_data struct on the "args" struct
				...
				for (i = 2; i < value.size; i++)
				{
					...
					{
						level._audio.veh.duck_starts[instance_name] = value[i];	// "true" signals that this "start_stop" ducks all other sounds playing all other params.
					}
				}
				break;
...
// BUG:  playing_presets isn't defined anywhere.  If this is ever actually called, it will crash.
VM_stop(fade_)
{
...
VM_stop_preset_instance(instance_name, fade_)
{
	...
	if (IsDefined(level._audio.veh.instances[instance_name]))
	{
		...
		if (level._audio.veh.instances[instance_name].size > 0)
		{
			...
			{
				if (key != "entity" && key != "speed" && key != "throttle")	// NOTE:  This data struct was originally an indexed list of sndEntitys, then other "keyed" items were added. So... change this design. 
				{
		...
		wait(fade + 0.05);
		if (level._audio.veh.instances[instance_name].size > 0)	// Unecessary check here, should refactor to thread off scaled, then delete.
		{
			foreach( key, sndEntity in level._audio.veh.instances[instance_name] )
			{
...
// WTF?  Need full explanation here, especially for the count arrays.
...
// WTF?  I don't understand this comment.  Is it that the function returns whether or not the instnace's loops should *currently* be ducked?
...
// Returns true if an intance's "start sound" is currently playing.
...
// Returns true if an intance's "stop sound" is currently playing.
...
// WTF? Shouldn't the offset be a vector, not a single coord?
VMx_play_start_sound(name, vehicle_ent, instance, offset_)
{
	...
	ent.ref = level._audio.veh.start_ent_count[instance];	// WTF?
	ent playsound(name, "sounddone");
	ent thread VMx_monitor_start_ent(instance);
	level._audio.veh.start_ents[instance][ent.ref] = ent;	// WTF?  Holy fuckikng hell.
	level._audio.veh.start_ent_count[instance]++;			// WTF?  Holy fuckikng hell II
}
...
VMx_monitor_start_ent(instance)
{
	...
	self endon("kill");
	self waittill("sounddone");
	level._audio.veh.start_ents[instance][self.ref] = undefined; // WTF?
	// stop any duck instances if they exist
	if (IsDefined(level._audio.veh.ducked_instances[instance]))
	{
		level._audio.veh.ducked_instances[instance] = undefined;	// WTF? Does this cause something to get unducked?
	}
	self delete();
}
...
VMx_do_start_stop_callback(args)
{
	...
	while(true)
	{
		...
		// TOGGLING STATES: This entire block is for starting and canceling start and stop sounds.
