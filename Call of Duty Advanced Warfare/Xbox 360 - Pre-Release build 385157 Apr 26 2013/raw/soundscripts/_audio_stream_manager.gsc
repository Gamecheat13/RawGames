...
SM_start_preset(alias, fade_, priority_, interrupt_fade_, volscale_)
{
	...
	{
		println( "Warning: SM_start_preset() alias doesn't exist: " + alias );
		return;
	}	
...
SM_start_music(alias, fade_in_time_, cross_fade_out_time_, volscale_, prev_alias)
{
	...
	if ( !SoundExists( alias ) )
	{
		println( "Warning: SM_start_music() alias doesn't exist: " + alias );
		return;
	}	
...
SM_stop_ambient_alias(alias, fade_)
{
	...
	if (alias != "none")
	{
		if ( !SoundExists( alias ) )
		{
			println( "Warning: SM_stop_ambient_alias() alias doesn't exist: " + alias );
			return;
		}	
...
SM_stop_music_alias(alias, fade_)
{
	...
	if ( !SoundExists( alias ) )
	{
		println( "Warning: SM_stop_music_alias() alias doesn't exist: " + alias );
		return;
	}
...
SM_mix_ambience(info_array)
{
	...
	for (i = 0; i < info_array.size; i++)
	{
		...
		if (!SoundExists(alias))
		{
			println("ERROR: ambient alias '" + alias + "' doesn't exist.");
			continue;
		}
