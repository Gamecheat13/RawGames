...
init_animset_cover_prone()
{
	...
	initAnimSet[ "straight_level" ] = %prone_aim_5;

	initAnimSet[ "legs_up" ] = %prone_aim_feet_45up;
	initAnimSet[ "legs_down" ] = %prone_aim_feet_45down;

	initAnimSet[ "fire" ] = %prone_fire_1;
	initAnimSet[ "semi2" ] = %prone_fire_burst;
	initAnimSet[ "semi3" ] = %prone_fire_burst;
	initAnimSet[ "semi4" ] = %prone_fire_burst;
	initAnimSet[ "semi5" ] = %prone_fire_burst;

	initAnimSet[ "single" ] = [ %prone_fire_1 ];

	initAnimSet[ "burst2" ] = %prone_fire_burst;// ( will be limited to 2 shots )
	initAnimSet[ "burst3" ] = %prone_fire_burst;
	initAnimSet[ "burst4" ] = %prone_fire_burst;
	initAnimSet[ "burst5" ] = %prone_fire_burst;
	initAnimSet[ "burst6" ] = %prone_fire_burst;

	initAnimSet[ "reload" ] = %prone_reload;

	initAnimSet[ "look" ] = [ %prone_twitch_look, %prone_twitch_lookfast, %prone_twitch_lookup ];

	initAnimSet[ "grenade_safe" ] = [ %prone_grenade_A, %prone_grenade_A ];
	initAnimSet[ "grenade_exposed" ] = [ %prone_grenade_A, %prone_grenade_A ];

	initAnimSet[ "exposed_idle" ] = [ %prone_idle ];
	initAnimSet[ "twitch" ] = [ %prone_twitch_ammocheck, %prone_twitch_look, %prone_twitch_scan, %prone_twitch_lookfast, %prone_twitch_lookup ];
	...
	initAnimSet[ "stand_2_prone" ] = %stand_2_prone_nodelta;
	initAnimSet[ "crouch_2_prone" ] = %crouch_2_prone;
	initAnimSet[ "prone_2_stand" ] = %prone_2_stand_nodelta;
	initAnimSet[ "prone_2_crouch" ] = %prone_2_crouch;
	initAnimSet[ "stand_2_prone_firing" ] = %stand_2_prone_firing;
	initAnimSet[ "crouch_2_prone_firing" ] = %crouch_2_prone_firing;
	initAnimSet[ "prone_2_stand_firing" ] = %prone_2_stand_firing;
	initAnimSet[ "prone_2_crouch_firing" ] = %prone_2_crouch_firing;

	assert( !isdefined( anim.archetypes["soldier"]["cover_prone"] ) );
	anim.archetypes[ "soldier" ]["cover_prone"] = initAnimSet;
}
...
{
	self UpdateProne( self lookupAnim( "cover_prone", "legs_up" ), self lookupAnim( "cover_prone", "legs_down" ), 1, time, 1 );
...
//	self resetGiveUpOnEnemyTime();
	...
	for ( ;; )
	{
		self animscripts\utility::UpdateIsInCombatTimer();// reset our in - combat state
...
setup_cover_prone()
{
	...
	self.a.array = self lookupAnimArray( "cover_prone" );
}
