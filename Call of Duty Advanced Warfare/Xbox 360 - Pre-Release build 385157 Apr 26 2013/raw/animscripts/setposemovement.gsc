...
BlendIntoCrouchRun()
{
	...
	else
	{
		...
		// When holding an SMG, we usually want to use the global "smg_crouch_run" animset.
		// The exception is when self has an "override" customMoveAnimSet for "run".
		if ( usingSMG() && self.a.movement == "run" && !( isdefined( self.customMoveAnimSet ) && isdefined( self.customMoveAnimSet[ "run" ] ) ) )
		{
			anim_set = "smg_crouch_run";
			self thread animscripts\run::UpdateMoveAnimWeights( "crouchrun", lookupAnim( anim_set, "crouch" ), lookupAnim( anim_set, "crouch_b" ), lookupAnim( anim_set, "crouch_l" ), lookupAnim( anim_set, "crouch_r" ) );
		}
		else
		{
			// Note: GetMoveAnim[x] is implemented as self.a.moveAnimSet[x].
