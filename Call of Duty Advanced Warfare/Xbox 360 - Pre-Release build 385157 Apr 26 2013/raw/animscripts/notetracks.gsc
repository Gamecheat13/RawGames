...
HandleDogSoundNoteTracks( note )
{
	...
	{
		self notetrack_playsound("dogstep_run_default");
		return true;
	}
...
{
	self notetrack_playsound( "weap_reload_shotgun_loop_npc" );
}

noteTrackPistolRechamber( note, flagName )
{
	self notetrack_playsound("weap_reload_pistol_chamber_npc");
}
...
noteTrackFootStep( note, flagName )
{
	...
	if ( IsDefined( cloth_sound ) && cloth_sound != "" )
		self notetrack_playsound( cloth_sound );
	...
	if ( IsDefined( self.weapon ) )
	{
		...
		if ( IsDefined( equip_sound ) && equip_sound != "" )
			self notetrack_playsound( equip_sound );
	}
}
...
customNotetrackFX( note, flagName )
{
	...
	self notetrack_playsound( soundAlias );
}
...
noteTrackFootScrape( note, flagName )
{
	...
	self notetrack_playsound( "step_scrape_" + groundType );
}
...
noteTrackLand( note, flagName )
{
	...
	self notetrack_playsound( "land_" + groundType );
...
noteTrackBodyFall( note, flagName )
{
	...
	self notetrack_playsound( "bodyfall_" + groundType + suffix );
}
...
HandleNoteTrack( note, flagName, customFunction )
{
	...
	case "rechamber":
		if ( weapon_pump_action_shotgun() )
			self notetrack_playsound( "weap_reload_shotgun_pump_npc" );
		self.a.needsToRechamber = 0;
		break;
	case "no death":
...
playFootStep( is_left, is_large )
{
	if ( ! isAI( self ) )
	{
		self notetrack_playsound( "step_run_dirt" );
		return;
	}
	...
	run_type = get_notetrack_movement();
	self notetrack_playsound( "step_" + run_type + "_" + groundType );
