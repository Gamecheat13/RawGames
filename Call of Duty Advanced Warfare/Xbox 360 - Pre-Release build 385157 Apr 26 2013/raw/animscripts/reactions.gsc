...
bulletWhizbyReaction()
{
	...
	if ( enemyNear || cointoss() )
	{
		...
		if ( usingSMG() )
		{
			reactAnim[ 0 ] = %smg_exposed_idle_reactA;
			reactAnim[ 1 ] = %smg_exposed_idle_reactB;
			reactAnim[ 2 ] = %smg_exposed_idle_twitch;
			reactAnim[ 3 ] = %smg_exposed_idle_twitch_v4;
		}
		else
		{
...
getNewEnemyReactionAnim()
{
	...
	if ( !isdefined( reactAnim ) )
	{
		...
		if ( usingSMG() )
		{
			reactAnimArray[ 0 ] = %smg_exposed_backpedal;
			reactAnimArray[ 1 ] = %smg_exposed_idle_reactB;
		}
		else
		{
...
stealthNewEnemyReactAnim()
{
	...
	{
		self orientmode( "face enemy" );
		reactAnim = %exposed_idle_reactB;
		if ( usingSMG() )
		{
			reactAnim = %smg_exposed_idle_reactB;
		}
	...
	{
		self orientmode( "face enemy" );
		backPedalAnim = %exposed_backpedal;
		backPedalAnim_v2 = %exposed_backpedal_v2;
		if ( usingSMG() )
		{
			backPedalAnim = %smg_exposed_backpedal;
			backPedalAnim_v2 = %smg_exposed_backpedal_v2;
		}
		self setFlaggedAnimKnobRestart( "reactanim", backPedalAnim, 1, 0.2, 1 );
		time = getAnimLength( backPedalAnim );
		self DoNoteTracksForTime( time * 0.8, "reactanim" );	

		self orientmode( "face current" );

		self clearanim( %root, 0.2 );
		self setFlaggedAnimKnobRestart( "reactanim", backPedalAnim_v2, 1, 0.2, 1 );
