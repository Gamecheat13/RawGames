...
main()
{
	...
	if ( throwDist < 600 && isLowThrowSafe() )
	{
		if ( throwDist < 300 )
		{
			// really short range
			if ( usingSMG() )
				animarray = self lookupAnim( "grenade", "return_throw_short_smg" );
			else
		...
		else
		{
			// long range
			if ( usingSMG() )
				animArray = self lookupAnim( "grenade", "return_throw_long_smg" );
			else
	...
	if ( animArray.size == 0 )
	{
		...
		if ( usingSMG() )
			animArray = self lookupAnim( "grenade", "return_throw_default_smg" );
		else
