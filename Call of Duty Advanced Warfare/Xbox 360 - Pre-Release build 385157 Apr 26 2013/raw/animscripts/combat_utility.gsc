...
{	
	...
	return randomfloatrange(minDelay, maxDelay);
}
...
{
	// don't start doing another burst firing pattern if one is already going on
	if ( IsDefined( self.currently_burst_firing ) && self.currently_burst_firing )
	{
		burstDelay();
		self.currently_burst_firing = false;
	...
	self.currently_burst_firing = true;
	while ( 1 )
	{
		if (!firsttime)
...
FireUntilOutOfAmmo( fireAnim, stopOnAnimationEnd, maxshots )
{
	...
	fireTime = weaponFireTime( self.weapon );
	
	FireUntilOutOfAmmoInternal( animName, fireAnim, stopOnAnimationEnd, maxshots, fireTime );
