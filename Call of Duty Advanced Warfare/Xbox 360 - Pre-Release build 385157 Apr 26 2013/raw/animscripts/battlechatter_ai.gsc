...
*/	
	...
	if ( isdefined( self.script_friendname ) )
	{
		...
		if ( IsSubStr( friendname, "burke" ) )
		{
		    	self.npcID = "brk";
		}
		else if ( IsSubStr( friendname, "carter" ) )
		{
		    	self.npcID = "ctr";
		} 
		else if ( IsSubStr( friendname, "joker" ) )
		{
		    	self.npcID = "jkr";
		}	
		else if ( IsSubStr ( friendname, "alpha_leader" ) )
		{
			self.npcID = "aldr";
		}
		else if ( IsSubStr ( friendname, "cormack" ) )
		{
			self.npcID = "aldr";
		}
		else if ( IsSubStr ( friendname, "maddox" ) )
		{
			self.npcID = "jkr";
		}
		else if ( IsSubStr ( friendname, "security_officer_1" ) )
		{
			self.npcID = "ctr";
		}
		/* DEPRECATED
...
{
	if( id == "UK" || id == "US" || id == "NS" || id == "TF" || id == "SS" || id == "XS" )
	{
		return true;
	}
...
init_aiBattleChatter()
{
	...
	// global
	// new temp xslice bc does not have these callouts
	...
	{
		if ( self.voice != "british" && self.voice != "pmc" && self.voice != "xslice" ) // UK, PMC, XSLICE does not have this callout
			self addAllowedThreatCallout( "ai_obvious" );
		self addAllowedThreatCallout( "ai_contact_clock" );
		if( self.voice != "xslice")	// new temp xslice bc does not have these callouts
		{
	...
	if( self.team == "allies" )
	{
		...
		if ( self.voice == "seal" || self.voice == "xslice" )
		{
			self addAllowedThreatCallout( "concat_location" );
			self addAllowedThreatCallout( "player_target_clock_high" );
			self addAllowedThreatCallout( "ai_target_clock_high" );
...
aiDeathEnemy()
{
	...
	if( !IsDefined( attacker.countryID ) || attacker.countryID != "NS" || attacker.countryID != "XS" )
	{
		return;
	}
...
player_friendlyfire_waiter_damage()
{
	...
	while( 1 )
	{
		...
		{
			if( damage_is_valid_for_friendlyfire_warning( type, attacker ) )
			{
				self player_friendlyfire_addReactionEvent();
			}
		}
...
damage_is_valid_for_friendlyfire_warning( type, attacker )
{
	if( !IsDefined( type ) )
	{
		return false;
