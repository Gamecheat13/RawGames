...
init_animset_combat()
{
	...
	initAnimSet["smg_trans_to_combat"] = %smg_casual_stand_idle_trans_out;
...
TransitionToCombat()
{
	...
	if ( self.prevScript == "stop" && !self IsCQBWalking() && self.a.pose == "stand" )
	{
		...
		trans_anim = undefined;
		if ( usingSMG() )
			trans_anim = self lookupAnim( "combat", "smg_trans_to_combat" );
		else
...
rpgDeath()
{
	...
	if ( randomFloat( 1 ) > 0.5 )
		self SetFlaggedAnimKnobAll( "deathanim", self lookupAnim( "combat", "rpg_death" ), %root, 1, .05, 1 );
	else
		self SetFlaggedAnimKnobAll( "deathanim", self lookupAnim( "combat", "rpg_death_stagger" ), %root, 1, .05, 1 );
