...
getHdrLdrColorDvar( dvar )
{
	if (IsUsingHDR())
	{
		...
		g = float(elms[1]);
		b = float(elms[2]);
		hdrscale = float(elms[3]) * 16.0;
		color = ( r * hdrscale, g * hdrscale, b * hdrscale );
		return color;
	}
	else
		return getdvarvector( dvar );
}

setfogsliders()
{
	/#
	fogcolor = getHdrLdrColorDvar( "g_fogColorReadOnly" );
	...
	sunFogColor = getHdrLdrColorDvar( "g_sunFogColorReadOnly" );
...
updateFogFromScript()
{
	...
	{
		SetExpFog( 2000000000, 2000000001, 0, 0, 0, 0, 0 );// couldn't find discreet fog disabling other than to never set it in the first place
	}
}
