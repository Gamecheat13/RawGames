...
setup_destructibles( cached )
{
	...
	{
		self thread destructibleCoverWatcher();			//Initialize thread that watches distance of player to destructible to enable/disable setdamage
	}
	
	self thread destructible_fx_spawnImmediate();	// Launch any fx that need to spawn immediately	
}
...
=============
///ScriptDocBegin
"Name: destructible_fx_spawn_immedate( <tagName> , <fxName> , <useTagAngles> , <damageType> , <groupNum> , <fxCost> , <spawnImmediate>, < stateChangeKill > )"
"Summary: Play FX after the health of the previously defined destructible state reaches zero.  Same as destructible_fx except that
the spawn_immedate flag is always set to true.  Used explicitly by the destructible UI tool."
"Model: Destructible"
"CallOn: Nothing"
...
"OptionalArg: <spawnImmediate>: Launch the fx immediately regarless of the state."
"OptionalArg: <stateChangeKill>: When set to one, the fx will soft kill on the next state change - or - when the destructible reaches its final state."
...
=============
///ScriptDocBegin
"Name: destructible_fx( <tagName> , <fxName> , <useTagAngles> , <damageType> , <groupNum> , <fxCost> , <spawnImmediate>, < stateChangeKill > )"
...
"OptionalArg: <fxCost>: If defined (to a number, generally 1), prevents too many destructibles entering a state on the same frame, ie prevents too many FX being started on the same frame."
"OptionalArg: <spawnImmediate>: Launch the fx immediately regarless of the state."
"OptionalArg: <stateChangeKill>: When set to one or two, the fx will soft kill or hard kill respectively ( hardkill only works if "usetagangles" is enabled ) on the next state change - or - when the destructible reaches its final state."
"Example: destructible_fx( "tag_death_fx", "fx/explosions/small_vehicle_explosion", false );"
"SPMP: both"
///ScriptDocEnd
=============
...
=============
///ScriptDocBegin
"Name: destructible_explode( <force_min> , <force_max> , <rangeSP> , <rangeMP> , <mindamage> , <maxdamage> , <continueDamage> , <originOffset> , <earthQuakeScale> , <earthQuakeRadius> , <originOffset3d> , <delaytime>, <angularImpulse_min>, <angularImpulse_max> )"
...
"OptionalArg: <angularImpulse_min>: Initial angular impulse applied to each thrown part with a magnitude between this and angularImpulse_max.  Setting angularImpulse_min and angularImpulse_max also means that force_min and force_max get treated as impulse units and not velocity."
"OptionalArg: <angularImpulse_max>: Initial angular impulse applied to each thrown part with a magnitude between angularImpulse_min and this.  Setting angularImpulse_min and angularImpulse_max also means that force_min and force_max get treated as impulse units and not velocity."
...
/*
"Summary: Worker function called by destructible_part() and destructible_state()."
*/
...
//			prof_end( "_destructible" );
		...
		// send a message to force kill fx that have state_change_kill set
...
{
	//  Wait til the current part changes state- or the entire destructible dies to kill any fx
	...
	//self endon( ( "FX_State_Change_Kill0" ) );
	for(;;)
	{
...
//
// spawnImmediate_think detects the state change of the destructible & will attempt
//
...
destructible_fx_spawn_think( eModel, fxid, fx_tag , i, usetagangles, stateChangeKill )
{
	...
	{							
		if ( usetagangles )
		{
			...
			wait(.05); // Will not spawn & kill on the same frame
...
{
	//wait( 1.0 );  // Wait for everything else to get started
	if ( !isdefined( level.destructible_type[ self.destructibleInfo].parts ) )
		return;
	eModel = get_dummy();
...
destructible_fx_think( action_v, eModel, damageType, partIndex, groupNum )
{
	...
	for ( idx = 0; idx < fx_size; idx++ )
	{
		...
		// Ignoring all fx that are spawned immediately
