...
handle_audio()
{
	...
	audio_on		= false;
	dist_threshold	= 500 * 12;
	fade_out_time	= 1.0;
	
	self vehicle_scripts\_gaz_dshk_aud::snd_init_gaz();
	self thread monitor_death_stop_sounds();
	...
	while ( 1 )
	{
		if(!IsDefined(self.script_disablevehicleaudio) || !self.script_disablevehicleaudio)
		{
			...
			if (audio_on && dist > dist_threshold)
			{
				self vehicle_scripts\_gaz_dshk_aud::snd_stop_gaz(fade_out_time);
				audio_on = false;
				wait(0.1);
				assert(!IsDefined(self.snd_instance));
			}
			else if (!audio_on && dist < dist_threshold)
			{
				self vehicle_scripts\_gaz_dshk_aud::snd_start_gaz();
				audio_on = true;
			}
		}
		else if (audio_on)
		{
			self vehicle_scripts\_gaz_dshk_aud::snd_stop_gaz(1.0);
			audio_on = false;
		}
...
build_gaz_death( classname )
{
	...
	build_deathfx( "fx/explosions/vehicle_explosion_gaz", "tag_deathfx",        "veh_gaz_expl_3d" ,            undefined ,       undefined ,      undefined,   undefined, undefined,  undefined);
		
			   //   effect 					      tag 		      sound      bEffectLooping    delay      bSoundlooping    waitDelay   
	build_deathfx( "fx/fire/firelp_med_pm_nolight", "tag_cab_fx"  , "veh_gaz_fire_lp", undefined		, undefined, true			, 0 );
