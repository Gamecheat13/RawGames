...
//	thread bcs_disabler();
	...
	add_start( "melee", maps\bog_a_code::start_melee );
	add_start( "breach", maps\bog_a_code::start_breach );
	add_start( "alley", maps\bog_a_code::start_alley );
	add_start( "shanty", maps\bog_a_code::start_alley );
	add_start( "bog", maps\bog_a_code::start_bog );
	add_start( "zpu", maps\bog_a_backhalf::start_zpu );
	add_start( "cobras", maps\bog_a_backhalf::start_cobras );
	add_start( "end", maps\bog_a_backhalf::start_end );
...
//	maps\_blackhawk::main( "vehicle_blackhawk_low" );
	...
	// hint strings
	add_hint_string( "nvg", "", ::nightVis_check );
...
//	flag_init( "laundrymat_open" );
	...
	thread do_in_order( ::flag_wait, "friendlies_charge_alley", ::activate_trigger_with_targetname, "friendly_alley_charge_trigger" );
	...
	breach_guy_2 thread add_spawn_function( ::die_after_spawn, 2.950 ); 
...
ambush()
{
	...
	level.price.animplaybackrate = 1.0;
...
//	bridge_go_trigger wait_for_trigger_or_timeout( 3.5 ); // 3.8
	...
	friendly1.animplaybackrate = 1.1;
	friendly2.animplaybackrate = 1.2;
	friendly4.animplaybackrate = 1.4;
	friendly3.animplaybackrate = 1.0;
	friendly5.animplaybackrate = 1.1;
	friendly6.animplaybackrate = 1.4;
	friendly7.animplaybackrate = 1.1;
	...
	objective_add( 2, "active", "Eliminate all enemy forces in the apartment.", level.balcony_objective_org );
	objective_current( 2 );
...
ambush_trigger()
{
	...
	for ( ;; )
	{
		...
		if ( other == level.player )
		{
			...
			wait( 4 );
			assertEx( 0, "Player should be dead by now" );
			return;
		}
...
price_moves_up_and_waves_player_on()
{
	...
	thread price_waits_at_node_and_waves( node );
...
friendly_bridge_flank_grabber()
{
	...
	// dont send guys down before price gets over and actually directs them down
	flag_wait( "price_reaches_end_of_bridge" );
	for ( ;; )
	{
...
apartment_second_floor()
{
	...
	enemies = getentarray( "window_enemies", "script_noteworthy" );
...
//	right_guy.ignoreme = true;
	...
	second_floor_door_breach_trigger waittill( "trigger" );
...
//	self animscripts\shared::placeWeaponOn( "javelin", "right" );
	...
	node = getnode( "javelin_circle_node", "targetname" );
...
//	self dodamage( self.health + 50, (0,0,0) );
	...
	jav = spawn( "weapon_javelin", (0,0,0) );
//	jav.spawnflags = 1;	
//	jav linkto( model, "TAG_ORIGIN", (0,0,0), (0,0,0) );
	...
	objective_add( 4, "active", "Get the Javelin.", jav.origin );
	objective_current( 4 );
...
price_reminds_player_about_javelin()
{
	...
	for ( ;; )
	{
		...
		if ( even )
		{
			// Carver! Pick up the Javelin and get to the second floor!! Move!!!
			level.price anim_single_queue( level.price, "jav_reminder_1" );
		}
		else
		{
			// Carver! Get to the second floor and take out those APCs!
			level.price anim_single_queue( level.price, "jav_reminder_2" );
		}
...
//	wait_until_alley_is_clear_of_enemies();
	...
	// wait until the friendly triggers have been disabled by the flag being set
	waittillframeend;
	
	activate_trigger_with_targetname( "alley_circle_trigger" );
...
bridge_attackers_attack()
{
	self.goalradius = 64;
	self.ignoreme = true;
	self endon( "death" );
	timer = debugvar( "timer2", 1.8 );
	wait( timer );
...
bridge_attackers_attack_javelin_guy()
{
	...
	level.javelin_guy waittill( "death" );
...
defend_the_roof_with_javelin()
{
	...
	bridge_attackers = getentarray( "bridge_attacker", "targetname" );
	array_thread( bridge_attackers, ::add_spawn_function, ::bridge_attackers_attack );
	array_thread( bridge_attackers, maps\_spawner::spawn_ai );
	...
	thread bridge_attackers_attack_javelin_guy();
	price_circle_node thread price_responds_to_overpass();
	price_circle_node thread anim_single_solo( level.price, "javelin_briefing" );
...
//	allies = remove_heroes_from_array( allies );
	...
	level.javelin_helper delaythread( 1, ::anim_single_queue, level.javelin_helper, "hit_vehicles" );
	
	objective_add( 4, "active", "Destroy the armored vehicles on the overpass.", roof_node.origin );
	objective_current( 4 );
	
	roof_guy waittill( "goal" ); // goal gets notified while the guy is in the goal, not just once
...
shanty_opens()
{
	delaythread( 2.3, maps\_flare::flare_from_targetname, "alley_flare" );
...
//	activate_trigger_with_targetname( "shanty_start_trigger" );
	
	
	flag_set( "shanty_open" );
	clear_colors();
	
	activate_trigger_with_targetname( "backhalf_friendly_start_trigger" );
	...
	for ( i=0; i < allies.size; i++ )
	{
		...
		allies[ i ] thread shanty_ai_think();
	}
	...
	array_thread( bog_ambient_spawners, maps\_spawner::spawn_ai );
	...
	// This way let’s go!
	level.price thread anim_single_queue( level.price, "this_way" );
/*
...
nightVis_check()
{
	thread maps\_nightvision::NVGHintPrint();
	return false;
}
