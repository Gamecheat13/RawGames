...
{	
	...
	for(;;)
	{
		...
		tagName = fakeTag;
//		if ( fakeTag == "tag_glass_front" )
//			fakeTag = "tag_glass_front_d";
...
physics_launch( partIndex, stateIndex, point, initial_velocity )
{
	...
	if ( !self.destructible_parts[ partIndex ] isLinked() )
		return;
	
	// Detach the part
	self.destructible_parts[ partIndex ] unlink();
	self.destructible_parts[ partIndex ].unlinked = true;
	self.destructible_parts[ partIndex ] physicsLaunch( point, initial_velocity );
}
...
explode( partIndex, force_min, force_max, range, mindamage, maxdamage )
{
	...
	if ( isdefined( level.destructible_type[self.destuctableInfo].parts ) )
	{
		for( i = ( level.destructible_type[ self.destuctableInfo ].parts.size - 1 ) ; i >= 0  ; i-- )
		{
			...
			{
				self.destructible_parts[ i ] notify ( "stop_damage_mirror" );
