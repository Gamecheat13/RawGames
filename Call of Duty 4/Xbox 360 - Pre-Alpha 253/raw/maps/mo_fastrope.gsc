...
//	pivot endon("custom_all");
	...
	{		
		self thread fastrope_playsound(pivot, "start");
...
fastrope_playsound(pivot, notetrack)
{
...
fastrope_ropethink(ai)
{
	...
	if(!isdefined(level.scr_anim[self.targetname][xanim]))
		return;
...
/****************************************************************************************************/
/***************************					NEW CODE				*****************************/
/*		this is some serious hacky shit that should probably be animation driven to get the			*/
/*		player to look down while he's repeling, then repel him down and get him to start			*/
/*		looking back up again AND on top of that nudge him into this spot near the window			*/
/*		that offers him a good view of the scene...it's REALLY touchy...so edit with care			*/
/****************************************************************************************************/
...
/***************************************************************************************************/
	...
	//link anim_single_solo(camera, "land");
...
fastrope_player_viewshift(heli)
{
	pang = level.player getplayerangles();
	ang = heli.angles + (0,-15,0);
	
	range = ang - pang;
	time = .075;
	frac = .01;
	num = time / frac;
	range = vector_multiply(range, 1/num);
	for(i=0; i<num; i++)
	{
		pang = level.player getplayerangles();
		level.player setplayerangles(pang + range );
		wait frac;
	}	
...
fastrope_heli_setup(name)
{
	...
	heli.modelname 		= "vehicle_blackhawk";
	heli.type 			= "blackhawk";
	...
	if(heli.player && heli.modelname == "vehicle_blackhawk")
		heli.modelname = "vehicle_blackhawk_hero";
...
fastrope_ropeanimload(xidle, xdrop, side)
{	
...
fastrope_animload(heli)
{
	...
	switch(type)
	{
		case "blackhawk":{
			...
			level.scr_anim[name]["idle"][0]		= %bh_3_idle;
			level.scr_anim[name]["grab"]		= %bh_3_begining;//-->
			level.scr_anim[name]["loop"][0]		= %bh_fastrope_loop;
			level.scr_anim[name]["land"]		= %bh_fastrope_land;
			...
			level.scr_anim[name]["idle"][0]		= %bh_7_idle;
			level.scr_anim[name]["grab"]		= %bh_7_begining;//-->
			level.scr_anim[name]["loop"][0]		= %bh_fastrope_loop;
			level.scr_anim[name]["land"]		= %bh_fastrope_land;
