...
//test(eVolume)
//{
//	while (!isdefined(eVolume.closestAI))
//		wait (0.05);
//
//	for (;;)
//	{
//		if (isdefined(eVolume.closestAI))
//			print3d (eVolume.closestAI.origin + (0,0,64), "closest", (1,1,1), 1, 0.25);
//		wait (0.05);
//	}	
//}
...
breacher_think(eVolume, sBreachType)
{
	...
	eVolume.animEnt anim_single_solo (self, sAnimStart);
...
breach_enemies_stunned(eRoomVolume)
{
	...
	if ( distance(self.origin, eRoomVolume.animEnt.origin) <= level.detpackStunRadius )
	{
		...
		self breach_set_animname("hostile");
		self.allowdeath = true;
		self anim_single_solo(self, sStunnedAnim);
		self breach_reset_animname();
	}
}
