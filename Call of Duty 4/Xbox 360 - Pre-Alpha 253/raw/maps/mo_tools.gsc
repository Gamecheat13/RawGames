...
{		
	...
	{
		array[i].mo_tools_array_wait = true;	
		array[i] thread array_waitlogic1(msg, timeout);
	}
	for(i=0;i<array.size; i++)
	{
		if(isdefined(array[i]) && array[i].mo_tools_array_wait)
			array[i] waittill("mo_tools_array_wait");	
	}
}

array_waitlogic1(msg, timeout)
{
	self array_waitlogic2(msg, timeout);	
	self.mo_tools_array_wait = false;
	self notify("mo_tools_array_wait");
}

array_waitlogic2(msg, timeout)
{
	self endon(msg);
...
ai_clear_dialog_logic(array, spawners, end_ent, end_msg, dlg_ent, dlg_msg)
{
	...
	{	
		...
		if(!isdefined(guy))
		{
			...
			battleChatter_On_thread( "allies" );
			return;
		}
		num =2;
