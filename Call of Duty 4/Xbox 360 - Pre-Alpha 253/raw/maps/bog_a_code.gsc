...
{
	self.count = 1;
	spawn = self maps\_spawner::spawn_ai();
...
delayed_spawn_and_kill( msg, timer )
{
	...
	spawn = spawner maps\_spawner::spawn_ai();
	if ( spawn_failed( spawn ) )
		return;
...
heli_guys_run_in()
{
	...
	{
		spawn = spawners[ i ] maps\_spawner::spawn_ai();
		if ( spawn_failed( spawn ) )
			continue;
...
//	wall_blocker solid();
	...
	friendly = friendly_spawner maps\_spawner::spawn_ai();
	...
	enemy = enemy_spawner maps\_spawner::spawn_ai();
...
saved_from_melee( enemy )
{
	...
	self anim_single_solo( self, self.end_melee_animation );
...
set_target_on_second_floor()
{
	self endon( "death" );
	OFFSET = ( 0, 0, 60 );
	for ( ;; )
	{
		flag_wait( "player_can_shoot_jav" );
		target_set( self, OFFSET );
		target_setAttackMode( self, "top" );
		target_setJavelinOnly( self, true );
		flag_waitopen( "player_can_shoot_jav" );
		target_remove( self );
	}
}
...
//		level.javelin_helper thread anim_single_queue( level.javelin_helper, "hit_target_" + level.bmps_killed_by_player );
		...
		radiusdamage( self.origin, 150, 10000, 10000 );
	}
}
...
//	waitSpread( 0, 9 );
	...
	bmp thread set_target_on_second_floor();
...
drop_javelin( origin )
{
	for( ;; )
	{
		...
		javelin = getentarray( "weapon_javelin", "classname" );
		if ( javelin.size )
			continue;

		// delete extra javelins
		for ( i=1; i < javelin.size; i++ )
			javelin[ i ] delete();
...
price_waits_at_node_and_waves( node )
{
	level endon( "price_flanks_apartment" );
	assertEx( !flag( "price_flanks_apartment" ), "Flag set too soon!" );
...
run_to_end_of_shanty()
{
	...
	self.goalradius = 100;
	self setGoalPos( dest.origin );
	for ( ;; )
	{
		self waittill( "goal" );
		if ( distance( self.origin, dest.origin ) > 64 )
		{
	...
	self notify( "reached_shanty_end" );
	self.interval = 96;
...
showPlaybackRate()
{
	...
	for ( ;; )
	{
		print3d( self.origin + (0,0,64), self.animPlayBackRate, (1,1,1), 1, 1 );
		wait( 0.05 );
	}
}
...
alley_balcony_guy()
{
	...
	for ( ;; )
	{
		...
		axisCount = getaiarray( "axis" ).size;
		if ( axisCount > 4 )
		{
			wait( 2 );
			continue;
...
wait_until_javelin_guy_died_or_must_die()
{
	if ( !isalive( self ) )
		return;
/#
	flag_assert( "alley_first_shot" );
#/
	level endon( "alley_first_shot" ); // comes from price's ambush notetrack
	self waittill( "death" );
}
