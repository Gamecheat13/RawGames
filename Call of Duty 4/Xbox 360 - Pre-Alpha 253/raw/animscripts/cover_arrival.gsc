...
canUseSawApproach( node )
{
	...
	if ( isDefined( self.enemy ) && distance( self.enemy.origin, self.node.origin ) < 384 )
		return false;
		
	if ( GetNodeYawToEnemy() > 30 || GetNodeYawToEnemy() < -30 )
		return false;
...
startCornerApproach( approachType, approachPoint, approachNodeYaw, approachFinalYaw, approach_dir )
{
	...
	{
		/#
		debug_arrival("approach aborted at last minute: current motion angle is not forward");
		#/
		return;
	}
...
doLastMinuteExposedApproach()
{
	...
	{
		/#
		debug_arrival("Aborting exposed approach because self.cqbwalking is true");
		#/
		return;
	}
...
alignToNodeAngles()
{
	...
	while(1)
	{
		if ( !isdefined( self.node ) || !isdefined( self.pathGoalPos ) || distanceSquared( self.node.origin, self.pathGoalPos ) > 1 )
			return;
...
startCornerLeave()
{
	...
	if ( isdefined( self.prevNode ) )
	{
		...
		if ( isdefined( self.prevNode.approachtype ) && self.prevNode.approachtype != "exposed" && self.prevNode.approachtype != "stand_saw" && self.prevNode.approachType != "crouch_saw" )
		{
			...
			anglediff = AbsAngleClamp180( self.angles[1] - self.prevNode.angles[1] - getNodeStanceYawOffset( self.prevNode.approachtype ) );
...
faceEnemyOrMotionAfterABit()
{
	...
	while ( isdefined( self.pathGoalPos ) && distanceSquared( self.origin, self.pathGoalPos ) < 150*150 )
		wait .25;
...
calculateNodeTransitionAngles( angleDataObj, approachtype, isarrival, arrivalYaw, approach_dir, maxDirections, excludeDir )
{
	...
	// if we're trying to run around cover in one direction,
	// don't run in a direction on the other side of excludeDir
	if ( excludeDir == 9 )
	{
		excludeDirYaw = arrivalYaw + 45;
		approachDirYaw = vectorToAngles( approach_dir )[1];
		yawDiff = AngleClamp180( approachDirYaw - excludeDirYaw );
		if ( yawDiff > 0 && yawDiff < 135 )
		{
			angleDataObj.transitions[6] = -1.0002;
			angleDataObj.transitions[3] = -1.0001;
		}
		else if ( yawDiff < 0 && yawDiff > -135 )
		{
			angleDataObj.transitions[8] = -1.0002;
			angleDataObj.transitions[7] = -1.0001;
		}
	}
	else if ( excludeDir == 7 )
	{
		excludeDirYaw = arrivalYaw - 45;
		approachDirYaw = vectorToAngles( approach_dir )[1];
		yawDiff = AngleClamp180( approachDirYaw - excludeDirYaw );
		...
		{
			angleDataObj.transitions[8] = -1.0002;
			angleDataObj.transitions[9] = -1.0001;
		}
		else if ( yawDiff < 0 && yawDiff > -135 )
		{
			angleDataObj.transitions[4] = -1.0002;
			angleDataObj.transitions[1] = -1.0001;
		}
	}
}
...
checkCoverExitPos( exitpoint, exityaw, exittype, approachNumber )
{
	...
	if ( approachNumber <= 6 || exittype == "exposed" || exittype == "exposed_crouch" )
		return true;
