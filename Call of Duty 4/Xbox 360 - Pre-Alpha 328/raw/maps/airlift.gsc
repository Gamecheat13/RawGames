...
main()
{
	...
	add_start( "cobraflight", ::start_cobraflight );
	...
	maps\_treadfx::setallvehiclefx( "seaknight", undefined ); //disables seaknight treads, but not seaknight_player
...
dialogue_smoketown()
{
	...
	level.crewchief dialogue_execute( "airlift_hcc_letsgo" );
	...
	level.vasquez thread dialogue_execute( "airlift_vsq_moveout" );	
	...
	level.vasquez dialogue_execute( "airlift_vsq_gettolz" );	
...
orange_smoke()
{
	...
	BlueSmokeEffect = spawnFx( getfx("smoke_blue_signal"), org_orange_smoke.origin );
	triggerFx( BlueSmokeEffect );
	//flag_wait( "cobraCrashStart" );
	flag_wait( "cobra_hit" );
	BlueSmokeEffect delete();
}
...
smoketown_flyover()
{
	...
	if ( level.usingstartpoint )
	{
		iprintlnbold( "skipping function. using startpoint" );
		return;
	}
...
smoketown_land()
{
	...
	eNode anim_reach_solo (level.smokeAT4dude, "RPG_conceal_idle_start");
	...
	level.smokeAT4dude anim_single_solo( level.smokeAT4dude, "RPG_conceal_2_standR" );
	...
	flag_wait( "player_smoketown_junkyard" );
	level.smokeAT4dude notify( "stop magic bullet shield" );
	level.smokeAT4dude delete();
...
smoketown_lz_advance()
{
	...
	for(i=0;i<aAI_to_delete.size;i++)
	{
		...
		if ( !isalive( aAI_to_delete[i] ) )
			continue;
		aAI_to_delete[i] notify( "stop magic bullet shield" );
...
smoketown_enemy_heli()
{
	...
	-------------------------*/
	delaythread (0, maps\_vehicle::create_vehicle_from_spawngroup_and_gopath, 40 );
...
smoketown_at4_impact( smoketown_at4_hostile, attractor )
{
	...
	playfx( getfx( "palace_at4" ), smoketown_physics_explosion.origin );
	wait(0.1);
...
{	
	...
	while( 1 )
	{
		musicPlay( "airlift_cobradown_music" );
		wait( 96 );
	}
}
...
dialogue_cobraflight()
{
	...
	thread autosave_by_name( "cobraflight" );
...
//	maps\_vehicle::delete_group( missile_source );	
	...
	dummy linkto( level.wingman, "tail_rotor_jnt" );
...
cobra_crash_fx()
{
	...
	wait(6.5);
	cobraCrash03 playsound( "airlift_cobra_crash_end_02" );
	...
	playfx ( getfx( "smoke_cobra_crash" ), cobra_smoke.origin );	
...
cobrastreets_crewchief_think()
{
	...
	level.crewchief notify( "stop magic bullet shield" );
...
cobra_streetfight()
{
	...
	setdvar( "friendlyfire_enabled", "0" ); //set because the seaknight will sometimes land on hidden friendlies in cobrastreets
	...
	//level.cobrapilot = dronespawn( spawner );
	...
	setdvar( "friendlyfire_enabled", "1" ); //set because the seaknight will sometimes land on hidden friendlies in cobrastreets
	level.seaknight waittill ( "all_ai_unloaded" );
	...
	-------------------------*/	
	ent = getent( "obj_rescue_pilot", "targetname" );
	Seaknightrepulsor = Missile_CreateRepulsorEnt( ent, 5000, 100 );
...
cobra_crashsite_think()
{
	...
	repulsor = Missile_CreateRepulsorEnt( eTarget, 5000, 100 );
...
cobrapilot_think()
{
	...
	level.ePlayerview lerp_player_view_to_tag( "tag_player", 0.5, 0.9, 35, 35, 45, 0 );
...
music_nuke()
{
	...
	//musicPlay( "icbm_combat_tension_music" );
...
dialogue_nuke()
{
	...
	radio_dialogue_queue( "airlift_hqr_confirmed" );
	flag_wait( "nuke_explodes");
	//HACK.....CUT OFF DIALOGUE!!!!!!!!
	
	flag_wait( "shockwave_about_to_hit_player");
	//Lt. Vasquez
	//Everyone hang onnnn!!!
	radio_dialogue_queue( "airlift_vsq_hangon" );
}
...
nuke_flight()
{
	...
	if ( isdefined( level.cobrapilot ) )
		...
		level.ePlayerview waittillmatch ( "single anim", "end" );
...
nuke()
{
	...
	level.seaknight thread nuke_seaknight_fx();
	...
	level.seaknight thread play_loop_sound_on_entity ("ch46_helicopter_dying_loop");
	level.player playlocalsound( "airlift_ch46_nuke_exp" );
	...
	level.player playloopsound( "shellshock_loop" );
	level.player freezeControls(true);
	level.seaknight notify ( "stop sound" + "ch46_helicopter_dying_loop" );
	level.seaknight notify ( "stop sound" + "airlift_heli_alarm_loop" );
	level.seaknight StopSounds();
	level.seaknight delete();
	wait(.5);
	level.player StopSounds();
	level.player thread play_sound_in_space( "ceiling_collapse" );
	wait(1);
	level.player thread play_sound_in_space( "airlift_cobra_crash_end_02" );

	wait(2);
	level.player play_sound_in_space( "breathing_better" );
	wait(6.5);
	changeLevel( "aftermath", false );
}
...
nuke_seaknight_fx()
{
	while( isdefined( self ) )
	{
		playfxOnTag( getfx( "seaknight_nuke_dust" ), self, "tag_door_rear");
		playfxOnTag( getfx( "seaknight_nuke_haze" ), self, "tag_door_rear");
		wait .05;
	}
...
{
	self waittill( "death" );
	earthquake( 0.1, 2, self.origin, 8000);
}
...
nuke_trees()
{
	...
	/*-----------------------
	FIND THE FURTHEST TREE AWAY TO GET THE MULTIPLIER
	-------------------------*/	
	eFurthestTree = getFarthest( nuke_origin.origin, nuke_trees );
...
initPrecache()
{
	...
	precachestring( &"SCRIPT_PLATFORM_HINT_MOUNT_MARK19" );
...
seaknight_player_triggers()
{
	...
	assert( isdefined( trigger_seaknight_gun ) );
   	trigger_seaknight_gun.origin = level.seaknight gettagorigin( "tag_player" );	
}
