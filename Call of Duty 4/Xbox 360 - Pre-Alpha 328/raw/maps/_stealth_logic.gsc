...
enemy_event_awareness( type )
{
	...
	while( 1 )
	{		
		self waittill( type, var1, var2, var3, var4, var5 );
...
enemy_corpse_death()
{
	...
	//stop listeners on a corpse if it hasn't been deleted
	if( isdefined( self ) )
	{
		for(i=0; i<self._stealth.logic.event.listener.size; i++)
			self removeAIEventListener( self._stealth.logic.event.listener[ i ] );
	}
	//this means the ai was deleted - not killed
	if( !isdefined( self.origin ) )
		return;
	
	//corpses have some body mass, so we add 6 inches to compensate for any foilage clip
	corpse = spawn("script_origin", self.origin + (0,0,6) );
	
	corpse.targetname = "corpse";
	corpse.ai_number = id;
	corpse.script_noteworthy = corpse.targetname + "_" + corpse.ai_number;

	//this wait give the body a chance to fall...it delays the ai
	//logic which makes them look more human, but hopefully in the 
	//future it will also allo the origin to be in a more accurate spot
	//because reguardless of where the body falls right now, the "corpse"
	//is always at the origin of when he was alive
	wait 1;
