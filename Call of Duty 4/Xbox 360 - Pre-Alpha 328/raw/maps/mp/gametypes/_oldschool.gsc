...
init()
{
	...
	/*
	Stopping Power = specialty_bulletdamage
	Juggernaut = specialty_armorvest
	Double Tap = specialty_rof
	Last Stand = specialty_pistoldeath
	Martyrdom = specialty_grenadepulldeath
	Sleight of Hand = specialty_fastreload
	*/
}
...
trackPickup( pickup, id )
{
	...
	while(1)
	{
		...
		else
		{
			...
			player thread hidePerkNamesAfterTime( 3.0 );
			
			player.numPerks++;
		}
		
		thread playSoundinSpace( "oldschool_pickup", origin );
		...
		pickup playSound( "oldschool_return" );
...
hidePerkNamesAfterTime( delay )
{
	...
	self thread hidePerk( 0, 2.0, true );
	self thread hidePerk( 1, 2.0, true );
	self thread hidePerk( 2, 2.0, true );
