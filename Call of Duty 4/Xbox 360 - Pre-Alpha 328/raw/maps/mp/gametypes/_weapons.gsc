...
//		if ( getDvarInt( "scr_stow_weapons" ) > 0 )
...
claymoreDetonation()
{
	...
	while(1)
	{
		...
		if ( getdvarint("scr_debug_claymores") != 1 )
		{
...
shouldAffectClaymore( claymore )
{
	...
	dirToPos = vectornormalize( pos - claymore.origin );
...
detach_all_weapons()
{
	...
	{
		detach_model = getWeaponModel( self.tag_stowed_back );

		if ( detach_model == "weapon_rpg7" )
			detach_model = "weapon_rpg7_stow";
...
attach_primary()
{
	...
	weapon_model = getWeaponModel( current );
	self detach( weapon_model, "tag_weapon_right" );
	self attach( weapon_model, "tag_weapon_right" );
}
...
stow_on_back()
{
	...
	{
		self.tag_stowed_back = "rpg_mp";
	}
	else if ( self hasWeapon( "at4_mp" ) && current != "at4_mp" )
	{
		self.tag_stowed_back = "at4_mp";
	}
	else
	{
		for ( idx = 0; idx < self.weapon_array_primary.size; idx++ )
		{
			if ( self.weapon_array_primary[idx] == current )
				continue;
			...
			self.tag_stowed_back = self.weapon_array_primary[idx];
		}
	}
	...
	weapon_model = getWeaponModel( self.tag_stowed_back );

	if ( weapon_model == "weapon_rpg7" )
		weapon_model = "weapon_rpg7_stow";
