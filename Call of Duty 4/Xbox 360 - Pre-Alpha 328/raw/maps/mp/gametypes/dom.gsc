...
//	allowed[1] = "hardpoint";
	...
	maps\mp\gametypes\_rank::registerScoreInfo( "capture", 15, &"MP_NULL" );

	maps\mp\gametypes\_rank::registerScoreInfo( "defend", 10, &"MP_NULL" );
	maps\mp\gametypes\_rank::registerScoreInfo( "defend_assist", 2, &"MP_NULL" );

	maps\mp\gametypes\_rank::registerScoreInfo( "assault", 10, &"MP_NULL" );
	maps\mp\gametypes\_rank::registerScoreInfo( "assault_assist", 2, &"MP_NULL" );
...
//		[[level._setTeamScore]]( oldTeam, getTeamFlagCount( oldTeam ) );
	...
	if ( team != "neutral" )
	{
		if ( oldTeam == "neutral" )
		{
			thread printOnTeamArg( &"MP_NEUTRAL_FLAG_CAPTURED_BY", "allies", player.name );
			thread printOnTeamArg( &"MP_NEUTRAL_FLAG_CAPTURED_BY", "axis", player.name );
		}
		else
		{
...
//		[[level._setTeamScore]]( team, getTeamFlagCount( team ) );
		...
		{
			self.touchList[team][players[index]] thread [[level.onXPEvent]]( "capture" );
			maps\mp\gametypes\_globallogic::givePlayerScore( "capture", self.touchList[team][players[index]] );
		}
