...
setPickedUp( player )
{
	if ( isDefined( player.carryObject ) )
	{
		if ( isDefined( self.onPickupFailed ) )
			self thread [[self.onPickupFailed]]( player );
	...
	if ( isDefined( self.onPickup ) )
		self thread [[self.onPickup]]( player );
...
//		dropOrigin = self.safeOrigin;
	...
	if ( isDefined( self.onDrop ) )
		self thread [[self.onDrop]]( self.carrier );
...
updateCarrierHeadicon()
{
	...
	self.carrier.headicon = self.worldIcons[ relativeTeam ];
	...
	while ( true )
	{
		...
		if ( isDefined( self.keyObject ) && (!isDefined( player.carryObject ) || player.carryObject != self.keyObject ) )
		{
			if ( isDefined( self.onCantUse ) )
				self thread [[self.onCantUse]]( player );
			continue;
		}
		...
		if ( self.useTime > 0 )
		{
			if ( isDefined( self.onBeginUse ) )
				self thread [[self.onBeginUse]]( player );

			result = self useHoldThink( player );
			
			if ( isDefined( self.onEndUse ) )
				self thread [[self.onEndUse]]( player, result );	
		}
...
useObjectProxThink()
{
	...
	while ( true )
	{
		if ( self.useTime && self.curProgress >= self.useTime )
		{
			...
			if ( isDefined( self.onEndUse ) )
				self thread [[self.onEndUse]]( self.claimPlayer, true );

			if ( isDefined( self.onUse ) )
				self thread [[self.onUse]]( self.claimPlayer );
		...
		if ( self.claimTeam != "none" )
		{
			if ( self.useTime )
			{
				...
				{
					if ( isDefined( self.onEndUse ) )
						self thread [[self.onEndUse]]( self.claimPlayer, false );
		...
		if ( self canInteractWith( player.pers["team"] ) && self.claimTeam == "none" )
		{
			if ( !isDefined( self.keyObject ) || (isDefined( player.carryObject ) && player.carryObject == self.keyObject ) )
			{
				...
				if ( self.useTime && isDefined( self.onBeginUse ) )
					self thread [[self.onBeginUse]]( self.claimPlayer );
			}
			else
			{
				if ( isDefined( self.onCantUse ) )
...
triggerTouchThink( object )
{
	...
	object.touchList[team][touchName] = self;
...
useHoldThink( player )
{
	...
	if ( isDefined( player ) )
	{
		if ( isDefined( player.attachedUseModel ) && player.attachedUseModel )
		{
			player detach( "prop_suitcase_bomb", "tag_inhand" );
			player.attachedUseModel = false;
		}
...
useHoldThinkLoop( player, lastWeapon )
{
	...
	while( isAlive( player ) && player isTouching( self.trigger ) && player useButtonPressed() && self.curProgress < self.useTime && (self.useRate || waitForWeapon) && !(waitForWeapon && timedOut > 1.0) )
	{
		timedOut += 0.05;
