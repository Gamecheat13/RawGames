...
placeSpawnPoints( spawnPointName )
{
	...
	{
		origin = spawnPoints[index].origin;
	...
	if (numplayers > 0)
	{
		for (i = 0; i < spawnpoints.size; i++)
		{
			...
			if ( pretendEvenTeams )
			{
				...
				spawnpoints[i].weight = (enemyDistSum - alliedDistanceWeight*allyDistSum) / 2;
			}
			else
				spawnpoints[i].weight = (enemyDistSum - alliedDistanceWeight*allyDistSum) / numplayers;
		}
	}
	prof_end(" sumdists");
...
avoidWeaponDamage(spawnpoints)
{
	...
	for (i = 0; i < spawnpoints.size; i++)
	{
		...
		for ( j = 0; j < level.artilleryDangerCenters.size; j++ )
		{
			artilleryDangerCenter = level.artilleryDangerCenters[j];
			centerdir = spawnpoints[i].origin - artilleryDangerCenter;
...
lastMinuteSightTraces( spawnpoint )
{
	...
	{
		if ( bullettracepassed( closest.origin       + (0,0,50), spawnpoint.origin + (0,0,50), false, undefined) )
			return true;
	}
	if ( isdefined( secondClosest ) )
	{
		if ( bullettracepassed( secondClosest.origin + (0,0,50), spawnpoint.origin + (0,0,50), false, undefined) )
			return true;
	}
