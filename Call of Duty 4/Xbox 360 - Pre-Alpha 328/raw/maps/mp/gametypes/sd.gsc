...
onStartGameType()
{
	...
	maps\mp\gametypes\_rank::registerScoreInfo( "kill", 30, &"MP_KILL" );
	maps\mp\gametypes\_rank::registerScoreInfo( "headshot", 30, &"MP_HEADSHOT" );
	maps\mp\gametypes\_rank::registerScoreInfo( "assist", 6, &"MP_ASSIST" );
...
onDeadEvent( team )
{
	...
	if ( team == "all" )
	{
		if ( level.bombPlanted )
		{
			...
			sd_endGame( game["attackers"], game["strings"][game["attackers"]+"_mission_accomplished"] );
		}
		else
		{
	...
	{
		announcement( game["strings"][game["defenders"]+"_eliminated"] );
...
onTimeLimit()
{
	if ( level.teamBased )
	{
		...
		announcement( game["strings"]["time_limit_reached"] ); // more informative
		sd_endGame( game["defenders"], game["strings"]["time_limit_reached"] );
	}
	else
	{
		announcement( game["strings"]["time_limit_reached"] );
...
updateGametypeDvars()
{
	level.plantTime = dvarFloatValue( "planttime", 5.0, 0, 20 );
...
bombs()
{
	...
	if ( !level.multiBomb )
	{
		...
		level.sdBomb.showHeadicon = true;
	...
	for ( index = 0; index < bombZones.size; index++ )
	{
		...
		{
			badtrig = getent( visuals[0].target, "targetname" );
...
onUse( player )
{
	...
	if ( !self maps\mp\gametypes\_gameobjects::isFriendlyTeam( player.pers["team"] ) )
	{
		...
		if ( !level.multiBomb )
		{
			self.keyObject maps\mp\gametypes\_gameobjects::allowCarry( "none" );
			self.keyObject maps\mp\gametypes\_gameobjects::setVisibleTeam( "none" );
			self.keyObject maps\mp\gametypes\_gameobjects::setDropped();
		}
...
bombPlanted( objects )
{
	...
	objects[0] thread maps\mp\gametypes\_globallogic::playTickingSound();
	...
	objects[0] maps\mp\gametypes\_globallogic::stopTickingSound();
...
bombDefused( objects )
{
	...
	// this is redundant when it just said "x defused the bomb"
	//announcement( game["strings"][game["defenders"]+"_mission_accomplished"] );
	sd_endGame( game["defenders"], game["strings"][game["defenders"]+"_mission_accomplished"] );
}
