...
isBestPlayerOnTeam( team, ignore )
{
	if ( level.placement[team].size == 0 )
		return false;
	
	bestscore = level.placement[team][0].score;
	foundone = false;
	for ( i = 0; i < level.placement[team].size; i++ )
	{
		player = level.placement[team][i];
		
		if ( isdefined( ignore ) && player == ignore )
			continue;
		
		if ( foundone && player.score != bestscore )
			break;
...
ch_kills( data, time )
{
	...
	if ( player.killedPlayers[data.victim.name] == 10 )
	...
	if ( data.victim.score > 0 )
	{
		if ( level.teambased )
		{
			...
			{
				if ( data.victim isBestPlayerOnTeam( victimteam ) && level.placement[victimteam].size >= 3 )
					player killedBestEnemyPlayer();
			}
		}
		else
		{
			if ( data.victim isBestPlayerOnTeam( "all", player ) && level.players.size >= 4 )
			{
				player killedBestEnemyPlayer();
			}
		}
	...
	if ( data.sMeansOfDeath == "MOD_PISTOL_BULLET" || data.sMeansOfDeath == "MOD_RIFLE_BULLET" )
	{
		...
		if ( isMG( data.sWeapon ) )
			...
			player processChallenge( "ch_marksman_dragunov_" );
			...
			player processChallenge( "ch_marksman_barrett_" );
		else if ( isStrStart( data.sWeapon, "mp44_" ) )
			player processChallenge( "ch_marksman_mp44_" );
			...
			player processChallenge( "ch_marksman_remington700_" );
			...
			player processChallenge( "ch_marksman_m21_" );
		else if ( isStrStart( data.sWeapon, "m40a3_" ) )
			player processChallenge( "ch_marksman_m40a3_" );
		else if ( isStrStart( data.sWeapon, "_" ) )
			player processChallenge( "ch_marksman__" );
	}
	else if ( isSubStr( data.sMeansOfDeath, "MOD_GRENADE" ) || isSubStr( data.sMeansOfDeath, "MOD_EXPLOSIVE" ) || isSubStr( data.sMeansOfDeath, "MOD_PROJECTILE" ) )
	{
		...
		if ( isDefined( data.victim.explosiveInfo["damageTime"] ) && data.victim.explosiveInfo["damageTime"] == time )
		{
			...
			expId = data.victim.explosiveInfo["damageId"];
			if ( !isDefined( player.explosiveKills[expId] ) )
			{
				player.explosiveKills[expId] = 0;
				player thread clearIDAfterFrame( expId );
			}
...
ch_roundplayed( data )
{
	...
	if ( player.wasAliveAtMatchStart )
	{
		...
		if ( deaths == 0 && maps\mp\gametypes\_globallogic::getTimePassed() / 1000 > 7 )
...
monitorMisc()
{
	...
	while(1)
	{
		result = self waittill_any_return( "destroyed_explosive", "begin_airstrike", "destroyed_car", "destroyed_helicopter" );
