...
//	precachemodel( level.cobra_missile_models["cobra_Sidewinder"] );
	...
	level.heli_sound["allies"]["spin"] = "cobra_helicopter_dying_loop";
	...
	level.heli_sound["axis"]["spin"] = "hind_helicopter_dying_loop";
...
heli_damage_monitor()
{
	...
	for( ;; )
	{
		...
		if( !heli_friendlyfire )
			continue;
		if(	heli_friendlyfire && attacker == self.owner )
			continue;
...
spinSoundShortly()
{
	...
	self playSound( level.heli_sound[self.team]["spin"] );
}
...
check_owner()
{
	if( !isdefined( self.owner.pers["team"] ) || self.owner.pers["team"] != self.team )
	{
