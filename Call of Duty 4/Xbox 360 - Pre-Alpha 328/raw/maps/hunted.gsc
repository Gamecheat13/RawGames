...
/*
	...
	- Road
		- Make more and more ai move away the longer the player is stealthy.
	- Greenhouse
		- Get code for the stinger.
		- Check the placement of friendlies during the stinger sequence.
		- Add nag to follow price once the helicopter is down.
	-	AC130
		- Make the two guys running towards the player run more to their right, and go into cover as they get closer to the player.
		- Make the tank kill the player if he moves up to far or opens fire.
		- Make enemies react and take cover if player moves up to far.
		- Block road down by the bridge.
		- Make dazed enemies move more to their right.
	- Generic
...
dirt_path_helicopter()
{
	...
	helicopter thread damage_hints_attach();
...
//	axis = get_ai_group_ai( "barn_enemies" );
//	array_thread( axis, ::set_goalradius, 2048 );
...
//	farmer detachall();
	...
	ai_friendlyFireBlockDuration = getdvarfloat( "ai_friendlyFireBlockDuration" );
...
//	array_thread( get_ai_group_ai( "barn_enemies" ), ::expand_goalradius_ongoal );
	...
	{
//		anim_ent thread anim_single_solo( farmer, "farmer_altending");
	...
	setsaveddvar("ai_friendlyFireBlockDuration", ai_friendlyFireBlockDuration );
}
...
barn_interrogation_anim( anim_ent, actors, farmer )
{
	...
	door rotateyaw( 100, 0.7, 0.5, 0.2 );
	door connectpaths();

//	farmer setgoalpos( (440, -3606, 135) );
//	farmer.goalradius = 4;
//	farmer waittill( "goal" );
...
field_dialgue()
{
	...
	flag_wait_or_timeout( "field_spoted", 20 );

	flag_set( "field_moveon" );

	if ( !flag( "field_spoted" ) )
	{
		level.price queue_anim( "helismoving" );
...
field_stay_down()
{
	...
	level.price queue_anim( "staydown" );
}
...
/*
	...
	field_blocker playsound( "wood_door_fisthit" );
...
field_helicopter()
{
	...
	level.helicopter thread damage_hints_attach();
...
basement_flash()
{
	...
	level.mark thread queue_anim( "warn_flashbang" );
...
farm_dialogue()
{
	...
	wait 2;
	level.price thread queue_anim( "staysharp" );
...
greenhouse_stinger()
{
	...
	level.heli_guy_health_multiplier = 1;
	level.heli_guy_respawn_delay = 12;
...
area_ac130_init()
{
	...
	*/
	...
	flag_wait_or_timeout( "mission_end_trigger", 20 );
...
ac130_gas_station()
{
	...
	PhysicsExplosionSphere( (6432, 11312, 200), 600, 350, 4 );
}
...
set_flag_on_player_action( flag_str, flash, grenade )
{
	...
	while ( true )
	{
		...
		if ( msg == "weapon_fired" )
			break;
		if ( msg == "player_frag" && isdefined( grenade ) && level.player isthrowinggrenade() )
		{
			wait 5;
			break;
		}
		if ( msg == "player_flash" && isdefined( flash ) && level.player isthrowinggrenade() )
		{
			wait 2;
			break;
		}
...
damage_hints()
{
	...
	add_hint_string( "armor_damage", &"HUNTED_DAMAGE_HINT", undefined );
...
fire_trigger()
{
	...
	while( true )
	{
		...
		while( level.player istouching( trigger ) )
		{
			count++;
			if ( count > 10 )
				level.player dodamage( level.player.health * 2.5 , trigger.origin );

			level.player dodamage( 50, trigger.origin );
			wait 0.25;
		}
