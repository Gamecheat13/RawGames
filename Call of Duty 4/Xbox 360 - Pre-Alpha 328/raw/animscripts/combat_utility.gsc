...
cheatAmmoIfNecessary()
{
	...
	self.nextCheatTime = gettime() + 2000;
...
shootUntilShootBehaviorChange()
{
	...
	{
		if ( isDefined( self.enemy ) && self.enemy != level.player && isSentient( self.enemy ) )
...
considerChangingTarget( throwingAt )
{
	if ( !isPlayer( throwingAt ) && self.team == "axis" )
	{
		if ( gettime() < anim.grenadeTimers[self.activeGrenadeTimer] )
		{
			...
			{
				if ( abs( self GetYawToSpot( level.player.origin ) ) < 60 )
				{
...
{
	if ( weapon == "aw50" ) return true;
	if ( weapon == "dragunov" ) return true;
	if ( weapon == "m14_scoped" ) return true;
	if ( weapon == "m14_scoped_silencer" ) return true;
	if ( weapon == "m14_scoped_silencer_woodland" ) return true;
	if ( weapon == "m40a3" ) return true;
	if ( weapon == "remington700" ) return true;
	return false;
}
