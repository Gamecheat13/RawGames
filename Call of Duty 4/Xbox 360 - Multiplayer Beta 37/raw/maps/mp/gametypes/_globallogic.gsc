...
default_onScoreLimit()
{
	...
	// well, because it's the easiest way to avoid the round-end screen with our endGame function that's trying to do too much. =(
	// i'd like to fix it but i'd also like to redesign the whole end of round functionality anyway.
...
updateGameEvents()
{
	if ( level.rankedMatch && !level.inGracePeriod && level.teamBased )
	{
...
spawnPlayer()
{
	...
	// TODO: remove or fix this... it shouldn't be based on grace period	
	...
	self logstring( "Spawned " + self.origin[0] + " " + self.origin[1] + " " + self.origin[2] );
...
endGame( winner, endReasonText )
{
	...
	if ( level.subRound )
	{
		...
		roundEndWait( level.subRoundEndDelay );
		
		if ( level.subRoundOnly )
		{
		...
		endReasonText = level.realEndReasonText;
	}
		...
		if ( !level.skipRoundEnd )
		{
			...
			if ( level.displayHalftimeText )
			{
				...
				for ( index = 0; index < players.size; index++ )
				{
					...
					player thread maps\mp\gametypes\_hud_message::teamOutcomeNotify( level.halftimeType, true, level.halftimeSubCaption );
					player setClientDvar( "ui_hud_hardcore", 1 );
				}
...
roundEndWait( defaultDelay )
{
	...
	for ( index = 0; index < players.size; index++ )
	{
		if ( players[index].notifyQueue.size > 0 )
		{
			nextNotifyData = players[index].notifyQueue[0];
			
			newQueue = [];
			for ( i = 1; i < players[index].notifyQueue.size; i++ )
				players[index].notifyQueue[i-1] = players[index].notifyQueue[i];
			players[index].notifyQueue[i-1] = undefined;
			
			players[index] thread maps\mp\gametypes\_hud_message::showNotifyMessage( nextNotifyData );
		}
	}
...
menuAutoAssign()
{
	...
	if ( level.teamBased )
	{
		...
		if ( level.rankedMatch )
			self logString( "team: " + assignment );
	}
	else
	{
		self.sessionteam = "none";
...
//		playSoundOnPlayers( "US_mpc_mission_draw", "allies" );
	...
	{
		iPrintLn( game["strings"]["allies_win"] );
	...
	{
		iPrintLn( game["strings"]["axis_win"] );
...
Callback_StartGameType()
{
	...
	if ( !isDefined( game["gamestarted"] ) )
	{
		...
		game["strings"]["waiting_for_match"] = &"MP_WAITING_MATCH";
	...
	level.subRound = false;
	level.subRoundOnly = false;
...
//	self thread fakeLag();
	...
	if ( level.hardcoreMode )
		self setClientDvar( "cg_hudGrenadeIconMaxRangeFrag", 0 );
//	else
//		self setClientDvar( "cg_hudGrenadeIconMaxRangeFrag", 250 );
...
//	self maps\mp\gametypes\_weapons::updateWeaponUsageStats();
	...
	self logstring( "died: damage from " + sMeansOfDeath + "(" + sWeapon + "), attacker: " + attackerString + ", damage:" + iDamage + ", hitLoc:" + sHitLoc + " at " + self.origin[0] + " " + self.origin[1] + " " + self.origin[2] );
	...
	// add "artillery_mp" for airstrike kill cam
	...
	if ( doKillcam && level.killcam && !level.splitscreen )
	{
		...
		self maps\mp\gametypes\_killcam::killcam( lpattacknum, killcamentity, sWeapon, postDeathDelay + deathTimeOffset, psOffsetTime, willRespawnImmediately, timeUntilRoundEnd(), perks );
	}
...
getPerks( player )
{
	...
	if ( isPlayer( player ) && !level.oldschool )
	{
		...
		if ( level.onlineGame && !isdefined( player.pers["isBot"] ) && isSubstr( player.pers["class"], "CLASS_CUSTOM" ) && isdefined(player.custom_class) )
		{
...
Callback_PlayerLastStand( eInflictor, attacker, iDamage, sMeansOfDeath, sWeapon, vDir, sHitLoc, psOffsetTime, deathAnimDuration )
{
	...
	notifyData = spawnStruct();
	notifyData.titleText = "Last Stand!";
