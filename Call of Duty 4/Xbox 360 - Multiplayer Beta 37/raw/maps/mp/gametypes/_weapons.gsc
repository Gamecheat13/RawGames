...
stow_on_back()
{
	...
	else
	{
		for ( idx = 0; idx < self.weapon_array_primary.size; idx++ )
		{
			...
			if ( isDefined( self.custom_class ) && isDefined( self.custom_class[self.class_num]["camo_num"] ) )
				self.tag_stowed_back = getWeaponModel( self.weapon_array_primary[idx], self.custom_class[self.class_num]["camo_num"] );
			else
				self.tag_stowed_back = getWeaponModel( self.weapon_array_primary[idx], 0 );
		}
	}
