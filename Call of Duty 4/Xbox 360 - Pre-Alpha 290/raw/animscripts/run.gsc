...
MoveCrouchRunNormal()
{
	...
	self setanimknob( GetCrouchRunAnim(), 1, 0.4 );

	animWeights = animscripts\utility::QuadrantAnimWeights( self getMotionAngle() );
	self setanim(%crouch_fastwalk_F, animWeights["front"], 0.2, 1);
...
{
	animWeights = animscripts\utility::QuadrantAnimWeights( self getMotionAngle() );
	self setanim(frontAnim, animWeights["front"], 0.2, self.moveplaybackrate );
	self setanim(backAnim,  animWeights["back"] , 0.2, self.moveplaybackrate );
	self setanim(leftAnim,  animWeights["left"] , 0.2, self.moveplaybackrate );
	self setanim(rightAnim, animWeights["right"], 0.2, self.moveplaybackrate );
}
