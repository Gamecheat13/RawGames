...
init()
{
	...
	if ( level.xenon && getDvarInt( "xblive_privatematch" ) == 0 )
...
{	
	...
	matchStartTimer = createServerTimer( "objective", 2.0 );
...
spawnPlayer()
{
	...
	if ( game["state"] != "prematch" )
	{
		if ( level.inPrematchPeriod )
		{
			...
			team = self.pers["team"];
			thread maps\mp\gametypes\_hud_message::notifyMessage( game["strings"][team + "_name"], undefined, game["icons"][team], game["colors"][team], game["music"]["spawn_" + team] );
			thread maps\mp\gametypes\_hud_playerscore::showClientScoreBar( 5.0 );
		}
		else
		{
...
testMenu()
{
	...
	{
		wait ( 10.0 );
		self thread maps\mp\gametypes\_hud_message::outcomeNotify( "allies" );
	}
}
...
//		hp = hps[randomInt(hps.size)];
		...
		{
			self thread maps\mp\gametypes\_hud_message::hintMessage( level.streakHints[3], level.hardpointHints[hp] );
...
endGame( winner )
{
	...
	for ( index = 0; index < players.size; index++ )
	{
		...
		// load level gameplay summary into dvars for displaying in lobby menu, xboxlive only
		if( isdefined( player.summary["xp"] ) && isdefined( player.summary["score"] ) && isdefined( player.summary["challenge"] ) && isdefined( player.summary["match"] ) )
		{
			player setclientdvar( "player_summary_xp", player.summary["xp"] );
			player setclientdvar( "player_summary_score", player.summary["score"] );
			player setclientdvar( "player_summary_challenge", player.summary["challenge"] );
			player setclientdvar( "player_summary_match", player.summary["match"] );
		}
...
updateGameScores( winner )
{
	if ( level.teamBased )
	{
		...
		if ( winningTeam != "tie" )
		{
			...
			winnerSPM = maps\mp\gametypes\_rank::getScoreInfoValue( "win" );
			loserSPM = maps\mp\gametypes\_rank::getScoreInfoValue( "loss" );
		}
		else
		{
			winnerSPM = maps\mp\gametypes\_rank::getScoreInfoValue( "tie" );
			loserSPM = maps\mp\gametypes\_rank::getScoreInfoValue( "tie" );
		}
		...
		{
			winnerScore = int(winnerSPM * (game["timepassed"] / 60) );
			loserScore = int(loserSPM * (game["timepassed"] / 60) );
		}
		else
		{
			winnerScore = int(winnerSPM * level.timeLimit);
			loserScore = int(loserSPM * level.timeLimit);
		}
		...
		for( i = 0; i < players.size; i++ )
		{
			...
			if ( winningTeam == "tie" )
			{
				playerScore = int( winnerScore * (player.timePlayed["total"] / game["timepassed"]) );
				player maps\mp\gametypes\_rank::giveRankXP( "tie", playerScore );
			}
			else if ( isDefined( player.pers["team"] ) && player.pers["team"] == winningTeam )
			{
				playerScore = int( winnerScore * (player.timePlayed["total"] / game["timepassed"]) );
				player maps\mp\gametypes\_rank::giveRankXP( "win", playerScore );
			}
			else if ( isDefined(player.pers["team"] ) && player.pers["team"] == losingTeam )
			{
				playerScore = int( loserScore * (player.timePlayed["total"] / game["timepassed"]) );
				player maps\mp\gametypes\_rank::giveRankXP( "loss", playerScore );
			}
		}
	}
	else
	{
		playerSPM = maps\mp\gametypes\_rank::getScoreInfoValue( "tie" );

		if ( game["timepassed"] )
		{
			if ( !level.timeLimit )
				totalScore = int(playerSPM * 8);
			else
				totalScore = int(playerSPM * level.timeLimit);
			...
			for( i = 0; i < players.size; i++ )
			{
				...
				playerScore = int( totalScore * (player.timePlayed["total"] / game["timepassed"]) );
				player maps\mp\gametypes\_rank::giveRankXP( "win", playerScore );
			}
		}
...
checkTimeLimit()
{
	...
	{
		setGameEndTime( int( level.startTime + ( level.timeLimit * 60 * 1000 ) ) );
	}
...
{
	if ( event == "assist" )
		level.teamScores[team] += 2;
	else
		level.teamScores[team] += 10;
}
...
updateTeamStatus()
{
	...
	for ( i = 0; i < level.players.size; i++ )
	{
		...
		if ( isDefined( team ) && team != "spectator" && isdefined( class ) && class != "" )
		{
...
getClock( timeLeft, type, forceTimer )
{
	...
	{
		level.clock = createServerTimer( "objective", 1.5 );
		level.clock.type = type;
		level.clock.foreground = true;

		if ( level.teamBased )
			level.clock setPoint( "TOPLEFT", undefined, 10, 22 );
		else
...
{	
	loser maps\mp\gametypes\_persistence_util::statAdd( "stats", "ties", 1 );
	loser updatePersRatio( "wlratio", "wins", "losses" );
	loser maps\mp\gametypes\_persistence_util::statSet( "stats", "cur_win_streak", 0 );	
}
...
//			else
	...
	if ( !level.prematchPeriod )
		level.prematchPeriod = 0.05;
	else if ( level.prematchPeriod > 2.0 )
	...
	if ( game["state"] == "prematch" )
	{
		level.prematchElement = createServerFontString( "objective", 2.0 );
...
Callback_PlayerKilled(eInflictor, attacker, iDamage, sMeansOfDeath, sWeapon, vDir, sHitLoc, psOffsetTime, deathAnimDuration)
{
	...
	if( isPlayer( attacker ) )
	{
		...
		else
		{
			...
			else
			{
				...
				if ( isDefined( level.hardpointItems ) && isAlive( attacker ) && attacker.cur_kill_streak >= 3 )
				{
					streak = (attacker.cur_kill_streak - 3) % 6;
	
					if ( streak == 0 )
					{
						if ( attacker thread maps\mp\gametypes\_hardpoints::giveHardpointItem( "radar_mp" ) )
							attacker thread hardpointNotify( "radar_mp", 3 );
					}
					else if ( streak == 2 )
					{
						if ( attacker thread maps\mp\gametypes\_hardpoints::giveHardpointItem( "airstrike_mp" ) )
							attacker thread hardpointNotify( "airstrike_mp", 3 );
					}
					else if ( streak == 4 )
					{
						if ( attacker thread maps\mp\gametypes\_hardpoints::giveHardpointItem( "helicopter_mp" ) )
							attacker thread hardpointNotify( "helicopter_mp", 3 );
					}

	//				if ( attacker.cur_kill_streak % 3 == 0 )
	//					attacker thread maps\mp\gametypes\_hardpoints::upgradeHardpointItem();
				...
				if ( level.teamBased )
				{
					...
					if ( isdefined( self.attackers ) )
					{
						for ( j = 0; j < level.players.size; j++ )
						{
							...
							givePlayerScore( "assist", player, self );
							giveTeamScore( "assist", player.pers["team"], player, self );

							player thread maps\mp\gametypes\_missions::playerAssist();
						}
					}
	...
	if ( isPlayer( attacker ) /*&& doKillcam */ )
	{
		if ( level.onlineGame )
		{
			class_num = attacker.class_num;
			if ( isDefined( attacker.custom_class[class_num]["specialty1"] ) )
				perks[0] = attacker.custom_class[class_num]["specialty1"];
			if ( isDefined( attacker.custom_class[class_num]["specialty2"] ) )
				perks[1] = attacker.custom_class[class_num]["specialty2"];
			if ( isDefined( attacker.custom_class[class_num]["specialty3"] ) )
				perks[2] = attacker.custom_class[class_num]["specialty3"];
		}
		else
		{
			if ( isDefined( level.default_class[attacker.curClass][0] ) )
				perks[0] = level.default_class[attacker.curClass][0];
			if ( isDefined( level.default_class[attacker.curClass][1] ) )
				perks[1] = level.default_class[attacker.curClass][1];
			if ( isDefined( level.default_class[attacker.curClass][2] ) )
				perks[2] = level.default_class[attacker.curClass][2];
		}
	}
	...
	if ( doKillcam && level.killcam  )
		self maps\mp\gametypes\_killcam::killcam( lpattacknum, sWeapon, delay, psOffsetTime, !(level.numLives && !self.pers["lives"]), timeUntilRoundEnd(), perks );
...
{
	self maps\mp\gametypes\_hud_message::notifyMessage( level.streakHints[streakVal], level.hardpointHints[hardpointType], undefined, undefined, level.hardpointInforms[hardpointType] );
	self playLocalSound( level.hardpointInforms[hardpointType+"_voice"] );
}
...
Callback_PlayerLastStand( eInflictor, attacker, iDamage, sMeansOfDeath, sWeapon, vDir, sHitLoc, psOffsetTime, deathAnimDuration )
{
	...
	self thread maps\mp\gametypes\_hud_message::notifyMessage( "Last Stand!", undefined, undefined, (1,0,0) );
	// check if player has pistol
	for( i = 0; i < weaponslist.size; i++ )
	{
		sPistol = weaponslist[i];
		if( isdefined( level.side_arm_array[ sPistol ] ) )
		{
			// take away all weapon and leave this pistol
			self takeallweapons();
			self giveweapon( sPistol );
			self giveMaxAmmo( sPistol );
			self switchToWeapon( sPistol );
