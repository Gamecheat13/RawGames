...
#/ 	
	...
	if ( getdvar( "cheat_slowmo" ) != "" )
		maps\_cheat::slowmo_main();
...
{
	// pre-game levels
	if ( level.script == "training" || level.script == "cargoship" || level.script == "coup" )
		return true;
	// early gameplay levels
	if ( level.script == "zipline" || level.script == "armada" )
		return true;
...
player_special_death_hint()
{
	...
	if ( cause == "MOD_EXPLOSIVE" )
	{
		if ( isdefined( attacker.targetname ) && ( attacker.targetname == "destructible" ) )
		{
			...
			thread vehicle_death_indicator_hudelement();
		}
		return;
	}
...
vehicle_death_indicator_hudelement()
{
	wait( 1.5 );
	overlay = newHudElem();
	overlay.x = 0;
	overlay.y = 40;
	overlay setshader( "hud_burningcaricon", 96, 96 );
	overlay.alignX = "center";
	overlay.alignY = "middle";
	overlay.horzAlign = "center";
	overlay.vertAlign = "middle";
...
trigger_lookat( trigger )
{
	...
	if ( isdefined( trigger.target ) )
	{
		...
		assertex( orgs.size < 2, "Trigger_lookat at " + trigger.origin + " targets multiple script origins" );
		if ( orgs.size == 1 )
		{
...
handle_starts()
{
	...
	for ( i = 0; i < dvars.size; i ++ )
	{
		if ( start == dvars[ i ] )
		{
			thread indicate_start( start );
			level.start_point = start;
			thread indicate_start( start );
			thread [[ level.start_functions[ start ] ]]();
			return;
		}
	}

	if ( isdefined( level.default_start ) || start == "default" )
	{
...
display_starts() 
{
	...
	for ( i = 0; i < dvars.size; i++ )
	{
		elems[ elems.size ] = create_start( dvars[ i ], dvars.size - i );
	}
	...
	{	
		...
		if ( level.player buttonPressed( "kp_enter" ) || level.player buttonPressed( "BUTTON_A" ) )
		{
			...
			missionSuccess( level.script, false );
		}
		wait( 0.05 );
	}
