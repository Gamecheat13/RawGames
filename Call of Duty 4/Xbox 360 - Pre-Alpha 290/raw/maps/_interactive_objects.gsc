...
main()
{
	...
	array_thread(getentarray ("glass", "script_noteworthy"), ::glass_logic);
...
{
	ent = spawnstruct();
	ent.cracked 		= undefined;
	ent.damage_model 	= undefined;
	ent.crackedContents = undefined;
	ent.whole 			= self;
	ent.info 			= undefined;
	ent.prevdamage 		= 0;
	ent.glasshealth		= 0;
	ent.direction_vec 	= undefined;
	
	self glass_setup( ent );
	
	if( isdefined( ent.damage_model ) )
		ent.damage_model glass_waittill_whole_break( ent );
	else
		ent.whole glass_waittill_whole_break( ent );

	ent.whole hide();

	if ( isdefined( ent.cracked ) )
		ent.cracked glass_waittill_cracked_break( ent ); 
	
	ent.whole glass_play_break_fx( ent.info,  ent.direction_vec);
	
	ent.whole delete();
	if( isdefined( ent.damage_model ) )
		ent.damage_model delete();
}

glass_setup( ent )
{
	//this should be fixed in the next game to just detect targetname, not see if it or script_noteworthy
	//were used to set this to glass, too late in this game
	if( isdefined( self.targetname ) && self.targetname != "glass" )
		ent.damage_model = getent(self.targetname, "target");	
	
	if( isdefined(self.target) )
	{
		ent.cracked = getent(self.target, "targetname"); 
		assertex( isdefined(ent.cracked), "Destructible glass at origin (" + self.origin + ") has a target but the cracked version doesn't exist" );
	}
	...
	{
		case "glass_large":
			ent.info = "glass_large";
			break;
		case "glass_med":
			ent.info = "glass_med";
			break;
		case "glass_small":
			ent.info = "glass_small";
			break;
		default:
	...
	{
		ent.glasshealth = 99;
		ent.cracked linkto( ent.whole );
		ent.cracked hide();
		// Set the contents to 0 to make it non-solid.
		// Note "notSolid()" won't work since it forgets the contents.
		ent.crackedContents = ent.cracked setContents( 0 );
	}
	
	if( isdefined( ent.whole.script_health ) )
		ent.glasshealth = ent.whole.script_health;
	else if( isdefined( ent.cracked ) )
		ent.glasshealth = 99;
	else
		ent.glasshealth = 250;
}

glass_waittill_whole_break( info_struct )
{
	...
	while(info_struct.glasshealth > 0)
	{
		self waittill( "damage", damage, attacker, direction_vec, point, damageType, modelName, tagName );
		if ( damageType == "MOD_GRENADE_SPLASH" )
			damage = damage * 1.75;
		if ( damageType == "MOD_IMPACT" )
			damage = 100000;
		info_struct.glasshealth -= damage;		
	}
	
	info_struct.prevdamage = info_struct.glasshealth * -1;
	info_struct.direction_vec = direction_vec;
}

glass_waittill_cracked_break( info_struct )
{
	self show();
	self setcandamage(true);
	direction_vec = undefined;
	
	info_struct.glasshealth = (200) - info_struct.prevdamage;

	// Set the contents back so that it's solid
	self setContents( info_struct.crackedContents );

	while(info_struct.glasshealth > 0)
	{
		self waittill("damage", damage, other, direction_vec, point, damageType, modelName, tagName );
		if ( damageType == "MOD_GRENADE_SPLASH" )
			damage = damage * 1.75;
		if ( damageType == "MOD_IMPACT" )
			break;
...
glass_play_break_fx( info, direction_vec)
{
