...
obstacleTraining()
{
	...
	keyHint( "sprint" ); // make this not disappear
	...
	for ( ;; )
	{
		...
		for ( index = 0; index < level.buddies.size; index++ )
		{
			...
			if ( buddy.origin[1] > avgYPos )
			{
				...
				buddy.moveplaybackrate = 1 - frac * 0.2;
//				buddy thread printAboveHead (buddy.moveplaybackrate, 1, undefined, (0,1,0) );
			}
			else if ( buddy.origin[1] < avgYPos )
			{
				frac = (avgYPos - buddy.origin[1]) / (avgYPos - minYPos);
				buddy.moveplaybackrate = 1 + frac * 0.2;
//				buddy thread printAboveHead (buddy.moveplaybackrate, 1, undefined, (1,0,0) );
			}
		}
		wait ( 1.0 );
	}
//	oldmoveplaybackrate = self.moveplaybackrate;
}
...
fragTraining()
{
	...
	frag_target_1 = getEnt( "grenade_damage_trigger1", "targetname" );
	frag_target_2 = getEnt( "grenade_damage_trigger2", "targetname" );
	frag_target_3 = getEnt( "grenade_damage_trigger3", "targetname" );
	
	frag_target_1 enablegrenadetouchdamage();
	frag_target_2 enablegrenadetouchdamage();
	frag_target_3 enablegrenadetouchdamage();

	flag_trigger_init( "frag_target_1", frag_target_1 );
	flag_trigger_init( "frag_target_2", frag_target_2 );
	flag_trigger_init( "frag_target_3", frag_target_3 );
...
targetDummyThink()
{
	...
	for( ;; )
	{
		...
		if ( self.laneID == 4 )
		{
			level.player playSound( "training_target" );
			/*
...
{
	level.actionBinds = [];
	registerActionBinding( "objectives",		"pause",				&"TRAINING_HINT_CHECK_OBJECTIVES_PAUSED" );
