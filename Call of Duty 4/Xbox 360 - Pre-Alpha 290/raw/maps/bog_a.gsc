...
defend_the_roof_with_javelin()
{
	...
	array_thread( bridge_attackers, ::spawn_ai );
...
//	activate_trigger_with_targetname( "shanty_start_trigger" );
	...
	for ( i=0; i < allies.size; i++ )
	{
		...
		if ( level.start_point == "shanty" && count >= 3 )
		{
...
should_break_grenade_launcher_hint( nothing )
{
	...
	if ( !isdefined( stored_ammo[ "m203_m4" ] ) )
		return true;	
	
	// dont do the hint if we're out of ammo
	if ( stored_ammo[ "m203_m4" ] == 0 )
		return true;	
		
	// don't do the hint if we're not currently using the m4_grenadier that has the launcher attachment		
	return level.player getcurrentweapon() != "m4_grenadier";
}