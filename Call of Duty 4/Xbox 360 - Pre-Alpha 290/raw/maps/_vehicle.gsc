...
trigger_process( trigger, vehicles )
{
	...
	script_vehicletriggergroup = 		isdefined( trigger.script_vehicletriggergroup );
	...
	script_flag_set = 							isdefined( trigger.script_flag_set );
	script_flag_wait = 							isdefined( trigger.script_flag_wait );
	...
	if ( script_flag_set )
	{
		if ( !isDefined( level.flag[ trigger.script_flag_set ] ) )
		{
			flag_init( trigger.script_flag_set );
		}
	}
	
	if ( script_flag_wait )
	{
		if ( !isDefined( level.flag[ trigger.script_flag_wait ] ) )
		{
			flag_init( trigger.script_flag_wait );
		}
	}
	...
	while ( gotrigger )
	{
		...
		if ( script_flag_set )
		{
			other.vehicle_flags[ trigger.script_flag_wait ] = true;
			other notify( "vehicle_flag_arrived", trigger.script_flag_wait );
		...
		else if ( detoured && triggererisdefined )
		...
		if ( script_vehicleGroupDelete )
		{
			...
			deletegroups = array_merge( level.vehicle_DeleteGroup[ trigger.script_vehicleGroupDelete ], targs );
// 			deletegroups = array_merge( get_deletegroups( trigger.script_vehicleGroupDelete ), targs );
// 			array_levelthread( deletegroups, ::delete_group, trigger.script_vehicleGroupDelete );// was _links
			array_levelthread( deletegroups, ::deleteEnt );
		}
...
jeepride_detourhack( node )
{
	// the WIP_starts sometimes happen just before a detour node
	wait .05;
...
vehicle_paths( node, bhelicopterwaitforstart )
{
	...
	// special hack for jeepride.  LD's wouldn't normally set up a path like this but an autogenerated one at anypoint in time in Jeepride would.
// 	if ( level.script == "jeepride" && isdefined( self.attachedpath.detoured ) )
// 		thread jeepride_detourhack( self.attachedpath  );
	...
	for ( i = 0; i < pathpoints.size; i ++ )
	{
		...
		// this is to aid in jeepride pathing.  It's not fun to crawl back a chain in radiant.  to find out what the last speed was. I'd rather look at eh freeze frame export map.
			if ( isdefined( nextpoint.speed ) )
				self.lastnodespeed = nextpoint.speed;  
		///////////////////////////// 
...
kill()
{
	...
	if ( !isdefined( self ) )
	{
		if ( isdefined( self.walkers ) )
			for ( j = 0; j < self.walkers.size; j ++ )
				self.walkers[ j ] delete();
...
get_script_modelvehicles()
{
	...
	{
		if ( isdefined( models[ i ].targetname ) && ( models[ i ].targetname == "destructible" || models[ i ].targetname == "zpu" ) )
			continue; 
...
node_trigger_process()
{
		...
		// todo.. put this somewhere else this is creating unused variables for other triggers - nate
		if ( isdefined( self.script_linkname ) )
		...
		( 
			 ...
			 || 	isdefined( self.script_flag_set )
		 )
		processtrigger = true; 
...
// 	guy setmodel( "" );
	...
	level.player playerlinktodelta( guy, "tag_player", 1.0, 40, 40, 30, 30 );
