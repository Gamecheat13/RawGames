...
testAnims()
{
	...
	testanims[0] = %pistol_death_1;
	testanims[1] = %pistol_death_2;
	testanims[2] = %pistol_death_3;
	testanims[3] = %pistol_death_4;
...
exposedCombatMainLoop()
{
	...
	for(;;)
	{
		...
		// The 'continue' in here is causing an assert with developer_script 0. Commented out because we don't use this anyway.
		// The assert is being fixed, bug #48457
		/*
		/#
		// dev only
...
turn( direction, amount )
{
	...
	else if ( isdefined( self.enemy ) && self canSee( self.enemy ) && distanceSquared( self.enemy.origin, self.origin ) < 512*512 )
	{
	...
	if ( isdefined( self.node ) && self.node.type == "Guard" && distanceSquared(self.origin, self.node.origin) < 1 )
...
{
	if ( !grenadeCoolDownElapsed( throwAt ) ) // early out for efficiency
		return false;
