...
DoShoot( turret )
{
	self setTurretAnim( %technicalGunner_aim );
	self setAnimKnobRestart( %technicalGunner_aim, 1, 0, 1 );
...
DoAim( turret )
{
	...
	self setTurretAnim( %technicalGunner_aim );
	self setAnimKnobRestart( %technicalGunner_aim, 1, 0, 0 );	// Rate 0 so it doesn't shake
}
