...
init()
{
	...
	level.player thread StingerRemove();
}
...
StingerRemove()
{
	// todo: this shouldn't be needed once the proper weapon is made.
	while ( true )
	{
...
StingerDisable()
{
	level.player SetWeaponAmmoStock( "stinger", 0 );
	level.player SetWeaponAmmoClip( "stinger", 0 );
}

StingerEnable()
{
	level.player SetWeaponAmmoStock( "stinger", 0 );
	level.player SetWeaponAmmoClip( "stinger", 1 );
}
...
*/
		...
		if ( level.stingerLockFinalized )
		{
			...
			thread LoopLocalLockSound( "javelin_clu_lock", 0.85 );
...
PlayerStingerAds()
{
	...
	if ( level.player getammocount( "stinger") )
		StingerDisable();
...
LoopLocalLockSound( alias, interval )
{
	...
	{
		level.player playLocalSound( alias );
		level.player PlayRumbleLoopOnEntity( "stinger_lock_rumble" );
		wait interval;
		level.player StopRumble( "stinger_lock_rumble" );
	}
	level.player.stingerlocksound = undefined;
