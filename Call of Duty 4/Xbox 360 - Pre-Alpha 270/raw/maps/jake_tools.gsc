...
print3Dthread(sMessage, org)
{
	if (!isdefined(org))
		...
		{
			if (isdefined(self))
				print3d (self.origin + (0,0,64), sMessage, (1,1,1), 1, 0.25);
			wait (0.05);
		}		
	}
	else
	{
		for (;;)
		{
			print3d (org, sMessage, (1,1,1), 1, 0.25);
			wait (0.05);
		}	
	}
...
triggersEnable(triggerName, noteworthyOrTargetname, bool)
{
	...
	if (bool == true)
		array_thread(aTriggers, maps\_utility::trigger_on);
	else
		array_thread(aTriggers, maps\_utility::trigger_off);
}
