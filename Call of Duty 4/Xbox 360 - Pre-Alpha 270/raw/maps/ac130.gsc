...
blackhawk_land()
{
	...
	vehicle vehicle_detachfrompath();
	vehicle setgoalyaw( self.angles[ 1 ] );
	vehicle vehicle_land();
	
	// wait until it has landed before loading guys
	//vehicle notify ( "load", array );
	
	// take off and fly away once guys are loaded
	//vehicle vehicle_liftoff();
	//vehicle vehicle_resumepath();
}
