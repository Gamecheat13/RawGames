...
init()
{
	...
	precacheShader( "faction_128_usmc_blue" );
	precacheShader( "faction_128_arab_blue" );
	precacheShader( "faction_128_ussr_blue" );
	precacheShader( "faction_128_sas_blue" );
	precacheShader( "faction_128_usmc_red" );
	precacheShader( "faction_128_arab_red" );
	precacheShader( "faction_128_ussr_red" );
	precacheShader( "faction_128_sas_red" );
	...
	if ( game["state"] != "prematch" )
	{
		if ( level.inGracePeriod )
		{
			...
			if ( self.pers["team"] == "allies" )
				thread maps\mp\gametypes\_hud_message::teamNotify( "Marine Force Recon", "faction_128_usmc_blue" );
			else
				thread maps\mp\gametypes\_hud_message::teamNotify( "OpFor", "faction_128_arab_blue" );
		}
		else
		{
...
testHPs()
{
	...
	for ( ;; )
	{
		self thread maps\mp\gametypes\_hardpoints::giveHardpointItem( hps[randomInt(hps.size)] );
...
endGame( winner )
{
	...
	maps\mp\gametypes\_hardpoints::planeFlyOver( coord );
	*/
	...
	if( isPlayer( attacker ) )
	{
		...
		else
		{
			...
			else
			{
				...
				if ( attacker.cur_kill_streak == 3 )
					self thread maps\mp\gametypes\_hardpoints::giveHardpointItem( "radar_mp" );
				else if ( attacker.cur_kill_streak == 5 )
					self thread maps\mp\gametypes\_hardpoints::giveHardpointItem( "airstrike_mp" );
				else if ( attacker.cur_kill_streak == 7 )
					self thread maps\mp\gametypes\_hardpoints::giveHardpointItem( "helicopter_mp" );
				...
				if ( level.teamBased )
				{
					...
					if ( isdefined( self.attackers ) )
					{
						for ( j = 0; j < level.players.size; j++ )
						{
							...
							giveTeamScore( "assist", player.team, player, self );
						}
					}
