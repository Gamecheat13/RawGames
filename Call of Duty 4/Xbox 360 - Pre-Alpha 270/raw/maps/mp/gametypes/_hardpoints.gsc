...
init()
{
	...
	level.hardpointItems["radar_mp"] = 0;
	level.hardpointItems["airstrike_mp"] = 1;
	level.hardpointItems["helicopter_mp"] = 2;
...
giveHardpointItem( hardpointType )
{
	...
	if ( !maps\mp\gametypes\_tweakables::getTweakableValue( "hardpoint", "allowartillery" ) && hardpointType == "airstrike_mp" )
		return;
	else if ( !maps\mp\gametypes\_tweakables::getTweakableValue( "hardpoint", "allowuav" ) && hardpointType == "radar_mp" )
		return;
	else if ( !maps\mp\gametypes\_tweakables::getTweakableValue( "hardpoint", "allowhelicopter" ) && hardpointType == "helicopter_mp" )		
		return;
	else if ( !isDefined( level.heli_paths ) || !level.heli_paths.size && hardpointType == "helicopter_mp" )
		return;

	if ( isDefined( self.hardPointItem ) )
	{
		if ( level.hardpointItems[hardpointType] < level.hardpointItems[self.hardPointItem] )
			return;
	}
	...
	self.hardPointItem = hardpointType;
...
hardpointItemWaiter()
{
	...
	if ( isDefined( self.hardpointItem ) )
		self giveHardpointItem( self.hardpointItem );
		...
		switch( currentWeapon )
		{
			...
			case "helicopter_mp":
				...
				{			
					...
					self.hardPointItem = undefined;
				}
				else
				{
					// feedback about failure
				}
...
useRadarItem()
{
	...
	if ( !teamHasRadar( team ) || getTime() - level.radarLastUsageTime[ team ] > 1000*5 )
	{
		...
		if ( !teamHasRadar( team ) ) 
		{ // don't play the sound if they already have radar
			playSoundOnPlayers( "US_Tmp_stm_uavonline", team );
			playSoundOnPlayers( "US_Tmp_stm_enemyuavonline", otherTeam );
		}
