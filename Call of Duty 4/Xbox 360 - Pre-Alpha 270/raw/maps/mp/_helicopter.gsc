...
precachehelicopter(model,type)
{
	...
	switch(model)	
	{
		case "vehicle_cobra_helicopter_fly":
			...
			level.vehicle_deathmodel[model] = "vehicle_cobra_helicopter_fly";
			break;
		case "vehicle_blackhawk_low":
			precachemodel("vehicle_blackhawk_low");
			level.vehicle_deathmodel[model] = "vehicle_blackhawk_low";
			break;
		...
		case "vehicle_mi24p_hind_woodland_opened_door":
			...
			break;
		case "vehicle_mi17_woodland_fly":
			precachemodel("vehicle_mi17_woodland_fly");
			level.vehicle_deathmodel[model] = "vehicle_mi17_woodland_fly";
			break;
	}
...
heli_update_global_dvars()
{
	for( ;; )
	{
		...
		level.heli_health_degrade = heli_get_dvar_int( "scr_heli_health_degrade", "12" );	// health degradation after injured, health descrease per second for heavy injury, half for light injury
		...
		level.heli_armor_bulletdamage = heli_get_dvar( "scr_heli_armor_bulletdamage", "0.25" );			// damage multiplier to bullets onto helicopter's armor
...
heli_think( owner, startnode, heli_team )
{
	...
	if( heli_team == "allies" )
	{
		...
		chopper.team = "allies";			// the team helicopter belongs to
	}
	else
	{
		//chopper = spawn_helicopter( owner, heliOrigin, heliAngles, "cobra_mp", "vehicle_cobra_helicopter_fly" );
...
canTarget_turret( player )
{
	...
	if ( isdefined( player.pers["team"] ) && ( player.pers["team"] == self.team || player.pers["team"] == "spectator" ) )
		return false;
...
heli_damage_monitor()
{
	...
	for( ;; )
	{
		...
		if ( isdefined ( self.attacker ) )
			println( "Health: " + ( self.maxhealth - self.damageTaken ) + " Attacker: " + self.attacker.name + "\n" ); // debug
	}
}
...
heli_fly( currentnode )
{
	...
	{	
		...
		// flight none stop until final destination
		if ( !isdefined( nextnode.target ) )
			stop = 1;
		else
			stop = 0; // dont stop at node until final destination
...
missile_support( target_player, rof, instantfire, endon_notify )
{
	...
	{
		player = players[i];
		if ( isdefined( target_player ) && isdefined( player.pers["team"] ) && player.pers["team"] == self.team && distance( player.origin, target_player.origin ) <= level.heli_missile_friendlycare )
		{
...
turret_target_flag( turrettarget )
{
	...
	while( 1 )
	{
		...
		// undefine flag when target player dies or disconnects
	...
	assertex( isdefined( turrettarget) && isdefined( self.turrettarget ), "Target became undefined or changed" );
