...
init()
{
	...
	if ( !isDefined( level.scr_animSound ) )
		level.scr_animSound = [];
	if ( !isDefined( level.scr_generic_anim ) )
		level.scr_generic_anim = [];
...
anim_custom_animmode_on_guy( guy, custom_animmode, anime, org, angles )
{
	assertEx( isdefined( guy.animname ), "Animating character of type " + guy.classname + " has no animname." );
	assertEx( isdefined( level.scr_anim[ guy.animname ][ anime ] ), "Guy with animname " + guy.animname + " is trying to do scene " + anime + " but level.scr_anim[ " + guy.animname + " ][ " + anime + " ] does not exist!" );
...
// 			guy thread animsound_start_tracker( anime );
			...
			if ( doAnimation )
			{
				...
				start_notetrack_wait( guy, anim_string, anime );
...
start_notetrack_wait( guy, anim_string, anime )
{
	...
	if ( isdefined( level.scr_notetrack[ guy.animname ] ) )
		thread notetrack_wait( guy, anim_string, self, anime );
}
 ...
 */ 
		...
		if ( doAnimation )
		{
			...
			thread start_notetrack_wait( guy, anim_string, anime );
...
// 	println( guy[ 0 ].animname, " doing animation ", anime );
	...
	for ( i = 0; i < guys.size; i ++ )
	{
		...
		#/ 
		threads ++ ;
		guy thread begin_anim_reach( ent, startOrg );
	}
