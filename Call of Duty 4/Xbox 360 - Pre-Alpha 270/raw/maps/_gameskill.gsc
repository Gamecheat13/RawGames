...
//	createprintchannel ("script_autodifficulty");
	...
	*/
	...
	level.difficultySettings["playerDifficultyHealth"]["normal"] = 300;
	...
	level.difficultySettings["missTimeDistanceFactor"]["hardened"] = 0;//0.15 / 1000;
	level.difficultySettings["missTimeDistanceFactor"]["veteran"]  = 0;//0.085 / 1000;
	...
	//{
		...
		level.difficultySettings["invulTime_preShield" ]["hardened"] = 0.05;
		...
		level.difficultySettings["invulTime_postShield"]["hardened"] = 0.0;
...
{
	if ( self.a.script == "move" && isalive( self.enemy ) && isPlayer( self.enemy ) )
	{
...
redFlashingOverlay(overlay)
{
	...
	if ( isAlive( level.player ) && takeCoverWarnings > 3 && level.gameskill < 2 && level.coverwarnings )
	{
		// get to cover!
