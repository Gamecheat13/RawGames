...
{	
	...
	{
		key = keys[i];
		array[ key ].mo_tools_array_wait = true;	
		array[ key ] thread array_waitlogic1(msg, timeout);
	}
	for(i=0;i<keys.size; i++)
	{
		key = keys[i];
		if(isdefined(array[ key ]) && array[ key ].mo_tools_array_wait)
			array[ key ] waittill("mo_tools_array_wait");	
	}
}
