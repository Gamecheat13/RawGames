...
shootAsTold()
{
	...
	while(1)
	{
		while(1)
		{
			...
			shootUntilShootBehaviorChange( true );
...
shootUntilShootBehaviorChangeForTime( time )
{
	...
	shootUntilShootBehaviorChange( false );
...
shootUntilShootBehaviorChange( runAngleRangeThread )
{
	...
	while(1)
	{
		...
		{
			if ( self.a.cornerMode == "lean" )
				self FireUntilOutOfAmmo( animArray("lean_fire"), false, animscripts\shared::decideNumShotsForFull() );
			else
		...
		else if ( self.shootStyle == "burst" || self.shootStyle == "single" || self.shootStyle == "semi" )
		{
			...
			{
				if ( numShots == 1 )
					fireAnim = animArray( "lean_single" );
				else
					fireAnim = animArray( "lean_fire" );
			}
			else
			{
				if ( numShots == 1 )
					fireAnim = animArray( "single" );
				else
					fireAnim = animArray( self.shootStyle + numShots );
			}
			self FireUntilOutOfAmmo( fireAnim, true, numShots );
		}
		else
		{
...
cornerReload()
{
	...
	self waittillmatch("cornerReload","end");
	self animscripts\weaponList::RefillClip();
...
set_standing_animarray_aiming() 
{
	...
	if ( self usingShotgun() )
		self.a.array["single"] = %shotgun_stand_fire_1A;
	else
...
set_crouching_animarray_aiming() 
{
	...
	if ( self usingShotgun() )
		self.a.array["single"] = %shotgun_crouch_fire;
	else
