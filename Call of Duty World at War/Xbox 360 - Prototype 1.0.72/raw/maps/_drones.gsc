...
large_group(groupnum, node, ender, flagguy, dummymodel)
{
	...
	for (i=1;i<=level.large_group_size[groupnum];i++)
	{
		...
		else
		{
			...
			//guy attach ("weapon_mosinnagant", "tag_weapon_Right");
		}
...
//		thread play_sound_on_tag(random (level.scr_dyingguy["sound"]), level.scr_dyingguy["tag"][tag_index]);
...
init()
{
	...
	setAnimArray();
	level._effect["impact_flesh1"]				= loadfx ("impacts/flesh_hit");
	level.drone_muzzleflash = loadfx ("muzzleflashes/standardflashworld");
...
drone_axis_assignWeapon()
{
	...
	self.bulletsInClip = self weaponClipSize( self.weapon );
}
...
ShooterRun(destinationPoint, event)
{
	...
	{
		//TREYARCH: Removed as this anim no longer exists.
...
ShooterShootThread(target)
{
    ...
    while(isdefined(self))
    {
        ...
        for (i=0; i<numShots; i++)
        {
			...
			if (self.team == "axis")
			{
				//self playsound ("weap_kar98k_fire");		//TREYARCH: no WW2 weapon sounds yet	
			}
			else
			{
				switch(level.campaign)
				{
					case "american":
            			//self playsound ("weap_m1garand_fire");	//TREYARCH: no WW2 weapon sounds yet
            			break;
            		case "russian":
