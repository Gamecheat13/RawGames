...
activate_color_trigger()
{
	if( isdefined( self.script_color_allies ) )
	{
		self thread get_colorcodes_and_activate_trigger( self.script_color_allies, "allies" );
	}
	
	if( isdefined( self.script_color_axis ) )
	{
		self thread get_colorcodes_and_activate_trigger( self.script_color_axis, "axis" );
	}
}
...
//			println( "sent ai to node" );
		closestAI notify( "stop_color_move" );
		closestAI.currentColorCode = colorCode;			
		closestAI thread process_color_order_to_ai( node, self, ai_count, original_ai_array, counter, colorCode );
		counter++;
...
ai_sets_goal( node )
{
	...
	if( isdefined( level.colorCoded_volumes[ self.team ][ self.currentColorCode ] ) )
	{
		self setFixedNodeSafeVolume( level.colorCoded_volumes[ self.team ][ self.currentColorCode ] );
	}
	else
	{
...
process_color_order_to_ai( node, trigger, delay_size, ai, counter, colorCode )
{
	thread decrementColorUsers( node );
...
colorNode_replace_on_death()
{
	...
	for( ;; )
	{
		...
		// MikeD (4/27/2007): Removed level.player
