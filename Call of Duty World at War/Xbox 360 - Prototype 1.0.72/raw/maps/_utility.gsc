...
///ScriptDocBegin
"Name: flag_wait( <msg> )"
"Summary: Waits until a level notify( msg ) is sent and the level.flag[msg] is set to true or 1."
"CallOn: level"
"Module: _utility.gsc"
"MandatoryArg: <msg> ( string ) The flag string."
"Example: level flag_wait( "event1_Tank_Destroyed" )"
"SPMP: singleplayer"
///ScriptDocEnd
 ============= 
*/
...
/*
 ============= 
///ScriptDocBegin
"Name: flag_wait_either( <flag1>, <flag2> )"
"Summary: Waits until either flag is notified (level notify( flag1 ) or level notify( flag2 )"
"CallOn: level"
"Module: _utility.gsc"
"MandatoryArg: <flag1> ( string ) The Flag1 String."
"MandatoryArg: <flag2> ( string ) The Flag2 String."
"Example: level flag_wait_either( "event1_tank_destroyed", "event1_foley_dead" )"
"SPMP: singleplayer"
///ScriptDocEnd
 ============= 
*/
...
/*
 ============= 
///ScriptDocBegin
"Name: flag_waitopen( <msg> )"
"Summary: Waits until a flag is off along with a notify of the msg."
"CallOn: level"
"Module: _utility.gsc"
"MandatoryArg: <msg> ( string ) The Flag String."
"Example: level flag_waitopen( "event1_tank_destroyed" )"
"SPMP: singleplayer"
///ScriptDocEnd
 ============= 
*/
...
/*
 ============= 
///ScriptDocBegin
"Name: flag_trigger_init( <message>, <trigger>, [continuous] )"
"Summary: Associates the given trigger to a flag. The trigger then will thread _flag_wait_trigger( message, continuous )"
"CallOn: level"
"Module: _utility.gsc"
"MandatoryArg: <message> ( string ) The Flag String."
"MandatoryArg: <trigger> ( entity ) The trigger entity."
"OptionalArg: [continuous] Determines if it's going to continuously check if the trigger is being touched in _flag_wait_trigger(). Default is FALSE"
"Example: level flag_waitopen( "event1_tank_destroyed", truck_trigger )"
"Example: level flag_waitopen( "event1_tank_destroyed", truck_trigger, true )"
"SPMP: singleplayer"
///ScriptDocEnd
 ============= 
*/
...
/*
 ============= 
///ScriptDocBegin
"Name: flag_triggers_init( <message>, <triggers>, [continuous] )"
"Summary: Associates multiple triggers to a flag. The triggers then will thread _flag_wait_trigger( message, continuous )"
"CallOn: level"
"Module: _utility.gsc"
"MandatoryArg: <message> ( string ) The Flag String."
"MandatoryArg: <triggers> ( entity array ) The array of triggers."
"OptionalArg: [continuous] Determines if it's going to continuously check if the triggers is being touched in _flag_wait_trigger(). Default is FALSE"
"Example: level flag_waitopen( "event1_tank_destroyed", event1_triggers )"
"Example: level flag_waitopen( "event1_tank_destroyed", event1_triggers, true )"
"SPMP: singleplayer"
///ScriptDocEnd
 ============= 
*/
flag_triggers_init( message, triggers, continuous )
{
	flag_init( message ); 
	...
	for( index = 0; index < triggers.size; index++ )
	{
		...
		triggers[index] thread _flag_wait_trigger( message, continuous ); 
	}
...
// SCRIPTER_MOD
// MikeD (3/23/2007): No more level.player, only save if the host is alive, since we're only saving on the host.
// TODO: Change players[0] to GetHostPlayer();
...
//	ent.origin = level.player GetOrigin(); 
	players = get_players();
	ent.origin = players[0] GetOrigin(); 
	ent SetModel( model ); 
	ent Delete(); 
}
...
activate_exploder( num )
{
	...
	for( i = 0; i < level.createFXent.size; i++ )
	{
		...
		else if( IsDefined( ent.v["soundalias"] ) )
		{
		...
		else if( ( ent.v["exploder_type"] == "exploderchunk" ) ||( ent.v["exploder_type"] == "exploderchunk visible" ) )
		{
...
// SCRIPTER_MOD
// MikeD (3/23/2007): We do not need this, use demigod instead.
//player_god_on()
//{
...
// SCRIPTER_MOD
// JesseS (3/30/07): Removed, not being used in _friendlyfire anymore
...
radio_dialogue( msg )
{
	...
	players = get_players();
	players[0] play_sound_on_entity( level.scr_radio[ msg ] );
}
...
remove_global_spawn_function( team, function )
{
 ...
 ============= 
 ///ScriptDocBegin
"Name: add_spawn_function( <func> , <param1> , <param2> , <param3> )"
...
fileprint_map_header( bInclude_blank_worldspawn )
{
	...
	assert( isdefined( level.fileprint ) );
	 /#
	fprintln( level.fileprint, "iwmap 4" );
	fprintln( level.fileprint, "\"000_Global\" flags  active" );
	fprintln( level.fileprint, "\"The Map\" flags" ); 
...
fileprint_map_keypairprint( key1, key2 )
{
	 /#
	assert( isdefined( level.fileprint ) );
	fprintln( level.fileprint, "\"" + key1 + "\" \"" + key2 + "\"" );
	#/ 
}
...
fileprint_map_entity_start()
{
	...
	assert( isdefined( level.fileprint ) );
	fprintln( level.fileprint, "// entity " + level.fileprint_mapentcount );
	fprintln( level.fileprint, "{" );
...
fileprint_end()
{
	...
	assertex( saved == 1, "File not saved( see above message? ) " );
...
// SCRIPTER_MOD
// JesseS( 3/21/2007 ): get any random player
get_random_player()
{
		players = get_players(); 
		
		array_randomize( players ); 
