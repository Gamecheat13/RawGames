...
debug_reflection()
{
	/#
		...
		{
				// SCRIPTER_MOD
				// JesseS (3/20/2007): level.player references replaced with players[0]
				players = get_players();
				level.debug_reflection = true;
				level.debug_reflectionobject = spawn ( "script_model", players[0] geteye() + ( vector_multiply ( anglestoforward ( players[0].angles ), 100) ));
				level.debug_reflectionobject setmodel("test_sphere_silver");
				level.debug_reflectionobject.origin = players[0] geteye() + ( vector_multiply ( anglestoforward ( players[0] getplayerangles() ), 100) );
...
draw_animsounds_in_hud()
{
	...
	level.animsound_hud_animname.label = "Actor: " + guy.animname;
