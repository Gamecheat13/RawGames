...
playerBreathingSound( healthcap )
{
	...
	for( ;; )
	{
		...
		self play_sound_on_entity( "breathing_hurt" );
...
create_warning_elem( ender, player )
{
	...
	hudelem thread destroy_warning_elem( player );
...
destroy_warning_elem( player )
{	
	player playerWasHitAgain();
	self notify( "death" );
	self destroy();
}
...
auto_adjust_difficulty()
{
	...
	for( ;; )
	{
		...
		level.difficulty_spot = self.origin;		
...
{
	return distance( self.origin, org ) < 140;
}
...
auto_adjust_difficulty_debug()
{
 ...
 // 	hud_debug_add( "Player Max health: ", level.player.maxhealth );
	...
	hud_debug_add( "threatbias: " , self.threatbias );
