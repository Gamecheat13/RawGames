...
init()
{
	...
	level.artilleryCanonShellCount = 6;
	level.artilleryCanonCount = 3;
...
//	wait 2;
	...
	callArtilleryStrike( owner, targetpos, yaw, flakDistance );
...
callArtilleryStrike( owner, coord, yaw, distance )
{	
	owner endon("disconnect");
	...
	minYawRandom = 1;
	maxYawRandom = 3;	
	thread startArtilleryCanon( owner, coord, yaw - RandomIntRange( minYawRandom, maxYawRandom ) , distance - RandomFloatRange( minDistanceRandom, maxDistanceRandom ), RandomFloatRange( minInitialDelay, maxInitialDelay ));
	thread startArtilleryCanon( owner, coord, yaw, distance, RandomFloatRange( minInitialDelay, maxInitialDelay ));
	thread startArtilleryCanon( owner, coord, yaw + RandomIntRange( minYawRandom, maxYawRandom ) , distance - RandomFloatRange( minDistanceRandom, maxDistanceRandom ), RandomFloatRange( minInitialDelay, maxInitialDelay ));
}

startArtilleryCanon(  owner, coord, yaw, distance, initial_delay)
{
	owner endon("disconnect");
	wait ( initial_delay );
...
triggerHardPoint( hardpointType )
{
	...
	else if ( hardpointType == "dogs_mp" )
	{
		...
		self thread maps\mp\_dogs::dog_manager_spawn_dogs( team, otherteam );
