...
set_animarray_standing()
{
	...
	else
	{
		...
		self.a.array["burst2"] = %exposed2_shoot_burst3; // ( will be stopped after second bullet)
	...
	// close fighting
	else if ( self isCQBWalking() || self is_banzai() ) // CCheng (8/22/2008): These were using the same anims, so I just combined them.
	{
...
set_animarray_crouching()
{
	...
	if ( weaponAnims() == "rocketlauncher" )
	{
		...
		// TBD: Do we need special RPG anims for stand<->crouch transitions?
	}
	// flamethrower
	else if ( weaponAnims() == "gas" )
	{
...
//	self thread attackEnemyWhenFlashed();
	...
	for(;;)
	{
		...
		// basically only allow movement and melee, skip firing if a zombie
		if (self is_zombie())
		{
			if ( needToTurn() )
			{
...
transitionTo( newPose )
{
	...
	// Do not play a traverse if zombie
	if( self is_zombie() )
	{
		self setAnim( %add_idle );
		self thread idleThread();
...
exposedRechamber()
{
	// this is some hacky stuff until we get prone aims and rechambers
	if ( self.a.pose != "prone" )
	{	
		...
		//self notify("abort_rechamber"); // make sure threads that doRechamberAnim() started finish
		self.a.needsToRechamber = 0;
...
doRechamberAnim( rechamber_anim )
{
	...
	//self setflaggedanimknoballrestart( flagName, rechamber_anim, %root, 1, .2, animRate );
