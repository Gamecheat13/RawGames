...
// MikeD: checks if a custom function exists for a given notetrack, function, and anime
...
// MikeD (3/27/2008):  There isn't a addNotetrack_animFXOnTag(), so I created one. There's already checks within notetrack_wait in _anim.gsc
...
// MikeD (10/2/2007): "animate linked ents" script.
anim_ents( ents, anime, tag, node, tag_entity, animname )
{
	entity = convert_tagent_to_ent( node, tag_entity );
	pos = get_anim_position( tag, entity );	
...
// guzzo 6-16-08 - Conserva's fix for prone-to-run banzai transition
...
=============
///ScriptDocBegin
"Name: anim_single_solo_earlyout( <guy> , <anime> , <tag> , <entity> , <tag_entity>, <animname_override>, <early_out_time> )"
"Summary: Do an anim on a guy from level.scr_anim[ animname ][ anime ]"
"Module: Anim"
"CallOn: The base for an animation scene, such as a node"
"MandatoryArg: <guy>: Guy that animates"
...
"OptionalArg: <tag_entity>: Overwrite the base for the tag"
"OptionalArg: <tag_entity>: Overwrite the base for the tag"
"OptionalArg: <animname_override>: Animation to play instead of guy.animname"
"OptionalArg: <early_out_time>: How long before the end of the anim to stop waiting (default = 0.2)"
"Example: level.price anim_single_solo_earlyout( level.price, "whip_it_good" );"
"SPMP: singleplayer"
///ScriptDocEnd
=============
*/
...
earlyout_animscripted( guy, anime, cut_time )
{
	...
	{
		cut_time = 0.2; // this matches the blend out time in anim_stopanimscripted();
	}
