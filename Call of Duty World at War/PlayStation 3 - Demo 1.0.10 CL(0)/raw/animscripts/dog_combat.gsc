...
#/		
		...
		if ( dog_cant_kill_in_one_hit(player) )
		{
			...
			//chris_p - adding specific zombie dog attack when the player is close
			//self.melee_attack_anim is set on the dogs when they spawn in _zombiemode_spawner.gsc and is dynamically set depending on 
			//distance from the player
			if(self is_zombie() )
			{
				if(self.melee_attack_anim == %ai_zombie_dog_attack_v1)
				{
					self animMode("zonly_physics");
				}
				self setflaggedanimrestart( "meleeanim", self.melee_attack_anim, 1, 0.2, 1 );
...
dog_cant_kill_in_one_hit(player)
{
	...
	if ( getTime() - level.lastDogMeleePlayerTime > 8000 && !self is_zombie() )	// regular dogs
	{
		level.dogMeleePlayerCounter = 0;
	}
	else if (getTime() - level.lastDogMeleePlayerTime > 1250 && self is_zombie() )	// for zombie dogs
	{
		level.dogMeleePlayerCounter = 0;
		return true;
	}	
