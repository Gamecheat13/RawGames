...
onPickup( player )
{
	...
	{
		printOnTeamArg( &"MP_EXPLOSIVES_RECOVERED_BY", team, player.name );
		printOnTeamArg( &"MP_EXPLOSIVES_RECOVERED_BY", otherTeam, &"THE_ENEMY" );
...
onDrop( player )
{
	if ( isDefined( player ) && isDefined( player.name ) )
		...
		printOnTeamArg( &"MP_EXPLOSIVES_DROPPED_BY", self maps\mp\gametypes\_gameobjects::getOwnerTeam(), &"YOUR_TEAM" );
...
abandonmentThink( delay )
{
	...
	printOnTeamArg( &"MP_EXPLOSIVES_DROPPED_BY", self maps\mp\gametypes\_gameobjects::getOwnerTeam(), &"THE_ENEMY" );
...
bombPlanted( object, team )
{
	...
	level.bombClock = createServerTimer( "default", 2.0 );
	level.bombClock setPoint( "TOP", undefined, 0, 2 );
