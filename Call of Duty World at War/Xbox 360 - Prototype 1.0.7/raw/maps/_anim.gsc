...
#/
	...
	while (1)
	{
		...
		for (i=0;i<guy.size;i++)
		{
			...
			if ((isdefined (level.scr_sound[guy[i].animname])) && 
				(isdefined (level.scr_sound[guy[i].animname][anime])) &&
				(isdefined (level.scr_sound[guy[i].animname][anime][idleanim])))
			{
				doDialogue = true;
				dialogue = level.scr_sound[guy[i].animname][anime][idleanim];
			}
...
anim_single (guy, anime, tag, node, tag_entity)
{
	...
	for (i=0;i<guy.size;i++)
	{
		...
		assertEx( isdefined( guy[i].animname ), "Animname is not defined on an actor in a scene" );
		if ((isdefined (level.scr_face[guy[i].animname])) &&
			(isdefined (level.scr_face[guy[i].animname][anime])))
		{
			doFacialanim = true;
...
*/
		...
		{
			doDialogue = true;
			dialogue = level.scr_sound[guy[i].animname][anime];
		}
		...
		for (i=0;i<level.scr_notetrack[guy.animname].size;i++)
		{
			if (notetrack == level.scr_notetrack[guy.animname][i]["notetrack"])
			{
				...
				if (isdefined (level.scr_notetrack[guy.animname][i]["sound"]))
					guy thread play_sound_on_tag (level.scr_notetrack[guy.animname][i]["sound"]);
...
{
	thread anim_reach (guy, anime, tag, node, tag_entity);
	...
	{
		ent.reachers++;
		thread idle_on_reach( guy[ i ], anime_idle, ender, tag, node, tag_entity, ent );
	}
...
idle_on_reach( guy, anime_idle, ender, tag, node, tag_entity, ent )
{
	...
	if ( isalive( guy ) )	
		anim_loop_solo( guy, anime_idle, tag, ender, node, tag_entity );
}
	
get_org_and_angles( tag, node, tag_entity )
{
	org = undefined;
	angles = undefined;
	if (isdefined (tag))
...
//	println (guy[0].animname, " doing animation ", anime);
	array = get_org_and_angles( tag, node, tag_entity );
	org = array[ "origin" ];
	angles = array[ "angles" ];
	...
	for (i=0;i<guy.size;i++)
	{
		...
		if (isdefined(level.scr_anim[guy[i].animname][anime]))
		...
		threads++;
		guy[i] thread begin_anim_reach (ent);
	}
...
begin_anim_reach (ent)
{
	...
	self.goalradius = 64;
	self waittill ("goal");
	// now for the last step put the ai in a threat bias group that ignores
	// all the other groups so he doesnt get distracted and go into exposed while his goal radius is too small
	self ignoreAllEnemies( true );
...
queue_anim( anime, node, tag, tag_entity )
{
	if (isdefined(node))
	{
		assertEx (!isdefined(tag), "Can't play an animation on a tag for a node, from Animation: " + anime);
		assertEx (!isdefined(tag_entity), "Can't play an animation on a tag for a node, from Animation: " + anime);
		queue_anim_single_solo_with_reach (self, anime, undefined, node);
		return;
	}
	
	if (isdefined(tag))
	{
		assertEx (!isdefined(node), "Can't play an animation on a tag for a node, from Animation: " + anime);
		queue_anim_single_solo_with_reach (self, anime, tag, undefined, tag_entity);
		return;
	}
...
queue_anim_single_solo_with_reach ( guy, anime, tag, node, tag_entity )
{
	...
	animePacket process_queue_packet ( guy, anime, tag, node, tag_entity );
}
...
process_queue_packet ( guy, anime, tag, node, tag_entity )
{
	guy endon ("death");
	self.guy = guy;
	self.anime = anime;
	...
	if ( isdefined( node ) )
		self.anime_base = node;
	if ( isdefined( tag_entity ) )
		self.anime_base = tag_entity;
	...
	for (;;)
	{
		...
		if (packet.reach)
			packet.anime_base anim_reach_solo (packet.guy, packet.anime, packet.tag, packet.node, packet.tag_entity);
	
		packet.anime_base anim_single_solo (packet.guy, packet.anime, packet.tag, packet.node, packet.tag_entity);

		packet.guy set_battleChatter(true);
