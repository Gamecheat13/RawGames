...
getEnemyTarget( fRadius, iFOVcos, getAITargets, doSightTrace, getVehicleTargets, randomizeTargetArray )
{
	...
	for( i = 0 ; i < possibleTargets.size ; i++ )
	{
		...
		if ( doSightTrace )
		{
			...
			sightTracePassed = sighttracepassed( self.origin, possibleTargets[i].origin + ( 0, 0, 150 ), false, self );
			if ( !sightTracePassed )
				continue;
		}
