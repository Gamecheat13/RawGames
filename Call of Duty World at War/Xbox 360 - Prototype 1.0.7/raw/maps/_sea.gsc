...
main()
{
	...
	sea = getent("sea","targetname");
	level._sea_org = spawn("script_origin", sea.origin);
	level._sea_org.offset = vector_multiply(sea.origin, -1);
	level._sea_link = spawn("script_origin", sea.origin);
	level._sea_link.offset = level._sea_org.offset;
	sea linkto(level._sea_link);
...
//	level endon("_sea_bob");
	...
	/*	rotationy = randomfloatrange(1, 1.75);
		rotationp = randomfloatrange(.4, .75);
		rotationr = randomfloatrange(1, 2);
		self.time = randomfloatrange(3, 4);
		self.rotation = (rotationp, rotationy, rotationr);*/
...
{
	//sometimes for randomness - don't create a wave
	if(randomint(100) < 25)
		return;
...
{
	wait randomfloat(.15);
	//playfx ( level._effect["watersplash"], self.origin + vec + (0,0, randomfloatrange(32, 96) ), self.up, self.forward);
...
sea_viewbob()
{
	...
	while(1)
	{
		...
		//ang = ((ang[0] - ong[0] + .7) * .1, ang[1], ang[2] * vectordot(for1, for2));
