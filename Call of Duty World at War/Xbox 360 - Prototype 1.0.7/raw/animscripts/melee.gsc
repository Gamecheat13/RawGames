...
//	self endon(anim.scriptChange);
    ...
    for ( ;; )
    {
		...
		self setflaggedanimknoballrestart("meleeanim", %melee_1, %body, 1, 0, 1);
...
CanMeleeInternal( state )
{
	...
	// check if the path to charge is clear.
...
PrepareToMelee()
{
	...
	raiseGunDuration = getanimlength(%run_2_melee_charge) * 1000 - 100;
	raiseGunStartTime = 0;
	...
	while ( 1 )
	{
		...
		else
		{
			...
			if ( time - raiseGunStartTime >= raiseGunDuration || (!willBeWithinRangeWhenGunIsRaised && !withinRangeNow) )
			{
				self SetFlaggedAnimKnobAll("chargeanim", runAnim, %body, 1, .3, 1);
				raisingGun = false;
			}
		...
		predictedEnemyPosAfterRaiseGun = self.enemy.origin + vectorScale( enemyVel, raiseGunDuration );
		...
		if ( raisingGun && self.enemyDistanceSq <= shouldMeleeDistSq && gettime() - raiseGunStartTime >= raiseGunDuration )
			break;
...
meleeStruggleVsDog()
{
	...
	angles = vectorToAngles( self.attacker.origin - self.origin );
	...
	self setflaggedanimrestart( "aianim", meleeSeqAnim[1], 1, 0.2, 1 );
	self waittillmatch( "aianim", "end" );

	for ( seq = 1; seq < 5; seq++ )
	{
		self clearanim( meleeSeqAnim[seq], 0 );
		self setflaggedanimrestart( "aianim", meleeSeqAnim[seq + 1], 1, 0, 1 );
		self waittillmatch( "aianim", "end" );
