...
StandToStandWalk()
{
	assertEX(self.a.pose == "stand", "SetPoseMovement::StandToStandWalk "+self.a.pose);
	assertEX(self.a.movement == "stop", "SetPoseMovement::StandToStandWalk "+self.a.movement);
	
	// %stand_walk_start_straight looks ugly IMO, so just blend straight into walk.
	BlendIntoStandWalk();
...
{
	yaw = getYawToEnemy();
	if ( yaw < 30 && yaw > -30 && self canSeeEnemy() )
	{
		if ( TryFlee() )
			return;
	}
...
TryFlee()
{
	// can be moved to an init function:
	fleeAnims = [];
	fleeAnims[0] = %flee_stand_2_run_short;
	fleeAnims[1] = %flee_stand_2_run_med;
	fleeAnimsPermutation = [];
	fleeAnimDeltas = [];
	fleeAnimDeltaDists = [];
	fleeAnimDeltaYaws = [];
	for ( i = 0; i < fleeAnims.size; i++ )
	{
		fleeAnimsPermutation[i] = i;
		
		fleeAnimDeltas[i] = getMoveDelta(fleeAnims[i], 0, 1);
		fleeAnimDeltaDists[i] = length(fleeAnimDeltas[i]);
		fleeAnimDeltaYaws[i] = getAngleDelta(fleeAnims[i], 0, 1);
	}
	
	// actual functionality:
	
	relativeMoveYaw = vectortoangles(self.lookaheaddir)[1] - self.angles[1];
	fleeAnim = undefined;

	// check all fleeing animations, in a random order,
	// and if we find one that works, use it
	for ( i = 0; i < fleeAnims.size; i++ )
	{
		switchIndex = i + randomint(fleeAnims.size - i);
		fleeAnimIndex = fleeAnimsPermutation[switchIndex];
		fleeAnimsPermutation[switchIndex] = fleeAnimsPermutation[i];
		fleeAnimsPermutation[i] = fleeAnimIndex;
		
		fleeAnim = fleeAnims[fleeAnimIndex];
		
		okdist = fleeAnimDeltaDists[fleeAnimIndex] + 100;
		//println(switchIndex);
		//println(" okdist: ", okdist);
		if ( distanceSquared( self.origin, self.goalpos ) < okdist * okdist )
			continue;
		
		// if we're running farther, we don't need the exit angle to be that close to the way we're trying to run
		yawLeeway = 25 + (distance(self.origin, self.goalpos) - okdist) / 1024.0 * 20.0;
		
		yawdiff = AngleClamp(relativeMoveYaw - fleeAnimDeltaYaws[fleeAnimIndex], "-180 to 180");
		//println(" yawdiff: ", yawdiff);
		//println(" yawLeeway: ", yawLeeway);
		if ( abs( yawdiff ) > yawLeeway )
			continue;
		
		//showDebugLine( self.origin, self.origin + applyOrientationToDelta( forward, right, fleeAnimDeltas[fleeAnimIndex] ), (.3,.7,1), 30 );
		if ( !self maymovetopoint( self localtoworldcoords( fleeAnimDeltas[fleeAnimIndex] ) ) )
			continue;
		
		// we're ok to do this animation
		break;
	}
	
	// give up if we couldn't find a good flee animation
	if ( i == fleeAnims.size )
		return false;
	...
	self setFlaggedAnimKnobAllRestart( "flee", fleeAnim, %body, 1, .3, 1);
	self animscripts\shared::DoNoteTracks("flee");
	self.a.movement = "run";
	...
	// go quickly into run
	PlayBlendTransition(%combatrun_forward_1, .2, "stand", "run", 0);
...
//			self setanimknob(self.a.combatrunanim, 1, 0.0, 1);

			if (usingDefaultRun() && !self.exitingCover)
...
//			self setanimknob(%combat_run_fast_rampup, 1, 0.0, 1);
	...
	else
	{
		...
		else
		{
			...
			if (self.preCombatRunEnabled && !self.exitingCover)
...
usingDefaultRun()
{
	...
	{
		if (self.a.combatrunanim == anim.combatRunAnim[i])
			return true;
	}
	return false;
}