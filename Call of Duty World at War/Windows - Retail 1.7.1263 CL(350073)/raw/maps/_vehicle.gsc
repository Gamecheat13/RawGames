...
init_vehicles()
{
	...
	// MikeD (06/14/07): Removed, and converted it into level.aircraft_list
...
// SCRIPTER_MOD: JesseS (3/14/2008): 			Took out this buggy script which breaks crashpath variations
//		self notify( "newpath" );
...
vehicle_paths( node , baircraftwaitforstart )
{
	...
	if( !isdefined( baircraftwaitforstart ) )
		baircraftwaitforstart = false; // aircrafts emulate startpath() function waiting for a special scripted notify before going

	// SCRIPTER_MOD: JesseS (10/1/2007):  Took this out for now to fix planes		
	//if( baircraftwaitforstart )
	//	self endon( "death" );
...
{
	self endon( "newpath" );
	 // this handles a single node on aircraft path.  they are script_structs in radiant, or script_origins
...
//		else if( is_aircraft() ) // MikeD (07/23/07): Aircrafts should be able to go on splines...
//		{
...
//		}
	...
	// SCRIPTER_MOD: JesseS (10/1/2007):  Added is_aircraft since it doesnt matter anymore
...
spawner_setup( vehicles, message, from )
{
	...
	for( i = 0; i < vehicles.size; i ++ )
	{
		...
		// SCRIPTER_MOD: JesseS (8/13/200):  script_struct instead of script_origin
...
setspawnervariables( vehicle, from )
{
 // 	self.spawnerclassname = vehicle.classname; 
	...
	// SCRIPTER_MOD: JesseS (5/7/07) - script_flak88 added
...
vehicle_spawn( vspawner, from )
{
	...
	// JesseS (05/08/07): script_flak88 added
...
vehicle_init( vehicle )
{
 ...
 // 	vehicle.tanksquadfollow = false; 
	...
	// Guzzo 5-19-08 This should be off now; moved to clientscripts
 ...
 // 		vehicle.script_badplace = true; 
	...
	// CODER_MOD: 5/7/2008 - JamesS - This is now setup in the client scripts
...
kill_damage( type )
{
	...
	// SCRIPTER_MOD: JesseS (8/10/2007): removed level.player
...
// MikeD (07/26/07): Removed and put in aircraft_crash();
//helicopter_crash()
//{
//	self.crashing = true; 
//
...
// MikeD (07/26/07): Remove, put in aircraft_crash_move() instead
...
// MikeD (07/26/07): No longer needed, since we do not have helicopters
...
precache_scripts()
{
 ...
 // 	vehicles = array_combine( vehicles, getentarray( "vehiclespawnmodel", "targetname" ) );
	...
	if( !level.bScriptgened && level.needsprecaching.size > 0 )
	{
		...
		println( " -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- " );
		// CODER_MOD - JamesS - See2 is having some load problems after IW integrate
...
mginit()
{
	...
	// dpg 3/12/08 removed is_aircraft()
...
build_turret( info, tag, model, bAicontrolled, maxrange, defaultONmode, defaultOFFmode, deletedelay, max_turrets )
{
	...
	// dpg 3/12/08 added max turrets incase you want two different models with the same vehicletype	
...
vehicle_rumble()
{
 // makes vehicle rumble
	...
	if(level.clientScripts)
		return;								// CODER_MOD : DSL 06/06/08 - moving vehicle rumble to the client, 4tw.
 ...
 // 	( rumble, scale, duration, radius, basetime, randomaditionaltime )
	...
	while( 1 )
	{
		...
		// SCRIPTER_MOD: JesseS (5/17/2007) - level.player istouching removed, changed to player array
		players = get_players();
		...
		while( player_touching && self.rumbleon && self getspeedmph() > 0 )
		{
			...
			// SCRIPTER_MOD: JesseS (5/17/2007) -  more co-op player checking
			players = get_players();
			player_touching = false;
			for (i = 0; i < players.size; i++)
			{
...
/*
// CODER_MOD: JamesS - 5/8/2008 - moved to client scripts
vehicle_treads()
{
	...
	else if (self.vehicletype == "buffalo" || self.vehicletype == "amtank" || self.vehicletype == "rubber_raft" || self.vehicletype == "jap_ptboat" || self.vehicletype == "jap_shinyo")
	{
		self thread tread( "tag_wake", "back_left" );	
	}
	else
	{
...
turret_shoot()
{
	...
	index = 0;
	turrets = [];
	// CODER_MOD - JamesS check if vehicle_mainTurrets is defined
...
{
	// If we've explicitly set up a vehicle to not build the deathmodel, set it to the regular model
...
//	level.vehicle_shoot_shock[ level.vtmodel ] = shock; // Moved to CSC
}

/* 
 ============= 
 ///ScriptDocBegin
"Name: build_shoot_rumble( <rumble> )"
"Summary: called in individual vehicle file - assigns a rumble file to be played when main cannon on a tank fires "
"Module: vehicle_build( vehicle.gsc )"
"CallOn: "
"MandatoryArg: <rumble> : the rumble gdt entry"
"Example: build_shoot_rumble( "tank_fire" );"
"SPMP: singleplayer"
 ///ScriptDocEnd
 ============= 
 */ 
...
{
	PrecacheRumble( rumble );
//	level.vehicle_shoot_rumble[ level.vtmodel ] = rumble; // Moved to CSC
}
