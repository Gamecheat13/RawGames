...
perks_a_cola_watcher()
{
	...
	while(1)
	{
		iprintlnbold("client:" + self.perk_jingle_playing);
		self waittill("jingle");
		if(self.perk_jingle_playing == 0)
		{
			iprintlnbold(self.perk_jingle_playing);
			self.perk_jingle_playing = 1;			
		}
		else
		{
			iprintlnbold("client resetting:" + self.perk_jingle_playing);
			self.perk_jingle_playing = 0;	
		}
		wait(0.5);
	}
...
{	
	...
	self thread perks_a_cola_watcher();
	while(1)
	{
