...
function infil_zone_selection()
{
	a_volume_list = getentarray( self.target, "targetname" );
 	...
 	if( isdefined( s_sm_volume ) )
 	{
		s_sm_volume thread spawn_infil_zones();
 	}
}
...
function spawn_infil_zones()
{
	if( !isdefined( self.script_string ) )
	{
		...
		while( !b_is_touched )
		{
			...
			{
				if( isdefined(player.sessionstate) && player.sessionstate != "spectator" && player IsTouching( self ) )
				{
					b_is_touched = true;	
				}
			}
	...
	foreach( entity in a_entities )
	{
		if( IsSpawner( entity ) && !isdefined( level._infil_actor_off ) && isdefined( s_handler ) )
		{
			...
			if( !isDefined(entity.script_int) )
			{
				entity handle_role_assignment( s_handler );
			}
			else
			{
				// allow additional infil spawner to be used based on player numbers
				// this works the best if this infil spawn is spawn on touch not at beginning of the game.
				if( level.players.size >= entity.script_int )
				{
					entity handle_role_assignment( s_handler );		
				}
