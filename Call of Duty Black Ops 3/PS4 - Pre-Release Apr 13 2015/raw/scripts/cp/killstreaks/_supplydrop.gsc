...
function registerCrateType( category, type, name, weight, hint, hint_gambler, shareStat, giveFunction, landFunctionOverride )
{
	...
	crateType.type = type;
	crateType.name = name;
	crateType.weapon = GetWeapon( name );
...
function giveSpecializedCrateWeapon( weapon )
{
	...
	{
	case "minigun":
		level thread _popups::DisplayTeamMessageToAll( &"KILLSTREAK_MINIGUN_INBOUND", self );
		level weapons::add_limited_weapon( weapon, self, 3 );
		break;
	case "m32":
		level thread _popups::DisplayTeamMessageToAll( &"KILLSTREAK_M32_INBOUND", self );
		level weapons::add_limited_weapon( weapon, self, 3 );
		break;
	case "m202_flash":
		level thread _popups::DisplayTeamMessageToAll( &"KILLSTREAK_M202_FLASH_INBOUND", self );
		level weapons::add_limited_weapon( weapon, self, 3 );
		break;
	case "m220_tow":
		level thread _popups::DisplayTeamMessageToAll( &"KILLSTREAK_M220_TOW_INBOUND", self );
		level weapons::add_limited_weapon( weapon, self, 3 );
		break;
	case "mp40_blinged":
		level thread _popups::DisplayTeamMessageToAll( &"KILLSTREAK_MP40_INBOUND", self );
		level weapons::add_limited_weapon( weapon, self, 3 );
		break;
...
function incCrateKillstreakUsageStat(weapon)
{
	...
	case "inventory_supplydrop":
		...
		level thread _popups::DisplayKillstreakTeamMessageToAll( "supply_drop", self );
	...
	case "inventory_ai_tank_drop":
		...
		level thread _popups::DisplayKillstreakTeamMessageToAll( "ai_tank_drop", self );
		level.globalKillstreaksCalled++;
...
#/
	...
	chopper thread heliDropCrate( weapon, owner, rear_hatch_offset_local, killCamEnt, killstreak_id, package_contents_id );
	chopper endon("death");
