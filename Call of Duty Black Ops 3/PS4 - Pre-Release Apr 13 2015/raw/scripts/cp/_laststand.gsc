...
function __init__()
{
	...
	{
		level.laststandpistol = GetWeapon( "pistol_standard" );
	}
...
function Laststand_Bleedout( delay )
{
	...
	VisionSetLastStand( "zombie_death", delay * 0.5 );//TODO T7 - update to CP
...
//	util::setClientSysState("lsm", "0", self);	// Notify client last stand ended.
	...
	self thread respawn_player_after_time( 60.0 );
...
function player_laststand( eInflictor, attacker, iDamage, sMeansOfDeath, weapon, vDir, sHitLoc, psOffsetTime, deathAnimDuration )
{
	...
	n_cybercom_status = self HasCyberComRig("cybercom_emergencyreserve");
	if ( n_cybercom_status >= 1  )
	{
		if ( self.lives > 0 )
		{
	...
	{
		// Players without emergency reserves don't get extra lives.
		self.lives = 0;
	}
	...
	VisionSetLastStand( "zombie_last_stand", 1 );//TODO T7 - update to CP 
}
