...
#using scripts\cp\sidemissions\_sm_type_sabotage;
...
{
	o_sidemission = new cSabotageCaches();
	[[o_sidemission]]->init();
	[[o_sidemission]]->start_sidemission();
}
...
function onSpawnPlayer(predictedSpawn, question)
{
	...
	if ( ( isdefined( level.use_legacy_spawnpoints ) && level.use_legacy_spawnpoints ) )
	{
		spawnPoints = spawnlogic::get_spawnpoint_array( "mp_coop_spawn" );
	}
	else
	{
		spawnPoints = spawnlogic::get_spawnpoint_array( "cp_coop_spawn" );
