...
function main()
{
	...
	level.maxRank = int(tableLookup( "gamedata/tables/cp/cp_ranktable.csv", 0, "maxrank", 1 ));
	level.maxPrestige = int(tableLookup( "gamedata/tables/cp/cp_rankIconTable.csv", 0, "maxprestige", 1 ));
	
	rankId = 0;
	rankName = tableLookup( "gamedata/tables/cp/cp_ranktable.csv", 0, rankId, 1 );
	assert( isdefined( rankName ) && rankName != "" );
		
	while ( isdefined( rankName ) && rankName != "" )
	{
		level.rankTable[rankId][1] = tableLookup( "gamedata/tables/cp/cp_ranktable.csv", 0, rankId, 1 );
		level.rankTable[rankId][2] = tableLookup( "gamedata/tables/cp/cp_ranktable.csv", 0, rankId, 2 );
		level.rankTable[rankId][3] = tableLookup( "gamedata/tables/cp/cp_ranktable.csv", 0, rankId, 3 );
		level.rankTable[rankId][7] = tableLookup( "gamedata/tables/cp/cp_ranktable.csv", 0, rankId, 7 );
		level.rankTable[rankId][14] = tableLookup( "gamedata/tables/cp/cp_ranktable.csv", 0, rankId, 14 );
...
function initScoreInfo()
{
	...
	for( row = 1; row < 512; row++ )
	{
		...
		if ( type != "" )
		{
			...
			registerScoreInfo( type, scoreValue, label, teamscore_material );
...
function registerScoreInfo( type, value, label, teamscore_material )
{
...
{
	return tableLookupIString( "gamedata/tables/cp/cp_ranktable.csv", 0, rankId, 16 );
}

function getRankInfoIcon( rankId, prestigeId )
{
	return tableLookup( "gamedata/tables/cp/cp_rankIconTable.csv", 0, rankId, prestigeId+1 );
}

function getRankInfoLevel( rankId )
{
	return int( tableLookup( "gamedata/tables/cp/cp_ranktable.csv", 0, rankId, 13 ) );
}

function getRankInfoCodPointsEarned( rankId )
{
	return int( tableLookup( "gamedata/tables/cp/cp_ranktable.csv", 0, rankId, 17 ) );
}
...
function onPlayerConnect()
{
	for(;;)
	{
		...
		assert( isdefined(player.cur_rankNum), "rank: "+ rankId + " does not have an index, check gamedata/tables/cp/cp_ranktable.csv" );
...
function giveRankXP( type, value, devAdd )
{
	...
	if( level.rankedMatch )
	{
		// Removed a blackbox print for "mpplayerxp" from here
	}
...
{
	if ( GetDvarint( "livestats_giveCPXP" ) == 0 ) 
	{
		return;
	}
	...
	// TODO T7 - UI - clean this up to only receive 1 rank up event when new notifications are created
	// "client_rank_up" sent to all players for the CP player list, uses client num to check which player in the game should update their image
	// "rank_up" only sent to the individual player and those spectating that player
	if ( ( level.gametype === "raid" ) )
	{
		LUINotifyEvent( &"client_rank_up", 3, self getEntityNumber(), rank, prestige );
	}
	...
	self LUINotifyEvent( &"weapon_unlocked", 2, rank, prestige );
}

function getItemIndex( refString )
{
	itemIndex = int( tableLookup( util::getStatsTableName(), 4, refString, 0 ) );
