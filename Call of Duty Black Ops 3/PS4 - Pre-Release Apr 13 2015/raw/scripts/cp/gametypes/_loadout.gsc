...
#precache( "material", "waypoint_second_chance" );
...
function giveLoadout( team, weaponclass )	  
{
	...
	if ( sidearm != level.weaponNull )
	{
		...
		self GiveWeapon( sidearm, secondaryWeaponOptions );
	...
	if ( primaryWeapon != level.weaponNull )
	{
		...
		self GiveWeapon( primaryWeapon, primaryWeaponOptions );		
	...
	if ( ( level.gametype === "coop" ) )
	{
		...
		cybercoreMap["cybercore_control"] = 0;
		cybercoreMap["cybercore_martial"] = 1;
		cybercoreMap["cybercore_chaos"] = 2;

		cybercore_type = self GetLoadoutItemRef( class_num, "cybercore" );
		if ( cybercore_type != "weapon_null" )
		{
			self SetCyberComAbilities( cybercoreMap[cybercore_type], true );
		}

		rig1 = self GetLoadoutItemRef( class_num, "cybercom_tacrig1" );
		rig2 = self GetLoadoutItemRef( class_num, "cybercom_tacrig2" );

		// <msoriano:06.29.15> TODO: change when the upgrade/unlock system is in
		rig1_upgraded = 1;
		rig2_upgraded = 1;

		if ( ( rig1 == "cybercom_multicore" ) || ( rig2 == "cybercom_multicore" ) )
		{
			self SetCyberComAbilities( 0, true );
			self SetCyberComAbilities( 1, true );
			self SetCyberComAbilities( 2, true );
		}
	...
	//	self cybercom::setAbilitiesByFlags( self.cybercom.flags ); //unneccesarry; happens in player spawned callback
	}
	// cybercom end
