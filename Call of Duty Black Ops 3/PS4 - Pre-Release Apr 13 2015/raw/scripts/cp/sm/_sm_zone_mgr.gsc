...
function __init__()
{
	...
	//	"manual_debug" - Zones will only activate via scripted/devgui activation.  No adjacencies.
...
function get_mode()
{
	return level.zone_mgr.str_mode;
}
...
function clear_all_objective_zones()
{
	a_keys = GetArrayKeys( level.zones );
	
	for ( i = 0; i < a_keys.size; i++ )
	{
		level.zones[ a_keys[ i ] ] flag::clear( "objective" );
	}
}
...
//*****************************************************************************
// Returns the name of the zone at the current position.  Undefined if no valid zone
//		exists at that point.
function get_zone_from_position( pos, b_ignore_enabled_check )
{
	str_zone	= undefined;
	v_pos		= undefined;
	e_check		= undefined;
	...
	{
		e_check		 = spawn( "script_origin", pos );
		b_delete	 = false;
	}
	
	foreach( str_zone_name,s_zone in level.zones )
	{
		if ( e_check entity_in_zone( str_zone_name, b_ignore_enabled_check )  )
		{
			str_zone = str_zone_name;
			break;
		}
	}
...
function zone_init( zone_name )
{
	...
	s_zone = level.zones[ zone_name ];
	
	s_zone.zone_name = zone_name;

	s_zone flag::init( "enabled" );	// The zone is not enabled.  You can stop looking at it
									//		until it is.
	s_zone flag::init( "occupied" );	// The zone is occupied by a player.  
	s_zone flag::init( "objective" );	// The zone contains an objective.  
	s_zone flag::init( "active" );	// The spawners will not be added to the spawning list
									//		until this true.  Zones adjacent to objectives 
									//		or players may become active, depending on the 
									//		operating mode
	s_zone flag::init( "spawning_allowed" );

	s_zone.adjacent_zones = [];		// NOTE: These must be defined in a separate level-specific initialization via add_adjacent_zone
	...
	for ( i=0; i<volumes.size; i++ )
	{
		...
		{
			s_zone.volumes[ s_zone.volumes.size ] = volumes[i];
		}
	}

	assert( isdefined( s_zone.volumes[0] ), "zone_init: No volumes found for zone: "+zone_name );

	s_zone.a_spawn_locs = [];
	s_zone.a_spawn_locs[ "general" ] = [];
	if ( isdefined( s_zone.volumes[0].target )  )
	{
		a_s_spawn_locs = struct::get_array( s_zone.volumes[0].target, "targetname" );
		a_nd_spawn_locs = GetNodeArray( s_zone.volumes[0].target, "targetname" );
		a_spawn_locs = ArrayCombine( a_s_spawn_locs, a_nd_spawn_locs, false, false );
		foreach( spawn_loc in a_spawn_locs )
		{
			spawn_loc.zone_name = zone_name;
			is_enabled = false;
			if ( !( isdefined( spawn_loc.is_blocked ) && spawn_loc.is_blocked ) )
			{
				is_enabled = true;
			}
			spawn_loc flag::init( "enabled", is_enabled );

			// Use the spawner_id to indicate spawning possibilities,
			//	e.g. single, double, warlord, soldier, etc.
			if ( isdefined( spawn_loc.spawner_id ) )
			{
				a_str_tokens = StrTok( spawn_loc.spawner_id, " ," );
				
				foreach ( str_token in a_str_tokens )
				{
					if ( !isdefined( s_zone.a_spawn_locs[ str_token ] ) )
					{
						s_zone.a_spawn_locs[ str_token ] = [];
					}
					if ( !isdefined( s_zone.a_spawn_locs[ str_token ] ) ) s_zone.a_spawn_locs[ str_token ] = []; else if ( !IsArray( s_zone.a_spawn_locs[ str_token ] ) ) s_zone.a_spawn_locs[ str_token ] = array( s_zone.a_spawn_locs[ str_token ] ); s_zone.a_spawn_locs[ str_token ][s_zone.a_spawn_locs[ str_token ].size]=spawn_loc;;
				}
			}
			else
			{
				if ( !isdefined( s_zone.a_spawn_locs[ "general" ] ) ) s_zone.a_spawn_locs[ "general" ] = []; else if ( !IsArray( s_zone.a_spawn_locs[ "general" ] ) ) s_zone.a_spawn_locs[ "general" ] = array( s_zone.a_spawn_locs[ "general" ] ); s_zone.a_spawn_locs[ "general" ][s_zone.a_spawn_locs[ "general" ].size]=spawn_loc;;
			}
		}
	}
}
...
function reinit_zone_spawners()
{
	foreach( str_zone,s_zone in level.zones )
	{
		...
		if ( isdefined( s_zone.volumes[0].target )  )
		{
			...
			foreach( spawn_loc in a_spawn_locs )
			{
				spawn_loc.zone_name = str_zone;
				if ( !( isdefined( spawn_loc.is_blocked ) && spawn_loc.is_blocked ) )
				{
					spawn_loc flag::set( "enabled" );
				}
				else
				{
					spawn_loc flag::clear( "enabled" );
				}
...
//				}
				...
				if ( !isdefined( s_zone.a_spawn_locs ) ) s_zone.a_spawn_locs = []; else if ( !IsArray( s_zone.a_spawn_locs ) ) s_zone.a_spawn_locs = array( s_zone.a_spawn_locs ); s_zone.a_spawn_locs[s_zone.a_spawn_locs.size]=spawn_loc;;
			}
		}
	}
}
...
function manage_zones( initial_zone )
{
	...
	while( GetDvarInt( "noclip" ) == 0 ||GetDvarInt( "notarget" ) != 0	 )
	{
		...
		if ( level.zone_mgr.str_mode != "manual" )
		{
...
function debug_show_spawn_locations()
{
	if ( ( isdefined( level.toggle_show_spawn_locations ) && level.toggle_show_spawn_locations )  )
	{
		...
		foreach( location in level.enemy_spawn_locations )
		{
...
function create_spawner_list( a_str_keys )
{
	...
	{
		s_zone = level.zones[ str_key ];
		if ( s_zone flag::get( "enabled" ) && s_zone flag::get( "active" ) && s_zone flag::get( "spawning_allowed" ) )
		{
			foreach ( str_type,a_s_spawn_locs in s_zone.a_spawn_locs )
			{
				if ( !isdefined( level.enemy_spawn_locations[ str_type ] ) )
				{
					level.enemy_spawn_locations[ str_type ] = [];
				}
				
				foreach( s_spawn_loc in a_s_spawn_locs )
				{
					if ( s_spawn_loc flag::get( "enabled" ) )
					{
						if ( !isdefined( level.enemy_spawn_locations[ str_type ] ) ) level.enemy_spawn_locations[ str_type ] = []; else if ( !IsArray( level.enemy_spawn_locations[ str_type ] ) ) level.enemy_spawn_locations[ str_type ] = array( level.enemy_spawn_locations[ str_type ] ); level.enemy_spawn_locations[ str_type ][level.enemy_spawn_locations[ str_type ].size]=s_spawn_loc;;
					}
				}
			}
		}
...
//#define GREY   	( 0.75, 0.75, 0.75 )
	...
	// colors from shared.gsh
	a_colors = [];
	a_colors[ "general" ]	= ( 1, 0, 0 );
	a_colors[ "hunter" ]	= ( 1, 0, 1 );
	a_colors[ "quadtank" ]	= ( 0, 1, 0 );
	v_color = ( 1, 0, 0 );
		...
		{	
			foreach ( s_zone in level.zones )
			{
				if ( ( s_zone flag::get( "active" ) || s_zone flag::get( "occupied" ) ) && s_zone flag::get( "spawning_allowed" ) )
				{
					foreach( str_type,a_s_spawn_locs in s_zone.a_spawn_locs )
					{
						if ( isdefined( a_colors[ str_type ] ) )
						{
							v_color = a_colors[ str_type ];
						}
						else
						{
							v_color = ( 1, 0, 0 );
						...
						foreach ( s_spawn_point in a_s_spawn_locs )
						{
							s_spawn_point debug_show_spawner_location( v_color );
						}
					}
				}
			}	
...
function debug_show_spawner_location( v_color = ( 1, 0, 0 ) )
{
	...
	Box( self.origin, v_box_min, v_box_max, box_yaw, v_color, COLOR_ALPHA, DEPTH_TEST, DURATION );
}


//
//	Sets zone_manager to manual mode and clears all active flags.
//	Zones can then be activated by the manual activate zone 
function manual_clear_all_zones()
{
	set_mode( "manual" );
	...
	for ( i = 0; i < a_keys.size; i++ )
	{
		level.zones[ a_keys[ i ] ] flag::clear( "active" );
	}
	
	iPrintLnBold( "Manual Mode engaged" );
}

//
//	Manually activates a zone.  
function manual_activate_zone()
{
	if ( get_mode() != "manual" )
	{
		iPrintLnBold( "WARNING:  This function should only be used if the zone manager is in \"manual\" mode" );
	}
	
	str_zone = get_zone_from_position( level.players[0].origin, true );
	if ( isdefined( str_zone ) )
	{
		s_zone = level.zones[ str_zone ];
		s_zone flag::set( "active" );
		
		iPrintLnBold( "Zone: "+ s_zone.zone_name + " activated" );
	}
}
