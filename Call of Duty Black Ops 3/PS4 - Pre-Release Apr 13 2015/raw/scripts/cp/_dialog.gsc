...
{
	if ( IsPlayer( self ) && self flagsys::get( "shared_igc" ) )
	{
...
{
	self endon( "death" );
	self thread _say( str_vo_line, n_delay, b_fake_ent, e_to_player );
...
function player_say( str_vo_line, n_delay )
{
	if ( self == level )
	{
		...
		array::wait_till_match( level.players, "done speaking", str_vo_line );
	}
	else
	{
...
@/
function remote( str_vo_line, n_delay )
{
	...
	ent say( str_vo_line, n_delay, true );
	ent Delete();
