...
#precache( "material", "mech_flame_bar" );
#precache( "material", "mech_flame_arrow_flipped" );
...
function init()
{
	...
	//killstreaks::register_dialog( AI_TANK_AGR_NAME, "mpl_killstreak_ai_tank", "kls_aitank_used", "","kls_aitank_enemy", "", "kls_aitank_ready" );
	killstreaks::devgui_scorestreak_command( "inventory_ai_tank_drop", "Debug Routes", "set devgui_tank routes");
		
	killstreaks::register( "ai_tank_drop", "ai_tank_drop", "killstreak_ai_tank_drop", "ai_tank_drop_used",&useKillstreakAITankDrop, false, false, undefined, false );
	...
	killstreaks::register_dialog( "ai_tank_drop", "mpl_killstreak_ai_tank", 0, undefined, 91, 109, 91 );

	remote_weapons::RegisterRemoteWeapon( "killstreak_ai_tank", &"MP_REMOTE_USE_TANK", &startTankRemoteControl, &endTankRemoteControl );
...
{
	if ( !crate valid_location() || !isdefined( owner ) || team != owner.team || owner EMP::EnemyEMPActive() )
	{
...
function valid_location()
{
	...
	node = GetClosestPointOnNavMesh( self.origin, 256, 64 );
...
function ai_tank_killstreak_start( owner, origin, killstreak_id, category )
{
	...
	drone entityheadicons::setEntityHeadIconsHiddenWhileControlling();
	
	// create the influencers
	drone spawning::create_entity_enemy_influencer( "small_vehicle", drone.team );
...
function tank_abort_think()
{
	...
	if (killstreaks::should_not_timeout("inventory_ai_tank_drop"))
	{
		return;
	}		
	self notify( "death" );
...
function tank_damage_think()
{
	...
	tableHealth = killstreaks::get_max_health( "inventory_ai_tank_drop" );
...
#/
		...
		weapon_damage = killstreaks::get_weapon_damage( "inventory_ai_tank_drop", maxhealth, attacker, weapon, mod, damage, flags, chargeLevel );		
		...
		damage_taken += weapon_damage;

		if ( damage_taken >= maxhealth )
		{
			self.health = 0;
		...
		if ( !low_health && damage_taken > maxhealth / 1.8 )
		{
...
#/
	...
	for ( ;; )
	{
		...
		{
			nodes = GetNavPointsInRadius( avg_position, 0, 256 );
		}
		else
		{
			nodes = GetNavPointsInRadius( self.owner.origin, 256, 1024, 128, 1 );
		}
		...
		if ( self SetVehGoalPos( node, true, true ) )
		{
...
function tank_ground_abort_think()
{
	...
	{
		wait( 1 );
		nodes = GetNavPointsInRadius( self.origin, 0, 512, GRID_CELL_SIZE, 1 );
...
function endTankRemoteControl( drone, exitRequestedByOwner ) // self == player
{
	self.owner destroy_remote_hud();
