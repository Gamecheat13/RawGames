...
function init()
{
	...
	killstreaks::devgui_scorestreak_command( "helicopter_guard", "Debug Paths", "toggle scr_devHeliPathsDebugDraw 1 0");
...
function register()
{
	...
	clientfield::register( "vehicle", "vehicle_is_firing", 1, 1, "int");	
}
...
function createHeliGuardSupport( lifeId, killstreak_id )
{
	...
	heliGuard clientfield::set( "enemyvehicle", 1 );
	...
	heliGuard spawning::create_entity_enemy_influencer( "helicopter", heliGuard.team );
...
#/
	...
	if ( killstreaks::should_not_timeout("helicopter_guard") )
	{
		return;
	}
	self thread heliGuardSupport_leave();
