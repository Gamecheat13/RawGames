...
function countdown( localClientNum, oldVal, newVal, bNewEnt, bInitialSnap, fieldName, bWasDemoJump )
{
	...
	// self vehicle::lights_off( localClientNum ); // TODO: add this back when the KillFx bug is fixed.

	for( ;; )
	{
		self PlaySound( localClientNum, "wpn_rcxd_alert" );
		// self vehicle::lights_on( localClientNum ); // TODO: add this back when the KillFx bug is fixed.
