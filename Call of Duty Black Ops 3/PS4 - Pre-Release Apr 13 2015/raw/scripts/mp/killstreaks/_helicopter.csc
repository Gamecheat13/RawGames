...
function useKillstreakHelicopter( hardpointType )
{
	...
	self killstreaks::pick_pilot( hardpointType, 2 );
...
//	level.coptertailrotordamaged_fx = "_t6/vehicle/props/fx_huey_small_blade_dmg";
	...
	if ( tweakables::getTweakableValue( "killstreak", "allowhelicopter_comlink" ) )
	{
		...
		killstreaks::register_dialog("helicopter_comlink", "mpl_killstreak_heli", 28, 39, 106, 124, 106 );
...
function heli_update_global_dvars(debug_refresh)
{
	do
	{
		...
		level.heli_turret_spinup_delay = heli_get_dvar( "scr_heli_turret_spinup_delay", "1.5" );		// seconds it takes for the helicopter mini-gun to spin up before shots fired
...
function OnFlakDroneDestroyed()
{
	...
	if ( isdefined( chopper.owner ) )
	{
		chopper.owner killstreaks::play_taacom_dialog( "helicopter_comlink", 11 );
	}
...
function heli_think( owner, startnode, heli_team, missilesEnabled, protectLocation, hardpointType, armored, killstreak_id )
{
	...
	chopper spawning::create_entity_enemy_influencer( "helicopter", chopper.team );
}
...
function heli_kill_monitor( hardpointtype )
{
	...
	/#	PrintLn( "got killed notify");	#/
		...
		if ( last_kill_vo + kill_vo_spacing < GetTime() )
		{
			...
			if (self.bda == 0) // Huh How does this happen?
			{
				bdaDialog = 15;
			}
			else if (self.bda == 1)
			...
			if ( isdefined( self.owner ) )
			{
				self.owner killstreaks::play_pilot_dialog( "helicopter_comlink", bdaDialog );
			}
...
function heli_damage_monitor( hardpointtype )
{
	...
	tableHealth = killstreaks::get_max_health( hardpointtype );
	...
	for( ;; )
	{
		...
		weapon_damage = killstreaks::get_weapon_damage( hardpointType, self.maxhealth, attacker, weapon, type, damage, flags, chargeLevel );
...
function heli_set_active_camo_state( state )
{
	...
	else if( state == ( 1 ) )
	{
		...
		if ( isdefined( heli.owner ) )
		{
			...
			{
				heli.owner killstreaks::play_pilot_dialog( "helicopter_comlink", 14 );
...
function heli_health( hardpointType, player, playerNotify )
{
	...
	for ( ;; )
	{
		...
		else if ( self.damageTaken >= (self.maxhealth * 0.66) && damageState >= 2 )
		{
			if ( isdefined( self.owner ) )
			{
				self.owner killstreaks::play_pilot_dialog( "helicopter_comlink", 1 );
			}
...
function play_going_down_vo( delay )
{
	...
	if ( isdefined( self.owner ) )
	{
		self.owner killstreaks::play_pilot_dialog( "helicopter_comlink", 2 );
	}
}
...
function heli_leave( hardpointType )
{
	...
	if ( isdefined( self.owner ) )
	{
		self.owner killstreaks::play_pilot_dialog( hardpointType, 5 );
		self.owner killstreaks::play_taacom_dialog_response( hardpointType, 6 );
	}
