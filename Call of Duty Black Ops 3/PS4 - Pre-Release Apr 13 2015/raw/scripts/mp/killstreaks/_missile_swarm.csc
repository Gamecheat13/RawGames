...
#precache( "fx", "killstreaks/fx_hrpy_swrm_os_circle" );
#precache( "fx", "killstreaks/fx_hrpy_swrm_exhaust_trail_close" );
...
function init()
{
	...
	level.swarm_fx[ "swarm" ] = "killstreaks/fx_hrpy_swrm_os_circle";
	...
	killstreaks::create_killstreak_timer( "missile_swarm" );
...
function swarm_killstreak_end( owner, detonate, killstreak_id )
{
		...
		foreach( missile in missiles )
		{
			if ( isdefined( missile ) )
			{
				...
				if ( isdefined( missile.goal ) )
					missile.goal.origin = missile.origin + forward * level.missile_swarm_flydist;
			}
		}
	}
	...
	if ( isdefined( recordStreakIndex ) && isdefined( owner ) )
	{
...
function projectile_spawn( owner )
{
	...
	projectile = projectile_spawn_utility( owner, target, origin, getweapon( "missile_swarm_projectile" ), "swarm_missile", true );
...
function projectile_spawn_utility( owner, target, origin, weapon, targetname, moveGoal )
{
	...
	p clientfield::set( "enemyvehicle", 1 );
/#
