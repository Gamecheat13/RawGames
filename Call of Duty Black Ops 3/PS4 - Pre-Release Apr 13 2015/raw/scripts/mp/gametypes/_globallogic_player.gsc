...
function givePlayerMomentumNotification( score, label, descValue, countsTowardRampage, weapon, combatEfficiencyBonus )
{
	...
	if ( score != 0 )
	{
		self LUINotifyEvent( &"score_event", 4, label, score, rampageBonus, combatEfficiencyBonus );
		self LUINotifyEventToSpectators( &"score_event", 4, label, score, rampageBonus, combatEfficiencyBonus );
	}
...
function resetPlayerMomentumOnSpawn()
{
	if ( isdefined( level.usingScoreStreaks ) && level.usingScoreStreaks )
	{
		...
		{ 
			...
			_setPlayerMomentum( self, start, false );
		}	
		else
		{
...
function onTeamScore_PostProcess( team )
{
	...
	if ( isWinning.size == 1 )
	{
		...
		foreach( team in isWinning )
		{
			...
			globallogic_audio::leader_dialog( "lead_taken", team, "status" );
		}
	}
	...
	if ( level.wasWinning.size == 1)
	{
		...
		foreach( team in level.wasWinning )
		{
			...
			globallogic_audio::leader_dialog( "lead_lost", team, "status" );
		}
	}
...
function trackAttackeeDeath( attackerName, rank, xp, prestige, xuid )
{
	...
	LUINotifyEvent( &"track_victim_death", 2, self.pers["killed_by"][attackerName], selfKillsTowardsAttacker );
