...
#using scripts\mp\gametypes\_persistence;//DO NOT REMOVE - needed for system registration
...
{	
	...
	// need to do a big pass to fully support team free and get rid of the
	// last vestiges of always needing to have "axis" and "allies" teams.
	...
	globallogic_audio::register_dialog_group( "item_destroyed", true );
	globallogic_audio::register_dialog_group( "introboost", true );
	globallogic_audio::register_dialog_group( "status", true );
...
{	
	...
	waitForPlayers();
	globallogic_audio::set_music_global( "spawnPreLoop" );
	level thread sndSetPrematchSnapshot( 1 );
	...
	if ( countTime >= 2 )
	{
		while ( countTime > 0 && !level.gameEnded )
		{
			...
			if( countTime == 3 )
			{
				level thread sndSetPrematchSnapshot( 0 );
...
//					else
//					{
//						player globallogic_audio::set_music_on_player( "none" );
//					}
				}
			}
...
function sndSetPrematchSnapshot( num )
{
	wait(.05);
	level clientfield::set( "sndPrematch", num );
}
...
#/
	...
	for ( index = 0; index < level.players.size; index++ )
	{
		...
		if ( level.wagerMatch )
		{
			...
			player wager::set_after_action_report_stats();
			player persistence::set_after_action_report_stat( "wagerMatch", 1 );
		}
		else
		{
			player persistence::set_after_action_report_stat( "wagerMatch", 0 );
		}
		player persistence::set_after_action_report_stat( "wagerMatchFailed", 0 );
...
function displayGameEnd( winner, endReasonText )
{
	...
	if ( level.teamBased )
	{
		thread globallogic_audio::announce_game_winner( winner, level.postRoundTime / 2 );
...
function updateGameTypeDvars()
{
	...
	while ( game["state"] == "playing" )
	{
		...
		roundScoreLimit = math::clamp( GetGametypeSetting( "roundScoreLimit" ), level.scoreLimitMin, level.scoreLimitMax );
...
{
	if ( weapon.doannihilate&& isPlayer( attacker ) && sMeansOfDeath == "MOD_IMPACT" ) //this does the annihilation effect and launches ragdoll
	{
		if (IsActor(body))
		{
