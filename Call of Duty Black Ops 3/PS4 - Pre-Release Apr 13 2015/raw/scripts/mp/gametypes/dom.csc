...
function main()
{
	...
	globallogic_audio::set_leader_gametype_dialog ( 2, 17, 48, 48 );
	
	game["dialog"]["securing_a"] = 63;
	game["dialog"]["securing_b"] = 64;
	game["dialog"]["securing_c"] = 65;
	
	game["dialog"]["secured_a"] = 66;
	game["dialog"]["secured_b"] = 67;
	game["dialog"]["secured_c"] = 68;
	
	game["dialog"]["secured_all"] = 69;
	
	game["dialog"]["losing_a"] = 73;
	game["dialog"]["losing_b"] = 74;
	game["dialog"]["losing_c"] = 75;
	
	game["dialog"]["lost_a"] = 76;
	game["dialog"]["lost_b"] = 77;
	game["dialog"]["lost_c"] = 78;
	
	game["dialog"]["lost_all"] = 79;
	
	game["dialog"]["enemy_a"] = 70;
	game["dialog"]["enemy_b"] = 71;
	game["dialog"]["enemy_c"] = 72;
	
	game["dialogUpdateTime"] = [];
	game["dialogUpdateTime"]["securing_a"] = 0;
	game["dialogUpdateTime"]["securing_b"] = 0;
	game["dialogUpdateTime"]["securing_c"] = 0;
	
	game["dialogUpdateTime"]["losing_a"] = 0;
	game["dialogUpdateTime"]["losing_a"] = 0;
	game["dialogUpdateTime"]["losing_a"] = 0;
	...
	globallogic_audio::register_dialog_group( "gamemode_objective_a", false );
	globallogic_audio::register_dialog_group( "gamemode_objective_b", false );
	globallogic_audio::register_dialog_group( "gamemode_objective_c", false );
	globallogic_audio::register_dialog_group( "gamemode_changing_a", false );
	globallogic_audio::register_dialog_group( "gamemode_changing_b", false );
	globallogic_audio::register_dialog_group( "gamemode_changing_c", false );
}
...
function onSpawnPlayer(predictedSpawn)
{
	...
	if ( !level.useStartSpawns )
	{
		flagsOwned = 0;
		enemyFlagsOwned = 0;
		enemyTeam = util::getOtherTeam( self.pers["team"]  );
		...
		enemyTeam = util::getOtherTeam( spawnteam );
		...
		if ( flagsOwned == level.flags.size )
		{
			...
			spawnpoint = spawnlogic::get_spawnpoint_near_team( level.spawn_all, getSpawnsBoundingFlag( enemyBestSpawnFlag ) );
		}
		else if ( flagsOwned > 0 )
		{
			// spawn near any flag we own that's nearish something we can capture
			spawnpoint = spawnlogic::get_spawnpoint_near_team( level.spawn_all, getBoundaryFlagSpawns( spawnteam ) );
		}
		else
		{
			// own no flags!
			...
			spawnpoint = spawnlogic::get_spawnpoint_near_team( level.spawn_all, bestFlag.nearbyspawns );
		}
	}
...
function onUseUpdate( team, progress, change )
{
	if ( progress > 0.05 && change && !self.didStatusNotify )
	{
		...
		if ( ownerTeam == "neutral" )
		{
			...
			//statusDialog( "losing"+self.label, otherTeam, "gamemode_changing" + self.label );
		}
		else
		{
	...
	else if( level.flagCanBeNeutralized && ( progress > 0.49 ) && change && self.didStatusNotify )
	{
		...
		if ( ownerTeam != "neutral" )
		{
			...
			statusDialog( "lost"+self.label, ownerTeam, "gamemode_changing" + self.label );
		}
	}
}
...
function flushAllDialog()
{
	globallogic_audio::flush_group_dialog( "gamemode_objective_a" );
	globallogic_audio::flush_group_dialog( "gamemode_objective_b" );
	globallogic_audio::flush_group_dialog( "gamemode_objective_c" );
	globallogic_audio::flush_group_dialog( "gamemode_changing_a" );
	globallogic_audio::flush_group_dialog( "gamemode_changing_b" );
	globallogic_audio::flush_group_dialog( "gamemode_changing_c" );
}

function statusDialog( dialog, team, group, flushGroup )
{
	if ( isdefined( flushGroup ) )
	{
		globallogic_audio::flush_group_dialog( flushGroup );
	}

	dialogIndex = game["dialog"][dialog];
	
	dialogUpdateTime = game["dialogUpdateTime"][dialog];
	time = GetTime();
	
	if ( !isdefined( dialogUpdateTime ) || time > dialogUpdateTime )
	{
		globallogic_audio::leader_dialog( dialogIndex, team, group );
		
		if ( isdefined( dialogUpdateTime ) )
		{
			game["dialogUpdateTime"][dialog] = time + 8000;
		}
	}
}
...
function onEndUse( team, player, success )
{
	if ( !success )
	{
		globallogic_audio::flush_group_dialog( "gamemode_changing" + self.label );
	}
}
...
function onUse( player )
{
	...
	if ( oldTeam == "neutral" )
	{
		...
		{
			statusDialog( "secured_all", team, "gamemode_objective" );
