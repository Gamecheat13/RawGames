...
function updateHardpoints()
{
	...
	for ( i = 0; i < keys.size; i++ )
	{
		...
		if( isdefined( enemyDvar ) && getdvarint( enemyDvar ) == 1 )
		{
			...
			//		foreach ( player in level.players )
			//		{
			//			if( ( host.team != player.team ) && !player util::is_bot() )
			//			{
			//				player killstreaks::give( keys[i] );
			//				break;
			//			}
...
function addEnemyHeli()
{
	...
	case 1:
		...
		break;
	case 2:
		ent helicopter_gunner::ActivateHeliGunner( "helicopter_player_gunner" );
		break;
//	case 3:
//		ent thread _helicopter_player::useKillstreakHelicopterPlayer( "helicopter_player_firstperson" );
//		break;
	}
