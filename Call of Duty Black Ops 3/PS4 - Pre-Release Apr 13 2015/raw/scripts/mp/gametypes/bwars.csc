...
function onSpawnPlayer(predictedSpawn)
{
	...
	if ( !level.useStartSpawns )
	{
		...
		if ( flagsOwned == level.flags.size )
		{
			...
			enemyBestSpawnFlag = level.bestSpawnFlag[ util::getOtherTeam( self.pers["team"] ) ];
		...
		else if ( flagsOwned > 0 )
		{
			...
			spawnpoint = spawnlogic::get_spawnpoint_near_team( level.spawn_all, getBoundaryFlagSpawns( myTeam ) );
		}
		else
		{
			// own no flags!
	...
	{
		if ( spawnteam == "axis")
			spawnpoint = spawnlogic::get_spawnpoint_random(level.spawn_axis_start);
		else
			spawnpoint = spawnlogic::get_spawnpoint_random(level.spawn_allies_start);
	}
...
function updateGametypeDvars()
{
	...
	level.flagCaptureLPM = util::dvarFloatValue( "maxFlagCapturePerMinute", 3, 0, 10 );
	level.playerCaptureLPM = util::dvarFloatValue( "maxPlayerCapturePerMinute", 2, 0, 10 );
	level.playerCaptureMax = util::dvarFloatValue( "maxPlayerCapture", 1000, 0, 1000 );
	level.playerOffensiveMax = util::dvarFloatValue( "maxPlayerOffensive", 16, 0, 1000 );
	level.playerDefensiveMax = util::dvarFloatValue( "maxPlayerDefensive", 16, 0, 1000 );
}
