...
function init_shared()
{
	...
	level.incendiaryfireDamageTickTime = GetDvarFloat( "scr_incendiaryfireDamageTickTime", 0.5 ); // time between damage hits
...
function watchForExplode( owner )
{
	...
	generateLocations( origin, owner, normal );
}
...
function generateLocations( position, owner, normal )
{
	startPos = position + getstepoutdistance( normal );
	rotation = RandomInt( 360 );
	
	killCamEnt = spawn( "script_model", startPos + (0,0,60) );
...
function spawnAllLocs( owner, startPos, normal, multiplier, rotation, killcament, fxCount )
{
	...
	startPos = startPos - getstepoutdistance( normal );
...
{
	fireEffectArea = spawn( "trigger_radius", position, 0, radius, height );
...
/#
	if( GetDvarint( "scr_draw_triggers" ) )
		level thread util::drawcylinder( position, radius, height, undefined, "incendiary_draw_cylinder_stop" );
#/
	...
	while (durationOfIncendiary > 0)
	{
		...
		for (i = 0; i < players.size; i++)
		{
			...
			{
				// since we're not in the pioson area, now see if we're in the fire area
				if (players[i] istouching(fireEffectArea) && players[i].sessionstate == "playing")
				{
					trace = bullettrace( position, players[i].origin + (0,0,12), false, players[i], true );
