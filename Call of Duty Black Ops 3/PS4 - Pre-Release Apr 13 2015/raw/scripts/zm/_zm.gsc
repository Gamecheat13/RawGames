...
#using scripts\zm\aats\_zm_aat_dead_paint;
#using scripts\zm\aats\_zm_aat_honey_business;

#using scripts\zm\bgbs\_zm_bgb_always_done_swiftly;
#using scripts\zm\bgbs\_zm_bgb_good_night_from_good_luck;
#using scripts\zm\bgbs\_zm_bgb_im_feelin_lucky;
#using scripts\zm\bgbs\_zm_bgb_in_plain_sight;
#using scripts\zm\bgbs\_zm_bgb_night_stalker;
...
function zbot_spawn()
{
	...
	bot thread zbot_spawn_init( spawnPoint.origin, yaw );
	
	return bot;
}

function zbot_spawn_init( origin, yaw )
{
	...
	wait( 2.0 );
	self zm_weapons::weapon_give(level.start_weapon, false, false);

	self zm_bot::spawn_init();	
}
...
function init_levelvars()
{
	...
	else //adjust the spawn delay rate 
	{
...
function player_too_many_weapons_monitor_takeaway_sequence( primary_weapons_to_take )
{
	...
	for ( i = 0; i < primary_weapons_to_take.size; i++ )
	{
		...
		self zm_score::minus_to_player_score( score_decrement );
		wait( 3 );
...
/#	println( "*************************Respawn Spectator***" );	#/
	...
	self clientfield::set_to_player( "zmbLastStand", 0 );
	self RevivePlayer();
				...
				for ( i=0; i<keys.size; i++ )
				{
					if ( level.zones[ keys[i] ].is_occupied )
					{
						...
						for ( k=0; k<akeys.size; k++ )
						{
							...
							{
								zm_ai_dogs::special_dog_spawn( undefined, 1 );
								level.zombie_total--;
	...
	foreach ( str_id, s in level.custom_ai_spawn_check_funcs )
	{
		if ( [[ s.func_check ]]() )
		{
			...
			if ( IsDefined( s.func_get_locations ) )
			{
				...
				level.zombie_spawn_locations = ArrayCombine( level.zombie_spawn_locations, a_locations, false, false );
			}
		}
		else
		{
...
function ai_zombie_health( round_number )
{
...
function player_damage_override( eInflictor, eAttacker, iDamage, iDFlags, sMeansOfDeath, weapon, vPoint, vDir, sHitLoc, psOffsetTime )
{
	...
	if ( players.size == 1 && level flag::get( "solo_game" ) && isdefined( level.ai_robot ) )
	{
		return finalDamage;
	}
...
#/
	...
	if( IsDefined( inflictor ) && IsDefined( inflictor.archetype ) && inflictor.archetype == "glaive" )
	{
		...
		{
			if( IsDefined( inflictor.enemy ) && inflictor.enemy != self )
			{
...
/#	println( "end_game TRIGGERED " );	#/
	...
	for ( i = 0; i < players.size; i++ )
	{
		players[i] clientfield::set_to_player( "zmbLastStand", 0 );
	}
...
function is_idgun_damage( weapon )
{
	...
	{
		foreach( idgun_weapon in level.idgun_weapons )
		{
			if( weapon == idgun_weapon )
			{
				return true;
			}
		}
