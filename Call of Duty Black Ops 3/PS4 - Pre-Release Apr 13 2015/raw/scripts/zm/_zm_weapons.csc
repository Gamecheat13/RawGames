...
function wallbuy_player_connect( localClientNum )
{
	...
	for ( i = 0; i < keys.size; i++ )
	{
		...
		target_model = zm_utility::spawn_weapon_model( localClientNum, wallbuy.weapon, target_struct.model, target_struct.origin, target_struct.angles );
		target_model Hide();
...
function wallbuy_callback_idx( localClientNum, oldVal, newVal, bNewEnt, bInitialSnap, fieldName, bWasDemoJump )
{
	...
	else if ( newVal > 0 )
	{
		...
		if ( !isdefined( struct.models[localClientNum] ) )
		{
			...
			target_model = zm_utility::spawn_weapon_model( localClientNum, weapon, model, target_struct.origin, angles); 
			target_model Hide();
			target_model.parent_struct = target_struct;
