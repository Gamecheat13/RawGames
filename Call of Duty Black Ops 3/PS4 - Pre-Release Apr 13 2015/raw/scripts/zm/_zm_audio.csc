...
function __init__()
{
	...
	level thread rpgIDGunImpactWatcher();
...
function rpgIDGunImpactWatcher()
{
	while(1)
	{
		level waittill( "explode", localClientNum, position, mod, weapon, owner_cent );
		
		if( weapon.name == "interdimensional_gun" )
		{
			level thread sndIDgunloopers(position);
		}
	}
}
function sndIDgunloopers(position)
{
	audio::playloopat( "wpn_idgun_portal_loop", position );
	wait(3);
	audio::stoploopat( "wpn_idgun_portal_loop", position );
	playsound( 0, "wpn_idgun_portal_stop", position );
}