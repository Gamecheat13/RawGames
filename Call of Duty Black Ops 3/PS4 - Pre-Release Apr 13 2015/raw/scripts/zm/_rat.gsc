...
function DerRieseZombieSpawnNavmeshTest( params, inRat = true )
{
	...
	foreach( zone in level.zones)
	{
		...
		{
			angles = (0,0,0);
			enemy ForceTeleport(zone.spawn_locations[i].origin, angles );
            ...
            {
	        	if( IsDefined(level.exterior_goals[j].script_string) && level.exterior_goals[j].script_string == zone.spawn_locations[i].script_string )
	            {
			...
			if( isdefined(node) )
			{
				...
				if ( !isPath )
				{
					failed_spawn_origin[size] = zone.spawn_locations[i].origin;
				...
				for( j = 0; j < node.attack_spots.size; j++ )
				{
					...
					if( !isAttackPath )
					{
						failed_attack_spot_spawn_origin[failed_attack_spot_size] = zone.spawn_locations[i].origin;
