...
function debug_show_exterior_goals()
{
	while ( 1 )
	{
		if ( ( isdefined( level.toggle_show_exterior_goals ) && level.toggle_show_exterior_goals ) )
		{
			...
			{
				foreach( location in zone.spawn_locations )
				{
...
function zombie_spawn_init( animname_set )
{
	...
	self.spawn_time = GetTime();	//	Time at spawning
	self.ignoreall = true; 
...
function zombie_findnodes()
{
	...
	if( !IsDefined(self.find_flesh_struct_string) && IsDefined( self.target ) && self.target != "" )
	{
		...
		node = array::get_closest( origin, level.exterior_goals ); 	
...
function zombie_damage( mod, hit_location, hit_origin, player, amount, team, weapon )
{
	...
	else
	{
		...
		{
			player zm_score::player_add_points( damage_type, mod, hit_location, self.isdog, team, self.damageweapon );
		}
	}
	...
	if ( "MOD_IMPACT" != mod && zm_utility::is_placeable_mine( self.damageweapon ) )
	{
		...
		{
			self DoDamage( level.round_number * randomintrange( 100, 200 ), self.origin, player, self, hit_location, mod, 0, self.damageweapon );
		}
		else
		{
			self DoDamage( level.round_number * randomintrange( 100, 200 ), self.origin, undefined, self, hit_location, mod, 0, self.damageweapon );
		}
	}
	...
	else if( mod == "MOD_PROJECTILE" || mod == "MOD_EXPLOSIVE" || mod == "MOD_PROJECTILE_SPLASH" )
	{
		if ( isdefined( player ) && isalive( player ) )
		{
			self DoDamage( level.round_number * randomintrange( 0, 100 ), self.origin, player, self, hit_location, mod, 0, self.damageweapon );
		}
		else
		{
			self DoDamage( level.round_number * randomintrange( 0, 100 ), self.origin, undefined, self, hit_location, mod, 0, self.damageweapon );
		}
	}
...
function zombie_damage_ads( mod, hit_location, hit_origin, player, amount, team, weapon )
{
	...
	else
	{
		...
		if ( !( isdefined( self.no_damage_points ) && self.no_damage_points ) )
		{				
			player zm_score::player_add_points( damage_type, mod, hit_location, undefined, team, self.damageweapon );
		}
	}
		...
		for( i=0; i < level.zombie_spawn_locations.size; i++ )
		{
			if ( ( isdefined( level.use_multiple_spawns ) && level.use_multiple_spawns ) && IsDefined( self.script_int ) )
			{
				if ( !(IsDefined( level.spawner_int ) && (level.spawner_int == self.script_int)) &&
				    !(IsDefined( level.zombie_spawn_locations[ i ].script_int ) || IsDefined( level.zones[ level.zombie_spawn_locations[ i ].zone_name ].script_int )) )
				{
					continue;
				}
