...
function get_player_weapon_with_same_base( weapon )
{
	...
	retweapon = get_weapon_with_attachments( rootWeapon );
	if ( !isdefined( retweapon ) )
	{
		if ( isdefined( level.zombie_weapons[rootWeapon] ) )
		{
			...
			{
				retweapon = get_weapon_with_attachments( level.zombie_weapons[rootWeapon].upgrade );
			}
		}
		else if ( IsDefined( level.zombie_weapons_upgraded[rootWeapon] ) )
		{
			retweapon = get_weapon_with_attachments( level.zombie_weapons_upgraded[rootWeapon] );
		}
	}
	...
	for ( ;; )
	{
		...
		if ( !player_has_weapon )
		{
			...
			if ( player.score >= cost )
			{
				...
				if ( self.weapon == level.weaponRiotshield ) 
				{
					player zm_equipment::give( level.weaponRiotshield );
		...
		else // If the player HAS the weapon, check for ammo update
		{
			...
			else if ( player.score >= ammo_cost ) // if the player does have this then give him ammo.
			{
				...
				if ( ammo_given )
				{
					player zm_score::minus_to_player_score( ammo_cost, true ); // this give him ammo to early
				}
			}
			else
			{
...
function weapon_give( weapon, is_upgrade, magic_box, nosound ) // is_upgrade and magic_box are ignored
{
	...
	{
		self GiveWeapon( weapon, self get_pack_a_punch_weapon_options( weapon ) );
	}
...
function play_weapon_vo( weapon, magic_box )
{
	...
	else
	{
		...
		{
			self zm_audio::create_and_play_dialog( "general", "wallbuy" );
		}
	}
}
...
{
	current = get_player_weapon_with_same_base( weapondata["weapon"] );
