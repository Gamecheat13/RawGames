...
#/	
	//TODO Generalize all "locations" so that we don't have to keep special casing this.
	zone.dog_locations = [];
	zone.screecher_locations = [];
	zone.margwa_locations = [];
	...
	if ( IsDefined( zone.volumes[0].target ) )
	{
		...
		for (i = 0; i < spots.size; i++)
		{
			...
			foreach ( token in tokens )
			{
				...
				{
					if ( !isdefined( zone.dog_locations ) ) zone.dog_locations = []; else if ( !IsArray( zone.dog_locations ) ) zone.dog_locations = array( zone.dog_locations ); zone.dog_locations[zone.dog_locations.size]=spots[i];;
				}
				else if( token == "screecher_location" )
				{
					if ( !isdefined( zone.screecher_locations ) ) zone.screecher_locations = []; else if ( !IsArray( zone.screecher_locations ) ) zone.screecher_locations = array( zone.screecher_locations ); zone.screecher_locations[zone.screecher_locations.size]=spots[i];;
				}
				else if( token == "margwa_location" )
				{
					if ( !isdefined( zone.margwa_locations ) ) zone.margwa_locations = []; else if ( !IsArray( zone.margwa_locations ) ) zone.margwa_locations = array( zone.margwa_locations ); zone.margwa_locations[zone.margwa_locations.size]=spots[i];;
				}
				else if( token == "avogadro_location" )
				{
					if ( !isdefined( zone.avogadro_locations ) ) zone.avogadro_locations = []; else if ( !IsArray( zone.avogadro_locations ) ) zone.avogadro_locations = array( zone.avogadro_locations ); zone.avogadro_locations[zone.avogadro_locations.size]=spots[i];;
				}				
				else if( token == "inert_location" )
				{
					if ( !isdefined( zone.inert_locations ) ) zone.inert_locations = []; else if ( !IsArray( zone.inert_locations ) ) zone.inert_locations = array( zone.inert_locations ); zone.inert_locations[zone.inert_locations.size]=spots[i];;
				}
				else if( token == "quad_location" )
				{
					if ( !isdefined( zone.quad_locations ) ) zone.quad_locations = []; else if ( !IsArray( zone.quad_locations ) ) zone.quad_locations = array( zone.quad_locations ); zone.quad_locations[zone.quad_locations.size]=spots[i];;
				}
				else if( token == "raps_location" )
				{
					if ( !isdefined( zone.raps_locations ) ) zone.raps_locations = []; else if ( !IsArray( zone.raps_locations ) ) zone.raps_locations = array( zone.raps_locations ); zone.raps_locations[zone.raps_locations.size]=spots[i];;
				}
				else if( token == "wasp_location" )
				{
					if ( !isdefined( zone.wasp_locations ) ) zone.wasp_locations = []; else if ( !IsArray( zone.wasp_locations ) ) zone.wasp_locations = array( zone.wasp_locations ); zone.wasp_locations[zone.wasp_locations.size]=spots[i];;
				}
				else if ( token == "mechz_location" )
				{
					if ( !isdefined( zone.mechz_locations ) ) zone.mechz_locations = []; else if ( !IsArray( zone.mechz_locations ) ) zone.mechz_locations = array( zone.mechz_locations ); zone.mechz_locations[zone.mechz_locations.size]=spots[i];;
				}
				else if( token == "astro_location" )
				{
					if ( !isdefined( zone.astro_locations ) ) zone.astro_locations = []; else if ( !IsArray( zone.astro_locations ) ) zone.astro_locations = array( zone.astro_locations ); zone.astro_locations[zone.astro_locations.size]=spots[i];;
				}
				else if( token == "napalm_location" )
				{
					if ( !isdefined( zone.napalm_locations ) ) zone.napalm_locations = []; else if ( !IsArray( zone.napalm_locations ) ) zone.napalm_locations = array( zone.napalm_locations ); zone.napalm_locations[zone.napalm_locations.size]=spots[i];;
				}
				else
				{
					if ( !isdefined( zone.spawn_locations ) ) zone.spawn_locations = []; else if ( !IsArray( zone.spawn_locations ) ) zone.spawn_locations = array( zone.spawn_locations ); zone.spawn_locations[zone.spawn_locations.size]=spots[i];;
				}	
			}
	...
	for ( i = 0; i < level.zones.size; i++ )
	{
		...
		if ( IsDefined( zone.volumes[0].target ) )
		{
			...
			for (j = 0; j < spots.size; j++)
			{
				...
				{
					spots[j].is_enabled = true;
				}
				else
				{
					spots[j].is_enabled = false;
				}
				...
				foreach ( token in tokens )
				{
					if ( token == "dog_location" )
					{
						if ( !isdefined( zone.dog_locations ) ) zone.dog_locations = []; else if ( !IsArray( zone.dog_locations ) ) zone.dog_locations = array( zone.dog_locations ); zone.dog_locations[zone.dog_locations.size]=spots[j];;
					}
					else if( token == "screecher_location" )
					{
						if ( !isdefined( zone.screecher_locations ) ) zone.screecher_locations = []; else if ( !IsArray( zone.screecher_locations ) ) zone.screecher_locations = array( zone.screecher_locations ); zone.screecher_locations[zone.screecher_locations.size]=spots[j];;
					}
					else if( token == "margwa_location" )
					{
						if ( !isdefined( zone.margwa_locations ) ) zone.margwa_locations = []; else if ( !IsArray( zone.margwa_locations ) ) zone.margwa_locations = array( zone.margwa_locations ); zone.margwa_locations[zone.margwa_locations.size]=spots[j];;
					}
					else if( token == "avogadro_location" )
					{
						if ( !isdefined( zone.avogadro_locations ) ) zone.avogadro_locations = []; else if ( !IsArray( zone.avogadro_locations ) ) zone.avogadro_locations = array( zone.avogadro_locations ); zone.avogadro_locations[zone.avogadro_locations.size]=spots[j];;
					}					
					else if( token == "quad_location" )
					{
						if ( !isdefined( zone.quad_locations ) ) zone.quad_locations = []; else if ( !IsArray( zone.quad_locations ) ) zone.quad_locations = array( zone.quad_locations ); zone.quad_locations[zone.quad_locations.size]=spots[j];;
					}
					else if( token == "raps_location" )
					{
						if ( !isdefined( zone.raps_locations ) ) zone.raps_locations = []; else if ( !IsArray( zone.raps_locations ) ) zone.raps_locations = array( zone.raps_locations ); zone.raps_locations[zone.raps_locations.size]=spots[j];;
					}
					else if( token == "wasp_location" )
					{
						if ( !isdefined( zone.wasp_locations ) ) zone.wasp_locations = []; else if ( !IsArray( zone.wasp_locations ) ) zone.wasp_locations = array( zone.wasp_locations ); zone.wasp_locations[zone.wasp_locations.size]=spots[j];;
					}
					else if( token == "mechz_location" )
					{
						if ( !isdefined( zone.mechz_locations ) ) zone.mechz_locations = []; else if ( !IsArray( zone.mechz_locations ) ) zone.mechz_locations = array( zone.mechz_locations ); zone.mechz_locations[zone.mechz_locations.size]=spots[j];;
					}
					else if( token == "astro_location" )
					{
						if ( !isdefined( zone.astro_locations ) ) zone.astro_locations = []; else if ( !IsArray( zone.astro_locations ) ) zone.astro_locations = array( zone.astro_locations ); zone.astro_locations[zone.astro_locations.size]=spots[j];;
					}
					else if( token == "napalm_location" )
					{
						if ( !isdefined( zone.napalm_locations ) ) zone.napalm_locations = []; else if ( !IsArray( zone.napalm_locations ) ) zone.napalm_locations = array( zone.napalm_locations ); zone.napalm_locations[zone.napalm_locations.size]=spots[j];;
					}
					else
					{
						if ( !isdefined( zone.spawn_locations ) ) zone.spawn_locations = []; else if ( !IsArray( zone.spawn_locations ) ) zone.spawn_locations = array( zone.spawn_locations ); zone.spawn_locations[zone.spawn_locations.size]=spots[j];;
					}	
				}
			}
		}
...
function add_adjacent_zone( zone_name_a, zone_name_b, flag_name, one_way )
{
...
function create_spawner_list( zkeys )
{
	...
	for( z=0; z<zkeys.size; z++ )
	{
		...
		if ( zone.is_enabled && zone.is_active && zone.is_spawning_allowed )
		{
			...
			{
				if ( zone.wasp_locations[x].is_enabled )
				{
					level.enemy_wasp_locations[ level.enemy_wasp_locations.size ] = zone.wasp_locations[x];
				}
			}
			...
			{
				if ( zone.raps_locations[x].is_enabled )
				{
					level.enemy_raps_locations[ level.enemy_raps_locations.size ] = zone.raps_locations[x];
				}
			}
			...
			// add zombie_margwa locations
			for(x=0; x<zone.margwa_locations.size; x++)
			{
				if ( zone.margwa_locations[x].is_enabled )
				{
					level.zombie_margwa_locations[ level.zombie_margwa_locations.size ] = zone.margwa_locations[x];
				}
			}			

			// add zombie_avogadro locations
...
function _debug_zones()
{
	...
	while ( true )
	{
		...
		if ( enabled )
		{
			...
			/#	
				text += zone.spawn_locations.size + " spawn";
				zone.debug_hud[5] SetText(zone.spawn_locations.size + " - " + zone.total_spawn_count + " - " + zone.round_spawn_count );
			#/
