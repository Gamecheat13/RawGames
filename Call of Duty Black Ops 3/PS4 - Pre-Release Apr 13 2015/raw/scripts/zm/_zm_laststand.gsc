...
function laststand_disable_player_weapons()
{
	...
	for( i = 0; i < weaponInventory.size; i++ )
	{
		...
		if ( weapon == level.weaponReviveTool )
		{
...
function laststand_clean_up_on_disconnect( playerBeingRevived, reviverWeapon )
{
	...
	self revive_give_back_weapons( reviverWeapon );
}
...
function Laststand_Bleedout( delay )
{
	...
	self clientfield::set_to_player( "zmbLastStand", 1 );
...
function revive_trigger_think()
{
	...
	while ( 1 )
	{
		...
		for ( i = 0; i < players.size; i++ )
		{
			...
			//CODER_MOD: TOMMY K
			revive_successful = reviver revive_do_revive( self, weapon );
			
			reviver revive_give_back_weapons( weapon );
...
function revive_give_back_weapons( weapon )
{
	// take the syrette
...
// self = reviver
function revive_do_revive( playerBeingRevived, reviverWeapon )
{
	...
	self thread laststand_clean_up_on_disconnect( playerBeingRevived, reviverWeapon );
