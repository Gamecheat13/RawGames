...
function state_strafe_update( params )
{
	...
	foreach ( point in queryResult.data )
	{
		...
		{
			point vehicle_ai::AddScore( "distAway", -distanceThreshold );
		}
		point vehicle_ai::AddScore( "distAway", sqrt( distanceToPointSqr ) );
		...
		{
			point vehicle_ai::AddScore( "directionChange", -2000 );
		}
		point vehicle_ai::AddScore( "directionChangeRaw", point.directionChange );
	}
...
function GetNextMovePosition_tactical( enemy )
{
	...
	foreach ( point in queryResult.data )
	{
		if ( !point.visibility )
		{
			point vehicle_ai::AddScore( "no visibility", -600 );
		}

		point vehicle_ai::AddScore( "engagementDist", -point.distAwayFromEngagementArea );
		
		// distance from origin
		point vehicle_ai::AddScore( "distToOrigin", MapFloat( 0, preferedDistAwayFromOrigin, 0, 600, point.distToOrigin2D ) );
...
function hunter_frontScanning()
{
	...
	while ( 1 )
	{
		...
		else if ( !IsDefined( self.enemy ) )
		{
			...
			if ( enemies.size > 0 )
			{
				closest_enemy = array::get_closest( self.origin, enemies );
