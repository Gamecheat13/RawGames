...
{
	ent = self.bot.threat.entity;
	return ( isdefined( ent ) && IsActor( ent ) );
}

function threat_is_actor()
{
	ent = self.bot.threat.entity;
	return ( isdefined( ent ) && IsActor( ent ) );
}
...
function threat_is_equipment()
{
	...
	return ( isdefined( ent.name ) && ent.name.isEquipment );
}
...
function update_cover()
{
	...
	else if ( cover_score > -0.25 )
	{
		...
		nodes = GetNodesInRadiusSorted( self.origin, 512, 0, 256 );

		if ( !nodes.size )
		{
			nodes = GetNodesInRadiusSorted( self.origin, 256, 0, 256 );
		}
...
function turret_set_dangerous( turret )
{
	...
	{
		microwave_radius = 750;
		microwave_cone_dot = cos( 15 );
		
		nodes = GetNodesInRadius( turret.origin, microwave_radius, 0 );
