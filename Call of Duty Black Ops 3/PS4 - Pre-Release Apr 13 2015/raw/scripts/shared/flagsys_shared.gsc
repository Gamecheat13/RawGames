...
function autoexec __init__sytem__() {     system::register("footsteps",&__init__,undefined,undefined);    }
...
function checkSurfaceTypeIsCorrect( moveType, surfaceType )
{
		...
		{
			println( "footsteps_shared.csc:" + surfaceType + " is an incorrect material override. " );
...
function createSoundAliasSlot( movementType, surfaceType, firstperson, isLouder )
{
	isPrecached = false;
...
function missing_ai_footstep_callback()
{
	...
	PrintLn("*** Ai type : " + type + " has a client-script footstep script callback specified, but has no callback specified.  Call footsteps::RegisterAITypeFootstepCB(\""+self._aitype+"\",&yourcbfunc); in your level main .csc file.");		
	#/
}
