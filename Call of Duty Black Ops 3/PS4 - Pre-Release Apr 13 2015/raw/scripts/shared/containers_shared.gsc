...
function update( mod, inflictor, perkFeedback, weapon, victim )
{
	...
	{	
		if ( isdefined( inflictor ) && isdefined( inflictor.soundMod ))
		{
			...
			{
				case "player":
					if ( isdefined( victim ) && isPlayer( victim ) && victim flagsys::get( "gadget_armor_on" ) )
					{
						self playlocalsound ("mpl_hit_alert_armor");				
					}
					else
					{
				...
				default:
				self thread play_hit_sound (mod, "mpl_hit_alert");		
					break;
			}
		}
		else
