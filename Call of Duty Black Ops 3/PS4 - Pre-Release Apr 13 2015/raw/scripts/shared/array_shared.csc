...
"SPMP: both"
@/
function wait_till( &array, str_notify, n_timeout )
{
	...
	foreach ( ent in array )
	{
		if ( isdefined( ent ) )
		{
			...
			ent thread util::timeout( n_timeout, &_waitlogic, s_tracker, str_notify );
		}
	}
...
function _waitlogic( s_tracker, str_notify )
{
	self util::waittill_any( "death", str_notify );
...
{
	return compare_sizes( org, array, dist,&closerFunc );
}
...
{
	return compare_sizes( org, array, dist,&fartherFunc ); 
}

function compare_sizes( org, &array, dist, compareFunc )
{
	if( !array.size )
	{
		return undefined;
...
{
	sorted_array = get_all_closest( org, array, excluders );
...
function merge_sort( &current_list, less_than, param = undefined )
{
	...
	left = merge_sort(left, less_than, param);
	right = merge_sort(right, less_than, param);
...
function merge( left, right, less_than, param )
{
	result = [];
...
{
	if ( array_or_val === value )
	{
		return true;
	}
	else if ( isArray ( array_or_val ) )
	{
