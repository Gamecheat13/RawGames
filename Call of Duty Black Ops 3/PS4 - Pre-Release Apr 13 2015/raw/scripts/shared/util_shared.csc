...
{
/*	if( GetDvarString( "language" ) == "japanese" )
	{
		return true;
	}
*/	
...
function wait_endon( waitTime, endOnString, endonString2, endonString3 )
{
...
function timer_wait( n_wait, n_end_time )
{
	wait n_wait;
	
	n_delta = get_time_in_seconds();
	
	if ( isdefined( n_end_time ) )
	{
		return ( n_delta < n_end_time );
	}
	else
	{
		return n_delta;
...
function init_button_wrappers()
{
	if ( !isdefined( level._button_funcs ) )
	{
		...
		level._button_funcs[ level.BUTTON_USE ]		= &UseButtonPressed;
		level._button_funcs[ level.BUTTON_ADS ]		= &AdsButtonPressed;
		level._button_funcs[ level.BUTTON_ATTACK ]	= &AttackButtonPressed;
...
"Summary: Deletes an entity when it's determined to be safe to delete (sight checks, times alive, etc.)"
...
@/
function query_ents( &a_kvps_match, &a_kvps_ingnore, b_ignore_spawners = false, b_match_substrings = false )
{
	a_ret = [];
...
function get_spawned_players()
{
	...
	foreach ( player in level.players )
	{
		...
		{
			if ( !isdefined( a_spawned ) ) a_spawned = []; else if ( !IsArray( a_spawned ) ) a_spawned = array( a_spawned ); a_spawned[a_spawned.size]=player;;
		}
	}	
	return a_spawned;
}
