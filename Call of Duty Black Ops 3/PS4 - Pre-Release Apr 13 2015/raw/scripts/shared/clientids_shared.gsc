	...
	while ( true )
	{
		...
		if ( !b_player_nearby )
		{
			...
			foreach( s in a_teleport )
			{
				...
				{
					hit_hero = array::get_closest( s.origin, level.heroes, n_telefrag_radius );
				...
				if ( self ForceTeleport( s.origin, s.angles, false, true ) )
				{
					s.teleport_cooldown = GetTime() + n_teleport_cooldown_ms;
					str_color = self.script_forceColor;
					self colors::disable();
					wait 1.0;
					self colors::set_force_color( str_color );
					return;
				}
			}
		}
