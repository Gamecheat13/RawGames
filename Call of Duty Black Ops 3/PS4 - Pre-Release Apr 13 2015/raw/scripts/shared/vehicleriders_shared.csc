...
//	level.vehiclerider_use_index[ "driver" ] = 0; // TODO: bring back if we get driver animations
...
function get_in( vh, str_pos, b_teleport = false )
{
	...
	{
		str_pos = vh find_next_open_position( ai );
...
function exit_variable()
{
	...
	n_fall_time = ( n_cur_height - n_target_height ) / bundle.DropSpeed;
	
	self notify( "falling", n_fall_time );
	
	e_move MoveTo( v_target_landing, n_fall_time );
...
function get_height( e_ignore )
{
	...
	trace = BulletTrace( self.origin + (0,0,height_diff), self.origin + ( 0, 0, -5000 ), false, e_ignore );
