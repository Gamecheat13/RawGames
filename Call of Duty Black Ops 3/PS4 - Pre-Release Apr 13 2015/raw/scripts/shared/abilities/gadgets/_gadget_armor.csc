...
function __init__()
{
	...
	duplicate_render::set_dr_filter_framebuffer( "armor_pl", 40, "armor_on", undefined, 0, "mc/mtl_power_armor" );
/#
...
function armor_update_shader_event( localClientNum, armorStatusNew )
{
	if ( armorStatusNew )
	{
		self duplicate_render::set_dr_flag( "armor_on", true );
	...
	{
		self duplicate_render::set_dr_flag( "armor_on", false );
		self duplicate_render::update_dr_filters();
	}
}
...
function armor_get_shader_color( armorStatusNew )
{
	...
	else if ( armorStatusNew == 4 )
	{
		color = ( 0.02, 0.65, 0.98 );
	}
	else if ( armorStatusNew == 3 )
	{
		color = ( 0.03, 0.82, 0.97 );
	}
	else if ( armorStatusNew == 2 )
	{
		color = ( 0.47, 0.97, 0.96 );
	}
	else if ( armorStatusNew == 1 )
	{
