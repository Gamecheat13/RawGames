...
function skeletonTargetService( behaviorTreeEntity)
{
	...
	if(behaviorTreeEntity.team == "allies")
	{
		...
		{
			behaviorTreeEntity SetGoal( behaviorTreeEntity.favoriteenemy.origin );	
			return true;			
		}
		if(isDefined(behaviorTreeEntity.enemy))
		{
			behaviorTreeEntity SetGoal( behaviorTreeEntity.enemy.origin );	
			return true;			
		}
		target = getClosestToMe(GetAITeamArray( "axis" ));
		if(isDefined(target))
		{
			behaviorTreeEntity SetGoal( target.origin );		
			return true;
		}
		else
		{
			...
			if ( IsDefined( player.last_valid_position ) )
			{
				...
				else
				{
					...
					if( isdefined(influencePos) )
					{
						...
						{
							behaviorTreeEntity SetGoal( influencePos );
							return true;
						}
