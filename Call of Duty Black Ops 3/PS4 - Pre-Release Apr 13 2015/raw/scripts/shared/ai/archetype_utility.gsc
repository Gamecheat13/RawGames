...
function BB_GetLocomotionExitYaw( blackboard, yaw )
{
	...
	///#recordEntText( "Exit Yaw: "+exitYaw, self, RED, "Animscript" );#/
...
function updateFrustrationLevel( behaviorTreeEntity )
{
	...
	if( IsActor( behaviorTreeEntity.enemy ) || IsPlayer( behaviorTreeEntity.enemy ) )
	{
		...
		if( !isAwareOfEnemy && !hasSeenEnemy && !hasAttackedEnemyRecently )
		{
			if( behaviorTreeEntity.frustrationLevel < 4 )
...
function shouldReactToNewEnemy( behaviorTreeEntity )
{
	...
	return stance == "stand" && behaviorTreeEntity.newEnemyReaction;
}
...
///BehaviorUtilityDocEnd
*/
function isSafeFromGrenades( behaviorTreeEntity )
{
	if( behaviorTreeEntity.grenadeAwareness > 0 )
		return behaviorTreeEntity IsSafeFromGrenade();
...
function setNextFindBestCoverTime( behaviorTreeEntity, node )
{
	...
	if( IsDefined( behaviorTreeEntity.enemy ) )
	{
		...
		{
			dist = Distance2D( behaviorTreeEntity.pathgoalpos, behaviorTreeEntity.enemy.origin );
		}
		else
		{
			dist = Distance2D( behaviorTreeEntity.origin, behaviorTreeEntity LastKnownPos( behaviorTreeEntity.enemy ) );
		}
			
		if( dist < behaviorTreeEntity.engageMinDist )
		{
			behaviorTreeEntity.nextFindBestCoverTime = currentTime + RandomIntRange( 6000, 8000 );
		}
		else if( dist > behaviorTreeEntity.engageMinDist && dist < behaviorTreeEntity.engagemaxdist )
		{
...
function shouldStealth( behaviorTreeEntity )
{
	if( behaviorTreeEntity ai::has_behavior_attribute( "stealth" ) 
	    && ( isdefined( behaviorTreeEntity ai::get_behavior_attribute( "stealth" ) ) && behaviorTreeEntity ai::get_behavior_attribute( "stealth" ) ) )
	{
		return true;
	}
...
function locomotionShouldStealth( behaviorTreeEntity )
{
	...
	if( behaviorTreeEntity HasPath() )
	{
		...
		goalRadius = behaviorTreeEntity.goalradius;
		if ( !isDefined( goalRadius ) || goalRadius > 1000 )
			goalRadius = 64;
		
		if ( goalDistSq <= ( goalRadius * goalRadius ) )
			return false;
...
{
	return ( !( isdefined( entity.stealth_reacting ) && entity.stealth_reacting ) && entity HasValidInterrupt( "react" ) );
}
...
function locomotionShouldPatrol( behaviorTreeEntity )
{
	...
	if( behaviorTreeEntity ai::has_behavior_attribute( "patrol" ) 
	    && ( isdefined( behaviorTreeEntity ai::get_behavior_attribute( "patrol" ) ) && behaviorTreeEntity ai::get_behavior_attribute( "patrol" ) ) )
	{
		return true;
	}
...
function meleeAcquireMutex( behaviorTreeEntity )
{
	if( isDefined( behaviorTreeEntity ) && isDefined( behaviorTreeEntity.enemy ))
	{
		...
		if( IsPlayer( behaviorTreeEntity.enemy ) )
		{
			...
			behaviorTreeEntity.enemy.meleeAttackers++;
		}
	}
}
...
function meleeReleaseMutex( behaviorTreeEntity )
{
	...
	if ( isDefined( behaviorTreeEntity.enemy ) )
	{
		behaviorTreeEntity.enemy.melee = undefined;
	}
	...
	if( IsPlayer( behaviorTreeEntity.enemy ) )
	{
		if( isDefined( behaviorTreeEntity.enemy.meleeAttackers) )
		{
			behaviorTreeEntity.enemy.meleeAttackers = behaviorTreeEntity.enemy.meleeAttackers - 1;
			if ( behaviorTreeEntity.enemy.meleeAttackers <= 0 )
			{
				behaviorTreeEntity.enemy.meleeAttackers = undefined;
			}
		}
	}
}
...
function shouldMutexMelee( behaviorTreeEntity )
{
	...
	if ( isDefined( behaviorTreeEntity.enemy))
	{
		...
		else
		{
			...
			if( behaviorTreeEntity.enemy.meleeAttackers < 1 )
			{
				return true;	
			}
			else
...
{
	if(	AiUtility::hasCloseEnemyToMelee( behaviorTreeEntity ) )
		return true;
...
function shouldMelee( entity )
{
	...
	if( entity ai::has_behavior_attribute( "can_melee" ) && !entity ai::get_behavior_attribute( "can_melee" ) )
		return false;
		
	if(	entity.enemy ai::has_behavior_attribute( "can_be_meleed" ) && !entity.enemy ai::get_behavior_attribute( "can_be_meleed" ) )
		return false;
