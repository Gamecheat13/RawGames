...
function private shouldLeanAtCoverCondition( behaviorTreeEntity )
{
	...
	if( (behaviorTreeEntity.node.type == "Cover Left") )
	{
		...
		legalAimYaw = yawToEnemyPosition <= ( aimLimitsForCover["aim_left"] + 10 ) && yawToEnemyPosition >= 0;
	}
	else if( (behaviorTreeEntity.node.type == "Cover Right") )
	{
		...
		legalAimYaw = yawToEnemyPosition >= ( aimLimitsForCover["aim_right"] - 10 ) && yawToEnemyPosition <= 0;
	}
	else if( (behaviorTreeEntity.node.type == "Cover Pillar") )
	{
		...
		if ( !legalAimYaw && !( (isdefined(behaviorTreeEntity.node.spawnflags)&&((behaviorTreeEntity.node.spawnflags & 2048) == 2048))) )
		{
			legalAimYaw = yawToEnemyPosition >= ( aimLimitsForCover["aim_right"] - 10 ) && yawToEnemyPosition <= 0;
		}
	}
...
{
	if( ( isdefined( behaviorTreeEntity ai::get_behavior_attribute( "coverIdleOnly" ) ) && behaviorTreeEntity ai::get_behavior_attribute( "coverIdleOnly" ) ) )
	{
		return true;
	}
