...
function RegisterBehaviorScriptFunctions()
{
	...
	BehaviorTreeNetworkUtility::RegisterBehaviorTreeAction("robotEmpIdleAction",&robotEmpIdleInitialize,&robotEmpIdleUpdate,undefined);;
	...
	BehaviorTreeNetworkUtility::RegisterBehaviorTreeScriptAPI("robotIsInPhalanx",&robotIsInPhalanx);;
	BehaviorTreeNetworkUtility::RegisterBehaviorTreeScriptAPI("robotIsInPhalanxStance",&robotIsInPhalanxStance);;
	BehaviorTreeNetworkUtility::RegisterBehaviorTreeScriptAPI("robotTogglePhalanxStance",&robotTogglePhalanxStance);;
...
function private robotTraverseStart( entity, asmStateName )
{
	...
	traversal.endPoint1 = GetClosestPointOnNavMesh( traversal.endNode.origin, 200, 0 );
...
function private robotIsInPhalanxStance( entity )
{
...
function private robotTogglePhalanxStance( entity )
{
	phalanxStance = entity ai::get_behavior_attribute( "phalanx_force_stance" );
...
function private robotIsInPhalanx( entity )
{
...
function robotCanTacticalJuke( entity )
{
	...
	{
		return AiUtility::calculateJukeDirection(
...
function private robotRushNeighborService( entity )
{
	...
	{
		findPathResult = entity FindPath(
			closestEnemy.origin, closestEnemy.enemy.origin, true, false );
...
function private moveToPlayerUpdate( entity, asmStateName )
{
	...
	shouldRepath = !IsDefined( entity.lastKnownEnemyPos );
	
	if ( !shouldRepath && IsDefined( entity.enemy ) )
	{
		if ( entity.nextMoveToPlayerUpdate <= GetTime() )
		{
	...
	if ( shouldRepath )
	{
		...
		{
			entity.lastKnownEnemyPos = positionOnNavMesh;
		}
		...
		{
			entity.lastKnownEnemyPos = queryResult.data[0].origin;
		}
		
		entity UsePosition( entity.lastKnownEnemyPos );
		
		// Randomized zig-zag path following if 20+ feet away from the enemy.
		if ( DistanceSquared( entity.origin, entity.lastKnownEnemyPos ) > ( (240) * (240) ) )
		{
			path = entity CalcApproximatePathToPosition( entity.lastKnownEnemyPos );
...
function private robotTookEmpDamage( entity )
{
	if ( IsDefined( entity.damageweapon ) && IsDefined( entity.damagemod ) )
	{
		...
		return entity.damagemod == "MOD_GRENADE_SPLASH" &&
			...
			weapon.rootweapon.name == "emp_grenade";
	}
	return false;
}
...
function private _robotOutsideMovementRange( entity, range, useEnemyPos )
{
	...
	return DistanceSquared( entity.origin, goalPos ) > ( (range) * (range) );
}
		...
		if ( !transitionRunning && !subStatePending && !transDecRunning && isBehaviorTreeInRunningState )
		{
			...
			if ( goingToDifferentNode )
			{
				...
				else
				{
					...
					coverNodePoints = GetNavPointsInRadius(
						node.origin,
						60 / 2,
						searchRadius,
						30,
	...
	inflictor, attacker, damage, meansOfDeath, weapon, dir, hitLoc, offsetTime )
{
	...
	entity ai::set_behavior_attribute( "robot_lights", false );
...
function rogueControlForceGoalAttributeCallback( entity, attribute, oldValue, value )
{
	...
	else
	{
		...
		entity UsePosition(
			entity ai::get_behavior_attribute( "rogue_control_force_goal" ) );
	}
}
