...
function update_nav_triggers()
{
	...
	spawner::add_global_spawn_function( "axis", &track_goal_pos );
	spawner::add_global_spawn_function( "allies", &track_goal_pos );
	spawner::add_global_spawn_function( "team3", &track_goal_pos );
	...
	// To test if the AI's goal is in the trigger, we need an entity to do an IsTouching check
	// TODO: Chris said he can provide a code API to check what nav tirgger a position is in so we don't need this
	e_test = util::spawn_model( "tag_origin" );
	...
	while ( true )
	{
		...
		/* Mark valid zones based on if the AI is touching it */
		
		foreach ( trig in a_nav_triggers )
		{
			foreach ( team, a_ai in level.ai )
			{
				if ( !level.navmesh_zones[ trig.targetname ] )
				{
					foreach ( ai in a_ai )
					{
						if ( IsActor( ai ) )
						{
							...
							{
								level.navmesh_zones[ trig.targetname ] = true;
								break;
							}
						}
					}
			...
			/* Mark valid zones based on if the AI goal is inside it */
			
			if ( !level.navmesh_zones[ trig.targetname ] )
			{			
				foreach ( pos in level.valid_navmesh_positions )
				{
					if ( IsVec( pos ) )
					{
						e_test.origin = pos;
					}
					else if ( isdefined( pos.origin ) )
					{
						e_test.origin = pos.origin;
					}
					else
					{
						continue;
...
function track_goal_pos()
{
	self endon( "death" );
	
	self thread track_goal_pos_death();
	
	self._old_goal = undefined;
	
	while ( true )
	{
		self waittill( "goal_changed", goal );
		
		ArrayRemoveValue( level.valid_navmesh_positions, self._old_goal );
		array::add( level.valid_navmesh_positions, goal, false );
		
		self._old_goal = goal;
	}
}

function track_goal_pos_death()
{
	self waittill( "death" );
	ArrayRemoveValue( level.valid_navmesh_positions, self._old_goal );
}
...
"SPMP: singleplayer"
@/
function spawn( b_force = false, str_targetname, v_origin, v_angles )
{
...
function add_spawn_function_group( str_value, str_key = "targetname", func_spawn, param_1, param_2, param_3, param_4, param_5 )
{
	...
	util::array_func( a_spawners, &add_spawn_function, func_spawn, param_1, param_2, param_3, param_4, param_5 );
}
...
function Track_NPC_Deaths()	
{	
	AddDeadNPCToList();
}

function AddDeadNPCToList()
{
		...
		if (GetDvarInt("EnableNPCDeathTracking") == 1)
		{
			...
			else
			{
				...
				else
				{
					...
					{
						level.s_KillsOutput = level.s_KillsOutput + DeadNPCTypeCount.strScoretype + ": " + DeadNPCTypeCount.iCount + "\n";
					}
				}
