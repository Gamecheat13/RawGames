...
function __init__()
{
	...
	foreach ( trig in get_all() )
	{
		...
		if ( isdefined( trig.script_flag_set_on_cleared ) )
		{
			level thread flag_set_on_cleared( trig );
		}
...
function flag_set_on_cleared( trigger )
{
	trigger endon( "death" );
	
	str_flag = trigger.script_flag_set_on_cleared;
	...
	while ( true )
	{
		...
		else
		{
			...
			if ( isdefined( e_entity ) )
			{
				...
				else if ( !e_entity IsTouching( self ) )
				{
					continue;
				}
			}
...
/@
"Name: use( <strName> , <strKey>, <ent> )"
"Summary: Activates a trigger with the specified key / value is triggered"
"Module: Trigger"
"CallOn: "
"OptionalArg: <strName> : Name of the key on this trigger. If name is not passed in, func will check against self"
...
"OptionalArg: <b_assert> : Set to false to not assert if the trigger doesn't exist"
"Example: use( "player_in_building1, "targetname", enemy );"
"SPMP: singleplayer"
@/
