...
function burn_off( local_client_num )
{
	...
	if( GetLocalPlayer( local_client_num ) == self )
	{
		self postfx::PlayPostfxBundle( "pstfx_burn_fade_out" );
	}
}
...
{
	if( ( GetLocalPlayer( local_client_num ) == self ) && ( isthirdperson( local_client_num ) ) )
	{
		self thread _burnBody( local_client_num );
	}
	else if( GetLocalPlayer( local_client_num ) != self )
	{
		self thread _burnBody( local_client_num );
	}
...
function private _burnTagsOn( localClientNum, tags )
{
	...
	while( 1 )
	{
		...
		{
			self.activeFX[self.activeFX.size] = self _burnTag( localClientNum, tags[i], "_loop" );
		}
...
{
	if( isDefined( self ) && isDefined( self.activeFX ) )
	{
