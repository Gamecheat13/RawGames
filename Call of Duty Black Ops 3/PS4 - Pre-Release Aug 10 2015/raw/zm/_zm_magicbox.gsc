		...
		while( !( isdefined( self.closed_by_emp ) && self.closed_by_emp ) )
		{
			...
			if( grabber == user || grabber == level )			
			{
				...
				else if( grabber == level )
				{
					// it timed out
					self.timedOut = true;
					bb::LogPurchaseEvent(user, self, user_cost, self.zbarrier.weapon, false, "_magicbox", "_returned");
					break;
				}
			}
...
//	model moveto( model.origin +( 0, 0, floatHeight ), 3, 2, 0.9 ); 
	...
	if ( rand.isDualWield )
	{
		self.weapon_model_dw = zm_utility::spawn_buildkit_weapon_model( player, rand, undefined, self.weapon_model.origin - ( 3, 3, 3 ), self.weapon_model.angles ); 
	}
