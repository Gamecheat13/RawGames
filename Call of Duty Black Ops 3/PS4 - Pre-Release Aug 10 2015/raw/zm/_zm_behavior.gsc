...
function private InitZmBehaviorsAndASM()
{
	...
	BehaviorTreeNetworkUtility::RegisterBehaviorTreeScriptAPI("zodZombieHasLegs",&zodZombieHasLegs);;
...
function zombieUpdateGoal()
{
	...
	if ( !shouldRepath && IsDefined( self.favoriteenemy ) )
	{
		...
		else if ( DistanceSquared( self.origin, self.favoriteenemy.origin ) <= ( (200) * (200) ) )
		{
...
function zodZombieHasLegs( behaviorTreeEntity )
{
...
function private zombieShouldKnockdown( behaviorTreeEntity )
{
...
function private zombieIsPushed( behaviorTreeEntity )
{
...
{	
	...
	if( IsDefined( behaviorTreeEntity.stumble ) && !IsDefined( behaviorTreeEntity.move_anim_end_time ) )
	{
		...
		behaviorTreeEntity.move_anim_end_time = behaviorTreeEntity.moveTime + GetAnimLength( boardActionAnimation );
	}
