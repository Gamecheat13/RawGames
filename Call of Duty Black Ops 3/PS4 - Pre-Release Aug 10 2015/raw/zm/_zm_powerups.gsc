...
function add_zombie_powerup( powerup_name, model_name, hint, func_should_drop_with_regular_powerups, only_affects_grabber, any_team, zombie_grabbable, fx, client_field_name, time_name, on_name, clientfield_version = 1, player_specific = false )
{
...
function powerup_set_player_specific( powerup_name, b_player_specific = true )
{	
...
function specific_powerup_drop( powerup_name, drop_spot, powerup_team, powerup_location, pickup_delay, powerup_player )
{
...
function powerup_setup( powerup_override,powerup_team, powerup_location, powerup_player )
{
	...
	{
		assert( !( isdefined( struct.player_specific ) && struct.player_specific ), "Player specific powerup dropped with no player specified" ); 
	}
...
function powerup_grab(powerup_team)
{
	...
	while (isdefined(self))
	{
		...
		{
			players = []; 
			players[0] = self.powerup_player;
		}
		else if( IsDefined(level.powerup_grab_get_players_override) )
		{
			players = [[level.powerup_grab_get_players_override]]();
		}
		else
		{
			players = GetPlayers();
		...
		for (i = 0; i < players.size; i++)
		{
			...
			if ( DistanceSquared( players[i].origin, self.origin ) < range_squared || ignore_range )
			{
				...
				*/
				...
				bb::logpowerupevent(self, players[i], "_grabbed");
				if( should_award_stat ( self.powerup_name ) && IsPlayer(players[i]) ) //don't do this for things that aren't really a powerup
				{
