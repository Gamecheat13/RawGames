...
function create_zombie_point_of_interest( attract_dist, num_attractors, added_poi_value, start_turned_on, initial_attract_func, arrival_attract_func,poi_team )
{
	...
	if ( !isdefined( level.zombie_poi_array ) ) level.zombie_poi_array = []; else if ( !IsArray( level.zombie_poi_array ) ) level.zombie_poi_array = array( level.zombie_poi_array ); level.zombie_poi_array[level.zombie_poi_array.size]=self;;
...
function generated_radius_attract_positions( forward, offset, num_positions, attract_radius )
{
	...
	for( i = offset; i < 360+offset; i += degs_per_pos )
	{
		...
		if(( isdefined( level.use_alternate_poi_positioning ) && level.use_alternate_poi_positioning )) // Attractors are defined and sent to the array here
		{
...
{
	if(!isdefined(self.ignorme_count))self.ignorme_count=0;
	self.ignorme_count++;
...
function get_player_perk_purchase_limit()
{
	...
	// bgb override may use the override from everything else as input
	n_perk_purchase_limit_override = self bgb::perk_purchase_limit_override( n_perk_purchase_limit_override );
...
// spawn ent at v_point and test if it's touching any active zone volume; return true/false
// this is a workaround for not having a good way of testing whether a point or struct is inside a volume for now
function is_point_inside_active_zone( v_origin )
{
