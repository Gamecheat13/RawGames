...
function link_spawn_points()
{
	foreach ( spawn_point in get_all_spawn_points() )
	{
		if ( isdefined( spawn_point.linkto ) )
		{
			...
			link_to = GetEnt( spawn_point.linkto, "linkname" );
			linked_spawn LinkTo( link_to );
			
			spawn_point struct::delete();
		}		
		else if ( isdefined( spawn_point.script_linkto ) ) // TODO: old method - delete at some point
		{
			linked_spawn = Spawn( "script_origin", spawn_point.origin );
	...
	while ( true )
	{
		...
		if ( b_debug )
		{
			foreach ( spawn_point in get_all_spawn_points( true ) )
			{
				...
				Print3d( spawn_point.origin + ( 0, 0, -35 ), spawn_point.targetname, color, 1, .3, 1 );				
				Print3d( spawn_point.origin + ( 0, 0, -43 ), (isdefined(spawn_point.script_objective)?spawn_point.script_objective:"no skipto"), color, 1, .3, 1 );
