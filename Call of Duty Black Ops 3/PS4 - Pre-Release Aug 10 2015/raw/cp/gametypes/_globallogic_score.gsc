...
function givePlayerMomentumNotification( score, label, descValue, countsTowardRampage )
{
	...
	if ( score != 0 ) 
	{
		...
		self LUINotifyEventToSpectators( &"score_event", 4, label, score, rampageBonus, combat_efficiency_factor );
	}
...
{	
	if ( rank::shouldSkipMomentumDisplay( event ) )
	{
		return;
	}
	...
	{
		/#
		iprintln( "^1Warning: " + event + " label undefined\n" );
		#/
	...
	for ( killstreakTypeIndex = 0 ; killstreakTypeIndex < killStreakTypeArray.size; killstreakTypeIndex++ )
	{
		...
		if ( momentumCost > oldMomentum && momentumCost <= momentum )
		{
			...
			else
			{
				...
				//player scoreevents::processScoreEvent( activeEventName, player );
			}
		}
	}
}
...
function processAssist( killed_sentient, damagedone, weapon, assist_level )
{
	...
	// should be fine endon disconnect for non players
...
function processKillstreakAssists( attacker, inflictor, weapon )
{
	...
	foreach( player in level.players )
	{
		...
		if ( player.activeCounterUAVs > 0 && !is_killstreak_weapon )
		{
			scoreGiven = thread scoreevents::processScoreEvent( "counter_uav_assist", player );
		...
		{
			if ( player.activeUAVs > 0 && !is_killstreak_weapon )
			{
				scoreGiven = thread scoreevents::processScoreEvent( "uav_assist", player );
				if ( isdefined ( scoreGiven ) )
				{
			...
			if ( player.activeSatellites > 0 && !is_killstreak_weapon )
			{
				scoreGiven = thread scoreevents::processScoreEvent( "satellite_assist", player );
				if ( isdefined ( scoreGiven ) )
				{
	...
	foreach( assistTeam in level.teams )
	{
		...
		if ( isdefined( activeEMPOwner ) && IsPlayer( activeEMPOwner ) )
		{
			if ( isdefined( attacker ) && ( activeEMPOwner != attacker ) )
			{
				...
				{
					scoreGiven = scoreevents::processScoreEvent( "emp_assist", activeEMPOwner );
					if ( isdefined ( scoreGiven ) )
					{
						activeEMPOwner challenges::earnedEMPAssistScore( scoreGiven );
					}
				}
			}
		}
