...
function Callback_PlayerConnect()
{
	...
	// Clear any existing checkpoints because they won't be valid anymore.  The checkpoint system will fall back to the skipto or we can generate a new checkpoint.
	CheckpointClear();
...
#/
	...
	else
	{
		...
		self globallogic_score::initPersStat( "gems", false );
		self.gems = self  globallogic_score::getPersStat( "gems" );
...
function Callback_PlayerDamage( eInflictor, eAttacker, iDamage, iDFlags, sMeansOfDeath, weapon, vPoint, vDir, sHitLoc, vDamageOrigin, psOffsetTime, boneIndex, vSurfaceNormal )
{
	...
	if ( ( sMeansOfDeath === "MOD_TRIGGER_HURT" ) && isdefined(eInflictor) && IsString( eInflictor.script_hazard ) )
	{
		...
		{
			if ( hazard::do_damage( eInflictor.script_hazard, iDamage, eInflictor ) )
			{
				return;
			}
		}
...
//				pixmarker( "END: Callback_PlayerDamage player" );
		...
		if ( sMeansOfDeath == "MOD_PISTOL_BULLET" || sMeansOfDeath == "MOD_RIFLE_BULLET" || sMeansOfDeath == "MOD_PROJECTILE" || sMeansOfDeath == "MOD_GRENADE_SPLASH" || sMeansOfDeath == "MOD_PROJECTILE_SPLASH" )
		{
			...
			{
				should_player_get_death_invulnerability = self gameskill::player_eligible_for_death_invulnerability();
				if( should_player_get_death_invulnerability )
				{
...
#/
	...
	if(self.sessionstate != "dead")
	{
		...
		lpattackerteam = "";
		lpVictimDowned = self laststand::player_is_in_laststand();
		...
		bb::logDamage(eAttacker, self, weapon, iDamage, sMeansOfDeath, sHitLoc, 0, lpVictimDowned );
...
function PlayerKilled_WeaponStats( attacker, weapon, sMeansOfDeath, wasInLastStand, lastWeaponBeforeDroppingIntoLastStand, inflictor )
{
	...
	if ( isPlayer( attacker ) && attacker != self && ( !level.teamBased || ( level.teamBased && self.team != attacker.team ) ) )
	{
		...
		if ( sMeansOfDeath != "MOD_FALLING" )
		{
			...
			{
				attacker AddWeaponStat( inflictor.ownerWeaponAtLaunch, "kills", 1, attacker.class_num, false, true );
			}
			else
			{
...
#/
	...
	if ( !died_in_vehicle && !hit_by_train && (!( isdefined( level.useFPSDeathCam ) && level.useFPSDeathCam ) || level.Players.size > 1 || ( isdefined( level.useCPKillcam ) && level.useCPKillcam ) ) )
	{
	...
	//do this before the entity gets deleted. this is to setup the killcamentity
	if( lpattacknum < 0 )
	{
		if( IsDefined( self.lastStandKillcam ) )
		{
			if( self.lastStandKillcam.eInflictorNum >= 0 && self.lastStandKillcam.attackerNum == self getEntityNumber() )
			{
				lpattacknum = self.lastStandKillcam.eInflictorNum;
			}
			else if( self.lastStandKillcam.attackerNum >= 0 )
			{
				lpattacknum = self.lastStandKillcam.attackerNum;
			}
			else if( self.lastStandKillcam.eInflictorNum >= 0 )
			{
				lpattacknum = self.lastStandKillcam.eInflictorNum;
			}
		}
	...
	{
		cp_player_cam::Apply_FirstPerson_Player_Cam(eInflictor, attacker, iDamage, sMeansOfDeath, weapon, vDir, sHitLoc, psOffsetTime, deathAnimDuration);
		self waittill("cp_playercam_ended");
	}
	
	defaultPlayerDeathWatchTime = 4.0;
	
	if ( SessionModeIsCampaignGame() && level.Players.size > 1) //coop bleeding camera
	{
	...
	if( ( isdefined( level.useCPKillcam ) && level.useCPKillcam ) )
	{
		...
		if( lpattacknum >= 0 )
		{
			self killcam::killcam( self getEntityNumber(), self getEntityNumber(), attacker, lpattacknum, killcamentitystarttime, weapon, self.deathTime, deathTimeOffset, psOffsetTime, true, undefined, perks, killstreaks, self );
...
function Callback_PlayerLastStand( eInflictor, eAttacker, iDamage, sMeansOfDeath, weapon, vDir, sHitLoc, psOffsetTime, delayOverride )
{
	...
	if ( self hascybercomrig("cybercom_emergencyreserve") )
	{
...
function SetZombieCampaignUnlocked()
{
	foreach( player in level.players )
	{
		if( player GetDStat( "allMapsUnlocked" ) )
		{
...
function SaveHeroWeapons()
{
	...
	else if( isPlayer( self ) )
	{
		...
		foreach( weapon in a_w_weapons )
		{
			...
			{
				current_hero_weapons = self savegame::get_player_data( savegame::get_mission_name() + "hero_weapon", undefined );
				if( !IsDefined( current_hero_weapons ) )
				{
					current_hero_weapons = "";
				}
				current_hero_weapons = current_hero_weapons + weapon.name + ",";
				self savegame::set_player_data(savegame::get_mission_name() + "hero_weapon", current_hero_weapons );
			}
		}
	}
}
