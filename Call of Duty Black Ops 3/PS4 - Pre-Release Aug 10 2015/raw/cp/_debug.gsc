...
function debug_nuke()
{
	...
	if( dvar == "on" )
	{
		ai = GetAITeamArray( "axis", "team3" );
		for( i=0;i<ai.size;i++ )
		{
			ignore = false;
			classname = ai[ i ].classname;
			if ( IsDefined( classname ) && ( IsSubStr( classname, "hero" ) || IsSubStr( classname, "boss" ) || IsSubStr( classname, "narrative" ) ) )
			{
				ignore = true;
			}
			if ( !ignore )
...
function debug_coop_bot_test()
{
    ...
    {  	
    	if ( GetDvarInt( "debug_coop_bot_joinleave" ) > 0 )
    	{
		    ...
		    {
		    	n_max_bots = 4 - human_count(); // do not go over 4 total players and bots
 			   	
		    	botCount = bot_count();
		    	
		    	if ( botCount > 0 && RandomInt( 100 ) > 60 )
		        {
	            	util::add_queued_debug_command( "set devgui_bot remove" );

	            	wait 2.0;
		        	
		            DebugMsg( "Bot was removed.   Count=" + bot_count() );
		        }
		        else if( botCount < n_max_bots ) 
		        {
		        	if ( botCount < GetDvarInt( "debug_coop_bot_joinleave" ) && RandomInt( 100 ) > 50 )
			        {
		    			botsAdded = true;
		        		util::add_queued_debug_command( "set devgui_bot add" );

		            	wait 2.0;

		            	DebugMsg( "Bot was added.  Count=" + bot_count() );
			        }
		        }
    	...
    	else if ( botsAdded )
    	{
    		...
    		{
    			util::add_queued_debug_command( "set devgui_bot remove" );
		        wait 2; // delay the disconnections
		        DebugMsg( "Bot was removed.   Count=" + bot_count() );
    		}
...
function dynamic_ai_spawner_find_spawners()
{
	...
	{
		if ( !IsDefined( foundSpawners[ spawner.classname ] ) )
		{
			foundSpawners[ spawner.classname ] = true;
			...
			if ( IsSubStr( className, "actor_" ) )
			{
				struct.radius = 64;
				struct.isVehicle = false;
			
				className = GetSubStr( className, 6 );
			}
			/*
			else if ( IsDefined( vehicleType ) )
			{
				struct.radius = 64;
				struct.isVehicle = true;
...
{	
	...
	while ( true )
	{
		...
		*/
		...
		level.dynamic_spawn_hud settext( "Press X to spawn AI " + level.aitype_index + "/" + level.aitypes.size + " : " + level.aitypes[ level.aitype_index ].classname );
		...
		if( self UseButtonPressed() )
		{
			...
			{
				spawn = SpawnVehicle( level.aitypes[level.aitype_index].classname, origin, angles, "dynamic_spawn_ai" );
			}
			else
			{
				spawn = SpawnActor( level.aitypes[ level.aitype_index ].classname, origin, angles, "dynamic_spawn_ai", true );
			}
...
function debug_health()
{
	...
	for( i = 0; i < guys.size; i++ )
	{
		...
		if (isdefined(guys[i] gettagorigin( "tag_eye" )))
		{
			v_print_pos = guys[i] gettagorigin( "tag_eye" ) + ( 0, 0, 15 );
		}
		else
		{
