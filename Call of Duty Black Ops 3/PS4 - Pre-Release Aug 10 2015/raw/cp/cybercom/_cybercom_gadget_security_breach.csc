...
function init_clientfields()
{
	...
	clientfield::register( "toplayer", "vehicle_hijacked", 1, 1, "int", &player_hijacked_vehicle, !true, !true );
...
function player_hijacked_vehicle(localClientNum, oldVal, newVal, bNewEnt, bInitialSnap, fieldName, bWasDemoJump)
{
	if(newVal)
	{
		self tmodeEnable(0);
		self oed::tmode_Off(localClientNum);
	}
	else
	{
		if(( isdefined( self.tmode_status ) && self.tmode_status ))
		{
			self tmodeEnable(1);
			self oed::tmode_On(localClientNum);
		}
	}
}
