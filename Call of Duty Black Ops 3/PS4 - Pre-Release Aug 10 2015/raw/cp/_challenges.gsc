...
function start_gametype()
{
	...
	if ( canProcessChallenges() )
	{
		...
		challenges::registerChallengesCallback( "actorKilled",&challengeActorKills );
...
{
	// TODO: If we plug up challenges for CP, we will likely need to respond to actor kills.
	// Currently, canProcessChallenges always returns false in CP.
}
...
function waitAndProcessActorKilledCallback( data )
{
	...
	level thread challenges::doChallengeCallback( "actorKilled", data );
	level thread challenges::doScoreEventCallback( "actorKilled", data );
}