...
function devgui_player_commands()
{
	...
	devgui_add_player_commands( level.player_devgui_base, "All:1", 0);
...
function devgui_add_player_commands( root, pname, index)
{
	...
	devgui_add_player_command( player_devgui_root, pid, "Add RankXP 100", 9, "add100RankXP" );
	devgui_add_player_command( player_devgui_root, pid, "Add RankXP 1000", 10, "add1000RankXP" );
	devgui_add_player_command( player_devgui_root, pid, "Add XP to weapon 100", 11, "add100WeaponXP" );
	devgui_add_player_command( player_devgui_root, pid, "Add XP to weapon 1000", 12, "add1000WeaponXP" );
	devgui_add_player_command( player_devgui_root, pid, "Unlock all collectibles", 13, "unlockAllCollectibles" );
	devgui_add_player_command( player_devgui_root, pid, "Unlock all outfits", 14, "unlockOutfits" );
}
...
function devgui_think()
{
	for ( ;; )
	{
		...
		case "infiniteSOLO":
			...
			break;
		case "add100RankXP":
			devgui_handle_player_command( cmd, &devgui_add_rankxp, 100 );
			break;
		case "add1000RankXP":
			devgui_handle_player_command( cmd, &devgui_add_rankxp, 1000 );
			break;
		case "add100WeaponXP":
			devgui_handle_player_command( cmd, &devgui_add_weaponxp, 100 );
			break;
		case "add1000WeaponXP":
			devgui_handle_player_command( cmd, &devgui_add_weaponxp, 1000 );
			break;
		case "unlockAllCollectibles":
			devgui_handle_player_command( cmd, &devgui::devgui_unlock_all_collectibles );
		case "unlockOutfits":
			devgui_handle_player_command( cmd, &devgui_unlock_all_outfits );
			break;
		case "":
			break;
...
function devgui_unlock_all_collectibles()
{
	mission_names = [];
	array::add( mission_names, "cp_mi_cairo_aquifer");
	array::add( mission_names, "cp_mi_cairo_infection");
	array::add( mission_names, "cp_mi_cairo_lotus");
	array::add( mission_names, "cp_mi_cairo_ramses");
	array::add( mission_names, "cp_mi_eth_prologue");
	array::add( mission_names, "cp_mi_sing_biodomes");
	array::add( mission_names, "cp_mi_sing_blackstation");
	array::add( mission_names, "cp_mi_sing_sgen");
	array::add( mission_names, "cp_mi_sing_vengeance");
	array::add( mission_names, "cp_mi_zurich_coalescence");
	array::add( mission_names, "cp_mi_zurich_newworld");
	foreach( mission in mission_names ) 
	{
		for( i = 0; i < 10; i++ )
		{
			self SetDStat( "PlayerStatsByMap", mission, "collectibles", i, 1 );
		}
	}
}

function devgui_unlock_all_outfits()
{
	mission_names = [];
	array::add( mission_names, "cp_mi_cairo_aquifer" );
	array::add( mission_names, "cp_mi_cairo_infection" );
	...
	{
		self SetDStat( "PlayerStatsByMap", mission_name, "hasBeenCompleted", 1 );
	}
}

function devgui_add_rankxp( numXP )
{
	assert( IsDefined( self ) );
	assert( IsPlayer( self ) );
	
	self AddRankXPValue( "kill_cqb", numXP );
}

function devgui_add_weaponxp( numXP )
{
	assert( IsDefined( self ) );
	assert( IsPlayer( self ) );
	
	weaponNum = Int( tableLookup( "gamedata/stats/cp/cp_statstable.csv", 3, self.currentweapon.rootweapon.displayname, 0 ) );
	
	currentWeaponXP = self GetDStat( "ItemStats", weaponNum, "xp" );
	
	self SetDStat( "ItemStats", weaponNum, "xp", numXP + currentWeaponXP );
}
...
function devgui_player_weapons()
{
	...
	AddDebugCommand( player_devgui_base_cp + "All:1" + "/Weapons/Toggle Use Give Cmd\" \"toggle " + "cp_weap_use_give_console_command_devgui" + " 1 0\" \n");
	AddDebugCommand( player_devgui_base_cp + "All:1" + "/Weapons/Toggle Debug Center Screen\" \"toggle " + "debug_center_screen" + " 1 0\" \n");
	AddDebugCommand( player_devgui_base_cp + "All:1" + "/Weapons/Toggle Weapon Asset Name Display\" \"toggle " + "cp_weap_asset_name_display_devgui" + " 1 0\" \n");

	devgui_add_player_weapons( player_devgui_base_cp, "All:1", 0, a_grenades_cp, "Grenades" );
	devgui_add_player_weapons( player_devgui_base_cp, "All:1", 0, a_weapons_cp, "Guns" );
	devgui_add_player_weapons( player_devgui_base_cp, "All:1", 0, a_misc_cp, "Misc");
	devgui_add_player_gun_attachments( player_devgui_base_cp, "All:1", 0, a_weapons_cp, "Attachments" );
