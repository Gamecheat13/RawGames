...
{	
	random_start_index = randomInt( level.heli_startnodes.size );
	startnode = level.heli_startnodes[ random_start_index ];
	path_index = ( random_start_index + 1 ) % level.heli_startnodes.size;
	
	noFlyZone = airsupport::crossesNoFlyZone( startnode.origin, dest );
	while ( isdefined( noFlyZone ) && path_index != random_start_index )
	{
	...
	return level.heli_startnodes[ path_index ] ;
}
...
function heli_think( owner, startnode, heli_team, missilesEnabled, protectLocation, hardpointType, armored, killstreak_id )
{
	...
	chopper = spawn_helicopter( owner, heliOrigin, heliAngles, "heli_ai_mp", chopperModelFriendly, (0,0,0), hardpointType, killstreak_id );
