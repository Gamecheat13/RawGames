...
function giveLoadoutOldschool( team, weaponclass )	  
{
	level.weaponOldschool = GetWeapon("pistol_standard");
	
	self giveLoadout_init();
			
	self.pers["primaryWeapon"] = level.weaponOldschool;
	self GiveWeapon(level.weaponOldschool);
	
	self.pers["changed_class"] = false;
	self.spawnWeapon = level.weaponOldschool;
	self.pers["spawnWeapon"] = self.spawnWeapon;
	self setSpawnWeapon( self.spawnWeapon );	
	self teams::set_player_model( team, self.spawnWeapon );
	
	self ClearPerks();
	
	self giveLoadout_finalize(self.spawnWeapon, self.spawnWeapon);
}

function giveLoadout_init()
{
	self takeAllWeapons();	  	
...
//		/#	println( "^5Loadout " + self.name + " GiveWeapon( " + secondaryOffhand.name + " ) -- secondaryOffhand" );	#/
	...
	/#
		...
		{
			heroGagdetName = GetDvarString( "scr_heroGadgetName_debug" );
			
			specialOffhand = level.weaponNone;
			if ( heroGagdetName != "weapon_null" )
			{
	...
	// Don't let bots have special weapons for now
...
//		/#	println( "^5Loadout " + self.name + " GiveWeapon( " + specialOffhand.name + " ) -- specialOffhand" );	#/
	...
	heroWeaponName = self GetLoadoutItemRef( class_num_for_global_weapons, "heroWeapon" );
	
	/#
		if ( GetDvarString( "scr_heroWeaponName_debug" ) != "" )
		{
			heroWeaponName = GetDvarString( "scr_heroWeaponName_debug" );
		}
	#/
	...
	pixendevent(); // "giveLoadout 8"
	pixbeginevent("giveLoadout 9");
	if ( heroWeapon != level.weaponNone )
	{
//		/#	println( "^5Loadout " + self.name + " GiveWeapon( " + heroWeapon.name + " ) -- heroWeapon" );	#/
	...
	pixendevent(); // "giveLoadout 12"
	pixbeginevent("giveLoadout 13");
	// cac specialties that require loop threads
	self cac_selector();
	
	self giveLoadout_finalize(spawnWeapon, primaryWeapon);
	
	pixendevent(); // "giveLoadout 13"
