...
{		
	...
	if ( !throwIfPossible ) 
	{
		...
		{
			if( IsDefined( grenadeThrowInfo.data.grendadeThrownAt ) && IsAlive( grenadeThrowInfo.data.grendadeThrownAt ) )
			{
				if( grenadeThrowInfo.data.grendadeThrownAt == behaviorTreeEntity.enemy )
				{
					return false;
				}
			}
...
{	
	...
	Blackboard::AddBlackboardEvent( "human_grenade_throw", grenadeThrowInfo, RandomIntRange( 7000, 10000 ) );
}
