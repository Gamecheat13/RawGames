...
function get_closest_valid_player( origin, ignore_player )
{
	...
	while ( targets.size && !done )
	{
		...
		{
			target = targets[i];
			if( !is_player_valid( target, true ) )
			{
	...
	while(sortedPotentialTargets.size)
		{
		if( is_player_valid( sortedPotentialTargets[0], true ) )
			{
			pixendevent();
...
function get_round_enemy_array()
{
	a_ai_enemies = [];
	a_ai_valid_enemies = [];
	a_ai_enemies = GetAiSpeciesArray( level.zombie_team, "all" );
...
function set_zombie_run_cycle( new_move_speed )
{
	...
	{
		debug_variant_type = GetDvarInt( "scr_zombie_variant_type" );
