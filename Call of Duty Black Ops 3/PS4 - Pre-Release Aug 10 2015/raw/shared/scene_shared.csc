...
function player_scene_skip( localClientNum, oldVal, newVal, bNewEnt, bInitialSnap, fieldName, bWasDemoJump ) // self = player
{
	if(newVal == 1)
	{
		//iprintlnbold((self GetPlayerName()) + " is skipping the scene.");
	}
	else
	{
		//iprintlnbold("");
...
function player_scene_animation_skip( localClientNum, oldVal, newVal, bNewEnt, bInitialSnap, fieldName, bWasDemoJump )
{
	...
	if(isdefined(anim_name) && anim_name != "")
	{
		...
		if(!is_looping)
		{
			...
			{
				PrintTopRightln( "player_scene_animation_skip: " + anim_name + " : " + GetTime(), (0.6, 0.6 , 0.6) );
			}
			#/
...
function __init__()
{
	...
	clientfield::register( "toplayer", "in_igc", 1, 1, "int", &in_igc, !true, !true );
	
	/*
	for(i = 0; i < 4; i++)
	{
		clientfield::register( "toplayer", "player_scene_skip_" + i, VERSION_SHIP, 1, "int", &play_vehicle_anim, !CF_HOST_ONLY, !CF_CALLBACK_ZERO_ON_NEW_ENT );
	}
	*/
	
	clientfield::register( "allplayers", "player_scene_skip", 1, 1, "int", &player_scene_skip, !true, !true );
	
	clientfield::register( "allplayers", "player_scene_animation_skip", 1, 2, "counter", &player_scene_animation_skip, !true, !true );
	clientfield::register( "actor", "player_scene_animation_skip", 1, 2, "counter", &player_scene_animation_skip, !true, !true );
	clientfield::register( "vehicle", "player_scene_animation_skip", 1, 2, "counter", &player_scene_animation_skip, !true, !true );
	clientfield::register( "scriptmover", "player_scene_animation_skip", 1, 2, "counter", &player_scene_animation_skip, !true, !true );
...
function in_igc( localClientNum, oldVal, newVal, bNewEnt, bInitialSnap, fieldName, bWasDemoJump )
{
	IGCactive( localClientNum, newVal ); // Tell code that an IGC is active so it can turn off tac-mode and stuff
}
...
function postfx_igc( localClientNum, oldVal, newVal, bNewEnt, bInitialSnap, fieldName, bWasDemoJump )
{
	...
	self notify( "postfx_igc" );
	self endon( "postfx_igc" );
	...
	filter::set_filter_base_frame_transition_warp( self, 5, 1.0 );
	filter::set_filter_base_frame_transition_boost( self, 5, 0.5 );
	filter::set_filter_base_frame_transition_durden( self, 5, 1.0 );
	filter::set_filter_base_frame_transition_durden_blur( self, 5, 1 );
	
	filter::set_filter_sprite_transition_elapsed( self, 5, 0 );
	filter::set_filter_sprite_transition_octogons( self, 5, 1.0 );
	filter::set_filter_sprite_transition_blur( self, 5, 0.0 );
	filter::set_filter_sprite_transition_boost( self, 5, 0.0 );
	
	filter::set_filter_frame_transition_light_hexagons( self, 5, 0.0 );
	filter::set_filter_frame_transition_heavy_hexagons( self, 5, 0.0 );
	filter::set_filter_frame_transition_flare( self, 5, 0.0 );
	filter::set_filter_frame_transition_blur( self, 5, 0.0 );
	filter::set_filter_frame_transition_iris( self, 5, 0.0 );
	filter::set_filter_frame_transition_saved_frame_reveal( self, 5, 0.0 );
	
	filter::set_filter_frame_transition_warp( self, 5, 0 ); //zoom in on screen
	filter::set_filter_sprite_transition_move_radii( self, 5, 0, 0 );  // inner (outer - 256), outer (2000)
	
	if ( !( isdefined( self.pstfx_cp_transition_sprite ) && self.pstfx_cp_transition_sprite ) )
	{
		CreateSceneCodeImage( localClientNum, "pstfx_cp_transition_sprite" );
		self.pstfx_cp_transition_sprite = true;
	}
	
	CaptureFrame( localClientNum, "pstfx_cp_transition_sprite" );
	setFilterPassCodeTexture( localClientNum, 5, 0, 0, "pstfx_cp_transition_sprite" );
	
	filter::set_filter_base_frame_transition_warp( self, 5, 1.0 );
	filter::set_filter_base_frame_transition_boost( self, 5, 1.0 );
	...
	// loop the effect
	i = GetTime() - startTime;
	while ( i < 2000 && isdefined( self ) ) //-- 4 SECOND LOOP
	{
		sT = i / 1000.0; //convert ms to sec
		
		if ( sT <= 0.5)
		{
			filter::set_filter_frame_transition_iris( self, 5, math::linear_map(sT, 0, 0.5, 0, 1 ) );
		}
		else if( sT > 0.5 && sT <= .85 )
		{
			filter::set_filter_frame_transition_iris( self, 5, 1 - math::linear_map(sT, 0.5, 0.85, 0, 1 ) );
		}
		else
		{
			filter::set_filter_frame_transition_iris( self, 5, 0.0 );
		...
		if ( sT > 0.5 && sT <= 1.0)
		{
			n_hex = math::linear_map(sT, 0.5, 1.0, 0, 1 );
			filter::set_filter_frame_transition_light_hexagons( self, 5, n_hex );
			
			if ( sT >= 0.8 )
			{
			    filter::set_filter_frame_transition_flare( self, 5, math::linear_map(sT, 0.8, 1, 0, 1 ) );
			}
		}
		else if( sT > 1.0 && sT < 1.5 )
		{
			filter::set_filter_frame_transition_light_hexagons( self, 5, 1.0 );
			filter::set_filter_frame_transition_flare( self, 5, 1.0 );
		}
		else
		{
			filter::set_filter_frame_transition_light_hexagons( self, 5, 0.0 );
		...
		if ( sT > 0.65 && sT <= 1.15)
		{
			filter::set_filter_frame_transition_heavy_hexagons( self, 5, math::linear_map(sT, 0.65, 1.15, 0, 1 ) );
		}
		else if( sT > 1.15 && sT < 1.50 )
		{
			filter::set_filter_frame_transition_heavy_hexagons( self, 5, 1.0 );
		}
		else
		{
			filter::set_filter_frame_transition_heavy_hexagons( self, 5, 0.0 );
		...
		if ( sT > 1.0 && sT <= 1.5 )
		{
			filter::set_filter_frame_transition_blur( self, 5, math::linear_map(sT, 1.0, 1.5, 0, 1 ) );
			filter::set_filter_sprite_transition_boost( self, 5, math::linear_map(sT, 1.0, 1.5, 0, 1 ) );
			filter::set_filter_frame_transition_saved_frame_reveal( self, 5, math::linear_map(sT, 1.0, 1.5, 0, 1 ) );
			filter::set_filter_base_frame_transition_durden_blur( self, 5, 1.0 - math::linear_map(sT, 1.0, 1.5, 0, 1 ) );
			filter::set_filter_sprite_transition_blur( self, 5, math::linear_map(sT, 1.0, 1.5, 0, 0.1 ) );
		}
		else if ( sT > 1.5 )
		{
			filter::set_filter_frame_transition_blur( self, 5, 1.0 );
			filter::set_filter_sprite_transition_boost( self, 5, 1.0 );
			filter::set_filter_frame_transition_saved_frame_reveal( self, 5, 1.0 );
			filter::set_filter_base_frame_transition_durden_blur( self, 5, 0.0 );
			filter::set_filter_sprite_transition_blur( self, 5, 0.1 );
		}
		
		if ( sT > 1.0 && sT <= 1.45 )
		{
			filter::set_filter_base_frame_transition_boost( self, 5, math::linear_map(sT, 1.0, 1.45, 0.5, 1 ) );
		}
		...
		{
			filter::set_filter_base_frame_transition_boost( self, 5, 1.0 - math::linear_map(sT, 1.75, 2.0, 0, 1 ) );
		}
		  
		if ( sT >= 1.75 )
		{
			val = 1.0 - math::linear_map(sT, 1.75, 2.0, 0, 1 );
		...
		if ( sT >= 1.25 )
		{
			val = 1.0 - math::linear_map(sT, 1.25, 1.75, 0, 1 );
		...
		if ( sT >= 1.75 && sT < 2.0 )
		{
			filter::set_filter_base_frame_transition_durden( self, 5, 1.0 - math::linear_map(sT, 1.75, 2.0, 0, 1 ) );
		}
		...
		if ( sT > 1.0 )
		{
			...
			outer_radii = math::linear_map(sT, 1.0, 1.50, 0, 2000 );
			filter::set_filter_sprite_transition_move_radii( self, 5, outer_radii - 256, outer_radii );
		}
		
		if (sT > 1.15 && sT < 1.85 )
		{
			filter::set_filter_frame_transition_warp( self, 5, -1 * (math::linear_map(sT, 1.15, 1.85, 0, 1 )) );
		}
		else if( sT >= 1.85 )
		{
			filter::set_filter_frame_transition_warp( self, 5, -1 * (1 - math::linear_map(sT, 1.85, 2.0, 0, 1 )) );
		}
	...
	FreeCodeImage( localClientNum, "pstfx_cp_transition_sprite" );
	if( isdefined( self ) )
	{
		self.pstfx_cp_transition_sprite = undefined;
	}
