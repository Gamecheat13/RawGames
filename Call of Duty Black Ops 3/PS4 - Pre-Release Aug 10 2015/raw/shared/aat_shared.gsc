	...
	for ( ;; )
	{
		...
		if ( aat_name != "" )
		{
			for ( i = 0; i < level.players.size; i++ )
			{
				...
				level.players[i] thread aat_set_debug_text( aat_name, false, false, false );
			}
		}
...
/#
	...
	percentage = "N/A";
	if ( IsDefined( level.aat[name] ) && name != "none" )
	{
...
/#
	...
	aat_percentage_override = GetDvarFloat( "scr_aat_percentage_override" );
	if ( aat_percentage_override > 0.0 )
	{
...
function watch_weapon_changes()
{
	...
	while ( isdefined( self ) )
	{
		...
		n_index = level.aat[ name ].n_index;
		clientfield::set_to_player( "aat_current", n_index );
	}
}
