...
function update_last_held_weapon_timings( newTime )
{
	if ( isdefined( self.currentWeapon ) && isdefined( self.currentWeaponStartTime ) )
	{
		...
		if ( totalTime > 0 )
		{
			self AddWeaponStat( self.currentWeapon, "timeUsed", totalTime, self.class_num, self.currentweapon.isPickedUp );
...
function watch_pickup()
{
	...
	// For isPickedUp, you don't have to be the one who killed the previous owner for this flag to be true.
	// Note that this logic is different than the player.tookWeaponFrom array.
	// Also, don't consider your own weapon "picked up"
...
function watch_usage()
{
	...
	{	
		...
		switch ( curWeapon.weapClass )
		{
			...
			case "rocketlauncher":
			case "grenade":
				self AddWeaponStat( curWeapon, "shots", 1, self.class_num, curWeapon.isPickedUp );
				break;
			default:
				break;
		}
...
function track_fire( curWeapon )
{
	...
	self TrackWeaponFireNative( curWeapon, 1, self.hits, 1, self.class_num, curWeapon.isPickedUp );
