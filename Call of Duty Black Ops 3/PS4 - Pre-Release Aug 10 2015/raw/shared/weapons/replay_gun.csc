...
function __init__()
{
	...
	duplicate_render::set_dr_filter_offscreen( "replay", 75, 
	                                "replay_locked",                        undefined,                    
	                                2, "mc/hud_outline_model_red", 0  );
...
function watch_lockon( localClientNum )
{
	...
	while(1)
	{
		...
		{
		   	self.replay_lock duplicate_render::change_dr_flags( undefined, "replay_locked" );
		...
		if ( IsDefined( target ) && (target IsPlayer() || target IsAi()) && IsAlive( target ) )
		{
			switch(state)
			{
				...
				case 1:
				case 3:
					target duplicate_render::change_dr_flags( undefined, "replay_locked" );
					break;

				case 2:
				case 4:
					target duplicate_render::change_dr_flags( "replay_locked", undefined );
