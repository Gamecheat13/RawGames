...
#precache( "objective", "cp_quadtank_rocket_icon" );
...
function state_combat_update( params )
{
	...
	self.hijackingPlayer = undefined;
	
	if ( isalive( self ))
	{
		self thread quadtank_target_hijackers();
		self thread qudtank_clear_target_when_hijack_stops();
	}
...
function quadtank_death( params )
{
	...
	if ( !isdefined( self.custom_death_sequence ) )
	{
		...
		// After about 3.3 is when we want to do the model swap during the death anim
		self vehicle_ai::waittill_asm_complete( "death@stationary", 3.3 );	
	}
	else
	{
...
{
	self.trophy_down = !isOn;
	self clientfield::set( "quadtank_trophy_state", isOn );
	self vehicle::toggle_lights_group( 1, isOn );
	self vehicle::toggle_ambient_anim_group( 1, isOn );
}
...
function quadtank_disabletrophy()
{
	...
	set_trophy_state( true ); // set it to true so the clientfields gets the correct state
	self.trophy_down = true; // manually overwrite here so play don't keep damaging it
	...
	util::wait_network_frame(); // Let the pain start before we mark trophy_disabled so it knows it should play a trophy down anim
	...
	if( trophy_destroyed() ) // just disabled temporarily
	{
		self HidePart( "tag_lidar_null", "", true );
	...
	{	
		startBlink = GetTime();
		while ( vehicle_ai::TimeSince( startBlink ) < 1.5 )
		{
...
//sets favorite enemy to the hijacker when someone starts to hijack the quadtank
function quadtank_target_hijackers()
{
	self endon( "death" );
	self endon( "change_state" );
	...
	While( 1 )
	{
		quadtank waittill( "ccom_lock_being_targeted", hijackingPlayer );
		
		if( isDefined( hijackingPlayer) && hijackingPlayer !== quadtank.hijackingPlayer )
		{
			quadtank.favoriteenemy = hijackingPlayer;
			quadtank.hijackingPlayer = hijackingPlayer;
		}
...
//clears out the favorite enemy when the hijack stops
function qudtank_clear_target_when_hijack_stops()
{
	self endon( "death" );
	self endon( "change_state" );
	...
	While( 1 )
	{
		quadtank util::waittill_any( "ccom_lockOnProgress_Cleared", "ccom_lost_lock" );
		quadtank.favoriteenemy = undefined;
		wait 0.05;
	}
...
function Attack_Thread_rocket()
{
	...
	while( true )
	{
		...
		if( isdefined( self.enemy) && vehicle_ai::IsCooldownReady( "javelin_rocket_launcher", 0.5 )  )
		{
			...
			{
				useJavelin = !self vehseenrecently( self.enemy, 3 ) || ( RandomInt( 100 ) < 2 );
			}
		}
...
function do_melee( shouldDoDamage, enemy )
{
	...
	if ( isdefined( enemy ) && isPlayer( enemy ) )
	{
		...
		globallogic_ui::triggerWeakpointRepulsed( self getEntityNumber(), &"tag_target_lower" );
...
// 			#/    
			...
			if( !trophy_disabled() )
			{
				...
				globallogic_ui::triggerWeakpointDamage( self getEntityNumber(), &"tag_target_lower" );
	...
	if ( sMeansOfDeath != "MOD_UNKNOWN" && !enemyIsQuadtank( eAttacker ) )
	{	
	...
	if( damageLevelChanged && sMeansOfDeath != "MOD_MELEE_ASSASSINATE" && ( !isdefined(eAttacker) || eAttacker.team !== self.team ) && !isdefined( driver ) ) // no pain for player driving version
	{
		self notify( "pain" );
	}
...
function quadtank_display_weakpoint_marker()
{
	if( self.displayweakpoint && self.combatactive && self.weakpointobjective !== 1 )
	{
		...
		globallogic_ui::createWeakpointWidget( self getEntityNumber(), &"tag_target_lower" );
	}
}
...
function quadtank_remove_weakpoint_marker()
{
	...
	{
		self.weakpointobjective = 0;
		globallogic_ui::destroyWeakpointWidget( self getEntityNumber(), &"tag_target_lower" );
	}
}
