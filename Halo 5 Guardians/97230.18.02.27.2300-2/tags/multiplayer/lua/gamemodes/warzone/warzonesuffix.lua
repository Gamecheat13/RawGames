...
function HasValidResponsesDefined(responseTable)
	for _, _ in pairs(responseTable) do
		return true;
	end
	return false;
end
function InsertResponseChain(definitionName, responseName, firstResponse, responseDefinition)
	...
	if HasValidResponsesDefined(responseRoot) then
		ResponseChain(
			responseRoot,
			firstResponse,
	...
	if defaultExtraResponse ~= nil then
		if next(responsesTable) == nil then
			if HasValidResponsesDefined(responseRoot) then
				...
				table.insert(responsesTable, firstResponse:Response(defaultExtraResponse));
			end
		else
