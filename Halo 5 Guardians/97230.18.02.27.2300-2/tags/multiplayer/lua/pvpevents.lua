...
onReachGrenadeLauncherKill = killTypeSelect:Add(
	function(context)
		return context.DamageSource == ReachGrenadeLauncherDamageSource;
	end
	);
onSentinelBeamKill = killTypeSelect:Add(
	function(context)
		return context.DamageSource == SentinelBeamDamageSource;
	end
	);
...
onHunterArmKill = killTypeSelect:Add(
	function (context)
		return context.DamageSource == HunterArmDamageSource;
	end
	);	
...
function FiveShotMagnumPerfect(context)
	local shotCount = 5;
	local shotTime = 3.2;
...
function SixShotMagnumPerfect(context)
	local shotCount = 6;
	local shotTime = 2.71;
	if (useBreakoutTuningForMagnumPerfect) then
		shotCount = 4;
	...
	return WeaponIsGunfighterMagnum(context.DamageSource, context.WeaponAttachments)
		and context.DeadObject ~= nil 
		and context.KillingObject ~= nil 
		and PerfectKill(context, MagnumDamageSource, 0, shotCount, shotCount, shotCount, shotTime);
end
function MagnumPerfectKill(context)
	return FiveShotMagnumPerfect(context) or SixShotMagnumPerfect(context);
end
...
function FiveShotFlagnumPerfect(context)
	local shotCount = 5;
	local shotTime = 3.2;
	if (useBreakoutTuningForMagnumPerfect) then
		shotCount = 3;
...
function SixShotFlagnumPerfect(context)
	local shotCount = 6;
	local shotTime = 2.71;
	if (useBreakoutTuningForMagnumPerfect) then
		shotCount = 4;
	...
	return WeaponIsGunfighterFlagnum(context.DamageSource, context.WeaponAttachments)
		and context.DeadObject ~= nil
		and context.KillingObject ~= nil
		and PerfectKill(context, FlagnumDamageSource, 0, shotCount, shotCount, shotCount, shotTime);
end
function FlagnumPerfectKill(context)
	return FiveShotFlagnumPerfect(context) or SixShotFlagnumPerfect(context);
end
...
if (enableOddballKillTypeMedals) then
	ballKillsInARowAccumulator = root:CreatePlayerAccumulator(GetMultiKillSeconds());
	onBallKillsInARow = onBallKill:PlayerAccumulator(
		ballKillsInARowAccumulator,
		function (context)
			return context.KillingPlayer;
		end
		);
	...
	onRoundStart:ResetAccumulator(ballKillsInARowAccumulator);
	onThreeBallKillsInARow = onBallKillsInARow:Filter(
		function (context)
			return ballKillsInARowAccumulator:GetValue(context.KillingPlayer) == 3
		end
		);
end -- end if enableOddballKillTypeMedals
