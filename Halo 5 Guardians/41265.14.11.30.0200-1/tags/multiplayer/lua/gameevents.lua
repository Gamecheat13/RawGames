...
function BaseBucketDefinition:Activate(context)
	...
	Log("Activating bucket definition");
	
	--
	-- Get all objects to sort in to buckets.
	--
	...
	--
	-- For each object, find the first bucket that it fits in to.
	--
	
	for _, value in pairs(targets) do
	
		Log("Testing value " .. tostring(value));
		...
		for _, bucketDefinition in ipairs(self.Buckets) do
			...
			if predicate == nil or predicate(context, value) then
				...
				Log("Passed predicate");
			
				--
				-- Add the object to the bucket output.
				--
...
function ActivationContext:GetSortedTeamRoundsWon()

	if self.__SortedTeamRoundsWon == nil then

		self.__SortedTeamRoundsWon = { };

		self.Engine:VisitSortedTeamsRoundsWon(
			function (roundsWon)
				table.insert(self.__SortedTeamRoundsWon, roundsWon);
			end
			);

	end

	return self.__SortedTeamRoundsWon;	
...
function RootDefinition:Activate(context)
	...
	-- Intentionally empty, root definition can not be activated.
...
function GetFirstDefinitionOnStack()
	...
	if debug ~= nil then
		...
		--
		-- Search each stack frame for a "self" value.
		--
  		...
  		while not foundDef do
			...
			--
			-- Check if each local value is "self" and if it is dervied from HostBaseDefinition.
			--
  			...
  			while not foundDef do
    			...
    			--
    			-- If out of arguments, move on to the next stack frame.
    			--
...
function RootDefinition:Start(context)
	...
	--
	-- Connect to all callbacks if running authoritatively.
	--
...
function RootDefinition:CreateAccumulator(timeout)
	...
	local accumulator = AccumulatorWrapper:new(timeout);
...
function RootDefinition:CreatePlayerAccumulator(timeout)
	...
	local accumulator = PlayerAccumulatorWrapper:new(timeout);
...
function ResponseDefinition.ValidateMusic(val)
	return type(val) == "string";
end
...
ResponseDefinition.ValidProperties =
{
	...
	Music = ResponseDefinition.ValidateMusic,
...
function ResponseDefinition:__init(response)
	...
	--
	-- Validate each event property when being defined.
	--
...
function ResponseDefinition:ApplyResponse(target, ...)
...
function ResponseDefinition:ApplyResponseWithText(target, ...)
	...
	if target ~= nil then
		...
		if type(target) == "table" then
			...
			for _, value in pairs(target) do
				...
				self:ApplyResponseWithText(value, ...);

			end

		else

			target:AddEventWithText(...);
...
function ResponseDefinition:Activate(context)
	...
	if self.EventHandle ~= nil then
		...
		local responseProps = self.ResponseData;		

		local score = 0;
		local fanfareText = nil;

		if dynamicProps.Score ~= nil then
			score = dynamicProps.Score(context);
		elseif responseProps.Score ~= nil then
			score = responseProps.Score;
		end

		if dynamicProps.FanfareText ~= nil then
			fanfareText = dynamicProps.FanfareText(context);
		elseif responseProps.FanfareText ~= nil then
			fanfareText = nil;
		end
		...
		if fanfareText ~= nil then
			self:ApplyResponseWithText(context._Targets, self.EventHandle, score, fanfareText);
		else
			self:ApplyResponse(context._Targets, self.EventHandle, score);
		end
...
function EmitDefinition:Activate(context, ...)
	...
	local childContext = CopyTable(context);
...
--
-- NOTE: This definition triggers execution to be passed from the host to the client.
--
...
function ObjectEventDefinition:Activate(context, ...)
	...
	--
	-- Convert from a player to a unit if possible.
	--
...
function SerializationContext:ExtractProperty(property, default)

	if property ~= nil and type(property) ~= "function" then
		return property;
	else
		return default;
	end
...
function SerializationContext:WriteResponse(response)
	...
	local score = self:ExtractProperty(response.Score, 0);
	local fanfare = self:ExtractProperty(response.Fanfare, nil);
	local fanfareText = self:ExtractProperty(response.FanfareText, nil);
	local sound = self:ExtractProperty(response.Sound, nil);
	local music = self:ExtractProperty(response.Music, nil);
	local medal = self:ExtractProperty(response.Medal, nil);

	local newIndex = eventStorage:AddEvent(score, fanfare, fanfareText, sound, music, medal);
