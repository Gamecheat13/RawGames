...
SubState_StandingCorner(changeReason, cornerAngle, nodeOrigin)
{
	...
	if ( (self.anim_pose == "crouch") || (self.anim_pose == "prone") )	// TODO: Prone should have its own animation.
	{
		...
		{
			animscripts\SetPoseMovement::PlayTransitionAnimation(%cornercrouch2stand_left, "stand", "stop", 0, 1);
		}
		else
		{
			animscripts\SetPoseMovement::PlayTransitionAnimation(%cornerb_crouch2stand_left, "stand", "stop", 0, 1);
		}
	}
...
Anims_CrouchingRifleA()
{
	...
	animarray["offset_grenade"]				= (-25,41,36);
	animarray["anim_grenade"]				= %corner_stand_grenade_throw_left;	// TODO - this is the wrong animation!
	animarray["gunhand_grenade"]			= "left";
	return animarray;
}
Anims_CrouchingRifleB()
{
	...
	animarray["offset_grenade"]				= (25,-41,36);
	animarray["anim_grenade"]				= %cornerb_stand_grenade_throw_left;	// TODO - this is the wrong animation!
	animarray["gunhand_grenade"]			= "left";
	return animarray;
}
...
SubState_CrouchingCorner(changeReason, cornerAngle, nodeOrigin)
{
	...
	else if ( (self.anim_pose == "stand") || (self.anim_pose == "prone") )	// TODO: Prone should have its own animation.
	{
		...
		if (self.anim_idleset == "a")
			...
			animscripts\SetPoseMovement::PlayTransitionAnimation(%cornerb_stand2crouch_left, "crouch", "stop", 0, 1);
	}
...
GoToCover(cornerAngle)
{
	...
	if (self.anim_special == "cover_right")
	{
		...
		{
			println ("cover_right::GoToCover : Invalid self.anim_idleSet "+self.anim_idleSet+" for cover_right character ("+self.anim_pose+", "+self.anim_movement+")");
			self.anim_idleSet = "a";
			if (self.anim_pose == "stand")
			{
				animarray = Anims_StandingRifleA();
	...
	else
	{
		...
		else if ( angleDifference >= 180 )
		{
			...
			if (self.anim_pose == "stand")
			{
				[[anim.println]]("cover_right stand - picked set a, playing run2corner anim");
				animarray = Anims_StandingRifleA();
			}
			else
			{
				self ExitProne(0.5); // In case we were in Prone.
				self.anim_pose = "crouch";
				[[anim.println]]("cover_right crouch - picked set a, playing run2corner anim");
				animarray = Anims_CrouchingRifleA();
			}
		}
		else
		{
			...
			if (self.anim_pose == "stand")
			{
				[[anim.println]]("cover_right stand - picked set b, playing run2corner anim");
				animarray = Anims_StandingRifleB();
			}
			else
			{
				self ExitProne(0.5); // In case we were in Prone.
				self.anim_pose = "crouch";
				[[anim.println]]("cover_right crouch - picked set b, playing run2corner anim");
				animarray = Anims_CrouchingRifleB();
			}
		}
