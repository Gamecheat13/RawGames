...
heli_update_global_dvars()
{
	for( ;; )
	{
		...
		level.heli_visual_range = heli_get_dvar_int( "scr_heli_visual_range", "4000" );		// distance radius helicopter will acquire targets (see)
...
heli_think( owner, startnode, heli_team, missilesEnabled, protectLocation, hardpointType, armored, killstreak_id )
{
	...
	chopper thread maps\mp\_heatseekingmissile::MissileTarget_ProximityDetonateIncomingMissile("crashing", "leaving");			// fires chaff if needed
...
heli_kill_monitor( hardpointtype )
{
	...
	/#	PrintLn( "got killed notify");	#/
		...
		if (self.bda == 0)
		{
			bdaDialog = "klsheli_killn";
		}
		if (self.bda == 1)
		{
			bdaDialog = "klsheli_kill1";
		}
		if (self.bda == 2)
		{
			bdaDialog = "klsheli_kill2";
		}
		if (self.bda == 3)
		{
			bdaDialog = "klsheli_kill3";
		}
		else if (self.bda > 3)
		{
			bdaDialog = "klsheli_killm";
		}
...
heli_damage_monitor( hardpointtype )
{
	...
	for( ;; )
	{
		...
		{		
			...
			self trackAssists( attacker, damage, false );
		}
			...
			&& ( !isDefined( self.owner ) || attacker != self.owner ) )
		{
			...
			if ( isdefined( event ) ) 
			{
				maps\mp\_challenges::destroyedHelicopter( attacker, weapon );
			...
			{
				level.players[i] LUINotifyEvent( &"player_callout", 2, &"KILLSTREAK_DESTROYED_HELICOPTER", attacker.entnum );
			}
...
heli_leave( hardpointType )
{
	...
	wait ( 1.5 );
	self setspeed( 150, 55 );		
	self SetVehGoalPos( self.origin + ( leavenode.origin - self.origin ) / 2 + ( 0, 0, HELI_ENTRANCE_Z_HELP ) );
...
heli_protect( startNode, protectDest, hardpointType, heli_team )
{
	...
	self SetVehGoalPos( self.origin + ( currentDest - self.origin ) / 2 + ( 0, 0, HELI_ENTRANCE_Z_HELP ) );
	self waittill( "near_goal" );
...
PlayPilotDialog( dialog, time, voice, shouldWait )
{
	...
	if ( isdefined ( self.owner ) )
	{
		while( isdefined(self.owner.pilotTalking) && self.owner.pilotTalking )
		{
			if ( isDefined( shouldWait ) && !shouldWait )
				return;
			wait 1;	
		}
		self.owner.pilotTalking = true;
		self.owner playLocalSound(soundAlias);
		wait 3;
		self.owner.pilotTalking = false;
	}
}
