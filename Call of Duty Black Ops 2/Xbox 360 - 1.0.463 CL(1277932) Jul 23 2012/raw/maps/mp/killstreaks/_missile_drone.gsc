...
#insert raw\maps\mp\killstreaks\_killstreaks.gsh;

init()
{	
	RegisterClientField( "toplayer", "missile_drone_active", 2, "int" );
	RegisterClientField( "missile", "missile_drone_projectile_active", 1, "int" );
	...
	//PrecacheItem( "inventory_missile_drone_mp" );
	
	registerKillstreak( "inventory_missile_drone_mp", "inventory_missile_drone_mp", "killstreak_missile_drone", "missile_drone_used", ::missile_drone_killstreak, true );
	...
	registerKillstreakDialog( "inventory_missile_drone_mp", "mpl_killstreak_missile_drone", "kls_drone_used", "", "kls_drone_enemy", "", "kls_drone_ready" );
	registerKillstreakDevDvar( "inventory_missile_drone_mp", "scr_givemissiledrone" );

	registerKillstreak( "missile_drone_mp", "missile_drone_mp", "killstreak_missile_drone", "missile_drone_used", ::missile_drone_killstreak, true );
	...
	registerKillstreakDialog( "missile_drone_mp", "mpl_killstreak_missile_drone", "kls_drone_used", "", "kls_drone_enemy", "", "kls_drone_ready" );
}
...
missileDroneWatcher( hardpointType )
{
	...
	killstreak_id = maps\mp\killstreaks\_killstreakrules::killstreakStart( hardpointType, team, false, false );
	if ( killstreak_id == -1 )
	{
	...
	self SetClientFieldToPlayer( "missile_drone_active", MISSILE_DRONE_TARGET_DISABLED );
	weapon thread waitThenDelete( 0.05 );

	self thread doMissileDrone( weapon, weapname, killstreak_id, hardpointType, team  );
	self maps\mp\killstreaks\_killstreaks::switchToLastNonKillstreakWeapon();
}


doMissileDrone( weapon, weapname, killstreak_id, hardpointType, team  )
{
	direction = self getPlayerAngles();
	forward = AnglesToForward( direction );
	target = weapon.origin + VectorScale( forward, 10000 );

	debug_line( weapon.origin, target, (0.9,0.1,0.1) );
		
	projectile = maps\mp\killstreaks\_missile_swarm::projectile_spawn_utility( self, target, weapon.origin, "missile_drone_projectile_mp", "drone_missile", false );
	...
	projectile killstreak_stop_think( killstreak_id, hardpointType, team  );
}
...
#/
			...
			currentPlayerRating += ORIGIN_SIGHT_CHECK_RATING;
		}
...
#/
			...
			currentPlayerRating += HEAD_SIGHT_CHECK_RATING;
		}
...
#/
			...
			currentPlayerRating += NOT_IN_BUILDING_RATING;
		}
		...
		if ( currentPlayerRating > bestPlayerRating )
		{
			...
			if ( bestPlayerRating >= MAX_TARGET_RATING )
				return target;
		}
	}

	if ( bestPlayerRating >= MIN_TARGET_RATING ) 
	{
		return target;	
	}
	return undefined;
...
updateTargetting()
{
	...
	for( ;; )
	{
		...
		if ( IsDefined( self.droneTarget ) )
		{
			...
			self SetClientFieldToPlayer("missile_drone_active", MISSILE_DRONE_TARGET_ACTIVE );
		}
		else
		{
			self SetClientFieldToPlayer( "missile_drone_active", MISSILE_DRONE_TARGET_ENABLED );
		}
