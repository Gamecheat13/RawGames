...
/* level.createFXent[] properties
ent.drawn					- (int) 0 if its HUDelem is not drawn, 1 if it is
ent.drawn_axis_model - (int) 1 if axis model is drawn on this ent, 0 if it isn't.
ent.last_fx_index	- (int) This doesn't exist until the ent has been selected.  When it is, this value becomes
										the ent's index within level.createFXent[].
ent.looper				- this is the visible effect returned by calling playLoopedFX() and triggerFX().
ent.textalpha			- (int) Not sure what this is.
ent.uniqueid			- (int) A unique number for each effect.
ent.v							- All of these ent.v properties are saved out to the map's createfx ambient fx script.
ent.v["angles"]		- (vector) The ent's angles.
ent.v["delay"]		- (float) The number of seconds before the effect plays.
ent.v["exploder"]	- (int) Maybe this is whether it's an exploder or not.
ent.v["exploder_id"]	- (int) The value used to trigger this exploder.
ent.v["exploder_type"]	- (string) Usually set to "normal".  Not sure what this does.
*/

// This function is not run when a level is run normally.
createfx()
{
	/#
	println("^2Running CreateFX 2.0");
	#/
	// Abstracts much of the differences between MP and SP so they can use the same script file.
	...
	if ( isMP() )
	{
		...
		//SetGametypeSetting( "timelimit", 0 );	// Keeps timer from counting down
	}
	else
	{
		init_sp_paths();
	...
	{
		level.cfx_uniqueid = 0;	// unique ids are assigned to each ent in CreateFX mode for undo
	}
	level.cfx_last_action = "none";	// if not "none", lets undo system know there are state changes
	
	if ( !isMP() )
	{
		level thread [[level.cfx_func_run_gump_func]]();	// run gump-related functions defined per level
	}
	...
	{
		//level.callbackStartGameType = ::empty;
	...
	// level thread camera_hud_toggle( "Normal Camera" );	// FIXME: DLM - This might be causing a variable overflow via dev UI text.
...
// This function will always run regardless of mode.
fx_init()
{
...
createEffect( type, fxid )	// Called by _utility.gsc
{
	...
	if(level.createFX_enabled)
	{
		...
		ent = spawnStruct();
		//ent.last_fx_index = level.createFXent.size;
...
// DLM - 2/24/2011 - Got rid of the really small numbers anglestoup() and anglestoforward()
//	would return when it should be returning 0.
set_forward_and_up_vectors()
{
	temp_up = anglestoup(self.v["angles"]);
	temp_forward = anglestoforward(self.v["angles"]);
	const arbitrary_tiny_value = 0.05;
	
	if (abs(temp_up[0]) < arbitrary_tiny_value)
	{
		temp_up = (0,temp_up[1],temp_up[2]);
	}
	if (abs(temp_up[1]) < arbitrary_tiny_value)
	{
		temp_up = (temp_up[0],0,temp_up[2]);
	}
	if (abs(temp_up[2]) < arbitrary_tiny_value)
	{
		temp_up = (temp_up[0],temp_up[1],0);
	}
	if (abs(temp_forward[0]) < arbitrary_tiny_value)
	{
		temp_forward = (0,temp_forward[1],temp_forward[2]);
	}
	if (abs(temp_forward[1]) < arbitrary_tiny_value)
	{
		temp_forward = (temp_forward[0],0,temp_forward[2]);
	}
	if (abs(temp_forward[2]) < arbitrary_tiny_value)
	{
...
createFxLogic()
{
	...
	level.createFx_hudElements = 30;	// TODO: Can this number be higher?
...
//		}
		...
		if ( button_is_clicked("z") )
		{
			if (shiftHeld)
			{
				//redo();	// TODO: DLM 12/01/2012 - Re-enable this when issue with missing/skipping states while undoing and redoing is fixed.
			}
			else
			{
				undo();
		...
		// Help menu
		// TODO: Replace this with a paged view of the help menu.
		...
		if ( menu("none") )
		{
			...
			{
				move_player_to_next_same_effect(true, lastSelectEntity);
			}
			else if (button_is_clicked( "leftarrow" ) )
			{
				move_player_to_next_same_effect(false, lastSelectEntity);
			}
		}
		...
		if (axisMode && level.selected_fx_ents.size > 0)
		{
			...
			// TODO: Get copy_angles_of_selected_ents() working again.
...
//				ent.v["angles"] = ent.v["angles"] + (level.selectedRotate_pitch, level.selectedRotate_yaw, 0);
//				ent set_forward_and_up_vectors();
//			}
//			
...
//	TODO: DLM - 9/23/2011 - This doesn't do anything of value.
//		if (distanceSquared(lastPlayerOrigin, player.origin) > 4096)	// actual distance: 64 units
//		{
...
//get_selected_fx_indices()
//{
//	temp_array = [];
//	
//	if ( !isdefined(level.selected_fx) || !isdefined(level.selected_fx_ents) )
//	{
//		return temp_array;
//	}
...
//		return temp_array;
//	}
//	for (i=0; i < level.selected_fx.size; i++)
//	{
//		if ( index_is_selected(i) )
//		{
//			temp_array[temp_array.size] = i;
//		}
//	}
//	return temp_array;
//}
...
// Changes the color of selected entities back to its non-selection color and clears selected ents.
...
{
	if ( !IsDefined( level.createfx_delay_done ) )
	{
		return;
	}
...
get_selected_move_vector()
{
	...
	if (buttonDown("kp_uparrow", "DPAD_UP"))
	{
		...
		if ( ctrlHeld )
		{
			level.selectedMove_forward = small_rate;
			wait 0.05;
		}
		else
		{
	...
	else if (buttonDown("kp_downarrow", "DPAD_DOWN"))
	{
		...
		if ( ctrlHeld )
		{
			level.selectedMove_forward = -1 * small_rate;
			wait 0.05;
		}
		else
		{
	...
	if (buttonDown("kp_rightarrow", "DPAD_RIGHT"))
	{
		...
		if ( ctrlHeld )
		{
			level.selectedMove_right = small_rate;
			wait 0.05;
		}
		else
		{
	...
	else if (buttonDown("kp_leftarrow", "DPAD_LEFT"))
	{
		...
		if ( ctrlHeld )
		{
			level.selectedMove_right = -1 * small_rate;
			wait 0.05;
		}
		else
		{
	...
	if (buttonDown("BUTTON_Y"))
	{
		...
		if ( ctrlHeld )
		{
			level.selectedMove_up = small_rate;
			wait 0.05;
		}
		else
		{
	...
	else if (buttonDown("BUTTON_B"))
	{
		...
		if ( ctrlHeld )
		{
			level.selectedMove_up = -1 * small_rate;
			wait 0.05;
		}
		else
		{
...
// DLM - 8/23/2011 - Changed output function to be more concise and quicker.
// type should be "server" for the GSC, "client" for the CSC, and "auto" for autosave.
...
/#	
	...
	else
	{
		...
		// Get rid of really small numbers in origins and angles
		const limit = 0.1;
		for ( p = 0; p < level.createFXent.size; p++ )
		{
			ent = level.createFXent[p];
		...
		{
			breather_pause = 5; // wait every 5 iterations for some loop protection
		}
		...
		for ( i = 0; i < level.createFXent.size; i++ )
		{
			...
			if (isdefined(e.model))
			{
				if ( isDefined(ent.v["exploder_type"]) && ent.v["exploder_type"] == "normal" )
				{
					println("^1Exploder ent " + i + " is probably a script_brushmodel that is trying to save out. Make sure it didn't.");
					println("^1Offending exploder location: " + e.v["origin"][0] + " " + e.v["origin"][1] + " " + e.v["origin"][2] );
				}
				continue;  // entities with models are from radiant and don't get reported
			}
			
			if ( e.v["fxid"] == "No FX" )	// catch certain script_exploders where the model has been deleted
			{
				continue;
			}
			...
			output_props += "ent.v[ \"origin\" ] = ( " + e.v["origin"][0] + ", " + e.v["origin"][1] + ", " + e.v["origin"][2] + " );";
			output_props += " ent.v[ \"angles\" ] = ( " + e.v["angles"][0] + ", " + e.v["angles"][1] + ", " + e.v["angles"][2] + " );";
...
// Return a string containing all of the ent's properties in a printable format
// Omits the "fxid" since that doesn't need to be output.
get_fx_options( ent )
{
	output_props = "";
	...
	for ( i=0; i<level.createFX_options.size; i++ )
	{
		...
		if ( option["type"] == "string" )
		{
			if ( option["name"] == "fxid" )	// Skip "fxid" because it's redundant to print them out
			{
				continue;
			}
...
rotate_over_time ( org, rotater )
{
	...
	const timer = 0.1;
	const max_timer = 2; // this should be timer * 20
	
	for (p=0; p<max_timer; p++)
	{
		if (level.selectedRotate_pitch != 0)
		{
...
show_help()
{
	...
	set_fx_hudElement("D                Drop selected effects to the ground");
...
paste_ents()
{
	...
	{
		// add unique ids here instead of in copy() so they're handed out sequentially in the order
		// they appear in level.createFXent[].
...
stop_fx_looper()
{
	...
	self [[level.cfx_func_stop_loopsound]]();	// path for SP: self maps\_fx::stop_loopsound();
}
...
restart_fx_looper()
{
	...
	if (self.v["type"] == "loopfx")
	{
		...
		self [[level.cfx_func_create_looper]]();	// path for SP: self maps\_fx::create_looper();
	}

	if (self.v["type"] == "oneshotfx")
	{
		// new entities from copy/paste wont have a looper
		self [[level.cfx_func_create_triggerfx]]();	// path for SP: self maps\_fx::create_triggerfx();
	}

	if (self.v["type"] == "soundfx")
	{
		// new entities from copy/paste wont have a looper
		self [[level.cfx_func_create_loopsound]]();	// path for SP: self maps\_fx::create_loopsound();
	}
}
...
draw_distance()
{
	...
	for (;;)
	{
		...
		/#
		// DLM - 09/20/2011 - Wait during saves to (hopefully) keep CreateFX from crapping out.
		...
		while ( distanceSquared(player.origin, last_pos) < 2500 )	// or less than 50 units
		{
			wait (0.1);
		}
...
/#	println( "^3#### CREATEFX SERVER " + savemode + " END (Time: " + ( GetTime() - old_time ) * 0.001 + " seconds)####" );	#/
	...
	/#	println( "^3#### CREATEFX " + savemode + " CANCELLED ####" );	#/
		createfx_emergency_backup();	// "createfx_saving" flag is cleared in this function.
	} else
	{
...
// forward_search is a boolean. If true, player will be moved to the next effect with the same ID. 
//   Otherwise, they'll be moved the previous effect with the same ID.
move_player_to_next_same_effect( forward_search, lastSelectEntity )
{
	...
	if ( level.selected_fx_ents.size <= 0 )
	{
		...
		if ( isdefined( ent ) )	// select and move to prefetched effect
		{
...
//		}
	...
	else	// go to next selected effect
	{
		...
		// Find index of selected effect that player is looking at
		for (i=0; i < level.selected_fx_ents.size; i++)
		{
...
// Returns the ent if it finds a matching id.  Returns the same ent if there are no other matches.
...
// Get ent info of previous effect with the same name.
// index is the effect index in level.createFXent to match. ent_id is its fxid.
...
{
	// begin searching for next same ent until beginning of array
	for (i = index-1; i > 0; i--)
	{
		if ( ent_id == level.createFXent[i].v["fxid"] )
		{
	...
	// finish looping through array from end
...
//	level.cfx_client_exploder = 			"clientscripts\\_fx::createExploder";	// Doesn't exist! Where'd it go?
...
{
	wait( 10 );
	level.createfx_delay_done = true;
}
