...
//t6todo	level._be StopAnimScripted();
	level._be Unlink();
	
//t6todo	level._be DontInterpolate();
	...
	{
//t6todo		level._be_origin_animate StopAnimScripted();
		level._be_origin_animate Delete();
	}

//t6todo	maps\mp\zombies\_zm_weap_quantum_bomb::quantum_bomb_register_result( "be2", undefined, 100, ::be2_validation );
	level._be_pos = level._be.origin;
	level waittill("be2_validation");
	
//t6todo	maps\mp\zombies\_zm_weap_quantum_bomb::quantum_bomb_deregister_result("be2");
...
bhb_teleport_loc_check( grenade, model, info )
{
	...
	//t6todo	grenade thread maps\mp\zombies\_zm_weap_black_hole_bomb::do_black_hole_bomb_sound( level._be, info ); // WW: This might not work if it is based on the model
...
//t6todo	level._be_vehicle maps\_vehicle::getonpath( road_start );
	...
	// start the animation
//t6todo	origin_animate thread maps\_anim::anim_loop_aligned( level._be_vehicle._be_model, "to_the_right", "tag_origin_animate_jnt");
	
	level._be_vehicle thread moon_be_think();
	
	// level._be StartPath();
//t6todo	level._be_vehicle thread maps\_vehicle::gopath();
	...
	while( IsDefined( self ) )
	{
		...
		if( IsDefined( node.script_string ) )
		{
			...
			while( !motivation )
			{
				...
				{
//t6todo					maps\mp\zombies\_zm_weap_microwavegun::add_microwaveable_object(d_trig);
					d_trig waittill("microwaved",vox_dude);
//t6todo					maps\mp\zombies\_zm_weap_microwavegun::remove_microwaveable_object(d_trig);
					motivation = true;
...
//t6todo			self maps\_vehicle::vehicle_pathdetach();
			...
			// move be to new spot
//t6todo			self maps\_vehicle::getonpath( next_chain_start );
	...
	//t6todo					maps\mp\zombies\_zm_weap_microwavegun::remove_microwaveable_object(d_trig);
			...
			self SetSpeed( level._my_speed );
//t6todo			self thread maps\_vehicle::gopath();
			self thread moon_be_resume_anim();
...
//t6todo	self._be_model StopAnimScripted();
	
	if( int_anim == 0 )
	{
//t6todo		self._be_org_anim thread maps\_anim::anim_loop_aligned( self._be_model, "to_the_left", "tag_origin_animate_jnt" );
	}
	else
	{
//t6todo		self._be_org_anim thread maps\_anim::anim_loop_aligned( self._be_model, "to_the_right", "tag_origin_animate_jnt" );
	}
