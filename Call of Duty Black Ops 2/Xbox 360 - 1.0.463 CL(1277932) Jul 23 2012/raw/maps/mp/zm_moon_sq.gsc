...
watch_for_respawn()
{
	...
	//t6todo	self SetMaxHealth( level.zombie_vars["zombie_perk_juggernaut_health"] );
...
//t6todo	while(level._num_overriden_models < (GetNumExpectedPlayers()))
	{
		wait(0.1);
	}	
...
init_sidequest()
{
	...
	{
		entnum = players[i].characterindex;// GetEntityNumber();
		...
		{	
			...
			else
			{
				if(level.onlineGame)
				{
				 	...
				 	if(maps\mp\zombies\_zm::is_sidequest_previously_completed("COTD") && maps\mp\zombies\_zm::is_sidequest_previously_completed("EOA"))
					{
						players[i] add_sidequest_icon("sq", "generator");
						level._all_previous_done = true;
						break;
					}
					else if(maps\mp\zombies\_zm::is_sidequest_previously_completed("COTD"))
					{
						players[i] add_sidequest_icon("sq", "vril");
						break;
					}
					else if(maps\mp\zombies\_zm::is_sidequest_previously_completed("EOA"))
					{
...
//t6todo		vehicle maps\_vehicle::getonpath( start );

//t6todo		vehicle thread maps\_vehicle::gopath();
...
play_end_lines_in_order()
{
	...
	players = GET_PLAYERS();
	players[randomintrange(0,players.size)] thread maps\mp\zombies\_zm_audio::create_and_play_dialog( "eggs", "quest8", undefined, 7, true );
	...
	if( isdefined( player ) )
	{
		player thread maps\mp\zombies\_zm_audio::create_and_play_dialog( "eggs", "quest8", undefined, 9, true );
		wait(5);
	}
	...
	if( isdefined( player ) )
	{
		player thread maps\mp\zombies\_zm_audio::create_and_play_dialog( "eggs", "quest8", undefined, 10, true );
	}
...
get_specific_player( num )
{
	...
	for(i = 0; i < players.size; i ++)
	{
		ent_num = players[i].characterindex;// GetEntityNumber();
...
//t6todo			SetSavedDvar("r_zombieDisableEarthEffect", 1);
			//stop_exploder( 2012 );
	...
	//t6todo		SetSavedDvar("r_zombieDisableEarthEffect", 0);
