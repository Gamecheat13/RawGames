...
init()
{
	...
	maps\mp\gametypes\_globallogic_audio::registerDialogGroup( "item_destroyed", true );
}
	...
	{		
		while( true )
		{
			...
			if ( self isOnGround( true ) && TestSpawnPoint( latestOrigin ) && 
			    ( !IsDefined( self.preventTacticalInsertion ) || !self.preventTacticalInsertion ) )
			{
	...
	if ( isdefined( attacker ) && isdefined( attacker.pers["team"] ) && isdefined( self.owner ) && isdefined( self.owner.pers["team"] ) )
	{
		if ( level.teambased )
		{
			if ( attacker.pers["team"] != self.owner.pers["team"] )
			{
				...
				maps\mp\_scoreevents::processScoreEvent( "destroyed_tac_insert", attacker, undefined, undefined, true );
			}
		}
		else
		{
...
spawnTacticalInsertion() // self == player
{
	...
	self.tacticalInsertion SetClientFlag( CLIENT_FLAG_TACTICAL_INSERTION );
