...
init()
{
		...
		game["dialog"]["sentry_hacked"] = "sentry_hacked";
		game["dialog"]["microwave_destroyed"] = "dest_microwave";
		game["dialog"]["microwave_hacked"] = "dest_microwave";
...
announceRoundWinner( winner, delay )
{
	...
	{
		//thread playSoundOnPlayers( "mus_round_win"+"_"+level.teamPostfix[winner] );
		//thread playSoundOnPlayers( "mus_round_loss"+"_"+level.teamPostfix[team] );
...
addGroupDialogToPlayer( dialog, group )
{
	...
	{
		if ( self.leaderDialogGroups[group].size == 0)
		{
			addToQueue = true;
		}
	}
	...
	if ( !level.dialogGroup[group].skipIfCurrentlyPlayingGroup )
	{
		...
		// if we are currently playing the group add one to the group queue
		if ( self.currentLeaderDialogGroup == group && self.leaderDialogGroups[group].size )
		{
			if ( !isdefined(self.leaderDialogGroups[group][1]) )
			{
				addToQueue = true;
			}
			self.leaderDialogGroups[group][1] = dialog;			
		}
		else
		{
			self.leaderDialogGroups[group][0] = dialog;			
		}
	}
	else
	{
...
/#
	...
	if ( count != self.leaderDialogGroups[group].size )
	{
		shit = 0;
	}
#/
...
playLeaderDialogOnPlayer( dialog )
{
	...
	{
		group = dialog;
		dialog = self.leaderDialogGroups[group][0];
		for ( i = 1; i < self.leaderDialogGroups[group].size; i++ )
			self.leaderDialogGroups[group][i-1] = self.leaderDialogGroups[group][i];
		self.leaderDialogGroups[group][i-1] = undefined;
