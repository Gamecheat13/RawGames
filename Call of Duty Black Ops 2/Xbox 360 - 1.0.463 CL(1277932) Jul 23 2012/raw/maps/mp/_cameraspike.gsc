...
cameraSpikeDetonate( attacker, emp )
{
	if ( !IsDefined( emp ) )
	{
		emp = false;
...
adjustCameraDirection( point )
{
	...
	self.cameraHead SetClientFlag( CLIENT_FLAG_CAMERA_SPIKE );
	self.cameraHead linkto( self );
}
...
watchCameraSpikeDamage( watcher ) // self == camera spike
{
	...
	while( true )
	{
		...
		if( self.damageTaken >= damageMax )
		{
			...
			watcher thread maps\mp\gametypes\_weaponobjects::waitAndDetonate( self, 0.0, attacker, ( weaponName == "emp_grenade_mp" ) );
		}
	}
}
