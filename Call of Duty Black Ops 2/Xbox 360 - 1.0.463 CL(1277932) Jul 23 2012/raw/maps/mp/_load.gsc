...
{	
	...
	if( !SessionModeIsZombiesGame() )
	{
		...
		//thread maps\mp\killstreaks\_helicopter_gunner::register();		
		maps\mp\_audio::init();
	...
	if( !SessionModeIsZombiesGame() )
	{
		//TFLAME - 10/12/11 - Set development DVARS that are in the tuning stage
...
start_intro_screen_zm( )
{
	...
	if( !IsDefined(level.introscreen) )
	{
		...
		level.introscreen.foreground = true;
