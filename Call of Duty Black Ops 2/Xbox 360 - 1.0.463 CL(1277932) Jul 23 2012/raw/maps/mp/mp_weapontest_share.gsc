...
change_weapon()
{
	...
	while(true)
	{
		...
		if (level.playerone.lastdroppableweapon== "riotshield_mp")
		{
			riot_shield_array = GetEntArray( "weapon_riotshield_mp", "classname");
			dropped_riot_shields = [];
			spawned_riot_shields = [];
			
			for (i=0; i< riot_shield_array.size; i++)
			{
				if (!isdefined(riot_shield_array[i].targetname) || ( isdefined(riot_shield_array[i].targetname) && riot_shield_array[i].targetname != "weapontest_weapon") )
				{
					dropped_riot_shields[dropped_riot_shields.size] = riot_shield_array[i];
				}
				else if ( isdefined(riot_shield_array[i].targetname) && riot_shield_array[i].targetname == "weapontest_weapon" )
				{
					spawned_riot_shields[spawned_riot_shields.size] =  riot_shield_array[i];
				}
			}
			
			for( i = 0; i < spawned_riot_shields.size; i++ )
			{
				if (i>3)
				{
					spawned_riot_shields[i] delete();
				}
			}
			for( i = 0; i < dropped_riot_shields.size; i++ )
			{
				if (i>3)
				{
					dropped_riot_shields[i] delete();
				}
			}
		}
