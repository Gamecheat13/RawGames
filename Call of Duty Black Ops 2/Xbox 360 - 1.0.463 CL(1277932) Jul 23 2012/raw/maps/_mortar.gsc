...
explosion_incoming( mortar_name )
{
	...
	//TUEY - changing this from waiting for a notify to waiting the shortest time for the asset to get rid of a delay issue.
	wait (0.47); 
}
...
//	}
	...
	{
		phys_origin = ( origin[0], origin[1], origin[2] - 0.5*physRadius ); // Fake the origin downward a bit to get objects airborne for dramatic effect
//		PhysicsExplosionSphere( origin, physRadius, physRadius * 0.25, 0.75 );
		PhysicsExplosionSphere( phys_origin, physRadius, physRadius * 0.25, 3.0 ); // 3x multiplier on force for some Michael Bay type action
	}	
