...
{	
	...
	PreCacheItem( "cobra_20mm" );
	precacheitem( "cobra_20mm_comlink_sp" );
...
useKillstreakHelicopter( hardpointType, protectLocation, team )
{
	if (!isDefined(team) )
		team = self.team;

	if ( self maps\sp_killstreaks\_killstreakrules::isKillstreakAllowed( hardpointType, team ) == false)
		return false;
	...
	if ( !isDefined(protectLocation) && hardpointType == "helicopter_comlink_sp" )
	{
		result = self maps\sp_killstreaks\_helicopter::selectHelicopterLocation( hardpointType );
	...
	if ( hardpointType == "helicopter_x2_sp" )
	{
	...
	armored = false;
	if ( hardpointType == "helicopter_comlink_sp" )
	{
		...
		{
			protectLocation = (protectLocation[0],protectLocation[1], startnode.origin[2]);
		}
		armored = true;
		startnode = getValidProtectLocationStart(random_path, protectLocation, destination );
	}
	
	if ( self maps\sp_killstreaks\_killstreakrules::killstreakStart( hardpointType, team ) == false )
		return false;

	self thread announceHelicopterInbound( hardpointType );
	
	
	thread maps\sp_killstreaks\_helicopter::heli_think( self, startnode, team, missilesEnabled, protectLocation, hardpointType, armored );
...
announceHelicopterInbound(hardpointType)
{
	...
	self maps\sp_killstreaks\_killstreaks::playKillstreakStartDialog( hardpointType, team, true);
	...
	{
		case "helicopter_player_sp":
		case "helicopter_player_firstperson_sp":
			statAddType = "HELICOPTER_PLAYER_USED";
			break;
		case "helicopter_gunner_sp":
			statAddType = "HELICOPTER_GUNNER_USED";
			break;
		case "helicopter_sp":
		case "helicopter_x2_sp":
			statAddType = "HELICOPTER_USED";
			break;
		case "helicopter_comlink_sp":
...
preload()
{
	...
	maps\sp_killstreaks\_killstreaks::registerKillstreak("helicopter_comlink_sp", "helicopter_comlink_sp", "killstreak_helicopter_comlink", "helicopter_used", ::useKillstreakHelicopter, true );
	maps\sp_killstreaks\_killstreaks::registerKillstreakStrings("helicopter_comlink_sp", &"KILLSTREAK_EARNED_HELICOPTER_COMLINK", &"KILLSTREAK_HELICOPTER_COMLINK_NOT_AVAILABLE", &"KILLSTREAK_HELICOPTER_COMLINK_INBOUND");
	maps\sp_killstreaks\_killstreaks::registerKillstreakDialog("helicopter_comlink_sp", "mpl_killstreak_heli", "kls_cobra_used", "","kls_cobra_enemy", "", "kls_cobra_ready");
	maps\sp_killstreaks\_killstreaks::registerKillstreakDevDvar("helicopter_comlink_sp", "scr_givehelicopter_comlink");
	maps\sp_killstreaks\_killstreaks::registerKillstreakAltWeapon("helicopter_comlink_sp", "cobra_20mm_comlink_sp" );
...
heli_update_global_dvars()
{
	for( ;; )
	{
		...
		level.heli_loopmax = heli_get_dvar_int( "scr_heli_loopmax", "600" );			// how many times helicopter will circle the map before it leaves
...
heli_think( owner, startnode, heli_team, missilesEnabled, protectLocation, hardpointType, armored )
{
	...
	if ( hardpointType == "helicopter_comlink_sp" )
		chopper.defaultWeapon = "cobra_20mm_comlink_sp";
	else
		chopper.defaultWeapon = "cobra_20mm";
	...
	if( IsDefined( level.const_flag_copterrotor ) )
...
heli_targeting( missilesEnabled,  hardpointType  )
{
	...
	{		
		...
		for (i = 0; i < enemies.size; i++)
		{
			...
			if ( self canTargetAI_turret( enemy,  hardpointType ) )
			{
				if( isdefined( enemy ) )
					targets[targets.size] = enemy;
			}
			if ( missilesEnabled && ( self canTargetAI_missile( enemy,  hardpointType  ) ) )
			{
				if( isdefined( enemy ) )
					targetsMissile[targetsMissile.size] = enemy;
			}	
			else
				continue;
		}
...
// targetability
canTargetAI_turret( ai,  hardpointType  )
{
	canTarget = true;
	...
	if ( distance( ai.origin, self.origin ) > level.heli_visual_range )
		return false;
...
//		return false;	
	...
	visible_amount = ai sightConeTrace( heli_turret_point, self);
...
// targetability
canTargetAI_missile( ai,  hardpointType  )
{
	canTarget = true;
	...
	if ( distance( ai.origin, self.origin ) > level.heli_missile_range )
		return false;
	
	if ( !isdefined( ai.team ) )
		return false;
		
	if ( self target_cone_check( ai, level.heli_missile_target_cone ) == false )
		return false;
...
heli_evasive( hardpointType )
{
	...
	if ( hardpointType == "helicopter_gunner_sp" )
	{
...
destroyHelicopter()
{
	...
	maps\sp_killstreaks\_killstreakrules::killstreakStop( self.hardpointType, team );
}
...
clear_client_flags()
{
	if( IsDefined( level.const_flag_warn_fired ) )
		self ClearClientFlag( level.const_flag_warn_fired );
	if( IsDefined( level.const_flag_warn_targeted ) )
		self ClearClientFlag( level.const_flag_warn_targeted );
	if( IsDefined( level.const_flag_warn_locked ) )
