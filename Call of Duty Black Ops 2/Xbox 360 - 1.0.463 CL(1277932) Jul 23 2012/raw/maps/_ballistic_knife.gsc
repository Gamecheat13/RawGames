...
watch_use_trigger( trigger, model, callback, weapon, playerSoundOnUse, npcSoundOnUse ) // self == retrievable_model
{
	...
	max_ammo = WeaponMaxAmmo( "knife_ballistic_sp" ) + 1;
	...
	while ( true )
	{
		...
		ammo_stock = player GetWeaponAmmoStock( "knife_ballistic_sp" );
		ammo_clip = player GetWeaponAmmoClip( "knife_ballistic_sp" );
		...
		hasReloaded = true;
		if( total_ammo > 0 && ammo_stock == total_ammo && current_weapon == "knife_ballistic_sp" )
		{
			hasReloaded = false;
		}
