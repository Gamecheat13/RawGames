...
trigger_issues_orders( color_team, team )
{
	...
	for( ;; )
	{
		...
		// MikeD (10/27/2007 7:18:24): Check to see if this trigger is enabled or not before activating the colors.
...
// MikeD (10/27/2007 7:13:45): Make it so color triggers are disabled immediately after first being,
// if self.color_enabled allows it to be.
trigger_auto_disable()
{
...
get_best_available_new_colored_node()
{
	...
	nodes = get_array_of_closest(self.origin, nodes); // 2/8/2010 - TFLAME - Order nodes so AI should go to closest available on next chain.  Can re-evaluate this after a few days/weeks

	
	for( i=0; i < nodes.size; i++ )
	{
		/*   2/8/2010 - TFLAME - doesn't seem to be any intrinsic value in making sure a guy moves nodes.  Designer should have ability to use same node
...
{
	node.color_user = self;
	assert( !IsDefined( self.color_node ), "Decrement had color_node" );
	self.color_node = node;

	/#
	assert( !IsDefined( self.color_node_debug_val ), "Guy had double color_node_debug_val" );
...
// 	assert( self.radius > 0, "Node " + self.type + " at origin " + self.origin + " does not have a radius set in Radiant." );
	colors = strtok( colorCode, " " );
...
colorNode_replace_on_death()
{
	...
	// JAS 7-30-08, team check because if you have enemies with a color set,
	// it'll otherwise spawn a friendly reinforcement when they die
	if( self.team == "axis" )
	{
		return;
	}
...
// SRS 12/20/2007: updated to allow for different color chains to be reinforced from different areas
...
spawn_hidden_reinforcement( classname, fromColor )
{
	...
	for( ;; )
	{
		...
		spawner script_wait(); // 1/19/10 - TFLAME - Wanted to add delay functionality on friendly respawn
