...
killplayer(player)
{
	...
	if ( isdefined( player.player_view ) ) 
	{
		...
		playfx( level._effect["dog_rip_throat"], tagPos + (0, 0, 15), anglestoforward( player.angles ), anglestoup( player.angles ) );
	}
	...
	if( IsDefined(self) )
	{
		player dodamage( damage, player.origin, self ); // set damage position to player origin to avoid knockback
	}
	else
	{
...
#/		
		...
		// Comment out if... else... upper block if the dog is to always end in minigame.
		if ( dog_cant_kill_in_one_hit(player) && !IS_TRUE(player.force_minigame))
		{
	...
	for ( ;; )
	{
		if ( !pressed )
		{
			...
			if ( isdefined( level.dog_free_kill ) && isdefined( self.melee_able_timer ) && gettime() - self.melee_able_timer <= press_time )
			{
				...
				self stopsounds();//kevin stoping attack sounds when neck break starts
...
PlayerView_EndSequence(player)
{
	...
	if ( isalive( player ) )
	{
		// don't do any animation, just release
		if( IS_TRUE(player.quickDogMeleeRelease) )
		{			
			player setcandamage( true );
