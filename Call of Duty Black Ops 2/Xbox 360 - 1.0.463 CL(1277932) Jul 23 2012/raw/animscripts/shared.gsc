...
HandleNoteTrack( note, flagName, customFunction, var1 )
{
	...
	if ( !IS_TRUE( self.ignore_vo_notetracks ) )
	{
		a_vo_note = StrTok( note, "#" );
		if( a_vo_note[0] == "vox" )
		{
			assert( IsDefined( a_vo_note[1] ), "VO alias not defined after a #vox notetrack" );
			self thread maps\_dialog::say_dialog( a_vo_note[1], undefined, true ); //-- set fakeent to true for the player body
		}
	}
...
trackLoopGetShootFromPos()
{
	...
	if( scriptNeedsTagAim() )
	{
		...
		/#
		if( !IsDefined(origin) && GetDvarint( "ai_debugAiming") )
...
trackLoopGetShootFromAngles()
{
	...
	if( scriptNeedsTagAim() )
	{
		...
		/#
		if( !IsDefined( angles ) && GetDvarint( "ai_debugAiming") ) 
			println("no tag_aim in model " + self.model);
		#/
...
{
	self endon("death");
	self waittill("trackLoopStart"); // notify is sent when the trackLoopStart() is called.
	...
	for(;;)
	{
		...
		if( debugAiming )
		{
			...
			// original angles of shootFromAngles ( can come from tag_aim or offset )	
		...
		if( debugAiming )
		{
			...
			// modified angles of shootFromAngles ( can come from tag_aim or offset but changed due to smooth blending )	
		...
		if ( !IsDefined( shootPos ) )
		{
			...
			if( IsDefined( self.node ) && IsDefined( anim.isCombatScriptNode[ self.node.type ] ) && DistanceSquared(self.origin, self.node.origin) < 16 * 16 )
			{
		...
		// AI_TODO - waittillframeend seems to actually get rid of pops on aiming. Investigate how this is related to the anim commands
...
shouldThrowDownWeapon()
{
	...
	const THROWDOWN_WEAPON_ENEMY_CLOSEBY_DISTSQ = 512 * 512;
	...
	if( IsDefined(self.shootPos) )
	{
		...
		if ( self.weaponClass == "rocketlauncher" && ( distSqToShootPos < THROWDOWN_WEAPON_ENEMY_CLOSEBY_DISTSQ || self.a.rockets < 1 ) )
			return true;
	}
...
// if RPG AI gets shot while swapping the weapons then this is a fallback
...
{	
	...
	if( IS_TRUE(self.a.weapon_switch_ASAP) )
	{
		assert( hasSecondaryWeapon(), "self.secondaryweapon not defined! check the aitype for this actor. " );
...
shouldSwitchWeaponForSituation()
{
	...
	// check that the enemy is in a designated volume near explodables - handled in _load.gsc//
	if( isEnemyInExplodableVolume() && secondaryWeaponClass == "rocketlauncher" && self.weaponclass != "rocketlauncher" )
	{
		return true;
	}
...
{	
	...
	isAimingAtEnemy = IS_TRUE( skipAimCheck ) || animscripts\combat_utility::aimedAtShootEntOrPos();
...
//--------------------------------------------------------------------------------
// event reactions 
// AI_TODO - Commented this out for now. We will look into this system next game
//--------------------------------------------------------------------------------
