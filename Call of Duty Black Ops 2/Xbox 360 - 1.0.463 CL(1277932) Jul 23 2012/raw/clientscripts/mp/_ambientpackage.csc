...
main()
{
	...
	setAmbientRoomSideChainDuck( "northwest_room", "north_west_voice_duck" );
...
AmbientPackageTrigger()
{
	...
	for (;;)
	{
		...
		{
			self local_player_trigger_thread_always_exit(trigPlayer, ::trig_enter, ::trig_leave);
		}
	}
}
...
init()
{
	...
	setAmbientRoomTone( "final_kill_cam", "mpl_final_kill_cam_loop",.25,2 );
...
declareAmbientPackage( package, defaultPackage )
{
	...
	{
		level.ambientPackages[package].defaultPackage = false;
	}
}
...
declareAmbientRoom( room, defaultRoom )
{
		...
		{
			level.ambientRooms[room].defaultRoom = false;
		}
}
...
init_trigger_data()
{
	...
	{		
		...
		if ( isdefined(defaultRoom) )
		{			
/#			println("Client : set default room " + defaultRoom.ent + " ambient package triggers.");  #/

			SetAmbientPackage(false, false, 2, defaultRoom.ent, true);				

			keys = getArrayKeys(defaultRoom.context);
			
			for(i=0; i<keys.size; i++)
			{
				setambientroomcontextengine( defaultRoom.ent, keys[i], defaultRoom.context[ keys[i]], true);
			}
		}
	}
	...
	for(i=0; i<trigKeys.size; i++)
	{
		...
		trigger SetVehicleTrigger();

		defaultRoom = false;	

		if ( isdefined( trigger.defaultRoom ) )
		{
			defaultRoom = trigger.defaultRoom;
		}

		hasAmbientRoom = isdefined ( trigger.script_ambientroom );
		useAmbientRoom = hasAmbientRoom && isdefined( level.ambientRooms[trigger.script_ambientroom] );

		hasAmbientPackage = isdefined ( trigger.script_ambientpackage );
		useAmbientPackage = hasAmbientPackage && isdefined( level.ambientPackages[trigger.script_ambientpackage] );

		entnum = trigger GetEntityNumber();
	
		SetAmbientPackage(hasAmbientRoom, hasAmbientPackage, trigger.script_ambientpriority, entnum, defaultRoom);		

		room = level.ambientRooms[trigger.script_ambientroom];
		...
		if ( isdefined(room) )
		{
			...
			{			
				...
				{
					setambientroomcontextengine( entnum, keys[j], room.context[ keys[j]], false);
				}
			}
		}
	}
...
findDefaultRoom()
{
	...
	if(isdefined(roomArray))
	{
		for ( i = 0; i < roomArray.size; i++ )
		{
			if(level.ambientRooms[roomArray[i]].defaultRoom == true)
			{
				return roomArray[i];
			}			
		}
...
findDefaultPackage()
{
	...
	if(isdefined(packageArray))
	{
		for ( i = 0; i < packageArray.size; i++ )
		{
			if(level.ambientPackages[packageArray[i]].defaultPackage == true)
			{
...
switchActiveAmbientRoom(ambientRoom, roomColliderCent, defaultRoom)
{		
	...
	else
	{
		...
		if(isdefined(defaultRoom))
		{
			...
			if(isdefined(ambientRoom) && !isdefined(ambientRoom.script_ambientroom))
			{
				ambientRoom.script_ambientroom = "default_script_room";	
			}
		}		
	}
...
switchActiveAmbientPackage(ambientPackage, packageColliderCent, defaultRoom)
{
	packageName = "";

	if(isdefined(ambientPackage) && isdefined(ambientPackage.script_ambientpackage))
	{
		newpackage = level.ambientPackages[ambientPackage.script_ambientpackage];
		packageName = ambientPackage.script_ambientpackage;
	}
	else
	{
		...
		if(isdefined(defaultPackage) && isdefined(defaultRoom))
		{
			if ( defaultRoom )
			{
				newpackage = level.ambientPackages[defaultPackage];
				ambientRoom = newpackage;
...
//
// code callback function that is called once per client frame to set the ambient state 
// for each player. Depending on how things go we may want to consolidate this function and 
// the one below
//
setCurrentAmbientState(ambientRoom, ambientPackage, roomColliderCent, packageColliderCent, defaultRoom)
{
	if(level.useCodeTriggers == false)
	{
		return;
	}
