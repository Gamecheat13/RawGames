...
init_session_mode_flags()
{
	...
	level.GAMEMODE_CLAN_MATCH			= 6;
	level.GAMEMODE_ARCADE				= 7;
	level.GAMEMODE_ZOMBIES				= 8;
	level.GAMEMODE_ZOMBIETRON			= 9;
	level.GAMEMODE_WAR					= 10;
}
...
array_remove_index( array, index, keepArrayKeys )
{
	newArray = [];
	keys = getArrayKeys( array );
	
	//TFLAME 1/20/11 - added ability to keep array keys from previous array
	if(IsDefined(keepArrayKeys))
	{
		for( i = ( keys.size - 1 );i >= 0 ; i-- )
		{
...
/@
"Name: array_combine( <array_1>, <array_2> )"
"Summary: Combines two normal arrays or two associative arrays and returns the resulting array. This function doesn't care if it produces duplicates in the array."
"Module: Array"
"CallOn: level"
"MandatoryArg: <array_1> The first array to concatinate."
"MandatoryArg: <array_2> The second array to concatinate."
"Example: a_combined = array_combine( array_1, array_2 );"
"SPMP: singleplayer"
@/
array_combine( array_1, array_2 )
{
	Assert( IsDefined( array_1 ), "array_1 is a required argument for array_combine()" );
	Assert( IsDefined( array_2 ), "array_2 is a required argument for array_combine()" );
	...
	a_new = array_1;
	a_keys = GetArrayKeys( array_2 );
	
	// Add array_2 items into the new array
	for ( i = 0; i < a_keys.size; i++ )
	{
		...
		if ( IsString( key ) )
		{
			Assert( !IsDefined( a_new[ key ] ), "array_combine() cannot combine two arrays that have the same index, " + key );
			a_new[ key ] = array_2[ key ];
		}
		else
		{
			a_new[ a_new.size ] = array_2[ key ];
		}
	}
...
/@
"Name: is_in_array( <array> , <findee> )"
"Summary: Returns true if <findee> is an entity in array <array> . False if it is not."
"Module: Array"
"CallOn: "
"MandatoryArg: <array> : array of entities to search through"
"MandatoryArg: <findee> : element to check if it's in the array"
"Example: b_in_array = is_in_array( a_targets, veh_plane1 );"
"SPMP: both"
@/ 
is_in_array( array, findee )
{
...
array_thread( entities, func, arg1, arg2, arg3, arg4, arg5, arg6 )
{
	...
	if ( IsArray( entities ) )
	{
		if (entities.size)
		{
			...
			for ( i = 0; i < keys.size; i++ )
			{
				single_thread( entities[keys[i]], func, arg1, arg2, arg3, arg4, arg5, arg6 );
			}
		}
	}
	else
...
trigger_wait( strName, strKey )
{
	...
	assert( IsDefined(triggers) && triggers.size > 0, "trigger not found: " + strName + " key: " + strKey );
...
struct_class_init()
{
	...
	for ( i=0; i < level.struct.size; i++ )
	{
		...
		{
			assert( !IsDefined( level.struct_class_names[ "script_linkname" ][ level.struct[ i ].script_linkname ] ), "Two structs have the same linkname" );
