...
BlendIntoStandRun()
{
	shouldTacticalWalk	   = call_overloaded_func( "animscripts\run", "ShouldTacticalWalk" );
	...
	else
	{
		...
		if( self.a.pose == "stand" )
		{
			if ( call_overloaded_func( "animscripts\move", "MayShootWhileMoving" ) && self.bulletsInClip > 0 && isValidEnemy( self.enemy ) )
			{
				...
				// if we are moving backwards to start with then change the orientmode to face against the lookahead
				if( call_overloaded_func( "animscripts\run", "ShouldShootWhileRunningBackward" ) )
				{
					self OrientMode( "face direction", VectorScale( self.lookaheaddir, -1 ) );
				}
			}
		}
