...
CoverGlobalsInit()
{
	...
	anim.coverGlobals.BLINDFIRE_WAIT_TIME_MIN			= 3000;
	anim.coverGlobals.BLINDFIRE_WAIT_TIME_MAX			= 12000;
		...
		if( isEnemyVisibleFromExposed() || canSuppressEnemyFromExposed() )
		{
			...
			if( self.team != "allies" && GetTime() >= nextAllowedBlindfireTime )
			{
				if( blindfire( behaviorCallbacks ) )
				{
					...
					if( !IS_TRUE( self.a.favor_blindfire ) )
						nextAllowedBlindfireTime += RandomIntRange( anim.coverGlobals.BLINDFIRE_WAIT_TIME_MIN, anim.coverGlobals.BLINDFIRE_WAIT_TIME_MAX );
...
coverReload( behaviorCallbacks, threshold )
{
	...
	if( !shouldForceBehavior("reload") && self.bulletsInClip > weaponClipSize( self.weapon ) * threshold )
	{
		return false;
	}
...
moveToNearbyCover()
{
	...
	if( self.fixedNode || self.doingAmbush || self.keepClaimedNode || self.keepClaimedNodeIfValid || self.keepClaimedNodeIfValid )
		return false;
...
shouldSwitchSides( forVariety )
{
	if(	self.a.atPillarNode )
	{
		...
		else if( forVariety && GetTime() > self.a.nextAllowedSwitchSidesTime && RandomFloat(1) > 0.5 )
		{
		...
		else if( !forVariety && IsDefined( self.enemy ) && GetTime() > self.a.lastSwitchSidesTime + RandomIntRange( 2000, 4000 ) )
		{
...
switchSides( behaviorCallbacks )
{
	...
	if( result )
	{
		...
		self.a.nextAllowedSwitchSidesTime = GetTime() + 5000;
