...
main()
{
	...
	// dk 5.26.11 - let the reaction event system know about this death
	...
	// AI_TODO - Check if we need this anynmore as we are not sure 
	removeSelfFrom_SquadLastSeenEnemyPos( self.origin ); 
	
	anim.painGlobals.numDeathsUntilCrawlingPain--; 
	anim.painGlobals.numDeathsUntilCornerGrenadeDeath--; 
...
playDeathSound()
{
	if( !damageLocationIsAny( "head", "helmet" ) )
	{
		if( !shouldDieQuietly() )
		{
			...
			{
				// Script models won't have a team
...
// SUMEET_TODO - Add crouch pains once MW3 animations are ported over
special_death()
{
	/#
...
play_bulletgibbed_death_anim()
{
	...
	const SNIPER_GIB_CHANCE			= 30;  // sniper has 30% chance within maxGibDist from GDT
...
//--------------------------------------------------------------------------------
// BJoyal (10/13/11) - added for sword gibs
// Sword gib deaths
//--------------------------------------------------------------------------------
getSwordGibRef()
{
	if ( !(RandomInt( 100 ) < 50) )	// % chance to gib
	{
		return false;	
	}
...
do_gib()
{
	...
	// BJoyal (10/13/11) - Moved to check for pulwar sword and not have to check damageweapon twice
	if ( IsDefined(self.damageWeapon) )
	{
		if (isSubStr( self.damageWeapon, "flame" ) || isSubStr( self.damageWeapon, "molotov" ) || isSubStr( self.damageWeapon, "napalmblob" ) )
			return;
		
		if( IsSubStr( self.damageWeapon, "pulwar" ) )
		{
			self getSwordGibRef();
		}
	}	
...
clearSightPosNear( org )
{
	...
	if( distancesquared( org, self.sightPos ) < (80*80) )
	{
