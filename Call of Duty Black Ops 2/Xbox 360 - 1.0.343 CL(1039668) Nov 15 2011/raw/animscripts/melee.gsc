...
#define KNIFE_ATTACK_FX_NAME	"melee_knife_ai"
#define KNIFE_ATTACK_FX_PATH	"impacts/flesh_hit_knife"
#define KNIFE_ATTACK_FX_TAG		"TAG_KNIFE_FX"
...
Melee_IsValid()
{
	...
	// MELEE_TODO - We are not using AI to AI melee yet
	if ( isAI( target ) )
	{ 
...
Melee_Standard_Main()
{
	...
	while ( true )
	{
		...
		self setflaggedanimknoballrestart( "meleeanim", animArray("melee", "combat" ), %body, 1, .2, 1 );
...
Melee_Standard_GetInPosition()
{
	...
	if ( enemyDistanceSq <= MELEE_RANGE_SQ )
	{
		...
		self SetFlaggedAnimKnobAll( "readyanim", animArray("stand_2_melee", "combat" ), %body, 1, .3, 1 );
	...
	raiseGunAnimTravelDist = length( getmovedelta( animArray("run_2_melee", "combat" ), 0, 1 ) );
	...
	raiseGunFullDuration = getanimlength( animArray( "run_2_melee", "combat" ) ) * 1000;
	...
	while ( 1 )
	{
		...
		if ( !raisingGun )
		{
			...
			{
				Melee_StartMovement();
				self SetFlaggedAnimKnobAllRestart( "chargeanim", animArray("run_2_melee", "combat" ), %body, 1, .2, 1 );
