...
main()
{
	...
	anim._effect["bigdog_explosion"] = LoadFx( "explosions/fx_large_vehicle_explosion" );
	...
	// bigdog damage areas
	self.partHealth = [];
	self.partHealth[ "FL" ]		= self.health;
	self.partHealth[ "FR" ]		= self.health;
	self.partHealth[ "RL" ]		= self.health;
	self.partHealth[ "RR" ]		= self.health;
	...
	animscripts\bigdog\bigdog_melee::melee_init();
	...
	self thread contextual_melee_shutoff();	
...
set_fight_dist()
{
	...
	// stop farther from enemy than normal AI (192)
	self.pathEnemyLookAhead = 384; 
	self.pathEnemyFightDist = 384; 
}
...
setup_bigdog_turret()
{
	...
	// create new turret
	self.turret = create_turret( self.origin, self.angles, self.team, "bigdog_dual_turret", self.headModel, (0,0,50) );
...
update_turret_target()
{
	...
	while(1)
	{		
		if( !self.turretIndependent && IsDefined(self.enemy) )
		{
			shootAtPos = self.enemy GetShootAtPos();

			self.turret set_turret_target( self.enemy, shootAtPos - self.enemy.origin );
		}
...
fire_when_on_target()
{
	...
	const n_fire_min = 1.2;  // minimum time we want to fire per burst
	const n_fire_max = 2.3;  // maximum time we want to fire per burst
	const n_wait_min = 1.0;  // minimum wait time between bursts
	const n_wait_max = 2.0;  // maximum wait time between bursts
	...
	const spinUpTime = 1.0; // same as gdt
	...
	while(1)
	{
		...
		{
			canShootTarget = BulletTracePassed( self GetTagOrigin( "tag_flash" ), currentTarget GetShootAtPos( self ), true, self );
		}
...
replaceTurretModel()
{
	if( IsDefined(self.turret) )
	{
		...
		// replace with fake model
		// AI_TODO: this doesn't actually work. "tag_turret" angles are not changing on the server
		if( IsAlive(self) )
		{
			fakeTurret = Spawn( "script_model", self GetTagOrigin("tag_turret") );
			fakeTurret.angles = self GetTagAngles("tag_turret");
			fakeTurret SetModel( self.headModel );
			fakeTurret LinkTo( self, "tag_turret" );
		}
	}
}
...
bigdog_damage_override( inflictor, attacker, damage, flags, meansofdeath, weapon, vpoint, vdir, sHitLoc, modelIndex, psOffsetTime, boneName )
{
	...
	if( legsHit.size )
	{
		...
		{
			if( bigdog_damage_leg( damage, meansofdeath, weapon, leg, boneName ) )
			{
				// dog's dead
				replaceTurretModel();
...
{	
	...
	if( shouldUseLauncher && IsDefined( self.enemy ) && GetTime() > self.launcherCooloffTime )
	{
...
{
	if( IsDefined( self.enemy ) && DistanceSquared( self.origin, self.enemy.origin ) > LAUNCHER_MIN_DISTANCE_SQR )
		return true;
...
bigdog_damage_leg( damage, mod, weapon, leg, boneName )
{
	...
	// lose the leg
	if( self.partHealth[leg] < (self.initialHealth / 2) )
	{		
...
tryToBreakOffLeg( leg, allOfIt )
{
	...
	if( IsDefined( self.missingLegs[ leg ] ) && self.missingLegs[ leg ] == "bottom" && allOfIt )
	{
		// hide the entire leg
...
bodyPieceFallsOff( point, anyPiece )
{
	...
	if( boneArray.size > 0 )
	{
		...
		self.bodyDamageMap[ direction ] = array_remove_index( boneArray, boneIndex );
...
contextual_melee_shutoff()
{
	//self endon("death");
	//SOUND - Shawn J - the above endon borked this script, so I had to comment it out.
	self waittill("claw_cntxt");
