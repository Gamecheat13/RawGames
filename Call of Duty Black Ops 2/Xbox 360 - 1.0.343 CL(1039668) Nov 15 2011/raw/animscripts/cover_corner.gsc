...
getCornerMode( node, point )
{
	...
	{
		// AFTER_GL_TODO - fix issues with pillar over after GL
		...
		if ( IsDefined( node ) && ( stanceSupported ) && ( yaw > self.leftAimLimit && self.rightAimLimit > yaw ) )
		{
			...
			//if( self.a.atPillarNode && self.a.pose == "crouch" && !self.coverNode isNodeDontStand() )
			//	modes = array_exclude( modes, array("over") );
			
			if( is_in_array( modes, forceCornerMode ) )
				return forceCornerMode;
		}
	}
	#/
	...
	if ( cornerLeftDirection )
	{
		...
		// try stepping out
		if ( !noStepOut && yaw < 14 && !usingPistol() ) //AI_TODO: add pistol anim support?
		{
	...
	else
	{
		...
		// try stepping out
		if ( !noStepOut && yaw > -12 && !usingPistol() ) //AI_TODO: add pistol anim support?
		{
...
changeStepOutPos()
{
	...
	// change self.a.special so that proper pains and deaths will be used
	self.a.special = "cover_" + self.cornerDirection + "_" + self.a.pose + "_" + positionToSwitchTo;
...
stepOut() /* bool */
{
	...
	setStepOutSpecial( newCornerMode );
	
	self StartAiming( undefined, fullbody, .3 );
	self thread animscripts\shared::trackShootEntOrPos();
	
	// AI_TODO: add tag_aim to lean_aim5's and get rid of this crap
	if( newCornerMode == "lean" && self.a.pose == "stand" && !usingPistol() && !self.a.atPillarNode )
	{
...
setStepOutSpecial( newCornerMode )
{
	// AI_TODO - Handle pillar over. It is not captured yet.
	// set the special so that it cab be used by pain/death properly.
	if( self.a.script == "cover_pillar" )
	{
		...
		if ( newCornerMode == "A" || newCornerMode == "B" )
			self.a.special = "none"; // use exposed pains/deaths for A and B at pillar node.
	}
	else
	{
		if( newCornerMode == "lean" )
			setAnimSpecial();
		else if ( newCornerMode == "A" || newCornerMode == "B" )
			self.a.special = "cover_" + self.cornerDirection + "_" + self.a.pose + "_" + newCornerMode;
		else if ( newCornerMode == "over" && self.a.pose == "crouch" )
			self.a.special = "cover_crouch_aim";
		else
			self.a.special = "none";
		
		// set up corner right grenade martyrdom death
		if ( newCornerMode == "B" && self.cornerDirection == "right" && coinToss() )
