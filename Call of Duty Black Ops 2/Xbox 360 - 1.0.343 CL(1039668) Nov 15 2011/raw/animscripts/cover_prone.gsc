...
main()
{
	...
	{
		call_overloaded_func( "animscripts\combat", "main" );
		return;
	}
	...
	if ( IsDefined( self.a.arrivalType ) && self.a.arrivalType == "prone_saw" )
	{
		...
		self call_overloaded_func( "animscripts\cover_wall", "useSelfPlacedTurret", "saw_bipod_prone", "weapon_saw_MG_Setup" );
	}
	else if ( IsDefined( self.node.turret ) )
	{
		self call_overloaded_func( "animscripts\cover_wall", "useStationaryTurret" );
	}
...
shouldFireWhileChangingPose()
{
	...
	{
		return call_overloaded_func( "animscripts\move", "MayShootWhileMoving" );
	}
