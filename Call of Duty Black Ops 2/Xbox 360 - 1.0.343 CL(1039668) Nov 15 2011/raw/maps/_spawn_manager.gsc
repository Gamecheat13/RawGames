...
spawn_manager_create_spawn_manager_struct( from_ent )
{
	...
	// add this newly created spawn manager to the global array
	level.spawn_managers[ level.spawn_managers.size ] = spawn_manager_ent;

	// return it to the calling function just in case
...
start_triggers(trigger_type)
{
	...
	{
		// Start waiting to be killed, trigger needs to have sm_kill on it to kill the spawn manager.
		if (IsDefined(triggers[i].sm_kill))
		{
			triggers[i] thread spawn_manager_kill_trigger_think();
		}
		
		// Start waiting to be enabled, trigger needs to be targeting the spawn manager
		if( IsDefined( triggers[i].target ) )
		{
			...
			// if targets array has atleast one element in it, then it means that this trigger is targeting 
			// a spawn manager
			if( targets.size > 0 )
			{
				for( j = 0; j < targets.size; j++ )
				{
					assert( is_in_array( level.spawn_managers, targets[j] ), "Spawn manager is found but doesnt exists in the level.spawn_managers" );

					sm_num++;
					triggers[i] thread spawn_manager_enable_trigger_think(targets[j]);

					assert( !(sm_num > 1), "Only one spawn manager can be triggered by a single trigger." );
				}
			}
		}
	}
...
spawn_manager_debug()
{
	for(;;)
	{
		while( GetDvar( "ai_debugSpawnManager" ) != "1" )
		{
			wait(0.2);
		}
