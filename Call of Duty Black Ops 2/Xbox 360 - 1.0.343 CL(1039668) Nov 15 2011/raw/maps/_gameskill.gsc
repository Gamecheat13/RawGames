...
// TFLAME - 2/22/11 - This appears to be run every time an AI fires, controls AI accuracy and thus much of the combat difficulty
...
set_accuracy_based_on_situation()
{
	...
	if ( isPlayer( self.enemy ) )
	{
		resetMissDebounceTime();  // TFLAME - 3/7/11 - sets my self.a.missTimeDebounce to 3 seconds from now
		if ( self.a.missTime > gettime() ) // TFLAME - 3/7/11 - only set in resetMissTime()
		{
			self.accuracy = 0;
			return;
		}
...
// TFLAME - 2/22/11 - This should still be working, will be set on any AI who's weapon is a semi-automatic rifle with a firetime greater than 0.2, so probably just bolt actions.  May want to change this
setSniperAccuracy()
{
	/*
	...
	*/
	...
	// TFLAME - 2/22/11 - Miss the first shot on a newly acquierd enemy, then become very and increasingly accurate as long as I have the same enemy
...
// TFLAME - 2/24/11 - Should only be used in zombies, if at all.  Once T6 gets zombies we'll see if this is missed.  Otherwise - delete.
/*
...
aa_player_attacks_enemy_with_ads( player, amount, type, point )
{
	...
	assert( GetDvarint( "aa_player_damage_dealt" ) > 0 );
...
// TFLAME - 2/24/11 - These should only be used in zombies, if at all.  Once T6 gets zombies we'll see if this is missed.  Otherwise - delete.
/*
bullet_attack( type )
{
