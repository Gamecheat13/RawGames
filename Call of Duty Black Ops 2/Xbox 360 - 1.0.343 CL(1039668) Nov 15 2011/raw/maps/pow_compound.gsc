...
start()
{
	...
	{
		start_nodes[ i ] thread print_3d_loop();
	}
	*/
	...
	while ( !can_hind_land )
	{
		...
		level.hind waittill_or_timeout( "start_animating", 16 ); // JoannaL - 5/11/11 - Fixes the issue that bug number 1475 has
		
		if ( !flag( "hind_cannot_land" ) )
		{
			can_hind_land = true;
		}
	}
	...
	//-- cleanup all the extra hind parts that get left around
		...
		// TFLAME - 4/6/11 - This is failing and breaking progression so commenting out for now
...
// JoannaL - 5/11/11 - waits for a notify or times out
// this version is more specific to solve bug number 1475 and for the hind in pow
waittill_or_timeout( str_msg, n_seconds )
{
	self endon( str_msg );
	
	wait n_seconds;
	
	flag_set( "hind_cannot_land" );
}

// JoannaL - 5/11/11 - used for debuggin bug number 1475
print_3d_loop()
{
	while ( true )
	{
		Print3d( self.origin, "node_start", (1, 1, 0), 1, 9 );
...
init_spawners_in_cave()
{
	...
	array_thread( pows, ::add_spawn_function, maps\_utility::change_movemode, "cqb" );
}
...
kravchenko_pacing()
{
	...
	level thread run_scene( "pacing" );
	
	flag_wait( "pacing_started" );
	
	level.krav = GetEnt( "kravchenko_ai", "targetname" );
	...
	//playsoundatposition( "mus_pow_new_ending" , (0,0,0) );
	level.krav Hide();
}
...
play_endfight_animation()
{
	...
	run_scene( "endfight_enter" );
	run_scene( "endfight_firstattack" );
	run_scene( "endfight_gundraw" );
	run_scene( "endfight_kickblock" );
...
play_endfight_animation_player()
{
	...
	//run_scene( "endfight_kickblock" );
	//run_scene( "endfight_gungrab" );
	//level thread run_scene( "endfight_shoot" );
...
play_landed_animation()
{
	...
	maps\_vehicle::scripted_spawn( level.vehiclespawngroup + 16 ); //-- other hind
...
setup_woods_kill_standing_guard() //-- this is woods killing the standing guard
{
	...
	if ( IsAlive( level.stander ) ) 
	{
			level.woods shoot_and_kill( level.stander );
	}
...
guy_mantle_onto_box()
{
	...
	add_generic_ai_to_scene( self, "mantle_on_box" );
	level thread run_scene( "mantle_on_box" );
	wait(0.5);
...
rpg_rollout_guy()
{
	...
	add_generic_ai_to_scene( self, "rpg_rollout" );
	run_scene( "rpg_rollout" );
	self.goalradius = 32;
	self SetGoalPos( self.origin );
...
bottom_guard_stand_animate()
{
	...
	add_generic_ai_to_scene( self, "stand_guard_idle" );
	level thread run_scene( "stand_guard_idle" );
...
bottom_guard_sit_animate()
{
	...
	add_generic_ai_to_scene( self, "sit_guard_react" );
	level thread run_scene( "sit_guard_react" );
	wait(0.75);
...
cave_flame_attack_animate()
{
	...
	struct thread anim_first_frame( self, "flame_over", undefined, "generic" );
	...
	add_generic_ai_to_scene( self, "flame_over" );
	run_scene( "flame_over" );
