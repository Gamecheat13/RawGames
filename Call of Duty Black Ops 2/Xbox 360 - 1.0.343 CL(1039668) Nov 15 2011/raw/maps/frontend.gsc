...
frontend_precache()
{
	...
	PrecacheMenu( "main_sp" );
	PrecacheMenu( "blackout_3d_tv" );
	PrecacheMenu( "menu_aar_summary" );
...
computer_hintstring_regulator()
{
	...
	trig SetHintString( &"FRONTEND_USE_COMPUTER" );
...
//			get_players()[0] CloseMainMenu( );
	...
	else if( state == "displayAAR" )
	{
		if( GetDvar( "ui_display_aar" ) == "1" )
		{
			//gets set when main_text is open; need to wait for that before opening blackout_3d_tv
			//flag_wait("allow_escape_buttons" );
			wait_for_first_player();
			wait(2);
			SetDvar( "ui_next_menu_name", "menu_aar_summary" );
			SetDvar( "ui_menu_parent_3d_tv_name", "main_text" );
			get_players()[0] OpenMainMenu( "blackout_3d_tv" );
			SetDvar( "ui_display_aar", "0" );
		}
	}
}
...
start_interrogation_questions()
{
	...
	levelnum = GetLocalProfileInt( "missions_unlocked" );
	...
	if (IsDefined(current_saved_level))
	{
		for (i=1; i < 10; i++)
		{
			...
			{
				ARRAY_ADD(level._savedgame_questions, current_saved_level+"_q"+i);
			}
		}
	}
	...
	while(1)
	{
		...
		for (i=0; i < generic_lines_amount; i++)
		{
			...
			wait 25;
			ARRAY_ADD(used_generic_lines, new_line_num);
		}
	}	
}
...
extracam_quadrant()
{
	for (i=2; i < 6; i++)
	{
		...
		tv delay_thread ( 0.6, ::set_cam_space, i);
	}
}
