...
/#
	assert( IsDefined( level.scr_anim["spiderhole"]["sprint"][0] ), "Missing spiderhole sprint animations (array)." );
	assert( IsDefined( level.scr_anim["spiderhole"]["spiderhole_idle_crouch"][0] ), "Missing spiderhole spiderhole_idle_crouch animations (array)." );
	
	assert( IsDefined( level.scr_anim["spiderhole"]["jump_out"] ), "Missing spiderhole jump_out animation." );
	assert( IsDefined( level.scr_anim["spiderhole"]["stumble_out"] ), "Missing spiderhole stumble_out animation." );
	assert( IsDefined( level.scr_anim["spiderhole"]["grenade_toss"] ), "Missing spiderhole grenade_toss animation." );
	assert( IsDefined( level.scr_anim["spiderhole"]["gun_spray"] ), "Missing spiderhole gun_spray animation." );
	assert( IsDefined( level.scr_anim["spiderhole"]["crouch2stand"] ), "Missing spiderhole crouch2stand animation." );
	assert( IsDefined( level.scr_anim["spiderhole"]["jump_attack"] ), "Missing spiderhole jump_attack animation." );
...
#/
	...
	---------------------------------------------------*/
	assert( IsDefined( self.target ), " Spiderhole Spawner @ " + self.origin + " does NOT target anything!" ); 
	...
	for( i = 0; i < ents.size; i++ )
	{
		...
		{
/#
			assert( IsDefined( level.scr_animtree["spiderhole_lid"] ), "Missing spiderhole lid animtree." );
			assert( IsDefined( level.scr_anim["spiderhole_lid"]["jump_out"] ), "Missing spiderhole lid jump_out animation." );
			assert( IsDefined( level.scr_anim["spiderhole_lid"]["stumble_out"] ), "Missing spiderhole lid stumble_out animation." );
			assert( IsDefined( level.scr_anim["spiderhole_lid"]["grenade_toss"] ), "Missing spiderhole lid grenade_toss animation." );
			assert( IsDefined( level.scr_anim["spiderhole_lid"]["gun_spray"] ), "Missing spiderhole lid gun_spray animation." );
			assert( IsDefined( level.scr_anim["spiderhole_lid"]["crouch2stand"] ), "Missing spiderhole_lid crouch2stand animation." );
			assert( IsDefined( level.scr_anim["spiderhole_lid"]["jump_attack"] ), "Missing spiderhole lid jump_attack animation." );
#/
	...
	---------------------------------------------------*/
	assert( IsDefined( anim_org ), " SpiderHole Spawner @ " + self.origin + " does NOT target a script_struct!" ); 	
...
spiderhole_charge( anim_org, spider_lid, closest_player )
{
	...
	// SUMEET_TODO - Had to comment this as we are not supporting banzai feature anymore
...
spiderhole_run_to_node( anim_org, spider_lid, target_node )
{
	...
	if( isDefined( target_node ) )
	{
		...
		assert( self.goalheight == 80, "Tried to set goalheight on guy with export " + self.export + " before running spawn_failed on the guy." ); 
...
/#
	while( 1 )
	{
		...
		pos2 = self GetTagOrigin( tagname ) + VectorScale( forward, 48 );
