...
#include maps\mp\zombies\_zm_type_zombiedog;
...
//t6todo if( isDefined( enemies[i].animname ) )
		{
...
zombie_human_spawn( dog_spawner )
{
	...
	{
		println( "Spawning dog at your crosshair position" );
	}
	...
	dog forceteleport( dog_spawner.origin );//trace["position"], direction );
...
//	dog thread maps\mp\gametypes\_weapons::monitor_dog_special_grenades();
	...
	level.zombies[ level.zombies.size ] = dog;
	
	println( "ZM >> spawn_zombie - zombie_dog_spawn count = " + level.zombies.size );
	
	return dog;
}

zombie_dog_spawn( dog_spawner )
{
	player = getHostPlayer();
...
{ 
	...
	{ 
		println( "ZM >> spawn_zombie - zombie_dog_spawn CALL" );
		//t6todo guy = spawner StalingradSpawn(); 
		//guy = zombie_dog_spawn( spawner );
		guy = zombie_human_spawn( spawner );
		println( "ZM >> spawn_zombie - zombie_dog_spawn DONE" );
			 
	} 
	else 
	{ 
		println( "ZM >> spawn_zombie - DoSpawn" );
		guy = spawner DoSpawn();  
	} 

println( "ZM >> spawn_zombie - spawner.count GO" );
	spawner.count = 666; 
println( "ZM >> spawn_zombie - spawner.count DONE" );
...
println( "ZM >> spawn_zombie - spawner.count GO" );
	...
	{ 
		...
		println( "ZM >> spawn_zombie - SUCCESS" );
		return guy;  
	}

	println( "ZM >> spawn_zombie - FAIL" );
	return undefined;  
}
...
generated_radius_attract_positions( forward, offset, num_positions, attract_radius )
{
	...
	for( i = offset; i < 360+offset; i += degs_per_pos )
	{
		...
		{
//T6todo			pos = maps\_zombiemode_server_throttle::server_safe_ground_trace( "poi_trace", 10, self.origin + rotated_forward + ( 0, 0, 10 ) ); //only trace from 10 units above instead of 100 units
		}
		else
		{
//T6todo			pos = maps\_zombiemode_server_throttle::server_safe_ground_trace( "poi_trace", 10, self.origin + rotated_forward + ( 0, 0, 100 ) );
		}
		...
		if(is_true(level.use_alternate_poi_positioning))
		{
			...
			{
				if( self.origin[2] >= pos[2] && self.origin[2] - pos[2] <= 150 )
				{
					pos_array = [];
					pos_array[0] = pos;
					pos_array[1] = self;
...
//T6todo	if(  player maps\_laststand::player_is_in_laststand() )
//T6todo	{
//T6todo		return false; 
//T6todo	}
...
{
//T6todo	maps\_zombiemode_powerups::include_zombie_powerup( powerup_name );
}
...
//T6todo	self thread maps\_hud_message::notifyMessage( notifyData );
}
...
{
	return (is_lethal_grenade( weaponname ) || is_tactical_grenade( weaponname ) || is_placeable_mine( weaponname ) || is_melee_weapon( weaponname ) );
}


is_player_offhand_weapon( weaponname )
{
	return (self is_player_lethal_grenade( weaponname ) || self is_player_tactical_grenade( weaponname ) || self is_player_placeable_mine( weaponname )  || self is_player_melee_weapon( weaponname ) );
}
