...
init_audio_functions()
{
	...
	PrintLn( "ZM >> audio - init_audio_functions numplayers=" + players.size );
...
do_zombies_playvocals( alias_type, zombie_type )
{
    ...
    PrintLn( "ZM >> audio - do_zombies_playvocals alias_type=" + alias_type );
...
{              
	...
	println( "ZM >> create_and_play_dialog called with = " + category + " and = " + type );
	...
	println( "ZM >> create_and_play_dialog a" );
	
	
	if( !IsDefined( level.plr_vox[category][type] ) )
	{
		PrintLn( "ZM >> No Category: " + category + " and Type: " + type );
		
		println( "ZM >> create_and_play_dialog b" );
		return;
	}
	
	println( "ZM >> create_and_play_dialog c" );
	//Preventing the player from spouting off a bunch of crap in laststand
//t6todo	if( self maps\_laststand::player_is_in_laststand() && ( type != "revive_down" || type != "revive_up" ) )
//t6todo	{
//t6todo	    return;
//t6todo	}
	...
	println( "ZM >> create_and_play_dialog d" );
	...
	if ( !IsDefined ( self.sound_dialog[ alias_suffix ] ) )
	{
		...
		println( "ZM >> create_and_play_dialog e" );
		...
		println( "ZM >> create_and_play_dialog f" );
	...
	println( "ZM >> create_and_play_dialog g" );
...
{
	println( "ZM >> do_player_playvox called with = " + sound_to_play );
...
weapon_toggle_vox( alias, weapon )
{
    ...
    self PlaySoundWithNotify( sound_to_play + "_0" );
}
