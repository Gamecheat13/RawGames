...
*/
		...
		// buy the thing
//t6todo		bbPrint( "zombie_uses: playername %s playerscore %d teamscore %d round %d cost %d name %s x %f y %f z %f type door", who.playername, who.score, level.team_pool[ who.team_num ].score, level.round_number, self.zombie_cost, self.script_flag, self.origin );
	}
...
//			self UseTriggerRequireLookAt();
		...
		//t6todo - removed cost - self door_delay( cost );
			self door_delay();
			break;
...
//t6todo	if( is_player_valid( who ) || who maps\_laststand::player_is_in_laststand())
//t6todo	{
//t6todo		self thread maps\_zombiemode_spawner::zombie_boardtear_offset_fx_horizontle( self, node );
//t6todo		level thread remove_chunk( self, node, true );
//t6todo		self update_states("destroyed");
//t6todo		self notify("destroyed");
//t6todo		self.unbroken = false;
...
//t6todo			bbPrint( "zombie_uses: playername %s playerscore %d teamscore %d round %d cost %d name %s x %f y %f z %f type repair", player.playername, player.score, level.team_pool[ player.team_num ].score, level.round_number, original_cost, self.target, self.origin );
			...
			{
				player maps\mp\zombies\_zm_score::player_add_points( "rebuild_board", cost );
...
//	self.type = undefined; 
	...
	if ( players.size > min_size )
	{
		...
		for(i=0;i<self.shutters.size;i++)
		{
			...
			{
				assert( IsDefined( self.shutters[i].script_animname ), "Blocker_init: You must specify a script_animname for "+self.shutters[i].targetname ); 
				assert( IsDefined( level.scr_anim[ self.shutters[i].script_animname ] ), "Blocker_init: You must define a level.scr_anim for script_anim -> "+self.shutters[i].script_animname ); 
				assert( IsDefined( level.blocker_anim_func ), "Blocker_init: You must define a level.blocker_anim_func" ); 
			}
...
//t6todo		shutter_lights[i].fx = maps\_zombiemode_net::network_safe_spawn( "trap_light_red", 2, "script_model", shutter_lights[i].origin );
...
//t6todo		shutter_lights[i].fx = maps\_zombiemode_net::network_safe_spawn( "trap_light_green", 2, "script_model", shutter_lights[i].origin );
		shutter_lights[i].fx setmodel("tag_origin");
		shutter_lights[i].fx.angles = shutter_lights[i].angles+(-90,0,0);
...
*/
	...
	//t6todo		bbPrint( "zombie_uses: playername %s playerscore %d teamscore %d round %d cost %d name %s x %f y %f z %f type door", who.playername, who.score, level.team_pool[ who.team_num ].score, level.round_number, self.zombie_cost, self.target, self.origin );
		}
