...
//t6todo #using_animtree( "generic_human" ); 
init()
{
...
//	array_thread(zombies, ::add_spawn_function, ::zombie_rise);
	array_thread(zombies, ::add_spawn_function, maps\mp\zombies\_zm::round_spawn_failsafe);
...
track_players_intersection_tracker()
{
	...
	while ( 1 )
	{
		...
		{
//t6todo	if ( players[i] maps\_laststand::player_is_in_laststand() || "spectator" == players[i].sessionstate )
			if ( "spectator" == players[i].sessionstate )			
			{
				continue;
			}
			...
			{
//t6todo		if ( i == j || players[j] maps\_laststand::player_is_in_laststand() || "spectator" == players[j].sessionstate )
				if ( i == j || "spectator" == players[j].sessionstate )
				{
					continue;
				}
...
tear_into_building()
{
	...
	//}
		...
		while( 1 )
		{
			...
			chunk maps\mp\zombies\_zm_blockers::update_states("target_by_zombie");
	...
	while( 1 )
	{
		...
		if( notetrack == "board" )
		{
			if( !chunk.destroyed )
			{
				...
				zomb = self;
				level thread maps\mp\zombies\_zm_blockers::remove_chunk( chunk, node, true, zomb );
...
//----------------------------------------------------					
		...
		else if( notetrack == "bar" )
		{
			if( !chunk.destroyed )
			{
				...
				level thread maps\mp\zombies\_zm_blockers::remove_chunk( chunk, node, true, self );
				chunk notify("destroyed");
...
//t6todo	if( !animscripts\utility::damageLocationIsAny( "head", "helmet", "neck" ) )
//t6todo	{
//t6todo		return false; 
//t6todo	}
...
headshot_blood_fx()
{
	...
	if(IsDefined( self ))
	{
//t6todo		if( self maps\_zombiemode_weap_tesla::enemy_killed_by_tesla() )
//t6todo		{
//t6todo			PlayFxOnTag( level._effect["tesla_head_light"], self, fxTag );
//t6todo		}
//t6todo	else
		{
...
//t6todo			if( self animscripts\utility::damageLocationIsAny( "head", "helmet", "neck" ) )
//t6todo			{
//t6todo				continue;
//t6todo			}
			...
			if( refs.size )
			{
//t6todo				self.a.gib_ref = animscripts\zombie_death::get_random( refs ); 
...
//t6todo				self thread animscripts\zombie_death::do_gib();
...
//t6todo	if ( self maps\_zombiemode_weap_freezegun::is_freezegun_damage( type ) )
//t6todo	{
//t6todo		return false; 
//t6todo	}
...
//t6todo			level thread maps\_zombiemode_powerups::powerup_drop( origin );
		}
		else
		{	
...
//t6todo			level thread maps\_zombiemode_powerups::powerup_drop( origin );
	...
	attacker maps\mp\zombies\_zm_score::player_add_points( event, mod, hit_location ); 
}
...
//t6todo	if( self maps\_zombiemode_weap_tesla::enemy_killed_by_tesla() || self maps\_zombiemode_weap_thundergun::enemy_killed_by_thundergun() )
//t6todo	{
//t6todo		return false;
//t6todo	}
//t6todo	if ( self maps\_zombiemode_weap_freezegun::should_do_freezegun_death( self.damagemod ) )
//t6todo	{
//t6todo		self thread maps\_zombiemode_weap_freezegun::freezegun_death( self.damagelocation, self.origin, self.attacker );

//t6todo		if ( "MOD_EXPLOSIVE" == self.damagemod )
	//t6todo	{
...
//t6todo				level thread maps\_zombiemode_powerups::powerup_drop( origin );
			...
			{	
				...
				origin = trace["position"];
//t6todo				level thread maps\_zombiemode_powerups::powerup_drop( self.origin );
			}	
		}
	}
	else
	...
	{
//t6todo		self thread animscripts\zombie_death::flame_death_fx();
	}
...
zombie_damage( mod, hit_location, hit_origin, player, amount )
{
	...
	else if( self zombie_flame_damage( mod, player ) )
	{
		...
		{
			player maps\mp\zombies\_zm_score::player_add_points( "damage", mod, hit_location, self.isdog );
		}
	}
//t6todo	else if( self maps\_zombiemode_weap_tesla::is_tesla_damage( mod ) )
//t6todo	{
//t6todo		self maps\_zombiemode_weap_tesla::tesla_damage_init( hit_location, hit_origin, player );
//t6todo		return;
//t6todo	}
	else
	{
//t6todo		if ( self maps\_zombiemode_weap_freezegun::is_freezegun_damage( self.damagemod ) )
//t6todo		{
//t6todo			self thread maps\_zombiemode_weap_freezegun::freezegun_damage_response( player, amount );
//t6todo		}
...
zombie_damage_ads( mod, hit_location, hit_origin, player, amount )
{
	...
	else if( self zombie_flame_damage( mod, player ) )
	{
		if( self zombie_give_flame_damage_points() )
		{
			player maps\mp\zombies\_zm_score::player_add_points( "damage_ads", mod, hit_location );
		}
	}
//t6todo	else if( self maps\_zombiemode_weap_tesla::is_tesla_damage( mod ) )
//t6todo	{
//t6todo		self maps\mp\zombies\_zm_weap_tesla::tesla_damage_init( hit_location, hit_origin, player );
//t6todo		return;
//t6todo	}
//t6todo	else
	{
...
//t6todo	self thread maps\_zombiemode_powerups::check_for_instakill( player, mod, hit_location );
}
...
//t6todo	if ( !zombie maps\_zombiemode_weap_freezegun::should_do_freezegun_death( zombie.damagemod ) )
//t6todo	{
//t6todo		zombie thread maps\mp\zombies\_zm_audio::do_zombies_playvocals( "death", zombie.animname );
//t6todo		zombie thread zombie_eye_glow_stop();
//t6todo	}

//t6todo	if ( maps\_zombiemode_weapons::is_weapon_included( "freezegun_zm" ) )
//t6todo	{
//t6todo		zombie thread maps\_zombiemode_weap_freezegun::freezegun_clear_extremity_damage_fx();
//t6todo		zombie thread maps\_zombiemode_weap_freezegun::freezegun_clear_torso_damage_fx();
//t6todo	}
	...
	//t6todo	bbPrint( "zombie_kills: round %d zombietype zombie damagetype %s damagelocation %s playername %s playerweapon %s playerx %f playery %f playerz %f zombiex %f zombiey %f zombiez %f",
	//t6todo			level.round_number, damagemod, damageloc, attacker.playername, weapon, attacker.origin, zombie.origin );
	}
	else
	{
...
zombie_pathing()
{
	...
	/#
//t6todo	self animscripts\debug::debugPushState( "zombie_pathing" );
	#/
	...
	/#
//t6todo	self animscripts\debug::debugPopState();
	#/
}
...
do_zombie_rise()
{
	...
	// face goal
	target_org = maps\mp\zombies\_zm_spawner::get_desired_origin();
...
//t6todo	self animscripts\zombie_shared::DoNoteTracks("rise", ::handle_rise_notetracks, undefined, spot);
...
zombie_disintegrate( player )
{
	...
	if ( self.health <= 0 )
	{
		player maps\mp\zombies\_zm_score::player_add_points( "death", "", "", self.isdog );
...
//t6todo		self thread animscripts\zombie_death::do_gib();
	...
	{
//t6todo		self.thundergun_handle_pain_notetracks = maps\_zombiemode_weap_thundergun::handle_thundergun_pain_notetracks;
