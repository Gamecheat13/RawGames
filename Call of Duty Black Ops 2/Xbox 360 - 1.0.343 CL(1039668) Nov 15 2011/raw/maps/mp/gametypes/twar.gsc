...
main()
{
	...
	maps\mp\gametypes\_globallogic_utils::registerRoundLimitDvar( level.gameType, 2, 0, 2 );
	...
	maps\mp\gametypes\_globallogic_utils::registerScoreLimitDvar( level.gameType, 5, 0, 5 );
...
onPrecacheGameType()
{
	...
	if ( game["allies"] == "marines" )
		game["flagmodels"]["allies"] = "mp_flag_a";
	else
		game["flagmodels"]["allies"] = "mp_flag_b";
	
	if ( game["axis"] == "german" ) 
		game["flagmodels"]["axis"] = "mp_flag_c"; 
	else
		game["flagmodels"]["axis"] = "mp_flag_d";
...
getTimeLimitDvarValue()
{
	...
	{
		return maps\mp\gametypes\_globallogic_utils::getValueInRange( GetDvarfloat( "twar_finalFightTimeLimit" ), 0, 1440 );
	}
...
onStartGameType()
{
	...
	maps\mp\gametypes\_globallogic_ui::setObjectiveText( "allies", &"OBJECTIVES_TWAR" );
	maps\mp\gametypes\_globallogic_ui::setObjectiveText( "axis", &"OBJECTIVES_TWAR" );

	if ( level.splitscreen )
	{
		maps\mp\gametypes\_globallogic_ui::setObjectiveScoreText( "allies", &"OBJECTIVES_TWAR" );
		maps\mp\gametypes\_globallogic_ui::setObjectiveScoreText( "axis", &"OBJECTIVES_TWAR" );
	}
	else
	{
		maps\mp\gametypes\_globallogic_ui::setObjectiveScoreText( "allies", &"OBJECTIVES_TWAR_SCORE" );
		maps\mp\gametypes\_globallogic_ui::setObjectiveScoreText( "axis", &"OBJECTIVES_TWAR_SCORE" );
	}
	maps\mp\gametypes\_globallogic_ui::setObjectiveHintText( "allies", &"OBJECTIVES_TWAR_HINT" );
	maps\mp\gametypes\_globallogic_ui::setObjectiveHintText( "axis", &"OBJECTIVES_TWAR_HINT" );
	...
	flagBaseFX["german"] = "misc/fx_ui_flagbase_gold";
	flagBaseFX["russian"] = "misc/fx_ui_flagbase_orange";
	flagBaseFX["neutral"] = "misc/fx_ui_flagbase_silver";
...
showClientGenericMessageBar( time, message )
{
	...
	{
		self SetClientUIGenericStatusBar( 1 );
		self SetClientGenericStatusBarMessage( message );
		wait ( time );
		self SetClientUIGenericStatusBar( 0 );
	}
	else
	{
		self SetClientUIGenericStatusBar( 0 );
...
onTimeLimit()
{
	...
	for ( index = 0; index < level.players.size; index++)
	{
		...
		if ( IsDefined( player ) )
			player thread showClientGenericMessageBar( undefined, undefined );
	}
...
twar_sendWarSBToAllPlayers( value, inc_count, time_inc )
{
	...
	for ( i = 0; i < level.players.size; i++ )
	{
		...
		{
			level.players[i] thread twar_setClientWarSB( value, current_time_value );
		}
...
twar_setClientWarSB( value, time )
{
	...
	self SetClientWarSB( value );
}


twar_setClientWarFlag( dvar, value, time )
{
	if ( isdefined(time ) && time != 0 )
	{
		self endon("disconnect");
	...
	if( dvar == "war_a" )
		SetClientWarFlag( 0, value );
	else if( dvar == "war_b" )
		SetClientWarFlag( 1, value );
	else if( dvar == "war_c" )
		SetClientWarFlag( 2, value );
	else if( dvar == "war_d" )
		SetClientWarFlag( 3, value );
	else if( dvar == "war_e" )
		SetClientWarFlag( 4, value );
}
...
sendFlagCallsignDvar( label, dvar, time )
{
	...
	self thread twar_setClientWarFlag( dvar, getDvar( dvar ), time );	// flag callsign
}
...
onPlayerConnect()
{
	for(;;)
	{
		...
		if ( isDefined( player ) )
		{
			...
			player thread twar_setClientWarSB( GetDvar( "war_sb" ) );  // score bar
			...
			previousMultiplier != game["war_momentum"][team + "_multiplier"] )
		{
			...
			if ( previousMultiplier < game["war_momentum"][team + "_multiplier"] )
			{
				...
				if ( game["war_momentum"][team + "_multiplier"] == momentumMaxMultiplier )
				{
					...
					maps\mp\gametypes\_globallogic_audio::leaderDialog( "blitz_friendly", team );
				...
				else
				{
					...
					maps\mp\gametypes\_globallogic_audio::leaderDialog( "momentum_friendly", team );
				}
			}
...
updateFlagStatusHudDvars( flag )
{
	...
	if ( !isDefined( flag ) || flag.claimTeam == "none" || ( flag.numTouching[flag.claimTeam] == 0 && flag.numTouching[otherTeam] == 0 ) )
	{
		...
		{
			SetDvar( "war_sb", "" );
			twar_sendWarSBToAllPlayers( "", 6, 0.1 );
		}
	}
	else
	{
		...
		if ( GetDvar( "war_sb" ) != dvarVal )
		{
			...
			twar_sendWarSBToAllPlayers( dvarVal, 6, 0.1 );
		}
	}
}	
...
onUseUpdate( team, progress, change )
{
	...
	{
		//removing old music call 2/09/10
...
flagNeutralTimer()
{
	...
	if ( time && level.flagSetupComplete )
	{
		for ( index = 0; index < level.players.size; index++)
		{
			...
			if ( IsDefined( player ) )
			{
				player thread showClientGenericMessageBar( time, "@MENU_WAR_STATUS_NEUTRAL_COOK" );
			}
		}
...
onUse( player )
{
	...
	if ( oldTeam == "neutral" )
	{
		...
		if( !level.inFinalFight )
		{
			...
			{
				thread playSoundOnPlayers( "mus_war_captured"+"_"+level.teamPostfix[team] );
	...
	score)
{
	...
	if (IsDefined(contested_flag))
	{
		...
		for (influencer_index= 0; influencer_index<placed_influencers.size; influencer_index++)
		{
			...
			distance_squared= distanceSquared(flag_origin, influencer_origin);
...
twar_gamemodeSpawnDvars()
{
	...
	ss.twar_contested_flag_influencer_radius =	set_dvar_float_if_unset("scr_spawn_twar_contested_flag_influencer_radius", "" + 15.0*get_player_height());
}
