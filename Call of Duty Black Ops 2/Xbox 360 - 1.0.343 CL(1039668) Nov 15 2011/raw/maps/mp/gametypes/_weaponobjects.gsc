...
init()
{
	...
	level.retrievableWeapons[ level.retrievableWeapons.size ] = "riotshield_mp";
...
setUpRetrievableHintStrings()
{
	...
	createRetrievableHint("riotshield", &"MP_RIOTSHIELD_PICKUP");
...
onPlayerSpawned() // self == player
{
	...
	for(;;)
	{
		...
		self maps\mp\_riotshield::createRiotShieldWatcher();
...
// This is when the object gets EMPed
waitAndDestroy( object )
{
	object endon("death");

	// playFx
	object thread weaponEMP();
...
weaponEMP() // self == weapon object
{
	...
	self SetClientFlag( level.const_flag_emp );
	
	self.emp_fx = Spawn( "script_model", origin );
	self.emp_fx SetModel( "tag_origin" );
	self thread empFxThink( self.emp_fx );
	wait ( 0.1 );

	PlayFXOnTag( level._equipment_emp_destroy_fx, self.emp_fx, "tag_origin" );
}
...
weaponObjectDamage(watcher) // self == weapon object
{
	...
	if ( weaponName == "emp_grenade_mp" )
		watcher thread waitAndDestroy( self );
	else
...
watchRiotShieldUse() // self == player
{
	...
	for ( ;; )
	{
		...
		// tagTMR<TODO>: remove riotshield from weaponobjects entirely
		//if ( IsDefined( shield_ent ))
		//{
		//	watcher = getWeaponObjectWatcherByWeapon( weaponName );
		//	if ( IsDefined( watcher ))
		//	{
		//		self addWeaponObject(watcher, shield_ent);
		//	}
		//}
	}
}
...
proximityWeaponObjectDetonation( watcher )
{
	...
	{
		self SetClientFlag( level.const_flag_proximity );
	}
...
watchHatchetTrigger( trigger, callback, playerSoundOnUse, npcSoundOnUse ) // self == weapon (for example: the claymore)
{
	...
	while ( true )
	{
		...
		else if ( player.grenadeTypeSecondary == "hatchet_mp" )
		{
...
{	
	...
	{		
		...
		if( level.players[i] HasPerk( "specialty_showenemyequipment" ) )
		{
			...
			case "t6_wpn_grenade_proximity_world_detect":
				hasReconModel = true;
				break;
