...
initScoreInfo()
{
		tableName = "mp/scoreInfo.csv";
		
		// first find the correct column for the gamemode
		gameModeColumn = 1;
		foundGameMode = false;
		...
		for ( ; gameModeColumn < 27; gameModeColumn += 2 )
		{
			if ( TableLookupColumnForRow( tableName, 0, gameModeColumn ) == level.gameType )
			{
				foundGameMode = true;
				break;
			}
		...
		for( row = 1; row < maxScoreInfos; row++ )
		{
			type = tableLookupColumnForRow( tableName, row, 0 );
			if ( type != "" )
			{
				labelString = tableLookupColumnForRow( tableName, row, 27 );
				label = undefined;
				if ( labelString != "" ) 
				{
					label = tableLookupIString( tableName, 0, type, 27 );
				}
				scoreValue = int( tableLookupColumnForRow( tableName, row, gameModeColumn ) );
				registerScoreInfo( type, scoreValue, label );
				
				if ( maps\mp\_utility::getRoundsPlayed() == 0 )
				{
					xpValue = float( tableLookupColumnForRow( tableName, row, gameModeColumn + 1 ) );
					
					doubleXPString = tableLookupColumnForRow( tableName, row, 29 );
				...
				rampageString = tableLookupColumnForRow( tableName, row, 28 );
				if ( rampageString == "TRUE" )
				{
					level.scoreInfo[ type ][ "rampage" ] = true;
				}
			}
		}
}
...
CodeCallback_RankUp( rank, prestige )
{
	updateRankAnnounceHud_internal( rank, prestige );
}
	...
	for ( tierNum = 1; tierNum <= level.numStatsMilestoneTiers; tierNum++ )
	{
		...
		for( idx = 0; idx < level.maxStatChallenges; idx++ )
		{
			...
			if ( row > -1 )
			{
				...
				level.challengeTable[ idx ] = [];
				level.challengeTable[ idx ][ "level" ] = currentLevel;
				level.challengeTable[ idx ][ "stattype" ] = statType;
				level.challengeTable[ idx ][ "maxval" ] = int( tableLookupColumnForRow( tableName, row, 2 ) );
				level.challengeTable[ idx ][ "xpreward" ] = int( tableLookupColumnForRow( tableName, row, 6 ) );
				level.challengeTable[ idx ][ "cpreward" ] = int( tableLookupColumnForRow( tableName, row, 7 ) );
				level.challengeTable[ idx ][ "unlockitem" ] = tableLookupColumnForRow( tableName, row, 9 );
			}
		}
	}
}
