...
createRaceObjective( name, origin, client, shader, offscreenShader, alpha, scale )
{
	...
	if ( isDefined( client ) )
		objPoint = newClientHudElem( client );
	else
		objPoint = newHudElem();
	...
	if ( isDefined( offscreenShader ) )
		objPoint setWayPoint( false, offscreenShader );
	else
