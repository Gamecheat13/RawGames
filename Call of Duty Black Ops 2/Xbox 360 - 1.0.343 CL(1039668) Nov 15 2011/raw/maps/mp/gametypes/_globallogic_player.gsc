...
//	self thread maps\mp\gametypes\_globallogic_utils::fakeLag();
	...
	{
		self AddPlayerStat( "MATCHES_PLAYED", 1 );
		self.pers["MATCHES_PLAYED_COMPLETED_STREAK"] = self GetDStat( "playerstatslist", "MATCHES_PLAYED_COMPLETED_STREAK", "StatValue" ) + 1;
		self SetDStat( "playerstatslist", "MATCHES_PLAYED_COMPLETED_STREAK", "StatValue", 0 );
		
		if ( !IsDefined( self.pers["matchesHostedStatsTracked"] ) && self IsLocalToHost() )
		{
			self AddPlayerStat( "MATCHES_HOSTED", 1 );
			self.pers["MATCHES_HOSTED_COMPLETED_STREAK"] = self GetDStat( "playerstatslist", "MATCHES_HOSTED_COMPLETED_STREAK", "StatValue" ) + 1;
			self SetDStat( "playerstatslist", "MATCHES_HOSTED_COMPLETED_STREAK", "StatValue", 0 );
	...
	self SetClientPlayerSprintTime( GetDvar( "scr_player_sprinttime" ) );
	...
	self  maps\mp\gametypes\_globallogic_score::initPersStat( "momentum" );
	...
	if( SessionModeIsZombiesGame() )
	{
		//Custom zombie pers stats
		self maps\mp\gametypes\_globallogic_score::initPersStat( "zscore", false );
		self maps\mp\gametypes\_globallogic_score::initPersStat( "zkills", false );
		self maps\mp\gametypes\_globallogic_score::initPersStat( "zteam", false );
		self.pers["zscore"] = 500;
		self.pers["zkills"] = 0;
		self.pers["zteam"] = 0;
	}
...
Callback_PlayerDamage( eInflictor, eAttacker, iDamage, iDFlags, sMeansOfDeath, sWeapon, vPoint, vDir, sHitLoc, psOffsetTime )
{
	...
	self maps\mp\bots\_bot::bot_damage_callback( eAttacker, iDamage, sMeansOfDeath, sWeapon, eInflictor, sHitLoc );
	...
	attackerIsHittingTeammate = level.teamBased && isPlayer( eAttacker ) && ( self != eAttacker ) && isDefined( eAttacker.team ) && ( self.pers[ "team" ] == eAttacker.team );
	...
	if ( sHitLoc == "riotshield" )
	{
		...
		if ( sMeansOfDeath == "MOD_PISTOL_BULLET" || sMeansOfDeath == "MOD_RIFLE_BULLET" && !attackerIsHittingTeammate )
		{
			...
			{
				score_event = "shield_damage_blocked";

				if (( self.shieldDamageBlocked > 2000 /*riotshield_damage_score_max*/ ))
				{
					score_event = "shield_damage_blocked_nerf";	
				}

				thread maps\mp\gametypes\_globallogic_score::givePlayerScore( score_event, self );
			}
		}
...
//				pixmarker( "END: Callback_PlayerDamage player" );
		...
		{			
			if ( doDamageFeedback( sWeapon, eInflictor ) )
			{
				...
				if ( iDamage > 0 )
				{
					eAttacker thread maps\mp\gametypes\_damagefeedback::updateDamageFeedback( hasBodyArmor, sMeansOfDeath, iDamage );
				}
			}
		}
...
{	
	...
	if ( isdefined(self.cac_body_type) && self.cac_body_type == "body_armor_mp" )
	{
		surface = "metal";
	}
...
GiveCustomGameModePlayerKilledScore( attacker, sMeansOfDeath )
{
	...
	if( isDefined( level.placement ) )
	{
		...
		{
			leaderbonus = int( GetDvar( "scr_" + level.gameType + "_bonus_leader" ) );
...
#/
	...
	maps\mp\gametypes\_globallogic_score::givePlayerScore( "death", self, self, sWeapon );
	...
	if( isPlayer( attacker ) )
	{
		...
		if ( attacker == self ) // killed himself
		{
			...
			if ( isDefined( self.switching_teams ) )
			{
				if ( !level.teamBased && ((self.leaving_team == "allies" && self.joining_team == "axis") || (self.leaving_team == "axis" && self.joining_team == "allies")) )
				{
					...
					{
						thread maps\mp\gametypes\_globallogic_score::givePlayerScore( "suicide", self );
		...
		else
		{
			...
			{
				thread maps\mp\gametypes\_globallogic_score::givePlayerScore( "teamkill", attacker );
			...
			else
			{
				...
				if ( IsDefined( killstreak ) )
				{
					...
					{
						maps\mp\gametypes\_globallogic_score::givePlayerScore( killstreak, attacker, self, sWeapon );
					}
					else
					{
						maps\mp\gametypes\_globallogic_score::givePlayerScore( "reward_kill", attacker, self, sWeapon );
					}
				}
				else
				{
					maps\mp\gametypes\_globallogic_score::givePlayerScore( "kill", attacker, self, sWeapon );
					if ( sMeansOfDeath == "MOD_HEAD_SHOT" )
					{
						maps\mp\gametypes\_globallogic_score::givePlayerScore( "headshot", attacker, self, sWeapon );
					}
				}
	...
	if( awardAssists )
	{
		...
		if ( isdefined( self.attackers ) )
		{
			for ( j = 0; j < self.attackers.size; j++ )
			{
				...
				player thread maps\mp\gametypes\_globallogic_score::processAssist( self, damage_done, sWeapon );
			}
		}
		
		if ( level.teamBased )
		{
			self maps\mp\gametypes\_globallogic_score::processKillstreakAssists( attacker );
		}
		
		if ( isDefined( self.lastAttackedShieldPlayer ) && isDefined( self.lastAttackedShieldTime ) && self.lastAttackedShieldPlayer != attacker )
		{
			if ( gettime() - self.lastAttackedShieldTime < 4000 /*GetDvarint("riotshield_assist_time")*/ )
			{
...
#/
	...
	//profilelog_endtiming( 8, "gs=" + game["state"] + " zom=" + SessionModeIsZombiesGame() );
