...
useKillstreakDogs(hardpointType)
{
	...
	self maps\mp\killstreaks\_killstreaks::playKillstreakStartDialog( "dogs_mp", team, true);
	...
	if ( self maps\mp\killstreaks\_killstreakrules::killstreakStart("dogs_mp", self.team) == false )
		return false;

	self thread dog_manager_spawn_dogs( team, otherTeam, ownerDeathCount );
...
dog_manager_spawn_dogs( team, enemyTeam, deathCount )
{
	...
	for ( i = 0; i < level.dog_count_max_at_once; i++ )
	{
		...
		wait ( randomfloatrange( level.spawnTimeWaitMin, level.spawnTimeWaitMax ) );
	}
...
dog_manager_dog_alive_tracker( team )
{
	...
	while (1)
	{
	 	...
	 	if ( alive_count == 0 )
	 	{
	 		...
	 		maps\mp\killstreaks\_killstreakrules::killstreakStop( "dogs_mp", team );
	 		return;
		}	 	
