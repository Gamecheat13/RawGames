...
{	
	...
	PrecacheItem( "mobilespawn_gun_mp" );
	...
	maps\mp\_treadfx::preloadtreadfx("helicopter_comlink_mp");
	maps\mp\_treadfx::preloadtreadfx("helicopter_gunner_mp");
...
useKillstreakHelicopter( hardpointType )
{
	...
	if ( hardpointType == "helicopter_comlink_mp" || hardpointType == "mobilespawn_mp" )
	{
		result = self selectHelicopterLocation( hardpointType );
		if ( !isdefined(result) || result == false )
			return false;
		
		if ( hardpointType == "mobilespawn_mp" )
		{
			validMobileSpawnLocations = GetEntArray( "mobilespawn_hover", "targetname" );
			if ( !IsDefined( validMobileSpawnLocations ) || validMobileSpawnLocations.size == 0 )
			{
				IPrintLnBold( "Need to add mobile spawn locations to the map\n" );
				return false;
			}
			IPrintLnBold( "NUMBER OF MOBILE SPAWN HOVERS: "+validMobileSpawnLocations.size+"\n" );
			
			self.helilocation = validMobileSpawnLocations[validMobileSpawnLocations.size-1].origin;
		}
	}
	...
	thread heli_think( self, startnode, self.team, missilesEnabled, protectLocation, hardpointType, armored );
...
announceHelicopterInbound(hardpointType)
{
	...
	if ( statAddType != "" )
	{
		self AddPlayerStat( statAddType, 1 );
...
init()
{
	...
	level.chopperComlinkFriendly = "veh_iw_air_hind";
	level.chopperComlinkEnemy = "veh_iw_air_hind";
	level.chopperRegular = "veh_iw_air_hind";
	...
	maps\mp\killstreaks\_killstreaks::registerKillstreak( "mobilespawn_mp", "mobilespawn_mp", "killstreak_mobilespawn", "mobilespawn_used", ::useKillstreakHelicopter, true );
	maps\mp\killstreaks\_killstreaks::registerKillstreakStrings( "mobilespawn_mp", &"KILLSTREAK_EARNED_MOBILESPAWN", &"KILLSTREAK_MOBILESPAWN_NOT_AVAILABLE", &"KILLSTREAK_MOBILESPAWN_INBOUND");
	maps\mp\killstreaks\_killstreaks::registerKillstreakDialog( "mobilespawn_mp", "mpl_killstreak_heli", "kls_attackheli_used", "","kls_attackheli_enemy", "", "kls_attackheli_ready" );
	maps\mp\killstreaks\_killstreaks::registerKillstreakDevDvar( "mobilespawn_mp", "scr_givemobilespawn");
	maps\mp\killstreaks\_killstreaks::registerKillstreakAltWeapon( "mobilespawn_mp", "mobilespawn_gun_mp" );
}
...
heli_think( owner, startnode, heli_team, missilesEnabled, protectLocation, hardpointType, armored )
{
	...
	else if ( hardpointType == "mobilespawn_mp" )
		chopper.defaultWeapon = "mobilespawn_gun_mp";
	else
	...
	if ( IsDefined(protectLocation) ) 
	{
		if ( hardpointType == "mobilespawn_mp" )
		{
			chopper thread heli_mobilespawn( protectLocation );
		}
		else
		{
...
heli_fire_chaff( missile, attacker )
{
	...
	if( isdefined(self.chaffcount) && self.chaffcount>0 )
	{
		...
		self.chaff_fx deleteAfterTime( 5.0 );
...
destroyHelicopter()
{
	...
	maps\mp\killstreaks\_killstreakrules::killstreakStop( self.hardpointType, team );
}
...
watchForEarlyLeave(hardpointtype)
{
	...
	self.heli heli_leave(hardpointtype);
}
