...
init()
{
	...
	level.microwave_turret_damage = 20;
	...
	level.auto_turret_settings["sentry"].stunFXTag = "TAG_aim";
	...
	level.auto_turret_settings["microwave"].hackerHintString = &"KILLSTREAK_TURRET_HACKING";
	...
	level.auto_turret_settings["microwave"].stunFX = loadfx("weapon/grenade/fx_spark_disabled_weapon_lg");
	level.auto_turret_settings["microwave"].loopSoundFX = "wpn_sgaurd_beam";
	level.auto_turret_settings["microwave"].stunFXTag = "TAG_arm_base";
	level.auto_turret_settings["microwave"].stunFXFrequencyMin = 0.1;
	level.auto_turret_settings["microwave"].stunFXFrequencyMax = 0.75;
	level.auto_turret_settings["microwave"].turretInitDelay = 3.0;
	level.auto_turret_settings["microwave"].timeout = 120.0;
	
	step = Int( level.microwave_radius / 4 );
	level.auto_turret_settings["microwave"].fx = [];
	level.auto_turret_settings["microwave"].fx[step * 1] = loadfx("weapon/silent_gaurdian/fx_sg_distortion_cone_a");
	level.auto_turret_settings["microwave"].fx[step * 2] = loadfx("weapon/silent_gaurdian/fx_sg_distortion_cone_b");
	level.auto_turret_settings["microwave"].fx[step * 3] = loadfx("weapon/silent_gaurdian/fx_sg_distortion_cone_c");
	level.auto_turret_settings["microwave"].fx[step * 4] = loadfx("weapon/silent_gaurdian/fx_sg_distortion_cone_d");
...
//		maps\mp\killstreaks\_killstreaks::setRemoveWeaponWhenUsed("autoturret_mp", true);
		...
		maps\mp\killstreaks\_killstreaks::registerKillstreakDialog("auto_tow_mp", "mpl_killstreak_auto_turret", "kls_sam_used", "","kls_sam_enemy", "", "kls_sam_ready");
		...
		maps\mp\killstreaks\_killstreaks::registerKillstreakDialog("tow_turret_drop_mp", "mpl_killstreak_auto_turret", "kls_sam_used", "","kls_sam_enemy", "", "kls_sam_ready");
		maps\mp\killstreaks\_killstreaks::registerKillstreakDevDvar("tow_turret_drop_mp", "scr_giveautotowturretdrop");


		// this causes the player to keep holding the weapon until the hardpoint is done
//		maps\mp\killstreaks\_killstreaks::setRemoveWeaponWhenUsed("autoturret_mp", true);
		maps\mp\killstreaks\_supplydrop::registerCrateType( "tow_turret_drop_mp", "killstreak", "auto_tow_mp", 1, &"KILLSTREAK_TOW_TURRET_CRATE", undefined, "MEDAL_SHARE_PACKAGE_TOW_TURRET", maps\mp\killstreaks\_supplydrop::giveCrateKillstreak );
...
//		maps\mp\killstreaks\_killstreaks::setRemoveWeaponWhenUsed("autoturret_mp", true);
	...
	level.turrets_headicon_offset["microwave"] = (0, 0, 70); 
...
updateTurretPlacement( turret )
{
  ...
  turret.canBePlaced = false;
	...
	while( 1 )
	{
		...
		//self SetPlacementHint( turret.canBePlaced );
...
watchDamage()
{
	...
	for( ;; )
	{
		...
		// Microwave turret's shield blocks all damage
		...
		if ( IsDefined( weaponName ) )
		{
			...
			case "flash_grenade_mp":
				...
				{
					//Temporarily removing the ability for Flash/Stun to affect turrets.- Leif
...
destroyTurret()
{
	...
	if( self.turretType == "sentry" )
		...
		maps\mp\killstreaks\_killstreakrules::killstreakStop( "microwaveturret_mp", self.team );
...
doesMicrowaveTurretAffectPlayer( player )
{
	if ( !IsPlayer( player ) )
		return false;
	...
	if ( !maps\mp\gametypes\_weaponobjects::friendlyFireCheck( self.owner, player, 0 ) )
		return false;
	
	if ( DistanceSquared( player.origin, self.origin ) > level.microwave_radius * level.microwave_radius )
		return false;
	
	entDirection = vectornormalize( player.origin - self.origin );
...
microwavePlayer( player )
{
	player endon( "disconnect" );
	
	player.beingMicrowaved = true;

	for ( ;; )
	{
		if ( !IsDefined( self ) || !self doesMicrowaveTurretAffectPlayer( player ) )
		{
			player.beingMicrowaved = false;
			if ( IsDefined( player.microwavePoisoning ) && player.microwavePoisoning )
			{
				player.microwavePoisoning = false;
			}
			return;
		}
		
		if ( player mayApplyScreenEffect() )
		{
			if ( !IsDefined( player.microwavePoisoning ) || !player.microwavePoisoning )
			{
				player.microwavePoisoning = true;
				player.microwaveEffect = 2;
			}
		}
		
		player PlayRumbleOnEntity( "rcbomb_slide" );
		player DoDamage(
			level.microwave_turret_damage, // iDamage Integer specifying the amount of damage done
		...
		player.microwaveEffect++;
		switch( player.microwaveEffect )
		{
			case 1:				  
				//player.radiationSound = "item_geigercouner_level2";
				//player playLoopSound( player.radiationSound );
				player ViewKick( 1, self.origin );
				break;
			case 3:				  
				player shellshock( "mp_radiation_low", 4 );
				...
				break;
			case 4:
				player shellshock( "mp_radiation_med", 5 );
				//player.radiationSound = "item_geigercouner_level3";
				//player stopLoopSound();
				//player playLoopSound( player.radiationSound );
				player ViewKick( 15, self.origin );
				break;
			case 6:
				player shellshock( "mp_radiation_high", 5 );
				//player.radiationSound = "item_geigercouner_level4";
				//player stopLoopSound();
				//player playLoopSound( player.radiationSound );
			...
			case 8:
				...
				//player stopLoopSound();
				//player playLoopSound( player.radiationSound );
				player ViewKick( 127, self.origin );
				player.microwaveEffect = 6;
...
turret_microwave_think( player )
{
	...
	for ( ;; )
	{
		...
		{
			self thread microwavePlayer( ent );
		}
	}
}
...
microwave_fx()
{
	...
	for ( i = 0; i < keys.size; i++ )
	{
		...
		{
			PlayFxOnTag( level.auto_turret_settings["microwave"].fx[ keys[i] ], self, "tag_flash" );
		}
	}
}
...
hackerThink( trigger, owner ) // self == turret entity
{
	...
	for ( ;; )
	{
		...
		{
			maps\mp\killstreaks\_killstreakrules::killstreakStop( "autoturret_mp", self.team );
			player maps\mp\killstreaks\_killstreakrules::killstreakStart( "autoturret_mp", player.team, true );
		}
		else if ( self.turretType == "tow" )
		{
			maps\mp\killstreaks\_killstreakrules::killstreakStop( "auto_tow_mp", self.team );
			player maps\mp\killstreaks\_killstreakrules::killstreakStart( "auto_tow_mp", player.team, true );
		}
		else if ( self.turretType == "microwave" )
		{
			maps\mp\killstreaks\_killstreakrules::killstreakStop( "microwaveturret_mp", self.team );
			player maps\mp\killstreaks\_killstreakrules::killstreakStart( "microwaveturret_mp", player.team, true );
		}
