...
/#
	set_dvar_if_unset( "scr_remoteUAVFlyTime", 60 );
#/

	maps\mp\killstreaks\_killstreaks::registerKillstreak( "remote_uav_mp", "killstreak_remote_uav_mp", "killstreak_remote_uav", "remote_uav_used", ::tryUseRemoteUAV );
	//maps\mp\killstreaks\_killstreaks::registerKillstreakAltWeapon( "remote_uav_mp", "killstreak_remote_uav_mp" );
	maps\mp\killstreaks\_killstreaks::registerKillstreakStrings( "remote_uav_mp", &"KILLSTREAK_EARNED_REMOTE_UAV", &"KILLSTREAK_REMOTE_UAV_NOT_AVAILABLE", &"KILLSTREAK_REMOTE_UAV_INBOUND" );
	maps\mp\killstreaks\_killstreaks::registerKillstreakDialog( "remote_uav_mp", "mpl_remote_uav_mp", "kls_remote_uav_used", "", "kls_remote_uav_enemy", "", "kls_remote_uav_ready" );
	maps\mp\killstreaks\_killstreaks::registerKillstreakDevDvar( "remote_uav_mp", "scr_giveuavremote" );
...
//	self giveWeapon( "killstreak_remote_uav_mp" );
...
createCarryRemoteUAV( streakName, owner )
{
	...
	carryRemoteUAV = spawnTurret( "misc_turret", pos, "auto_gun_turret_mp" );
	carryRemoteUAV.turretType = "sentry";
	carryRemoteUAV SetTurretType(carryRemoteUAV.turretType);
...
//	carryRemoteUAV makeTurretInoperable();
	...
	carryRemoteUAV SetOwner( carryRemoteUAV.owner );
	...
	{
		carryRemoteUAV.maxHeight = int(maps\mp\killstreaks\_airsupport::getMinimumFlyHeight());
...
//	self VisionSetNakedForPlayer( "black_bw", 0.0 );	
	...
	*/
	...
	if ( !self maps\mp\killstreaks\_killstreakrules::killstreakStart( "remote_missile_mp", self.team, false, true ) )
	{
		return false;
	}
...
{
	remoteUAV = spawnHelicopter( owner, origin, angles, "remote_uav_mp", "veh_t6_drone_t_hawk_mp" );	
	if ( !isDefined( remoteUAV ) )
		return undefined;
...
//	self RemoteControlVehicle( remoteUAV );	
	...
	self thread maps\mp\gametypes\_hud::fadeToBlackForXSec( 0, 0, 0, 1.0 );
//	self VisionSetNakedForPlayer( "black_bw", 0.0 );
//	if ( IsDefined( level.nukeDetonated ) )
...
//				remoteUAV.markedPlayers[id]["icon"] = entity maps\mp\_entityheadIcons::setHeadIcon( self, "veh_hud_target_marked", offset, 10, 10, false, 0.05, false, false, false, false );		
			...
			else if ( isDefined( remoteUAV.markedPlayers[id] ) && isDefined( remoteUAV.markedPlayers[id]["icon"] ) && isDefined( remoteUAV.markedPlayers[id]["icon"].shader ) && remoteUAV.markedPlayers[id]["icon"].shader != "veh_hud_target_marked" )
			{
				...
				remoteUAV.markedPlayers[id]["icon"] setWaypoint( false );				
			}
		}
		//	not marked yet
		else 
...
remoteUAV_unmarkRemovedPlayer( remoteUAV )
{
	...
	else 
	{
		if( isDefined( self.remoteUAVMarkedObjID01 ) )
			objective_delete( self.remoteUAVMarkedObjID01 );	
		if( isDefined( self.remoteUAVMarkedObjID02 ) )
			objective_delete( self.remoteUAVMarkedObjID02 );	
		if( isDefined( self.remoteUAVMarkedObjID03 ) )
			objective_delete( self.remoteUAVMarkedObjID03 );
	}
}
...
remoteUAV_leave_on_timeout()
{
	...
	flyTime = 60.0;
/#
	set_dvar_int_if_unset( "scr_remoteUAVFlyTime", flyTime );
	flyTime = GetDvarInt( "scr_remoteUAVFlyTime" );
#/	
...
Callback_VehicleDamage( inflictor, attacker, damage, iDFlags, meansOfDeath, weapon, point, dir, hitLoc, timeOffset, modelIndex, partName )
{
	...
	if( IsDefined( weapon ) )
	{
		...
		case "remote_missile_missile_mp":
...
//			thread teamPlayerCardSplash( "callout_destroyed_remote_uav", validAttacker );			
			validAttacker thread maps\mp\gametypes\_rank::giveRankXP( "kill", 50 );			
