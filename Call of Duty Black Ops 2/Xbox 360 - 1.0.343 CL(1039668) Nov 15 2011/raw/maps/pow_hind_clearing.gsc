...
start_non_demo()
{
	...
	maps\_vehicle::scripted_spawn(level.vehiclespawngroup + 31);
...
clearing_animation()
{
	...
	assert( IsDefined(align_struct), "Couldn't find the align node for the clearing animations" );
	...
	switch( self.animname )
	{
		...
		default:
			assert(false, "Didn't find a valid animname for this clearing spawner " + self.animname);
		break;
	}
...
delete_any_guys_in_tent(trigger_targetname)
{
	...
	array_thread(delete, maps\_utility::bloody_death);
}
