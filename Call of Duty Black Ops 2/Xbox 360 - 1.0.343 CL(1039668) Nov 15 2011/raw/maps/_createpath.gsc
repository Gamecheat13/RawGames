...
{
	// SCRIPTER_MOD
	// JesseS (3/20/2007): level.player replaces with players[0]
	players = get_players();
	
	flag_clear("path_Notviewing");
	level.pathmod setshader("psourcemodify",level.pathmodsize*2,level.pathmodsize);
	players[0] freezecontrols(true);
	players[0] setorigin( self.origin+(players[0].origin-players[0] geteye()) - VectorScale(anglestoforward(self.angles),3));
...
path_getvisible()
{
	...
	for(j=0;j<level.paths_selectid_list.size;j++)
	{
		...
		for(i=0;i<level.path_views[ident].size;i++)
		{
			...
			{
				newdistsq = distancesquared(players[0] geteye(),level.path_views[ident][i].origin);
				if(newdistsq < distsq)
				{
					distsq = newdistsq;
					index  = i;
...
path_viewwait(index)
{
	...
	while(1)
	{
		...
		if(distancesquared(FLAT_ORIGIN(self.origin),FLAT_ORIGIN(players[0].origin)) < 32*32)
		{
			wait .05;
			continue;
		}
		thread draw_arrow_time(self.origin,self.origin+VectorScale(anglestoforward(self.angles),arrowlength),(0,1,1),frametime);
...
plot_circle_star_fortime (radius,time,color)
{
	...
	while(gettime()<timer)
	{
		...
		{
			plotpoints[plotpoints.size] = self.origin+VectorScale(anglestoforward((angletoplayer+(rad,90,0))),radius);
			rad+=circleinc;
		}
...
path_trigger_create ( position, radius, ident)
{
	...
	view.origin = position;
	//TODO:  angle isn't coming from anywhere
	angle=(0,0,0);
	view.angles = angle;
	level.path_selectindex++;
	#/
