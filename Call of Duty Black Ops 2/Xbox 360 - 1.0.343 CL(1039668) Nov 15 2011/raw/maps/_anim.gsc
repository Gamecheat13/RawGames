...
anim_first_frame_on_guy( guy, scene, org, angles, animname_override )
{
	...
	//anim_ent.first_frame_time = GetTime();
...
anim_single_thread(guys, index, scene, org, angles, animname_override, aligned, n_lerp_time, tracker)
{
	...
	{
		guy notify("stop_loop"); // automatically stop any looping animations
		guy notify("stop_single"); // automatically stop any single animations
	}
...
#/
	...
	/*			
	if( animname  == "sniper" )
	println( "scene ", scene, " facial: ", level.scr_face[ animname ][ scene ] ); 
	println( facialanim );
	*/
	...
	if (IsDefined(animation))
	{
		...
		else	// ai and models use animscripted
		{
			if ( IS_TRUE( aligned ) )
			{
				...
				guy AnimScripted( anim_string, org, angles, animation, "normal", undefined, 1, blend_in, n_lerp_time );
			}
			else
			{
	...
	if ( IS_VEHICLE( self ) )
	{
		if ( GetSubStr( notetrack, 0, 4 ) == "fire" )
		{
			if ( GetSubStr( notetrack, 4, 9 ) == "start" )
			{
				...
				maps\_turret::fire_turret_for_time( -1,  n_weapon_index );				
			}
...
{
	guy endon( "death" );
	waittill_any_ents(self, ender, guy, ender);
	guy.loops--;
}
#/
...
anim_info_render_thread( guy, scene, org, angles, animname, ender, showBlends )
{
	...
	while(1)
	{
		...
		// render anim info
		recordEntText( "name: " + animname, guy, (1,1,0), "ScriptedAnim" );
		recordEntText( "scene: " + scene, guy, (1,1,0), "ScriptedAnim" );
