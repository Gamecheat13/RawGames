...
Radiant:	
	- create trigger multiple, give it targetname 'drone_axis' or 'drone_ally', depending
...
//	drones_assign_side_to_spawner( str_script_string_name, str_side )
...
drones_setup_spawner( is_trigger )
{
	...
	level thread drone_spawner_active( data );
...
drones_get_spawner( targetname, target )
{
	...
	if( IsDefined( target ) )
	{
		data.targeted = getstructarray( target, "targetname" );
	}
	assert( IsDefined(data.targeted) ); 
	assert( IsDefined(data.targeted[0]) ); 
...
drone_spawner_active( drones )
{
	...
	for( i=0; i<repeat_times; i++ )
	{
		...
		level thread drone_spawngroup( drones, drones.targeted, spawn_size, drones.team, 0 );
...
drones_respawner_created( drone_struct )
{
	...
	for( i=0; i<level.drones.drone_spawners.size; i++ )
	{
		...
		for( j=0; j<spawner.targeted.size; j++ )
		{
			if( spawner.targeted[j] == drone_struct )
			{
				trigger_alive = 1;
				break;
			}
...
{
	drone_structs = drone_spawner.targeted;
	...
	for( i=0; i<drone_structs.size; i++ )
	{
		...
		for( j=0; j<level.drones.respawners.size; j++ )
		{
			...
			{
				// Remove respawner
				level.drones_respawners = array_remove( level.drones.respawners, struct );
				return( 1 );
			}
		}
	}
...
drone_assign_weapon( team, start_struct, drones )
{
	...
	{
		self.weapon = start_struct.weaponinfo;
	}
	else if ( IsDefined( drones.weaponinfo ) )
	{
		self.weapon = drones.weaponinfo;
	}
	else
	{
		if( team == "allies" )
...
drone_think( firstNode, override_target_node )
{
	...
	self thread drone_done_running( firstNode );
}


/*=================================================================================
SELF: drone
PURPOSE: maybe play an idle when we're done running?
RETURNS: 
CREATOR: 
=================================================================================*/
drone_done_running( firstNode )
{
	self endon( "death" );
	self endon( "drone_death" );
	...
	{
		if ( IsDefined( firstNode.dr_animation ) )
		{
			if ( IsArray( level.drones.anims[ firstNode.dr_animation ] ) )
			{
				anim_idle = random( level.drones.anims[ firstNode.dr_animation ] );
			}
			else
			{
				anim_idle = level.drones.anims[ firstNode.dr_animation ];
			}
		}
	}
	...
	if ( IsDefined( anim_idle ) )
	{
		...
		{
			self AnimScripted( "drone_idle_anim", v_origin, v_angles, anim_idle ); 
			self waittillmatch( "drone_idle_anim", "end" ); 
		}
	}
	else
	{
		// Play a generic anim loop
		...
		while( IsDefined( self ) )
		{
			self AnimScripted( "drone_idle_anim", v_origin, v_angles, idleAnim[RandomInt( idleAnim.size )] ); 
			self waittillmatch( "drone_idle_anim", "end" ); 
		}
	}
}
...
drone_runChain( point_start )
{
	...
	while( IsDefined( self ) )
	{
		...
		else if( !IsDefined( point_start.targeted ) && !IsDefined( point_start.script_delete ) )
		{
			self.dontDelete = undefined; 
...
drone_delete( delayTime )
{
	...
	if( ( is_in_array( level.drones.team[self.team].array, self ) ) )
	{
		...
		switch( s_start.dr_event )
		{
			...
			default:
				...
				if ( IsDefined( level.drones.funcs[ event_params[0] ] ) )
				{
					...
					params = array_remove( event_params, event_params[0] );
					self [[ level.drones.funcs[ event_params[0] ] ]]( s_start, self.v_destination, params );
				}
				else
				{
...
drone_available_ragdoll( force_remove )
{
	...
	if( level.drones.ragdoll_bucket.size >= level.drones.max_ragdolls )
	{
		...
		else if( IsDefined(force_remove) ) //-- we have to remove one for this very important ragdoll
		{
			...
			level.drones.ragdoll_bucket = array_remove_index( level.drones.ragdoll_bucket, 0 );
			return true;
		}
...
/*====================================================================================
SELF: level
PURPOSE: self.targeted = array of drone start structs
...
pre_populate_drones( drones, spawn_min, spawn_max, team )
{
	...
	path_size = undefined;
	for( i=0; i<drones.targeted.size; i++ )
	{
		size = calc_drone_path_size( drones.targeted[i] );
	...
	while( dist < path_size )
	{
		...
		level thread drone_spawngroup( drones, drones.targeted, spawn_size, team, dist );
...
/*====================================================================================
SELF: 
PURPOSE: Assigns a group of spawners ( "allies", "axis", "neutral") to a drone spawner
RETURNS: 
CREATOR: MikeA
=====================================================================================*/
...
drones_assign_side_to_spawner( str_script_string_name, str_side )
{
	// Search through all the drone spawners for the spawner with our SCRIPT STRING name
	for( i=0; i<level.drones.drone_spawners.size; i++ )
	{
		s_drones = level.drones.drone_spawners[ i ];
		if( IsDefined(s_drones.script_string) )
		{
			// Is it our spawner?
			if( s_drones.script_string == str_script_string_name )
			{
				// Assign the ents of this side to the drone spawner
				if( str_side == "allies" )
				{
					a_side_spawners = level.drones.allies_spawners;
				}
				else
				{
					a_side_spawners = level.drones.axis_spawners;
				}
								
				for( num=0; num<a_side_spawners.size; num++ )
				{
					e_ent = a_side_spawners[ num ];
					if( !IsDefined(s_drones.unique_guys) )
					{
						s_drones.unique_guys = [];
					}
					s_drones.unique_guys[s_drones.unique_guys.size] = e_ent;
				}
			}
		}
	}
...
drone_add_spawner()
{
	...
	if( side == "AXIS" )
	{
		...
		level.drones.axis_spawners[ level.drones.axis_spawners.size ] = self;
	}
	else if( side == "ALLIES" )
	{
		// Don't add duplicate spawners with the same classname
		...
		level.drones.allies_spawners[ level.drones.allies_spawners.size ] = self;
	}
}
