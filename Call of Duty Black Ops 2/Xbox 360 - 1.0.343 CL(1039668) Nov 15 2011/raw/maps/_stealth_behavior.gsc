...
enemy_state_spotted()
{
	...
	if ( self._stealth.logic.dog )
	{
		// Alex Liu: changed level.player to nearest player
		self.favoriteenemy = get_closest_player( self.origin );
	}
	else if ( RandomInt( 100 ) > 25 )
	{
		// Alex Liu: changed level.player to nearest player
		self.favoriteenemy = get_closest_player( self.origin );// 75% chance that favorite enemy is the player
	}
...
{				
	...
	{
		// TODO (BB): add anims for proportioned skeleton
	}
...
{				
	...
	anime = undefined;
	if ( distancesquared( enemy.origin, self.origin ) < 256*256 )
	{
		if (is_skeleton("base"))
		{
...
// SUMEET - Expected AI behavior - AI will jog to half the distance from the lastknownspot
// and then start regular walk after that.
...
{	
	...
	// SUMEET_TODO - sets the regular patrol walk, why there is need to set it back again?
	...
	// SUMEET_TODO - why not wait or timeour here, similar to alerted once?	
	self waittill( "goal" );
...
{	
	if( !flag( "_stealth_getallnodes" ) )
	{
		...
		level delay_thread( .2, ::flag_clear, "_stealth_getallnodes" );
