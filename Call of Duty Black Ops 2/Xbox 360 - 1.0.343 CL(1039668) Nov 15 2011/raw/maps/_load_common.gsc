...
get_trigger_targs()
{
	...
	if( IsDefined( self.target ) )
	{
		...
		assert( orgs.size < 2, "Trigger at " + self.origin + " targets multiple script origins" ); 
		if( orgs.size == 1 )
		{
	...
	assert( IsDefined( target_origin ), self.targetname + " at " + self.origin + " has no target origin." ); 
...
{
	const DOT_DEFAULT = 0.78;
	
	if( IsDefined( trigger.script_dot ) )
	{
		n_dot_check = trigger.script_dot; 
	}
	else
	{
		n_dot_check = DOT_DEFAULT;
	}
	...
	else
	{
		if( !triggers.size )
		{
			assert( IsDefined( trigger.script_flag ) || IsDefined( trigger.script_noteworthy ), "Trigger_lookat at " + trigger.origin + " has no script_flag! The script_flag is used as a flag that gets set when the trigger is activated." ); 
		}
	}
	...
	for( ;; )
	{
		...
		assert( IsPlayer( other ), "trigger_lookat currently only supports looking from the player" ); 
...
trigger_CanSee( trigger )
{
	...
	else
	{
		if( !triggers.size )
		{
			assert( IsDefined( trigger.script_flag ) || IsDefined( trigger.script_noteworthy ), "Trigger_cansee at " + trigger.origin + " has no script_flag! The script_flag is used as a flag that gets set when the trigger is activated." ); 
		}
	}
	...
	for( ;; )
	{
		...
		trigger waittill( "trigger", other ); 
		assert( IsPlayer( other ), "trigger_cansee currently only supports looking from the player" ); 
