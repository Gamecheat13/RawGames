...
trigger_spawner_spawns_guys()
{
	...
	if ( IsSubStr( self.classname, "actor" ) )
	{
		...
		if ( IsDefined( self.script_drone ) )
		{
			...
			assert( IsDefined( level.drone_spawn_func ), "You need to put maps\_drones::init(); in your level script!" );
...
spawn_prethink()
{
	...
	// This is redundant, leaks script vars and should not be used (MikeA 3/1/11)
	if( IsDefined( self.script_drone ) )
	{
	...
	*/
	...
	self thread wait_for_spawned( deathflag_func );
}

wait_for_spawned( deathflag_func )
{
	self endon("death");
...
spawn_think( spawner )
{
	...
	if ( !GameModeIsMode( level.GAMEMODE_ZOMBIES ) && !IsDefined(self.name) && (self.type == "human"))
	{
...
//									IsSubStr( self.classname, "dp28" ); 
	...
	{
		assert( ( self.team == "allies" ) || ( self.team == "neutral" ), "Only use script_hero kvp on friendly AI, else use magic_bullet_shield directly." );
	...
	if ( self.team == "axis" )
	{
		...
		// TFLAME - 2/24/11 - This should only be needed in zombie mode, if at all.  There is no zombies on T6 for awhile so I'll just comment this out and see if its missed after we get zombies
	...
	/* GLocke: removed _spiderhole.gsc from common scripts, if you need this back talk to me.
	...
	for ( i = 0; i < target_names.size; i++)
	{
		...
		for ( i = 0; i < target_nodes.size; i++)
		{
			if ( IsNodeOccupied(target_nodes[i]) || is_true(target_nodes[i].node_claimed) ) 
			{
				...
				ARRAY_ADD(occupied_nodes, target_nodes[i]);
			}
			else
			{
				ARRAY_ADD(nodes, target_nodes[i]);
			}
		}
	...
	if( nodes.size == 0 )
	{
		while( nodes.size == 0 )
		{
			for ( i = 0; i < occupied_nodes.size; i++)
			{
				...
				{
					ARRAY_ADD(nodes, occupied_nodes[i]);
					break;
				}
			}
...
tanksquish()
{
	...
	{
		assert( level.noTankSquish, "level.noTankSquish must be true or undefined" );
		return;
	}
