...
// NOTE: There is currently a bug that causes this to never clear
	...
	level.spiderbot_speeds[level.WALK] = 23;
...
watch_for_jump() // self == horse
{
	...
	while( true )
	{
		if( self.driver JumpButtonPressed() && !self.in_air )
		{
			...
			wait( 0.2 ); 	// some small delay for landing shock absorbtion
			self.in_air = false;
		}
