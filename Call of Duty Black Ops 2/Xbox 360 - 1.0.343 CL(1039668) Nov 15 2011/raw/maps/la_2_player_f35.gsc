...
f35_health_regen_think()  // self = F35
{
	...
	if ( !IsDefined( level.n_last_damage_time ) )
	{
		level.n_last_damage_time = 0;	
	}
	
	if ( !IsDefined( level.f35_damage_shader ) )
	{
		level.f35_damage_shader = NewClientHudElem(level.player);
		level.f35_damage_shader.horzAlign = "fullscreen";
		level.f35_damage_shader.vertAlign = "fullscreen";
		level.f35_damage_shader SetShader( "compass_static", 640, 480 );
	...
	while ( true )
	{
		if ( self.health_regen.time_since_last_damaged > self.health_regen.time_before_regen )
		{
			...
			self waittill_either( "damage", "health_at_max" );
		}
            ...
            if ( b_has_target )
            {
	            ...
	            */
	        	...
	        	if ( b_do_damage )
	        	{
	        		...
	        		e_target DoDamage( n_damage, e_target.origin, level.player, level.player, "riflebullet" );
	        	}        
            }
...
{
	n_damage_closest = 125;
	n_distance_closest = 3000;
...
//	level.player.invulnerable = true; // keep invulnerability on after restart
...
//	level.player.invulnerable = undefined;
...
missile_incoming_watcher()
{
	...
	while ( true )
	{
		...
		//SOUND - Shawn J TODO - change to looping sound with conditional stoploopsound
...
setup_approach_points()
{
	...
	n_distance_behind = 1;  // distance behind the plane where enemy plane will approach
...
f35_collision_watcher()
{
	...
	n_earthquake_scale_major = 0.3;
	n_earthquake_scale_catastrophic = .4;
	...
	n_rumble_count_minor = 2;
	n_rumble_count_major = 2;
	n_rumble_count_catastrophic = 2;
	n_rumble_delay_minor = 0.5;
...
f35_death_blossom( target )
{
	...
	sort_vehicles = get_array_of_closest( v_origin, vehicles, excluded, 4, blossom_radius );
...
//			iDamage = 100;
	...
	//C. Ayers: Play Sounds on Bullet Impacts
...
f35_switch_modes()  // self = f35
{
	...
	if ( !flag( "dogfight_done" ) )
	{
		...
		level.player thread f35_tutorial( false, false, true, false, true );
	}
...
//	self SetSpeed( n_airspeed_max, n_acceleration, n_decceleration );	
	...
	*/
	...
	level.f35 thread _f35_jet_bink();	
}
...
//	self SetSpeed( n_airspeed_max, n_acceleration, n_decceleration );	
	...
	self thread _f35_jet_bink();
}

_f35_vtol_bink()
{
	Stop3DCinematic();
	level.f35 ent_flag_set( "playing_bink_now" );
	Start3DCinematic( "jet_to_vtol", !BINK_IS_LOOPING, BINK_IN_MEMORY );
	wait 2;
	level.f35 ent_flag_clear( "playing_bink_now" );
	Start3DCinematic( "f35_loop", BINK_IS_LOOPING, BINK_IN_MEMORY );
}
...
_f35_jet_bink()
{
	Stop3DCinematic();	
	level.f35 ent_flag_set( "playing_bink_now" );
	Start3DCinematic( "vtol_to_jet", !BINK_IS_LOOPING, BINK_IN_MEMORY );	
	wait 2;
	level.f35 ent_flag_clear( "playing_bink_now" );
	Start3DCinematic( "f35_loop", BINK_IS_LOOPING, BINK_IN_MEMORY );
}
...
{
	level.f35 ent_flag_set( "playing_bink_now" );
	Start3DCinematic( "f35_startup", !BINK_IS_LOOPING, BINK_IN_MEMORY );	
	wait 15;
	level.f35 ent_flag_set( "playing_bink_now" );
	Stop3DCinematic();
	Start3DCinematic( "f35_loop", BINK_IS_LOOPING, BINK_IN_MEMORY );		
...
f35_eject_bink()
{
	level.f35 ent_flag_set( "playing_bink_now" );
	Stop3DCinematic();
	Start3DCinematic( "ejection", BINK_IS_LOOPING, BINK_IN_MEMORY );
}

f35_try_to_play_damage_bink()  // self = f35
{
	if ( !self ent_flag( "playing_bink_now" ) )
	{
...
f35_damage_bink()
{
	//iprintlnbold( "playing " + str_movie_name + " bink" );
	self ent_flag_set( "playing_bink_now" );
	Stop3DCinematic();
	Start3DCinematic( "fa38_damage", !BINK_IS_LOOPING, BINK_IN_MEMORY, !BINK_START_PAUSED );
	//C. Ayers: Adding in Static Hits for this
	level.player playsound( "prj_bullet_impact_f35_static" );
	// this wait is here because GetCinematicTimeRemaining will return the f35_loop (or anything else playing) time instead of the damage bink, causing the screen to black out for the overlapped time (5 seconds normally)
	wait 0.1;  
	n_wait_time = GetCinematicTimeRemaining();  // convert to seconds (returns ms)
	wait n_wait_time;
	//iprintlnbold( "movie done" );
	//stop3DCinematic();
	self ent_flag_clear( "playing_bink_now" );
...
//		self SetHeliHeightLock( false );
	...
	thread _f35_vtol_bink();
}


#define PLANE_MODE 0
#define VTOL_MODE 1
#define N_HEIGHT_MESH_MAX_DIST 7600
#define N_HEIGHT_MESH_MIN_DIST 0
#define MIN_DOGFIGHT_FLY_HEIGHT_PLAYER 1200
	
_f35_set_vtol_mode_v2()
{
	self endon( "dogfight_done" );
	...
	while( true )
	{
		//-- TODO: fix flying forward and then switching to banking
		//-- if the control stick is pressed, then free-flight
		if ( level.player GetNormalizedMovement()[0] > 0.5 && b_turn_on_engines )
		{
			if(current_mode != PLANE_MODE)
			{
				if( self.origin[2] < MIN_DOGFIGHT_FLY_HEIGHT_PLAYER )
				{
					if( self _f35_flying_upwards())
					{
						self SetVehicleType( "plane_f35_player_vtol_fast" );
						self SetHeliHeightLock( false );
						current_mode = PLANE_MODE;
		...
		else if( current_mode != VTOL_MODE && self GetSpeedMPH() < level.f35.max_speed_vtol_dogfight + 20 )//-- if the control stick is only kind of pressed, then vtol mode
		{
		...
		else if( current_mode == PLANE_MODE && abs(level.player GetNormalizedMovement()[1]) >= 0.5 ) //-- I'm a plane, but i've been strafing for a while
		{
		    
		    while(abs(level.player GetNormalizedMovement()[1]) >= 0.5 && n_time_strafing < 2.0)
		    {
		    	//-- might want to slow down the max speed here
		    	n_time_strafing += 0.05;
		    	wait(0.05);
		    }
		    
		    if(n_time_strafing >= 2.0)
		    {
		    	self SetVehicleType( "plane_f35_player_vtol_dogfight" );
		...
		if( self GetSpeedMPH() > level.f35.max_speed_vtol_dogfight - 10 )
		{
			n_time_at_speed += 0.05;
			
			if( n_time_at_speed > 2.0 )
			{
				b_turn_on_engines = true;	
			}
		}
		else
		{
...
f35_tutorial( b_show_move_prompt, b_show_hover_prompt, b_show_weapon_prompt, b_show_ads_prompt, b_show_death_blossom_prompt )  // self = player
{	
	...
	if ( b_show_death_blossom_prompt )
	{
		...
		{
			self f35_tutorial_func( &"LA_2_HINT_DEATHBLOSSOM", undefined, ::f35_control_check_deathblossom );
		}
	}
...
f35_tutorial_func( str_message_1, str_message_2, func_button_check, str_flag )
{
...
{
	b_is_pressing_button = ( ( level.player AttackButtonPressed() ) || ( level.player AdsButtonPressed() ) );
