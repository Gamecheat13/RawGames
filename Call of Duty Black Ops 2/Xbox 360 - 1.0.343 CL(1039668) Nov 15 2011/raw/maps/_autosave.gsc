...
start_level_save()
{
	...
	//if( call_overloaded_func( "maps\_arcademode", "arcademode_complete" ) )
	//{
	//	return; 
	//}
...
//	if( GetDvar( "credits" ) != 1 )  // no start game autosave on credits map.  Res
	...
	//level thread call_overloaded_func( "maps\_arcademode", "arcademode_checkpoint_save" );
}
...
#/ 
		...
		{
			if( doPickyChecks || enemies[i] canShootEnemy() )
			{
/#
...
#/ 
		...
		if( enemies[i].a.alertness == "aiming" && enemies[i] canShootEnemy() )
		{
/#
