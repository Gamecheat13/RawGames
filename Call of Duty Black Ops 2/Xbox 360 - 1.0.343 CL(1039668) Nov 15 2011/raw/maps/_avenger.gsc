...
precache_extra_models()
{
	...
	self.m_objective_model = "veh_t6_drone_avenger_objective";
...
set_deathmodel( v_point, v_dir )
{
	...
	for ( i = 0; i < a_models.size; i++ )
	{
		...
		Assert( b_is_model_in_memory, str_model + " xmodel is not loaded in memory. Include vehicle_avenger in your level CSV!" );
	...
	if ( IsDefined( self.deathmodel_pieces ) )
	{
		...
		for ( i = 0; i < num_pieces; i++ )
		{
			...
			self.deathmodel_pieces[i] PhysicsLaunch( v_point, ( vel_dir * 2500 ) + ( 0, 0, 100 ) );
			self.deathmodel_pieces[i].b_launched = true;
			PlayFxOnTag( self.fx_crash_effects["fire_trail_lg"], self.deathmodel_pieces[i], "tag_origin" );			
		}
	}
}
