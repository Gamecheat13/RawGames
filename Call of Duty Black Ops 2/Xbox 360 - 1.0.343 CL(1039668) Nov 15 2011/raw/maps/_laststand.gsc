...
{
	if( !GameModeIsMode( level.GAMEMODE_ZOMBIES ) && sMeansOfDeath == "MOD_CRUSH" ) // if getting run over by a tank then kill the guy even if in laststand
	{
	...
	// last stand only if there's lives. ideally this shouldn't get here it all, but it's all in code. will deal with it later.
	if( GameModeIsMode( level.GAMEMODE_WAR ) && get_lives_remaining() <= 0 )
	{
		MissionFailed();
		return;
	}
	...
	// AI
	if ( !GameModeIsMode( level.GAMEMODE_WAR ) )
	{
		self.ignoreme = true;
	}
...
say_revived_vo()
{
	if( GameModeIsMode( level.GAMEMODE_ZOMBIES ) || IsSubStr( level.script, "nazi_zombie_" ) ) // CODER_MOD (Austin 5/4/08): zombiemode loadout setup
	{	
		players = get_players();
		for(i=0; i<players.size; i++)
		{
