...
setup_objectives()
{
	...
	level.OBJ_GET_RUSSIAN		= register_objective( &"POW_OBJ_GET_RUSSIAN" );
	level.OBJ_TUNNELS				= register_objective( &"POW_OBJ_TUNNELS" );
	level.OBJ_HIND					= register_objective( &"POW_OBJ_HIND" );
	level.OBJ_GET_IN				= register_objective( &"POW_OBJ_GET_IN" );
	level.OBJ_FLY_KRAV			= register_objective( &"POW_POW_OBJ_FLY_KRAV" );
	level.OBJ_NVA_DEF				= register_objective( &"POW_OBJ_NVA_DEF" );
	level.OBJ_HIP						= register_objective( &"POW_OBJ_HIP" );
	level.OBJ_SAM				  	= register_objective( &"POW_OBJ_SAM" );
	level.OBJ_NVA_DEF2			= register_objective( &"POW_OBJ_NVA_DEF" );
	level.OBJ_HINDS					= register_objective( &"POW_OBJ_HINDS" );
	level.OBJ_KILL_KRAV			= register_objective( &"POW_OBJ_KILL_KRAV" );
	level.OBJ_KILL_KRAV2		= register_objective( &"POW_OBJ_KILL_KRAV" );
	level.OBJ_POWS					= register_objective( &"POW_OBJ_POWS" );
}
...
pow_objectives( jumpto_objective )
{
	...
	*/
	...
	get_players()[0] SetClientDvar("cg_objectiveIndicatorFarFadeDist", 80000);
	...
	switch (level.skipto_point)
	{
		...
		case "tunnel_start":
		{
			...
			set_objective( level.OBJ_GET_RUSSIAN );
			pow_objectives_tunnel_breadcrumb( level.OBJ_GET_RUSSIAN );
			
			
			flag_wait("obj_russian_start");
			level notify("stop_breadcrumb_obj"); //-- kill the breadcrumb system
			obj_ent = Spawn("script_model", (0,0,0));
			obj_ent SetModel("tag_origin");
			if (isdefined(level.russian))
			{
				obj_ent.origin = level.russian.origin + (0,0,70);
				obj_ent LinkTo(level.russian);
			}
			set_objective( level.OBJ_GET_RUSSIAN, obj_ent, "shoot" );
			
			
			flag_wait("spetz_hit"); //-- turn off the objective when the russian gets shot
			set_objective( level.OBJ_GET_RUSSIAN, obj_ent, "remove" );
			set_objective( level.OBJ_GET_RUSSIAN, undefined, "done" );
			
			
			flag_wait( "obj_boost_woods" );
			obj_boostup = getstruct("obj_boost_woods_struct");
			set_objective( level.OBJ_TUNNELS, obj_boostup.origin, "" );
			
			
			flag_wait( "obj_boost_woods_complete" );
			set_objective( level.OBJ_TUNNELS, undefined, "done" );
			set_objective( level.OBJ_TUNNELS, undefined, "delete" );
		}
		
		case "hind_clearing":
		{
			flag_wait( "obj_kill_hind_crew" );
			set_objective( level.OBJ_HIND );
		}
		
		case "start_in_hind":
		{	
			flag_wait( "obj_take_hind" );
			set_objective( level.OBJ_HIND, undefined, "done" );
			obj_origin = level.hind GetTagOrigin("tag_driver"); //-- Specific location to enter the Hind Helicopter
			set_objective( level.OBJ_GET_IN, obj_origin, "" );
			
			
			flag_wait( "obj_player_in_hind" );
			set_objective( level.OBJ_GET_IN, undefined, "done" );
			set_objective( level.OBJ_GET_IN, undefined, "delete" );
			
			
			flag_wait( "obj_fly_to_base" );
				//-- Setup the objective to fly to The Bear's base.  Does not clear until you reach his base, and there are objectives in between
			level thread pow_objectives_flying_breadcrumb( level.OBJ_FLY_KRAV );
			...
			if(IsDefined(level.pipe_tank_destroyed) && !level.pipe_tank_destroyed)
			{
				...
				set_objective( level.OBJ_NVA_DEF, large_tank.origin, "" );
				set_objective( level.OBJ_FLY_KRAV, undefined, "remove" );
			
				flag_wait_any( "obj_truck_depot_complete", "obj_truck_depot_passed");
				if(flag("obj_truck_depot_complete"))
				{
					set_objective( level.OBJ_NVA_DEF, undefined, "done" );
				}
		
				set_objective( level.OBJ_NVA_DEF, undefined, "delete" );
				set_objective( level.OBJ_FLY_KRAV, undefined, "breadcrumb" );
			}
			...
			set_objective( level.OBJ_HIP, enemy_hip, "" );
			set_objective( level.OBJ_FLY_KRAV, undefined, "remove" );
			...
			{
				set_objective( level.OBJ_HIP, undefined, "done" );
			}
			set_objective( level.OBJ_HIP, undefined, "delete" );
			set_objective( level.OBJ_FLY_KRAV, undefined, "breadcrumb" );
			
			
			flag_wait( "obj_sam_cave" );
			obj_pos_struct = getstruct( "struct_sam_cave_explo", "targetname");
			set_objective( level.OBJ_SAM, obj_pos_struct.origin, "" );
			set_objective( level.OBJ_FLY_KRAV, undefined, "remove" );
			...
			{
				set_objective( level.OBJ_SAM, undefined, "done" );
			}
			set_objective( level.OBJ_SAM, undefined, "delete" );
			set_objective( level.OBJ_FLY_KRAV, undefined, "breadcrumb" );
			
			
			flag_wait( "obj_large_village" );	//-- this flag gets set by a trigger in radiant
			set_objective( level.OBJ_FLY_KRAV, undefined, "remove" );
			level.village_targets_destroyed = 0;
			level.village_targets = getentarray( "village_zsus", "targetname" );
			for ( i=0; i<level.village_targets.size; i++ )
			{
				level.village_targets[i] vehicle_object_linkto();
				set_objective( level.OBJ_NVA_DEF2, level.village_targets[i].obj_loc, "", -1 );
				level.village_targets[i] thread village_targets_watcher( level.OBJ_NVA_DEF2 );
			}
			radio_tower = getent( "fxanim_pow_radar_tower_mod", "targetname" );
			set_objective( level.OBJ_NVA_DEF2, radio_tower, "", -1 );
			radio_tower thread radio_tower_watcher( level.OBJ_NVA_DEF2 );
			...
			set_objective( level.OBJ_NVA_DEF2, undefined, "done" );
			set_objective( level.OBJ_NVA_DEF2, undefined, "delete" );
			set_objective( level.OBJ_FLY_KRAV, undefined, "breadcrumb" );
			
				
			flag_wait( "obj_enemy_hind" );
			level.n_hinds_dead = 2;
			set_objective( level.OBJ_HINDS, undefined, undefined, level.n_hinds_dead );
			hinds = getentarray( "hind_for_last_battle", "targetname" );
			hinds[0] vehicle_object_linkto();
			hinds[1] vehicle_object_linkto();
			set_objective( level.OBJ_HINDS, hinds[0].obj_loc, "", -1 );
			set_objective( level.OBJ_HINDS, hinds[1].obj_loc, "", -1 );
			hinds[0] thread hind_remove_marker();
			hinds[1] thread hind_remove_marker();
			set_objective( level.OBJ_FLY_KRAV, undefined, "remove" );
			...
			set_objective( level.OBJ_HINDS, undefined, "done" );
			set_objective( level.OBJ_HINDS, undefined, "delete" );
			set_objective( level.OBJ_FLY_KRAV, undefined, "breadcrumb" );
			
			
			flag_wait( "obj_hind_landing" );
			set_objective( level.OBJ_FLY_KRAV, undefined, "done" );
		}
		
		case "compound":
		{
			flag_wait( "obj_to_kravchenko" );
			set_objective( level.OBJ_KILL_KRAV, level.woods, "follow" );
			
		
			flag_wait( "woods_ready_to_save_pows" );
			goal_pos = getstruct("struct_woods_cell_open", "targetname");
			set_objective( level.OBJ_KILL_KRAV, undefined, "delete" );
			set_objective( level.OBJ_POWS, goal_pos.origin + (0,0,48), "" );
			
			
			flag_wait("player_saved_pows");
			set_objective( level.OBJ_POWS, undefined, "done" );
			set_objective( level.OBJ_POWS, undefined, "delete" );
			level thread pow_objectives_krav_breadcrumb( level.OBJ_KILL_KRAV2 );
			player = get_players()[0];
			player waittill("door_breached");
			set_objective( level.OBJ_KILL_KRAV2, undefined, "remove" );
			level waittill("swap_rez_now");
			set_objective( level.OBJ_KILL_KRAV2, undefined, "done" );
		}
	}
}


radio_tower_watcher( obj_number )
{
	flag_wait("radio_tower_destroyed");
	
	level.village_targets_destroyed++;
	
	set_objective( obj_number, self, "remove" );
	
	while( level.village_targets_destroyed < level.village_targets.size + 1 )
	{
		wait 0.1;
	}
...
village_targets_watcher( obj_number )
{
	self waittill( "death" );
	
	level.village_targets_destroyed++;
	
	set_objective( obj_number, self.obj_loc, "remove" );
}


vehicle_object_linkto()
{
	self.obj_loc = spawn("script_model", self.origin + (0, 0, 100));
...
hind_remove_marker()
{
	...
	set_objective( level.OBJ_HINDS, self.obj_loc, "remove" );
	
	set_objective( level.OBJ_HINDS, undefined, undefined, level.n_hinds_dead );
	
	if ( level.n_hinds_dead < 1 )
	{
...
update_fly_to_pos( obj_number )
{
	...
	while(1)
	{
		...
		set_objective( obj_number, obj_struct, "breadcrumb" );
	...
	while( targets.size > 0)
	{
		...
		for( ; i < 8 && i < targets.size; i++) //-- 8 is the limit for Objective_AdditionalPosition
		{
			if( targets[i].classname == "trigger_multiple" )
			{
				...
				//set_objective( obj_number, temp_ent.origin, "", -1 );
			}
			else
			{
				//Objective_AdditionalPosition( obj_number, i, targets[i] );
				set_objective( obj_number, targets[i], "", -1 );
			}
		}
		...
		for( i = targets.size - 1; i >= 0; i-- )
		{
			if(IsDefined(targets[i].obj_break))
			{
				...
				{
					set_objective( obj_number, targets[i], "remove" );
...
{
	while( IsDefined(self) && !self maps\_vehicle_death::is_corpse()) //TODO: check to see if this is a hack
	{
		wait(0.1);
	}
...
bridge_rocket_notify( bridge_name, level_notify )
{
	...
	assert(IsDefined(bridge), "Trying to destroy a bridge which doesn't exist");
...
invincible_hind()
{
	...
	assert( true, "Something broke and the invincible hind died");
}
...
{
	assert(IsDefined(level.barnes), "BARNES IS NOT VALID YET - normal mode");
...
{
	assert(IsDefined(level.barnes), "BARNES IS NOT VALID YET - headtracking");
...
{
	if( self maps\_vehicle_death::is_corpse() )
	{
	...
	assert( origin_array.size > 0, "zsu does not have any origins to drive to" );
...
zsu_basic_fire( target, fire_notify, notify_count, use_flag ) //self == zsu
{
	if( self maps\_vehicle_death::is_corpse() )
	{
...
{
	assert(IsDefined(ent_array), "The positional array for average_positions is not defined.");
...
drive_to_target_struct_delay( delay, struct_targetname )
{
	...
	assert(IsDefined(delay), "bad scripter");
	assert(delay > 0, "bad scripter");
...
drive_to_target_struct(struct_targetname)
{
	...
	{
		assert(IsDefined( self.script_string ), "Vehicle does not have a valid struct destination");
	...
	while(1)
	{
		...
		if( IsArray(level.pow_clean_up["vehicle"]))
		{
			...
			for(i = level.pow_clean_up["vehicle"].size - 1; i >= 0; i--)
			{
				...
				if(level.pow_clean_up["vehicle"][i] maps\_vehicle_death::is_corpse() && IsDefined(level.pow_clean_up["vehicle"][i].deathmodel))
				{
...
heli_crash_basic_crash( time )
{
	...
	{
		assert(false, "non defined type of helicopter going through the crash script");
	}
	...
	while(1)
	{
		...
		else if(self.animname != "player_hands" && self.animname != "hind_body")
		{
			...
			*/
			...
			//add_temp_dialog_line( self.animname, msg );
		...
		else
		{
			...
			else if( msg == "takeoff" )
			{
				...
				maps\_vehicle::scripted_spawn(level.vehiclespawngroup + 28); //-- the first 3 sampans
...
drone_lookat_restart( ) //-- self == lookat trigger
{
	...
	//-- The Lookat Trigger
	assert( IsDefined(self), "Drone lookat struct not defined");
	self ent_flag_init("triggered");
	
	
	//-- The Destructible Building
	building = GetEnt( self.script_string, "script_noteworthy");
	assert( IsDefined(building), "Drone building not defined" );
	...
	while(1)
	{
		...
		while( time_spawning < 60 )
		{
			...
			drone_trig thread maps\_drones::drones_setup_spawner( true ); // restart the drone trigger
			drone_trig UseBy(get_players()[0]);
		}
...
pow_dialog_with_wait( scene, wait_time, animation, play_anim_first, hold_dialog_time, abort_flag )
{
	...
	{
		assert( IsDefined( hold_dialog_time ), "hold_dialog_time not defined in pow_dialog_with_wait when playing the anim first");
...
overrideactordamage_fakefight( eInflictor, eAttacker, iDamage, iDFlags, sMeansOfDeath, sWeapon, vPoint, vDir, sHitLoc, modelIndex, psOffsetTime, boneName )
{
