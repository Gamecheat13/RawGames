...
// SCRIPTER_MOD: dguzzo: 4/7/2009 1: no longer used
...
//	
//	self notify( "off" );
//	self.usetrig death_notify_wrapper();
//		
...
explodable_oiltank_explode()
{
	...
	if(dot < .5)
	{
		start = (self.origin + VectorScale(up, 0));
		end  = physicstrace(start, (start + (0,0,-64)));
		offset = end - self.origin;
	}
	offset += (0,0,4);
	...
	const minDamage = 2000;
	const maxDamage = 4000;
	blastRadius = 400;
	if (isdefined(self.radius))
	{
		blastRadius = self.radius;
...
explodable_bagofbarrels_burn()
{
	...
	offset1 = (0,0,-116);
	offset2 =  VectorScale(up, -116);
	
	if(dot < .5)
	{
		offset1 = VectorScale(up, -116) - (0,0,30);
		offset2 = VectorScale(up, -116) + (0,0,14);	
	}
	while (self.damageTaken < level.tankHealth)
	{
		if (!startedfx)
...
explodable_bagofbarrels_explode()
{
	...
	if(dot < .5)
	{
		start = (self.origin + VectorScale(up, -116));
		end  = physicstrace(start, (start + (0,0,-64)));
		offset = end - self.origin;
	}
	offset += (0,0,4);
