...
squad_enemies_init()
{
	if( !IsDefined(level.squad_enemies_init) )
	{
		level.squad_enemies_globals_init = true;

		level.CONST_SQUAD_SIZE					= 4;//4;// number of people in a squad ( can not be more than squad spawner set in radiant )
		level.CONST_MIN_SQUAD_SPAWNERS			= 4;	// min required leader spawners in level, must be >=2
		level.CONST_MAINTAIN_SQUAD_NUM 			= 4;	// number of squads to maintain
		level.CONST_SQUAD_FILL_DELAY 			= 0.15;	// seconds delay of squad respawn to fill in
		level.CONST_MAX_FOLLOW_DIST 			= 650;	// max units squad members are following leader
		level.CONST_FOLLOW_DIST					= 400;	// units squad members following leader
		level.CONST_MERGE_SQUAD_MEMBER_MAX 		= 3;	// when merging squads, the new squad can not have more than this number of AI
		level.CONST_RIOTSHIELD_FOLLOW_DIST 		= 45;	// units squad member will follow behind riotshield leader, 45 or less is very close
		level.CONST_RIOTSHIELD_FOLLOW_ANGLE		= 145;	// angle for squad member to position behind riotshield leader
		level.CONST_RIOTSHIELD_FOLLOW_RADIUS	= 8;	// goal radius while squad members follow riotshield
		level.CONST_SQUAD_LEADER_GOALRADIUS		= 2048;	// default goal radius of a squad leader vs player
	}
}
...
{
	if ( !flag_exists( "squad_spawning" ) )
		flag_init( "squad_spawning" );

	squad_enemies_init();
	
	level.new_squad_logic = true;
	level.merge_squad_member_max = level.CONST_MERGE_SQUAD_MEMBER_MAX;
...
spawn_enemy_squads( follower_size )
{
	...
	if( !isdefined( level.desired_squads ) )
		level.desired_squads = level.CONST_MAINTAIN_SQUAD_NUM;
	else
	{
		msg = "Must have at least " + level.CONST_MIN_SQUAD_SPAWNERS + " squad leader spawners in level";
		assert( level.desired_squads >= level.CONST_MIN_SQUAD_SPAWNERS, msg );
	}
	...
	while( 1 )
	{
		wait level.CONST_SQUAD_FILL_DELAY;
...
{	
	...
	if( is_coop() ) 
	{ 
		avoid_locs[ avoid_locs.size ] = GetPlayers()[ 1 ]; 
	}
	...
	if ( isDefined(level.spawn_loc_cull_cb) )
		all_spawn_locs = [[level.spawn_loc_cull_cb]](all_spawn_locs);
	...
	while ( 1 )	
	{
		...
		foreach ( leader in level.leaders )
		{
			...
			{	
				...
				if( distance( leader.origin, other_leader.origin ) < level.CONST_FOLLOW_DIST )
				{
					foreach( follower in other_leader.squadmembers )
					{
						if ( isdefined( follower.saw_player ) && follower.saw_player )
							follower.goalradius = level.CONST_MAX_FOLLOW_DIST;
					}
				}
				else
				{
					...
					{
						if ( isdefined( follower.saw_player ) && follower.saw_player )
							follower.goalradius = level.CONST_FOLLOW_DIST;
					}					
				}
			}	
		}	
...
merge_squad()
{
	...
	while ( 1 )
	{
		...
		if ( merge_size <= level.CONST_MERGE_SQUAD_MEMBER_MAX )
		{
			// merging!
...
spawn_enemy_group( spawners, leader_class, follower_class, follower_size )
{
	...
	assert( isdefined( leader_class ) == isdefined( follower_class ), "Squad leader and follower class overrides must be either both or none" );
	...
	Assert( ( spawners.size > 0 ), "The array passed to array_spawn function is empty" );
	...
	Assert( follower_size < spawners.size, msg );
	...
	if ( custom_spawner || !isspawner( spawners[ 0 ] ) )
	{
		...
		assert( isdefined( leader_spawner ), "Trying to spawn: [" + leader_class + "] without spawner present in level" );
		assert( isdefined( follower_spawner ), "Trying to spawn: [" + follower_class + "] without spawner present in level" );
		...
		foreach( spawner in spawners )
		{
			...
			if( spawner.script_noteworthy == "leader" )
			{
				...
				leader_spawner.angles = (0,0,0);// (isDefined(spawner.angles)?spawner.angles:(0,0,0));
				guy = simple_spawn_single(leader_spawner);
				if ( isDefined(guy) )
			...
			if( spawner.script_noteworthy == "follower" )
			{
				...
				follower_spawner.angles = (0,0,0);// (isDefined(spawner.angles)?spawner.angles:(0,0,0));
				guy = simple_spawn_single(follower_spawner);
				if ( isDefined(guy) )
					group[ group.size ] = guy;		
			}
		}
	...
	{
		assert( isspawner( spawners[ 0 ] ), "The spawners passed in for spawn arent spawner entities, is this custom spawning?" );
...
setup_leader( group )
{
	...
	{
		self.goalradius = level.CONST_SQUAD_LEADER_GOALRADIUS;
			
		//self.pathenemyfightdist = 1028;
		//self.pathenemylookahead = 1028;
		player = getclosest( self.origin, GetPlayers() );
	...
	self thread sync_follower_movemode();
...
sync_follower_movemode()
{
	...
	while( 1 )
	{
		self waittill( "change_movemode", moveMode );

		if ( isdefined( self.squadmembers ) && self.squadmembers.size )
		{
			foreach( follower in self.squadmembers )
				follower change_movemode( moveMode );
		}
	}
}
...
wait_for_followers()
{
	...
	{
		wait 2;
		assert( isdefined( self ), "Function called on dead or removed AI." );
...
setup_follower( leader )
{
	...
	self.leader 		= leader;
	self.acting_squad_leader = undefined;
	
	self thread leader_follower_count( leader );
	
	assert( isdefined( leader ), "setup follower to an undefined leader" );
	assert( isalive( leader ), "setup follower to a dead leader" );
	...
	if ( isdefined( level.squad_follower_func ) )
		...
		self thread follow_leader_regular();
}
...
follow_leader_regular()
{
	...
	while( 1 )
	{
		...
		else
		{
			...
			{
				aNodes = GetNodesInRadiusSorted( self.leader.origin, level.CONST_MAX_FOLLOW_DIST, 0, 80 );	
				//aNodes = get_array_of_closest( self.leader.origin, aNodes_unsorted, undefined, 16, level.CONST_MAX_FOLLOW_DIST, 64 );
...
setup_follower_goalradius()
{
	...
	self.goalradius = level.CONST_FOLLOW_DIST;
...
setup_follower_goalradius_riotshield()
{
	...
	self.goalradius = level.CONST_RIOTSHIELD_FOLLOW_RADIUS;
	self waittill( "goal" );
...
get_riotshield_back_pos( direction, facing_player )
{
	...
	//thread draw_line_for_time( self.origin, self.origin + vector_multiply( vectornormalize( anglestoforward( front_angle ) ), level.CONST_RIOTSHIELD_FOLLOW_DIST ), 0, 1, 0, 0.2 );

	if ( direction == "left" )
		back_angle = ( front_angle[0], front_angle[1]-level.CONST_RIOTSHIELD_FOLLOW_ANGLE, front_angle[2] );
	else
		back_angle = ( front_angle[0], front_angle[1]+level.CONST_RIOTSHIELD_FOLLOW_ANGLE, front_angle[2] );

	pos_vector = ( VectorNormalize( AnglesToForward( back_angle ) ) * level.CONST_RIOTSHIELD_FOLLOW_DIST ); // its really 45.5
...
drawLeader()
{
	...
	while ( 1 )
	{
		while( GetDvarInt( "squad_debug" ) == 0 )
			wait( 0.2 );
