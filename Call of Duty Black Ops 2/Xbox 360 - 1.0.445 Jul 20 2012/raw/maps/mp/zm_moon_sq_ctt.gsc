...
{
	level._active_tanks = [];
	maps\mp\zombies\_zm_spawner::add_cusom_zombie_spawn_logic(::tank_volume_death_check);
...
ctt1_first_kill_vox()
{
	...
	for(i = 0; i < level._active_tanks.size; i ++)
	{
		...
		if( isdefined( player ) )
		{
			player thread maps\mp\zombies\_zm_audio::create_and_play_dialog( "eggs", "quest4", undefined, 0, true );
			return;
		}
	}
}
...
ctt1_fifty_percent_vox()
{
	...
	players = GET_PLAYERS();
	players[randomintrange(0,players.size)] thread maps\mp\zombies\_zm_audio::create_and_play_dialog( "eggs", "quest4", undefined, 1, true );
}
...
ctt1_full_vox()
{
	...
	players = GET_PLAYERS();
	players[randomintrange(0,players.size)] thread maps\mp\zombies\_zm_audio::create_and_play_dialog( "eggs", "quest4", undefined, 2, true );
}
...
ctt2_full_vox()
{
	...
	players = GET_PLAYERS();
	players[randomintrange(0,players.size)] thread maps\mp\zombies\_zm_audio::create_and_play_dialog( "eggs", "quest6", undefined, 6, true );
}
