...
pipemasterInit(breaks)
{
	...
	while(level.pipe_breaks.size)
	{
		...
		ArrayRemoveValue(level.pipe_breaks, sample);
...
pipemasterIterate(sample)
{
	...
	for(i=0; i<family.size; i++)
	{
		...
		ArrayRemoveValue(level.pipe_breaks, family[i]);
	}
...
pipebreakInit(pipes)
{
	...
	{	
		...
		ArrayRemoveValue(pipes, self[j].whole);
		
		self[j].fxnode = spawnstruct();
		self[j].fxnode.origin = self[j].origin;
		self[j].fxnode.forward = VectorScale(anglestoright(self[j].angles), -1);
		...
		switch(self[j].script_noteworthy)
		{
			...
			case "fire96":
			{
				vec1 = VectorScale(self[j].fxnode.up, 16);
			...
			case "fire128":
			{
				vec1 = VectorScale(self[j].fxnode.up, 32);
				self[j].hurtnode[self[j].hurtnode.size] = self[j].fxnode.origin + vec1;
				vec1 = VectorScale(self[j].fxnode.up, -32);
				self[j].hurtnode[self[j].hurtnode.size] = self[j].fxnode.origin + vec1;
			}
			...
			case "fire256":
			{
				self[j].fx_multinode = [];
				newnode = spawnstruct();
				vec1 = VectorScale(self[j].fxnode.up, 64);
			...
			case "fueltanker":
			{
				...
				newnode2.forward = self[j].fxnode.forward + VectorScale(self[j].fxnode.right, 1);
				...
				newnode = spawnstruct();
				vec1 = VectorScale(self[j].fxnode.up, 112);
				newnode.origin = self[j].fxnode.origin + vec1;
				newnode.forward = self[j].fxnode.forward;
				newnode.up = self[j].fxnode.up;
				self[j].fx_multinode[self[j].fx_multinode.size] = newnode;
				...
				newnode2.forward = newnode.forward + VectorScale(self[j].fxnode.right, 1);
				self[j].fx_multinode[self[j].fx_multinode.size] = newnode2;
				...
				self[j].hurtnode[self[j].hurtnode.size] = self[j].fxnode.origin;
				vec1 = VectorScale(self[j].fxnode.up, 184);
				self[j].hurtnode[self[j].hurtnode.size] = self[j].fxnode.origin + vec1;
				vec1 = VectorScale(self[j].fxnode.up, -184);
				self[j].hurtnode[self[j].hurtnode.size] = self[j].fxnode.origin + vec1;
			}
		...
		vec1 = VectorScale(self[j].fxnode.up, displacement);
		self[j].ends[self[j].ends.size] = self[j].fxnode.origin + vec1;
		vec1 = VectorScale(self[j].fxnode.up, (displacement * -1));
		self[j].ends[self[j].ends.size] = self[j].fxnode.origin + vec1;
	}
}
...
pipesetup()
{
	...
	if(isdefined(self.target))
	{
		...
		vec = VectorScale(vec, 128);
		self.B = self.A+vec;
	}
	else
	{
		vec = anglestoforward(self.angles);
		vec1 = VectorScale(vec, 64);
...
pipeimpact()
{
	...
	while(1)
	{
		...
		direction_vec = VectorScale(direction_vec, -1);
...
{	
	for(i=0; i< self.size; i++)
	{
		...
		level._effect["pipe_interactive"][self[i].script_noteworthy]	= loadfx("destructibles/fx_dest_mp_pipe_steam"); 	
	...
	for(i=0; i< self.size; i++)
	{
		...
		level._effect["pipe_interactive"][self[i].script_noteworthy]	= loadfx("destructibles/fx_dest_mp_pipe_water");  
	...
	for(i=0; i< self.size; i++)
	{
		...
		level._effect["pipe_interactive"][self[i].script_noteworthy	]	= loadfx("destructibles/fx_dest_mp_pipe_fire");	
		level._effect["pipe_interactive"]["fire64"]						= loadfx("env/fire/fx_pipe_explosion64"); 	
		level._effect["pipe_interactive"]["fire96"]						= loadfx("env/fire/fx_pipe_explosion96"); 	
	...
	level._effect["pipe_interactive"]["impact"]							= loadfx("impacts/fx_small_metalhit"); 	
}