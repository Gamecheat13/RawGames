...
plates()
{
	...
	while(1)
	{
		...
		{
			attacker thread maps\mp\zombies\_zm_audio::create_and_play_dialog( "eggs", "quest5", undefined, randomintrange(0,2), true );
			break;
		}
	}
	...
	{
//t6todo		plates[i] DontInterpolate();
		plates[i].origin = targs[i].origin;
		plates[i].angles = targs[i].angles;
	}
	
//t6todo	maps\mp\zombies\_zm_weap_quantum_bomb::quantum_bomb_register_result( "ctvg", ::dud_func, 100, ::ctvg_validation );
	level._ctvg_pos = targs[0].origin;
	level waittill("ctvg_validation");
//t6todo	maps\mp\zombies\_zm_weap_quantum_bomb::quantum_bomb_deregister_result("ctvg");
	
	players = GET_PLAYERS();
	players[randomintrange(0,players.size)] thread maps\mp\zombies\_zm_audio::create_and_play_dialog( "eggs", "quest5", undefined, randomintrange(4,6), true );
...
wire()
{
	...
	who thread maps\mp\zombies\_zm_audio::create_and_play_dialog( "eggs", "quest5", undefined, 7, true );
	...
	who thread maps\mp\zombies\_zm_audio::create_and_play_dialog( "eggs", "quest5", undefined, 8, true );
...
vg()
{
	...
	who thread maps\mp\zombies\_zm_audio::create_and_play_dialog( "eggs", "quest5", undefined, 9, true );
...
//t6todo		grenade thread maps\mp\zombies\_zm_weap_black_hole_bomb::do_black_hole_bomb_sound( plates[0], info ); // WW: This might not work if it is based on the model
...
teleport_target( grenade, models )
{
  level.teleport_target_trigger Delete();
	...
	for(i = 0; i < models.size; i ++)
	{
//t6todo		models[i] DontInterpolate();
	...
	for(i = 0; i < lines.size; i ++)
	{
		...
		switch(l.who)
		{
			case "rictofen":
				...
				{
					ent_num = players[j].characterindex;// GetEntityNumber();
...
do_bucket_fill(target)
{
	...
	for(i = 0; i < players.size; i ++)
	{
		...
		ent_num = player.characterindex;// GetEntityNumber();
...
wrong_presser_thread()
{
	...
	while(1)
	{
		if(IsDefined(level._charge_sound_ent))
		{
			...
			who thread maps\mp\zombies\_zm_audio::create_and_play_dialog( "eggs", "quest5", undefined, 11, true );
		}		
...
wrong_collector()
{
	...
	while(1)
	{
		...
		who thread maps\mp\zombies\_zm_audio::create_and_play_dialog( "eggs", "quest5", undefined, 27, true );
