...
gamemodeSpawnDvars(reset_dvars)
{
	...
	// turn on the ability to use enemy team spawns if team spawns are all blocked
	SetDvar( "spawnsystem_allow_non_team_spawns", "0" );
}
...
onPlayerKilled(eInflictor, attacker, iDamage, sMeansOfDeath, sWeapon, vDir, sHitLoc, psOffsetTime, deathAnimDuration)
{
	...
	if ( inBombZone && isPlayer( attacker ) && attacker.pers["team"] != self.pers["team"] && ( !isdefined( sWeapon ) || !maps\mp\killstreaks\_killstreaks::isKillstreakWeapon( sWeapon ) ) )
	{	
...
//		player playSound( "mpl_sd_bomb_plant" );
	  ...
	  thread maps\mp\gametypes\_globallogic_audio::set_music_on_team( "DEM_THEY_PLANT", enemyTeam, false, false, 5  );
		...
		if ( !isScoreBoosting( player, self ) )
		{
			...
			maps\mp\_scoreevents::processScoreEvent( "planted_bomb", player, undefined, undefined, true );
...
bombPlanted( destroyedObj, player )
{
	...
	if ( isdefined( player ) )
	{
		...
		maps\mp\_scoreevents::processScoreEvent( "bomb_detonated", player, undefined, undefined, true );
