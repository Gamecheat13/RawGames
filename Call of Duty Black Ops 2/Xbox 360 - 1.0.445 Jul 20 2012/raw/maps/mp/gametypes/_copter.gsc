...
myMagicBullet(pos, dir)
{
	...
	trace = bullettrace(pos, pos + VectorScale(dir, 10000), true, undefined);
...
copterMove()
{
	...
	while(1)
	{
		...
		// rotation
		newdir = self.destDir;
		//line(self.origin + level.copterCenterOffset, self.origin + level.copterCenterOffset + VectorScale(self.destDir,10000), (1,0,0));
		if (newdir[2] < -.4) {// *never* look too far down
			newdir = vectornormalize((newdir[0], newdir[1], -.4));
		}
		//line(self.origin + level.copterCenterOffset, self.origin + level.copterCenterOffset + VectorScale(newdir,10000), (1,1,0));
...
copterDie()
{
	...
	while(1)
	{
		self.vel = self.vel + VectorScale((0,0,-200), interval);
