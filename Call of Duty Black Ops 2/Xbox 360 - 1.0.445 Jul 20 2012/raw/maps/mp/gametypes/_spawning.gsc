...
init_spawn_system()
{
	...
	// get the spawning dvars now and reset them all to defaults
	level get_player_spawning_dvars(true);
...
onPlayerSpawned()
{
	...
	for(;;)
	{
		...
		self thread initialSpawnProtection( "specialty_nottargetedbyairsupport", true );
		self thread initialSpawnProtection( "specialty_nokillstreakreticle", false );
		self thread initialSpawnProtection( "specialty_nottargettedbysentry", false );
...
/*						 
	...
	{
		pixbeginevent("spawnsystem grenade_predict_endpoint_loop");
...
create_qrdrone_influencers( team )
{
	...
	self.influencer_qrdrone_cylinder = AddCylinderInfluencer( level.spawnsystem.eINFLUENCER_TYPE_NORMAL,
																 self.origin, 
																 (0,0,0),
																 (0,0,0),
																 level.spawnsystem.qrdrone_cylinder_influencer_radius,
																 level.spawnsystem.qrdrone_cylinder_influencer_length,
																 level.spawnsystem.qrdrone_cylinder_influencer_score,
																 other_team_mask,
																 "qrdrone_cyl,r,s",
																 get_score_curve_index(level.spawnsystem.qrdrone_cylinder_influencer_score_curve),
																 0,
																 self );

	return AddSphereInfluencer( level.spawnsystem.eINFLUENCER_TYPE_NORMAL,
						 self.origin, 
						 level.spawnsystem.qrdrone_influencer_radius,
						 level.spawnsystem.qrdrone_influencer_score,
						 other_team_mask,
						 "qrdrone,r,s",
						 get_score_curve_index(level.spawnsystem.qrdrone_influencer_score_curve),
						 0,
						 self );
}
...
create_aitank_influencers( team )
{
	...
	return AddSphereInfluencer( level.spawnsystem.eINFLUENCER_TYPE_NORMAL,
						 self.origin, 
						 level.spawnsystem.aitank_influencer_radius,
						 level.spawnsystem.aitank_influencer_score,
						 other_team_mask,
						 "aitank,r,s",
						 get_score_curve_index(level.spawnsystem.aitank_influencer_score_curve),
						 0,
						 self );
}
...
create_pegasus_influencer( origin, team )
{
	...
	return AddSphereInfluencer( level.spawnsystem.eINFLUENCER_TYPE_NORMAL,
						 origin, 
						 level.spawnsystem.pegasus_influencer_radius,
						 level.spawnsystem.pegasus_influencer_score,
						 other_team_mask,
						 "pegasus,r,s",
						 get_score_curve_index(level.spawnsystem.pegasus_influencer_score_curve),
						 0 );
}
			...
			case "sphere":
			{
				if (IsDefined(influencer_entity.radius))
				{
					...
					influencer_id = AddSphereInfluencer( level.spawnsystem.eINFLUENCER_TYPE_GAME_MODE,
														 ...
														 score,
														 getTeamMask(influencer_entity.script_team),
														 "*map_defined",
...
get_player_spawning_dvars(reset_dvars)
{
	...
	ss.qrdrone_cylinder_influencer_radius =	set_dvar_float_if_unset("scr_spawn_qrdrone_cylinder_influencer_radius", "" + 2000, reset_dvars);
	ss.qrdrone_cylinder_influencer_length =	set_dvar_float_if_unset("scr_spawn_qrdrone_cylinder_influencer_length", "" + 2500, reset_dvars);
	...
	// set all spawn dvars to a default state, override only if needed to be on
...
onSpawnPlayer_Unified(predictedSpawn)
{
	...
	//println("SPAWN:onSpawnPlayer_Unified(false)");
...
get_best_spawnpoint( point_team, influencer_team, player, predictedSpawn )
{
	...
	// new method just uses the first point that was returned (engine does the filtering)
