...
main()
{
	...
	maps\mp\gametypes\_globallogic_audio::registerDialogGroup( "gamemode_objective_a", false );
	maps\mp\gametypes\_globallogic_audio::registerDialogGroup( "gamemode_objective_b", false );
	maps\mp\gametypes\_globallogic_audio::registerDialogGroup( "gamemode_objective_c", false );
}
...
domFlags()
{
	...
	for ( index = 0; index < level.flags.size; index++ )
	{
		...
		domFlag = maps\mp\gametypes\_gameobjects::createUseObject( "neutral", trigger, visuals, (0,0,100), name );
...
//	SetDvar( "scr_obj" + self maps\mp\gametypes\_gameobjects::getLabel() + "_flash", 1 );	
	...
	if ( ownerTeam == "neutral" )
	{
		...
		statusDialog( "securing"+self.label, player.pers["team"], self.label );
...
onUseUpdate( team, progress, change )
{
	if ( progress > 0.05 && change && !self.didStatusNotify )
	{
		...
		if ( ownerTeam == "neutral" )
		{
			...
			statusDialog( "securing"+self.label, team, self.label );
			statusDialog( "losing"+self.label, otherTeam, self.label );
		}
		else
		{
			statusDialog( "losing"+self.label, ownerTeam, self.label );
			statusDialog( "securing"+self.label, team, self.label );
		}
...
statusDialog( dialog, team, label )
{
	thread delayedLeaderDialog( dialog, team, label );
}
...
//	self maps\mp\gametypes\_gameobjects::set3DIcon( "enemy", "waypoint_capture" + label );
	...
	thread give_capture_credit( touchList, string, oldTeam );
	...
	if ( oldTeam == "neutral" )
	{
		...
		{
			statusDialog( "secured"+self.label, team, self.label );
			statusDialog( "lost"+self.label, otherTeam, self.label );
...
//		thread playSoundOnPlayers( "mus_dom_captured"+"_"+level.teamPostfix[team] );
		...
		{	
			statusDialog( "secured"+self.label, team, self.label );
			statusDialog( "lost"+self.label, oldTeam, self.label );
...
give_capture_credit( touchList, string, lastOwnerTeam )
{
	wait .05;
...
updateDomScores()
{
	...
	while ( !level.gameEnded )
	{
		...
		if ( numFlags )
			maps\mp\gametypes\_globallogic_score::giveTeamScoreForObjective( "allies", numFlags );

		numFlags = getTeamFlagCount( "axis" );
		numOwnedFlags += numFlags;
		if ( numFlags )
			maps\mp\gametypes\_globallogic_score::giveTeamScoreForObjective( "axis", numFlags );
...
onPlayerKilled( eInflictor, attacker, iDamage, sMeansOfDeath, sWeapon, vDir, sHitLoc, psOffsetTime, deathAnimDuration )
{
	if (  isdefined( attacker ) && isplayer( attacker ) && ( !isdefined( sWeapon ) || !maps\mp\killstreaks\_killstreaks::isKillstreakWeapon( sWeapon ) ) )
	{
		for ( index = 0; index < level.flags.size; index++ )
		{
			...
			level.defaultOffenseRadius = 400;
...
dom_gamemodeSpawnDvars(reset_dvars)
{
	...
	ss.dom_owned_flag_influencer_score[0] = set_dvar_float_if_unset("scr_spawn_dom_owned_flag_A_influencer_score", "50", reset_dvars);
	...
	ss.dom_owned_flag_influencer_score[1] = set_dvar_float_if_unset("scr_spawn_dom_owned_flag_B_influencer_score", "50", reset_dvars);
	...
	ss.dom_owned_flag_influencer_score[2] = set_dvar_float_if_unset("scr_spawn_dom_owned_flag_C_influencer_score", "50", reset_dvars);
