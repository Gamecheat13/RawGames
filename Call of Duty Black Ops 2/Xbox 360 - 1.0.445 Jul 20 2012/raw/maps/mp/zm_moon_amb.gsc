...
//#include animscripts\zm_Utility;
...
poweron_vox()
{
	...
	{
		if( players[i] maps\mp\zombies\_zm_equip_gasmask::gasmask_active() )
		{
...
do_player_playvox_custom( prefix, index, sound_to_play, waittime, category, type, override )
{
	...
	if( level.player_is_speaking != 1 )
	{
		...
		if( !flag( "solo_game" ) && ( isdefined (level.plr_vox[category][type + "_response"] )))
		{
			...
			{
//T6todo			    level thread maps\mp\zombies\_zm_audio::setup_response_line( self, index, category, type ); 
			}
		}
	}
}
...
waitfor_first_player()
{
	...
	who thread maps\mp\zombies\_zm_audio::create_and_play_dialog( "general", "teleporter", undefined, undefined, true );
}
