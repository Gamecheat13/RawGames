...
useRCBomb( placement )
{
	...
	killstreak_id = self maps\mp\killstreaks\_killstreakrules::killstreakStart( hardpointtype, self.team, undefined, false );
	if ( killstreak_id == -1 )
	{
		if( isDefined( self.rcbomb ) )
		{
...
watchForScramblers() // self == scrambler
{
	...
	while( true )
	{
		scrambled = self GetClientFlag( CLIENT_FLAG_STUNNED );
...
blowup(attacker, weaponName)
{
	...
	{	
		...
		{
			maps\mp\_challenges::destroyRCBombWithHatchet( attacker );
		}
		thread maps\mp\_scoreevents::processScoreEvent( "destroyed_rcbomb", attacker, self.owner, weaponName );
