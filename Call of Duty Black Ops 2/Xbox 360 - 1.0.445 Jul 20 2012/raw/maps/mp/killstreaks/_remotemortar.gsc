...
init()
{
	...
	overrideEntityCameraInDemo("remote_mortar_mp", true);
	
	set_dvar_float_if_unset( "scr_remote_mortar_lifetime", 50.0 );
...
remote_mortar_killstreak( hardpointType )
{
	...
	killstreak_id = self maps\mp\killstreaks\_killstreakrules::killstreakStart( "remote_mortar_mp", self.team, false, true );
	if ( killstreak_id == -1 )
	{
		self clearUsingRemote();
		self EnableWeaponCycling();
	...
	remote thread maps\mp\_heatseekingmissile::MissileTarget_LockOnMonitor( self, "remote_end" );				// monitors missle lock-ons
	...
	self ClearClientFlag( CLIENT_FLAG_OPERATING_REAPER );
...
remote_mortar_spawn()
{
	self SetClientFlag( CLIENT_FLAG_OPERATING_REAPER );
...
{
	self PlayerLinkWeaponViewToDelta( remote, "tag_player", 1.0, 40, 40, 25, 65 );
...
//		triggerFX( remote.fx );
		...
		// must rebuild the influencer each frame
	...
	while( GetTime() < end_time )
	{
		...
		if (shot % 3 == 1)
		{
			if ( isdefined(remote.owner) && isdefined(remote.owner.pilotTalking) && remote.owner.pilotTalking )
				shot = 0;				
			remote thread PlayPilotDialog( "reaper_fire", 0.25, undefined, false );
		}
		shot = ( shot + 1 ) % 3;
...
{	
	...
	while( true )
	{
		...
		{
			thread maps\mp\_scoreevents::processScoreEvent( "destroyed_remote_mortar", attacker, self.owner, weapon);
