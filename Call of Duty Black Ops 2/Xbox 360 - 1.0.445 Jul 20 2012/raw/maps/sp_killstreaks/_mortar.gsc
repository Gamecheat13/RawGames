...
preload()
{
	...
	maps\sp_killstreaks\_killstreaks::registerKillstreak("mortar_sp", "mortar_sp", "killstreak_mortar", "mortar_used", ::useKillstreakMortar, true);
	maps\sp_killstreaks\_killstreaks::registerKillstreakStrings("mortar_sp", &"KILLSTREAK_EARNED_MORTAR", &"KILLSTREAK_MORTAR_NOT_AVAILABLE", &"KILLSTREAK_MORTAR_INBOUND");
	maps\sp_killstreaks\_killstreaks::registerKillstreakDialog("mortar_sp", "mpl_killstreak_mortar", "kls_mortars_used", "","kls_mortars_enemy", "", "kls_mortars_ready");
	maps\sp_killstreaks\_killstreaks::registerKillstreakDevDvar("mortar_sp", "scr_givemortar");
...
init()
{
	...
	level.mortarCanonShellCount =  3;
	level.mortarCanonCount = 4;
...
useKillstreakMortar(hardpointType)
{
	...
	result = self maps\sp_killstreaks\_mortar::selectMortarLocation( hardpointType );
...
useMortar( positions )
{
	if ( self maps\sp_killstreaks\_killstreakrules::killstreakStart( "mortar_sp", self.team ) == false )
		return false;
...
//		maps\sp_killstreaks\_airsupport::losRadiusDamage( bombPoint + (0,0,16), 256, 300, 50, owner); // targetpos, radius, maxdamage, mindamage, player causing damage
//	}
//}
...
getBestFlakDirection( hitpos, team )
{
	...
	spawns =  ArrayCombine( getentarray(targetname,"targetname"), getstructarray(targetname,"targetname"), true, false);
...
mortarImpactEffects( )
{
	...
	while ( level.mortarShellsInAir )
	{
		...
		if ( weapon == "mortar_sp" )
		{
			...
			maps\_shellshock::mortar_earthquake( position );
...
callStrike_mortarBombEffect( spawnPoint, bombdir, velocity, owner, requiredDeathCount, distance, ratio, pitch, yaw )
{
	...
	bomb = owner launchbomb( "mortar_sp", spawnPoint, bomb_velocity );
...
{	
	...
	owner maps\sp_killstreaks\_killstreaks::playKillstreakStartDialog( "mortar_sp", team );
	level.globalKillstreaksCalled++;
	//self AddWeaponStat( "mortar_sp", "used", 1 );
...
//	wait 2;
	...
	if ( !isDefined( owner ) )
	{
		...
		maps\sp_killstreaks\_killstreakrules::killstreakStop( "mortar_sp", team ); 
