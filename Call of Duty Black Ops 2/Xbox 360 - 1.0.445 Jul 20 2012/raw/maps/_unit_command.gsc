...
//	}
	...
	if(type == "stinger")
	{
		for(i = 0 ; i < 2; i++)
		{
			...
			units[i].crew = true;
			units[i].crew_type = "stinger";
			units[i].blocking_point = self.ndirection;
			
			wait(0.05);
		}
		
		units[ 0 ].ai_spotter = units[ 1 ];
		units[ 0 ] thread crew_stinger_logic();
		units[ 0 ] thread crew_monitor();
		units[ 1 ] thread magic_bullet_shield();
	}
	else if(type == "RPG")
	{
		for(i = 0 ; i < 2; i++)
	...
	else if(type == "sniper")
	{
		...
		units[ 0 ].ai_spotter = units[ 1 ];
		units[ 0 ] thread crew_sniper_logic();
		units[ 0 ] thread crew_monitor();
		units[ 1 ] thread magic_bullet_shield();
	}
...
remove_unit_from_blocking_point()
{
	if(isdefined(self.current_unit_type))
	{
		if( isdefined( self.current_unit[0] ) )
		{
			unit = self.current_unit;
			
			node = self.delete_node;
			unit[1] SetGoalPos(node.origin + (50, 100, 0));
			unit[0] SetGoalNode(node);
			
			unit[1] set_goalradius(32);
			unit[0] set_goalradius(32);
			
			wait 1;
			
			unit[0] waittill("goal");
			
			//move them and delete them.
			num = unit.size;
			for(i = 0; i < num; i++)
			{
				if(isDefined(unit[i]) && IsAlive(unit[i]))
				{
					unit[i] stop_magic_bullet_shield();
					unit[i] delete();
				}
			}
		}
