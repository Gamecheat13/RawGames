...
RemoveCallback(event, func)
{
	...
	ArrayRemoveValue( level._callbacks[event], func, true );
}
...
checkpoint_invulnerability()
{
	...
	self.checkpoint_invulnerability = true;
...
Callback_PlayerConnect()
{
	...
	{
		// Band aid to spawn clients at host position.
...
{
	if ( IS_TRUE( self.checkpoint_invulnerability ) && ( sMeansOfDeath != "MOD_FALLING" ) )
	{
		// protection from damage on a save restore
	...
	if( self HasPerk( "specialty_armorvest" ) && !maps\_utility_code::isHeadDamage( sHitLoc ) )
	{
		//If victim has body armor, reduce the damage by the cac armor vest value as a percentage
	   	...
	   	iDamage = Int( iDamage * ( level.cac_armorvest_data * 0.01 ) );  // scale down damage by percentage
		/#
...
//				println( "Perk/> " + eAttacker.name + "'s bullet damage did less damage to " + (isDefined(self.name)?self.name:"Player") );
	...
	if ( sWeapon == "emp_grenade_stage2_sp" )
	{
...
Callback_VehicleDamage( eInflictor, eAttacker, iDamage, iDFlags, sMeansOfDeath, sWeapon, vPoint, vDir, sHitLoc, psOffsetTime, damageFromUnderneath, modelIndex, partName )
{
	...
	{
		//-- used to print any messages if a different weapon other than the one currently used is needed
		if( IsPlayer(eAttacker) && IsDefined( level.vehicle_immune_notify_func ) )
		{
			self thread [[level.vehicle_immune_notify_func]]( sMeansOfDeath );
		}
	...
	if ( IS_TRUE( self.magic_bullet_shield ) )
	{
		// just like AI magic bullet shield. This way we still get a damage notify and can see how much damage
