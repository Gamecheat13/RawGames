...
main()
{
	...
	level.friendlyfire[ "civ_kill_points" ] 	= -900;		// participation point penalty for killing a civ
	level.friendlyfire[ "point_loss_interval" ] = .75;		// amount of time a point lasts
	
	SetDvar( "friendlyfire_enabled", "1" ); 
	
	level.friendlyfire_override_attacker_entity = ::default_override_attacker_entity;	// Use if you want a custom function to potentially override the attacker entity
...
//
// This is a function that can be used to override the attacker entity
//	
...
friendly_fire_think( entity )
{
	...
	for (;;)
	{
		...
		entity waittill ( "friendlyfire_notify", damage, attacker, undefined, undefined, method );
		...
		//if( damage < 1 )
		//{
		//	PrintLn("Friendly fire damage was -1 indicating death see KDrew if not desired outcome");
		//	continue;
		//}
		...
		/* GLOCKE: not currently in use in the game
		...
		*/
		...
		if(attacker.team == "allies")
		{
			...
			{
				same_team = true;	// If the player kills a civ, count it towards friendly fire - DSL.													
			}
		}
...
//				attacker maps\_stats::set_enemy_damage( method, entity.damagelocation ); 
		...
		else
		{
			...
			if( killed )
			{
				//player killed friendly AI
			}
			else
			{
				//player damaged friendly AI
			} 
		}
