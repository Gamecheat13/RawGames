...
#include animscripts\anims_table_juggernaut;

// --------------------------------------------------------------------------------
// ---- Init function for Juggernaut, called from level main script ----
// --------------------------------------------------------------------------------
#using_animtree( "generic_human" );
make_juggernaut( startHuntBehavior = true, disableSprint = false )
{
	if( !IsDefined( level.JUGGERNAUT_HEALTH ) )
	{
		...
		level.JUGGERNAUT_SPRINTDISTSQ      = 512 * 512;
	...
	self.maxfaceenemydist	  = level.JUGGERNAUT_SPRINTDISTSQ;
	...
	if( startHuntBehavior )
		self thread juggernaut_hunt_immediately_behavior( disableSprint );

	level notify( "juggernaut_spawned" );
}

juggernaut_hunt_immediately_behavior( disableSprint )
{
	self endon("death");
