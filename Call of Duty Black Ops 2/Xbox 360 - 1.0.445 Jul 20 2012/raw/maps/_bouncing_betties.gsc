...
betty_think( trigger )
{
	...
	const betty_radius = 90;
	
	//assert( trigger.classname == "trigger_radius", "bouncing betty trigger at origin '" + trigger.origin + "' needs to be a trigger radius." );

	// set up values
	jumpHeight = RandomIntRange( 68, 80 );  // world units
	dropHeight = RandomIntRange( 10, 20 );

	const jumpTime = 0.15;  // seconds
	const dropTime = 0.1;
	const clickWaitTime = 0.05;  // seconds after the "click" til the betty jumps up
	const radiusMultiplier = 1;  // how big of an area outside the touch trigger should dudes be damaged instead of killed?  1 = 100% bigger radius.
	const damageMultiplier = 2;  // how much less damage do guys near but not touching the trigger take?
...
betty_rotate()
{
	...
	const rotateAngles = 360;
	const rotateTime = 0.125;
...
betty_think_no_wires( trigger )
{
	...
	const jumpTime = 0.15;  // seconds
	const dropTime = 0.1;
	const clickWaitTime = 1.0;  // seconds after the "click" til the self jumps up
	const radiusMultiplier = 1;  // how big of an area outside the touch trigger should dudes be damaged instead of killed?  1 = 100% bigger radius.
	const damageMultiplier = 2;  // how much less damage do guys near but not touching the trigger take?
