...
#define NOTETRACK_SYNC			"sync"
#define NOTETRACK_UNSYNC		"unsync"
#define NOTETRACK_ATTACHKNIFE	"attach_knife"
#define NOTETRACK_DETACTKNIFE	"detach_knife"
#define NOTETRACK_STAB			"stab"
#define NOTETRACK_DEATH			"melee_death"
#define NOTETRACK_INTERACT		"melee_interact"

#define KNIFE_ATTACK_MODEL		"weapon_parabolic_knife"
#define KNIFE_ATTACK_TAG		"TAG_INHAND"
#define KNIFE_ATTACK_SOUND		"melee_knife_hit_body"
#define KNIFE_ATTACK_FX_NAME	"flesh_hit_knife"
#define KNIFE_ATTACK_FX_TAG		"tag_origin"
...
#define MELEE_RANGE_SQ						      sqr64
#define MELEE_ACTOR_BOUNDS_RADIUS			      32	// a little bigger than twice the radius of an actor's bounding box
#define MELEE_ACTOR_BOUNDS_RADIUS_MINUS_EPSILON   (MELEE_ACTOR_BOUNDS_RADIUS-0.1)	// used for asserts

#define CHARGE_RANGE_SQ						      160 * 160
#define CHARGE_RANGE_SQ_VS_PLAYER				  200 * 200

#define FAILED_INIT_NEXT_MELEE_TIME			  	  150			// basic IsValid() falure
#define FAILED_CHARGE_NEXT_MELEE_TIME			  1500			// charge failures (both standard/aiVSai)
#define FAILED_STANDARD_NEXT_MELEE_TIME		      2500			// standard melee failure

#define MELEE_NUM_VARIATIONS 2
...
Melee_UpdateAndValidateStartPos()
{
	...
	if ( distFromTarget2d < MELEE_ACTOR_BOUNDS_RADIUS )
	{
		...
		if( distFromTarget2d <= MELEE_ACTOR_BOUNDS_RADIUS_MINUS_EPSILON )
			return false;

		self.melee.startPos += dirToStartPos2d * (MELEE_ACTOR_BOUNDS_RADIUS - distFromTarget2d);
		assert( distance2d( self.melee.startPos, self.melee.target.origin ) >= (MELEE_ACTOR_BOUNDS_RADIUS_MINUS_EPSILON), "Invalid distance to target: " + distance2d( self.melee.startPos, self.melee.target.origin ) + ", should be more than " + ( MELEE_ACTOR_BOUNDS_RADIUS_MINUS_EPSILON) );
		ignoreActors = false;
	}
	...
	// MELEE_TODO - MW3 added ignoreActors parameter to mayMoveToPoint, investigate
	...
	// MELEE_TODO - MW3 added two more parameters to mayMoveToPointToPoint, investigate
	if ( !self mayMoveFromPointToPoint( self.melee.startPos, mayMoveTargetOrigin ) ) //, true, false ) )
		return false;

	// MELEE_TODO - Not sure why this trace check is needed
...
Melee_IsValid()
{
	...
	yawToEnemy = AngleClamp180( self.angles[ 1 ] - GET_YAW(self, target.origin ) );
...
Melee_Standard_Main()
{
	...
	while ( true )
	{
		...
		// pick one of the random available melee variations
		if( self.animType == "default" && animArrayAnyExist( "melee_2", "combat" ) && self weaponAnims() == "rifle" )
			self.melee.meleeAnimVarientIndex = RandomInt(MELEE_NUM_VARIATIONS);
		else
		...
		self setflaggedanimknoballrestart( "meleeanim", animArray("melee_"+self.melee.meleeAnimVarientIndex, "combat" ), %body, 1, .2, self.moveplaybackrate );
		...
		if ( note == "fire" )
		{
			if ( isdefined( self.melee.target ) )
			{
				...
				if ( isDefined( self.melee.target ) && (self.melee.target.health < oldhealth) )
				{
					...
					{
						self.melee.target playsoundtoplayer( "wpn_melee_hit", self.melee.target );
					}
...
Melee_Standard_GetInPosition()
{
	...
	// MELEE_TODO - Hack - we only have the standing to melee so force the stance to stand here.
	self.a.pose = "stand";

	if ( enemyDistanceSq <= MELEE_RANGE_SQ )
	{
		// just play a melee-from-standing transition
		self SetFlaggedAnimKnobAll( "readyanim", animArray("stand_2_melee_"+self.melee.meleeAnimVarientIndex, "combat" ), %body, 1, .3, self.moveplaybackrate );
	...
	self SetFlaggedAnimKnobAll( "chargeanim", runAnim, %body, 1, .3, self.moveplaybackrate );
	raisingGun = false;
	...
	while ( 1 )
	{
		...
		if ( !raisingGun )
		{
			...
			{
				Melee_StartMovement();
				self SetFlaggedAnimKnobAllRestart( "chargeanim", animArray("run_2_melee_"+self.melee.meleeAnimVarientIndex, "combat" ), %body, 1, .2, self.moveplaybackrate );
		...
		enemyDistanceSq = distance2dSquared( self.origin, self.melee.target.origin );
		enemyVel = VectorScale( self.melee.target.origin - prevEnemyPos, 1 / ( gettime() - time ) );// units / msec
		prevEnemyPos = self.melee.target.origin;
...
Melee_PartnerEndedMeleeMonitorThread()
{
	...
	if ( isDefined( self.melee.death ) )
	{
		...
		else
		{
			...
			{
				// MELEE_TODO - Jordan must have replaced melee_death to start_ragdoll causing this assert.
				// Investigate this whole survival after interruption thing in general.
...
Melee_HandleNoteTracks( note )
{
	...
	{
		self detach( KNIFE_ATTACK_MODEL, KNIFE_ATTACK_TAG );
	...
	else if ( note == NOTETRACK_STAB )
	{
		...
		// play special effect on special tag if needed
...
drawEventPointAndDir( position )
{
	...
	if( !GetDvarInt( "scr_debugmelee") ) 
		return;	
	
	while(1)
	{
		drawDebugCross( position, 1, (1,0,0), .05 );
