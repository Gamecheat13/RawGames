...
main()
{
	clientscripts\mp\zm_transit::gamemode_common_setup( "race", "tunnel", "zm_encounters_town", 2 );
	
	level thread setup_tunnel_race_arrows();
	level thread clean_up_tunnel_race_arrows();	
	
	/*
	level thread clientscripts\mp\zombies\_zm_game_mode_objects::init_game_mode_objects("race","tunnel");
...
setup_tunnel_race_arrows()
{
	level._race_arrows = [];
	all_structs = level.struct;
	foreach(struct in all_structs)
	{
		if(!isDefined(struct.name) || struct.name != "tunnel_arrows")
		{
			continue;
		}
		players = GetLocalPlayers();
