...
init()
{
	//RegisterClientField("vehicle", "vehicle_is_firing", 1, "int", ::vehicle_is_firing_function, true);
	
	RegisterClientField("helicopter", "vehicle_is_firing", 1, "int", ::vehicle_is_firing_function, true);
	//RegisterClientField("actor", "player_is_gunner", 1, "int", ::chppoer_gunner_loop, true);	
...
/*chppoer_gunner_loop(localClientNum, oldVal, newVal, bNewEnt, bInitialSnap, fieldName)
{
	if (newVal == 0)
	{
		playloopat ("mpl_cgunner_flir_lp", (0,0,0));
	}
	else
	{
		stoploopat ("mpl_cgunner_flir_lp", (0,0,0));
	}
}
*/
vehicle_is_firing_function(localClientNum, oldVal, newVal, bNewEnt, bInitialSnap, fieldName)
{
...
init_heli_sounds_heli_guard() //tag, type, bone, run
{
...
setup_terrain_brass_sounds( surface_type, alias ) //self == helicopter
{
...
play_player_drone_sounds() // self == helicopter Cobra
{
	// ******* qrdrone **************
...
/#	printLn( "^stated aircraft_dustkick" );	#/
	...
	while( isdefined( self ) )
	{
		...
		//assert( isdefined( vehicle.treadfx[trace["surfacetype"]] ), "UNKNOWN SURFACE TYPE: " + trace["surfacetype"] ); 
...
drone_button_watch()
{
	...
	while (1)
	{
		if ( abs (self.qrdrone_z_difference) > 5 && return_to_zero)
		{
			...
			check_speed = scale_speed( 5, 50, 0, 1, self.qrdrone_z_difference );
			while (check_speed < .1)
			{
				wait (.05);
				check_speed = scale_speed( 5, 50, 0, 1, self.qrdrone_z_difference );
