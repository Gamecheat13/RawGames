...
specialops_achievement_by_stars( specOpsString )
{
	...
	if ( get_num_of_levels_with_star( specOpsString, 3 ) >= 15 )//45
		self player_giveachievement_wrapper( "PROFESSIONAL" );
	
	// =============== Operative: 3 stars in 10 different SO
	if ( get_num_of_levels_with_star( specOpsString, 3 ) >= 10 )//30
		self player_giveachievement_wrapper( "OPERATIVE" );	
	
	
	// --------------- Specialist: earn 30 star in special ops
	if ( specOpsSum >= 30 )
		self delaythread( .05, ::player_giveachievement_wrapper, "SPECIALIST" );
	
	// =============== Honor Roll: 1 star in 23 different SO
	if ( get_num_of_levels_with_star( specOpsString, 1 ) >= 23 )
		self delaythread( .05, ::player_giveachievement_wrapper, "HONOR_ROLL" );	
	
	// --------------- Blackjack: earn 21 star in special ops
	if ( specOpsSum >= 21 )
		self delaythread( .05, ::player_giveachievement_wrapper, "BLACKJACK" );
	
	
	// =============== Operational Asset: 3 stars in 5 different SO
	if ( get_num_of_levels_with_star( specOpsString, 3 ) >= 5 )//15
		self delaythread( .1, ::player_giveachievement_wrapper, "OPERATIONAL_ASSET" );
		
	// =============== It Goes to Eleven: 1 star in 11 different SO
	if ( get_num_of_levels_with_star( specOpsString, 1 ) >= 11 )
		self delaythread( .1, ::player_giveachievement_wrapper, "IT_GOES_TO_ELEVEN" );
		
	// --------------- Charlie on our six: earn 8 star in special ops
	if ( specOpsSum >= 8 )
		self delaythread( .1, ::player_giveachievement_wrapper, "CHARLIE_ON_OUR_SIX" );
	
	
	// --------------- Hotel Bravo: earn 4 star in special ops
	if ( specOpsSum >= 4 )
		self delaythread( .15, ::player_giveachievement_wrapper, "HOTEL_BRAVO" );	
								
	// --------------- Gold Star: earn 1 star in special ops
	if ( specOpsSum >= 1 )
		self delaythread( .15, ::player_giveachievement_wrapper, "GOLD_STAR" );
}
