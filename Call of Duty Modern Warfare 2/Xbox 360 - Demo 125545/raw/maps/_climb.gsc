...
track_player_button_presses_for_holding_on()
{
	...
	for ( ;; )
	{
		left_pressed = level.player AdsButtonPressed();
		right_pressed = level.player AttackButtonPressed();
		if ( !level.console )
		{
			left_pressed = level.player AttackButtonPressed();
			right_pressed = level.player AdsButtonPressed();
		}
...
stop_hanging_arm_hint( arm )
{
	...
	if ( !level.player call [[ level.arm_ent_globals.arm_ents[ other_arm ].buttonCheck ]]() )
		return true;

	return level.player call [[ level.arm_ent_globals.arm_ents[ arm ].buttonCheck ]]();
}
