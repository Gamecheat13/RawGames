...
init()
{
	...
	if ( GetDvarInt( "scr_destructible_warning" ) == 1 )
	...
	if ( IsDefined( level.destructible_type[ self.destuctableInfo ].parts ) )
	{
		...
		for ( i = 0; i < level.destructible_type[ self.destuctableInfo ].parts.size; i++ )
		{
			...
			if ( IsDefined( level.destructible_type[ self.destuctableInfo ].parts[ i ][ 0 ].v[ "random_dynamic_attachment_1" ] ) )
			{
				...
				self thread do_random_dynamic_attachment( attachTag, attach_model_1, attach_model_2 );
			}
...
random_dynamic_attachment( tagName, attachment_1, attachment_2 )
{
...
destructible_think()
{
	...
	for ( ;; )
	{
		...
		/#
		if ( GetDvarInt( "debug_destructibles" ) == 1 )
		{
...
explode( partIndex, force_min, force_max, range, mindamage, maxdamage, continueDamage, originOffset, earthQuakeScale, earthQuakeRadius, attacker )
{
	...
	// Chad - temp commented out arcade mode stuff
	/*
	if ( arcadeMode_car_kill() )
	{
		if ( IsDefined( level.arcadeMode_kill_func ) )
			thread [[level.arcadeMode_kill_func]]( self.origin, true, "explosive", 200 );
	}
	*/
...
do_random_dynamic_attachment( tagName, attach_model_1, attach_model_2 )
{
	Assert( IsDefined( tagName ) );
