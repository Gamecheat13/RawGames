...
do_friendly_fire_reaction()
{
	if ( self IsMoveSuppressed() && self.prevScript == "move" && self.a.pose == "stand" && !isdefined( self.disableFriendlyFireReaction ) )
	{
		...
		if ( !self IsSuppressed() || self.team == "allies" )
		{
			self animmode( "zonly_physics" );
			self setFlaggedAnimKnobAllRestart( "react", %surprise_stop_v1, %root, 1, 0.2, 1 );
...
keepTryingToMelee()
{
	...
	while ( 1 )
	{
		...
		if ( isdefined( self.enemy ) && ( distanceSquared( self.enemy.origin, self.origin ) < sqr100 ) )
			tryMelee();
	}
}
