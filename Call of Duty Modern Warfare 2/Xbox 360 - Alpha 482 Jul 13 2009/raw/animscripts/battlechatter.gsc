...
playBattleChatter()
{
	...
	if ( self.team == "allies" && GetDvarInt( "bcs_forceEnglish" ) )
		return;
...
responseThreatCallout( respondTo, modifier )
{
	...
	if ( modifier == "echo" )
	{
		success = chatPhrase addThreatCalloutEcho( self.curEvent.reportAlias );
	}
	else if( modifier == "QA" )
	{
...
//	if ( GetDvar( "bcs_stealth" ) != "" && self.voice == "british" )
	...
	for ( i = 0; i < chatPhrase.soundAliases.size; i++ )
	{
		...
		else
		{
			...
			if ( GetDvarInt( "bcs_forceEnglish" ) )
			{
				self PlaySoundAsMaster( chatPhrase.soundAliases[ i ], chatPhrase.soundAliases[ i ], true );
			}
			else
...
addThreatCalloutEcho( reportAlias )
{
	ASSERT( IsDefined( reportAlias ) );

	alias = self createEchoAlias( reportAlias );
...
// self = a chatPhrase struct
createEchoAlias( reportAlias )
{
	...
	baseAlias = GetSubStr( reportAlias, 5, reportSuffixStartIndex );  // start 5 spots in to eliminate the original countryID and npcID info
