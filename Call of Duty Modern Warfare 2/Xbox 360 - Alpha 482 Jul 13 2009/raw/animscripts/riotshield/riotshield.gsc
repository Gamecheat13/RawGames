...
init_riotshield_AI()
{
	...
	self.shieldBulletBlockLimit = randomintrange( 8, 12 );	// fall over after getting hit this many times on the shield all within 0.3 seconds of each other
...
riotshield_startMoveTransition()
{
	...
	if ( self.a.pose == "crouch" )
	{
		...
		self setFlaggedAnimKnobAllRestart( "startmove", %riotshield_crouch2walk, %body, 1, .1, rate );
...
riotshield_pain()
{
	...
	if ( !isdefined( self.a.onback ) )
	{
		...
		{
			self setFlaggedAnimKnobAllRestart( "painanim", %riotshield_crouch_pain, %body, 1, .1, rate );
		}
	}
...
riotshield_flee_and_drop_shield()
{
	...
	self SetFlaggedAnimKnobAll( "fleeanim", fleeAnim, %root, 1, .1, 1 );
