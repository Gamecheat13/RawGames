...
aiThreadThreader()
{
	...
	if ( self.team == "allies" )
	{
		...
		self thread playerFriendlyFireWaiter();
	}
...
init_aiBattleChatter()
{
	...
	// global except secretservice
	if( self.voice != "secretservice" )
	{
	...
	if( self.team == "allies" )
	{
		...
		// all allies except secretservice
		if( self.voice != "secretservice" )
		{
...
playerFriendlyFireWaiter()
{
	...
	while( 1 )
	{
		...
		if( self canDoFriendlyFireEvent( shooter, whizbyDist ) )
		{
			/*
			endTime = GetTime() + 500;
			while( GetTime() < endTime )
			{
				Print3D( self GetEye() + ( 0, 0, 8 ), "checkfire", ( 1, 1, 1 ), 1, 0.5 );
				wait( 0.05 );
			}
			*/
...
canDoFriendlyFireEvent( shooter, whizbyDist )
{
