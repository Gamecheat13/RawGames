...
doImmediateRagdollDeath()
{
	...
	directionScale = initialImpulse * self.damagetaken;
...
getStrongBulletDamageDeathAnim()
{
	...
	{
		deathArray = array( %death_shotgun_spinL, %death_stand_sniper_spin1 );
	}
	else
	{
		deathArray = array( %death_shotgun_spinR, %death_stand_sniper_spin2 );
	}
...
getStandDeathAnim()
{
	...
	else
	{
		...
		if ( damageLocationIsAny( "torso_lower", "left_leg_upper", "left_leg_lower", "right_leg_lower", "right_leg_lower" ) )
		{
			...
			deathArray[ deathArray.size ] = tryAddDeathAnim( %stand_death_crotch );
			deathArray[ deathArray.size ] = tryAddDeathAnim( %stand_death_guts );
		}
		...
		if ( damageLocationIsAny( "torso_upper", "left_arm_upper" ) )
		{
			...
			deathArray[ deathArray.size ] = tryAddDeathAnim( %stand_death_shoulder_stumble );
		}		
		...
		if ( ( self.damageyaw > 135 ) || ( self.damageyaw <= -135 ) )// Front quadrant
		{
			if ( damageLocationIsAny( "neck", "head", "helmet" ) )
			{
				...
				deathArray[ deathArray.size ] = tryAddDeathAnim( %stand_death_head_straight_back );
			}

			if ( damageLocationIsAny( "torso_upper" ) )
			{
				deathArray[ deathArray.size ] = tryAddDeathAnim( %stand_death_chest_stunned );
			}
		}
