...
killplayer()
{
	...
	if ( isdefined( self.player_killed ) )
		return;
	...
	{
		knock_down_player_coop( self.meleeingPlayer );
		return;
	}
	...
	{
		PlayFXOnTag( level._effect[ "dog_bite_blood" ], self.meleeingPlayer.player_view, "tag_torso" );
	}
...
knock_down_player_coop( player )
{
	if ( isdefined( player.player_view ) )
		player.player_view endon( "pvd_melee_done" );
...
dog_hint()
{
	...
	self.meleeingPlayer.dogHintElem = self.meleeingPlayer maps\_hud_util::createClientFontString( "default", 2 );
...
PlayerView_EndSequence( player )
{
	...
	if ( isalive( player ) )
	{
		...
		{
			player notify( "deathshield", player.maxHealth * 10, self.dog );
