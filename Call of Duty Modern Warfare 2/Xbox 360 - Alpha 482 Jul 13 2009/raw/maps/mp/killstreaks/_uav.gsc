...
init()
{
	...
	level.uavBlockTime = 20; // time radar remains active
...
launchUAV( owner, team, duration, isCounter )
{
	...
	UAVModel linkTo( level.UAVRig, "tag_origin", (xOffset,yOffset,zOffset), (0,atan(yOffset/xOffset) - angleOffset,0) );
	...
	UAVModel waittill_notify_or_timeout( "death", duration - 7 );
	...
	else
	{
		...
		UAVModel moveTo( destPoint, (duration * 0.25), (duration * 0.25), 0.0 );
		PlayFXOnTag( level._effect[ "ac130_engineeffect" ] , UAVModel, "tag_origin" );
	
		UAVModel waittill_notify_or_timeout( "death", 7 );
	}
...
addUAVModel( UAVModel )
{
	if ( level.teamBased )
		...
		level.UAVModels[UAVModel.owner.guid] = UAVModel;	
}	
...
addActiveCounterUAV()
{
	...
	if ( level.teamBased )
	{
		...
		{
			if ( player.team == self.team )
				player iPrintLn( &"MP_WAR_COUNTER_RADAR_ACQUIRED", self.owner, level.radarViewTime );
			else if ( player.team == level.otherTeam[self.team] )
				player iPrintLn( &"MP_WAR_COUNTER_RADAR_ACQUIRED_ENEMY", level.radarViewTime  );
		}
	}	
	else
	{
