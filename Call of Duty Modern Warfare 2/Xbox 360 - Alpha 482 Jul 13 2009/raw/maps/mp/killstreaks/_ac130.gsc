...
init()
{
	...
	level.weaponReloadTime[ "ac130_25mm_mp" ] = 4.0;
	level.weaponReloadTime[ "ac130_40mm_mp" ] = 6.0;
	level.weaponReloadTime[ "ac130_105mm_mp" ] = 9.0;
...
tryUseAC130( lifeId )
{
	...
	*/
	...
	{
		self iPrintLnBold( "AC130 currently unavailable." );
		return false;
	}
	...
	if ( level.teambased )
	{
		...
		printOnTeamArg( &"MP_AC130_INBOUND", team, self );
	}
	else
	{
		//self leaderDialogOnPlayer( "helicopter_inbound" );
		//selfarray = [];
		//selfarray[0] = self;
		//leaderDialog( "enemy_helicopter_inbound", undefined, undefined, selfarray );
	}
...
damageTracker()
{
	for ( ;; )
	{
		...
		if ( isDefined( level.ac130player ) && isPlayer( attacker ) && attacker.team == level.ac130player.team )
		{
			self.health += damage;
			continue;
		}
...
attachPlayer( player )
{
	self PlayerLinkWeaponviewToDelta( level.ac130, "tag_player", 1.0, 35, 35, 30, 30 );
...
crashPlane( crashTime )
{
	...
	level.ac130.planeModel hide();
//	forward = vector_multiply( anglestoright( level.ac130.planeModel.angles ), 200 );
//	playFx( level._effect[ "ac130_explode" ], level.ac130.planeModel.origin, forward );
