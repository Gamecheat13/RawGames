...
init()
{
	...
	level.heli_maxhealth 	= 1000;	// max health of the helicopter
	...
	makeHeliType( "pavelow", "explosions/helicopter_explosion_cobra_low", ::pavelowLightFx );
	...
	addAirExplosion( "apache", "explosions/helicopter_explosion_cobra_low" );
...
pavelowLightFX()
{
	...
	wait ( 0.05 );
	playFXOnTag( level.chopper_fx["light"]["left"], self, "tag_light_cargo01" );
	wait ( 0.05 );
	playFXOnTag( level.chopper_fx["light"]["right"], self, "tag_light_cargo02" );
	wait ( 0.05 );
...
tryUseHelicopter( lifeId, heliType )
{
	...
	if ( !isDefined( heliType ) && isDefined( level.chopper ) )
	{
		self iPrintLnBold( "Helicopter will arrive when air space is clear." );
	...
	{
		self iPrintLnBold( "Helicopter currently unavailable." );
		return false;
	}		
...
startHelicopter( lifeId, heliType )
{
	...
	if ( level.teamBased )
		printOnTeamArg( &"MP_HELICOPTER_INBOUND", team, self );
...
heliRide( lifeId, chopper )
{
	...
	self setUsingRemote( "helicopter_" + chopper.heliType );
	...
	self VisionSetThermalForPlayer( game["thermal_vision"], 1.0 );
...
heli_think( lifeId, owner, startnode, heli_team, heliType )
{
	...
	chopper.lifeId = owner.lifeId;
	chopper.team = heli_team;
	chopper.pers["team"] = heli_team;	
	chopper.owner = owner;

	if ( heliType == "flares" )
		chopper.maxhealth = level.heli_maxhealth*3;			// max health
	else
...
makeGunShip()
{
	...
	mgTurret.owner = self.owner;
 	...
 	self.mgTurretLeft setMode( "manual" );
 	self.mgTurretRight setMode( "manual" );
...
heli_existance()
{
	...
	{
		player maps\mp\killstreaks\_killstreaks::usedKillstreak( "helicopter", true );
		player startHelicopter();
	}
	else
	{
		level.chopper = undefined;
...
heli_targeting()
{
	...
	{		
		...
		if ( isDefined( level.harrier ) )
		{
			if ( (level.teamBased && level.harrier.team != self.team) || (!level.teamBased && level.harrier.owner != self.owner) )
			{
				self notify( "secondary acquired" );
				self.secondaryTarget = level.harrier;
			}
		}
...
heli_damage_monitor()
{
	...
	for( ;; )
	{
		...
		if( self.damageTaken > self.maxhealth  && self.team != attacker.team )
		{
			thread maps\mp\gametypes\_missions::vehicleKilled( self.owner, self, undefined, attacker, damage, type );

			if ( isDefined( attacker.owner ) )
			{
				attacker.owner thread [[level.onXPEvent]]( "kill" );
				attacker.owner notify( "destroyed_helicopter" );
				
				if ( self.heliType == "flares" )
					thread teamPlayerCardSplash( "callout_destroyed_helicopter_flares", attacker.owner );
				else if ( self.heliType == "minigun" )
					thread teamPlayerCardSplash( "callout_destroyed_helicopter_minigun", attacker.owner );
				else
					thread teamPlayerCardSplash( "callout_destroyed_helicopter", attacker.owner );				
			}
			else
			{
				...
				if ( self.heliType == "flares" )
					thread teamPlayerCardSplash( "callout_destroyed_helicopter_flares", attacker );
				else if ( self.heliType == "minigun" )
					thread teamPlayerCardSplash( "callout_destroyed_helicopter_minigun", attacker );
				else
