...
init()
{
	...
	game["strings"]["stealth_airstrike_hint"] = &"MP_AIRSTRIKE_PICKUP";
	...
	level.airDropPoints = getEntArray( "mp_airdrop_point", "classname" );
...
/*
	...
	if ( !isDefined( level.airDropCrateCollision ) )
		assertMsg( "All levels must support SD: bug your friendly neighborhood LD... this one doesn't" ); 
	#/
*/
	...
	addCrateType( "airdrop",		"ammo", 					17,			::ammoCrateThink );
	addCrateType( "airdrop",		"uav", 						17,			::killstreakCrateThink );
	addCrateType( "airdrop",		"counter_uav", 				15,			::killstreakCrateThink );
	addCrateType( "airdrop",		"sentry", 					12,			::sentryCrateThink );
	addCrateType( "airdrop",		"predator_missile", 		12,			::killstreakCrateThink );
	addCrateType( "airdrop",		"precision_airstrike", 		11,			::killstreakCrateThink );
	addCrateType( "airdrop",		"harrier_airstrike", 		7,			::killstreakCrateThink );
	addCrateType( "airdrop",		"helicopter", 				7,			::killstreakCrateThink );
	addCrateType( "airdrop",		"helicopter_flares", 		5,			::killstreakCrateThink );
	addCrateType( "airdrop",		"stealth_airstrike", 		5,			::killstreakCrateThink );
	addCrateType( "airdrop",		"helicopter_minigun", 		3,			::killstreakCrateThink );
	addCrateType( "airdrop",		"ac130", 					3,			::killstreakCrateThink );
	addCrateType( "airdrop",		"emp", 						1,			::killstreakCrateThink );
	addCrateType( "airdrop",		"nuke", 					0,			::killstreakCrateThink );

	addCrateType( "airdrop_mega",	"ammo", 					12,			::ammoCrateThink );
	addCrateType( "airdrop_mega",	"uav", 						12,			::killstreakCrateThink );
	addCrateType( "airdrop_mega",	"counter_uav", 				16,			::killstreakCrateThink );
	addCrateType( "airdrop_mega",	"sentry", 					16,			::sentryCrateThink );
	addCrateType( "airdrop_mega",	"predator_missile", 		14,			::killstreakCrateThink );
	addCrateType( "airdrop_mega",	"precision_airstrike", 		10,			::killstreakCrateThink );
	addCrateType( "airdrop_mega",	"harrier_airstrike", 		5,			::killstreakCrateThink );
	addCrateType( "airdrop_mega",	"helicopter", 				5,			::killstreakCrateThink );
	addCrateType( "airdrop_mega",	"helicopter_flares", 		3,			::killstreakCrateThink );
	addCrateType( "airdrop_mega",	"stealth_airstrike", 		3,			::killstreakCrateThink );
	addCrateType( "airdrop_mega",	"helicopter_minigun", 		2,			::killstreakCrateThink );
	addCrateType( "airdrop_mega",	"ac130", 					2,			::killstreakCrateThink );
	addCrateType( "airdrop_mega",	"emp", 						0,			::killstreakCrateThink );
...
tryUseAirdrop( lifeId, dropType )
{
	...
	{
		self iPrintLnBold( "Air traffic too heavy." );
		return false;
	} 
...
createAirDropCrate( owner, dropType, crateType )
{
	dropCrate = spawn( "script_model", (0,0,0) );
	...
	dropCrate.friendlyModel = spawn( "script_model", (0,0,0) );
	dropCrate.friendlyModel setModel( "com_plasticcase_friendly" );
	dropCrate.enemyModel = spawn( "script_model", (0,0,0) );
...
{	
	...
	if ( !level.teamBased )
	{
		...
		self maps\mp\_entityheadIcons::setHeadIcon( self.owner, icon, (0,0,48), 14, 14 );
	}
	...
	else if ( mode == "team" )
	{
		...
		self maps\mp\_entityheadIcons::setHeadIcon( self.team, icon, (0,0,48), 14, 14 );
	}
...
dropTheCrate( dropPoint, dropType, lbHeight, dropImmediately, crateOverride )
{
	dropCrate = [];
	...
	dropCrate = createAirDropCrate( self.owner, dropType, crateType );
	...
	dropCrate Unlink();
	dropCrate PhysicsLaunchServer( (0,0,0), vector_multiply( anglestoforward( dropCrate.angles ), dropSpeed ) );		
...
getFlyHeightOffset()
{
	...
	{	
		...
		return( lbFlyHeight * level.airStrikeHeightScale + 256 );
	}
	else
		return ( lbFlyHeight );	
}
...
{
	flyHeight = self getFlyHeightOffset();
	...
	pathGoal = dropSite + (0,0,flyHeight);	
	...
	chopper thread dropTheCrate( dropSite, dropType, flyHeight );
...
{	
	...
	c130 thread dropTheCrate( dropSite, dropType, flyHeight );
	wait ( 0.05 );
...
dropNuke( dropSite, owner, dropType )
{
	...
	c130 thread dropTheCrate( dropSite, dropType, flyHeight, false, "nuke" );
	wait ( 0.05 );
	c130 notify ( "drop_crate" );
...
killstreakCrateThink( dropType )
{
	...
	for ( ;; )
	{
		...
		player thread maps\mp\killstreaks\_killstreaks::giveKillstreak( self.crateType, undefined, self.owner );
...
nukeCrateThink( dropType )
{
	...
	for ( ;; )
	{
		...
		player thread [[ level.killstreakFuncs[ self.crateType ] ]]();
...
useHoldThinkLoop( player )
{
    ...
    while( isDefined( self ) && isReallyAlive( player ) && player useButtonPressed() && self.curProgress < self.useTime )
    {
