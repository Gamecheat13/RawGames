...
init()
{
	...
	level.stealthbombfx = loadfx ("explosions/stealth_bomb_loop_mp");
...
tryUseAirstrike( lifeId, airStrikeType )
{
	...
	switch( airStrikeType )
	{
		...
		case "harrier":
			if ( level.planes > 1 )
			{
				self iPrintLnBold( "Air space too crowded." );
				return false;	
			}
			break;
		case "super":
...
callStrike_bomb( coord, owner, offset, showFx )
{
	...
	losRadiusDamage( bombPoint + (0,0,16), 768, 300, 50, owner, self, "artillery_mp" ); // targetpos, radius, maxdamage, mindamage, player causing damage
}
