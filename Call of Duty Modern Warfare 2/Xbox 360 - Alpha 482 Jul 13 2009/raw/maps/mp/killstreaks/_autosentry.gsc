...
SENTRY_SPINUP_TIME		= 1.5;
SENTRY_OVERHEAT_TIME	= 3.0;
...
init()
{
	...
	level.sentrySettings[ "sentry_minigun" ].burstMax = 60;
	level.sentrySettings[ "sentry_minigun" ].pauseMin = 0.5;
	level.sentrySettings[ "sentry_minigun" ].pauseMax = 1.5;
...
sentry_initSentry( sentryType, owner )
{
	...
	self setConvergenceTime( 1.5, "pitch" );
	self setConvergenceTime( 1, "yaw" );
...
sentry_handleDamage()
{
	...
	while ( self.health > 0 )
	{
		...
		if ( isDefined( attacker ) && isPlayer( attacker ) && attacker != self.owner && attacker isFriendlyToSentry( self ) )
		{
			self.health += amount;
			continue;
		}

		// 3x damage for explosives
		if ( isExplosiveDamage( type ) )
			self.health -= (amount * 6);
...
sentry_handleDeath()
{
	...
	self setSentryOwner( undefined );	//clear the owner; prevents the turret from drawing on minimap
...
sentry_spinUp()
{
	...
	rampTime = SENTRY_SPINUP_TIME;
	
	while ( self.momentum < 1 )
	{
		self.momentum += rampTime * 0.1;
	...
	for ( ;; )
	{
		...
		if ( self.heatLevel > SENTRY_OVERHEAT_TIME )
		{
			...
			while ( self.heatLevel )
			{
				...
				playFxOnTag( getFx( "sentry_overheat_mp" ), self, "tag_aim" ); // temp double fx
				wait ( 0.1 );
			}
