...
getTimeLimit()
{
	if ( isDefined( game[ "status" ] ) && game[ "status" ] == "overtime" && ( !isDefined(game[ "inNukeOvertime" ]) || !game[ "inNukeOvertime" ] ) )
		return 1.0;
...
_suicide()
{
	if ( self isUsingRemote() && !isDefined( self.fauxDead ) && self getRemoteName() != "remotemissile" && self getRemoteName() != "smartarrow_mp" )
	...
	else if(! self isUsingRemote() || isDefined( self getRemoteName() ) && self getRemoteName() == "remotemissile" )
		self suicide();	
}
...
showStatusMenu( duration )
{
	...
	foreach ( player in level.players )
	{
		...
		player thread statusMenu( duration );
	}
	*/
}
...
getWeaponAttachment( weapon ) //this doesnt properly handle bling weapons
{
...
getLastLivingPlayer( team )
{
	...
	foreach ( player in level.players )
	{
		...
		if ( !isAlive( player ) && !player.pers["lives"] )
			continue;
