...
init()
{
	...
	level.spawnGlowSplat = loadfx( "misc/glow_stick_destroy_red" );

	level.spawnGlowModel["enemy"] = "weapon_light_stick_tactical_red";
	level.spawnGlowModel["friendly"] = "weapon_light_stick_tactical_green";
	...
	level.scriptPerks["_smartarrow_mp"] = true;
...
cac_modified_damage( victim, attacker, damage, meansofdeath, weapon, impactPoint, impactDir, hitLoc )
{
	...
	else if ( isExplosiveDamage( meansOfDeath ) )
	{
		...
		if ( isKillstreakWeapon( weapon ) && attacker _hasPerk("specialty_dangerclose") )
			damageAdd += damage * .10;
	}
	...
	{
		if ( IsDefined( self.damageBlockedTotal ) && ( level.teambased && victim.team != attacker.team || !level.teambased) )
		{
...
{	
	...
	level.explosiveDamageMod = getIntProperty( "perk_explosiveDamage", 25 )/100;		// increased explosive damage by this %
	level.blastShieldMod = 25/100;														// percentage of damage you take
