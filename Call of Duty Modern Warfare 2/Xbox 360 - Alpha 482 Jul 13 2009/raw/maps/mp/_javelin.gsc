	...
	for( ;; )
	{
		...
		if ( self.javelinStage == 1 )	// SCANNING: try to find a good point to lock to
		{
			...
			if ( targets.size != 0 )
			{
				...
				{
					insideReticle = self WorldPointInReticle_Circle( target.origin, 65, 25 );
					if ( insideReticle )
		...
		if ( self.javelinStage == 2 )	// LOCKING: complete lockon to point
		{
			...
			insideReticle = self WorldPointInReticle_Circle( self.javelinTargetPoint, 65, 30 );
			if ( !insideReticle )
			{
				ResetJavelinLocking();
				continue;
...
GetTargetList()
{
	...
	if ( level.teamBased )
	{
		...
		if ( isDefined( level.harrier ) && ( level.harrier.pers["team"] != self.team || level.harrier.owner == self ) )
			targets[targets.size] = level.harrier;
	...
	else
	{
		...
		if ( isDefined( level.harrier ) && level.harrier.owner != self )
			targets[targets.size] = level.harrier;
