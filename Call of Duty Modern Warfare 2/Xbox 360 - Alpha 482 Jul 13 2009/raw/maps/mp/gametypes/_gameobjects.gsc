...
init()
{
	...
	precacheShader( "waypoint_digit_10s_0" );
	precacheShader( "waypoint_digit_10s_1" );
	precacheShader( "waypoint_digit_10s_2" );
	precacheShader( "waypoint_digit_10s_3" );
	precacheShader( "waypoint_digit_10s_4" );
	precacheShader( "waypoint_digit_10s_5" );

	precacheShader( "waypoint_digit_1s_0" );
	precacheShader( "waypoint_digit_1s_1" );
	precacheShader( "waypoint_digit_1s_2" );
	precacheShader( "waypoint_digit_1s_3" );
	precacheShader( "waypoint_digit_1s_4" );
	precacheShader( "waypoint_digit_1s_5" );
	precacheShader( "waypoint_digit_1s_6" );
	precacheShader( "waypoint_digit_1s_7" );
	precacheShader( "waypoint_digit_1s_8" );
	precacheShader( "waypoint_digit_1s_9" );
...
createUseObject( ownerTeam, trigger, visuals, offset )
{
	...
	useObject.objPoints["timer_10s"] = maps\mp\gametypes\_objpoints::createTeamObjpoint( "objpoint_timer_10s_" + useObject.entNum, useObject.curOrigin + offset, "all", undefined );
	useObject.objPoints["timer_1s"] = maps\mp\gametypes\_objpoints::createTeamObjpoint( "objpoint_timer_1s_" + useObject.entNum, useObject.curOrigin + offset, "all", undefined );
	useObject.objPoints["timer_10s"].sort = 999;
	useObject.objPoints["timer_1s"].sort = 1000;
	
	useObject.objPoints["allies"].alpha = 0;
	useObject.objPoints["axis"].alpha = 0;
	useObject.objPoints["timer_10s"].alpha = 0;
	useObject.objPoints["timer_1s"].alpha = 0;
...
getEarliestClaimPlayer()
{
	...
	if ( self.touchList[team].size > 0 )
	{
		...
		for ( index = 0; index < players.size; index++ )
		{
			...
			if ( !isdefined( earliestTime ) || touchdata.starttime < earliestTime && isReallyAlive( players[index] ) )
			{
...
triggerTouchThink( object )
{
	...
	while ( isReallyAlive( self ) && self isTouching( object.trigger ) && !level.gameEnded && object.useTime )
	{
...
{
	opName10s = "objpoint_timer_10s_" + self.entNum;			
	objPoint10s = maps\mp\gametypes\_objpoints::getObjPointByName( opName10s );

	opName1s = "objpoint_timer_1s_" + self.entNum;			
	objPoint1s = maps\mp\gametypes\_objpoints::getObjPointByName( opName1s );
	
	if ( showIcon )
	{
		objPoint10s setShader( "waypoint_digit_10s_" + int( seconds * 0.1), int(level.objPointSize*2), int(level.objPointSize*2) );
		objPoint10s.alpha = 1;
		objPoint10s.isShown = true;

		objPoint10s setWayPoint( true, true );	

		objPoint1s setShader( "waypoint_digit_1s_" + (seconds % 10), int(level.objPointSize*2), int(level.objPointSize*2) );
		objPoint1s.alpha = 1;
		objPoint1s.isShown = true;

		objPoint1s setWayPoint( true, true );	
	}
	else
	{
		objPoint10s fadeOverTime( 0.05 );
		objPoint10s.alpha = 0;
		objPoint10s.isShown = false;
		objPoint10s ClearTargetEnt();

		objPoint1s fadeOverTime( 0.05 );
		objPoint1s.alpha = 0;
		objPoint1s.isShown = false;
		objPoint1s ClearTargetEnt();
	}
}
