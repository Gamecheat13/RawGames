...
updateDevSettings()
{
	...
	if ( getDvar( "scr_dump_ranks" ) != "" )
	{
		...
		//LANG_ENGLISH        "Unlocked at Private First Class (Lv1)"
		...
		for ( rId = 0; rId <= level.maxRank; rId++ )
		{
			...
			//print( "LANG_ENGLISH        \"Unlocked at " );
			//print( rankName );
	...
	if ( getdvarint("scr_complete_all_challenges") == 1 )
	{
		foreach ( player in level.players )
		{
			player thread maps\mp\gametypes\_missions::completeAllChallenges();
		}
		...
		else
		{
			switch( splashType )
			{
				...
				case "challenge":
					...
					break;
				case "defcon":
					splashName = strTok( splashName, "_"  );
					defconNumber = int( splashName[1] );
					foreach( player in level.players )
						player thread maps\mp\gametypes\_hud_message::defconSplashNotify( defconNumber, true );
					break;
				default:
					break;
			}
