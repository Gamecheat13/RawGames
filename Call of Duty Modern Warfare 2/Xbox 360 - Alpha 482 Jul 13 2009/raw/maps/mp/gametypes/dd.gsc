...
main()
{
	...
	level.dd = true;
	level.ddTimeToAdd = 2; //how much time is added to the match when a target is destroyed
...
onPrecacheGameType()
{
	...
	precacheShader("waypoint_defend_timer_a");
	precacheShader("waypoint_defend_timer_b");
	precacheShader("waypoint_defuse");
	precacheShader("waypoint_defuse_timer_a");
	precacheShader("waypoint_defuse_timer_b");
...
onStartGameType()
{
	...
	maps\mp\gametypes\_spawnlogic::addSpawnPoints( "axis", "mp_dd_spawn_defender" );
	maps\mp\gametypes\_spawnlogic::addSpawnPoints( "allies", "mp_dd_spawn_attacker" );
...
getSpawnPoint()
{
	...
	{
		if (spawnteam == game["attackers"] )
			spawnpoint = maps\mp\gametypes\_spawnlogic::getSpawnpoint_NearTeam(level.spawn_attackers);
		else
			spawnpoint = maps\mp\gametypes\_spawnlogic::getSpawnpoint_NearTeam(level.spawn_defenders);
	}
...
resetBombZone()
{
	...
	self maps\mp\gametypes\_gameobjects::set2DIcon( "enemy", "compass_waypoint_target" + self.label );
...
setUpForDefusing()
{
	...
	self maps\mp\gametypes\_gameobjects::set3DIcon( "friendly", "waypoint_defuse_timer" + self.label );
	self maps\mp\gametypes\_gameobjects::set2DIcon( "enemy", "compass_waypoint_defend" + self.label );
	self maps\mp\gametypes\_gameobjects::set3DIcon( "enemy", "waypoint_defend_timer" + self.label );
	self maps\mp\gametypes\_gameobjects::setVisibleTeam( "any" );
}
...
bombPlanted( destroyedObj, player )
{
	...
	destroyedObj.visuals[0] thread playDemolitionTickingSound();
	...
	destroyedObj BombTimerWait(destroyedObj.label); //waits for bomb to explode!
...
restartTimer( timeToAdd )
{
	if( !isDefined( timeToAdd ) )
		timeToAdd = 0;
	
	if ( level.bombsPlanted <= 0 )
	{
		maps\mp\gametypes\_gamelogic::resumeTimer( timeToAdd );
...
{
	level endon("game_ended");
	level endon("bomb_defused" + siteLoc );

	waitTime = level.bombTimer;
