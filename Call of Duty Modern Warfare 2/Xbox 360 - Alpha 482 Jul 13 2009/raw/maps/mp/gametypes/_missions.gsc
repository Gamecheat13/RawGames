...
//				player processChallenge( "ch_???" );
		...
		case "harrier_airstrike":
			player processChallenge( "ch_harrier_airstrike" );
...
ch_vehicle_kills( data )
{
	...
	if ( player _hasPerk( "specialty_dangerclose" ) && (isSubStr( data.sMeansOfDeath, "MOD_GRENADE" ) || isSubStr( data.sMeansOfDeath, "MOD_EXPLOSIVE" ) || isSubStr( data.sMeansOfDeath, "MOD_PROJECTILE" )) )
...
ch_kills( data, time )
{
	...
	if ( isDefined( player.killedPlayers[data.victim.guid] ) && player.killedPlayers[data.victim.guid] == 10 )
	...
	{
		if ( isCACSecondaryWeapon( killData.sWeapon ) )
			secondaryCount++;
	...
	else if ( isSubStr( data.sMeansOfDeath, "MOD_GRENADE" ) || isSubStr( data.sMeansOfDeath, "MOD_EXPLOSIVE" ) || isSubStr( data.sMeansOfDeath, "MOD_PROJECTILE" ) )
	{
		...
		if ( isDefined( data.victim.explosiveInfo["damageTime"] ) && data.victim.explosiveInfo["damageTime"] == time )
		{
			...
			if ( baseWeapon == "javelin" || baseWeapon == "m79" || baseWeapon == "stinger" || baseWeapon == "at4" || baseWeapon == "rpg" )
			{
			...
			if ( isDefined( data.victim.explosiveInfo["stickKill"] ) && data.victim.explosiveInfo["stickKill"] )
			{
				...
				if ( isDefined( player.finalKill ) )
					self processChallenge( "ch_stickman" );
	...
	else if ( isSubStr( data.sMeansOfDeath,	"MOD_MELEE" ) && !isSubStr( data.sweapon, "riotshield_mp" ) )
	{
		...
		if ( isDefined( player.plague ) )
			data.victim processChallenge( "ch_plague" );
		else if ( isDefined( data.victim.plague ) )
	...
	else if ( isSubStr( data.sMeansOfDeath,	"MOD_IMPACT" ) )
	{
		...
		if ( baseWeapon == "gl" )
		{
			...
			if ( isDefined( level.challengeInf["ch_" + weaponAttachments[0] + "_" + "gl"] ) )
				player processChallenge( "ch_" + weaponAttachments[0] + "_" + "gl" );
	...
	if ( data.sMeansOfDeath == "MOD_PISTOL_BULLET" || data.sMeansOfDeath == "MOD_RIFLE_BULLET" || data.sMeansOfDeath == "MOD_HEAD_SHOT" && !isKillstreakWeapon( data.sweapon ) )
	{
		...
		if ( player _hasPerk( "specialty_bulletaccuracy" ) && !player playerAds() ) // TODO: fix this... we're testing for ADS after the kill has happened
...
ch_bulletDamageCommon( data, player, time, weaponClass )
{
	...
	else if ( weaponClass == "weapon_shogun" )
	{
		if ( isSubStr( data.sWeapon, "ranger" ) && isDefined( data.modifiers["bothbarrels"] ) )
...
playerKilled( eInflictor, attacker, iDamage, sMeansOfDeath, sWeapon, sHitLoc, modifiers )
{
...
monitorMiscCallback( result )
{
	...
	switch( result )
	{
		...
		case "destroyed_explosive":		
			if ( self _hasPerk( "specialty_detectexplosives" ) )
...
processChallenge( baseName, progressInc )
{
	...
	{
		self thread masteryChallengeProcess(baseName, progressInc);  //
...
masteryChallengeProcess( baseName, progressInc )
{
	...
	potentialUnlocked = tableLookup( "mp/unlockTable.csv", 0, baseName, 4 + ((progressInc-1)*2) );
	
	if ( potentialUnlocked == "")
		return;
	
	if ( !isDefined( potentialUnlocked ) )
		return;

	switch ( potentialUnlocked )
	{
		case "reflex":
		case "silencer":
		case "acog":
		...
		case "gl":
			...
			challengeTok = strTok( baseName, "_" );
			
			for ( i = 0; i < challengeTok.size; i++ ) 
			{
				validWeapon = tableLookup( "mp/statsTable.csv", 4, challengeTok[i], 2 ); 	
				
				if ( validWeapon == "" || validWeapon == "weapon_projectile" )
					continue;
			...
			hasMastery = tableLookup( "mp/allChallengesTable.csv", 0 , "ch_" + challengeTok[foundWeapon] + "_mastery", 1 );
			
			if ( hasMastery == "" )
				return;
		
			processChallenge( "ch_" + challengeTok[foundWeapon] + "_mastery" );
...
updateChallenges()
{
	...
	foreach ( challengeRef, challengeData in level.challengeInfo )
	{
		...
		{
			println( "requirement: " + challengeData["requirement"] + " not met!" );
			continue;
		}
...
buildChallegeInfo()
{
	...
	for ( tierId = 1; tierTable != ""; tierId++ )
	{
		...
		{
			requirement = tableLookupByRow( tierTable, challengeId, 1 );
			if ( requirement != "" )
