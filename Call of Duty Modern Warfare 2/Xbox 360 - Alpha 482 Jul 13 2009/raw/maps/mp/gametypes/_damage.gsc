...
handleWorldDeath( attacker, lifeId, sMeansOfDeath, sHitLoc )
{
	...
	{
		if ( isDefined( level.onNormalDeath ) && attacker.team != "spectator" )
...
{
	attacker playLocalSound( "mp_card_slide" );
...
handleNormalDeath( lifeId, attacker, eInflictor, sWeapon, sMeansOfDeath )
{
	...
	if ( isAlive( attacker ) )
	{
		...
		if ( isDefined( level.killStreakSpecialCaseWeapons[sWeapon] ) ) // this is an optimization
		{
			switch ( sWeapon )
			{
				...
				//case "sentry_minigun_mp": // note: would need to be in level.killStreakSpecialCaseWeapons
				//case "sentry_gun_mp": // note: would need to be in level.killStreakSpecialCaseWeapons
				case "harrier_20mm_mp":
				case "pavelow_minigun_mp":
					if ( isDefined( eInflictor ) && isDefined( eInflictor.lifeId ) )
						lifeId = eInflictor.lifeId;
					else
						lifeId = attacker.lifeId;
						
					if ( lifeId == attacker.pers["deaths"] )
						attacker.pers["cur_kill_streak"]++;
					break;
				default:
					attacker.pers["cur_kill_streak"]++;
	...
	if ( isDefined( self.lastAttackedShieldPlayer ) && isDefined( self.lastAttackedShieldTime ) )
	{
...
PlayerKilled_internal( eInflictor, attacker, victim, iDamage, sMeansOfDeath, sWeapon, vDir, sHitLoc, psOffsetTime, deathAnimDuration, isFauxDeath )
{
	...
	if ( !isDefined( victim.idFlags ) )
	{
		if ( sMeansOfDeath == "MOD_SUICIDE" )
			victim.idFlags = 0;
		else if ( sMeansOfDeath == "MOD_GRENADE" && sWeapon == "frag_grenade_short_mp" && iDamage == 100000 )
			victim.idFlags = 0;
		else
			assertEx( 1, "Victims ID flags not set, but means of death was not cooked grenade: " + sMeansOfDeath  );
	}
	...
	if ( ( attacker.classname == "trigger_hurt" || attacker.classname == "worldspawn" || attacker == victim ) && isDefined( self.attackers )  )
	{
	...
	if ( !isDefined( victim.switching_teams ) && !friendlyFire )
	{
	...
	{
		handleTeamChangeDeath();
		victim notify( "playerKilledChallengesProcessed" );
	}
	...
	else
	{
		doKillcam = true;
		handleNormalDeath( lifeId, attacker, eInflictor, sWeapon, sMeansOfDeath );
		victim thread maps\mp\gametypes\_missions::playerKilled( eInflictor, attacker, iDamage, sMeansOfDeath, sWeapon, sHitLoc, attacker.modifiers );
...
getKillcamEntity( attacker, eInflictor, sWeapon )
{
	...
	if ( sWeapon == "artillery_mp" || sWeapon == "pavelow_minigun_mp" )
...
{	
	...
	else if ( sHitLoc == "shield" )
	{
		...
		if ( isPlayer( eAttacker ) && (sMeansOfDeath == "MOD_PISTOL_BULLET" || sMeansOfDeath == "MOD_RIFLE_BULLET" || sMeansOfDeath == "MOD_EXPLOSIVE_BULLET") )
		{
			...
			if ( victim.shieldDamage >= 300 )
			{
				victim thread maps\mp\gametypes\_rank::giveRankXP( "shield_damage", 50 );
	...
	/*
	else if ( victim.hasRiotShieldEquipped && isSubstr( sMeansOfDeath, "BULLET" ) && ((sHitLoc == "left_leg_lower") || (sHitLoc == "right_leg_lower") || (sHitLoc == "left_foot") || (sHitLoc == "right_foot")) )
	{
		iDamage *= 0.2;
	}
	*/
...
{
	if ( (self isUsingRemote() && getRemoteName() != "remotemissile" ) && (iDamage >= self.health) && !(iDFlags & level.iDFLAGS_STUN) )
	{
		if ( !isDefined( vDir ) )
			vDir = ( 0,0,0 );
...
{	
	...
	else if ( self _hasPerk( "specialty_c4death" ) )
	{
		...
		notifyData.iconName = "specialty_c4_death";
	}
	else
	{
...
lastStandBleedOut( reviveOnBleedOut )
{
	if ( reviveOnBleedOut )
	{
...
useHoldThinkLoop( player )
{
	...
	while( isReallyAlive( player ) && player useButtonPressed() && self.curProgress < self.useTime  )
	{
		...
		if ( self.curProgress >= self.useTime )
		{
			...
			//why would we wait a frame here?
			//wait 0.05;
