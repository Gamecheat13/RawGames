...
updateGameEvents()
{
	if ( matchMakingGame() && !level.inGracePeriod )
	{
		if ( level.teamBased )
		{
			...
			if ( (level.hasSpawned["allies"] || level.console) && level.teamCount["allies"] < 1 && level.teamCount["axis"] > 0 && game["state"] == "playing" )
			{
				//allies forfeited
			...
			if ( (level.hasSpawned["axis"] || level.console) && level.teamCount["axis"] < 1 && level.teamCount["allies"] > 0 && game["state"] == "playing" )
			{
				//axis forfeited
	...
	if ( level.teamBased )
	{
		...
		if ( !level.aliveCount["allies"] && !level.livesCount["allies"] )
		{
			return [[level.onDeadEvent]]( "allies" );
		}

		// if axis were alive and now they are not
		if ( !level.aliveCount["axis"] && !level.livesCount["axis"] )
		{
			return [[level.onDeadEvent]]( "axis" );
		}

		// one ally left
		if ( level.aliveCount["allies"] == 1 && !level.livesCount["allies"] )
		{
		...
		// one axis left
		if ( level.aliveCount["axis"] == 1 && !level.livesCount["axis"] )
		{
...
checkHalfTime()
{
	...
	if ( getTimeLimit() )
	{
		if ( getTimePassed() == (getTimeLimit() * 60 * 1000) * 0.5 )
		{
...
Callback_StartGameType()
{
	...
	if ( getDvar( "scr_teamKillPunishCount" ) == "" )
		getDvar( "scr_teamKillPunishCount", "3" );
	...
	thread maps\mp\_perks::init(); // TODO: move to new directory
...
resumeTimer( additionalTime )
{
	...
	level.discardTime += gettime() - level.timerPauseTime + additionalTime;
}
...
endGameHalfTime()
{
	...
	foreach ( player in level.players )
	{
		player freezePlayerForRoundEnd( 0.05 );
		player freezeControls( true );
