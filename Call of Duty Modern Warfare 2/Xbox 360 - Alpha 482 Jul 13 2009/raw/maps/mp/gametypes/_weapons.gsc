...
bombSquadWaiter()
{
	...
	for ( ;; )
	{
		...
		if ( weaponName == "c4_mp" )
			weaponEnt thread createBombSquadModel( "weapon_c4_bombsquad", "tag_origin", team );
		else if ( weaponName == "claymore_mp" )
			weaponEnt thread createBombSquadModel( "weapon_claymore_bombsquad", "tag_origin", team );
		else if ( weaponName == "frag_grenade_mp" )
			weaponEnt thread createBombSquadModel( "projectile_m67fraggrenade_bombsquad", "tag_weapon", team );
		else if ( weaponName == "frag_grenade_short_mp" )
			weaponEnt thread createBombSquadModel( "projectile_m67fraggrenade_bombsquad", "tag_weapon", team );
		else if ( weaponName == "semtex_mp" )
			weaponEnt thread createBombSquadModel( "projectile_semtex_grenade_bombsquad", "tag_weapon", team );
	}
}


createBombSquadModel( modelName, tagName, teamName )
{
	...
	bombSquadModel thread bombSquadVisibilityUpdater( teamName );
...
bombSquadVisibilityUpdater( teamName )
{
	self endon ( "death" );
...
handleScavengerBagPickup( scrPlayer )
{
	...
	gotSomethingGood = false;

	destPlayer notify( "scavenger_pickup" );
	destPlayer playSound( "scavenger_pack_pickup" );
	...
	{		
		...
		if ( currentClipAmmo != destPlayer GetWeaponAmmoClip( offhand ) )
			gotSomethingGood = true;		
	}

	primaryWeapons = destPlayer getWeaponsListPrimaries();	
	foreach ( primary in primaryWeapons )
	{
		//if ( !isPrimaryWeapon( primary ) )
		//	continue;
	...
	/*
	// Weapon Ammo
	primaryWeapon = scrPlayer GetCurrentWeapon();
	secondaryWeapon = scrPlayer.secondaryweapon;
	if ( isDefined( primaryWeapon ) && (primaryWeapon != "none") )
	{	
		primaryAmmo = scrPlayer GetWeaponAmmoStock( primaryWeapon ) + scrPlayer GetWeaponAmmoClip( primaryWeapon );
		
		altWeapon = weaponAltWeaponName( primaryWeapon );
		if ( isDefined( altWeapon ) && (altWeapon != "none") )
			altAmmo = scrPlayer GetWeaponAmmoStock( altWeapon ) + scrPlayer GetWeaponAmmoClip( altWeapon );
		else
			altAmmo = 0;
	}
	else
	...
	if ( isDefined( secondaryWeapon ) )
		secondaryAmmo = scrPlayer GetWeaponAmmoStock( secondaryWeapon ) + scrPlayer GetWeaponAmmoClip( secondaryWeapon );
	else
		secondaryAmmo = 0;
	assert( isDefined( secondaryAmmo ) );

	// offhand Ammo
	offhand = scrPlayer GetCurrentOffhand();
	if ( isDefined( offhand ) && (offhand != "none") )
		offhandClipAmmo = scrPlayer GetWeaponAmmoClip( offhand );
	else
		offhandClipAmmo = 0;
	...
	// Distrubute ammo
	primaryWeapon = destPlayer GetCurrentWeapon();
	if ( isDefined( primaryWeapon ) && (primaryWeapon != "none") )
	{
		currentAmmo = destPlayer GetWeaponAmmoStock( primaryWeapon );
		destPlayer SetWeaponAmmoStock( primaryWeapon, currentAmmo + primaryAmmo );
		
		altWeapon = weaponAltWeaponName( primaryWeapon );
		if ( isDefined( altWeapon ) && ( altWeapon != "none" ) )
		{
			currentAmmo = destPlayer GetWeaponAmmoStock( altWeapon );
			destPlayer SetWeaponAmmoStock( altWeapon, currentAmmo + altAmmo );
		}
	}
	
	secondaryWeapon = destPlayer.secondaryWeapon;
	if ( isDefined( secondaryWeapon ) && (secondaryWeapon != "none") )
	{
		currentAmmo = destPlayer GetWeaponAmmoStock( secondaryWeapon );
		
		if ( getWeaponClass( secondaryWeapon ) != "weapon_projectile" )
			destPlayer SetWeaponAmmoStock( secondaryWeapon, currentAmmo + secondaryAmmo );
		else
			destPlayer setWeaponAmmoClip( secondaryWeapon, 1 );
		
		altWeapon = weaponAltWeaponName( secondaryWeapon );
		if ( isDefined( altWeapon ) && ( altWeapon != "none" ) )
		{
			currentAmmo = destPlayer GetWeaponAmmoStock( altWeapon );
			destPlayer SetWeaponAmmoStock( altWeapon, currentAmmo + altAmmo );
	...
	// Offhand ammo goes directly to the clip, because we can't reload it
	offhand = destPlayer GetCurrentOffhand();
	if ( isDefined( offhand ) && (offhand != "none") )
	{
		if ( destPlayer _hasPerk( "specialty_tacticalinsertion" ) && destPlayer getAmmoCount( "lightstick_mp" ) < 1 )
		{
			destPlayer _setPerk( "specialty_tacticalinsertion");	
		}
		else
		{
			currentClipAmmo = destPlayer GetWeaponAmmoClip( offhand );
			destPlayer SetWeaponAmmoClip( offhand, currentClipAmmo + offhandClipAmmo);
		}
	}
	*/
}
...
dropScavengerForDeath( attacker )
{
	...
	// Only drop scavenger bag in cases of attackers with the scavenger perk killed us
 	if( !isDefined( attacker ) || !isPlayer( attacker ) || !attacker hasPerk( "specialty_scavenger", true ) )
 		return;
...
watchWeaponUsage( weaponHand )
{
	...
	{	
		...
		totalShots = self maps\mp\gametypes\_persistence::statGet( "totalShots" ) + 1;
		hits = self maps\mp\gametypes\_persistence::statGet( "hits" );
		self maps\mp\gametypes\_persistence::statSet( "totalShots", totalShots );
		self maps\mp\gametypes\_persistence::statSet( "accuracy", int(hits * 10000 / totalShots) );		
		self maps\mp\gametypes\_persistence::statSet( "misses", int(totalShots - hits) );
	}
}
...
checkHit( weaponName, victim )
{
	...
	totalShots = self maps\mp\gametypes\_persistence::statGet( "totalShots" );		
	hits = self maps\mp\gametypes\_persistence::statGet( "hits" ) + 1;
