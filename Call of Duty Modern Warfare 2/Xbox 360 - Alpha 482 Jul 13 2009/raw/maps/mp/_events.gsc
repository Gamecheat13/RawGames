...
init()
{
	...
	maps\mp\gametypes\_rank::registerScoreInfo( "revenge", 200 );
	...
	maps\mp\gametypes\_rank::registerScoreInfo( "longshot", 100 );
	...
	precacheShader( "skull_black_full" );
...
killedPlayer( killId, victim, weapon, meansOfDeath )
{
	...
	if ( !isKillstreakWeapon( weapon ) )
	{
		...
		if ( isSubStr( weapon, "ranger" ) && isDefined( self.bothBarrels ) )
...
checkMatchDataWeaponKills( victim, weapon, meansOfDeath, weaponType )
{
	...
	switch( weaponType )
	{
		...
		case "weapon_sniper":
			...
			self incPlayerStat( "scopedkills", 1 );
			break;
		case "weapon_shotgun":
	...
	if ( isSubStr( weapon, "acog" ) && self PlayerAds() > .5 )
		self incPlayerStat( "scopedkills", 1 );
	
	if ( isSubStr( weapon, "thermal" ) && self PlayerAds() > .5 )
		self incPlayerStat( "thermalkills", 1 );
}
