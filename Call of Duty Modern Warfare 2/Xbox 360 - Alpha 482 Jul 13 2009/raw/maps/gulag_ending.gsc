...
gulag_run_for_it()
{
	...
	delayThread( 7.5, ::flag_set, "modify_ai_moveplaybackrate" );
	...
	*/
	...
	activate_trigger_with_targetname( "friendly_cafeteria_trigger" );
	...
	level.soap dialogue_queue( "gulag_plp_onwayhome" );
	wait( 1 );
	black_overlay = create_client_overlay( "black", 0, level.player );
	black_overlay FadeOverTime( 1 );
	black_overlay.alpha = 1;
...
//	level.price delayThread( 2.2, ::anim_single_solo, level.price, "gulag_pri_doorsopen" );
	...
	wait( 4.2 );
	/*
	ent thread anim_single( guys, "cafe_runback" );
//	wait( 0.05 );
	animtimes = [];
	animtimes[ "soap" ] = 0.17;
	animtimes[ "price" ] = 0.14;
	animtimes[ "redshirt" ] = 0.17;
	
	ai = GetAIArray( "allies" );
	foreach ( guy in ai )
	{
		animation = guy getanim( "cafe_runback" );
		guy SetAnimTime( animation, animtimes[ guy.animname ] );
	}
	*/
...
//	ent anim_reach_together( priceRed, "gate" );
//	ent thread anim_single_run( priceRed, "gate" );
//	ent thread anim_single_solo( anim_post, "gate" );

//	thread price_and_redshirt_to_door( ent, priceRed, guys );	
	wait( 2 );
	flag_waitopen( "player_falls_down" );// during cafeteria ending
	/*
	cafe_price_node = GetNode( "cafe_price_node", "targetname" );
	cafe_redshirt_node = GetNode( "cafe_redshirt_node", "targetname" );
	
	level.price SetGoalNode( cafe_price_node );
	level.redshirt SetGoalNode( cafe_redshirt_node );

	wait( 4 );
	
	cavein_spawners = GetEntArray( "cavein_spawner", "targetname" );
	array_spawn_function( cavein_spawners, ::cavein_spawner_think );
	array_thread( cavein_spawners, ::spawn_ai );

	// lock soap to his spot so he doesn't go wandering;
	animation = level.soap getanim( "cafe_dustcover_idle" )[0];
	org = GetStartOrigin( ent.origin, ent.angles, animation );
	level.soap SetGoalPos( org );
	level.soap.goalradius = 4;
	
	wait( 10 );

	flag_set( "enemy_cavein" ); // exploder
	wait( 2 );

	thread price_and_redshirt_cafe_dustcover( ent );
	wait( 2 );
	
	ent anim_reach_together( priceRed, "cafe_runback" );
	
	
	thread cafe_falls_apart();

	level notify( "more_soap_dialogue" );	
	delayThread( 2, ::exploder, "runback_exploder" );
	ent notify( "stop_loop" );
	ent anim_single( guys, "cafe_runback" );
	*/

}


price_and_redshirt_to_door( ent, priceRed, guys )
{
	level endon( "player_falls_down" );// during cafeteria ending
	cafe_price_node = GetNode( "cafe_price_node", "targetname" );
	cafe_redshirt_node = GetNode( "cafe_redshirt_node", "targetname" );
...
price_and_redshirt_cafe_dustcover( ent )
{
	ent thread anim_loop_solo( level.soap, "cafe_dustcover_idle" );
	ent thread anim_single_run_solo( level.redshirt, "cafe_dustcover" );
	wait( 1.0 );
	ent anim_single_run_solo( level.price, "cafe_dustcover" );
}
...
player_gets_groundref_and_opens_fov( tag_origin, player_rig )
{
	...
	open_up_fov( 0.5, player_rig, "tag_player", 45, 45, 90, 45 );
}
