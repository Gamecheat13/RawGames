...
//	thread deboost_trigger();
	
	friendlies_keep_pace_in_front_of_player();
	...
	{	
		...
		foreach ( guy in ai )
		{
			dist = distance( guy.origin, guy.goalpos );
...
stumble_baddie()
{
	delaythread( 11.6, ::exploder, "first_hallway_collapse" );
...
price_evac_light()
{
	price_evac_light = getstruct( "price_evac_light", "targetname" );
	targ = getstruct( price_evac_light.target, "targetname" );
	angles = vectortoangles( targ.origin - price_evac_light.origin );

	tag_origin = spawn_tag_origin();
	tag_origin.origin = price_evac_light.origin;
