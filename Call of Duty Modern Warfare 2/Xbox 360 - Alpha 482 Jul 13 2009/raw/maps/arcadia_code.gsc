...
{	
	...
	chopperModel[ 1 ] = "vehicle_pavelow";
...
/*
laser_targeting_device_remove( player )
{
	player notify( "remove_laser_targeting_device" );
	
	player setWeaponHudIconOverride( "actionslot4", "none" );
...
get_laser_designated_trace()
{
	...
	end = eye + vector_multiply( forward, 5000 );
...
laser_artillery( groupNum )
{
	...
	if ( flag( "stealth_bombed_0" ) && flag( "stealth_bombed_1" ) )
...
zpu_death_gunner( zpu, gunner, trigger )
{
	...
	zpu anim_single_solo( gunner, "death", "tag_driver" );
...
zpu_gunner_dismount( zpu, gunner, trigger )
{
	...
	zpu thread anim_single_solo( gunner, "dismount" , "tag_driver" );
}
