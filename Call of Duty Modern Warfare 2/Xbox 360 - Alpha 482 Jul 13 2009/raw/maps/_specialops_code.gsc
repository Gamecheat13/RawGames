...
isDefendMatch()
{
	return( getdvar( "pmc_gametype" ) == "mode_defend" );
}
...
challenge_timer_player_setup( start_flag, passed_flag, message )
{
	...
	if ( isdefined( level.challenge_time_limit ) )
		self.hud_so_timer_time = maps\_specialops::so_create_hud_item( 1, ypos, convert_to_time_string( level.challenge_time_limit, true ), self );
	else
...
challenge_timer_give_alert()
{
	if ( challenge_timer_should_pulse() )
	{
		...
		if ( !isdefined( level.challenge_time_silent ) || !level.challenge_time_silent )
		{
...
//	radio_dialogue_stop();
	...
	setdvar( "ui_time_failure", 1 );
	maps\_specialops::so_force_deadquote( "@SPECIAL_OPS_FAILURE_HINT_TIME" );
...
challenge_timer_wait_passed( hud_msg, hud_time, passed_flag )
{
	...
	time = ( level.challenge_end_time - level.challenge_start_time ) / 1000;
	if ( isdefined( level.challenge_time_limit ) )
		time = level.challenge_time_limit - time;
	hud_time = maps\_specialops::so_create_hud_item( 1, maps\_specialops::so_hud_ypos(), convert_to_time_string( time, true ), self );
	hud_time.alignx = "left";
	if ( isdefined( self.hud_so_timer_time ) )
	{
		hud_time.color = self.hud_so_timer_time.color;
		hud_time.glowcolor = self.hud_so_timer_time.glowcolor;
		hud_time.alpha = self.hud_so_timer_time.alpha;
		self.hud_so_timer_time Destroy();
	}
...
challenge_timer_destroy( hud_msg, hud_time, destroy_immediately )
{
	...
	if ( isdefined( self.waiting_hud ) )
		self.waiting_hud thread maps\_specialops::so_remove_hud_item( destroy_immediately );
}
...
specialops_mission_over_setup_success()
{
	...
	music_play( "so_victory_" + level.so_campaign, 0.5, 0, true );

	passed_hud = maps\_specialops::so_create_hud_item( 2, 0, &"SPECIAL_OPS_CHALLENGE_SUCCESS" );
...
specialops_mission_over_setup_failure()
{
	...
	music_play( "so_defeat_" + level.so_campaign, 0.5, 0, true );

	passed_hud = maps\_specialops::so_create_hud_item( 2, 0, &"SPECIAL_OPS_CHALLENGE_FAILURE" );
	passed_hud.alignX = "center";
	passed_hud.horzAlign = "center";
...
wait_all_players_are_touching( trigger_ent )
{
	for ( ;; )
	{
		...
		{
			thread display_waiting_message( level.player2, &"SPECIAL_OPS_WAITING_OTHER_PLAYER", 0.5 );
			continue;
		}
			
		if ( !level.player2 IsTouching( trigger_ent ) )
		{
			thread display_waiting_message( level.player, &"SPECIAL_OPS_WAITING_OTHER_PLAYER", 0.5 );
			continue;
		}
...
wait_all_players_have_touched( trigger_ent, touch_style )
{
	...
	for ( ;; )
	{
		...
		if ( level.player IsTouching( trigger_ent ) )
		{
			...
			{
				thread determine_waiting_message( level.player, &"SPECIAL_OPS_WAITING_OTHER_PLAYER", touch_style );
				//thread determine_waiting_message( level.player, &"SPECIAL_OPS_WAITING_P1", touch_style );
				continue;
			}
		}
		...
		if ( level.player2 IsTouching( trigger_ent ) )
		{
			...
			{
				thread determine_waiting_message( level.player2, &"SPECIAL_OPS_WAITING_OTHER_PLAYER", touch_style );
				//thread determine_waiting_message( level.player2, &"SPECIAL_OPS_WAITING_P1", touch_style );
				continue;
			}
		}
...
determine_waiting_message( player, message, touch_style )
{
	switch ( touch_style )
	{
		case "all"		: display_waiting_message( player, message, 0.5 ); break;
		case "any"		: display_waiting_message( player, message ); break;
		case "freeze"	: display_frozen_message( player, message ); break;
	}
}

display_waiting_message( player, message, expire_time )
{
	level endon( "challenge_timer_passed" );
	level endon( "challenge_timer_expired" );
	...
	// If we aren't displaying a message yet, then simply create it.
	// If we are displaying a message then we just want to restart the timer trying to remove it.
	if ( !isdefined( level.players_touching_hint ) )
	{
		player.waiting_hud = create_waiting_message( player, message );
		level.players_touching_hint = message;
	}
	else
	{
		// Conditions may have changed, so update the message if necessary.
		if ( level.players_touching_hint != message )
			player.waiting_hud.label = message;

		if ( expire_time > 0 )
		{
			wait expire_time;
			player.waiting_hud FadeOverTime( 0.5 );
			player.waiting_hud.alpha = 0;
			level.players_touching_hint = undefined;
		}
	}
}

display_frozen_message( player, message )
{
	// Don't need to fade out again if already faded.
	if ( isdefined( player.frozen_and_waiting ) && player.frozen_and_waiting )
		return;
		
	player.frozen_and_waiting = true;
	player EnableInvulnerability();
	...
	waiting_hud = create_waiting_message( player, message );
	
	flag_wait_any( "challenge_timer_expired", "challenge_timer_passed" );

	waiting_hud Destroy();
}	

create_waiting_message( player, message )
{
	waiting_hud = maps\_specialops::so_create_hud_item( 0, 0, message, player );
	waiting_hud.alignX = "center";
	waiting_hud.horzAlign = "center";
	waiting_hud set_hudelem_blue();

	waiting_hud.alpha = 0;
	waiting_hud FadeOverTime( 0.5 );
	waiting_hud.alpha = 1;
...
so_dialog_play( dialog, wait_time )
{
...
so_special_death_hint_tracker()
{
	...
	if ( isdefined( level.coop_death ) && level.coop_death )
	{
		if ( level.coop_death_reason[ "id" ] == my_id )
		{
			attacker = level.coop_death_reason[ "attacker" ];
			cause = level.coop_death_reason[ "cause" ];
		}
	}
...
so_dog_death( attacker )
{
	...
	setdvar( "ui_dog_death", 1 );
	switch( level.dog_death_type )
	{
		case "soon":	so_special_failure_hint_set( "@NEW_DOG_DEATH_TOO_SOON_ALT" ); break;
		case "late":	so_special_failure_hint_set( "@NEW_DOG_DEATH_TOO_LATE_ALT" ); break;
		default:		so_special_failure_hint_set( "@NEW_DOG_DEATH_DO_NOTHING_ALT" ); break;
	}
...
so_destructible_death( attacker, cause )
{
	...
	if ( issubstr( attacker.destructible_type, "vehicle" )  )
	{
		...
		setdvar( "ui_vehicle_death", 1 );
		so_special_failure_hint_set( "@SCRIPT_EXPLODING_VEHICLE_DEATH" );
	}
	else
	{
		// You were killed by an explosion.\nSome burning objects can explode.
		setdvar( "ui_destructible_death", 1 );
		so_special_failure_hint_set( "@SCRIPT_EXPLODING_DESTRUCTIBLE_DEATH" );
	}
...
so_exploding_barrel_death( cause )
{
	...
	setdvar( "ui_barrel_death", 1 );
	so_special_failure_hint_set( "@SCRIPT_EXPLODING_BARREL_DEATH" );
...
//		return false;
	...
	setdvar( "ui_grenade_death", 1 );
	so_special_failure_hint_set( "@SCRIPT_GRENADE_DEATH" );

	return true;
}

so_special_failure_hint_set( quote )
{
	maps\_specialops::so_force_deadquote( quote );
	level notify( "so_special_failure_hint_set" );
}

so_special_failure_hint_set_array( quotes )
{
	maps\_specialops::so_force_deadquote_array( quotes );
	level notify( "so_special_failure_hint_set" );
}
