...
escape_music()
{
	...
	while ( 1 )
	{
		MusicPlayWrapper( "cliffhanger_escape" );
		wait 114;
	}
}
...
satellite_music_loop()
{
	...
	while ( 1 )
	{
		MusicPlayWrapper( "cliffhanger_satellite" );
...
//level.strings[ "use_satelite_obj" ]		 = &"CLIFFHANGER_OBJ_USE_SATELITE";
	...
	// Download the files.
	registerObjective( "obj_satellite", &"CLIFFHANGER_OBJ_COMPUTER", dsm.origin );
...
objective_snowmobile()
{
	...
	Objective_Add( 9, "current", &"CLIFFHANGER_OBJ_EVACUATE", (0,0,0) );
	Objective_Current( 9 );
...
setObjectiveOnEntity( objName, ent, offset )
{
	objective = level.objectives[ objName ];
	if ( isdefined( offset ) )
		Objective_OnEntity( objective.id, ent, offset );
	else
		Objective_OnEntity( objective.id, ent );	
}
...
dialog_goto_hanger()
{
	...
	if ( !flag( "_stealth_spotted" ) )
	{
		...
		thread play_Sound_Over_Radio( "cliff_pri_radiotraffic", true );

		//Got it. Sounds like the satellite's in the far hangar.	
		thread play_Sound_Over_Radio( "cliff_pri_infarhangar", true );

		//Race you there. Oscar Mike. Out.	
		thread play_Sound_Over_Radio( "cliff_pri_oscarmike" );
	}
	else
	{
...
should_break_activate_heartbeat()
{
	...
	return level.player GetCurrentWeapon() == level.motiontrackergun_on || !has_masada;
}
...
enemies_rush_or_die()
{
	...
	foreach ( enemy in enemies )
	{
		...
		if ( enemy enemy_lives() )
		{
			...
			enemy.goalradius = 32;
			enemy SetGoalPos( level.price.origin );
