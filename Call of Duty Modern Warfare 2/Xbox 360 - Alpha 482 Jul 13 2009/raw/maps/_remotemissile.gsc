...
remove_uav_weapon()
{
// TODO: Waiting for icon art
//	self SetWeaponHudIconOverride( "actionslot4", "huw_grenadeicon" );
...
weapon_change_timeout()
{
	self endon( "stop_weapon_change_timeout" );

	self thread weapon_change_timeout_thread();
	wait( 1.5 );

	self.remote_missile_weapon = undefined;
}

weapon_change_timeout_thread()
{
	self waittill( "weapon_change", weap );
	self.remote_missile_weapon = weap;

	self notify( "stop_weapon_change_timeout" );
}
...
RemoteMissileDetonatorNotify()
{
	...
	self NotifyOnPlayerCommand( "switch_to_remotemissile", "+actionslot 4" );
	...
	for ( ;; )
	{
		...
		self weapon_change_timeout();

		if ( is_remote_missile_weapon( self.remote_missile_weapon ) )
		{
			weap = self.remote_missile_weapon;
			if( Isdefined( level.uav_is_destroyed ) )
			{
				thread remotemissile_offline( false, "uav_down" );
...
remotemissile_offline( extra_check, alias )
{
	...
	{
		level.last_uav_offline_time = GetTime();
...
UAVRemoteLauncherSequence( player, weap )
{
	if( weap == "remote_missile_detonator" )
	{
		player givemaxammo( "remote_missile_detonator" );
	}
...
draw_target()
{
	...
	{
		target_setShader( self, "hud_fofbox_self" );
	}
	else
	{
