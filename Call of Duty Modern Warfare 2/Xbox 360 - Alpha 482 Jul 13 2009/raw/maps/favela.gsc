...
- Transformers shouldn't kill player
- Level turns off after you clear area where you flank right on bridge then big fight starts back up as you round corner to RPG guys. Roof drones would help tie together.   CHAD.
- Meat is down and Soap saying move on is too cold and unrealistic.  STEVE/ZIED/CHAD 
...
main()
{
	...
	setsaveddvar( "ai_accuracy_attackerCountDecrease", 0.7 );
	...
	precacheString( &"FAVELA_FAUST_KILLED" );
	...
	flag_init( "faust_appearance_2_done" );
	flag_init( "faust_appearance_3_done" );
...
favela_music()
{
	...
	{
		thread music_play( "favela_tension" );
		wait 115;
	}
}
...
upper_village_music()
{
	...
	{
		thread music_play( "favela_uppervillage_start", 2.0 );
		wait 130;
	}
}
...
faust_music()
{
	...
	{
		thread music_play( "favela_moneyrun" );
		wait 30;
	}
}
...
faust_appearance_2()
{
	...
	level waittill( "faust_deleted" );
	
	flag_set( "faust_appearance_2_done" );
}

faust_appearance_3()
{
	flag_wait( "faust_appearance_2_done" );
	...
	flag_set( "faust_appearance_3_done" );
}
...
{
	flag_wait( "faust_appearance_2_done" );
	flag_wait( "faust_appearance_3_done" );
...
ending_sequence()
{
	...
	// wait for the player to look at the sequence, or timeout
...
soap_final_dialog( guy )
{
	flag_wait( "start_final_dialog" );
	
	// Frontrunner, this is Bravo Six. We've got the package.
	guy anim_single_solo( guy, "favela_cmt_gotpackage" );
}

ending_sequence_ghost()
{
	ghost = spawn_targetname( "ending_ghost_spawner", true );
	ghost set_ignoreme( true );
	ghost set_ignoreall( true );
	ghost thread magic_bullet_shield();
}
