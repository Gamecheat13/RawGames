...
vehicle_kill()
{
	...
	while ( 1 )
	{
		...
		self array_levelthread( self.riders, maps\_vehicle_aianim::guy_vehicle_death, attacker );
...
helicopter_crash( attacker, cause )
{
	...
	if ( IsDefined( attacker ) && IsPlayer( attacker ) && IsDefined( self.rocket_destroyed_for_achievement ) && level.player _hasweapon( "rpg" ) )
...
tread( tagname, side, relativeOffset, secondTag )
{
	self endon( "death" );
