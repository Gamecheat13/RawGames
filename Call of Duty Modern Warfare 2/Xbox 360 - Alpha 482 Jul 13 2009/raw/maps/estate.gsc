...
{	
    ...
    foreach( index, breach in exteriorBreaches )
    {
    	objective_additionalPosition( 2, index, breach.origin );
    }
    ...
    foreach( index, breach in interiorBreaches )
    {
    	if( index == 0 )
    	{
    		objective_position( 2, breach.origin );
    	}
    	else
    	{
    		objective_additionalPosition( 2, index, breach.origin );
...
objective_interior_breach_update( breachnum )
{
	...
	{
		objective_delete( 2 );
		spot = getent( "objective_location_breach", "targetname" );
		objective_add( 2, "current", &"ESTATE_OBJ_BREACH", spot.origin );
	}
}
...
ending_moments()
{
	...
	{	
		...
		level.player thread ending_drag_slackerfail();
	...
	if( !flag( "test_ending_body_toss" ) )
	{
		...
		level.player LerpViewAngleClamp( 5, 1, 1, 30, 30, 30, 15 );
		...
		//"NO!"
		ghost delaythread( 1.25, ::dialogue_queue, "est_gst_no" );
