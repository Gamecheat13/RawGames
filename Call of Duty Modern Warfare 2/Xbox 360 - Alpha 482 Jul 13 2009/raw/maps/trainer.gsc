...
main()
{
	...
	level.timeFail = 120;		//anything higher than this will fail
	level.timeEasy = 70;		//anything above this number will choose "easy"
	level.timeRegular = 45;		//anything above this number but less than timeEast will choose "regular"
	level.timeHard = 30;		//anything above this number but less than timeRegular will choose "hard"
	...
	level.timepenaltyForMissedEnemies = 5; //10 seconds
	level.timepenaltyForKilledCivvies = 5; //20 seconds
	...
	if ( is_specialop() )
	{
		add_start( "so_killspree_trainer_start_map", maps\so_killspree_trainer::start_map ); 
		default_start( maps\so_killspree_trainer::start_map ); 
	}
	else
	{
		default_start( ::start_default );pit = getent( "pit", "targetname" );
	}
	init_precache();
	...
	level.trainingactors = [];
	level.trainingactors[ 0 ] = level.translator;
	level.trainingactors[ 1 ] = level.trainee_01;
	level.traineeanimnode thread anim_loop_solo( level.translator, "training_intro_idle", "stop_idle" );
...
firing_range_anims_trainee_and_translator()
{
	level.traineeanimnode notify( "stop_idle" );
	level.traineeanimnode thread anim_loop_solo( level.translator, "training_intro_idle", "stop_idle" );
	level.traineeanimnode anim_single_solo( level.trainee_01, "training_intro_begining" );
...
//	}
	...
	tooslow_dialog = [];
	tooslow_dialog[ 0 ] = "train_fly_tooslow";//Sgt. Foley  Too slow. Do it faster this time.	
	tooslow_dialog[ 1 ] = "train_fly_speeditup";//Sgt. Foley  Speed it up. Do it again.	
	tooslow_dialog[ 2 ] = "train_fly_acquirenew";//Sgt. Foley  Private Allen, pop in and out of aiming down your sights to acquire new targets. 	
	tooslow_dialog[ 3 ] = "train_fly_poppinginandout";//Sgt. Foley  Private, show these men how to acquire new targets quickly by popping in and out of aiming down your sights.	
	...
	while ( 1 )
	{
		...
		level.foley dialogue_execute( tooslow_dialog[ dialogueLine ] );
		
		dialogueLine++;
		if ( dialogueLine > tooslow_dialog.size )
		{
			dialogueLine = 0;
		}
		wait 1;
...
go_to_pit_anims()
{
	...
	level.traineeanimnode anim_single( level.trainingactors, "training_intro_end" );
	
	level.foley.animnode thread anim_loop_solo( level.foley, "training_intro_end_idle", "stop_idle" );
	level.traineeanimnode anim_loop( level.trainingactors, "training_intro_end_idle", "stop_idle" );
}
...
AA_find_the_pit_sequence()
{
	...
	level.foley dialogue_execute( "train_fly_thanksforhelp" );
	...
	level.foley thread dialogue_execute( "train_fly_gofirst" );
...
AA_pit_sequence()
{
	...
	//Cpl. Dunn  Ok, grab any weapon you like and head on in. Timer starts as soon as the first target pops.	
	//level.pitguy thread dialogue_execute( "train_cpd_anyweapon" );
	...
	level.pitguy thread dialogue_execute( "train_cpd_smileforcameras" );
...
//			}
		...
		thread dialogue_nag_start_course();
		...
		SPRINT TO FINISH
		-------------------------*/	
		flag_wait_either( "player_course_end_03", "course_end_targets_dead" );
		...
		level.gate_cqb_exit thread door_close();
		...
		if ( level.recommendedDifficulty != 1000 )
		{
			...
			level.pitguy thread dialogue_execute( "train_cpd_runagain" );
...
dialogue_course()
{
	...
	else
	{
		if ( flag( "can_talk" ) )
		{
			...
			radio_dialogue( "train_cpd_lefttargets" );
			flag_set( "can_talk" );	
		}
	}
	...
	LAST AREA
	-------------------------*/	
	flag_wait( "player_course_end_01a" );
	...
	if ( flag( "can_talk" ) )
	{
		...
		flag_clear( "can_talk" );
		radio_dialogue( "train_cpd_sprint" );
		flag_set( "can_talk" );	
	}
}
...
{
	level endon( "player_exiting_course" );
	dialogue = [];
...
dialogue_nag_start_course()
{
	...
	while( true )
	{
		...
		level.pitguy thread dialogue_execute( "train_cpd_timerstarts" );
		
		wait( 60 );
		//Cpl. Dunn	Private Allen, we don't have all day. Get in The Pit.	
		level.pitguy thread dialogue_execute( "train_cpd_donthaveallday" );
		
		
		wait( 60 );
		//Cpl. Dunn	You're gonna get us both in trouble with the Colonel man. Get in The Pit and run the course.	
		level.pitguy thread dialogue_execute( "train_cpd_bothintrouble" );
...
end_sequence()
{
	...
	thread radio_end();
	array_call( guys_to_delete,::delete );
...
radio_end()
{
	...
	org play_sound_in_space( "train_fly_movingout" );

	org = getfarthest( level.player.origin, radio_orgs );
	//Ranger 2  They blew the damn bridge! We gotta move!	
	org play_sound_in_space( "train_ar2_blewthebridge" );
	
	org = getfarthest( level.player.origin, radio_orgs );
	//Ranger 1  BCT One is trapped across the river in the red zone! We've lost contact!	
	org play_sound_in_space( "train_ar1_trapped" );
}
...
target_think( targetType )
{
	...
	if ( ( isdefined( self.script_parameters ) ) && ( self.script_parameters == "use_rail" ) )
	{
		...
		// move targets twice as fast for so training of 
		if ( is_specialop() && level.gameSkill == 3 )
			self.lateralMoveTime = ( dist / 44 );
...
//				}
				...
				if ( targetType == "friendly" )
				{
					...
					if ( is_specialop() )
					{
						...
						{
							if ( isdefined( player.HUDcivviesKilled ) )
							{
								player.HUDcivviesKilled setValue ( level.friendlies_hit );
								player.HUDcivviesKilled.color = level.color[ "red" ];
								player.HUDcivvies.color = level.color[ "red" ];
							}
						}
				...
				else
				{
					...
					if ( is_specialop() )
					{
						foreach ( player in level.players )
						{
							if ( isdefined( player.HUDenemiesKilled ) )
							{
...
target_lateral_movement()
{
	...
	dummy thread delete_when_player_too_close();
	//self.orgEnt moveTo( self.lateralEndPosition.origin, self.lateralMoveTime );
	self waittill( "target_going_back_down" );
	dummy delete();
	//thread debug_message( "lateralEndPosition", undefined, 9999, self.lateralEndPosition );
	//wait( self.lateralMoveTime );
	//self target_reset_manual();

}

delete_when_player_too_close()
{
...
//	registerActionBinding( "crouch",			"+movedown",			&"TRAINER_HINT_CROUCH" );
...
ai_ambient_noprop_think()
{
	...
	switch( sAnim )
	{
		...
		case "afgan_caves_sleeping_guard_idle":
			self.eAnimEnt.origin = self.eAnimEnt.origin + ( 0, 0, -19 );
			break;
		...
		case "cliffhanger_welder_wing":
			...
			thread flashing_welding( self, "tag_tip_fx" );
...
flashing_welding( welder, tag )
{
	...
	welder thread play_loop_sound_on_entity( "scn_trainer_welders_working_loop" );
...
{	
	...
	HOW MUCH AMMO USED?
	-------------------------*/	
	bullets_used_by_player = starting_ammo - ending_ammo;
...
dialogue_ambient()
{
	...
	org = getfarthest( level.player.origin, conversation_orgs );
...
dialogue_ambient_pit_course()
{
	...
	org = getfarthest( level.player.origin, conversation_orgs_pit );
