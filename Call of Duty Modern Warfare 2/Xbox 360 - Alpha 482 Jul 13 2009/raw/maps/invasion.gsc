...
//	array_thread( collins, ::add_spawn_function, ::set_threatbias_group, "nates_defenders" );
	...
	add_hint_string( "hint_predator_drone_vs_bmps", &"HELLFIRE_USE_DRONE", ::should_break_use_drone_vs_bmps );
...
handler_house_destroyer_to_pizza()
{
	...
	thread friendlies_move_to_alley();
...
dialog_wells_intro()
{
	//level.raptor.ignoreall = true;
	//level.wells.ignoreall = true;
	//level.wells waittill_entity_in_range( level.raptor, 500 );
	
	//level.raptor setLookAtEntity( level.wells );
	
	//Seal Six-One:	Marine! Gimme a sitrep! Where's the President?	
	level.raptor dialogue_queue( "inv_six_gimmesitrep" );
	
	//level.wells setLookAtEntity( level.raptor );
	...
	//level.raptor setLookAtEntity();//clears it
	//level.wells setLookAtEntity();//clears it
	
	flag_set( "wells_intro_done" );
	//level.raptor.ignoreall = false;
	//level.wells.ignoreall = false;
}
...
handler_convoy()
{
	...
	level.player waittill_entity_in_range( level.convoy, 3000 );
...
smart_roaming_barney()
{
	...
	while( 1 )
	{
		if( isdefined( level.obj_pos ) )
		{
			...
			forward = (forward[0], forward[1], 0);//zero out the z to keep goal on the ground
			goal = forward + player;
		}
		else
...
dialog_handle_predator_infantry_kills()
{
	...
	while( 1 )
	{
		...
		kills = level.enemies_killed - old_num;
...
friendlies_move_to_alley()
{
	...
	if( !flag( "entered_alley" ) )
		activate_trigger_with_targetname( "friendlies_move_to_alley" );
}
...
dialog_time_to_destroy_BMPS()
{
	...
	level.player thread display_hint( "hint_predator_drone_vs_bmps" );
...
hellfire_attacks()
{
	...
	rocket = MagicBullet( "remote_missile_not_player", ( level.uav.origin + (0,0,-128) ), first_tgt.origin );
	...
	rocket = MagicBullet( "remote_missile_not_player", ( level.uav.origin + (0,0,-128) ), targetpos.origin );
	wait ( randomfloatrange( 3, 5 ) );
	...
	if( flag( "player_on_roof" ) )
	{
		...
		rocket = MagicBullet( "remote_missile_not_player", ( level.uav.origin + (0,0,-128) ), rocket_target );
...
hellfire_attacks_after_player_got_off_roof()
{
	...
	while( 1 )
	{
		...
		rocket = MagicBullet( "remote_missile_not_player", ( level.uav.origin + (0,0,-128) ), target_org );
		array_thread( ceiling_dust, ::drop_dust );
	}
}
...
dialog_semtex_that_bmp()
{
	...
	thread watch_for_semtex_throws();
	...
	while( 1 )
	{
		...
		if( player_has_semtex )
		{
			if( !flag( "threw_semtex" ) )
			{
				...
				level.player thread display_hint_timeout( "hint_throw_semtex", 5 );
			}
		}
		else
		{
			level.player thread display_hint_timeout( "hint_get_semtex", 5 );
...
setup_predator_deaths()
{
	...
	{
		if( ( cause != "MOD_PROJECTILE_SPLASH" ) || !isplayer( attacker ) || !isdefined( attacker.fired_hellfire_missile ) )
			self.skipDeathAnim = undefined;
	}
	//println( "cause: " + cause );
}
...
paradrops_ambient()
{
	...
	while( 1 )
	{
		...
		if( !isdefined( planes ) )
		{
			...
			println( "     %%%% ambient paradrop: " + drop_groups[selected] );
		...
		foreach( plane in planes )
		{
			...
			else
			{
				...
				//jets only fly over once
				drop_groups = array_remove( drop_groups, drop_groups[selected] );
			}
		}
		wait 20;//was 30
	}
...
setObjectiveLocation_nearest_enemy( objName )
{
	...
	while( objective.state != "done" )
	{
		...
		if( enemies.size < 1 )
		{
			level.obj_pos = level.player.origin;
			objective_position( objective.id, level.obj_pos );
			wait 3;
		}
		else
