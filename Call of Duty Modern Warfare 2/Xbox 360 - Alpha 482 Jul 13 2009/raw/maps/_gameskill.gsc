...
solo_player_in_special_ops()
{
	...
	// The delay before you regen health after getting hurt
	level.difficultySettings[ "playerHealth_RegularRegenDelay" ][ "normal" ] = 900;
	level.difficultySettings[ "playerHealth_RegularRegenDelay" ][ "hardened" ] = 900;
	level.difficultySettings[ "playerHealth_RegularRegenDelay" ][ "veteran" ] = 900;

	// Death Invulnerable Time controls how long the player is death-proof after going into red flashing
	// This protection resets after the player recovers full health.
	level.difficultySettings[ "player_deathInvulnerableTime" ][ "normal" ] = 1600;
	...
	level.difficultySettings[ "playerDifficultyHealth" ][ "normal" ] = 295;
	level.difficultySettings[ "playerDifficultyHealth" ][ "hardened" ] = 240;
...
setSniperAccuracy()
{
	...
	*/
	...
	//dont do this if your an ally of the player, changed by z for Price cliffhanger sniping
	if ( self.team != "allies" && ( !isDefined( self.lastMissedEnemy ) || self.enemy != self.lastMissedEnemy ) && DistanceSquared( self.origin, self.enemy.origin ) > 500 * 500 )
	{
		// miss
		self.accuracy = 0;
...
{
	mods = [];
	mods[ "MOD_GRENADE" ] = ::grenade_dirt_on_screen;
	mods[ "MOD_GRENADE_SPLASH" ] = ::grenade_dirt_on_screen;
	mods[ "MOD_PROJECTILE" ] = ::grenade_dirt_on_screen;
	mods[ "MOD_PROJECTILE_SPLASH" ] = ::grenade_dirt_on_screen;
	mods[ "MOD_EXPLOSIVE" ] = ::grenade_dirt_on_screen;
...
reduceTakeCoverWarnings()
{
	...
	if ( !self takeCoverWarningsEnabled() )
		return;
...
healthOverlay()
{
	...
	pulseTime = 0.8;
//	thread overlay_non_flashing_alpha( overlay );
...
redFlashingOverlay( overlay )
{
	// THIS FUNCTION ISNT CALLED ANYMORE
	Assert( IsPlayer( self ) );
...
setTakeCoverWarnings()
{
	...
	isPreGameplayLevel = ( level.script == "killhouse" || level.script == "cliffhanger" );
...
{
	// TODO: separate cover warnings out per player?
	...
	if ( !level.player takeCoverWarningsEnabled() )
		return;
