...
start_deck1()
{
	...
	MusicPlayWrapper( "oilrig_suspense_01_music" );
	flag_set( "player_ready_to_be_helped_from_water" );
}
...
dialogue_rig_to_first_breach()
{
	...
	//zzzz to do need MacTavish "clear"
	//thread delaythread( 1,::radio_dialogue( "oilrig_roomclear_soap_03" );
...
upper_room_setup()
{
	...
	ambush_room_light = getent( "ambush_room_light", "targetname" );
	assert( isdefined( ambush_room_light ) );
	//ambush_room_light setLightIntensity( 0 );
	///xxx turn this light off after c4 triggered
}
...
ambush_sequence()
{
	...
	ac4Friendlies[ 0 ] = level.friendly03;
	...
	thread radio_dialogue( "oilrig_nsl_goingloud" );
	...
	-------------------------*/
	//level.friendly02 forceUseWeapon( "saw", "primary" );
...
{
	self endon( "death" );
	self waittill_any( "damage", "enemy", "alerted" );
}
...
heli_kill_nag( eHeli, sEndFlag )
{
	...
	while ( !flag( sEndFlag ) )
	{
		...
		if ( !level.player player_using_missile() )
		{
			...
			iLastDialogiLastDialogueNumberNoRocketueNumber = iDialogueNumberNoRocket;
		}
		else
		{
...
smoke_throw_old( aSmokeOrgs )
{
	dude = get_closest_ai( level.player.origin, "allies" );
	smokeTarget = undefined;
	...
	{
		dude.grenadeAmmo++ ;
		smokeTarget = getent( org.target, "targetname" );
		dude MagicGrenade( org.origin, smokeTarget.origin, 2 );
		smokeTarget thread play_sound_in_space( "grenade_bounce_concrete" );
...
c4_barrels_think()
{
	...
	{
		eDamageTrigger waittill( "damage", damage, attacker, direction_vec, point, type );
		if ( ( isdefined( attacker ) ) && ( isplayer( attacker ) ) )
		{
...
escape_sequence()
{
	...
	flag_set( "sub_comes_through_ice" );
...
obj_hostages_secure()
{
	...
	objective_additionalposition( objective_number, 1, obj_positions[ 0 ].origin );
	objective_additionalposition( objective_number, 2, obj_positions[ 1 ].origin );
	// Secure hostages ( &&1 remaining )
	Objective_String( objective_number, &"OILRIG_OBJ_HOSTAGES_SECURE", level.totalHostages );
	
	flag_wait( "lower_room_breached" );
	objective_additionalposition( objective_number, 1, ( 0, 0, 0 ) );
	...
	Objective_OnEntity( objective_number, level.friendly02, ( 0, 0, 70 ) );
	Objective_SetPointerTextOverride( objective_number, &"OILRIG_OBJ_GHOST" );
	...
	Objective_String( objective_number, &"OILRIG_OBJ_HOSTAGES_SECURE", 0 );
	objective_state( objective_number, "done" );
}
...
AI_axis_think()
{
	...
	switch( level.gameSkill )
	{
		...
		case 1:// regular
			...
			baseaccuracyFactor = 2;
			break;
		case 2:// hardened
			doorFlashChanceFactor = 1.5;
			baseaccuracyFactor = 2.5;
			break;
		case 3:// veteran
			doorFlashChanceFactor = 1.5;
			baseaccuracyFactor = 3;
			break;
	}
...
dialogue_breach_nag( iBreachGroup )
{
	...
	while ( true )
	{
		if ( iBreachGroup == 300 )
		{
			//don't nag if there are enemies that still need to be cleared
			breach_safe_volumes = getentarray( "breach_safe_volume", "targetname" );
			if ( !volumes_clear_of_enemies( breach_safe_volumes ) )
			{
				wait( 3 );
				continue;
			}
		...
		radio_dialogue( "oilrig_nsl_intopostion" );
