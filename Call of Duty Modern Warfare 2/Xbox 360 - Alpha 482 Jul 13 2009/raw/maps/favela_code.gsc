...
adjustAccuracy()
{
	...
	foreach( spawner in spawners )
		spawner.script_accuracy = ENEMY_BASE_ACCURACY;
}
...
ignore_and_delete_on_goal()
{
	self thread set_ignoreme( true );
	self thread set_ignoreall( true );
	self thread delete_ai_at_goal();
}

faust_spawn_func()
{
	// he drops money and runs his path. When he gets to the end of his path he gets deleted
	self endon( "death" );
	...
	self thread faust_spills_money();
	self thread faust_mission_fail();
	
	self waittill( "reached_path_end" );
	
	level notify( "faust_deleted" );
...
faust_mission_fail()
{
	...
	setdvar( "ui_deadquote", "@FAVELA_FAUST_KILLED" );
...
try_balcony_death()
{
	...
	// if one just happened within 20 seconds dont do it
	if ( elapsedTime < 20 * 1000 )
		return false;
	
	// if it's been more than 80 seconds force the balcony death
	doBalcony = undefined;
...
{	
	...
	sound[ 2 ] = "scn_favela_car_traffic_skid1";
