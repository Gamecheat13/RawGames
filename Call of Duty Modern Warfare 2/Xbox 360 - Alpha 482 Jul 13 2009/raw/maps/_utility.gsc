...
convert_to_time_string( timer, show_tenths )
{
	...
	tenths_float = ( timer - ( minutes * 60 ) - seconds ) * 10;
	tenths_int = Int( tenths_float );
	if ( ( tenths_float - tenths_int ) >= 0.5 )
	{
		tenths_int++;
		if ( tenths_int >= 10 )
		{
...
============= 
///ScriptDocBegin
"Name: getNearestDist( <org> , <array> , <dist> )"
"Summary: Returns the distance to the closest entity in < array > to location < org > "
"Module: Distance"
"CallOn: "
"MandatoryArg: <org> : Origin to be closest to."
"MandatoryArg: <array> : Array of entities to check distance on"
"OptionalArg: <dist> : Minimum distance to check"
"Example: distance = getNearestDist( level.player.origin, allies );"
"SPMP: singleplayer"
///ScriptDocEnd
============= 
*/ 
getNearestDist( org, array, dist )
{
	return compareSizesDistance( org, array, dist, ::closerFunc );
}
...
compareSizesDistance( org, array, dist, compareFunc )
{
	if ( !array.size )
		return undefined;
	if ( IsDefined( dist ) )
...
=============
///ScriptDocBegin
"Name: force_crawling_death( <num_crawls> )"
"Summary: "
"Module: Entity"
"CallOn: An entity"
"MandatoryArg: <param1>: "
...
=============
///ScriptDocBegin
"Name: anim_stopanimscripted( <anim_stopanimscripted> )"
"Summary: "
"Module: Entity"
"CallOn: An entity"
"MandatoryArg: <param1>: "
...
=============
*/
waittill_player_lookat( dot, timer, dot_only, timeout, ignore_ent )
{
	...
	for ( ;; )
	{
		...
		if ( player_looking_at( org, dot, dot_only, ignore_ent ) )
		{
			count--;
			if ( count <= 0 )
				return true;
...
waittill_player_lookat_for_time( timer, dot, dot_only, ignore_ent )
{
	...
	waittill_player_lookat( dot, timer, dot_only, ignore_ent );
}
	...
	while ( aAI_to_delete.size > 0 )
	{
		...
		for ( i = 0; i < aAI_to_delete.size; i++ )
		{
			...
			if ( players_within_distance( fDist, aAI_to_delete[ i ].origin ) )
				continue;
			if ( players_looking_at( aAI_to_delete[ i ].origin + ( 0, 0, 48 ), true ) )
				continue;
			/*-----------------------
...
//	level._effect[ "footstep_snow" ]					= LoadFX( "impacts/footstep_snow" );
	...
	for ( ;; )
	{
		...
		mydistance = Distance( level.player GetVelocity(), ( 0, 0, 0 ) );
		...
		if ( Distance( level.player GetVelocity(), ( 0, 0, 0 ) ) <= 154 )
		{
...
=============
///ScriptDocBegin
"Name: music_loop( <name> , <length>, <fade_time> )"
...
=============
*/
music_loop( name, length, fade_time, timescale, overrideCheat )
{
	thread music_loop_internal( name, length, fade_time, timescale, overrideCheat );
}
...
player_is_near_live_grenade()
{
	...
	for ( i = 0; i < grenades.size; i++ )
	{
		...
		for ( playerIndex = 0; playerIndex < level.players.size; playerIndex++ )
		{
			...
			if ( Distance( grenade.origin, player.origin ) < 250 )// grenade radius is 220
			{
...
"Summary: returns difficulty in "easy","medium", "hard", or "fu" format. note that each player has their own difficulty, this is just the average "
"Module: Utility"
"CallOn: Level"
"Example: "
"SPMP: singleplayer"
...
get_other_player( player )
{
	...
	AssertEx( 0, "Tried to get other player but there is no other player" );
}
...
{
	ent = SpawnStruct();
	ent thread process_blend( func, self, var1, var2, var3 );
	return ent;
}
