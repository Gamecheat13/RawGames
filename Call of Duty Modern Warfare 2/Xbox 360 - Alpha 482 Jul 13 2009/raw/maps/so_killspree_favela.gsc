...
main()
{
	...
	flag_init( "enemy_population_info_availabe" );
	flag_init( "detailed_enemy_population_info_availabe" );
...
so_favela_setup_regular()
{
	...
	level.min_enemy_population		= 18;	// min enemies in level, refills a wave if drops below this number
...
so_favela_setup_veteran()
{
	...
	level.min_enemy_population		= 16;	// min enemies in level, refills a wave if drops below this number
...
so_favela_init()
{
	...
	thread music_loop( "favela_moneyrun", 260 );
	...
	add_global_spawn_function( "axis", ::hunter_register_death );
...
custom_eog_summary()
{
	foreach ( player in level.players )
	{
		player set_custom_eog_summary( 4, 1, "@SO_KILLSPREE_FAVELA_KILLS_CIVILIANS" );
		player set_custom_eog_summary( 4, 2, player.civilians_killed );

		if ( level.max_dogs_at_once > 0 )
		{
			player set_custom_eog_summary( 5, 1, "@SO_KILLSPREE_FAVELA_KILLS_DOGS" );
			player set_custom_eog_summary( 5, 2, player.dogs_killed );
		}
	}
}