...
create_hunter_enemy( already_registered )
{
	if ( !isdefined( already_registered ) || !already_registered )
		...
		else
		{
			...
			{	
				...
				if ( !isdefined( level.hunters_all_in_active ) )
				{
					...
					set_group_advance_to_enemy_parameters( 5, level.hunters_all_in );
				}
...
dialog_shot_down_heli()
{
	wait 3;
	//Nice one Roach. 	
	radio_dialogue( "inv_six_niceone" );
}
...
dialog_fill_diner_stinger()
{
	...
	level.diner_dialog[ level.diner_dialog.size ] = "inv_tco_dispatchchopper";
	//Ramirez! There's a Stinger missile in that stockpile to the west! 	
	level.diner_dialog[ level.diner_dialog.size ] = "inv_tco_insidediner";
	//Ramirez! Next to the gas station to the west is a diner! Check there for a Stinger missile!	
	level.diner_dialog[ level.diner_dialog.size ] = "inv_tco_nexttostation";
}
...
dialog_fill_nates_stinger()
{
	...
	level.nates_dialog[ level.nates_dialog.size ] = "inv_tco_roofofnates";
	//Ramirez! There's Stingers on the roof of Nate's restaurant! 	
	level.nates_dialog[ level.nates_dialog.size ] = "inv_tco_killthathelo";
	//Ramirez! Check the roof of Nate's restaurant! I saw some Stingers up there!	
	level.nates_dialog[ level.nates_dialog.size ] = "inv_six_checktheroof";
	//Ramirez! There's some Stingers by the supply drop on the roof of Nate's!	
	level.nates_dialog[ level.nates_dialog.size ] = "inv_six_supplydroponroof";
}
...
/*	target_Set( hellfire_uav, ( 0, 0, 0 ) );
	target_SetJavelinOnly( hellfire_uav, true );
	hellfire_uav godoff();
	hellfire_uav Vehicle_SetSpeed( 20, 100, 100 );*/
...
hellfire_monitor_player( player )
{
	...
	while( 1 )
	{
		...
		index = RandomInt( level.dialog[ "drone_spotted" ].size );
		radio_dialogue( level.dialog[ "drone_spotted" ][ index ] );
		...
		if ( hellfire_check_player_available( player ) )
		{
			...
			index = RandomInt( level.dialog[ "drone_shooting" ].size );
			radio_dialogue( level.dialog[ "drone_shooting" ][ index ] );
			while( hellfire_check_player_available( player ) )
			{
...
hellfire_check_player_available( player )
{
	...
	// Perhaps give the player more of a breather if their partner is incapped as well?
	// Fully incapped players no longer targetted.
	if ( isdefined( player.down_part2_proc_ran ) && player.down_part2_proc_ran )
		return false;
...
hellfire_attack_player( player, num_shots )
{
	...
	for ( i = 0; i < num_shots; i++ )
	{
		...
		attack_range_z = 0;
		attack_spot = player.origin + ( attack_range_x, attack_range_y, attack_range_z );
...
hellfire_fire_missile( target_origin )
{
	...
	MagicBullet( "remote_missile_not_player", ( self.origin + (0,0,-128) ), target_origin );
}
...
{
	hudelem = so_create_hud_item( 0, -4, &"SO_DEFENSE_INVASION_UAV_SPOTTED", player );
...
{
	hudelem_title = so_create_hud_item( level.hud_display_enemies + 1, so_hud_ypos(), enemy_title, self );
	hudelem_count = so_create_hud_item( level.hud_display_enemies + 1, so_hud_ypos(), "", self );
	hudelem_count.alignx = "left";

/*	thread info_hud_handle_fade( hudelem_title, "wave_complete" );
	thread info_hud_handle_fade( hudelem_count, "wave_complete" );*/
	...
	while ( enemy_total > 0 )
	{
		if ( enemy_death == "hunter_death" ) 
		{
			if ( enemy_total <= 5 )
			{
				hudelem_title so_hud_pulse_loop_default( "blue" );
				hudelem_count so_hud_pulse_loop_default( "blue" );
			}
		}
		
		hudelem_title thread so_hud_pulse_create();
		hudelem_count thread so_hud_pulse_create( enemy_total );
	...
	hudelem_title thread so_hud_pulse_create();
	hudelem_count thread so_hud_pulse_create( &"SPECIAL_OPS_DASHDASH" );

	hudelem_title.pulse_loop = false;
	hudelem_count.pulse_loop = false;

	hudelem_title	set_hud_blue();
	hudelem_count	set_hud_blue();
