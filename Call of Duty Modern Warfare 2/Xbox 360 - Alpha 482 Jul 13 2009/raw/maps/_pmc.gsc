...
main()
{
	assert( getdvar( "pmc_gametype" ) != "" );
	assert( getdvar( "pmc_enemies" ) != "" );
	
	if ( isDefendMatch() )
		assert( getdvar( "pmc_defend_enemy_count" ) != "" );
	
	level.allJuggernauts = false;
	if ( getdvar( "pmc_all_juggernauts" ) == "1" )
		level.allJuggernauts = true;
...
initialize_gametype()
{
	...
	setDvarIfUninitialized( "pmc_defend_setup_time", string( 1 * 180 ) );
	setDvarIfUninitialized( "pmc_defend_time", string( 5 * 60 ) );
	
	//chad - might want to turn this on if not coop, but coop should have a map by default already
	//setdvar( "hud_ingamemap", "0" );
	...
	level.pmc.defendSetupTime = getdvarint( "pmc_defend_setup_time" );
	level.pmc.defendTime = getdvarint( "pmc_defend_time" );
	
	enemies_alive = getdvarint( "pmc_enemies_alive" );
	if ( enemies_alive > 0 )
		level.pmc.max_ai_alive = enemies_alive;
	else
...
start_pmc_gametype()
{
	...
	if ( isDefendMatch() )
		...
		set_up_preplaced_enemy_sentry_turrets( level.sentry_enemies );
	...
	assertEx( level.pmc.enemy_spawners_full_list.size >= getdvarint( "pmc_enemies" ), "There aren't enough enemy spawners in the level." );
...
set_gametype_vars()
{
	...
	level.pmc.enemies_kills_to_win = getdvarint( "pmc_enemies" );
	assert( level.pmc.enemies_kills_to_win > 0 );
	
	if ( isDefendMatch() )
		level.pmc.max_ai_alive = getdvarint( "pmc_defend_enemy_count" );
}
...
enemy_set_goal_when_player_spotted()
{
	...
	if ( ! isdefined( self.hunt_immediately ) )
...
juggernaut_sound_when_player_close()
{
	...
	while ( 1 )
	{
		...
		if ( distance( player.origin, self.origin ) <= JUGGERNAUT_MUSIC_DISTANCE )
		{
			tag_flash_loc = self getTagOrigin( "tag_flash" );
			if ( BulletTracePassed( tag_flash_loc, player geteye(), false, undefined ) )
				break;
		}
		wait .3;
	}
...
isEliminationMatch()
{
	return( getdvar( "pmc_gametype" ) == "mode_elimination" );
}

isObjectiveMatch()
{
	return( getdvar( "pmc_gametype" ) == "mode_objective" );
}
...
setup_ammo_cache_entities()
{
	possible_ammo_caches = [];

	baseModels = getentarray( "ammo_cache", "targetname" );
	//sort the caches from closest to furthest to the player
	baseModels = get_array_of_closest( getAveragePlayerOrigin(), baseModels );

	for ( i = 0 ; i < baseModels.size ; i++ )
	{
		possible_ammo_caches[ i ] = spawnStruct();
		possible_ammo_caches[ i ].base = baseModels[ i ];
		possible_ammo_caches[ i ].parts = [];

		if ( isdefined( baseModels[ i ].target ) )
		{
			// add the remaining parts to the list, and find the trigger and make that it's .trigger
			otherComponents = getentarray( baseModels[ i ].target, "targetname" );
			for ( j = 0 ; j < otherComponents.size ; j++ )
			{
				possible_ammo_caches[ i ].parts[ possible_ammo_caches[ i ].parts.size ] = otherComponents[ j ];
			}
		}
	}
	...
	ammo_caches[ 0 ] = possible_ammo_caches[ 0 ];
	possible_ammo_caches = array_remove( possible_ammo_caches, possible_ammo_caches[ 0 ] );
	
	// Delete unused ammo cache entities
	for ( i = 0 ; i < possible_ammo_caches.size ; i++ )
	{
		for ( j = 0 ; j < possible_ammo_caches[ i ].parts.size ; j++ )
		{
			if ( possible_ammo_caches[ i ].parts[ j ].classname == "script_brushmodel" )
				possible_ammo_caches[ i ].parts[ j ] connectpaths();
			possible_ammo_caches[ i ].parts[ j ] notify( "deleted" );
			possible_ammo_caches[ i ].parts[ j ] delete();
		}
		possible_ammo_caches[ i ].base delete();
	}
}
...
player_use_objective_think()
{
	...
	for ( ;; )
	{
		...
		self.objective_bar = self createClientProgressBar( self );
	...
	foreach ( player in level.players )
	{
		player notify( "laptop_picked_up" );
...
freeze_controls_while_using_laptop()
{
	...
	self waittill( "laptop_picked_up" );
...
wait_objective_complete()
{
	...
	// Reach the extraction zone before time runs out.
	objective_add( 2, "active", &"PMC_OBJECTIVE_EXTRACT", extraction_info.script_origin.origin );
	objective_current( 2 );
	...
	waittill_all_players_in_trigger( extraction_info.trigger );
...
objective_add_laptop( objNum )
{
	...
	// Enter the abort codes into the laptop before time runs out.
	objective_add( objNum, "active", &"PMC_OBJECTIVE_ABORT_CODES", level.pmc.objective.trigger.origin );
...
objective_add_defend()
{
	...
	// Set up a defensive position before enemy attack.
	objective_add( 1, "active", &"PMC_OBJECTIVE_SETUP_DEFENSES", ( 0, 0, 0 ) );
	objective_current( 1 );
	...
	objective_state( 1, "done" );
	// Survive until time runs out.
	objective_add( 2, "active", &"PMC_OBJECTIVE_DEFEND", ( 0, 0, 0 ) );
	objective_current( 2 );
...
waittill_all_players_in_trigger( trigger )
{
	// Waits for all players to be inside the specified trigger
	assert( isdefined( trigger ) );
	for ( ;; )
	{
		trigger waittill( "trigger" );
		allPlayersInZone = true;
		foreach ( player in level.players )
		{
...
staged_pacing_system()
{
	...
	level.pmc.max_juggernauts = int( level.pmc.enemies_kills_to_win / 10 );
