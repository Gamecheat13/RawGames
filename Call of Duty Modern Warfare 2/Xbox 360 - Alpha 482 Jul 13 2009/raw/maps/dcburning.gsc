...
music_bunker()
{
	thread music_loop( "dcburning_intropad", 81.7 );
...
dialogue_bunker()
{
	...
	bunker_radio_org_room2 thread  play_sound_in_space( "dcburn_gm5_gotwounded" );
...
dialogue_trenches()
{
	...
	radio_dialogue("dcburn_mcy_reqairstrike");
	...
	radio_dialogue("dcburn_mcy_buytime");
	...
	radio_dialogue("dcburn_mcy_haulingpastus");
	...
	if (  !flag( "player_commerce_trench_03" ) )
	{
		...
		radio_dialogue("dcburn_mcy_firelow");
	}
...
samsite_c4_think()
{
	...
	self.base PlayRumbleOnEntity( "damage_heavy" );
	flag_wait( "player_entering_top_elevator_area" );
	fx delete();
}
...
samsite_ai_think( eNode )
{
	...
	while( !flag( "commerce_samsite_revealed"  ) )
	{
		self waittill ( "damage", amount, attacker, direction_vec, impact_org, type );
		if ( !isdefined( attacker ) )
			continue;
		if ( isPlayer( attacker ) )
		{
...
//	}
	...
	level.player play_sound_on_entity( "dcburn_ar1_lincolnmemorial" );
...
music_to_crowsnest()
{
	...
	thread music_loop( "dcburning_evilcrowsnest_approach", 141 );
}
...
music_crowsnest_armor()
{
	...
	music_stop();
	wait( .1 );
	thread music_loop( "dcburning_ordnance_and_run", 96.2 );
}
...
heli_roof_death_failsafe()
{
	self endon( "death" );
	wait( 1.8 );
	self notify( "damage", 5000, "worldspawn", undefined, undefined, "MOD_PROJECTILE" );
}
...
dialogue_heli_ride2()
{
	...
	radio_dialogue( "dcburn_bhp_incoming" );
...
obj_commerce_defend_javelin()
{
	...
	objective_add( objective_number, "active", &"DCBURNING_OBJ_COMMERCE_DEFEND_JAVELIN", obj_position.origin );
	objective_current( objective_number );
...
obj_rooftop()
{
	...
	objective_add( objective_number, "active", &"DCBURNING_OBJ_ROOFTOP" );
	objective_current( objective_number );
	Objective_OnEntity( objective_number, level.teamleader, ( 0, 0, 70 ) );
...
AI_javelin_think()
{
	...
	eTarget = undefined;
	newMissile = undefined;
	self attach( "weapon_javelin", "TAG_INHAND", 1 );
	self.javelin = true;
	iFireNodeCounter = 0;
	/*-----------------------
	SHOOT AT TARGETS WHILE ALIVE AND UNALERTED
...
ai_ambient_prop_think()
{
	...
	switch( eProp.model )
	{
		case "com_folding_table":
			...
			laptop.angles = self.angles + ( 0, 90, 0 );
			ent = spawnstruct();
			ent.entity = laptop;
			ent.forward = 25;
			ent.up = 33.5;
			...
			chair.angles = self.angles + ( 0, 90, 0 );
