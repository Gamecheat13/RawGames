...
{	
	...
	music_loop( "jeepride_defend_music", 134 );
...
{	
	...
	trigger_wait_targetname( "leaving_crash_site" );
...
timer_start( gametype )
{
	...
	enable_challenge_timer(  undefined, "specop_challenge_completed", &"SO_AC130_CO_HUNTED_SPECOP_TIMER" );
...
objective( gametype )
{
	...
	switch( level.gameskill )
	{
		...
		case 3:
			...
			volume = getent( "barn_volume", "targetname" );
			break;
		default:
	...
	if ( isdefined( volume ) )
	{
		// only end when all enemies in the volume are dead
		touching = true;
		while( touching )
		{
			touching = false;
