	...
	for ( i = 0;i < ents.size;i++ )
	{
		if ( ( isdefined( ents[ i ].classname ) ) && ( getsubstr( ents[ i ].classname, 0, 7 ) == "weapon_" ) )
		{
			...
			if( IsDefined( weap.script_ammo_max ) )
			{
				...
				weap ItemWeaponSetAmmo( max, max );  // primary mode
				weap ItemWeaponSetAmmo( max, max, 0, 1 );  // altmode
				weap ItemWeaponSetAmmo( max, max, 1, 0 );  // left-hand (for akimbo)
...
flag_set_trigger_specialops( trigger )
{
	...
	player_arr = level.players;
	for ( ;; )
	{
		trigger waittill( "trigger", other );

		player_arr = array_remove( player_arr, other );
		if ( player_arr.size )
			continue;
...
ammo_pickup( sWeaponType )
{
	...
	if ( isdefined( emptyActionSlotAmmo ) )
		level.player giveWeapon( weaponToGetAmmo );	//this will only be for C4 and claymores if the player is totally out of them
	else
	{
		rounds = 1;
		...
		level.player SetWeaponAmmoStock( weaponToGetAmmo, level.player getWeaponAmmoStock( weaponToGetAmmo ) + rounds );
	}
...
claymore_pickup_think_global()
{
	...
	// optional - set ammo count for claymore pickup
	give_ammo_count = self.script_ammo_clip;
	ammo_count_offset = -1;
	...
	if( isdefined( give_ammo_count ) && give_ammo_count > 0 )
	{
		ammo_count = player getammocount( "claymore" );
		player setweaponammostock( "claymore", ( ammo_count + give_ammo_count + ammo_count_offset ) );
	}
	else
	{
...
ammo_cache_think_global()
{
	...
	self setHintString( &"WEAPON_CACHE_USE_HINT" );
	self makeUsable();
	//thread objective_waypoint( self.base.origin, "waypoint_ammo", false, "remove_caches" );
...
handle_getviewpos()
{
	...
	while(1)
	{
		while(! getdvarint( "launcher_viewpos" ) )
			wait .05;
...
handle_getviewangle()
{
	...
	while(1)
	{
		while(! getdvarint( "launcher_viewangle" ) )
			wait .05;
