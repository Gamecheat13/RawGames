...
music_loop_internal( name, length, fade_time, timescale, overrideCheat, stop_for_stealth )
{
	if ( IsDefined( fade_time ) && fade_time > 0 )
	{
		thread music_loop_internal_stop_with_fade_then_call( name, length, fade_time, timescale, overrideCheat, stop_for_stealth );
		return;
	}
	music_stop();
	...
	{
		level endon( "_stealth_spotted" );
		thread music_loop_stealth_pause( name, length, fade_time );
	}
