...
intro_main()
{
	...
	add_wait( ::flag_wait, "player_set_speed_lobby" );	
	add_func( ::flag_clear_delayed, "_escalator_on", 8 );
	thread do_wait();
	
	add_wait( ::flag_wait, "player_set_speed_stairs" );	
	add_func( ::flag_clear, "_escalator_on" );
	thread do_wait();
}
...
intro_civilians()
{
	...
	//delayThread( 16, ::music_stalk );
	delaythread( 16, ::music_HZ_airport );
...
{
	node thread anim_first_frame_solo( self, "elevator_scene" );
...
//	flag_wait( "player_set_speed_lobby" );
	...
	if ( !flag( "do_not_save" ) )
		thread autosave_by_name( "stairs_go_up" );
...
escape_main()
{
	...
	//Hallway clear.
	thread radio_dialogue( "airport_mkv_hallway" );
...
escape_end_sequence()
{
	...
	level.makarov add_func( ::anim_generic, level.makarov, "stand_exposed_wave_halt_v2" );
	add_func( ::radio_dialogue, "airport_mkv_holdfire" );
...
//	delayThread( delay, ::music_doublecross );
	...
	//GET THE PLAYER IN	
	if ( escape_end_waitforplayer( 45, true, .5 ) )
