...
/*
// custom command line
+exec scripter +set start "ride" +difficultyhard +set cg_drawfps 1 +set debug_colorfriendlies "off" +r_showmissinglightgrid 0
*/

/*
	BONEYARD HITLIST
	DONE - ac130 and flares need audio.  ROGER/MILLER
	- nicolai should call out what he's doing pre-flare (none left, etc).
	STEVE/ROGER
	??? - rumble on ac130 needs to be made/tweaked.  Constant pulse now.  ROGER
	NEED TO GET TEST TO REPRO - guy in suburban on gun is standing using his own gun in other
	direction.  Global?  ROGER
	- does chopper not blow up vehicle anymore?  Missed it.
	DONE - drive change?   Collided with car of guys who ragdolled on and then
	through my car.  ROGER
	NOT CUT - guy died at wheel, but kept driving, but I'm driving.  Then caught
	edge of ramp and went into a roll.  Then said I died to a grenade.
	Death thing is cut, this is silly.   Just drive in and finish level.
	ROGER
*/
...
main()
{
	...
	maps\_vehicle::build_aianims( maps\boneyard_anim::truck_overrides, maps\_truck::set_vehicle_anims );
...
//	flag_init( "music_boneyard_jeepride" );
//	flag_init( "music_boneyard_planechase" );
...
intro_dialogue()
{
	...
	// Soap! Shepherd's trying to wipe out *us* and Makarov at the same time! Head for rally point Bravo to the north!
	radio_dialogue( "byard_pri_wipeoutus" );
	// Trust no one!
	radio_dialogue( "byard_pri_donttrust" );
...
scene_a_rpg_guy( vehicle )
{
	...
	sight_ent waittill_player_lookat( undefined, undefined, undefined, 15, vehicle );
...
//	no more forced exit. :-/
//	level thread flyby_force_exit();

	flag_wait( "flyby_dialogue" );
...
flyby_force_exit()
{
	...
	while( true )
	{
		flag_waitopen_or_timeout( "flyby_force_exit", timeout );
		if ( flag( "flyby_force_exit" ) )
			break;
	...
	spawner = getent( "flyby_force_exit_guy", "targetname" );
	ai = spawner spawn_ai();
	ai endon( "death" );

	level thread flyby_force_exit_throw();
}

flyby_force_exit_throw()
{
	trigger = getent( "flyby_force_exit_damage", "targetname" );
	trigger waittill( "trigger" );

	// if the player moved on.
	if ( !flag( "flyby_force_exit" ) )
	{
		exploder( "flyby_forceexit_exploder" );
		return;
	}

	ents = getentarray( "flyby_force_exit_destination", "targetname" );
	ents = sortbydistance( ents, level.player.origin );

	Earthquake( 0.5, 1.5, level.player.origin, 1000 );
	level.player shellshock( "default", 1 );
	level.player PlayRumbleOnEntity( "damage_heavy" );
	exploder( "flyby_forceexit_exploder" );

	// move player
	vector = vectornormalize( ( ents[0].origin ) - level.player.origin );
	vector *= 300;

	tag_origin = ents[0] spawn_tag_origin();
	level.player PlayerLinkToBlend( tag_origin, "tag_origin", 0.5 );
	wait 0.5;
	level.player unlink();

	level.player setVelocity( vector );
}
*/
...
/*	path = getent( "littlebird_ride_start", "targetname" );
	heli_spawner.origin = path.origin;
	heli_spawner.angles = path.angles;
	level.heli = heli_spawner spawn_vehicle();
	level.heli thread ride_littlebird();
*/
	music_loop( "boneyard_flyby", 184 );
...
ride()
{
	...
	level thread ramp_sunsample_over_time( 1, 4 );
...
//	// Hang on, we are almost th...
//	level.rook thread dialogue_queue( "byard_rok_hangon" );
...
//	array_thread( self.riders, ::ride_runway_suburban_riders );
}
/*
ride_runway_suburban_riders()
{
	...
	flag_wait( "runway_suburban_rightside" );
...
//	might make the player take more damage if I don't make the vehicle sentient.
//	self thread makesentient( "allies" );
	self.dontunloadonend = true;
...
ride_uaz_player_crash()
{
	...
	while( true )
	{
		...
		force = abs( vector[0] ) + abs( vector[1] );
