...
iss_camera_setup()
{
	...
	level.player playerlinktodelta( camera, "tag_origin", 1, 5, 5, 5, 5 );
...
{	
	...
	node = getent( "iss_player_link", "targetname" );
/*	node2 = spawn( "script_origin", node.origin + diff );
	node2.angles = (15,150,0);*/
	...
	switch( self.targetname )
	{
		case "emp_delete":
			...
			if( isdefined( self.script_noteworthy ) )
			{
				...
				{
					case "lamp":
						playfx( level._effect[ "transformer_spark_runner" ], self getorigin() );
						thread play_sound_in_space( "glass_pane_blowout", self getorigin() );
						break;	
					case "window":
						playfx( level._effect[ "highrise_glass_74x44" ], self getorigin(), anglestoforward( (0,270,0) ) );//, anglestoright( (0,270,0) ) );
						thread play_sound_in_space( "glass_pane_blowout", self getorigin() );
						break;	
				}
			}
...
emp_heli_spotlight()
{
	...
	heli.animname = "emp_heli_spotlight";
...
emp_heli_rappel()
{
	...
	heli thread anim_single_solo( heli, "crash" );
	
	wait randomfloatrange( 5, 5.5 );
...
//	array_thread( level.team, ::notify_delay, "killanimscript", time );
	...
	level.team[ "marine2" ] delaycall( length * .93, ::stopanimscripted );	
	...
	length = getanimlength( getanim_generic( "bog_b_spotter_react" ) );
	level.team[ "marine3" ] delaycall( length * .93, ::stopanimscripted );
...
corner_truck_engine_crash()
{
	...
	self thread corner_vehicle_engine_crash_move();
...
meetup_runner_dunn()
{
	...
	self anim_generic( self, "stand_exposed_wave_halt_v2" );	
...
//	add_wait( ::flag_wait, "iss_done" );
//	add_func( ::noself_delaycall, .05, ::PlayFXonTag, getfx( "dcemp_nuke_spotlight_fade" ), model, "TAG_ORIGIN" ); 
//	thread do_wait();	
...
obj_foley()
{
	while( !flag( "whitehouse_moveout" ) )
	{
		wait( 0.05 );
		Objective_Position( level.objnum, level.foley.origin );
	}	
}
