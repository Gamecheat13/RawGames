...
main()
{
	...
	add_start( "helifly", 		::start_clifftop, 		"Destroy them", 			::clifftop_main );
...
clifftop_main()
{
	...
	/#
	if ( level.start_point == "helifly" )
		thread maps\cobrafly::cobra_think();
	#/
...
hangerpath_main()
{
	...
	//The satellite is in the far hangar. Stay low.	
	//level.price anim_single_queue( level.price, "cliff_pri_satelliteinhangar" );
	//thread dialog_weather_change();
...
hanger_main()
{
	...
	//for ( ;; )
	//{
	//	if ( distance( level.player.origin, price.origin ) < 650 )
	//		break;
	//	wait( 0.25 );
	//}
	...
	price disable_ai_color();
	price forceUseWeapon( "ak47_arctic", "primary" );
...
{
	autosave_by_name( "used_computer" );
