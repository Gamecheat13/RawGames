...
drone_death_thread( drone )
{
	...
	deathanim = level.drone_anims[ drone.team ][ "stand" ][ "death" ];
	if( isdefined( drone.deathanim ) )
		deathanim = drone.deathanim;
	
	// Make drone die
	drone notify( "death" );
	
	if ( !( isdefined( drone.noragdoll ) && isdefined( drone.skipDeathAnim ) ) )
	{
		if ( isdefined( drone.noragdoll ) )
		{
			drone drone_play_scripted_anim( deathanim, "deathplant" );
		}
		else
	...
	if( isdefined( drone ) && isdefined( drone.nocorpsedelete ) )
		return;

	wait 10;
	while( isdefined( drone ) )
	{
		if ( !within_fov( level.player.origin, level.player.angles, drone.origin, DEATH_DELETE_FOV ) )
			drone delete();
		wait 5;
	}
}
...
drone_shoot()
{
	...
	PlayFXOnTag( getfx( fx ), self, "tag_flash" );
