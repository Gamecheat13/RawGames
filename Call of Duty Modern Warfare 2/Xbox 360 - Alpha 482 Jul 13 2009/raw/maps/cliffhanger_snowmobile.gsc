...
go_to_mig()
{
	...
	setObjectiveLocation( "obj_exfiltrate", mig_obj_struct.origin );
...
go_to_slide()
{
	...
	setObjectiveLocation( "obj_exfiltrate", final_obj_struct.origin );
	setObjective_pointerText( "obj_exfiltrate", "" );
}
...
price_got_new_colornode( node )
{
	...
	if ( gettime() > self.last_comment_time + 5000 )
	{
		...
		if ( isdefined( remark ) )
		{
			...
			self.last_comment_time = gettime();
		}
	}
...
//	level.gaz.baseAccuracy = 0;
	...
	level.player MountVehicle( level.player_snowmobile );
