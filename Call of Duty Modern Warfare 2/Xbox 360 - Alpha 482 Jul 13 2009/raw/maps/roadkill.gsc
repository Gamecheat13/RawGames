...
roadkill_intro_common()
{
	...
	if ( is_default_start() )
	{		
		run_thread_on_targetname( "radio_scene", ::radio_scene );
...
roadkill_riverbank()
{
	...
	level.player delaythread( 3, maps\_gameskill::grenade_dirt_on_screen, "right" );
...
roadkill_crazy_ride()
{
	...
	set_player_attacker_accuracy( 0.2 );
...
//	array_spawn_function_noteworthy( "school_spawner", ::join_school_threatbias_group );
	...
	slideModel = Spawn( "script_origin", level.player.origin );
	slideModel.angles = level.player.angles;
	level.player PlayerLinkTo( slideModel );


	movetime = 0.7;
	slideModel MoveTo( player_slide_crash.origin, movetime, 0.2, 0.2 );
	wait( movetime );
	slideModel Delete();
...
dismount_dialogue_and_friendly_progression_logic()
{
	...
	foley_line( "roadkill_fly_getoffstreet" );
	...
	radio_line( "roadkill_ar2_sitrep" );
	// Hunter 2-1 copies, we're almost there! Hit a bump in the road, over!	
	foley_line( "roadkill_fly_bumpintheroad" );
	...
	radio_line( "roadkill_ar2_undertheirnoses" );
	// Solid copy Hunter 2-3, will do!	
	foley_line( "roadkill_fly_willdo" );
	...
	thread radio_line( "roadkill_ar2_suppressingfire" );

	delaythread( 2.6, ::flag_set, "friendlies_suppress_school" );
	flag_wait( "lets_go_trigger" );
...
roadkill_the_end()
{
	...
	delaythread( 2, ::flag_set, "heli_takes_off" );
