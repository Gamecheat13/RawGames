...
slowmo_breach_init()
{
	...
	level.has_special_breach_anim[ "mg4_artic" ] = true;
	level.has_special_breach_anim[ "mg4_artic_reflex" ] = true;
	level.has_special_breach_anim[ "mg4_artic_thermal" ] = true;
	...
	level.has_special_breach_anim[ "spas12_artic" ] = true;
    level.has_special_breach_anim[ "spas12_artic_eotech" ] = true;
    level.has_special_breach_anim[ "spas12_artic_grip" ] = true;
    level.has_special_breach_anim[ "spas12_artic_heartbeat" ] = true;
    level.has_special_breach_anim[ "spas12_artic_heartbeat_attach" ] = true;
    level.has_special_breach_anim[ "spas12_artic_reflex" ] = true;
...
breach_anims()
{
	...
	level.scr_stub[ "generic" ][ "execution_shield_hostage_idle" ][ 0 ]	 = "hostage_stand_idle";
...
wait_for_breach_or_deletion( ent )
{
	...
	for ( ;; )
	{
		...
		if ( isalive( other ) && !isdefined( passive ) )
		{
			// make sure we're not the passive breacher
			if ( other IsReloading() )
			{
...
{
	level notify( "breach_displaying_friendly_hint" );
	level endon( "breach_displaying_friendly_hint" );
	...
	if ( is_coop() )
	{
		...
		hintString = &"SCRIPT_BREACH_NEED_PLAYER";
		if ( IsSplitScreen() )
			hintOffset = -30;
	}
	else
	{
		...
		hintString = &"SCRIPT_BREACH_NEED_FRIENDLY";
	}
	thread hint( hintString, 3, hintOffset );
	thread breach_friendly_hint_cleanup();
}

breach_too_many_enemies_hint()
{
	level notify( "breach_displaying_too_many_enemies_hint" );
	level endon( "breach_displaying_too_many_enemies_hint" );

	hintOffset = 0;
	// Cannot breach. Clear the area of all enemies
	hintString = &"SCRIPT_BREACH_TOO_MANY_ENEMIES";
	thread hint( hintString, 3, hintOffset );
	thread breach_too_many_enemies_hint_cleanup();
}
...
breach_reloading_hint()
{
	...
	hintString = &"SCRIPT_BREACH_RELOADING";
	thread hint( hintString, 3, hintOffset );
...
breach_too_many_enemies_hint_cleanup()
{
	level notify( "breach_displaying_too_many_enemies_hint" );
	level endon( "breach_displaying_too_many_enemies_hint" );
	level waittill_notify_or_timeout( "breaching", 3 );
...
sort_breachers( breach_players )
{
	// the closest player to the breach spot will do the breach		
/*	closest_players = get_array_of_closest( self.origin, players );
	player = closest_players[ 0 ];
	other_player = closest_players[ 1 ];
	players = closest_players;*/

	// Until a better solution is written, the person who instigates
	// the breach is the one who places the breach, regardless of position.
...
player_breach( ent, player )
{
	...
	if ( is_coop() )
	{
		...
		if ( breach_array == ent )
		{
			...
			// Find out who...
			breach_players = sort_breachers( breach_players );
		}
		else
		{
	...
	if ( is_special_breach )
	{
		...
		set_player_breach_rig_anim_special();
	...
	else
	{
		...
		set_player_breach_rig_anim_normal();
...
slomo_difficulty_dvars()
{
	...
	//Switch some values depending on difficulty
	switch( level.gameSkill )
	{
		...
		case 2:// hardened
		case 3:// veteran
			SetSavedDvar( "bullet_penetration_damage", 0 );// May want to eventually change this to "1" to make it harder, but it changes the rules a bit too much
			break;
	}
		...
		if ( IsDefined( attacker ) )
		{
			if ( IsPlayer( attacker ) )
			{
				...
				if ( level.last_player_damage == GetTime() )
				{
					...
					SetDvar( "ui_deadquote", &"SCRIPT_MISSIONFAIL_KILLEDHOSTAGE_THROUGH_ENEMY" );
				}
				else
				{
					// Mission failed. You killed a hostage.
					SetDvar( "ui_deadquote", &"SCRIPT_MISSIONFAIL_KILLEDHOSTAGE" );
				}
			...
			else if ( ( IsDefined( attacker.team ) ) && ( attacker.team == "allies" ) && ( !IsPlayer( attacker ) ) )
			{
				...
				// Mission failed. A hostage was executed.
				SetDvar( "ui_deadquote", &"SCRIPT_MISSIONFAIL_HOSTAGEEXECUTED" );
				missionFailedHostage = true;
			}
			else
			{
				if ( self coop_breached_from_same_door_in_a_muliti_door_room( room_volume ) )
				{
					...
					SetDvar( "ui_deadquote", &"SCRIPT_MISSIONFAIL_HOSTAGEEXECUTED_USEMULTIDOOR" );
				}
				else
				{
					// Mission failed. A hostage was executed.
...
set_player_breach_rig_anim_special()
{
	// Currently there are no special anims needing setup.
}
