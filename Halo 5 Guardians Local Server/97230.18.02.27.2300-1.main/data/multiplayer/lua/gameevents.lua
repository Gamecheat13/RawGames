...
CustomStats =
{
	-- corresponds to WarzoneTuningValues.CustomStatisticNames.PersonalScoreStatisticUpper
	WZPersonalScore = "PersonalScoreUpper",

	-- correspond to the statistics defined in PvETuningValues.PvERoundBonusesCustomStatisticNames
...
function ResponseDefinition:SetResponse(response)
	...
	-- Also validate that if a CustomStat or CustomStatChange was defined, so is the other
	--
...
function ResponseDefinition:SetCustomStatistic(target, customStatTarget, customStatNewValue, variant)
	...
	if target ~= nil and customStatTarget ~= nil and customStatNewValue ~= nil and variant ~= nil then
		-- by this time the properties should be of the proper base type as we've already resolved any dynamic props
...
function ResponseDefinition:Activate(context)
	...
	if self.EventHandle ~= nil then
		...
		-- If there is a dynamic prop, then resolve it, otherwise just take the responseProp field
		...
		-- Personal Score (syntactic sugar for the WZ Personal Score custom statistic)
		...
		-- If Score should map to PersonalScore instead (like in the case of PvE) then consume it here and set it to 0 to prevent it from incrementing the Engine score later
		...
		-- Resolve dynamic props and set them on override props for consumption in GameEventsMicroSystem.cpp
		local overrideProps = { };
