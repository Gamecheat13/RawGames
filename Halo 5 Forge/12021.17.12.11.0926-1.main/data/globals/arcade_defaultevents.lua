...
onFriendlyPlayerKill = onKill:Filter(
	function (context)
		return (context.KillingPlayer ~= nil and context.DeadPlayer ~= nil and context.KillingPlayer:IsFriendly(context.DeadPlayer) and (context.KillingPlayer ~= context.DeadPlayer));
	end
	);
...
onVehicleKillFromPlayer = onKill:Filter(
	function (context)
		...
		if (context.DeadObject == nil or context.KillingPlayer == nil or context.DeadObject.GetCampaignMetagameType == nil or context.DeadObject:IsHostileCampaign(context.KillingObject) == false) then
		   ...
		   metagameObjectType == CampaignMetagameType.Phaeton or
		   metagameObjectType == CampaignMetagameType.Wasp)
		end
	end
);
...
-- Wasp kill		
onWaspKillByPlayer = onVehicleKillFromPlayer:Filter(		
	function (context)	
	--Log("onWaspKillByPlayer");	
		return (context.DeadObject:GetCampaignMetagameType() == CampaignMetagameType.Wasp);
	end	
);
...
-- Wasp kill		
onWaspKillByMusketeer = onVehicleKillFromMusketeer:Filter(		
	function (context)	
	--Log("onWaspKillByMusketeer");	
		return (context.DeadObject:GetCampaignMetagameType() == CampaignMetagameType.Wasp);
	end	
);
...
function IsSwordShoulderBashKill(context)
	return (context.DamageType == SwordDamageType and
			context.DamageSource == SpartanDamageSource);
end

onSwordKill = onEnemyAIKillFromPlayer:Filter(
	function (context)
		return table.any(SwordDamageSources,
			...
			or IsSwordShoulderBashKill(context);
	end
	);
...
onAirAssassination = killTypeSelect:Add(
	function (context)
		...
		and context.DeadObject ~= nil
		and context.DeadObject:IsAirborne()
...
onShoulderBashKill = onEnemyAIKillFromPlayer:Filter(
	function(context)
...
onBeamRifleKill = killTypeSelect:Add(
	...
	-- end
	function (context)
		return table.any(BeamRifleDamageSources,
			function(damageSource)
				return context.DamageSource == damageSource;
			end
			);
...
--	end
--	);
onGenericSplatter = onEnemyAIKillFromPlayer:Filter(
	function(context)
		return context.KillingPlayer:GetUnit() ~= nil and context.KillingPlayer:GetUnit():IsDrivingVehicle() and context.DamageType == CollisionDamageType and
			table.any(AllVehicleDamageSources, function(vehicleDamageSource) return context.DamageSource == vehicleDamageSource end);
	end
 );
...
-- Due to 30fps vs 60fps shenanigans, the actual value in practice is half of this.
local multikillSeconds = 3;
...
onTenKillsInARow:ResetPlayerAccumulator(
	killsInARowAccumulator,
	function (context)
		return context.KillingPlayer;
	end
...
onSprayPray = onEnemyAIKillFromPlayer:Filter(
	function (context)
		return context.DeadObject ~= nil 
		and context.KillingObject ~= nil 
		and context.DeadObject:GetDistance(context.KillingObject) >= 16
...
function FindWheelman(context)
	...
	local killingPlayerDrivingVehicle = KillingPlayerUnit:IsDrivingVehicle() ;

	if KillingPlayerUnit == nil or KillingPlayerUnit:IsDrivingVehicle() then
		return nil;
	end

	local killingPlayerVehicle = KillingPlayerUnit:GetVehicle();
	...
	return table.filter(context:GetAllPlayers(),
		function(player)
			...
			and playerUnit:IsFriendly(KillingPlayerUnit);
		end
		);
end
