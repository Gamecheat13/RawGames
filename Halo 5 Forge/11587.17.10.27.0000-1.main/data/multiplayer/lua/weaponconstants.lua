...
OddballSkullDummyAttachmentName = ResolveString("dummy_attachment_oddball_skull");
...
--
-- Helper function for determining if a weapon is the CQC magnum (gunfighter). Looks at the dummy attachment we have on that weapon
--

GunfighterMagnumDummyAttachmentName = ResolveString("magnum_cqc_dummy");

function WeaponIsGunfighterMagnum(damageSource, attachmentList)
	if (damageSource ~= MagnumDamageSource or attachmentList == nil) then
		return false;
	end

	return table.contains(attachmentList, GunfighterMagnumDummyAttachmentName);
end

function WeaponIsGunfighterFlagnum(damageSource, attachmentList)
	if (damageSource ~= FlagnumDamageSource or attachmentList == nil) then
		return false;
	end
...
KillFeedItemReference_dmgSourceIndex = 1;
KillFeedItemReference_killFeedStringIndex = 2;
KillFeedItemReference_attachmentIndex = 3;

KillFeedItemReferenceDamageSourcesWithAttachmentSpec =
{
	{BallDamageSource, "skull", OddballSkullDummyAttachmentName},
}
