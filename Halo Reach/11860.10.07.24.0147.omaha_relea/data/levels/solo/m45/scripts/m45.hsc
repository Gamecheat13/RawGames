...
; 
; Source file: m45.hsc
; Start time: 2018-02-05 12:54:03 PM
; 
...
(script startup void space
    ...
    (sleep_until (or (= b_crv_completed true) (>= s_insertion_index s_brd_encounter_index)) 1)
    (if (<= s_insertion_index s_brd_encounter_index)
        (wake brd_encounter)
    )
    (sleep_until (or (= b_brd_completed true) (>= s_insertion_index s_com_encounter_index)) 1)
    (if (<= s_insertion_index s_com_encounter_index)
        (wake com_encounter)
    )
    (sleep_until (or (= b_com_completed true) (>= s_insertion_index s_hgr_encounter_index)) 1)
    (if (<= s_insertion_index s_hgr_encounter_index)
        (wake hgr_encounter)
    )
    (sleep_until (or (= b_hgr_completed true) (>= s_insertion_index s_grm_encounter_index)) 1)
    (if (<= s_insertion_index s_grm_encounter_index)
        (wake grm_encounter)
    )
    (sleep_until (or (volume_test_players "tv_bridge_start") (>= s_insertion_index s_brg_encounter_index)) 1)
    (if (<= s_insertion_index s_brg_encounter_index)
        (wake brg_encounter)
    )
    (sleep_until (or (= b_brg_completed true) (>= s_insertion_index s_esc_encounter_index)) 1)
    (if (<= s_insertion_index s_esc_encounter_index)
        (wake esc_encounter)
    )
    (sleep_until (or (volume_test_players "tv_final_start") (>= s_insertion_index s_fin_encounter_index)) 1)
...
; Decompilation finished in ~0.2652548s
