...
static long S_zoneset_prepare_delay =												0;
...
script static boolean zoneset_prepare( long l_zoneset_id, boolean b_check_loaded )
	...
	if ( ((zoneset_current() != l_zoneset_id) and (L_zoneset_preparing != l_zoneset_id)) or (not b_check_loaded) ) then
		...
		if ( zoneset_prepare_delay() > 0 ) then
			sleep( zoneset_prepare_delay() );
		end
...
script static long zoneset_preparing()
	L_zoneset_preparing;
end

// === zoneset_prepare_delay: Get/get the zone set prepare delay
script static void zoneset_prepare_delay( short s_delay )
	S_zoneset_prepare_delay = s_delay;
end
script static short zoneset_prepare_delay()
	S_zoneset_prepare_delay;
end
...
static long S_zoneset_load_delay =													0;
...
script static boolean zoneset_load( long l_zoneset_id, boolean b_check_loaded )
	...
	if ( ((zoneset_current_active() != l_zoneset_id) and (L_zoneset_loading != l_zoneset_id)) or (not b_check_loaded) ) then
		...
		if ( zoneset_load_delay() > 0 ) then
			sleep( zoneset_load_delay() );
		end
...
script static long zoneset_loading()
	L_zoneset_loading;
end

// === zoneset_load_delay: Get/get the zone set load delay
script static void zoneset_load_delay( short s_delay )
	S_zoneset_load_delay = s_delay;
end
script static short zoneset_load_delay()
	S_zoneset_load_delay;
end
