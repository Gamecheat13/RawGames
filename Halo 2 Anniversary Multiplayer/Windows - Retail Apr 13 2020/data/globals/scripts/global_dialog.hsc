...
script static void notify_dialog_muted()
	sys_dialog_mute_dialog = TRUE;
end

script static void notify_dialog_unmuted()
...
sys_dialog_debug( "dialog_line_chief", "" );
	...
	sleep_until( not b_chief_is_speaking );
	b_chief_is_speaking = TRUE;
...
sys_dialog_debug( "dialog_line_cortana", "" );
	...
	sleep_until( not b_cortana_is_speaking );
	b_cortana_is_speaking = TRUE;
...
sys_dialog_debug( "sys_dialog_line_thread", "" );
	...
	if ( r_type != DEF_DIALOG_TYPE_NONE() and not sys_dialog_mute_dialog) then
		...
		if ( snd_sound != NONE ) then
			...
			r_pad = r_pad + frames_to_seconds_authored( l_snd_time );
