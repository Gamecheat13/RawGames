...
init(use_model)
{
	...
	if (!IsDefined(use_model) || use_model)
	{
		...
		if ( level.ps3 )
		{
			level._securitycamera["spotlight"]			= Loadfx( "props/security_camera_on_PS3" );
		}
		else
		{
			level._securitycamera["spotlight"]			= Loadfx( "props/security_camera_on" );
		}
		//depot/trees/jb/share/raw/fx/props/security_camera_on_PS3.efx
	}
...
cone_trace( entCamera )
{
	...
	// MikeA: Fix for cameras not working in SHIP
	// Please leave, see MikeA for more info if needded
	//wait( 1.0 );
	...
	while( true )
	{
		...
		// PLEASE LEAVE THIS CODE IN: SEE MIKEA FOR MORE INFORMATION
