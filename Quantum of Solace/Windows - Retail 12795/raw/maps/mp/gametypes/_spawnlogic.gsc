...
onPlayerConnect()
{
	for(;;)
	{
		...
		game["playercount"][player.name] = 0;
...
removePlayerOnDisconnect()
{
	...
	game["playercount"][name] = undefined;
...
addSpawnPoints( team, spawnPointName )
{
	...
	{
		maps\mp\_utility::error2("No " + spawnPointName + " spawnpoints in level!");
...
addTeamSpawnPointsToLevelList( team )
{
	if( !isDefined( level.teamSpawnPoints ) || !isDefined( level.teamSpawnPoints[ team ] ) )
		return;
	...
	for( teamSpawnPointIter = 0; teamSpawnPointIter < level.teamSpawnPoints[ team ].size; teamSpawnPointIter++ )
	{
		level.spawnpoints[ level.spawnpoints.size ] = level.teamSpawnPoints[ team ][ teamSpawnPointIter ];
	}
}

addSpawnPointsWithTeamStarts( genericSpawnPoints, alliesStartSpawnPoints, axisStartSpawnPoints )
{
	...
	level.spawnpoints = [];
	level.spawn_generic = getentarray(genericSpawnPoints, "classname");
	if(!level.spawn_generic.size)
	{
		maps\mp\_utility::error2("No " + genericSpawnPoints + " spawnpoints in level!");
		maps\mp\gametypes\_callbacksetup::AbortLevel();
		return;
	}
	for(i = 0; i < level.spawn_generic.size; i++)
		level.spawnpoints[level.spawnpoints.size] = level.spawn_generic[i];

	level.spawn_axis_start = getentarray(axisStartSpawnPoints, "classname");
	if(!level.spawn_axis_start.size)
	{
		maps\mp\_utility::error2("No " + axisStartSpawnPoints + " spawnpoints in level!");
		maps\mp\gametypes\_callbacksetup::AbortLevel();
		return;
	}
	...
	level.spawn_allies_start = getentarray(alliesStartSpawnPoints, "classname");
	if(!level.spawn_allies_start.size)
	{
		maps\mp\_utility::error2("No " + alliesStartSpawnPoints + " spawnpoints in level!");
		maps\mp\gametypes\_callbacksetup::AbortLevel();
		return;
	}
	...
	{
		origin = level.spawnpoints[i].origin;
...
//	level endon("intermission");
	...
	numplayers = 0;
	if( isdefined(obj.allies) )
		numplayers = numplayers + obj.allies.size;
	if( isdefined(obj.enemies) )
		numplayers = numplayers + obj.enemies.size;
...
getSpawnpoint_BondVS(spawnpoints)
{
	// There are no valid spawnpoints in the map
	if(!isdefined(spawnpoints))
		return undefined;
	...
	if (numplayers > 0)
	{
		for (i = 0; i < spawnpoints.size; i++)
		{
			...
			// high average enemy distance plus high minimum enemy distance is good
			spawnpoints[i].weight = (enemyDistSum / numplayers) + enemyMinDist;
		}
	}

	// try and pick one where bond is hidden
	avoidVisibleEnemies(spawnpoints, true);
...
trackproxmines()
{
	while ( 1 )
	{
		level.proxmines = getentarray("proxmine", "targetname");
		wait .05;
	}
}
...
avoidWeaponDamage(spawnpoints)
{
	...
	for (i = 0; i < spawnpoints.size; i++)
	{
		...
		for (j = 0; j < level.proxmines.size; j++)
		{
			if ( !isdefined( level.proxmines[j] ) )
				continue;

			if (level.proxmines[j] maps\mp\_proxmines::isPointVulnerable(spawnpoints[i].origin)) {
				spawnpoints[i].weight -= weaponDamagePenalty;
				/#
				if (level.storeSpawnData)
					spawnpoints[i].spawnData[spawnpoints[i].spawnData.size] = "Was near proxmine: -" + weaponDamagePenalty;
				#/
			}
		}
