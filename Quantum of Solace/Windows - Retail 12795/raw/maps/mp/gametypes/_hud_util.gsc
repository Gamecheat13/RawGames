...
createServerBarNoFill( bgShader, bgColor, width, height, team, selected, selectedColor )
{
	hasSelection = isDefined( selected ) && selected;

	barElemFrame = undefined;
	if( hasSelection )
	{
		if ( isDefined( team ) )
			barElemFrame = newTeamHudElem( team );
		...
		barElemFrame.children = [];
		barElemFrame.sort = -1;
		barElemFrame.color = selectedColor;
		barElemFrame setParent( level.uiParent );
		barElemFrame setShader( "progress_bar_selection", width, height );
		barElemFrame.hidden = false;
	}
...
createServerDTimer( width, height, team )
{	
	if ( isDefined( team ) )
		timerElem = newTeamHudElem( team );
	else
		timerElem = newHudElem( self );
	timerElem.elemType = "dtimer";
	timerElem.x = 0;
	timerElem.y = 0;
	timerElem.width = width;
	timerElem.height = height;
	timerElem.xOffset = 0;
	timerElem.yOffset = 0;
	timerElem.children = [];
	timerElem setParent( level.uiParent );
//	timerElem.color = (1,1,0);
	...
	timerElem setShader( "dtimer_bg_border", width, height );
	
	timerElem.digits = [];

	digitWidth = int( width / 8 );
	colonWidth = int( width / 16 );

	for ( index = 0; index < 6; index++ )
	{
		timerElem.digits[index] = createServerIcon( "dtimer_0", digitWidth, height );
		timerElem.digits[index] setParent( timerElem );
		timerElem.digits[index].color = (1,0,0);
		timerElem.digits[index].sort = 100;
	}
	
	offset = int( colonWidth / 2 );
	timerElem.digits[0] setPoint( "TOPLEFT", undefined, offset, 0 );
	offset += digitWidth;
	timerElem.digits[1] setPoint( "TOPLEFT", undefined, offset, 0 );
	offset += digitWidth;
	offset += colonWidth;
	timerElem.digits[2] setPoint( "TOPLEFT", undefined, offset, 0 );
	offset += digitWidth;
	timerElem.digits[3] setPoint( "TOPLEFT", undefined, offset, 0 );
	offset += digitWidth;
	offset += colonWidth;
	timerElem.digits[4] setPoint( "TOPLEFT", undefined, offset, 0 );
	offset += digitWidth;
	timerElem.digits[5] setPoint( "TOPLEFT", undefined, offset, 0 );
...
setDTimerTime( time )
{
	...
	setDTimerElems( mins, self.digits[0], self.digits[1] );
	setDTimerElems( secs, self.digits[2], self.digits[3] );
	setDTimerElems( msecs, self.digits[4], self.digits[5] );
}

setDTimerElems( value, firstElem, secondElem )
{
	if ( value < 10 )
		digitString = ("0" + value);
	else
	...
	firstElem = firstElem setIconShader( "dtimer_" + digitString[0] );
	secondElem = secondElem setIconShader( "dtimer_" + digitString[1] );
}
...
{
	bar = createBar( (0.6, 0.75, 0.75), level.primaryProgressBarWidth, level.primaryProgressBarHeight );
	bar setPoint("CENTER", undefined, 0, level.primaryProgressBarY);
...
{
	bar = createBar( (0.6, 0.75, 0.75), level.secondaryProgressBarWidth, level.secondaryProgressBarHeight );
