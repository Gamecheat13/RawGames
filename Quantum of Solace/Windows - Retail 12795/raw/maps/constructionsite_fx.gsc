...
initFXModelAnims()
{
	...
	if (IsDefined(ent8)) { ent8 thread shed_roof();   println("************* FX: shed_roof *************"); }
	...
	//if (IsDefined(ent11)) { ent11 thread necklace();   println("************* FX: necklace *************"); }
...
{	
	...
	{
		//println("************* FX: NOTE FOUND - pipe_burst *************");		
...
propane_fall()
{
	...
	//delayed till the end of the animation
...
#using_animtree("fxanim_shed_roof");
shed_roof()
{
	level waittill("shed_roof_start");
	self UseAnimTree(#animtree);
	self animscripted("a_shed_roof", self.origin, self.angles, %fxanim_shed_roof);
}
...
revivetest()
{
	...
	//reviveeffectcenter( contrast, brightness, desaturation, Light Tint Red, Light Tint Green, Light Tint Blue, Dark Tint Red, Dark Tink Green, Dark Tint Blue );
	//reviveeffectcenter( 1.9, 0.5, 0.8, 1.56, 1.57, 1.69, 0.97, 1.0, 0.97 );
	//reviveeffectedge(   blur radius, motion blur, contrast, brightness, desaturation, Light Tint Red, Light Tint Green, Light Tint Blue, Dark Tint Red, Dark Tink Green, Dark Tint Blue );
...
precacheFX()
{
	...
	level._effect["fxflys"] 		= loadfx ( "maps/constructionsite/const_gnats_fiesty" ); //temporarily using gnats for flys
	...
	level._effect["const_birds_takeoff1"] 		= loadfx ( "maps/Siena/siena_birds_flying4" );
	level._effect["const_birds_takeoff2"] 		= loadfx ( "maps/Siena/siena_birds_flying4" );
...
if ( level.ps3 == true ) //GEBE
	{
		level._effect["fxdusthaze2"] 	= loadfx ( "maps/constructionsite/const_dusthaze2_PS3" );
		level._effect["falling_sand"] 	= loadfx ( "maps/constructionsite/const_falling_sand_PS3" );
		level._effect["water_pipe"] 	= loadfx ( "maps/constructionsite/const_water_pipe1_PS3" );		
	}
else
	{
