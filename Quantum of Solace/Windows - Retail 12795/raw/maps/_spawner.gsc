...
main()
{
	...
	*/
	...
	level.portable_mg_gun_tag = "R_UpperArm"; // need to get J_gun back to make it work properly
	...
	for (i=0;i<ai.size;i++)
	{
		...
		*/
		...
		// insert rumble stuff here

		ai[i] thread spawn_think ();
		ai[i] thread handleQKRumble ();
		ai[i] thread maps\_fx::QKill_fx_on_ai ();
	}
...
doAutoSpawn(spawner)
{
	...
	for (;;)
	{
		...
		spawn_failed(guy);			
		//if (spawn_failed(guy))
		//spawner notify ("spawn_failed");	// moved into _utility::spawn_failed
...
spawn_grenade(origin, team)
{
	...
	//bb - temp until weapons revision is done
	return undefined;
...
spawn_prethink()
{
	...
	// BOND_MOD: spawn guys based on flags being set
	self.active = false;
...
// BOND_MOD:
// flag_spawn - spawn guys when flags get set
//
flag_spawn()
{
	self endon("death");

	// when any of these flags are true, the spawner will spawn
	...
	if (!self.active)
	{
		if (IsDefined(self.targetname))
		{
			self.targetname = self.targetname + "_active";	// change the targetname of this spawner so it doesn't start spawning again when it's triggered
		}
...
#/
	...
	// BOND MOD
	// MQL 2/7: add "neutral" civilian cowering and bumping behavior
...
#/
	...
	{
		//Removed by MIchel because this was an UGLY fix.
		//The GDT is the place to set health, not script
	...
	// Puts AI in a specific alert state
	...
	// Constraint an AI movement within a boundary
	...
	//This should be set in Radiant.. make sure we set this after we set the combat role itself
	...
	// Set the movement speed of an AI
	if ( isdefined(self.script_speed) )
	{
		self SetScriptSpeed( self.script_speed );
	}

	// Turn ON or OFF the possibility of an AI to use traversals
	if ( isdefined(self.script_enabletraversal) )
	{		 
		self enabletraversal( self.script_enabletraversal );
	}

	// Turn ON or OFF the ability of AI to throw grenades
...
// BOND_MOD: zone_alert() - alert spawners when AI in same zone get alerted
zone_alert(ai)
{
	...
	for (i = 0; i < zones.size; i++)
	{
		...
		{
			this_zone = zones[i];
			break; // do an array if we want overlapping zones
		}
	}
	...
	if (IsDefined(this_zone))
	{
		...
		for (i = 0; i < spawners.size; i++)
		{
			if (!IsDefined(spawners[i].script_alert))	// don't change alert on a spawner is the designer specifies an alert
			{
...
go_to_node( node )
{
	...
	*/
	...
	// only go to the first node of a patrol
	if (node.type == "Patrol")
		return;
	...
	if (isdefined (node.target))
	{
		...
		{
			self waittill ("goal");
			// BOND_MOD: allows us to cancel this function with the "stop_going_to_node" notify from a function that gets triggered at the fisrt node.
			waittillframeend;
			// !BOND_MOD
			for (;;)
			{
...
aigroup_create( aigroup )
{
	...
	if (!isdefined(level.flag[aigroup + "_alert"]))	// may already be set by triggers or somewhere else
	{
...
// BOND_MOD: spawn backup guys when guys get alerted.
aigroup_alert()
{
	self endon("death");
		...
		if (IsDefined(level._ai_group[self.script_aigroup + "_backup"]))
		{
			for (i = 0; i < level._ai_group[self.script_aigroup + "_backup"].spawners.size; i++)
			{
				...
				if (IsDefined(spawner) && !spawner.active)
				{
					...
					{
						spawner.script_alert = "alert_red";	// default alert state if none is specified
					}
...
///////////////////////////////////////////////////////////////////////////
// BOND_MOD
// function waits for death
// and sends a notify to the level based on the type for achievements
// runs on level
actor_death( ent_actor )
{
	// thread off the function for watching death
	ent_actor thread maps\_achievements::ach_headshot_watch();

	// thread off function for take downs
	ent_actor thread maps\_achievements::ach_quick_kill_watch();

	//// wait for the death notify
	//ent_actor waittill( "death", strAttacker, strType, strWeapon );
}