...
//Added Temp Music Just For Beta
//MusicPlay( "mus_opera_temp", 0 );
...
//************************************************************************************************
 ...
 //***************   
             ...
             declareAmbientPackage( "operahouse_ext_1_pkg" );            
   			...
   			//addAmbientElement( "operahouse_ext_1_pkg", "cistern_1shot", 6, 45, 2000, 2000 );
        
 //***************
 
 //operahouse_ext_2_pkg
 ...
 //***************   
             ...
             declareAmbientPackage( "operahouse_ext_2_pkg" );            
   			...
   			//addAmbientElement( "operahouse_ext_2_pkg", "cistern_1shot", 6, 45, 2000, 2000 );
...
//*************************************************************************************************
	...
	loopSound("boat_rock", (3775, -1210, -371), 11.050);
...
//*************************************************************************************************
 	    ...
 	    //MusicPlay( "mus_operahouse_temp", 0 ); 
	    ...
	    array_thread(GetEntArray("dockwood_trigger", "targetname"), ::dockwood); 	
	    
	    array_thread(GetEntArray("palatte_trigger", "targetname"), ::palatte); 	
	    
	    array_thread(GetEntArray("dump_trigger", "targetname"), ::dump); 
...
start_dvorak()
{
	...
	//big fires on dock
	large_dock_fire_zone_var = getent("large_dock_fire_zone", "targetname");
	
	if(IsDefined(large_dock_fire_zone_var))
	{
		large_dock_fire_zone_var playloopsound("dock_fire_03", 2.8);
	}	
}
...
dockwood()
{
		...
		while (1)
		{
			...
			if ( 100 < lengthsquared( level.player getplayervelocity() ) )
			{
				...
				//iprintlnbold ("SOUND: dockwood");
				level notify("noise_trigger_dockwood");
				while ( level.player isTouching(self))
...
palatte()
{
		...
		while (1)
		{
			...
			//if ( 100 < lengthsquared( level.player getplayervelocity() ) )
			//{
				self playsound ("palatte");
				//iprintlnbold ("SOUND: palatte");
				level notify("noise_trigger_palatte");
				while ( level.player isTouching(self))
...
dump()
{
		...
		while (1)
		{
			...
			//{
				self playsound ("dump");
				//iprintlnbold ("SOUND: dump");
				level notify("noise_trigger_dump");
				while ( level.player isTouching(self))
