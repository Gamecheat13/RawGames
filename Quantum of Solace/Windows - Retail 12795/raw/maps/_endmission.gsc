...
//BEENOX-PC END
	...
	missionSettings addLevel( "operahouse", false, undefined, undefined, 2 );
	...
	missionSettings addLevel( "eco_hotel", false, "PROGRESSION_HAINES", "haines_estate_load", undefined );
...
nextmission()
{
	...
	if( getdvarint("mis_01") < getMissionDvarIndex( levelindex ) + 1 )
		_setMissionDvar( "mis_01", getMissionDvarIndex( levelIndex ) + 1 );
...
{
	// need to skip siena, since we're faking that it and rooftops are one big level
...
{
	// siena gets ignored for mission dvar purposes, since it is levelIndex,
	// we can handle it by just subtracting 1 from every levelIndex except the 0th (whites_estate, the only level before it)
	if ( !levelIndex )
		return levelIndex;
	else
		return levelIndex - 1;
...
getLowestSkill()
{
	...
	// -1 on the levels size since we need to skip siena, because it doesn't get added to the mission string
