...
spawn_selected_object( model_name, with_trace )
{
	...
	vector = level.player GetEye() + VectorScale( forward, level.selected_object_dist );
	level.selected_object = Spawn( "script_model", vector );
	level.selected_object SetModel( model_name );

//	level.selected_object LinkTo( level.player );
...
//	self thread model_info_update();
	...
	while( true )
	{
		...
		if( with_trace )
		{
			vector = level.player GetEye() + VectorScale( forward, 5000 );
			trace = BulletTrace( level.player GetEye(), vector, false, self );
...
{
	forward = AnglesToForward( level.player GetPlayerAngles() );
	vector = level.player GetEye() + VectorScale( forward, 48 );
...
spray_trajectory()
{
	...
	while( 1 )
	{
		...
		velocity = VectorScale( forward, level.spray["power"] );
		sub_vel = VectorScale( velocity, time_inc );
		start_pos = level.player GetEye() + VectorScale( forward, 48 );;
		gravity = GetDvarInt( "g_gravity" );
		for( i = 1; i < segments + 1; i++ )
		{
			pos = start_pos + VectorScale( sub_vel, i );
...
plant_bomb()
{
	...
	forward = AnglesToforward( level.player GetPlayerAngles() );
	vector = level.player GetEye() + VectorScale( forward, 5000 );
	trace = BulletTrace( level.player GetEye(), vector, false, undefined );
...
object_highlight( objects )
{
	...
	for( i = 0; i < objects.size; i++ )
	{
		...
		difference = VectorNormalize( ent.origin - ( level.player.origin + ( 0, 0, 55 ) ) );
	...
	{
//		level thread draw_line( highlighted_object, level.player );
...
attackbutton_hold_think()
{
	...
	while( true )
	{
		...
		{
			if( level.attack_button_held && !level.player AttackButtonPressed() )
			{
