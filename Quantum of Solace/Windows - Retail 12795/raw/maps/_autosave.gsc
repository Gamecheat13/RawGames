...
_autosave_game_now( suppress_print )
{
	...
	//If we have the phone up then stall here with the save till we put it away
	
	
	//iprintlnbold("weapon 1" + level.player getcurrentweapon());
	
	while ( level.player getcurrentweapon() == "sony_phone" )
		wait 1;
	
	//iprintlnbold("weapon 2" + level.player getcurrentweapon());
...
{
	return _autosave_game_now( suppress_print );
}
...
autoSaveAmmoCheck()
{
	...
	if (weapons.size == 0)
	{
		return true;	//player deosn't have a weapon, it's ok to save
	}
...
autoSaveThreatCheck( doPickyChecks )
{
	...
	//Never save when an AI is dying but not a corpse yet
	if( isanyaidying("all") )
	{
		return false;
	}
	...
	for ( i = 0; i < enemies.size; i++ )
	{
		...
		if ( distance( enemies[ i ].origin, level.player.origin ) < 200 )
		{
			 /# AutoSavePrint( "autosave failed: Too Close" ); #/ 
			return( false );
		}
		...
		if ( enemies[ i ] GetTimeSinceAction("Shoot") < 0.5 )
		{
		...
		if ( enemies[ i ] GetTimeSinceAction("Aim") < 0.2 && enemies[ i ] animscripts\utility::canShootEnemy() )
		{
...
// autosave_by_name: moved from _utiltiy.gsc
autosave_by_name( name, timeout )
{
...
autosave_by_name_thread( name, timeout )
{
	...
	result = level tryAutoSave(level.curAutoSave, "autosave", imagename, timeout);
