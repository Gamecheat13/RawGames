...
create_stream(stream_name, speed)
{
	stream = GetEnt(stream_name,"targetname");
		
	
	vec_direction = undefined;

	
	assertex( isdefined( stream.target ), "" + stream.targetname + " has no target, can't get the angles" );

	
	ent_direction = getent( stream.target, "targetname" );

	
	
	vec_direction = anglestoforward( ent_direction.angles ) * speed;

	
	stream setconveyor( vec_direction );
}
