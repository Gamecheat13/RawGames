...
glass_logic()
{
	if( isDefined(self.target) )
	{
		...
		if ( IsDefined(link.script_parameters) && link.script_parameters == "linked" )
		{
			self	LinkTo( link );
			cracked	= undefined; 
			broken = undefined;	
		...
		else if( IsDefined( cracked.target ) )
		{
...
mousetrap_stun(offset)
{
	...
	CreateThreatExplosion( selfWorldVec, level.player );

	level.player RadiusDamage( selfWorldVec, (stunmaxrange * 0.9), dmg, 40, level.player, "MOD_COLLATERAL", "WEAPON_NOT_SPECIFIED", false, 0, 0, 0, false );		
...
mousetrap_stun_electrical(offset)
{
	...
	level.player RadiusDamage( selfWorldVec, (stunmaxrange), dmg, 0, level.player, "MOD_COLLATERAL", "WEAPON_NOT_SPECIFIED", false, 0, 0, 0, false );		
...
mousetrap_stun_halon_AI_think(endpos)
{
	...
	while(1)
	{
		...
		level.player RadiusDamage( (endpos[0],endpos[1],(endpos[2] + 48)), 92, 0, 0, self, "MOD_COLLATERAL", "WEAPON_NOT_SPECIFIED", false, 0, 0, 0, false );		
		wait 4;
	}
}
...
mousetrap_kill(offset)
{
	...
	RadiusDamage( selfWorldVec, maxrange, dmg, 1, level.player, "MOD_EXPLOSIVE", "WEAPON_NOT_SPECIFIED", false, 0, 0, 0, false );		
	
	PhysicsExplosionSphere( selfWorldVec, 300, 32, 20 );

	Earthquake( 0.25, 2, self.origin, 1000 );
	
	
	if (self.model == "p_msc_propane_tank" || self.model == "d_exploding_propane_tank_med_full")
	{
		if (isDefined(self))
		{
			self delete();
...
mousetrap_kill_nodelay(offset)
{
	...
	RadiusDamage( selfWorldVec, 300, 300, 1, level.player, "MOD_EXPLOSIVE", "WEAPON_NOT_SPECIFIED", false, 0, 0, 0, false );		
...
mousetrap_kill_lg(offset)
{
	...
	if ( isalive(self) )
	{
		badplace_cylinder("", 5, self.origin, 500, 128, "axis");
	...
	RadiusDamage( selfWorldVec, 500, 500, 1, level.player, "MOD_EXPLOSIVE", false, 0, 0, 0, false );		

	PhysicsExplosionSphere( selfWorldVec, 500, 128, 40 );
