...
create_fx_menu()
{
	...
	else if (menu("change_targetname"))
	{
		if (button_is_clicked("m"))
		{
			increment_targetlist_offset();
			draw_effects_list();		
		}
...
menu_fx_creation()
{
	...
	if (menu("change_targetname"))
	{
		keys = level._createFX_events;	
	}
	else
	{
	...
	{
		apply_option_to_selected_fx( get_option( "targetname" ), picked_fx );
		level.effect_list_offset = 0;
		clear_fx_hudElements();
		setMenu("none");
		return;	
...
menu_init()
{
	...
	addOption("string", "targetname", "Script Event", "<edit to select event>", "all");
	...
	level.createfxMasks["all"]["targetname"] = true;
	...
	level.createfxMasks["fx"]["targetname"] = true;
	...
	level.createfxMasks["targetname"] = [];
	level.createfxMasks["targetname"]["targetname"] = true;	
}
...
prepare_option_for_change( option, drawnCount )
{
	...
	else if ( option["name"] == "targetname" )
	{
		setMenu( "change_targetname" );
		draw_targetname_list();
		return;
	}
...
display_fx_info( ent )
{
	...
	set_fx_hudElement("Total FX Count: " + level.createFXent.size);
	...
	else
	{
		...
		for ( i=0; i<level.createFX_options.size; i++ )
		{
			...
			if ( count > level.createFx_hudElemens )
				break;
		}
	}
}
...
{
	clear_fx_hudElements();
	set_fx_hudElement("Select a Targetname:");
