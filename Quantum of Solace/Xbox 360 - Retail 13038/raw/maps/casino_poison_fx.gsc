...
main()
{
	...
	//this will create and place particle effects on script_models
...
initFXModelAnims()
{
	...
	ent4 = getent( "fxanim_curtain_1", "targetname" );
	ent5 = getent( "fxanim_curtain_2", "targetname" );
	...
	if (IsDefined(ent4)) { ent4 thread curtain_1();   println("************* FX: curtain_1 *************"); }
	if (IsDefined(ent5)) { ent5 thread curtain_2();   println("************* FX: curtain_2 *************"); }
}
...
#using_animtree("fxanim_curtain_1");
curtain_1()
{
	level waittill("curtain_1_start");
	self UseAnimTree(#animtree);
	self animscripted("a_curtain_1", self.origin, self.angles, %fxanim_curtain_1);
}

#using_animtree("fxanim_curtain_2");
curtain_2()
{
	level waittill("curtain_2_start");
	self UseAnimTree(#animtree);
	self animscripted("a_curtain_2", self.origin, self.angles, %fxanim_curtain_2);
}
...
precacheFX()
{
	...
	level._effect["casino_fountain2"] 			= loadfx ("maps/casino/casino_fountain2");
	...
	level._effect["casino_fountain_foam1"]	 	= loadfx ("maps/casino/casino_fountain_foam1");
	level._effect["casino_fountain_foam1"]	 	= loadfx ("maps/casino/casino_fountain_foam2");	
}


//this will automatically place the candle flame effects on every candle prop
auto_createfx_candle()
{
	wait( 1 );
	all_cntrpiece = GetEntArray("cntrpiece", "script_noteworthy");
	//println("****** FX: total centerpieces found = " + all_cntrpiece.size + " ******");
	...
	if (IsDefined(all_cntrpiece)) 
	{
		...
		{
			//println("****** FX: link_effect_to_ent: = " + (i+1) + " ******");
...
//lets link an effect to an entity
