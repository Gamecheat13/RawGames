...
main()
{
	...
	strings = [];
	strings[0] = &"UNLOCK_WHITES_ESTATE_DEBRIEFING";
	strings[1] = &"UNLOCK_WHITES_ESTATE_LAKE_COMO";
...
set_up_models()
{
	idleAnim = %male_relax_stand_idleshow_none;
...
stop_rotate_p99( whites_p99, trigger, stop_trigger )
{
	...
	tutorial_message("@UNLOCK_WHITES_ESTATE_WEAPON_01", 1);
...
stop_rotate_1911( whites_1911, trigger, stop_trigger )
{
	...
	tutorial_message("@UNLOCK_WHITES_ESTATE_WEAPON_02", 1);
	level.player waittill( "tutorialclosed" );
...
stop_rotate_saf45( whites_saf45, trigger, stop_trigger )
{
	...
	tutorial_message("@UNLOCK_WHITES_ESTATE_WEAPON_03", 1);
	level.player waittill( "tutorialclosed" );
...
stop_rotate_phone( whites_phone, trigger, stop_trigger )
{
	...
	tutorial_message("@UNLOCK_WHITES_ESTATE_WEAPON_04", 1);
	level.player waittill( "tutorialclosed" );
...
concept_info_01()
{
	...
	while ( 1 )
	{
		...
		tutorial_message("@UNLOCK_WHITES_ESTATE_CONCEPT_01", 1);
		level.player waittill( "tutorialclosed" );
		wait( .1 );
		level.player unlink();
		org delete();
...
concept_info_02()
{
	...
	while ( 1 )
	{
		...
		tutorial_message("@UNLOCK_WHITES_ESTATE_CONCEPT_02", 1);
		level.player waittill( "tutorialclosed" );
		wait( .1 );
		level.player unlink();
		org delete();
...
concept_info_03()
{
	...
	while ( 1 )
	{
		...
		tutorial_message("@UNLOCK_WHITES_ESTATE_CONCEPT_03", 1);
		level.player waittill( "tutorialclosed" );
		wait( .1 );
		level.player unlink();
		org delete();
...
concept_info_04()
{
	...
	while ( 1 )
	{
		...
		tutorial_message("@UNLOCK_WHITES_ESTATE_CONCEPT_04", 1);
		level.player waittill( "tutorialclosed" );
		wait( .1 );
		level.player unlink();
		org delete();
...
link_weapons()
{
	base = getent("whites_saf45", "targetname");
...
level_end1()
{
	...
	trigger sethintstring (&"UNLOCK_WHITES_ESTATE_EXIT");
	trigger waittill("trigger");
	changelevel( "", false, 0 );
}
