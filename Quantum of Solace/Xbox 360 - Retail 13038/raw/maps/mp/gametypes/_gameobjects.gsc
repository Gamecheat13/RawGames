...
Sets this use object to require any one of these carry objects
=============
*/
...
useObjectUseThink()
{
	...
	while ( true )
	{
		...
		if ( self.useTime > 0 )
		{
			...
			{
//				//iprintln( "end" );
//				//player SetAnimScriptEvent( "phonedownloaddrop" );
//				//player ClearAnimScriptEventSoon( 0.75 );
...
//ClearAnimScriptEventSoon( time )
//{
//	wait time;
//	self ClearAnimScriptEvent();
//	self SwitchToWeapon( self.preSearchWeapon );
//}
...
useHoldThink( player )
{
	...
	{
//		player SetAnimScriptEvent( "phonedownloadraise" );
//		player.loopStart = getTime() + 460;
	}
	...
	while( isAlive( player ) /*&& player isTouching( self.trigger )*/ && player useButtonPressed() && self.curProgress < self.useTime ) // commented out 'player isTouching( self.trigger )' as this would cause the interaction to stop immediately when the interaction range was greater than the trigger distance
	{
		self.curProgress += (100 * self.useRate);
...
//		if ( player.loopStart != 0 && player.loopStart <= getTime() )
//		{
//			player.loopStart = 0;
//			//iprintln( "middle" );
//			player SetAnimScriptEvent( "phonedownload" );
//		}
