...
guy_enter ( guy, vehicle, lastguy )
{
	...
	if( isai( guy ) )
	{
		...
		{
 			//assumes first turret is the only turret for now
	...
	//in some vehicles (the halftrack in CoD2) the driver is the last 
	//guy out this gives him a larger delay than the last guy.
...
guy_runtovehicle ( guy, vehicle )
{
	...
	if(!isdefined(chosenorg))
		return; //the spot has been taken
...
/*
	...
	{
		guy hide();
		level.player playerlinktodelta (guy,"tag_origin",1.0,120,120,90,90);
...
//	range = int(dist );
	...
	if(time < 0)
		time = 1; //crazy math not working out so well
