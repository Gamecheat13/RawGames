...
main()
{
	...
	level thread bus_brakelights();
	level thread rooftile_effects();
}
...
create_boss_fight_fx()
{
	...
	level thread maps\_fx::play_igcEffect(level.badguy, level.player, "fx_r_shoulder_dust", "qkill_hit_blood_r", "L_Wrist", -1);  // note is on wrong guy
	...
	level thread maps\_fx::play_igcEffect(level.badguy, level.badguy, "fx_spinemid_big_impact", "qkill_dust_big", "SpineUpper", -1);

	level.player waittillmatch( "anim_notetrack", "fx_pigeons_launch" );
	level notify("fx_tower_birds");
	
	level.player waittillmatch( "anim_notetrack", "tower_wall_hit_2" );
	level notify("fx_tower_wall_hit2");
}	
...
rooftile_effects()
{		
	level thread rooftile_effects1();
	level thread rooftile_effects2();
...
rooftile_effects1()
{
	...
	println("****** FX: rooftile_effects1 found! ******");
	
	
	while(1)
	{
		wait(0.033);
		mitchell_model = getent("mitchell", "targetname");
		if( isdefined( mitchell_model ) )
		{
			break;
		}			
	}	
	
	mitchell_model waittillmatch("anim_notetrack", "mitch_slide_start");
	level notify ( "fx_rooftiles" );	
...
rooftile_effects2()
{
	...
	{
		wait(0.033);
		bond_model = level.player;
		if( isdefined( bond_model ) )
		{
			break;
		}			
	}	
	bond_model waittillmatch("anim_notetrack", "bond_slide_start");
		wait(0.333);
	level notify ( "fx_rooftiles2" );	
...
bell_tower_top_01()
{
	...
	wait(.15);
	//added by K. Drew to destroy wall sections
...
bus_brakelights()
{
	the_bus = getent("gallery_bus", "targetname");
	//gallery_bus
	
	if ( IsDefined(the_bus) )
	{
		//low
		the_bus link_effect_to_ent( "siena_brakelight1", "tag_light_L_back", (125,-22,15), (0,0,0) );
		the_bus link_effect_to_ent( "siena_brakelight1", "tag_light_R_back", (125,22,15), (0,0,0) );
		//high
		the_bus link_effect_to_ent( "siena_brakelight2", "tag_light_L_back", (123,-10,97.5), (0,30,0) );
		the_bus link_effect_to_ent( "siena_brakelight2", "tag_light_R_back", (123,10,97.5), (0,-30,0) );
	}
}
...
precacheFX()
{
	...
	level._effect["siena_broken_pipe"]	 		= loadfx ( "maps/siena/siena_broken_pipe");
	//level._effect["siena_rats1"]	 			= loadfx ( "maps/siena/siena_rats1"); //work-in-progress, disabled for now
	...
	level._effect["siena_brakelight1"]	 		= loadfx ( "maps/siena/siena_bus_light1");
	level._effect["siena_brakelight2"]	 		= loadfx ( "maps/siena/siena_bus_light2");
	
	//from test_bossfight_qk	
    ...
    level._effect["qkill_dust_big"]         = loadfx("impacts/qkill_hit_dust_big");
	...
	level._effect["siena_rooftiles"]	 		= loadfx ( "maps/siena/siena_rooftiles");
}
