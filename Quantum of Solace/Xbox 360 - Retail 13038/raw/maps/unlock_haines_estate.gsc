...
main()
{
	...
	strings = [];
	strings[0] = &"UNLOCK_HAINES_ESTATE_DEBRIEFING";
	strings[1] = &"UNLOCK_HAINES_ESTATE_QUANTUM_PART_II";
...
stop_rotate_p99( whites_p99, trigger, stop_trigger )
{
	...
	tutorial_message("@UNLOCK_HAINES_ESTATE_WEAPON_01", 1);
...
stop_rotate_1911( whites_1911, trigger, stop_trigger )
{
	...
	tutorial_message("@UNLOCK_HAINES_ESTATE_WEAPON_02", 1);
	level.player waittill( "tutorialclosed" );
...
weapons_display_saf45()
{
	...
	thread attach_ammo_bucket_func();
...
//avulaj
//attach the ammo bucket to the m60
attach_ammo_bucket_func()
{
	whites_saf45 = GetEnt( "whites_saf45", "targetname" );

	attachment007 = getent("attachment007", "targetname");
	attachment007 linkto( whites_saf45, "TAG_CLIP_M60E3", ( 0, 0, 0 ), ( 0, 0, 0 ));
}
...
stop_rotate_saf45( whites_saf45, trigger, stop_trigger )
{
	...
	tutorial_message("@UNLOCK_HAINES_ESTATE_WEAPON_03", 1);
	level.player waittill( "tutorialclosed" );
...
stop_rotate_phone( whites_phone, trigger, stop_trigger )
{
	...
	tutorial_message("@UNLOCK_HAINES_ESTATE_WEAPON_04", 1);
	level.player waittill( "tutorialclosed" );
...
concept_info_01()
{
	...
	while ( 1 )
	{
		...
		tutorial_message("@UNLOCK_HAINES_ESTATE_CONCEPT_01", 1);
		level.player waittill( "tutorialclosed" );
		wait( .1 );
		level.player unlink();
		org delete();
...
concept_info_02()
{
	...
	while ( 1 )
	{
		...
		tutorial_message("@UNLOCK_HAINES_ESTATE_CONCEPT_02", 1);
		level.player waittill( "tutorialclosed" );
		wait( .1 );
		level.player unlink();
		org delete();
...
concept_info_03()
{
	...
	while ( 1 )
	{
		...
		tutorial_message("@UNLOCK_HAINES_ESTATE_CONCEPT_03", 1);
		level.player waittill( "tutorialclosed" );
		wait( .1 );
		level.player unlink();
		org delete();
...
concept_info_04()
{
	...
	while ( 1 )
	{
		...
		tutorial_message("@UNLOCK_HAINES_ESTATE_CONCEPT_04", 1);
		level.player waittill( "tutorialclosed" );
		wait( .1 );
		level.player unlink();
		org delete();
...
link_weapons()
{
	base = getent("whites_p99", "targetname");
	...
	base2 = getent("whites_1911", "targetname");
...
level_end1()
{
	...
	trigger sethintstring (&"UNLOCK_HAINES_ESTATE_EXIT");
	trigger waittill("trigger");
	changelevel( "", false, 0 );
}
