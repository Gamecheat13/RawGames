...
fx_init()
{
	...
	waittillframeend; // so the effects get setup by the maps fx thread
		...
		for ( i = 0; i < level.createFXent.size; i++ )
		{
			...
			if (isdefined(highlightedEnt) && ent == highlightedEnt)
			{
				...
				if (leftClick)
				{
					...
					selecting = !entWasSelected; // used for drag select/deselect
					if (!ctrlHeld)
					{
...
/#	
	...
	cfxprintln (file,"#include maps\\_utility;");
...
show_help()
{
	...
	set_fx_hudElement("S              SAVES your work");
