...
RestorePlayerWeaponStatePersistent( slot )
{
	...
	{
		//for playing the second part of the level only in dev
		game[ "gaveweapons" ] = 1;
...
give_loadout()
{
	...
	else if (level.script == "sciencecenter_b" || level.script == "sciencecenter_b_geo" )
	{
		...
		if ( !isDefined( game[ "weaponstates" ] ) )
		{
			//if the player is loading science center b from the chapter select or restarting the level
			game[ "gaveweapons" ] = 1;
			level.player giveWeapon("p99_s");
			level.player switchtoWeapon("p99_s");
		...
		else
		{
			...
			level.player takeallweapons();
			//more hard coded transfer of weapons if the player is coming directly from science center a
			for ( weapIdx = 0; weapIdx < game[ "weaponstates" ][ slot ][ "list" ].size; weapIdx++ )
			{
				weapName = game[ "weaponstates" ][ slot ][ "list" ][ weapIdx ][ "name" ];

				//handles conversion of wet version to dry for p99
...
setup_bond_sinkhole()
{
	level.player character\character_bond_sinkhole::main();
	level.campaign = "american";
}
...
setup_bond_siena_roof()
{
	level.player character\character_bond_sienarooftop::main();
	level.campaign = "american";
}
