...
main()
{
	...
	// Register FX Events
	registerFXTargetName("fx_debug");							// This level notify will be sent to the level when event occurs, to hook particle triggering
	...
	//exterior events
	registerFXTargetName("davinchi_swing");				//level notify starts the davinchi mousetrap
	...
	//IGC KnifeFight Effects
	//level thread igcFXknifefight();
	...
	//set fog
//	setExpFog(0,2900,0.17,0.23,0.3,0.3);
...
initFXModelAnims()
{
	...
	//dav_scaffolding = getent( "fxanim_scaffolding", "targetname" );
	...
	ent1 = getent( "fxanim_rope_dangle", "targetname" );
	...
	if (IsDefined(ent1)) { ent1 thread rope_dangle();   println("************* FX: rope_dangle *************"); }
	if (IsDefined(ent1)) { ent1 thread rope_wiggle();   println("************* FX: rope_wiggle *************"); }
	...
	start_fxfunc( "fxanim_sc_banner_01",		::sc_banner_animation_01,		"sc_banner_01_start" );
	start_fxfunc( "fxanim_sc_banner_02",		::sc_banner_animation_02,		"sc_banner_02_start" );
	start_fxfunc( "fxanim_sc_banner_03",		::sc_banner_animation_03,		"sc_banner_03_start" );
	start_fxfunc( "fxanim_sc_banner_04",		::sc_banner_animation_04,		"sc_banner_04_start" );
	start_fxfunc( "fxanim_sc_banner_05",		::sc_banner_animation_05,		"sc_banner_05_start" );
	start_fxfunc( "fxanim_sc_banner_06",		::sc_banner_animation_06,		"sc_banner_06_start" );
	
	if (IsDefined(dav_swing)) 
	{ 
		//attach the burning rope effects (why don't these move?)
		//level thread davinchi_rope01();
		//level thread davinchi_rope02();
		//level thread davinchi_rope03();
		//level thread davinchi_rope04();
	...
	/*if (IsDefined(dav_scaffolding)) 
	{
		dav_scaffolding thread davinchi_scaffolding();   
		println("************* davinchi scaffolding ****************"); 
	}*/
...
placeVert_effect()
{
	...
	//println("************* FX: Moving vent effect ****************"); 
	ent_tag moveto( (-136.5, 5735.7, 816.205), 0.1 ); //CG - moving the effect beyond it's clip
}
...
davinchi_swing()
{
		...
		//start the particle runners
		level notify ("cowbell_particles_start");
}

/*#using_animtree("fxanim_scaffolding");
davinchi_scaffolding()
{
		level waittill("davinchi_start");
		self UseAnimTree(#animtree);
		self animscripted("d_scaffolding", self.origin, self.angles, %fxanim_scaffolding);
}*/
...
#using_animtree("fxanim_sc_banner_01");
sc_banner_animation_01( msg )
{
	self UseAnimTree(#animtree);
	while (IsDefined(self))
	{
		...
		self animscripted("a_sc_banner", self.origin, self.angles, %fxanim_sc_banner_01);
	}
}

#using_animtree("fxanim_sc_banner_02");
sc_banner_animation_02( msg )
{
	self UseAnimTree(#animtree);
	while (IsDefined(self))
	{
		level waittill( msg );
		self stopanimscripted();
		self animscripted("a_sc_banner", self.origin, self.angles, %fxanim_sc_banner_02);
	}
}

#using_animtree("fxanim_sc_banner_03");
sc_banner_animation_03( msg )
{
	self UseAnimTree(#animtree);
	while (IsDefined(self))
	{
		level waittill( msg );
		self stopanimscripted();
		self animscripted("a_sc_banner", self.origin, self.angles, %fxanim_sc_banner_03);
	}
}

#using_animtree("fxanim_sc_banner_04");
sc_banner_animation_04( msg )
{
	self UseAnimTree(#animtree);
	while (IsDefined(self))
	{
		level waittill( msg );
		self stopanimscripted();
		self animscripted("a_sc_banner", self.origin, self.angles, %fxanim_sc_banner_04);
	}
}

#using_animtree("fxanim_sc_banner_05");
sc_banner_animation_05( msg )
{
	self UseAnimTree(#animtree);
	while (IsDefined(self))
	{
		level waittill( msg );
		self stopanimscripted();
		self animscripted("a_sc_banner", self.origin, self.angles, %fxanim_sc_banner_05);
	}
}

#using_animtree("fxanim_sc_banner_06");
sc_banner_animation_06( msg )
{
	self UseAnimTree(#animtree);
	while (IsDefined(self))
	{
		level waittill( msg );
		self stopanimscripted();
		self animscripted("a_sc_banner", self.origin, self.angles, %fxanim_sc_banner_06);
	}
}

#using_animtree("fxanim_rope_dangle");
rope_dangle()
{
		level waittill("rope_dangle_start");
		self UseAnimTree(#animtree);
		self animscripted("a_rope_dangle", self.origin, self.angles, %fxanim_rope_dangle);
}
...
#using_animtree("fxanim_rope_wiggle");
rope_wiggle()
{
		level waittill("rope_wiggle_start");
		self StopAnimScripted();
		self StopUseAnimTree();
		self UseAnimTree(#animtree);
		self animscripted("a_rope_wiggle", self.origin, self.angles, %fxanim_rope_wiggle);
		wait(2.6);
		self rope_dangle_resume();
}
...
//	level._effect["rain8"] 								= loadfx("maps/miamisciencecenter/science_rain_8");
//	level._effect["rain9"] 								= loadfx("maps/miamisciencecenter/science_rain_9");
	level._effect["science_dripping_rain01"] 			= loadfx("maps/miamisciencecenter/science_dripping_rain01");
		
	//PLEASE do not use deprecated FX! ****
//	level._effect["rain"] 								= loadfx("weather/rain_heavy");
//	level._effect["rain_mist"] 							= loadfx("weather/rain_heavy_mist");
	
	//PLEASE do not use deprecated FX! ****
	//level._effect["lights_expl"] 						= loadfx("explosions/wall_small_explosion");
	...
	level._effect["rocket_hotness"] 					= loadfx("smoke/smoke_trail_rocket"); // fake rocket launcher that shoots at the player.
	...
	//level._effect["science_elevator_bullet_hits"]   = loadfx ("maps/miamisciencecenter/science_elevator_bullet_hits");
	...
	//level._effect["science_column_dest01"]	= loadfx ("maps/miamisciencecenter/science_column_dest01");
	
	level._effect["science_elevator_exp"]   	= loadfx ("maps/miamisciencecenter/science_elevator_exp");
	level._effect["science_gas_exp"]   			= loadfx ("maps/miamisciencecenter/large_exp");
	...
	//Fire for DaVinci from rpg stolen from airport
	level._effect[ "rpg_fire" ] 			= loadfx( "maps/miamisciencecenter/science_rpg_fire" );
	level._effect[ "floor_fire" ] 			= loadfx( "maps/miamisciencecenter/science_floor_smallfire" );
	//level._effect[ "rpg_fire" ] 			= loadfx( "maps/MiamiAirport/airport_propane_fire" );
	
	//for elevator IGC
	level._effect[ "combat_punch1" ] 		= loadfx( "impacts/combat_punch1" );
	level._effect[ "combat_wall_dust1" ] 	= loadfx( "impacts/combat_wall_dust1" );
	
	level._effect[ "science_vent_light_vol" ] 	= loadfx( "maps/miamisciencecenter/science_vent_light_vol" );
...
// notes for jean deleted heli and shadow brush

// grabed table with objects and turned them into physics

// anims to place inside of the csv
// rawfile,animtrees/fxanim_davinchi_swing.atr
//rawfile,animtrees/fxanim_scaffolding.atr
