...
main(bScriptgened)
{
	...
	if (level.ps3)
	{
		// turn off SSAO for PS3
		setdvar("r_ssaoEnable", 0);
	}
...
//	level.ai_array = [];
	...
	level.player thread maps\_utility::mantle_monitor();
...
phone_anim_2d_safety_notify()
{
	self endon("phonemenu"); // closed phone
	wait( 1.0 ); // wait 1 second
	level.player NotifyPhoneRaised(); // open 2d phone
}
...
phone_anim()
{
	...
	while( true )
	{
		...
		if( phone_state == 1 )
		{
			...
			level thread phone_anim_2d_safety_notify();
		}
		else if( phone_state == 0 )
		{
			...
			if( IsDefined( curr_weapon ) )
			{
				if( curr_weapon == "none" )
				{
					if (level.player isweaponholstered())
					{
						level.player SwitchToNoWeapon( );
					}
					else
					{
						level.player cycleweapon();
					}
				} else {
...
disable_corner_aim(trig)
{
	while (isdefined(trig))
	{
		...
		while (isdefined(trig) && level.player IsTouching(trig))
		{
			wait .05;
		}
