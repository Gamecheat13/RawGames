...
methodsInit()
{
	...
	level._pipe_methods["MOD_HEAD_SHOT"] 			= ::pipe_calc_assert;
	level._pipe_methods["MOD_CRUSH"] 				= ::pipe_calc_assert;
	level._pipe_methods["MOD_TELEFRAG"] 			= ::pipe_calc_assert;
	level._pipe_methods["MOD_FALLING"] 				= ::pipe_calc_assert;
	level._pipe_methods["MOD_SUICIDE"] 				= ::pipe_calc_assert;
	level._pipe_methods["MOD_TRIGGER_HURT"] 		= ::pipe_calc_splash;
	level._pipe_methods["MOD_EXPLOSIVE"] 			= ::pipe_calc_splash;
	level._pipe_methods["MOD_COLLATERAL"] 			= ::pipe_calc_splash;
}
...
{
	assertMsg("Pipe at (" + self getorigin() + ")impacted with unknown type: " + type + ".");
}
...
pipefx(P, vec)
{
	if(self.script_noteworthy != "fire")
	{
		playfx ( level._effect["pipe_interactive"][self.script_noteworthy], P, vec ); //(vec[0],0,vec[2]));
...
{	
	...
	for(i=0; i< self.size; i++)
	{
		...
		//debug
		//println("pipeSteam: Precache FX Load");
...
// water = wine  - DO --- No it doesn't - KDW
...
// BOND MOD 
// KDW 7/23: True wine implementation
	for(i=0; i< self.size; i++)
	{
		if(self[i].script_noteworthy != "wine")
			continue;
			
		level._effect["pipe_interactive"][self[i].script_noteworthy]		= loadfx("impacts/pipe_wine");  
		level._sound["pipe_interactive"][self[i].script_noteworthy] 		= "mtl_water_pipe_hit";	
		level.pipe_fx_time[self[i].script_noteworthy]			= 2.6;
		break;
	}
