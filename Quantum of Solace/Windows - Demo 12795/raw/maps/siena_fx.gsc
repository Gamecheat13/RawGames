...
main()
{
	...
	//level thread stagger_water_flow();
...
column_fall()
{
	...
	if (animhasnotetrack(%fxanim_column_fall, "mid_section") && animhasnotetrack(%fxanim_column_fall, "water_hit_1") && animhasnotetrack(%fxanim_column_fall, "water_hit_2"))
	{
		...
		self waittillmatch("a_column_fall", "top_btm_section");
			...
			//Steve G - column hit wall
			//crash_org = spawn("script_origin", gate getorigin());
		...
		//self waittillmatch("a_column_fall", "water_hit_2");
		//level notify("fx_brick_dust");
	}
}
...
spawn_water_fx()
{
	//array key 0-5 = "siena_statue_water1", 1-11 = "siena_statue_splash", 
	//12-22 = "siena_water_flow_start1", 23-33 = "siena_water_splash1", 34-40 = "siena_water_foam2"
	all_water_fx_pos = [];
	all_water_fx_ang = [];
	all_water_markers = [];
	
	all_water_fx_pos[0]  = (-360.148,-626.952,-160.892); //not used	
	...
	all_water_fx_pos[36] = (-1360.57,-505.657,-210.0);//first right edge
	...
	for(i = 0; i < all_water_fx_pos.size; i++)
		all_water_fx_ang[i] = (270,0,0); //initialize angles with the default
	...
	all_water_fx_ang[36] = (270,318.384,-46.3838);//first right edge   	
	...
	//master_ent_tag = Spawn("script_model", all_water_fx_pos[0]); //all moving markers are parented to this one
	...
	for(i = 0; i < all_water_fx_pos.size; i++)
	{
		...
		all_water_markers[i] = ent_tag; //collecting the markers
	}
	
	for(i = 1; i < 12; i++) //creating the statue water splash effects
		PlayFxOnTag(level._effect["siena_statue_splash"], all_water_markers[i], "tag_origin");	
	
	level thread play_water_flow(all_water_markers[12], all_water_markers[23], "fx_water_flow1"); //left side
	...
	level thread play_water_flow(all_water_markers[19], all_water_markers[30], "fx_water_flow4"); //right side
	...
	//commence the jiggling (moving the effects up)!
	...
	for(i = 34; i < all_water_markers.size; i++) //creating the water foam effects
		PlayFxOnTag(level._effect["siena_water_foam2"], all_water_markers[i], "tag_origin");	
	
	level thread move_these_fx_ents(all_water_markers,1,12,-25,20); //statue splashes
	level thread move_these_fx_ents(all_water_markers,34,all_water_markers.size,-25,20); //water foam
}
...
precacheFX()
{
	...
	level._effect["siena_collapse4"]	 		= loadfx ( "maps/siena/siena_collapse4");
	...
	level._effect["siena_wall_spray_hit"]	 	= loadfx ( "maps/siena/siena_wall_spray_hit");
...
//	level._effect["siena_pillar_splash"]	 	= loadfx ( "maps/siena/siena_pillar_splash_r");
	...
	level._effect["rats"]	 					= loadfx ( "maps/siena/siena_rats_spawner");
	level._effect["siena_intro_dust"]	 		= loadfx ( "maps/siena/siena_intro_dust");
	...
	if ( level.ps3 == true || level.bx ) //GEBE
	{
	level._effect["siena_collapse1"]	 		= loadfx ( "maps/siena/siena_collapse1_PS3");
	level._effect["siena_collapse2"]	 		= loadfx ( "maps/siena/siena_collapse2_PS3");
	level._effect["siena_collapse2_r"]	 		= loadfx ( "maps/siena/siena_collapse2_r_PS3");
	level._effect["siena_collapse4"]	 		= loadfx ( "maps/siena/siena_collapse4_PS3");
	level._effect["siena_pillar_aftermath1"]	= loadfx ( "maps/siena/siena_pillar_aftermath1_PS3");
	level._effect["siena_pillar_aftermath2"]	= loadfx ( "maps/siena/siena_pillar_aftermath2_PS3");
	}
else
	{
	level._effect["siena_collapse1"]	 		= loadfx ( "maps/siena/siena_collapse1");
	...
	level._effect["siena_collapse2_r"]	 		= loadfx ( "maps/siena/siena_collapse2_r");
	level._effect["siena_collapse4"]	 		= loadfx ( "maps/siena/siena_collapse4");
	level._effect["siena_pillar_aftermath1"]	= loadfx ( "maps/siena/siena_pillar_aftermath1");
