/////////
// Scripted damage system for bond vehicles
/////////

///////////////////////////////////////////////////////////////////
// bond_veh_damage - Do localized damage to vehicle
///////////////////////////////////////////////////////////////////
main()
{
	//
	// note: bullet damage doesn't affect the vehicle model, other than impact decals.
	//       only collisions affect model swaps
	...
	while (true)
	{
		...
		//Line(self.origin, self.origin + v_point);
...
damage_front()
{
	...
	while (true)
	{
		...
		if ((front_damage > damage_swap) && (damage_stage < 2))	// cap at 2 stages
		{
			damage_stage++;
			front_damage = 0;

			//iPrintLnBold("damage front");
			...
			{
				// There is no front model that's damaged in the front without grind damage on both sides, we need one here without left side damage
				//attach_part("front_f", damage_stage, "front_rf");
			}
			else if (self.damaged_left)
			{
				// There is no front model that's damaged in the front without grind damage on both sides, we need one here without right side damage
				//attach_part("front_f", damage_stage, "front_lf");
			}
			else
			{
				// There is no front model that's damaged in the front without grind damage on both sides, we need one here without any side damage
				//attach_part("front_f", damage_stage, "front", 0);
			}
		}
	}
}
...
damage_left()
{
	...
	while (true)
	{
		...
		damage_swap = self.currenthealth / 4;
		if ((left_damage > damage_swap) && (damage_stage < 2))	// cap at 2 stages
		{
			damage_stage++;
			left_damage = 0;

			//iPrintLnBold("damage left");
...
damage_right()
{
	...
	while (true)
	{
		...
		damage_swap = self.currenthealth / 4;
		if ((right_damage > damage_swap) && (damage_stage < 2))	// cap at 2 stages
		{
			damage_stage++;
			right_damage = 0;

			//iPrintLnBold("damage right");
...
////
// setup - setup arrays that have the model names of all the parts
////
...
//	{
//		return level.vehicle_parts[self.base_model][part][stage]["attached"];
//	}
//
//	return false;
//}
