...
/#
	...
	if( !IsDefined( level.mdl_previewer_model ) )
	{
		player = GetEnt( "player", "classname" );
		level.mdl_previewer_model = Spawn( "script_model", player.origin );
		level.mdl_previewer_model SetModel( dvar );
		
		level.mdl_previewer_model thread debug_mdl_model_movement();
	}	
	
	if( dvar != level.mdl_previewer_model.model )
	{
		level.mdl_previewer_model SetModel( dvar );
	}
#/
...
/#
	...
	while( 1 )
	{
		...
		pos = player GetEye() + ( 0, 0, -40 ) + vector_multiply( forward, 100 );
