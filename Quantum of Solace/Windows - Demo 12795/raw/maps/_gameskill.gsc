...
{	
	...
	while(1)
	{
		...
		//Only do any work if something changed
...
setSkill( reset )
{
 ...
 // 	createprintchannel( "script_autodifficulty" );
	...
	if( !isdefined( level.bx ) ) //GEBE
		level.bx = ( getdvar( "BxGame" ) == "true" );
	...
	level.difficultySettings[ "playerDifficultyHealth" ][ "easy" ]		= 300;	// damage is reduced   to 33%
	level.difficultySettings[ "playerDifficultyHealth" ][ "normal" ]	= 200;	// damage is reduced   to 50%
	level.difficultySettings[ "playerDifficultyHealth" ][ "hardened" ]	= 150;	// damage is increased to 66%
	level.difficultySettings[ "playerDifficultyHealth" ][ "veteran" ]	= 100;	// damage is increased to 100%
	...
	// how long it takes to regen the low, medium, and high regions of your health (<.33, <.66, <=1)
	...
	level.difficultySettings[ "highRegenTime" ][ "veteran" ]  = getdvarint( "regentime_high" );
	 ...
	 // Sniper Breath time difficulty adjustments
	...
	if( level.xenon ) //GEBE PC version seems ok
	{
		 // xenon
	...
	else
	{
		...
		level.difficultySettings[ "playerHealth_RegularRegenDelay" ][ "easy" ] = 250; // 1500; // 4700;
		level.difficultySettings[ "playerHealth_RegularRegenDelay" ][ "normal" ] = 250; // 1500; // 4700;
		level.difficultySettings[ "playerHealth_RegularRegenDelay" ][ "hardened" ] = 250; // 1500; // 4700;
		level.difficultySettings[ "playerHealth_RegularRegenDelay" ][ "veteran" ] = 250; // 1500; // 4700;
...
setDifficultyFractionalValues()
{
	...
	// regen rate is 1%.  this determines how long we delay between 1% regens.
...
playerHealthRegen()
{
	...
	// add var to suspend health regen
...
/*
	level.hudinvul = newHudElem();
	level.hudinvul.alignX = "left";
	level.hudinvul.alignY = "bottom";
	level.hudinvul.horzAlign = "left";
	level.hudinvul.vertAlign = "bottom";
	level.hudinvul.x = 20;
	level.hudinvul.y = 10;
	level.hudinvul setshader( "white", 32, 8 );
	level.hudinvul.color = ( 1, 0, 0 );
	level.hudinvul.alpha = 0.2;
...
*/
	...
	//adam
	regenRate = .009;	//This is the fastest way to tweak our regen. (was.015, lowered b/c we 2x while incover -jc)
	...
	//SetDVar("cg_barrelHealthLevel","0.1");
	//SetDVar("cg_barrelMinAlpha","0.3");
	//SetDVar("cg_barrelMaxAlpha","0.6");
	...
	for( ;; )
	{
		// check to see if health regen has been suspended
...
//				playerJustGotRedFlashing = true;
		...
		if( player.health / player.maxHealth >= oldratio )
		{
			...
			{
				//recover faster in cover - jc
				if( level.player isincover() )
				{
					newHealth += (1.8 * regenRate);	//adam
				}
				else
				{
					newHealth += regenRate;
		...
		//This function can be too slow at time... pause for 1 frame
		wait(0.05);
		...
		if( player.health <= 1 )
		{
//			level.hudinvul.alpha = 1;
//			thread hideInvul( getCurrentDifficultySetting("player_deathInvulnerableTime") / 1000 );
...
computeBlurVal( healthRel )
{
	...
	{
		//Dont make this linear... 
		//This will ramp in between linear and "exponentially"
		blurRel = (1-healthRel);
...
//		{
//			blurVal = computeBlurVal( self.health / self.maxHealth );
//			
//			setblur( blurVal, 0.30 );			
//		}
//		else
//		{
//			setblur( 0, 0 );
	...
	else
	{
		...
		while(1)
		{
			if( (self.health/self.maxHealth) < .25)
			{
				...
				while( (self.health/self.maxHealth) < min( getdvarfloat("player_deathInvulnerableReactivatePercent") + .1, 1.0) )
				{
				//	if( (self.health/self.maxHealth) < .1)
				//	{
				//		VisionSetSecondary(1);
				//	}
				//	else
				//	{
				//		VisionSetSecondary(.7);
				//	}
				//	//VisionSetSecondary( 1 - ( (self.health / self.maxHealth) * 2) );
					wait(0.05);
				}
				cur_sat = 0.8;
				inc_sat = 1 / 24;
			...
			{
				//VisionSetSecondary( (1 - self.health/self.maxHealth) * 0.5);
				VisionSetSecondary(0);
			}
			wait(0.05);
		}
...
auto_adjust_difficulty()
{
	...
	for( ;; )
	{
		...
		gameSkill = getdvarint( "level_gameskill" );
		 ...
		 // cap at 50% through hardened to Double-Oh
