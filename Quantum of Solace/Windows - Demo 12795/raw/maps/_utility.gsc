...
stinger_ondeath()
{
	...
	if ( level.ps3 )
	{
		self playsound("death_stinger_ps3");
	}
	else
	{
...
missionFailedWrapper()
{
	...
	if ( level.ps3 )
	{
		level.player playsound("death_stinger_ps3");
	}
	else
	{
		level.player playsound("death_stinger");
	}
...
{
	self endon("death");
	level.player.mantle_active = 0;

	while(1)
	{
		self waittill("mantle", notice);

		if(notice == "begin")
		{
			level.player.mantle_active = 1;
			level.player notify( "mantle_begin" );
		}
		else if(notice == "abort")
		{
			level.player.mantle_active = 0;
			level.player notify( "mantle_abort" );
		}
		else if(notice == "end")
		{
			level.player.mantle_active = 0;
			level.player notify( "mantle_end" );
		}
	}
}
...
play_dialogue( sound_alias, play_as_radio )
{
	if(isdefined(self))
	{
		...
		{
			if( isdefined(self.isBeingQuickKilled) && !self.isBeingQuickKilled )
			{
	...
	if (IsDefined(self.model) && (self.model != ""))
	{
		if (ModelHasTag(self.model, "TAG_REDLIGHT") && !IsDefined(self.light_fx_red))
		{
			...
			else
			{
				...
				{
					tag_angles = self GetTagAngles("TAG_REDLIGHT");
					vec_forward = AnglesToForward(tag_angles);
					vec_up = AnglesToUp(tag_angles);

					self.light_fx_red = SpawnFx(level._effect["light_red"], self GetTagOrigin("TAG_REDLIGHT"), vec_forward, vec_up);
...
green_light()
{
	...
	if (IsDefined(self.model) && (self.model != ""))
	{
		...
		{
			tag_angles = self GetTagAngles("TAG_GREENLIGHT");
			vec_forward = AnglesToForward(tag_angles);
			vec_up = AnglesToUp(tag_angles);

			self.light_fx_green = SpawnFx(level._effect["light_green"], self GetTagOrigin("TAG_GREENLIGHT"), vec_forward, vec_up);
...
timer_restart( iSeconds )
{
	...
	wait(1);
	level.player playlocalsound("ui_timer_reset");
...
{
	level.player playlocalsound("ui_timer_activate");
	timer_run_internal( iSeconds );
