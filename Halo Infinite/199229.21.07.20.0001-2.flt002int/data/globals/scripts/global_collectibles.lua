...
function GlobalSkull(device:object, p_player:object)
	...
	SoundImpulseStartServer(TAG('sound\004_modes\004_mod_openworld\004_mod_ow_skulls_pickup.sound') , p_player);
	UnlockCollectible(device)
	object_destroy (device);
end
...
function UnlockCollectible(collectibleObject:object)
	dprint ("got collectible ",collectibleObject, "this doesn't do anything anymore");
--removing
