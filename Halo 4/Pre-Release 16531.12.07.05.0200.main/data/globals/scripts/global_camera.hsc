...
local boolean b_return = FALSE;
	...
	if ( b_lock ) then
		if ( (player_count() >= 1) and ( (s_player < 0) or (s_player == 0) ) ) then
			L_gazelock_p0_thread = f_thread_cleanup( L_gazelock_p0_thread, 0 );
			player_control_lock_gaze( player_get(player_00), pr_point, r_velocity );
			//pr_player0 = pr_point;
			b_return = TRUE;
		end
		if ( (player_count() >= 2) and ( (s_player < 0) or (s_player == 1) ) ) then
			L_gazelock_p1_thread = f_thread_cleanup( L_gazelock_p1_thread, 0 );
			player_control_lock_gaze( player_get(player_01), pr_point, r_velocity );
			//pr_player1 = pr_point;
			b_return = TRUE;
		end
		if ( (player_count() >= 3) and ( (s_player < 0) or (s_player == 2) ) ) then
			L_gazelock_p2_thread = f_thread_cleanup( L_gazelock_p2_thread, 0 );
			player_control_lock_gaze( player_get(player_02), pr_point, r_velocity );
			//pr_player2 = pr_point;
			b_return = TRUE;
		end
		if ( (player_count() >= 4) and ( (s_player < 0) or (s_player == 3) ) ) then
			L_gazelock_p3_thread = f_thread_cleanup( L_gazelock_p3_thread, 0 );
			player_control_lock_gaze( player_get(player_03), pr_point, r_velocity );
			//pr_player3 = pr_point;
			b_return = TRUE;
		end
	end
...
local boolean b_return = FALSE;
	...
	if ( b_lock ) then
		if ( (player_count() >= 1) and ( (s_player < 0) or (s_player == 0) ) ) then
			L_gazelock_p0_thread = f_thread_cleanup( L_gazelock_p0_thread, 0 );
			player_control_clamp_gaze( player_get(player_00), pr_point, r_angle );
			//pr_player0 = pr_point;
			b_return = TRUE;
		end
		if ( (player_count() >= 2) and ( (s_player < 0) or (s_player == 1) ) ) then
			L_gazelock_p1_thread = f_thread_cleanup( L_gazelock_p1_thread, 0 );
			player_control_clamp_gaze( player_get(player_01), pr_point, r_angle );
			//pr_player1 = pr_point;
			b_return = TRUE;
		end
		if ( (player_count() >= 3) and ( (s_player < 0) or (s_player == 2) ) ) then
			L_gazelock_p2_thread = f_thread_cleanup( L_gazelock_p2_thread, 0 );
			player_control_clamp_gaze( player_get(player_02), pr_point, r_angle );
			//pr_player2 = pr_point;
			b_return = TRUE;
		end
		if ( (player_count() >= 4) and ( (s_player < 0) or (s_player == 3) ) ) then
			L_gazelock_p3_thread = f_thread_cleanup( L_gazelock_p3_thread, 0 );
			player_control_clamp_gaze( player_get(player_03), pr_point, r_angle );
			//pr_player3 = pr_point;
			b_return = TRUE;
		end
...
local boolean b_return = FALSE;
	...
	if ( b_lock ) then
		if ( (player_count() >= 1) and ( (s_player < 0) or (s_player == 0) ) ) then
			L_gazelock_p0_thread = f_thread_cleanup( L_gazelock_p0_thread, 0 );
			player_control_clamp_gaze_marker( player_get(player_00), obj_target, str_marker, r_angle );
			//pr_player0 = pr_point;
			b_return = TRUE;
		end
		if ( (player_count() >= 2) and ( (s_player < 0) or (s_player == 1) ) ) then
			L_gazelock_p1_thread = f_thread_cleanup( L_gazelock_p1_thread, 0 );
			player_control_clamp_gaze_marker( player_get(player_01), obj_target, str_marker, r_angle );
			//pr_player1 = pr_point;
			b_return = TRUE;
		end
		if ( (player_count() >= 3) and ( (s_player < 0) or (s_player == 2) ) ) then
			L_gazelock_p2_thread = f_thread_cleanup( L_gazelock_p2_thread, 0 );
			player_control_clamp_gaze_marker( player_get(player_02), obj_target, str_marker, r_angle );
			//pr_player2 = pr_point;
			b_return = TRUE;
		end
		if ( (player_count() >= 4) and ( (s_player < 0) or (s_player == 3) ) ) then
			L_gazelock_p3_thread = f_thread_cleanup( L_gazelock_p3_thread, 0 );
			player_control_clamp_gaze_marker( player_get(player_03), obj_target, str_marker, r_angle );
			//pr_player3 = pr_point;
			b_return = TRUE;
		end
...
local boolean b_return = FALSE;
	...
	if ( (player_count() >= 1) and ( (s_player < 0) or (s_player == 0) ) ) then
		L_gazelock_p0_thread = f_thread_cleanup( L_gazelock_p0_thread, 0 );
		player_control_unlock_gaze( player_get(player_00) );
		b_return = TRUE;
	end
	if ( (player_count() >= 2) and ( (s_player < 0) or (s_player == 1) ) ) then
		L_gazelock_p1_thread = f_thread_cleanup( L_gazelock_p1_thread, 0 );
		player_control_unlock_gaze( player_get(player_01) );
		b_return = TRUE;
	end
	if ( (player_count() >= 3) and ( (s_player < 0) or (s_player == 2) ) ) then
		L_gazelock_p2_thread = f_thread_cleanup( L_gazelock_p2_thread, 0 );
		player_control_unlock_gaze( player_get(player_02) );
		b_return = TRUE;
	end
	if ( (player_count() >= 4) and ( (s_player < 0) or (s_player == 3) ) ) then
		L_gazelock_p3_thread = f_thread_cleanup( L_gazelock_p3_thread, 0 );
		player_control_unlock_gaze( player_get(player_03) );
		b_return = TRUE;
	end
...
script static void start_camera_shake_loop (string shake_type, string shake_frequency)
	...
	repeat
		if (shake_type == "weak") then
			thread (camera_shake_all_coop_players ( real_random_range(0.25, 1.5), real_random_range(0.1, 0.23), real_random_range(0.25, 1.5), real_random_range(0.1, 1.0)));
			print("low shake!"); 
		elseif (shake_type == "medium") then
			thread (camera_shake_all_coop_players ( real_random_range(0.75, 2.25), real_random_range(0.23, 0.36), real_random_range(0.75, 2.25), real_random_range(0.5, 1.5)));
			print("medium shake!"); 
		elseif (shake_type == "heavy") then
			thread (camera_shake_all_coop_players ( real_random_range(1.25, 3), real_random_range(0.35, 0.7), real_random_range(1.5, 3), real_random_range(1, 2.5)));
			print("heavy shake!"); 
		else
			thread (camera_shake_all_coop_players ( real_random_range(0.75, 2.25), real_random_range(0.23, 0.36), real_random_range(0.75, 2.25), real_random_range(0.5, 1.5)));
			print("default shake!"); 
