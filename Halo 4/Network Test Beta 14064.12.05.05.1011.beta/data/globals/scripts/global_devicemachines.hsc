...
//			r_pos_start [real] = Starting position of the animation sequence
//				[r_pos_start < 0] = current position
//			r_pos_end [real] = Ending position of the animation sequence
//				[r_pos_end < 0] = current position
//			[parameters] see: device_animate_position
script static void device_animate_position_relative( device dm_device, real r_pos_start, real r_pos_end, real r_time, real r_blend_in, real r_blend_out, boolean b_blend_apply )
local real r_pos_scale = 1.0;

	// defaults
	if ( r_pos_start < 0.0 ) then
		r_pos_start = device_get_position( dm_device );
	end
	if ( r_pos_end < 0.0 ) then
		r_pos_end = device_get_position( dm_device );
	end

	// calculate the relative scale
	r_pos_scale = abs_r( 1.0 - ((device_get_position(dm_device) - r_pos_start) / (r_pos_end - r_pos_start)) );

	device_animate_position( dm_device, r_pos_end, r_time * r_pos_scale, r_blend_in * r_pos_scale, r_blend_out * r_pos_scale, b_blend_apply );
