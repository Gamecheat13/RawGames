...
script dormant f_jump_start()
	sleep_until(volume_test_players(tv_jump_lower_weapon),1);
		...
		// [mboulton 9/6/2012] Suppress command buffer panic-flush during jump to avoid nasty frame rate hitch
		dprint("suppress command buffer panic-flush");
		command_buffers_suppress_panic_flush = true;
