...
//set crate folder names to create
	...
	f_add_crate_folder(eq_e1_m5_unsc_ammo); //cov crates all around the main area				
...
script static void f_start_events_e1_m5_1
	//start a bunch of threads and end when the player has hit the close trigger volume
	...
	//create some objects, turn off distortion and place the turret
	...
	//turning off distortion for perf, turning back on after big battle
...
script static void f_e1_m5_shield_effects
	print ("shield effects on");
		//turn on the shield effects
	sleep_until (object_valid (cr_tunnel_shield1), 1);
	object_set_function_variable (cr_tunnel_shield1, "shield_on", 1, 0.5);
...
script static void f_attach_panels_e1_m5
	//attaching all the device machines and controls
...
script static void f_ranger_drop
	//spawn the ranger when the player hits the trigger volume
	//call turret VO
...
script static void f_phantom_flyover
	//spawn 3 phantoms when the player has hit the trigger volume and there aren't too many AI
...
script static void f_start_events_e1_m5_switch
	//tell the player about the switch, wait until it's flipped then derez the switch and animate the pillars
	sleep_until (LevelEventStatus("start_e1_m5_switch_1"), 1);
	print ("starting start_e1_m5_switch_1");
	
	b_wait_for_narrative_hud = true;
	
	//turning distortion back on
	...
	//commenting this out and controlling the device_machine through the puppeteer
	...
	print ("power switch at position 1, de-rezzing");
	dm_power_switch->SetDerezWhenActivated();
...
script static void switch_move_1
	//raise the towers, spawn the phantom and rangers, power up the switches, call the derez the switches script
	print ("switch move 1");
...
script static void f_start_events_e1_m5_1_5
	...
	//create the rest of props, call VO when the switches are flipped
	thread (f_music_e1_m5_1_5_start());
	
	f_e1_m5_create_folders();
	//device_set_power (door_switch_1, 1);
	//device_set_power (door_switch_2, 1);
	//b_wait_for_narrative_hud = true;
	//sleep (30 * 7);
...
//these derez scripts were hacked in for E3:(
script static void f_derez_door_switch_1
	...
	// have to play the sound manually because this animation hack thing completely messes with the sounds hooked up to the machine tag
	sound_impulse_start('sound\environments\multiplayer\valhalla\machines\new_machines\machine_50_small_control_panel_activate', door_switch_1, 1);
	...
	dm_door_switch_1->SetDerezWhenActivated();
...
script static void f_derez_door_switch_2
	...
	// have to play the sound manually because this animation hack thing completely messes with the sounds hooked up to the machine tag
	sound_impulse_start('sound\environments\multiplayer\valhalla\machines\new_machines\machine_50_small_control_panel_activate', door_switch_2, 1);
	...
	dm_door_switch_2->SetDerezWhenActivated();
	print ("derez-ing door_switch_2 ");

end

//create prop folders now to try to gain some performance
script static void f_e1_m5_create_folders
	print ("creating folders");
	object_create_folder (cr_e1_m5_cov_misc);
	object_create_folder (cr_e1_m5_cov_cover_back);

end

//hacky way to call a script
...
//call door moves and control the shake
...
//call VO and music (also spawn a turret)
...
//=== SWITCHING ZONE SETS OCCURS HERE

script static void f_start_events_e1_m5_3

	//all the enemies have been killed -- tell the player to take down the shields, then blip and turn on the controls
	//animate the controls when done
	//sorry about the ugly script -- this was the E3 mission 
...
//pausing for drama	
	...
	//commenting out and performing this action in the puppetteer
...
script static void f_end_events_e1_m5_3
	...
	object_create_folder (eq_e1_m5_fore_ammo);
...
script static void f_event_blow_up_shields_e1_m5
	//turn off the shields and shake the camera
	print ("_blow shields_");
	
	b_wait_for_narrative_hud = true;
	
	//wait until the animation on the button is complete
	sleep_until (device_get_position (dm_e1_m5_cov_switch) == 1, 1);
...
//	object_destroy_folder (cr_tunnel_shield);
	...
	object_set_function_variable (cr_tunnel_shield1, "shield_on", 0, 0.5);
	object_destroy (cr_tunnel_shield1);
...
// this is used to call in tunnel pawns through Bonobo or another level event
...
//effects for the pawn spawns -- should have put those in their command or placement script
...
script static void f_start_events_e1_m5_4
	//spawn some knights and control the VO - flip a boolean that says the players are through the tunnel
...
script static void f_knight_phase_e1_m5
	//wait until a player is looking at the right place and spawn in some knights
...
//	ai_place (sq_sniper_left);
	...
	sleep_s (1);
	ai_place_with_birth (guards_5_bishop);
	sleep_s (1);
...
//when all the enemies are dead, call some VO, then power up the controls, then animate and derez when done
//sorry for the hacky way of doing it
...
//	f_new_objective (title_e1_m5_8);
	...
	//commenting this out and controlling starting the device machine through the puppeteer
...
script static void f_end_events_e1_m5_5
	//controls the VO for the attack after players get the artifact
...
//put this vo someplace else	
	...
	dm_art_switch->SetDerezWhenActivated();
	print ("derezzing dm_art_switch");
	
	device_set_power (dm_art_switch, 0);
		
	//drop the pillar holding the switch
...
//spawn in the pelican, call some music and VO
...
//	sound_looping_stop (music_start);
	...
	//place the pelican
	ai_place (squads_1);
	object_cannot_take_damage (ai_vehicle_get (squads_1.pelican_0));
	object_immune_to_friendly_damage (ai_vehicle_get(squads_1.pelican_0), true);
...
//pausing for drama
	...
	//start the outro
	sound_impulse_start('sound\storm\multiplayer\pve\events\mp_pve_e1m5_vin_sfx_outro', NONE, 1);
	cinematic_start();
	b_wait_for_narrative = true;
	scene_narrative_out();
...
//		intro_vignette_e1_m5();
	...
	//	sleep_s (8);
		...
		sound_impulse_start('sound\storm\multiplayer\pve\events\mp_pve_e1m5_vin_sfx_intro', NONE, 1);
		intro_vignette_e1_m5();
	end
	//intro();
	//firefight_mode_set_player_spawn_suppressed(false);
...
script static void f_rvb_interact
	print ("start RVB");
	object_create (e1_m5_rvb);
	//object_set_health (e1_m5_rvb, 0.01);
	sleep_until (object_get_health (e1_m5_rvb) < 1, 1);
	print ("rvb interacted");
	object_cannot_take_damage (e1_m5_rvb);
...
script static void f_e1_m5_cov_switch (object dev, unit player)
//script static void f_push_fore_switch (unit player)
	print ("pushing the cov switch");
	g_ics_player = player;
//	g_ics_player = player0;
	pup_play_show (e1_m5_push_cov_button);
