...
script startup instanced f_init()
 	print ("set EC5 base overlay animation to 'any:idle'");
...
script static instanced void f_animate()
	...
	print ("escape_collapse_4_start");
//	device_set_position_track(	this, 'any:idle', 1 );
	device_animate_position (this, 0.30, 7, 0.1, 0.1, TRUE);
	
	sleep_until (device_get_position (this) >= 0.30);
			
			device_animate_position (this, 0.80, 9, 0.1, 0.1, TRUE);
	
	sleep_until (device_get_position (this) >= 0.80);
	
			device_animate_position (this, 1, 0.5, 0.1, 0.1, TRUE);
