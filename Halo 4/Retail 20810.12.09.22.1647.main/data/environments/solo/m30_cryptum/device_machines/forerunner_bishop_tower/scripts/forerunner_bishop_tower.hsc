...
script startup instanced f_init()
  ...
  device_animate_overlay (this, 1, 0.1, 0.1, 0.1);
  print ("device machine set to the end of it's animation");
  
end

script static instanced void start_animating()

	print ("bishop tower is moving");
//	device_set_position_track(	this, 'any:idle', 1 );
	device_animate_overlay (this, 0, 9.0, 0.1, 0.1);

end

script static instanced void f_animate_loop()
	
	repeat
		
		//sleep_until (device_get_position (this) >= 0.95, 1);
		
		print ("bishop tower is moving up");
		...
		print ("bishop tower is moving down");
		
		device_animate_overlay (this, 1, 9.0, 0.1, 0.1);
