...
script startup instanced f_init()
 	print ("set EC7 base overlay animation to 'any:idle'");
  device_set_position_track(this, 'any:idle', 0);
  
end

script static instanced void f_animate(trigger_volume animate_trigger)

	print ("escape_collapse_7_start");
//	device_set_position_track(	this, 'any:idle', 1 );
	device_animate_position (this, 0.30, 2.0, 0.1, 0.1, TRUE);
	
	sleep_until ((device_get_position (this) >= 0.30) and (volume_test_players (animate_trigger)), 1);

		device_animate_position (this, 1, 5.0, 0.1, 0.1, TRUE);
