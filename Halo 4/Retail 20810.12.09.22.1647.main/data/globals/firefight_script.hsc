...
script dormant firefight_won_game
	...
	//adding the real 'r_game_end_timer' to adjust the end time of spartan ops matches
	//setting default to 9 seconds
	//for big fix 45546 - gmurphy
	//code reviewed by Robert Kingsley
...
//			v_print ("spawn in knight effects");
//		end
...
script static void f_spawn_squads()
	...
	//		v_print ("wave_squad_location doesn't = 0");
	//		ai_place_wave (firefight_mode_get_wave_squad(), firefight_mode_get_squad_at(wave_squad_location), 1);
			...
			//inspect (firefight_mode_get_wave_squad());
			//if there is a valid squad written in the "squad type" in the tag then spawn that wave template at the squad location
			if firefight_mode_get_wave_squad() >= 0 then
				ai_place_wave_in_limbo (firefight_mode_get_wave_squad(), firefight_mode_get_squad_at(wave_squad_location), 1);
				v_print ("spawning with WAVE spawn");
			else
				...
				ai_place_in_limbo (firefight_mode_get_squad_at(wave_squad_location));
				v_print ("spawning with spawn points");
			end
	//		v_print ("placed the waves");
		end
//		v_print ("wave_squad_location = 0");
...
//		object_create_anew (obj_drop_pod_4);
	...
	elseif
		...
		v_print ("drop pod falling randomly");
...
//loads the drop pod -- set waves to false when calling this script in a mission
...
//		ai_set_objective (squad, obj_drop_pod);
		...
		if vehicle_test_seat (vehicle(pod), "") == false then
			v_print ("no seats taken, the squad didn't get put into the drop pod :(");
		else
			v_print ("squad teleported into drop pod");
		end
	end
	...
	if waves == true and firefight_mode_get_current_squad_to_place(1) > 0 then
		...
		v_print ("squad 1 teleported into drop pod");
	end
	
//	ai_vehicle_enter_immediate (squad, vehicle(pod));
//	v_print ("squad 0 teleported into drop pod");
	sleep (1);
...
//	sleep (30 * 5);
	...
	//device_set_position_track(device_drop, 'pod_large_idle', 1 );
	//device_animate_position (device_drop, 1, 3.3, 1, 0, 0);
...
script static void objective_destroy_watch (object_name objective_obj)
	...
	if object_get_health(objective_obj) == 0 then
		if device_get_position(device(objective_obj)) == 0 and device_get_power(device(objective_obj)) == 0 then
			v_print ("---destroy goal is currently off and needs to get turned on and switched on");
			//f_blip_object (objective_obj, "activate");
			...
			v_print ("an object was turned on");
		elseif device_get_position(device(objective_obj)) == 0 and device_get_power(device(objective_obj)) == 1 then
			v_print ("---destroy goal is a device machine that is currently on or a device control that is currently off");
			...
			sleep_until (device_get_power(device(objective_obj)) < 1 or device_get_position(device(objective_obj)) > 0, 1);
			v_print ("an object was turned on or off");		
		else
			//f_blip_object (objective_obj, "activate");
			...
			sleep_until (device_get_position(device(objective_obj)) < 1, 1);
			v_print ("an object was turned off");
		end
		//f_unblip_object (objective_obj);
		f_unblip_object_cui (objective_obj);
	else
...
script static void pause_test (string_id pause_text_id)
	print ("pause test");
	//cui_hud_set_new_objective (pause_text_id);
