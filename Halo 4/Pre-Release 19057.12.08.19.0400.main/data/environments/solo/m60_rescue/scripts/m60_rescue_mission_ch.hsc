...
script dormant f_caves_main()
	...
	sleep_until (volume_test_players (tv_cave_lb), 1);
		...
		then
			...
			print (":  : Trails Marines Survived :  :");
		else
			print (":  : Trails Marines Died! You monster! :  :");
		end
...
script static void rally_intro_ghost()
	...
	sleep_until (ai_living_count (sg_rally_intro) < 6
	
		OR
							volume_test_players (tv_0)
							
		OR		
							player_in_vehicle (ve_rally_scorpion) == 1);
...
script static void rally_pelican_hog()
	...
	sleep_until 
		...
		(ai_living_count (sg_intro_bowl_drop) > 1
...
script dormant f_initial_spawn()
	...
	sleep_until
		(ai_living_count (sg_rally_intro) < 12
			OR
		player_in_vehicle (ve_rally_scorpion) == 1
			OR
		volume_test_players (tv_0));
...
script dormant f_door_open()
	...
	sleep_until (ai_living_count (sg_bowl_vehicle) < 1);
...
//	switch_zone_set (infinity_berth);
	...
	volume_teleport_players_not_inside (tv_berth_enter, tank_lift_teleport);
	...
	wake (inf_berth);
		...
		AND
			(ai_living_count (sq_bowl_hum_mid) == 3)
		then
