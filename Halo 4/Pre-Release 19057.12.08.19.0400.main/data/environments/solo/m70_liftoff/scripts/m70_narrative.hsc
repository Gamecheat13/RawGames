...
script dormant f_nar_flight_03()
	sleep_until( F_Flight_Zone_Active() and s_flight_state >= S_DEF_FLIGHT_STATE_THIRD_COMPLETE(), 1);
...
script dormant f_nar_spire_gondola_ride_02()
	...
	if f_check_first_spire(DEF_SPIRE_01) then
		thread(f_fx_interior_scan( fx_didact_scan_spire1  ));
		sleep_s(0.75);
...
script dormant f_nar_spire_gondola_carrier_wave_generator()
	...
	if f_check_first_spire(DEF_SPIRE_01) == TRUE then
		...
		sleep_until(device_get_position(dc_spire01_gondola_switch) != 0 or sp01_gondola_moving == TRUE, 1);
		sleep_s(5);
		...
		sleep_s(0.5);
		b_sp01_bishop_attack = TRUE;
		sleep_until(ai_is_shooting(sq_end_bishop) or ai_combat_status(sq_end_bishop) >= 9 or device_get_position(dc_spire01_gondola_switch) != 0 or sp01_gondola_moving == TRUE, 1);
		
		if device_get_position(dc_spire01_gondola_switch) == 0 then
...
script dormant f_nar_spire_3_intro()
  ...
  thread(f_m70_chapter_title(ch_spire_03));
	...
	//thread(f_fx_interior_scan( fx_didact_scan_spire3 ));		
	sleep_s(1);
...
script static void f_activator_get( object obj_control, unit activator )
	...
	if ( obj_control == dc_terminal_domain ) then
		f_narrative_domain_terminal_interact( 5, cr_70_domain, dc_terminal_domain, activator, 'pup_domain_terminal' );
	end
...
script dormant f_nar_m70_waypoint_terminal()
    f_narrative_domain_terminal_setup( 5, cr_70_domain, dc_terminal_domain );
end
