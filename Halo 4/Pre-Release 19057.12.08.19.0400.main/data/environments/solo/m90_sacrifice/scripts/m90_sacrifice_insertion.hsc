...
// Insertion
global short s_start_idx								= 0;	//     ( start )
global short s_trans_01_idx							= 1;	// trench ab split
global short s_trench_b_idx							= 2;	// trench b

global short s_trans_02_idx							= 3;  // 
global short s_trench_c_idx							= 4;  // 
global short s_trans_03_idx							= 5;  //
global short s_trench_d_idx							= 6;  // 
global short s_trans_04_e_idx						= 7;  //
global short s_tren_e_eye_idx						= 8;  //
global short s_eye_trans_idx						= 9;  //
global short s_eye_idx									= 10; //eye
global short s_crash_idx 								= 11;	//
global short s_final_idx 								= 12;	//
global short s_arcade_idx 							= 13;	//
global short s_arcade_drop_idx 					= 14;	// 
global short s_drop_teleports_idx 			= 15;	//
global short s_walls_teleport_room_idx 	= 16;	// 
global short s_walls_coldant_idx 				= 18;	// 

global short s_teleport_rooms_idx 			= 20;	// 
global short s_jump_idx 								= 21;	//
global short s_ending_game_idx 					= 25;	// 
global short s_cin_m90_cavalry					= 22;
global short s_cin_m91_sacrifice				= 23;
global short s_cin_m91b_sacrifice				= 26;
global short s_cin_m092_didact					= 17;
global short s_cin_m93_kiss							= 24;
...
script static void ins_trench_b()
	if b_debug then
		...
		f_insertion_zoneload( s_trans_01_idx, INSERTION_INDEX_TRENCH_B, FALSE );
		f_insertion_playerwait();
		f_space_particles_on( TRUE );
...
script static void ins_trench_c()
	...
	s_zoneset_index = s_trans_02_idx;
	
	thread(f_master_cleanup_m90());	
	switch_zone_set ("trans_02");
...
script static void ins_trench_d()
	...
	f_m90_reset_completetion_flags();
	s_zoneset_index = s_trans_03_idx	;



	
	thread(f_master_cleanup_m90());	
 switch_zone_set ("trans_03");
...
script static void ins_trench_e()
	...
	f_m90_reset_completetion_flags();
	s_zoneset_index = s_trans_04_e_idx	;



	
	thread(f_master_cleanup_m90());	
	switch_zone_set ("trans_04_e");
...
local zone_set zs_return = "start";
	...
	if ( s_index == s_trans_01_idx ) then
	 zs_return = "trans_01";
	end
	if ( s_index == s_trench_b_idx ) then
	 zs_return = "trench_b";
	end
	if ( s_index == s_trans_02_idx ) then
	 zs_return = "trans_02";
	end
	if ( s_index == s_trench_c_idx ) then
	 zs_return = "trench_c";
	end
	if ( s_index == s_trans_03_idx ) then
	 zs_return = "trans_03";
	end
	if ( s_index == s_trench_d_idx ) then
	 zs_return = "trench_d";
	end
	if ( s_index == s_trans_04_e_idx ) then
	 zs_return = "trans_04_e";
	end
	if ( s_index == s_tren_e_eye_idx ) then
	 zs_return = "tren_e_eye";
	end
