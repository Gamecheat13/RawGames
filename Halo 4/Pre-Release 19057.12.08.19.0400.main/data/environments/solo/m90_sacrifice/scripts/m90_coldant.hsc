...
global object g_ics_plinth2 = NONE;
global object g_ics_plinth3 = NONE;
...
//			dprint("a");
	...
	if b_reverse then
		...
		sound_impulse_start ( 'sound\environments\solo\m090\amb_m90_device_machines\amb_m90_dm_plinth_close.sound', plinth, 1 ); //AUDIO!
		object_move_by_offset(plinth,1.0,0,0,-0.37 );


	else
		//object_move_by_offset(plinth,0.5,0,0,0.405 );
		sound_impulse_start ( 'sound\environments\solo\m090\amb_m90_device_machines\amb_m90_dm_plinth_open.sound', plinth, 1 ); //AUDIO!
...
script dormant f_left_complete()
	...
	//sleep_until ( volume_test_players( tv_cold_plinth_left ) and b_cold_left_top_all_spawned, 1 );
	sleep_until ( b_cold_left_top_all_spawned, 3 );
		sleep_s(2);
		dprint("waiting for top to be clear...");
		local long l_safety_timer = timer_stamp( 300 ); 
	//sleep_until ( not (volume_test_objects( tv_cold_left_top_clear, ai_actors( sg_left_all ) )) and not (objects_can_see_object( Players(), sg_left_all, 22.5 )) , 1 );
	...
	sleep_until ( volume_test_players( tv_cold_left_top_clear) , 1 );
		...
		f_cold_activate_plinth( cr_plinth_left, dc_comp_switch_left, dc_comp_switch_left, false, false );
		b_cold_left_switch_ready = TRUE;
		sleep(3);
		if game_is_cooperative() and device_get_position( dc_comp_switch_right ) > 0.0 then
		 	sleep(15);
		 	sleep_until( not pup_is_playing( pup_show_right ), 1 );
		 		dprint("waiting for right plinth to finish ics");
		 		sleep(15);
		end
		device_set_power ( dc_comp_switch_left, 1 );
 		f_blip_object ( dc_comp_switch_left,"activate" );		
	sleep_until( device_get_position( dc_comp_switch_left ) > 0.0, 1 );
		//kill the right side if its on at the same time
		if b_cold_right_switch_ready then
			device_set_power ( dc_comp_switch_right, 0 );
		...
		if b_pup_first_shield_done then
			...
			pup_show_left = pup_play_show( cortana_splinter3 );
		else
			//g_composer_state = 3;
			
			g_ics_plinth2 = dc_comp_switch_left;
			pup_show_left = pup_play_show( cortana_splinter2 );
			sleep(1);

		end
		
		sleep( 5);
	sleep_until( not pup_is_playing( pup_show_left ), 1);
		sleep(1);
		b_pup_first_shield_done = true;
		if not b_cold_right_core_active then
		...
		composer_core_count = (composer_core_count - 1);
		...
		if b_save_game then
			...
			wake( f_comp_save_pre_pier_left );
		end
		
		if b_cold_right_core_active and b_cold_right_switch_ready then
			//reactivate right side
			device_set_power ( dc_comp_switch_right, 1 );
 			f_blip_object ( dc_comp_switch_right , "activate");
		end
...
script dormant f_right_complete()
	...
	//sleep_until ( volume_test_players( tv_cold_plinth_right ) and b_cold_right_top_all_spawned, 1 );
	...
	sleep_until ( volume_test_players( tv_cold_right_top_clear) , 1 );
		...
		f_cold_activate_plinth( cr_plinth_right , dc_comp_switch_right, dc_comp_switch_right, false, false );
		...
		if game_is_cooperative() and device_get_position( dc_comp_switch_right ) > 0.0 then
		 	...
		 	sleep_until( not pup_is_playing( pup_show_left ), 1 );
		 		sleep(15);
		 		dprint("waiting for left plinth to finish ics");
		end
		device_set_power ( dc_comp_switch_right, 1 );
 		f_blip_object ( dc_comp_switch_right,"activate" );		

	sleep_until( device_get_position( dc_comp_switch_right ) > 0.0, 1 );
		//kill the left side if its on at the same time
		if b_cold_left_switch_ready then
			device_set_power ( dc_comp_switch_left, 0 );
 			f_unblip_object ( dc_comp_switch_left);
 		end
	...
	sleep_until( not pup_is_playing( pup_show_left ), 1);
		if b_pup_first_shield_done then			
				...
				music_set_state('Play_mus_m90_v12_cortana_splinter_3');
				pup_show_right = pup_play_show( cortana_splinter3 );
		else
			//g_composer_state = 3;

			g_ics_plinth2 = dc_comp_switch_right;
			pup_show_right = pup_play_show( cortana_splinter2 );
			sleep(1);
	...
	sleep_until( not pup_is_playing( pup_show_right ), 1);	
		...
		ai_kill(sg_right_all); 
		sleep( 1 );
		if not b_cold_left_core_active then
			thread( f_maelstrom_shield_strip() );
			ai_kill(sg_pier);
		else 
			thread( f_cold_set_composer_state_3() );
...
//		thread(f_core_entry_hit());
		...
		if b_save_game then
			...
			wake( f_comp_save_pre_pier_right );		
		end

		if b_cold_left_core_active and b_cold_left_switch_ready then
			//reactivate right side
			device_set_power ( dc_comp_switch_left, 1 );
 			f_blip_object ( dc_comp_switch_left , "activate");
		end
...
script static void f_composer_start_firing()
		...
		thread( start_camera_shake_loop( "medium", "medium"));
