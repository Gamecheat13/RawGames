...
script dormant f_crater_main()
	...
	//thread (f_ambient_cruiser_01());
	//thread (f_ambient_cruiser_02());
	//thread (f_ambient_cruiser_03());
...
// =================================================================================================
// AFTER PLAYER ENTERS 3 OF THE 4 AREAS OF INTEREST, SPAWN PHANTOM TO DROP OFF TROOPS
// =================================================================================================

script dormant spawn_crater_phantom()
	sleep_until ( 
	s_crater_phantom_volumes == 3 or 
	volume_test_players (tv_sniper_spawn_phantom)
	,1);
	
	game_save_no_timeout();
	
	ai_place (crater_phantom_01);
	dprint( "SPAWNING DROPSHIP" );
	
	sleep (30*3);
	
	// Play Cortana line and blip ship
	wake (f_dialog_scouting_party_arrive);
	
	sleep (30*2);
	f_blip_ai (crater_phantom_01.driver, "default");
		
end

script command_script cs_crater_phantom_center()
	cs_vehicle_speed (0.5);
			
	f_load_phantom (crater_phantom_01, right, crater_phantom_sq_01, none, none, none);
	f_load_phantom (crater_phantom_01, left, crater_phantom_sq_02, none, none, none);
		
	cs_ignore_obstacles (TRUE);
			
	wake (f_crater_phantom_blind_01);
	wake (f_crater_phantom_blind_02);
	
	cs_fly_to (ps_phantom_crater_01.p5);
	cs_fly_by (ps_phantom_crater_01.p0);
	cs_fly_by (ps_phantom_crater_01.p1);
	cs_fly_by (ps_phantom_crater_01.p2);
	
	cs_vehicle_speed (0.3);
	
	cs_fly_to_and_face (ps_phantom_crater_01.p2, ps_phantom_crater_01.p3);
	cs_fly_to_and_dock (ps_phantom_crater_01.p2, ps_phantom_crater_01.p3, 3);
	
	//ai_braindead (crater_phantom_01, TRUE);
	
	sleep (30*2);
	
	f_unload_phantom (crater_phantom_01, right);
	
	f_unblip_ai (crater_phantom_01.driver);
	
	f_unload_phantom (crater_phantom_01, left);
		
	wake (f_crater_enemy_blip);
	
	sleep (30*2);
	
	cs_vehicle_speed (0.1);
	
	//ai_braindead (crater_phantom_01, FALSE);
	
	cs_fly_to_and_face (ps_phantom_crater_01.p7, ps_phantom_crater_01.p6);
	
	cs_vehicle_speed (0.4);
	
	sleep (30*1);
	
	cs_fly_by (ps_phantom_crater_01.p6);
	cs_fly_by (ps_phantom_crater_01.p4);
	cs_fly_by (ps_phantom_crater_01.p9);
	cs_fly_by (ps_phantom_crater_01.p10);
	cs_fly_by (ps_phantom_crater_01.p11);
...
//PHANTOM SEES YOU
script dormant f_crater_phantom_blind_01()
	sleep_until (
	object_get_health (ai_vehicle_get_from_spawn_point (crater_phantom_01.driver) ) < 1 or
	volume_test_players (tv_cr_phantom_sees_you)
	, 1);
	
	dprint ("I SEE YOU!");
	
	ai_set_deaf (crater_phantom_01, 0);
	ai_set_blind (crater_phantom_01, 0);
	
end

script dormant f_crater_phantom_blind_02()
	sleep_until (ai_strength (sg_crater_phantom) < 1);
...
//BLIP EXIT WHEN ENEMIES ARE DEAD
script dormant f_crater_enemy_blip()
	dprint ("waiting for crater enemies to be killed before blipping exit...");
	sleep_until (ai_living_count (sg_crater_phantom) <= 0, 1);
	
	if not volume_test_players (tv_cliff_blip_off) then
		dprint ("enemies are dead, playing scouting party dialog now...");
		wake (f_dialog_scouting_party_defeated);
		game_save_no_timeout();
		sleep (30*8);
		dprint ("preparing to blip exit...");
...
script dormant f_crater_unblip_01()
	...
	sleep_forever (f_crater_enemy_blip);
...
script dormant f_drive_main()
	...
	//wake (f_drive_ember_fx_01);
	//wake (f_drive_ember_fx_02);
	thread (f_drive_ember_fx_on (player0));
	thread (f_drive_ember_fx_on (player1));
	thread (f_drive_ember_fx_on (player2));
	thread (f_drive_ember_fx_on (player3));
...
//EMBER EFFECTS
script static void f_drive_ember_fx_on(player p_player)
	repeat
		if
...
/*
script dormant f_drive_ember_fx_01()
	repeat
	sleep_until (volume_test_players (tv_drive_ember_fx_on_01), 1);
	effect_attached_to_camera_new (environments\solo\m020\fx\embers\embers_ambient_floating.effect);
	dprint ("turning on ember fx");
	
	sleep_until (volume_test_players (tv_drive_ember_fx_off_01), 1);
	effect_attached_to_camera_stop (environments\solo\m020\fx\embers\embers_ambient_floating.effect);
	dprint ("turning off ember fx");
	until (1 == 0);

end

script dormant f_drive_ember_fx_02()
	repeat
	sleep_until (volume_test_players (tv_drive_ember_fx_off_02), 1);
	effect_attached_to_camera_stop (environments\solo\m020\fx\embers\embers_ambient_floating.effect);
	dprint ("turning off ember fx");
	
	sleep_until (volume_test_players (tv_drive_ember_fx_on_02), 1);
	effect_attached_to_camera_new (environments\solo\m020\fx\embers\embers_ambient_floating.effect);
	dprint ("turning on ember fx");
	until (1 == 0);
...
script dormant f_kill_all_field_encounters()
	ai_erase (sq_field_front_grunts);
	ai_erase (sq_field_01);
	ai_erase (sq_field_04);
	ai_erase (sq_field_04_sniper);
	ai_erase (sq_field_end_turret);
	ai_erase (sq_field_06);
	ai_erase (field_phantom_01);
	ai_erase (field_phantom_02);
	ai_erase (field_phantom_03);
	ai_erase (sq_phantom_elite_left);
	ai_erase (sq_phantom_jackals_left);
	ai_erase (sq_field_08);
...
script static void bridge_enemy_cleanup()
	...
	f_ai_garbage_erase (sq_bridge_start_jackals, 8, -1, -1, -1, FALSE);
	f_ai_garbage_erase (sq_bridge_start_grunts, 8, -1, -1, -1, FALSE);
	f_ai_garbage_erase (sq_bridge_start_grunt_patrol, 8, -1, -1, -1, FALSE);
	
	sleep_until (volume_test_players (tv_enemy_cleanup_02), 1);
	dprint ("attempting to clean up bridge first middle");
	f_ai_garbage_erase (sq_bridge_firsthalf_1a, 8, -1, -1, -1, FALSE);
	f_ai_garbage_erase (sq_bridge_firsthalf_1e, 8, -1, -1, -1, FALSE);
	f_ai_garbage_erase (sq_bridge_firsthalf_2, 8, -1, -1, -1, FALSE);
	f_ai_garbage_erase (sq_bridge_firsthalf_under, 8, -1, -1, -1, FALSE);
	f_ai_garbage_erase (sq_bridge_middle_elite_02, 8, -1, -1, -1, FALSE);
	
	sleep_until (volume_test_players (tv_enemy_cleanup_03), 1);
	dprint ("attempting to clean up bridge second middle");
	f_ai_garbage_erase (sq_bridge_middle_main_1, 8, -1, -1, -1, FALSE);
	f_ai_garbage_erase (sq_bridge_middle_main_2, 8, -1, -1, -1, FALSE);
	f_ai_garbage_erase (sq_bridge_middle_main_3, 8, -1, -1, -1, FALSE);
	f_ai_garbage_erase (sq_bridge_middle_top_1, 8, -1, -1, -1, FALSE);
	f_ai_garbage_erase (sq_bridge_middle_top_2, 8, -1, -1, -1, FALSE);
	
	sleep_until (volume_test_players (tv_enemy_cleanup_04), 1);
	dprint ("attempting to clean up bridge end front");
	f_ai_garbage_erase (sq_bridge_end_front, 8, -1, -1, -1, FALSE);
	f_ai_garbage_erase (sq_bridge_end_front_st, 8, -1, -1, -1, FALSE);
	f_ai_garbage_erase (sq_bridge_end_middle_2, 8, -1, -1, -1, FALSE);
...
//GARBAGE CLEANUP
script static void f_bridge_halfway_cleanup()
	dprint ("attempting to clean up the first half of the bridge");
	f_ai_garbage_erase (sq_bridge_start_jackals, 8, -1, -1, -1, FALSE);
	f_ai_garbage_erase (sq_bridge_start_grunts, 8, -1, -1, -1, FALSE);
	f_ai_garbage_erase (sq_bridge_start_grunt_patrol, 8, -1, -1, -1, FALSE);
	f_ai_garbage_erase (sq_bridge_firsthalf_1a, 8, -1, -1, -1, FALSE);
...
script dormant bridge_doors_open()
	...
	dm_door_to_courtyard_01->auto_trigger_close( tv_to_courtyard_doors, FALSE, TRUE, FALSE );
...
script static void destroy_all_banshees()
	...
	object_damage_damage_section (list_get(volume_return_objects_by_campaign_type (tv_to_courtyard_doors, 30), 0), "hull", 500);
	object_damage_damage_section (list_get(volume_return_objects_by_campaign_type (tv_to_courtyard_doors, 30), 0), "canopy", 500);
	object_damage_damage_section (list_get(volume_return_objects_by_campaign_type (tv_to_courtyard_doors, 30), 0), "wing_right", 500);
	object_damage_damage_section (list_get(volume_return_objects_by_campaign_type (tv_to_courtyard_doors, 30), 0), "wing_left", 500);

	object_damage_damage_section (list_get(volume_return_objects_by_campaign_type (tv_to_courtyard_doors, 30), 1), "hull", 500);
	object_damage_damage_section (list_get(volume_return_objects_by_campaign_type (tv_to_courtyard_doors, 30), 1), "canopy", 500);
	object_damage_damage_section (list_get(volume_return_objects_by_campaign_type (tv_to_courtyard_doors, 30), 1), "wing_right", 500);
	object_damage_damage_section (list_get(volume_return_objects_by_campaign_type (tv_to_courtyard_doors, 30), 1), "wing_left", 500);

	object_damage_damage_section (list_get(volume_return_objects_by_campaign_type (tv_to_courtyard_doors, 30), 2), "hull", 500);
	object_damage_damage_section (list_get(volume_return_objects_by_campaign_type (tv_to_courtyard_doors, 30), 2), "canopy", 500);
	object_damage_damage_section (list_get(volume_return_objects_by_campaign_type (tv_to_courtyard_doors, 30), 2), "wing_right", 500);
	object_damage_damage_section (list_get(volume_return_objects_by_campaign_type (tv_to_courtyard_doors, 30), 2), "wing_left", 500);
	
	object_damage_damage_section (list_get(volume_return_objects_by_campaign_type (tv_to_courtyard_doors, 30), 3), "hull", 500);
	object_damage_damage_section (list_get(volume_return_objects_by_campaign_type (tv_to_courtyard_doors, 30), 3), "canopy", 500);
	object_damage_damage_section (list_get(volume_return_objects_by_campaign_type (tv_to_courtyard_doors, 30), 3), "wing_right", 500);
	object_damage_damage_section (list_get(volume_return_objects_by_campaign_type (tv_to_courtyard_doors, 30), 3), "wing_left", 500);
	
	object_damage_damage_section (list_get(volume_return_objects_by_campaign_type (tv_to_courtyard_doors, 30), 4), "hull", 500);
	object_damage_damage_section (list_get(volume_return_objects_by_campaign_type (tv_to_courtyard_doors, 30), 4), "canopy", 500);
	object_damage_damage_section (list_get(volume_return_objects_by_campaign_type (tv_to_courtyard_doors, 30), 4), "wing_right", 500);
	object_damage_damage_section (list_get(volume_return_objects_by_campaign_type (tv_to_courtyard_doors, 30), 4), "wing_left", 500);
	
	object_damage_damage_section (list_get(volume_return_objects_by_campaign_type (tv_to_courtyard_doors, 30), 5), "hull", 500);
	object_damage_damage_section (list_get(volume_return_objects_by_campaign_type (tv_to_courtyard_doors, 30), 5), "canopy", 500);
	object_damage_damage_section (list_get(volume_return_objects_by_campaign_type (tv_to_courtyard_doors, 30), 5), "wing_right", 500);
	object_damage_damage_section (list_get(volume_return_objects_by_campaign_type (tv_to_courtyard_doors, 30), 5), "wing_left", 500);
	
	print ("Destroying Banshees");

	until (list_count(volume_return_objects_by_campaign_type (tv_to_courtyard_doors, 30)) == 0, 1);
...
script static void courtyard_enemy_cleanup()
	sleep_until (
	volume_test_players (tv_gs_courtyard) or
	volume_test_players (tv_court_right_safe_spawn)
	, 1);
	dprint ("attempting to clean up courtyard front");

	f_ai_garbage_erase (sq_courtyard_front_01, 10, -1, -1, -1, FALSE);
	f_ai_garbage_erase (sq_courtyard_front_02, 10, -1, -1, -1, FALSE);
	f_ai_garbage_erase (sq_courtyard_center, 10, -1, -1, -1, FALSE);
	f_ai_garbage_erase (sq_courtyard_right, 10, -1, -1, -1, FALSE);
	f_ai_garbage_erase (sq_courtyard_left, 10, -1, -1, -1, FALSE);
	f_ai_garbage_erase (sq_courtyard_left_sniper, 10, -1, -1, -1, FALSE);
	f_ai_garbage_erase (sq_courtyard_ghost_1, 10, -1, -1, -1, FALSE);
	f_ai_garbage_erase (sq_courtyard_ghost_2, 10, -1, -1, -1, FALSE);
	f_ai_garbage_erase (sq_courtyard_end_g, 10, -1, -1, -1, FALSE);
	f_ai_garbage_erase (sq_courtyard_end_e, 10, -1, -1, -1, FALSE);
	f_ai_garbage_erase (sq_cy_phantom_start_01, 10, -1, -1, -1, FALSE);
	f_ai_garbage_erase (sq_cy_phantom_start_02, 10, -1, -1, -1, FALSE);
	...
	volume_test_players (tv_court_right_safe_spawn)
	, 1);
	dprint ("attempting to clean up courtyard hall");

	f_ai_garbage_erase (sq_court_hall_grunts, 10, -1, -1, -1, FALSE);
	f_ai_garbage_erase (sq_court_hall_elite, 10, -1, -1, -1, FALSE);
	...
	, 1);
	dprint ("attempting to clean up courtyard middle");

	f_ai_garbage_erase (sq_hall_left_1, 10, -1, -1, -1, FALSE);
	f_ai_garbage_erase (sq_hall_right_1, 10, -1, -1, -1, FALSE);
	f_ai_garbage_erase (sq_hall_left_ramp, 10, -1, -1, -1, FALSE);
	f_ai_garbage_erase (sq_hall_right_ramp, 10, -1, -1, -1, FALSE);
	f_ai_garbage_erase (sq_hall_left_main, 10, -1, -1, -1, FALSE);
	f_ai_garbage_erase (sq_hall_right_main, 10, -1, -1, -1, FALSE);
	f_ai_garbage_erase (sq_hall_middle_1, 10, -1, -1, -1, FALSE);
	f_ai_garbage_erase (sq_hall_middle_2, 10, -1, -1, -1, FALSE);
	f_ai_garbage_erase (sq_hall_jackal_left_1, 10, -1, -1, -1, FALSE);
	f_ai_garbage_erase (sq_hall_jackal_left_2, 10, -1, -1, -1, FALSE);
	f_ai_garbage_erase (sq_hall_jackal_right_1, 10, -1, -1, -1, FALSE);
	f_ai_garbage_erase (sq_hall_jackal_right_2, 10, -1, -1, -1, FALSE);
...
script static void term_curtain_01a()
	...
	term_curtain_01a->auto_trigger_close( tv_tower_elevator, FALSE, TRUE, TRUE );

end

script static void term_curtain_01b()
	term_curtain_01b->open();
	term_curtain_01b->auto_trigger_close( tv_tower_elevator, FALSE, TRUE, TRUE );

end

script static void term_curtain_02a()
	term_curtain_02a->open();
	term_curtain_02a->auto_trigger_close( tv_tower_elevator, FALSE, TRUE, TRUE );

end

script static void term_curtain_02b()
	term_curtain_02b->open();
	term_curtain_02b->auto_trigger_close( tv_tower_elevator, FALSE, TRUE, TRUE );

end

script static void term_curtain_03a()
	term_curtain_03a->open();
	term_curtain_03a->auto_trigger_close( tv_tower_elevator, FALSE, TRUE, TRUE );

end

script static void term_curtain_03b()
	term_curtain_03b->open();
	term_curtain_03b->auto_trigger_close( tv_tower_elevator, FALSE, TRUE, TRUE );

end

script static void term_curtain_04a()
	term_curtain_04a->open();
	term_curtain_04a->auto_trigger_close( tv_tower_elevator, FALSE, TRUE, TRUE );

end

script static void term_curtain_04b()
	term_curtain_04b->open();
	term_curtain_04b->auto_trigger_close( tv_tower_elevator, FALSE, TRUE, TRUE );

end

script static void term_curtain_05a()
	term_curtain_05a->open();
	term_curtain_05a->auto_trigger_close( tv_tower_elevator, FALSE, TRUE, TRUE );

end

script static void term_curtain_05b()
	term_curtain_05b->open();
	term_curtain_05b->auto_trigger_close( tv_tower_elevator, FALSE, TRUE, TRUE );
