...
//DELETE ANY VEHICLES PLAYERS BROUGHT IN
	sleep (30*2);
...
script dormant f_post_one_core_activated()
	...
	//player activated didact core (right), disable librarian (left) core temporarily
	if 
		b_player_activated_core_right == TRUE 
	then
		device_set_power (m20_librarian_terminal, 0);
		f_unblip_flag (flag_left_core);
		dprint ("waiting to finish VO");
				
		//wait until VO lines completed
		sleep_until (b_first_core_complete == TRUE, 1);
		sleep (30*2);
		f_blip_flag (flag_left_core, "activate");
		device_set_power (m20_librarian_terminal, 1);
	end
	
	//player activated librarian core (left), disable didact (right) core temporarily
	if 
		b_player_activated_core_left == TRUE
	then
		device_set_power (m20_didact_terminal, 0);
...
//SPAWN ENEMIES	
	...
	if (ai_living_count (sg_gpi_left) <= 0) then
		ai_place (sq_gpi_middle_left_grunts_01);
		dprint ("SPAWNING EXTRA SQUAD");
	end
	
	if (ai_living_count (sg_gpi_right) <= 0) then
