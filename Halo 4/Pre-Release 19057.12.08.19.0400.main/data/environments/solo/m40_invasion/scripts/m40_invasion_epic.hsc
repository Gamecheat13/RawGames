...
script dormant epic_main_script()
	...
	sleep (1);
//	cs_run_command_script (epic_marines_tank_01, epic_tank_shoot_ghost);
	object_create_folder (epic_crates);
...
//	);
	...
	f_chapter_title (leadin_title_ord);
...
script static void epic_wraiths_rear_spawn()
	sleep_until (ai_living_count (epic_small_vehicles) < 4
	and
	epic_obj_state > 14
	and
...
//	cs_shoot_point (true, epic_vehicle_pt.p7);
	...
	sleep_s(3);
	if player_in_vehicle(epic_scorpion_01) == false then
...
script dormant missile_prototype()
  ...
  thread(target_designator_check_trigger_down_time_end());
	...
	fade_out (255,255,255, 5);
	sleep (30 * .5);
	object_create_folder (asteroids_00);
	object_create_folder (dm_asteroids);
	
	object_create (missile_01);
...
if
		m_ics_player0 == true
	then
		unit_enter_vehicle_immediate (player0, missile_01, "banshee_d");
		
		unit_enter_vehicle_immediate (player1, missile_01, "warthog_d");
		unit_enter_vehicle_immediate (player2, missile_01, "ghost_d");
		unit_enter_vehicle_immediate (player3, missile_01, "mongoose_d");
	elseif
		m_ics_player1 == true
	then
		unit_enter_vehicle_immediate (player1, missile_01, "banshee_d");
		
		unit_enter_vehicle_immediate (player0, missile_01, "warthog_d");
		unit_enter_vehicle_immediate (player2, missile_01, "ghost_d");
		unit_enter_vehicle_immediate (player3, missile_01, "mongoose_d");
	elseif
		m_ics_player2 == true
	then
		unit_enter_vehicle_immediate (player2, missile_01, "banshee_d");
		
		unit_enter_vehicle_immediate (player3, missile_01, "warthog_d");
		unit_enter_vehicle_immediate (player0, missile_01, "ghost_d");
		unit_enter_vehicle_immediate (player1, missile_01, "mongoose_d");
	elseif
		m_ics_player3 == true
	then
		unit_enter_vehicle_immediate (player3, missile_01, "banshee_d");
		
		unit_enter_vehicle_immediate (player0, missile_01, "warthog_d");
		unit_enter_vehicle_immediate (player1, missile_01, "ghost_d");
		unit_enter_vehicle_immediate (player2, missile_01, "mongoose_d");
	end 
	effect_kill_from_flag (environments\solo\m40_invasion\fx\energy\tractor_beam.effect, fx_tractor_base ); 		
	sleep (15);
		wake (f_asteroid_control);
	fade_in (0,0,0,20);		
			
	player_disable_movement (true);
	thread (camera_shake_long());
	screen_effect_new (environments\solo\m40_invasion\fx\missile_strike\missile_strike_m40_warp_static.area_screen_effect, tractor_failure_flag);
	wake (ending_1);
...
script dormant ending_1()
	sleep_until (volume_test_players (tv_tractor_035), 1);
	f_unblip_flag (tractor_target_flag);
	sleep_until (volume_test_players (tv_tractor_03), 1);
	...
	if (s_asteroid_hit == 0) then
		td_tried_to_fire_input = true;
		achievement_grant_to_all_players ("m40_special2");
		print ("give Pop Lock and Rocket");
	end
	sleep (30 * .9);
	object_teleport (player0, tractor_failure_flag);
	object_teleport (player1, tractor_failure_flag);
	object_teleport (player2, tractor_failure_flag);
	object_teleport (player3, tractor_failure_flag);
	fade_in (0,0,0,20);
	player_disable_movement (false);
	object_destroy_folder (asteroids_00);
	object_destroy (missile_capsule);
	object_destroy (rock_swirl_01);
	object_destroy (rock_swirl_02);	
	
	screen_effect_delete (environments\solo\m40_invasion\fx\missile_strike\missile_strike_m40_warp_static.area_screen_effect, tractor_failure_flag);
...
script dormant f_asteroid_control ()

	thread(get_pos(dm_asteroid_0));
	thread(get_pos(dm_asteroid_1));
	thread(get_pos(dm_asteroid_2));
	thread(get_pos(dm_asteroid_3));
	
	
	
	thread(get_pos(dm_asteroid_a_1));
	thread(get_pos(dm_asteroid_a_2));
	thread(get_pos(dm_asteroid_a_3));
	thread(get_pos(dm_asteroid_a_4));
	thread(get_pos(dm_asteroid_a_5));
	thread(get_pos(dm_asteroid_a_6));
	thread(get_pos(dm_asteroid_a_7));
	
	thread(get_pos(dm_asteroid_b_1));
	thread(get_pos(dm_asteroid_b_2));
	thread(get_pos(dm_asteroid_b_3));
	thread(get_pos(dm_asteroid_b_4));
	thread(get_pos(dm_asteroid_b_5));
	thread(get_pos(dm_asteroid_b_6));
	thread(get_pos(dm_asteroid_b_7));
	
	thread(get_pos(dm_asteroid_c_1));
	thread(get_pos(dm_asteroid_c_2));
	thread(get_pos(dm_asteroid_c_3));
	thread(get_pos(dm_asteroid_c_4));
	thread(get_pos(dm_asteroid_c_5));
	thread(get_pos(dm_asteroid_c_6));
	thread(get_pos(dm_asteroid_c_7));

	thread(get_pos(dm_asteroid_d_1));
	thread(get_pos(dm_asteroid_d_2));
	thread(get_pos(dm_asteroid_d_3));
	thread(get_pos(dm_asteroid_d_4));
	thread(get_pos(dm_asteroid_d_5));
	thread(get_pos(dm_asteroid_d_6));
	thread(get_pos(dm_asteroid_d_7));
	
	thread(get_pos(dm_asteroid_e_1));
	thread(get_pos(dm_asteroid_e_2));
	thread(get_pos(dm_asteroid_e_3));
	thread(get_pos(dm_asteroid_e_4));
	thread(get_pos(dm_asteroid_e_5));
	thread(get_pos(dm_asteroid_e_6));
	thread(get_pos(dm_asteroid_e_7));
	
	
	thread(get_pos(dm_asteroid_f_1));
	thread(get_pos(dm_asteroid_f_2));
	thread(get_pos(dm_asteroid_f_3));
	thread(get_pos(dm_asteroid_f_4));
	thread(get_pos(dm_asteroid_f_5));
	thread(get_pos(dm_asteroid_f_6));
	thread(get_pos(dm_asteroid_f_7));
