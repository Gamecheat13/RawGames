...
script dormant f_bubble_control
	print ("Bubble Control");
		...
		soft_ceiling_enable ("prechopper_chopper_divider", false);
...
script static void cliffside_ai_kills()
	...
	then
		...
		then
			object_teleport (unit_get_vehicle (player0), prechop_teleport_02);
		else
			object_teleport (player0, prechop_teleport_02);
		end
	end
	...
	kill_volume_enable (kill_soft_prechop_backtrack);	
