...
script static void set_cryptum_shield_stage(short s_shield, short s_stage, boolean b_wait)
	...
	if (s_shield == 2) then
		...
		if (s_stage == 2) then
			...
			// Apply the effects for stage 2
			effect_new(environments\solo\m30_cryptum\fx\core\core_stage2.effect, fx_cryptum_shield2);
		end
	...
	if (s_shield == 3) then
		if (s_stage == 2) then
			...
			// Apply the effects for stage 2
			effect_new(environments\solo\m30_cryptum\fx\core\core_stage2.effect, fx_cryptum_shield3);
		end
...
script dormant cryptshield_1_canyon()
	sleep_until (volume_test_players ("begin_zone_set:1_canyon"), 1);
...
script dormant cryptshield_1_forts()
	sleep_until (volume_test_players ("begin_zone_set:1_forts"), 1);
