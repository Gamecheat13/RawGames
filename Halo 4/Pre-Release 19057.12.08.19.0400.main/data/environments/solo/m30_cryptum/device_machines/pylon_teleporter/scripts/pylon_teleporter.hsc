...
script startup instanced f_init()
 	print ("pylon teleporter set up");
  device_set_position_track(this, 'any:idle', 0);
  
end

script static instanced void f_animate(device shaft, object_name portal)

	sleep_until (device_get_position (shaft) >= 0.85);

	dprint ("pylon teleporter raising");
