...
script startup instanced f_init()
 	print ("pylon shaft has been set up");
...
script static instanced void f_animate_loop(device switch, device core)
  local boolean core_position = FALSE;
  local boolean switch_off = FALSE;
//	device_set_position_track(	this, 'any:idle', 1 );
	repeat
		
		device_set_position_track(this,'any:idle', 0);
		dprint ("pylon shaft looping");
		device_animate_position (this, 0.5, 3.0, 0.1, 0.1, TRUE);
		sleep_until (device_get_position (this) >= 0.5);
		core_position = device_get_position (core) == 1;
		inspect(core);
		inspect (core_position);
		switch_off = device_get_position (switch) != 0;
		inspect(switch);
		inspect (switch_off);
	until (core_position == TRUE and switch_off == TRUE, 1);
	
	dprint ("pylon shaft shutting down");
