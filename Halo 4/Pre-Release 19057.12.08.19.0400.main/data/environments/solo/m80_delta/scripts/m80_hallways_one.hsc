...
script dormant f_hallways_one_door_last_init()
	...
	// setup auto disable	
	door_crash_exit_maya->auto_enabled_zoneset( FALSE, S_ZONESET_TO_AIRLOCK_ONE_B, -1 );
...
script dormant f_hallways_one_door_last_trigger()
	...
	until ( (f_hallways_one_narrative_powerloss_active() == FALSE) and (door_to_airlock_one_exit->position_open_check() or (object_valid(door_to_airlock_one_exit) == FALSE) or (zoneset_current_active() >= S_ZONESET_TO_AIRLOCK_ONE_B)), 1 );

	// close
	sleep_until( f_hallways_one_narrative_powerloss_complete(), 1 );
