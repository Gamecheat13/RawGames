...
script dormant f_crash_trigger()
	...
	// start objective
	sleep_until( dialog_id_played_check(L_dlg_crash_landing) or dialog_foreground_id_line_index_check_greater_equel(L_dlg_crash_landing, S_dlg_crash_landing_objective_line_index), 1 );
...
script dormant f_crash_puppeteer_sticky_test()
	door_crash_exit_maya->speed_open( 3.0 );
	wake( f_crash_puppeteer_sticky_trigger );
	door_crash_exit_maya->open();
end
...
script dormant f_crash_puppeteer_sticky_trigger()
	...
	// aciton
	//sleep_until( (zoneset_current_active() >= S_ZONESET_CRASH_TRANSITION) and object_valid(door_crash_exit_maya) and (device_get_position(door_crash_exit_maya) > 0.0), 1 );
	sleep_until( object_valid(door_crash_exit_maya) and (device_get_position(door_crash_exit_maya) > 0.0), 1 );
...
script static boolean f_crash_puppeteer_sticky_go()
	( zoneset_current_active() >= S_ZONESET_CRASH_TRANSITION ) and ( device_get_position(door_crash_exit_maya) >= (R_crash_puppeteer_door_pos * 0.25) );
end
...
//local long l_thread = 0;
	...
	// set speed
	sleep_until( object_valid(door_crash_exit_maya) and object_active_for_script(door_crash_exit_maya), 1 );
	door_crash_exit_maya->speed_open( 3.0 );

	// setup auto disable	
	thread( door_crash_exit_maya->auto_enabled_zoneset(FALSE, S_ZONESET_TO_HORSESHOE, -1) );

	// open
	door_crash_exit_maya->zoneset_auto_open_setup( S_ZONESET_CRASH_TRANSITION, TRUE, TRUE, S_ZONESET_CRASH, S_ZONESET_CRASH_TRANSITION, TRUE );
	door_crash_exit_maya->auto_distance_open( -2.75, FALSE );
	
	// setup auto open/close
	//l_thread = thread( door_crash_exit_maya->auto_distance_open_close(-3.0, TRUE) );
	
	// close
	door_crash_exit_maya->zoneset_auto_close_setup( S_ZONESET_TO_HORSESHOE, TRUE, TRUE, S_ZONESET_CRASH, S_ZONESET_CRASH_TRANSITION, TRUE );
	door_crash_exit_maya->auto_trigger_close_all_out( tv_crash_door_close_out, TRUE );

	//sleep_until( zoneset_current() >= S_ZONESET_TO_HORSESHOE, 1 );
	
	// force closed
	door_crash_exit_maya->close_immediate();
