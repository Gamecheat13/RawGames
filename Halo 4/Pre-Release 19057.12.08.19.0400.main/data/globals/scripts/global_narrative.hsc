...
local boolean b_active = FALSE;
	...
	repeat
		...
		sleep_until( (not object_valid(obj_terminal)) or (not object_valid(obj_button)) or (f_narrative_domain_terminal_sphere(obj_terminal) == NONE) or (b_active != f_narrative_domain_terminal_active(s_terminal_id)), 1 );
		
	until( f_narrative_domain_terminal_sphere(obj_terminal) == NONE, 1 );
...
script static boolean f_narrative_domain_terminal_active( short s_terminal_id )
	IsNarrativeFlagSetOnAllPlayer( s_terminal_id, FALSE );
end
