...
local boolean b_conditional_ai_see_check = ( r_ai_see_degrees > 0.0 );
	...
	repeat
		...
		if (
			( obj_object != NONE )
			and 
			( cs_number_queued(object_get_ai(obj_object)) < 1)
			and
			(
				( 
...
script static real spops_ai_morale_weight( ai ai_ally, ai ai_enemy, real r_distance_ally_player, real r_distance_enemy_player, real r_weight_ally_player, real r_weight_enemy_player, real r_weight_ally, real r_weight_enemy, real r_weight_ally_leader, real r_weight_enemy_leader )
	local real aw = 0;
	local real ew = 0;
	aw = spops_ai_morale_weight_ally( ai_ally, r_weight_ally, r_weight_ally_leader, r_distance_ally_player, r_weight_ally_player ) ;
	sleep(2);
	ew = spops_ai_morale_weight_enemy( ai_enemy, r_weight_enemy, r_weight_enemy_leader, r_distance_enemy_player, r_weight_enemy_player );
	sleep(2);
	aw - ew;
end
...
local real basiccount  = ai_task_count(ai_morale) + ai_living_count(ai_morale) ;
local real r_weight = ( basiccount ) * r_weight_base;
	...
	if ( (basiccount > 0) and (r_distance_player > 0.0) and (r_weight_player != 0.0)) then
		if (spops_player_living_cnt() > 0)  then
...
// ----------------------------------------------------------------------------------------------------------------------------------------------------------
// ----------------------------------------------------------------------------------------------------------------------------------------------------------
// *** SPOPS: AI: SECONDARY WEAPON SWITCH ***
// ----------------------------------------------------------------------------------------------------------------------------------------------------------
// ----------------------------------------------------------------------------------------------------------------------------------------------------------
// ----------------------------------------------------------------------------------------------------------------------------------------------------------

script static long spops_ai_secondary_sword_manage( ai ai_enemy, short s_sword_combat_status_min, real r_sword_distance_min, real r_sword_shield_min, real r_sword_health_min )
	if ( unit_has_weapon(ai_enemy, 'objects\weapons\melee\storm_energy_sword\storm_energy_sword.weapon') ) then
		//dprint( "spops_ai_secondary_sword_manage: SWORD" );
		thread( sys_spops_ai_secondary_sword_manage(ai_enemy, s_sword_combat_status_min, r_sword_distance_min, r_sword_shield_min, r_sword_health_min) );
	else
		//dprint( "spops_ai_secondary_sword_manage: NO SWORD" );
		0;
	end

end

script static void sys_spops_ai_secondary_sword_manage( ai ai_enemy, short s_sword_combat_status_min, real r_sword_distance_min, real r_sword_shield_min, real r_sword_health_min )
local boolean b_combat_status_check = ( s_sword_combat_status_min >= 0 );
local boolean b_distance_check = ( r_sword_distance_min >= 0.0 );
local boolean b_shield_check = ( r_sword_shield_min >= 0.0 );
local boolean b_health_check = ( r_sword_health_min >= 0.0 );
local object obj_enemy = ai_get_object( ai_enemy );

	// force abs values
	s_sword_combat_status_min = abs_s( s_sword_combat_status_min );
	r_sword_distance_min = abs_r( r_sword_distance_min );
	r_sword_shield_min = abs_r( r_sword_shield_min );
	r_sword_health_min = abs_r( r_sword_health_min );

	//dprint( "sys_spops_ai_secondary_sword_manage: START" );
	repeat

		sleep_until( (unit_get_health(ai_enemy) <= 0.0) or (not unit_has_weapon_readied(ai_enemy, 'objects\weapons\melee\storm_energy_sword\storm_energy_sword.weapon')) );
		if ( unit_get_health(ai_enemy) > 0.0 ) then
			sleep_until(
				( unit_get_health(ai_enemy) <= 0.0 )
				or
				(
					( (ai_combat_status(ai_enemy) >= s_sword_combat_status_min) == b_combat_status_check )
					and
					( (unit_get_shield(ai_enemy) >= r_sword_shield_min) == b_shield_check )
					and
					( (unit_get_health(ai_enemy) >= r_sword_health_min) == b_health_check )
					and
					(
						( spops_player_living_cnt() > 0 )
						and
						( (objects_distance_to_object(Players(),obj_enemy) >= r_sword_distance_min) == b_distance_check )
					)
				)
			, 1 );

			// go berzerk
			ai_berserk( ai_enemy, TRUE );
			sleep_s( 3.0 );
			sleep_until( (unit_get_health(ai_enemy) <= 0.0) or (not unit_has_weapon_readied(ai_enemy, 'objects\weapons\melee\storm_energy_sword\storm_energy_sword.weapon')) );
		end
	
	until( unit_get_health(ai_enemy) <= 0.0 );
	//dprint( "sys_spops_ai_secondary_sword_manage: END" );
