...
script static void hud_rampancy_players_set( real r_rampancy )
	if ( isthreadvalid(L_cortana_hud_rampancy_thread) ) then
		kill_thread( L_cortana_hud_rampancy_thread );
	end
	L_cortana_hud_rampancy_thread = thread( sys_hud_rampancy_players_set(r_rampancy) );
end
script static void hud_rampancy_players_scale( real r_rampancy, real r_time )
	if ( isthreadvalid(L_cortana_hud_rampancy_thread) ) then
		kill_thread( L_cortana_hud_rampancy_thread );
	end
	L_cortana_hud_rampancy_thread = thread( sys_hud_rampancy_players_scale(r_rampancy, r_time) );
end
script static void sys_hud_rampancy_players_set( real r_rampancy )
	R_cortana_hud_rampancy = r_rampancy;
	hud_set_rampancy_intensity( player0, R_cortana_hud_rampancy );
	hud_set_rampancy_intensity( player1, R_cortana_hud_rampancy );
	hud_set_rampancy_intensity( player2, R_cortana_hud_rampancy );
	hud_set_rampancy_intensity( player3, R_cortana_hud_rampancy );
end
script static void sys_hud_rampancy_players_scale( real r_rampancy, real r_time )
local long l_time_start = 0;
local long l_time_end = 0;
local real r_time_range = 0.0;
local real r_rampancy_range = 0.0;
local real r_rampancy_start = 0.0;
local real r_rampancy_delta = 0.0;

	// starting RAMPANCY
	r_rampancy_start = R_cortana_hud_rampancy;

	// defaults
	if ( r_time < 0.0 ) then
		r_time = abs_r( r_rampancy - r_rampancy_start );
	end
	...
	// setup variables
	r_rampancy_range = r_rampancy - r_rampancy_start;
	r_time_range = l_time_end - l_time_start;
	
	if ( (r_rampancy_range > 0.001) and (r_time_range > 0.001) ) then

		repeat
			r_rampancy_delta = real( game_tick_get()-l_time_start ) / r_time_range;
			// set gravity to the current % of time progress
			sys_hud_rampancy_players_set( r_rampancy_start + (r_rampancy_range * r_rampancy_delta) ); 
		until ( game_tick_get() >= l_time_end, 1 );
	
	end
	sys_hud_rampancy_players_set( r_rampancy );
...
// scale up and fade in cortana over 6 frames. needs a scale value for the final size
script static void f_global_cortana_show( object cor, real cor_scale )
	...
	object_set_scale(cor,cor_scale,6);
	...
	sleep(1);
	SetObjectRealVariable(cor,VAR_OBJ_LOCAL_A,0);
	render_hologram_screen_bound_scale=scale;
end

// scale down and fade out cortana over 6 frames
script static void f_global_cortana_hide( object cor )
	local real scale=render_hologram_screen_bound_scale;
	render_hologram_screen_bound_scale=scale*1.5; // scale up the bounding box by 50%. otherwise the mini-cortana sticks out of the box a bit
