...
script static void OnShardSpawnEvent_Callback(object shardObject, long eventType, long placeTarget, long spawnDelay)
	print("Shard spawn event callback.");
...
script static void OnKnightTaintEvent_Callback(object taintObject, long eventType, long resTaskIndex, long unused1)
	print("Shard spawn event callback.");
...
script static void InitializeCapturableUnit(object captiveObject)
	print("Initialize capturable unit.");
...
script static void OnCapturableObjectEvent_Callback(object captiveObject, long eventType, long arg1, long arg2)
	print("Capturable Object event callback.");
	if eventType == ON_INTERACT then
...
script static void InitializeAutomatedTurret(vehicle turret, boolean startsActive)
	print("Initialize automated turret.");
	ai_braindead(ai_vehicle_get_driver(turret), true);
    object_hide(turret, true);
	
	SetObjectLongVariable(turret, VAR_TURRET_ACTIVE, 0);
	...
	if startsActive then
		AutomatedTurretActivate(turret);
	end
end


script static void AutomatedTurretActivate(vehicle turret)
	if GetObjectLongVariable(turret, VAR_TURRET_ACTIVE) == 0 then
		print("Activating automated turret");
		
		//	Currently no rez in effect.  Just unhide.
		object_hide(turret, false);		
		ai_braindead(ai_vehicle_get_driver(turret), false);
		SetObjectLongVariable(turret, VAR_TURRET_ACTIVE, 1);
	end
end


script static void AutomatedTurretSwitchTeams(object turret, object newTeamObj)
	print("Switch turret team.");	
...
script static void OnAutomatedTurretEvent_Callback(vehicle turret, long eventType, long arg1)
	print("Automated turret event callback.");
	if eventType == ON_INTERACT then
		//	Interact logic varies depending on if turret is active or not.
		if GetObjectLongVariable(turret, VAR_TURRET_ACTIVE) == 0 then
			//	Activate the inactive turret.
			AutomatedTurretActivate(turret);
		else
