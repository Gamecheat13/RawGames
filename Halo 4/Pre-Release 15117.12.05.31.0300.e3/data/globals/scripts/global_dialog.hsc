...
sys_dialog_debug( "sys_dialog_line_thread", "" );
	...
	if ( r_type != DEF_DIALOG_TYPE_NONE() ) then
		...
		sleep_until( (game_tick_get() > l_timer) or ((str_speaker_id == DEF_DIALOG_SPEAKER_ID_CHIEF()) and (player_living_count() == 0)) or ((not IsThreadValid(l_dialog_id)) and b_line_interruptable), 1 );
