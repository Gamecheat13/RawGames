...
script static long checkpoint_timeout( boolean b_auto_cancel, real r_time, string str_debug, real r_checkpoint_time_min )
	if ( timer_expired(L_checkpoint_timer, r_checkpoint_time_min) ) then
		checkpoint_timeout( b_auto_cancel, r_time, str_debug );
	else
		0;
	end
end
...
script static long checkpoint_no_timeout( boolean b_auto_cancel, string str_debug, real r_checkpoint_time_min )
	if ( timer_expired(L_checkpoint_timer, r_checkpoint_time_min) ) then
		checkpoint_no_timeout( b_auto_cancel, str_debug );
	else
		0;
	end
end
...
local long l_timer = 0;
	...
	// dprint if saving during streaming
	if ( current_zone_set_fully_active() == -1 ) then
		dprint( "CHECKPOINT ATTEMPTED WHILE STREAMING !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!" );
		dprint( str_debug );
		dprint( "!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!" );
	end	
