...
local real 		r_type 					= DEF_DIALOG_TYPE_BACKGROUND();
	...
	if ( b_condition and dialog_id_active_check(l_dialog_id) and (r_type != DEF_DIALOG_TYPE_NONE()) and ((o_object == NONE) or (ai_get_object(object_get_ai(o_object)) == NONE) or (object_get_health(o_object) > 0.0)) ) then
		...
		sys_dialog_line_thread( l_dialog_id, s_line_index, str_speaker_id, snd_sound, b_line_interruptable, o_object, r_pad, str_debug );
...
sys_dialog_debug( "dialog_line_npc_ai", "" );
	...
	if ( ai_living_count(ai_speaker) > 0 ) then
		b_line_spoken = dialog_line_npc( l_dialog_id, s_line_index, b_condition, snd_sound, b_line_interruptable, ai_get_object(ai_speaker), r_pad, str_notify, str_debug, b_exclusive );
	end
...
sys_dialog_debug( "sys_dialog_line_thread", "" );
	...
	if ( r_type != DEF_DIALOG_TYPE_NONE() ) then
		...
		sleep_until( 
			( game_tick_get() >= l_timer )
			or
			(
				( str_speaker_id == DEF_DIALOG_SPEAKER_ID_CHIEF() )
				and
...
local long l_dialog_id = DEF_DIALOG_ID_NONE();
					l_dialog_id = dialog_start_foreground( "MY_FIRST_DOMAIN", l_dialog_id, TRUE, DEF_DIALOG_STYLE_QUEUE(), FALSE, "", 0.25 );    
						dialog_line_cortana( l_dialog_id, 0, TRUE, 'sound\dialog\mission\m20\m20_domain_00100', FALSE, NONE, 0.0, "", "Cortana : This node is caught in a loop trying to access something it's calling the Domain." );
						dialog_line_cortana( l_dialog_id, 1, TRUE, 'sound\dialog\mission\m20\m20_domain_00101', FALSE, NONE, 0.0, "", "Cortana : An offworld data repository of some kind, though I'm only able to extract bits and pieces of the complete exchange. " );
						dialog_line_cortana( l_dialog_id, 2, TRUE, 'sound\dialog\mission\m20\m20_domain_00102', FALSE, NONE, 0.0, "", "Cortana : I'll log it for investigation later." );						
